...
from main.feature_pipeline.data_preprocessor import (
    DataPreprocessor, 
    PreprocessorConfig
)
...
def sample_news_data():
    ...
    dates = pd.date_range('2024-01-01', periods=20, freq='H', tz=timezone.utc)
...
class TestMarketDataPreprocessing:
    ...
    def test_ohlc_consistency_validation(self, preprocessor):
        ...
        data = pd.DataFrame({
            'timestamp': pd.date_range('2024-01-01', periods=3, freq='D', tz=timezone.utc),
            'open': [100, 101, 102],
    ...
    def test_duplicate_timestamp_handling(self, preprocessor):
        ...
        timestamps = pd.date_range('2024-01-01', periods=2, freq='D', tz=timezone.utc)
        data = pd.DataFrame({
...
class TestOutlierHandling:
    ...
    def test_no_outlier_detection(self, default_config):
        ...
        data = pd.DataFrame({
            'value': [1, 2, 3, 4, 5, 100]  # 100 would be an outlier
        })
...
class TestAlternativeDataPreprocessing:
    ...
    def test_preprocess_news_data(self, preprocessor, sample_news_data):
        ...
        assert not any(headline.startswith(' ') or headline.endswith(' ') 
    ...
    def test_preprocess_social_data(self, preprocessor):
        ...
        data = pd.DataFrame({
            'text': ['Good post about stocks', 'Duplicate', 'Duplicate', 'Short', 'Another good post'],
...
class TestDataQualityValidation:
    ...
    def test_large_time_gaps_warning(self, preprocessor, caplog):
        ...
        timestamps = [
            ...
            datetime(2024, 1, 15, tzinfo=timezone.utc)  # 13-day gap
        ]
