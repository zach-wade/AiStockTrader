...
class TestFeatureComputationWorker:
    ...
    def mock_feature_group_mapper(self):
        ...
        mapper = Mock()
        mapper.group_features_by_type = Mock(return_value={
    ...
    @pytest.fixture
    async def worker(self, mock_feature_orchestrator, mock_event_bus, 
                    mock_stats_tracker, mock_feature_group_mapper, mock_yaml_config):
    ...
    def test_initialization(self, mock_feature_orchestrator, mock_event_bus,
                          mock_stats_tracker, mock_feature_group_mapper):
    ...
    @pytest.mark.asyncio
    async def test_process_request_success(self, worker, mock_feature_orchestrator,
                                         mock_event_bus, mock_stats_tracker):
        ...
        assert mock_feature_orchestrator.compute_features.call_count == 4  # 2 symbols * 2 feature groups
    ...
    async def test_compute_features_use_features_in_group(self, worker, mock_feature_orchestrator):
        ...
        mock_feature_orchestrator.compute_features.assert_called_with(
            symbols=["TSLA"],
            feature_sets=["realized_vol", "implied_vol"]
        )
    ...
    @pytest.mark.asyncio
    async def test_process_request_partial_failure(self, worker, mock_feature_orchestrator,
                                                 mock_event_bus):
    ...
    @pytest.mark.asyncio
    async def test_process_request_complete_failure(self, worker, mock_event_bus,
                                                  mock_stats_tracker):
        ...
        # Make worker fail
        worker._compute_features_for_group = AsyncMock(
    ...
    async def test_metrics_recording(self, worker, mock_feature_orchestrator):
        ...
        with patch('main.events.handlers.feature_pipeline_helpers.feature_computation_worker.record_metric') as mock_metric:
            with patch('main.events.handlers.feature_pipeline_helpers.feature_computation_worker.timer') as mock_timer:
                ...
                await worker._compute_features_for_group(
                    "AAPL", 
                    "price_features",
                    ["price"]
                )
    ...
    @pytest.mark.asyncio
    async def test_empty_results_handling(self, worker, mock_feature_orchestrator,
                                        mock_event_bus):
