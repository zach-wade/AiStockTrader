...
from main.events.handlers.feature_pipeline_helpers.deduplication_tracker import (
...
class TestDeduplicationTracker:
    ...
    async def test_cleanup_old_entries(self, tracker):
        ...
        # Add recent entry
        tracker._recent_requests["new_hash"] = datetime.now(timezone.utc)
    ...
    async def test_auto_cleanup_trigger(self, tracker, sample_request):
        ...
        tracker._last_cleanup = datetime.now(timezone.utc) - timedelta(seconds=1)
...
class TestCreateRequestHash:
    ...
    def test_hash_considers_feature_order(self):
        ...
        request2 = FeatureRequest(
            symbol="AAPL",
            feature_groups=[FeatureGroup.VOLUME, FeatureGroup.PRICE]
        )
...
class TestIsDuplicateRequest:
    ...
    async def test_is_duplicate_with_id(self):
        ...
        request = FeatureRequest(
            symbol="GOOGL",
            feature_groups=[FeatureGroup.VOLUME]
        )
