...
from tests.fixtures.events.mock_database import (
    create_mock_db_pool,
    MockConnection,
    create_mock_batch_upsert,
    create_mock_transaction_context,
    create_mock_execute_with_retry
)
...
class TestDeadLetterQueueManager:
    ...
    async def dlq_manager(self, mock_db_pool):
        ...
        with patch('main.events.event_bus_helpers.dead_letter_queue_manager.DatabasePool') as mock_pool_class:
    ...
    async def test_add_failed_event(self, dlq_manager):
        ...
        with patch('main.events.event_bus_helpers.dead_letter_queue_manager.batch_upsert') as mock_batch_upsert:
            mock_batch_upsert.return_value = 1
            
            with patch('main.events.event_bus_helpers.dead_letter_queue_manager.transaction_context') as mock_tx:
    ...
    async def test_add_failed_event_records_metrics(self, dlq_manager):
        ...
        with patch('main.events.event_bus_helpers.dead_letter_queue_manager.batch_upsert'):
            with patch('main.events.event_bus_helpers.dead_letter_queue_manager.transaction_context') as mock_tx:
                with patch('main.events.event_bus_helpers.dead_letter_queue_manager.record_metric') as mock_metric:
                    mock_conn = MockConnection()
                    mock_tx.return_value.__aenter__.return_value = mock_conn
    ...
    async def test_get_failed_events(self, dlq_manager):
        ...
        mock_rows = [
            ...
            },
            {
                ...
                "retry_count": 1,
                "created_at": datetime.now(timezone.utc),
                "last_retry": datetime.now(timezone.utc)
            }
        ]
    ...
    async def test_get_failed_events_by_type(self, dlq_manager):
        ...
        await dlq_manager.get_failed_events(
        ...
        mock_rows = [
            {
                ...
                "retry_count": 1,
                "created_at": datetime.now(timezone.utc),
                "last_retry": datetime.now(timezone.utc) - timedelta(minutes=10)
            }
        ]
    ...
    async def test_delete_event(self, dlq_manager):
        ...
        with patch('main.events.event_bus_helpers.dead_letter_queue_manager.execute_with_retry') as mock_retry:
    ...
    async def test_error_handling_in_add_failed_event(self, dlq_manager):
        ...
        with patch('main.events.event_bus_helpers.dead_letter_queue_manager.batch_upsert') as mock_batch:
