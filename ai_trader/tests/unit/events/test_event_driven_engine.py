...
class TestEventDrivenEngine:
    ...
    async def test_initialize_components(self, engine):
        ...
        with patch('main.events.handlers.event_driven_engine.initialize_event_bus') as mock_init_bus:
            with patch('main.events.handlers.event_driven_engine.initialize_scanner_feature_bridge') as mock_init_bridge:
                with patch('main.events.handlers.event_driven_engine.FeaturePipelineHandler') as mock_handler_class:
                    # Setup mocks
                    mock_bus = AsyncMock()
    ...
    async def test_monitor_loop(self, engine):
        ...
        engine.components = {
            "event_bus": Mock(get_stats=Mock(return_value={
                "events_published": 100,
                "events_processed": 95
            })),
            "scanner_bridge": Mock(get_stats=Mock(return_value={
        ...
        # Setup monitors
        engine.monitors = {
            "health": Mock(check_health=Mock(return_value={
                "status": "healthy",
                "uptime": 3600
            })),
            "performance": Mock(get_metrics=Mock(return_value={
                "cpu_usage": 45.5,
                "memory_usage": 60.2
            }))
        }
    ...
    async def test_shutdown(self, engine):
        ...
        with patch('main.events.handlers.event_driven_engine.cleanup_scanner_feature_bridge') as mock_cleanup_bridge:
            with patch('main.events.handlers.event_driven_engine.cleanup_event_bus') as mock_cleanup_bus:
    ...
    async def test_component_stats_aggregation(self, engine):
        ...
        engine.components = {
            ...
            "scanner_bridge": Mock(get_stats=Mock(return_value={
                "alerts_processed": 250,
                "batch_size": 10
            })),
            "feature_pipeline": Mock(get_stats=Mock(return_value={
    ...
    async def test_graceful_degradation(self, engine):
        ...
        with patch('main.events.handlers.event_driven_engine.initialize_event_bus') as mock_bus:
            with patch('main.events.handlers.event_driven_engine.initialize_scanner_feature_bridge') as mock_bridge:
