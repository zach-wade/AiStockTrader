...
def multi_asset_data() -> dict[str, pd.DataFrame]:
    ...
    qqq_prices = 150 + base_series * 0.8 + np.random.randn(100) * 0.1 # Strong positive corr with SPY
    tlt_prices = 120 - base_series * 0.9 + np.random.randn(100) * 0.1 # Strong negative corr with SPY
...
def test_rolling_correlations(cross_asset_calculator, multi_asset_data):
    ...
    # Act
    features_df = cross_asset_calculator._calculate_rolling_correlations(target_df, returns_data, target_symbol)
    ...
    assert abs(features_df[corr_commodities_col].iloc[-1]) < 0.5 # A lenient threshold for random data
...
def test_handles_missing_assets_gracefully(cross_asset_calculator, multi_asset_data):
    ...
    try:
        ...
        features_df = cross_asset_calculator.calculate_features(multi_asset_data, target_symbol='SPY')
