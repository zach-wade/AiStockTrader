...
def mock_news_collector():
    ...
    sample_news = [
        {'timestamp': datetime(2025, 6, 23, 10, 0, 0, tzinfo=timezone.utc), 'sentiment_score': 0.8},
        {'timestamp': datetime(2025, 6, 23, 14, 0, 0, tzinfo=timezone.utc), 'sentiment_score': -0.4},
        {'timestamp': datetime(2025, 6, 23, 15, 30, 0, tzinfo=timezone.utc), 'sentiment_score': 0.6},
        # This one is older and should be outside most rolling windows
        {'timestamp': datetime(2025, 6, 22, 12, 0, 0, tzinfo=timezone.utc), 'sentiment_score': 0.1},
    ]
...
def sample_market_data_for_news() -> pd.DataFrame:
    ...
    index = pd.to_datetime(pd.date_range(start='2025-06-23 09:30:00', end='2025-06-23 16:00:00', freq='H'))
...
@pytest.mark.asyncio
async def test_calculate_features_success(news_feature_calculator, mock_news_collector, sample_market_data_for_news):
    """
    ...
    expected_cols = ['news_sentiment_24h', 'news_count_6h', 'sentiment_momentum_24h', 'sentiment_vs_avg']
...
@pytest.mark.asyncio
async def test_no_news_data_returns_empty_features(news_feature_calculator, mock_news_collector, sample_market_data_for_news):
