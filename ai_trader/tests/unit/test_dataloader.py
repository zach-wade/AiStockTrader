...
def mock_market_data_repo():
    ...
    repo.get_data_range = MagicMock(return_value=(
...
def mock_feature_store():
    ...
    store.get_data_range = MagicMock(return_value=(
        datetime(2024, 1, 1, tzinfo=timezone.utc),
        datetime(2024, 12, 31, tzinfo=timezone.utc)
    ))
    return store
...
class TestMarketDataSource:
    ...
    @pytest.mark.asyncio
    async def test_load_data_from_repo(self, mock_market_data_repo, mock_historical_manager, sample_market_data):
        """Test loading data from repository."""
        # Setup mocks
        mock_market_data_repo.get_data.return_value = sample_market_data
        
        with patch('main.feature_pipeline.dataloader.HistoricalManager', return_value=mock_historical_manager):
            with patch('main.feature_pipeline.dataloader.MarketDataRepository', return_value=mock_market_data_repo):
    ...
    @pytest.mark.asyncio
    async def test_load_data_fallback_to_historical(self, mock_market_data_repo, mock_historical_manager, sample_market_data):
...
class TestFeatureDataSource:
    ...
    async def test_load_specific_features(self, mock_feature_store, sample_feature_data):
        ...
        with patch('main.feature_pipeline.dataloader.FeatureStoreRepository', return_value=mock_feature_store):
...
class TestDataLoader:
    ...
    async def test_load_market_data_only(self, mock_config, sample_market_data):
        ...
        with patch('main.feature_pipeline.dataloader.get_config', return_value={'data_loader': mock_config}):
            with patch.object(MarketDataSource, 'load_data', new_callable=AsyncMock) as mock_load:
                ...
                with patch('main.feature_pipeline.dataloader.DataPreprocessor') as mock_preprocessor_class:
    ...
    async def test_load_with_features(self, mock_config, sample_market_data, sample_feature_data):
        ...
        with patch('main.feature_pipeline.dataloader.get_config', return_value={'data_loader': mock_config}):
            with patch.object(MarketDataSource, 'load_data', new_callable=AsyncMock) as mock_market_load:
                with patch.object(FeatureDataSource, 'load_data', new_callable=AsyncMock) as mock_feature_load:
    ...
    @pytest.mark.asyncio
    async def test_convenience_load_combined_data(self, mock_config, sample_market_data, sample_feature_data):
    ...
    def test_get_available_symbols(self, mock_config):
        ...
        with patch('main.feature_pipeline.dataloader.get_config', return_value={'data_loader': mock_config}):
            with patch.object(MarketDataSource, 'get_available_symbols', return_value=['AAPL', 'MSFT']):
                loader = DataLoader(mock_config)
...
class TestErrorHandling:
    ...
    async def test_source_loading_failure_continues(self, mock_config, caplog):
        ...
        with patch('main.feature_pipeline.dataloader.get_config', return_value={'data_loader': mock_config}):
            with patch.object(MarketDataSource, 'load_data', side_effect=Exception("Market data failed")):
                with patch.object(FeatureDataSource, 'load_data', new_callable=AsyncMock) as mock_feature_load:
    ...
    async def test_preprocessing_failure_fallback(self, mock_config, sample_market_data, caplog):
        ...
        with patch('main.feature_pipeline.dataloader.get_config', return_value={'data_loader': mock_config}):
            with patch.object(MarketDataSource, 'load_data', new_callable=AsyncMock) as mock_load:
                ...
                with patch('main.feature_pipeline.dataloader.DataPreprocessor') as mock_preprocessor_class:
                    mock_preprocessor = MagicMock()
                    mock_preprocessor.preprocess_market_data.side_effect = Exception("Preprocessing failed")
                    mock_preprocessor_class.return_value = mock_preprocessor
