...
from main.feature_pipeline.feature_orchestrator import FeatureOrchestrator, FeatureCache
from main.feature_pipeline.unified_feature_engine import UnifiedFeatureEngine
from main.feature_pipeline.feature_adapter import FeatureAdapter, create_feature_adapter
    ...
    def generate_signals(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        for symbol, features in data.items():
            if not features.empty and 'sma_10' in features.columns:
                signals[symbol] = {
                    'signal': 'BUY' if features['sma_10'].iloc[-1] > features['close'].iloc[-1] else 'HOLD',
                    'confidence': 0.75,
...
class TestFeaturePipelineFlow:
    ...
    async def test_complete_feature_pipeline_flow(
        ...
        with patch('main.feature_pipeline.feature_orchestrator.get_event_bus', return_value=mock_event_bus):
            with patch('main.feature_pipeline.feature_orchestrator.get_archive', return_value=mock_data_archive):
    ...
    async def test_feature_calculation_with_caching(
        ...
        assert mock_market_data_repo.get_data_for_symbols_and_range.call_count == initial_database_calls
...
class TestFeatureStorageIntegration:
    ...
    async def test_storage_failure_resilience(
        ...
        mock_market_data_repo.get_data_for_symbols_and_range.return_value = sample_market_data
        mock_feature_store_repo.store_features.side_effect = Exception("PostgreSQL connection failed")
...
class TestEventDrivenFeatures:
    ...
    async def test_batch_processing_low_priority_requests(
        ...
        low_priority_request = FeatureRequestEvent(data={
            'symbols': ['GOOGL'],
...
class TestFeatureAdapterIntegration:
    ...
    async def test_end_to_end_feature_to_strategy_flow(
        ...
        strategy_features = adapter.prepare_features_for_strategy(
            features, "test_strategy"
        )
