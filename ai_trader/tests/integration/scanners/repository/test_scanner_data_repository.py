...
class TestScannerDataRepository:
    ...
    async def test_get_social_sentiment(
        ...
        with patch.object(scanner_repository, '_execute_social_query', new_callable=AsyncMock) as mock_query:
    ...
    async def test_concurrent_queries(
        ...
        with patch.object(scanner_repository, '_execute_query', new_callable=AsyncMock) as mock_query:
            ...
            tasks = [
                scanner_repository.get_market_data(test_symbols[:2], query_filter)
                for _ in range(5)
            ]
    ...
    async def test_query_performance(
        ...
        with patch.object(scanner_repository, '_execute_query', new_callable=AsyncMock) as mock_query:
            ...
            assert query_time_ms < threshold, f"Query took {query_time_ms}ms, threshold is {threshold}ms"
    ...
    async def test_large_symbol_list_query(
        ...
        with patch.object(scanner_repository, '_execute_query', new_callable=AsyncMock) as mock_query:
            ...
            assert query_time_ms < threshold, f"Large query took {query_time_ms}ms, threshold is {threshold}ms"
    ...
    async def test_get_sector_data(
        ...
        with patch.object(scanner_repository, '_execute_sector_query', new_callable=AsyncMock) as mock_query:
