...
class TestEarningsScannerRepository:
    ...
    async def test_earnings_surprise_analysis(
        ...
        with patch.object(scanner_repository, 'get_earnings_data') as mock_earnings:
            ...
            surprise_earnings = [
                {
                    ...
                    'period_ending': datetime.now(timezone.utc) - timedelta(days=31),
                    'eps_actual': 3.50,
                    'eps_estimate': 3.25,
                ...
                },
                {
                    ...
                    'period_ending': datetime.now(timezone.utc) - timedelta(days=32),
                    'eps_actual': 1.20,
                    'eps_estimate': 1.45,
                ...
                },
                {
                    ...
                    'period_ending': datetime.now(timezone.utc) - timedelta(days=33),
                    'eps_actual': 2.75,
                    'eps_estimate': 2.73,
            ...
            positive_surprises = [
                earning for earning in result 
                if earning['surprise_percent'] > 5.0
            ]
            negative_surprises = [
                earning for earning in result 
                if earning['surprise_percent'] < -5.0
    ...
    async def test_earnings_calendar_upcoming(
        ...
        with patch.object(scanner_repository, 'get_earnings_data') as mock_earnings:
            ...
            upcoming_earnings = [
                {
                    'symbol': 'AAPL',
                    'report_date': datetime.now(timezone.utc) + timedelta(days=2),  # Upcoming
                    'period_ending': datetime.now(timezone.utc) - timedelta(days=1),
                    'eps_estimate': 3.75,
                ...
                {
                    'symbol': 'GOOGL',
                    'report_date': datetime.now(timezone.utc) + timedelta(days=5),  # Next week
                    'period_ending': datetime.now(timezone.utc) - timedelta(days=4),
                    'eps_estimate': 1.55,
                ...
                {
                    'symbol': 'MSFT',
                    'report_date': datetime.now(timezone.utc) + timedelta(days=1),  # Tomorrow
                    'period_ending': datetime.now(timezone.utc) - timedelta(days=2),
                    'eps_estimate': 2.85,
            ...
            near_term = [
                earning for earning in result 
                if (earning['report_date'] - datetime.now(timezone.utc)).days <= 2
            ]
            ...
            confirmed_earnings = [
                earning for earning in result 
                if earning.get('confirmed', False)
            ]
    ...
    async def test_earnings_guidance_analysis(
        ...
        with patch.object(scanner_repository, 'get_earnings_data') as mock_earnings:
            ...
            positive_guidance = [
                earning for earning in result 
                if earning.get('guidance_raised', False)
            ]
            conservative_guidance = [
                earning for earning in result 
                if not earning.get('guidance_raised', True)
            ]
            ...
            assert aapl_earnings['next_quarter_eps_guidance'] > aapl_earnings['next_quarter_eps_consensus']
        ...
        with patch.object(scanner_repository, 'get_earnings_data') as mock_earnings:
            ...
            for i, (eps, revenue) in enumerate(zip(quarterly_eps, quarterly_revenue)):
                historical_earnings.append({
                    ...
                    'fiscal_year': 2024,
                    'yoy_eps_growth': (eps - quarterly_eps[0]) / quarterly_eps[0] * 100 if i > 0 else 0,
                    'qoq_eps_growth': (eps - quarterly_eps[i-1]) / quarterly_eps[i-1] * 100 if i > 0 else 0
                })
            ...
            assert all(revenue_values[i] <= revenue_values[i+1] for i in range(len(revenue_values)-1))
    ...
    async def test_earnings_conference_call_sentiment(
        ...
        with patch.object(scanner_repository, 'get_earnings_data') as mock_earnings:
            ...
            positive_calls = [
                earning for earning in result 
                if earning.get('conference_call_sentiment', 0.5) > 0.7
            ]
            negative_calls = [
                earning for earning in result 
                if earning.get('conference_call_sentiment', 0.5) < 0.4
            ]
    ...
    async def test_earnings_estimate_revisions(
        ...
        with patch.object(scanner_repository, 'get_earnings_data') as mock_earnings:
            ...
            upward_revisions = [
                earning for earning in result 
                if earning.get('revision_trend') == 'upward'
            ]
            downward_revisions = [
                earning for earning in result 
                if earning.get('revision_trend') == 'downward'
            ]
