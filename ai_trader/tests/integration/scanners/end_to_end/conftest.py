...
from main.interfaces.scanners import IScanner, IScannerRepository, IScannerOrchestrator
from main.events.core.event_bus_factory import EventBusFactory
from main.events.types.event_types import ScannerAlertEvent, EventType
...
from tests.fixtures.data_pipeline.database_fixtures import real_database
...
async def end_to_end_repository(
    ...
    repository = ScannerDataRepository(
        db_adapter=real_database,
        storage_router=mock_storage_router,
        event_bus=end_to_end_event_bus
    )
...
async def end_to_end_scanner_factory(
    ...
    factory = ScannerFactory(
        config=end_to_end_config,
        repository=end_to_end_repository,
        event_bus=end_to_end_event_bus
    )
    return factory
...
def comprehensive_test_symbols():
    ...
    return [
        'AAPL', 'GOOGL', 'MSFT', 'AMZN', 'TSLA',  # Large cap tech
...
async def event_collector():
    ...
    class ComprehensiveEventCollector:
        ...
        def get_events_by_scanner(self, scanner_name: str) -> List[ScannerAlertEvent]:
            ...
            return [
                event for event in self.scanner_alerts
        ...
        def get_events_by_symbol(self, symbol: str) -> List[ScannerAlertEvent]:
            ...
            return [
                event for event in self.scanner_alerts
                if getattr(event, 'symbol', '') == symbol
            ]
        ...
        def get_high_confidence_alerts(self, threshold: float = 0.8) -> List[ScannerAlertEvent]:
            ...
            return [
                event for event in self.scanner_alerts
                if getattr(event, 'score', 0) >= threshold
            ]
        ...
        def get_performance_metrics(self) -> Dict[str, Any]:
            ...
            return {
                ...
                'events_per_second': len(self.all_events) / (max(self.processing_times) / 1000) if self.processing_times else 0,
