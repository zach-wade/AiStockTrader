...
from main.interfaces.scanners import IScannerOrchestrator, IScanner
...
class TestScannerOrchestratorIntegration:
    ...
    async def test_sequential_scanner_execution_workflow(
        ...
        if len(execution_order) > 1:
            ...
            assert execution_order == sorted(execution_order, reverse=True)  # Assuming priority ordering
    ...
    async def test_scanner_priority_orchestration(
        ...
        if hasattr(end_to_end_orchestrator, 'config') and \
           getattr(end_to_end_orchestrator.config, 'execution_strategy', None) == 'sequential':
    ...
    async def test_scanner_deduplication_workflow(
        ...
        mock_scanners = [
            MockDuplicateScanner('scanner_a'),
            MockDuplicateScanner('scanner_b')
        ]
    ...
    async def test_orchestrator_error_isolation(
        ...
        working_scanner_alerts = [
            ...
            if alert.scanner_name != 'failing_scanner'
        ]
    ...
    async def test_orchestrator_resource_management(
        ...
        class MockResourceScanner:
            ...
            async def scan(self, symbols: List[str]) -> List[ScanAlert]:
                ...
                resource_usage['memory_allocations'] += len(symbols) * 1000  # Simulate memory per symbol
        ...
        max_concurrent = max(resource_usage.get('concurrent_operations', 0), 3)  # Allow some tolerance
