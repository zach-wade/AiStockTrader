...
from main.interfaces.scanners import IScanner, IScannerOrchestrator
...
class TestEventBusScannerIntegration:
    ...
    async def test_event_aggregation_and_correlation(
        self,
        test_event_bus: IEventBus,
        test_symbols
    ):
    ...
    async def test_event_filtering_and_routing(
        self,
        test_event_bus: IEventBus,
        test_symbols
    ):
    ...
    async def test_event_bus_health_monitoring(
        self,
        test_event_bus: IEventBus,
        test_symbols
    ):
    ...
    async def test_scanner_event_circuit_breaker(
        self,
        test_event_bus: IEventBus,
        test_symbols
    ):
        ...
        class CircuitBreakerScanner(MockEventPublishingScanner):
            ...
            async def publish_alerts_to_event_bus(self, alerts):
                ...
                if self.circuit_open:
                    ...
                    if (datetime.now(timezone.utc) - self.last_failure_time).total_seconds() < 10:
