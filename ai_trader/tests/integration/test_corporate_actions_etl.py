...
from main.data_pipeline.types import RawDataRecord, DataPipelineResult
...
class TestCorporateActionsETL:
    ...
    @pytest.mark.asyncio
    async def test_corporate_actions_etl_flow(self, processing_manager, mock_db_adapter,
                                              sample_corporate_actions_data):
        ...
        with patch('main.data_pipeline.processing.manager.get_archive') as mock_get_archive:
            ...
            mock_archive.query_raw_records = AsyncMock(return_value=[
                RawDataRecord(
                    source='polygon',
    ...
    @pytest.mark.asyncio
    async def test_corporate_action_adjustments(self, config, mock_db_adapter,
                                                 sample_market_data):
        ...
        with patch.object(transformer.dividends_repo, 'get_corporate_actions_for_period',
            ...
            pre_div_data = adjusted_data[
                (adjusted_data.index < '2024-11-08') & 
                (adjusted_data.index >= '2024-06-10')
            ]
