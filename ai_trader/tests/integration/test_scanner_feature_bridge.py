...
from main.events.types import ScannerAlertEvent, FeatureRequestEvent, EventType, ScanAlert, AlertType
from main.events.scanner_bridge_helpers.feature_request_batcher import FeatureRequestBatch
...
def mock_config():
    ...
    return {
        'events': {
            'batch_size': 5,
            'batch_interval_seconds': 0.1  # Fast for testing
        },
        'scanner_bridge': {
...
class TestScannerFeatureBridgeFlow:
    ...
    async def test_basic_alert_to_feature_request_flow(
        ...
        mock_event_bus.subscribe.assert_called_with(
            ...
            bridge._handle_scanner_alert_event
        )
    ...
    @pytest.mark.asyncio
    async def test_multiple_alerts_batching(
        self,
        mock_config,
        mock_event_bus,
        mock_scanner
    ...
    @pytest.mark.asyncio
    async def test_alert_type_feature_mapping(
        self,
        mock_config,
        mock_event_bus,
        mock_scanner
        ...
        test_cases = [
            ...
            },
            {
                ...
                'expected_features': ['price_features', 'trend_features', 'support_resistance_features']
            },
            {
                'symbol': 'GOOGL',
                'alert_type': 'catalyst_detected',
...
class TestPriorityHandling:
    ...
    @pytest.mark.asyncio
    async def test_urgent_alert_priority_boost(
        self,
        mock_config,
        mock_event_bus,
        mock_scanner
...
class TestBatchProcessing:
    ...
    @pytest.mark.asyncio
    async def test_time_based_batch_processing(
        self,
        mock_config,
        mock_event_bus,
        mock_scanner
    ...
    @pytest.mark.asyncio
    async def test_batch_overflow_handling(
        self,
        mock_config,
        mock_event_bus,
        mock_scanner
...
class TestEventIntegration:
    ...
    @pytest.mark.asyncio
    async def test_event_correlation_preservation(
        self,
        mock_config,
        mock_event_bus,
        mock_scanner
    ...
    async def test_invalid_event_handling(
        ...
        assert "Invalid ScannerAlertEvent" in caplog.text or "missing symbol or alert_type" in caplog.text
...
class TestErrorHandlingResilience:
    ...
    @pytest.mark.asyncio
    async def test_batch_processor_task_cancellation(
        self,
        mock_config,
        mock_event_bus
    ):
...
class TestEdgeCasesAndBoundaryConditions:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_symbol_handling(
        self,
        mock_config,
        mock_event_bus,
        mock_scanner
    ...
    @pytest.mark.asyncio
    async def test_zero_and_extreme_scores(
        self,
        mock_config,
        mock_event_bus,
        mock_scanner
...
class TestPerformanceStatistics:
    ...
    async def test_statistics_accuracy(
        ...
        assert stats['unique_symbols_processed_count'] == len(unique_symbols)  # Unique symbols count
