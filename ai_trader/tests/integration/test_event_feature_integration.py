...
class MockScanner:
    ...
    async def send_alert(self, symbol: str, alert_type: str, data: Dict[str, Any]):
        ...
        alert_event = ScannerAlertEvent(data={
            'symbol': symbol,
...
class TestScannerFeatureIntegration:
    ...
    async def test_scanner_alert_triggers_feature_calculation(
        ...
        with patch('main.feature_pipeline.feature_orchestrator.get_event_bus', return_value=event_bus):
            with patch('main.feature_pipeline.feature_orchestrator.get_archive', return_value=mock_dependencies['data_archive']):
    ...
    async def test_multiple_scanner_alerts_parallel_processing(
        ...
        for symbol, alert_type in zip(symbols, alert_types):
            task = scanner.send_alert(
                symbol=symbol,
                alert_type=alert_type,
                data={'test_metric': np.random.random()}
            )
    ...
    @pytest.mark.asyncio
    async def test_alert_type_to_feature_mapping(
        self,
        mock_config,
        mock_dependencies
    ):
        ...
        for case in test_cases:
            ...
            expected_features = orchestrator._alert_feature_mapping.get(
                case['alert_type'],
                orchestrator._alert_feature_mapping['default']
            )
...
class TestFeatureRequestEvents:
    ...
    async def test_high_priority_feature_request_immediate_processing(
        ...
        feature_request = FeatureRequestEvent(data={
            'symbols': ['AAPL', 'MSFT'],
    ...
    async def test_low_priority_feature_request_batch_processing(
        ...
        assert orchestrator.calculation_stats['feature_requests_processed'] >= len(low_priority_requests)
    
    @pytest.mark.asyncio
    async def test_feature_request_priority_ordering(
        self,
        mock_config,
        mock_dependencies
    ):
        ...
        requests = [
            FeatureRequestEvent(data={
                'symbols': ['LOW1'], 'features': ['basic'], 'priority': 2,
                'requestor': 'low_priority_1', 'request_id': 'low_1'
            }),
            FeatureRequestEvent(data={
                'symbols': ['HIGH1'], 'features': ['basic'], 'priority': 8,
                'requestor': 'high_priority_1', 'request_id': 'high_1'
            }),
            FeatureRequestEvent(data={
                'symbols': ['MED1'], 'features': ['basic'], 'priority': 5,
                'requestor': 'medium_priority_1', 'request_id': 'med_1'
            }),
            FeatureRequestEvent(data={
                'symbols': ['HIGH2'], 'features': ['basic'], 'priority': 9,
                'requestor': 'high_priority_2', 'request_id': 'high_2'
            })
        ]
...
class TestEventBusResilience:
    ...
    @pytest.mark.asyncio
    async def test_event_processing_error_recovery(
        self,
        mock_config,
        mock_dependencies
    ):
        ...
        with patch('main.feature_pipeline.feature_orchestrator.get_event_bus', return_value=event_bus):
            with patch('main.feature_pipeline.feature_orchestrator.get_archive', return_value=mock_dependencies['data_archive']):
                ...
                call_count = [0]
                original_get_data = mock_dependencies['market_data_repo'].get_data_for_symbols_and_range
                ...
                mock_dependencies['market_data_repo'].get_data_for_symbols_and_range.side_effect = failing_get_data
        ...
        assert orchestrator.calculation_stats['scanner_triggered_calculations'] > 0  # Second call succeeded
