...
class MockAPIClient:
    ...
    async def get_market_data(
        ...
        **kwargs
        ...
        # Generate mock data
        date_range = pd.date_range(start_time, end_time, freq='D', tz=timezone.utc)
        data_frames = []
...
class MarketDataGenerator:
    ...
    def generate_intraday_data(
        ...
        for symbol in symbols:
            ...
            closes = prices
            highs = [max(o, c) * (1 + abs(secure_numpy_normal(0, 0.002))) for o, c in zip(opens, closes)]
            lows = [min(o, c) * (1 - abs(secure_numpy_normal(0, 0.002))) for o, c in zip(opens, closes)]
            
            # Generate volume with pattern
            base_volume = 50000
            volume_pattern = np.sin(np.arange(len(date_range)) * 2 * np.pi / 24) * 20000 + base_volume
    ...
    def generate_feature_data(
        ...
        date_range = pd.date_range(start_date, end_date, freq='D', tz=timezone.utc)
        features_dict = {}
