...
from main.scanners import ScanAlert, AlertType
...
class TestEndToEnd:
    ...
    async def test_data_validation_flow(self, sample_market_data):
        ...
        assert feature_ready_result.passed, f"Feature-ready validation failed: {feature_ready_result.errors}"
    ...
    async def test_quality_score_calculation(self, sample_market_data):
        ...
        assert 0 < quality_score < 1.0, f"Quality score should be between 0 and 1, got {quality_score}"
        assert quality_score < 0.8, f"Quality score should be low due to data issues, got {quality_score}"
