...
from main.trading_engine.core.execution_engine import (
    ExecutionEngine, ExecutionEngineStatus, ExecutionMode
)
from main.trading_engine.core.trading_system import TradingMode, TradingSystemStatus
from main.models.common import Order, OrderSide, OrderType, OrderStatus, Position
...
class TestExecutionEngineIntegration:
    ...
    async def test_emergency_stop(self, execution_engine):
        ...
        execution_engine.trading_systems = {
            'broker1': mock_system1,
            'broker2': mock_system2
        }
    ...
    async def test_comprehensive_status(self, execution_engine):
        ...
        execution_engine.session_start_time = datetime.now(timezone.utc)
        ...
        mock_system = Mock()
        mock_system.get_system_status = AsyncMock(return_value={
            'status': 'running',
            'trading_enabled': True,
            'active_orders': 2
        })
    ...
    async def test_broker_selection_logic(self, execution_engine):
        ...
        mock_system1 = Mock()
        mock_system1.get_system_status = AsyncMock(return_value={
            'status': 'running',
            'trading_enabled': True
        })
        
        mock_system2 = Mock()
        mock_system2.get_system_status = AsyncMock(return_value={
            'status': 'running',
            'trading_enabled': False  # Not available
        })
...
class TestExecutionManagerIntegration:
    ...
    async def test_position_management(self, execution_manager):
        ...
        # Mock portfolio manager
        mock_portfolio = Mock()
        mock_portfolio.get_position = AsyncMock(return_value={
            'symbol': 'AAPL',
            'quantity': 100,
            'avg_price': 150.0,
            'unrealized_pnl': 500.0
        })
        mock_portfolio.get_all_positions = AsyncMock(return_value={
            'AAPL': {'quantity': 100},
            'GOOGL': {'quantity': 50}
        })
