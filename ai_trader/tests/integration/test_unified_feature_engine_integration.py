...
mock_modules = {
    ...
    'textblob': type('module', (), {'TextBlob': lambda x: type('blob', (), {'sentiment': type('sentiment', (), {'polarity': 0.0})})()}),
    'scipy.stats': type('module', (), {'percentileofscore': lambda x, y: 50.0}),
    'sklearn.preprocessing': type('module', (), {'StandardScaler': lambda: type('scaler', (), {'fit_transform': lambda self, x: x, 'transform': lambda self, x: x})()}),
    'sklearn.decomposition': type('module', (), {'FactorAnalysis': lambda **kwargs: type('fa', (), {'fit_transform': lambda self, x: np.random.randn(len(x), 5), 'components_': np.random.randn(5, 10)})()}),
    'sklearn.cluster': type('module', (), {'DBSCAN': lambda **kwargs: type('dbscan', (), {'fit_predict': lambda self, x: np.secure_randint(-1, 3, len(x))})()})
}
...
class TestUnifiedFeatureEngineIntegration(unittest.TestCase):
    ...
    def test_individual_calculator_instantiation(self):
        ...
        for calc_name in calculator_classes:
            try:
                ...
                if calc_name == 'TechnicalIndicatorsCalculator':
                    from main.feature_pipeline.calculators.technical_indicators import TechnicalIndicatorsCalculatorCalculator
                    ...
                    from main.feature_pipeline.calculators.insider_analytics import InsiderAnalyticsCalculator
                    ...
                    from main.feature_pipeline.calculators.enhanced_cross_sectional import EnhancedCrossSectionalCalculator
                ...
                failed_imports.append(f"{calc_name} - {str(e)}")
        ...
        self.assertGreaterEqual(len(successfully_imported), 3, 
                               "Should have at least 3 core calculators working")
    ...
    def test_feature_calculation_pipeline(self):
        ...
        try:
            ...
            print(f"✅ Feature calculation pipeline successful: {len(final_features.columns)} features calculated")
    ...
    def test_feature_naming_consistency(self):
        ...
        try:
            ...
            self.assertEqual(len(unique_names), len(feature_names), 
            ...
            for name in feature_names:
                ...
                self.assertRegex(name, r'^[a-zA-Z0-9_]+$', 
                               f"Feature name '{name}' contains invalid characters")
...
class TestCalculatorSpecificFunctionality(unittest.TestCase):
    ...
    def test_sector_analytics_specific(self):
        ...
        try:
            ...
            sector_features = [col for col in features.columns if any(x in col for x in 
