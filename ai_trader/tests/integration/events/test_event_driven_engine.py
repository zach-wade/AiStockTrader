...
from main.events.types import (
    ...
    ScanAlert, RiskEvent, PositionEvent
)
...
class TestEventDrivenEngineIntegration:
    ...
    async def test_complete_trading_workflow(self, event_engine, mock_broker, mock_components):
        ...
        mock_components['order_manager'].create_order = AsyncMock(return_value={
            'order_id': 'ORD123',
            'status': 'pending'
        })
    ...
    async def test_event_ordering_and_dependencies(self, event_engine):
        ...
        event2 = Event(
            event_type=EventType.GENERIC,
            event_id="EVT2",
            metadata={'depends_on': ['EVT1']}
        )
        
        event3 = Event(
            event_type=EventType.GENERIC,
            event_id="EVT3",
            metadata={'depends_on': ['EVT1', 'EVT2']}
        )
    ...
    async def test_event_compensation_pattern(self, event_engine):
        ...
        try:
            ...
            event2 = Event(
                event_type=EventType.WORKFLOW_STEP,
                metadata={'step': 2, 'should_fail': True}
            )
            await event_engine.publish(event2)
