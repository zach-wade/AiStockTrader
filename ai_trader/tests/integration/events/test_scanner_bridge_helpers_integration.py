...
class TestScannerBridgeHelpersIntegration:
    ...
    async def test_complete_alert_to_feature_request_flow(
        ...
        assert bridge_stats['unique_symbols_processed'] == len(set(alert.symbol for alert in sample_scanner_alerts))
    ...
    @pytest.mark.asyncio
    async def test_priority_calculation_and_boosting(
        self,
        scanner_bridge_components
    ):
        ...
        test_cases = [
            ...
            {'score': 0.5, 'alert_type': 'catalyst_detected', 'expected_range': (5, 9)},  # Should get boost
            {'score': 0.8, 'alert_type': 'catalyst_detected', 'expected_range': (8, 10)}, # High score + boost
            {'score': 0.3, 'alert_type': 'breakout', 'expected_range': (1, 5)},
            {'score': 0.9, 'alert_type': 'opportunity_signal', 'expected_range': (8, 10)}, # Another urgent type
        ]
    ...
    @pytest.mark.asyncio
    async def test_feature_request_batching_logic(
        self,
        scanner_bridge_components
    ):
    ...
    @pytest.mark.asyncio
    async def test_request_deduplication_across_batches(
        self,
        scanner_bridge_components
    ):
    ...
    @pytest.mark.asyncio
    async def test_dispatcher_event_publishing(
        self,
        scanner_bridge_components
    ):
    ...
    @pytest.mark.asyncio 
    async def test_high_volume_processing_coordination(
        self,
        scanner_bridge_components
    ):
    ...
    @pytest.mark.asyncio
    async def test_component_coordination_with_mixed_priorities(
        self,
        scanner_bridge_components
    ):
        ...
        priority_test_cases = [
            ...
            {'symbol': 'HIGH1', 'alert_type': 'catalyst_detected', 'score': 0.9}, # High priority + boost
            {'symbol': 'MED1', 'alert_type': 'breakout', 'score': 0.6},         # Medium priority
            {'symbol': 'HIGH2', 'alert_type': 'opportunity_signal', 'score': 0.8}, # High priority + boost
        ...
        catalyst_request = next(r for r in processed_requests if r['alert_type'] == 'catalyst_detected')
        assert catalyst_request['priority'] >= 8
        
        opportunity_request = next(r for r in processed_requests if r['alert_type'] == 'opportunity_signal')
