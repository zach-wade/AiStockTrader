...
from main.utils.resilience import CircuitBreakerError, RetryError
...
def failing_scenarios():
    ...
    return {
        'network_failures': [
            ...
            {'type': 'intermittent', 'failure_rate': 0.7, 'error': Exception("Intermittent failure")}
        ],
        'data_failures': [
...
class TestErrorHandlingIntegration:
    ...
    async def test_circuit_breaker_protection(
        ...
        assert "circuit breaker" in caplog.text.lower() or "failure threshold" in caplog.text.lower()
        ...
        assert failure_count < len(failing_requests)  # Not all requests should have reached the service
    ...
    @pytest.mark.asyncio
    async def test_dead_letter_queue_processing(
        self,
        error_resilient_event_system,
        caplog
    ):
        ...
        async def selective_failure_service(symbols, features, **kwargs):
            ...
            return {
                ...
                for symbol in symbols if 'DLQ_FAIL' not in symbol
            }
        ...
        test_events = [
            FeatureRequestEvent(
                symbols=['SUCCESS_SYMBOL_1'],
                features=['price_features'],
                requester='dlq_test'
            ),
            FeatureRequestEvent(
                symbols=['DLQ_FAIL_SYMBOL_1'],
                features=['volume_features'], 
                requester='dlq_test'
            ),
            FeatureRequestEvent(
                symbols=['SUCCESS_SYMBOL_2'],
                features=['trend_features'],
                requester='dlq_test'
            ),
            FeatureRequestEvent(
                symbols=['DLQ_FAIL_SYMBOL_2'],
                features=['price_features'],
                requester='dlq_test'
            )
        ]
    ...
    @pytest.mark.asyncio
    async def test_retry_mechanism_across_components(
        self,
        error_resilient_event_system,
        caplog
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_and_healing(
        self,
        error_resilient_event_system,
        caplog
    ):
        ...
        assert final_pipeline_stats['requests_received'] >= 15  # Should have processed recovery phase
    ...
    @pytest.mark.asyncio
    async def test_error_correlation_and_tracking(
        self,
        error_resilient_event_system,
        caplog
    ):
        ...
        bridge_stats = bridge.get_stats()
        assert bridge_stats['alerts_received_total'] >= len([alert for scenario in correlation_scenarios for alert in scenario['alerts']])
