...
from main.events.types import (
    OrderEvent, FillEvent, RiskEvent, ScanAlert
)
...
class TestEventBusIntegration:
    ...
    async def test_event_bus_under_load(self, event_bus):
        ...
        while events_received < total_events and (asyncio.get_event_loop().time() - start_wait) < max_wait:
            await asyncio.sleep(0.1)
...
class TestEventBusRealWorldScenarios:
    ...
    async def test_order_lifecycle_events(self, event_bus):
        ...
        # 3. Submit to broker
        # 4. Receive fill
        await event_bus.publish(FillEvent(
            order_id=order_id,
            symbol="AAPL",
            quantity=100,
            direction="buy",
