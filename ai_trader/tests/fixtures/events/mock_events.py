...
from main.interfaces.events import (
    Event, EventType, EventPriority, OrderEvent, MarketEvent
)
from main.events.types import (
    ScanAlert, AlertType, FillEvent, RiskEvent
)
...
def create_feature_request_event(
    symbols: list = None,
    features: list = None,
    **kwargs
) -> Event:
...
def create_feature_computed_event(
    symbols_processed: list = None,
    computation_time: float = 1.5,
    **kwargs
) -> Event:
...
def create_error_event(
    error_type: str = "computation_failed",
...
def create_multiple_alerts(count: int = 5, **kwargs) -> list:
    ...
    for i in range(count):
        ...
        alerts.append(create_scan_alert(
            symbol=symbol,
            alert_type=alert_type,
            score=min(score, 1.0),
            **kwargs
