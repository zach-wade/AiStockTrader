...
try:
    from main.utils.core import ExtendedJSONEncoder, JSONUtils, to_json, from_json
except ImportError as e:
...
try:
    from main.monitoring.metrics.unified_metrics import VaRCalculator, RiskMetricsCalculator, VaRMethod
except ImportError as e:
...
try:
    from main.data_pipeline.sources.base_api_client import BaseAPIClient, AuthMethod, RateLimitConfig
except ImportError as e:
...
try:
    from main.utils.data import BatchProcessor, DataChunker, BatchConfig, ChunkConfig
except ImportError as e:
...
try:
    from main.utils.resilience import unified_retry, create_retry_config
except ImportError as e:
...
def test_unified_validator():
    ...
    def test_market_data_validation():
        ...
        # Test valid data
        valid_data = [
            {'symbol': 'AAPL', 'timestamp': datetime.now(), 'open': 150.0, 'close': 155.0, 'volume': 1000}
        ]
        ...
        # Test invalid data
        invalid_data = [
            {'symbol': '', 'timestamp': 'invalid', 'open': -1, 'close': None}
        ]
