...
class TestSystemHealthValidation:
    ...
    @pytest.mark.asyncio
    @pytest.mark.health
    async def test_resource_monitoring_accuracy(self, performance_monitor, resource_monitor, health_config):
        """
        ...
        try:
            ...
            assert stats['peak_memory_mb'] > stats['avg_memory_mb'], "Peak memory should exceed average"
            ...
            print(f"❌ PERF-TEST 6.1 FAILED: {str(e)}")
            raise
        finally:
        ...
        try:
            ...
            for scenario in error_scenarios:
                ...
                try:
                    ...
                    print(f"   ⚠️  {scenario}: Recovery failed - {str(e)}")
                    continue
            ...
            assert len(recovery_times) >= 3, f"Too few successful recoveries: {len(recovery_times)}/5"
            assert avg_recovery_time < 30, f"Recovery too slow: {avg_recovery_time:.2f}s (>30s)"
            assert max(recovery_times) < 60, f"Slowest recovery too slow: {max(recovery_times):.2f}s"
            
            print(f"✅ PERF-TEST 6.2 PASSED")
            ...
            print(f"❌ PERF-TEST 6.2 FAILED: {str(e)}")
            raise
    ...
    async def test_logging_system_performance(self, health_config):
        ...
        try:
            ...
            assert logs_per_second >= target_logs_per_second, f"Logging too slow: {logs_per_second:.0f} logs/s (target: {target_logs_per_second})"
            ...
            print(f"❌ PERF-TEST 6.3 FAILED: {str(e)}")
            raise
    ...
    async def test_health_check_performance(self):
        ...
        try:
            ...
            assert elapsed_time < target_total_time, f"Health check too slow: {elapsed_time:.2f}s (target: {target_total_time}s)"
            assert avg_check_time < target_avg_check_time, f"Average check too slow: {avg_check_time:.3f}s"
            assert healthy_checks >= total_checks * 0.8, f"Too many unhealthy checks: {healthy_checks}/{total_checks}"
            
            print(f"✅ PERF-TEST 6.4 PASSED")
            ...
            print(f"❌ PERF-TEST 6.4 FAILED: {str(e)}")
            raise
    ...
    async def test_monitoring_overhead(self, performance_monitor, resource_monitor):
        ...
        try:
            ...
            assert time_overhead_percent < max_time_overhead, f"Time overhead too high: {time_overhead_percent:.1f}%"
            assert cpu_overhead_percent < max_cpu_overhead, f"CPU overhead too high: {cpu_overhead_percent:.1f}%"
            assert elapsed_time < 15, f"Test took too long: {elapsed_time:.2f}s"
            
            print(f"✅ PERF-TEST 6.5 PASSED")
            ...
            print(f"❌ PERF-TEST 6.5 FAILED: {str(e)}")
            raise
    ...
    async def _check_memory_health(self):
        ...
        return {'memory_percent': memory_percent, 'status': 'healthy' if memory_percent < 80 else 'warning'}
    ...
    async def _check_disk_health(self):
        ...
        return {'disk_percent': disk_percent, 'status': 'healthy' if disk_percent < 90 else 'warning'}
...
class TestSystemHealthBenchmarks:
    ...
    def test_health_check_speed_benchmark(self):
        ...
        assert duration < target_duration, f"Health check too slow: {duration:.3f}s (target: {target_duration}s)"
