...
class TestArchiveWidget:
    ...
    def mock_metrics_collector(self):
        ...
        collector.get_operation_stats = Mock(return_value={
        ...
        collector._get_recent_operations = Mock(return_value=[
            {'timestamp': datetime.now(timezone.utc), 'operation': 'write'}
        ] * 60)
    ...
    async def test_update_data(self, widget, mock_metrics_collector):
        ...
        assert abs(metrics['storage']['total_size_mb'] - 5428.67) < 0.1  # Allow small floating point differences
    ...
    async def test_overview_panel(self, widget):
        ...
        assert abs(float(targets['Storage Used']['value']) - 5428.67) < 0.1  # Allow small floating point differences
