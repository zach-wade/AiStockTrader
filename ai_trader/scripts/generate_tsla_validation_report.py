...
class TSLAValidationReport:
    ...
    async def _validate_features(self, db_adapter):
        ...
        if tech_result and tech_result['feature_count'] > 0:
            print(f"Technical Features: ✓ {tech_result['feature_count']} features, {tech_result['total_records']:,} records")
    ...
    async def _validate_model_readiness(self, db_adapter):
        ...
        if data_result:
            ...
            if hourly >= 1000:
                ...
                section["score"] -= 40
                self.results["issues"].append(f"Insufficient training data ({hourly} hourly records)")
    ...
    def _generate_recommendations(self):
        ...
        if any("missing" in issue.lower() and "data" in issue.lower() for issue in self.results["issues"]):
