    ...
    for symbol in test_symbols:
        ...
        if details:
            ...
            for key, value in details.items():
                if isinstance(value, str) and any(term in str(value).lower() for term in ['sp500', 's&p 500', 'spx']):
                    sp500_fields.append(f"{key}: {value}")
                elif isinstance(value, list):
                    for item in value:
                        if isinstance(item, str) and any(term in item.lower() for term in ['sp500', 's&p 500', 'spx']):
                            sp500_fields.append(f"{key}: {item}")
            
            if sp500_fields:
                print(f"   Found S&P 500 references in: {sp500_fields}")
            else:
                print(f"   No S&P 500 references found in ticker details")
                
            # Print all available fields for investigation
            print(f"\n   📋 All available fields:")
            for key in sorted(details.keys()):
                if key not in ['name', 'market_cap', 'sic_description', 'primary_exchange', 'type', 'active']:
