...
try:
    ...
    logger.error("Make sure you're running from the project root and virtual environment is activated")
    print("\nTo run this script:")
...
class Layer1BackfillRunner:
    ...
    def check_disk_space(self, required_gb: int = 150) -> bool:
        ...
        if available_gb < required_gb:
            logger.error(f"Insufficient disk space. Required: {required_gb}GB, Available: {available_gb}GB")
            return False
        else:
    ...
    def run_backfill_stage(self, stage_name: str, stage_config: Dict) -> bool:
        ...
        # Build command
        cmd = [
            sys.executable, 'ai_trader.py', 'backfill',
            '--stage', stage_name,
            '--symbols', 'layer1',  # Always use Layer 1, not Layer 0!
        ...
        try:
            ...
            for line in process.stdout:
                ...
                if any(keyword in line for keyword in ['✅', '❌', 'ERROR', 'Downloaded', 'records', 'symbols']):
                    print(f"  {line}")
    ...
    def print_summary(self):
        ...
        if self.failed_stages:
            ...
            for stage in self.failed_stages:
                ...
                logger.info(f"   python ai_trader.py backfill --stage {stage} --symbols layer1 --days {days}")
...
async def main():
    ...
    enabled_stages = [(name, config) for name, config in runner.stages.items() 
                      if config.get('enabled', True)]
    
    total_days = 0
    for i, (stage_name, stage_config) in enumerate(sorted(enabled_stages, 
                                                          key=lambda x: x[1].get('priority', 999))):
        days = stage_config.get('lookback_days', 365)
        total_days += days
        logger.info(f"  {i+1}. {stage_name}: {days} days of {stage_config.get('description', 'data')}")
    ...
    if os.environ.get('SKIP_CONFIRM') != 'true':
        ...
        print(f"  - Storage: ~60-120GB estimated")
        print(f"  - Time: 4-8 hours (depends on API limits)")
        print(f"  - Note: Uses Layer 1 symbols (most liquid ~2,000), NOT Layer 0 (all ~10,000)")
