...
async def check_aa_liquidity():
    ...
    try:
        ...
        if result and result['avg_dollar_volume']:
            ...
            print(f'  Actual date range: {result["oldest_data"].date()} to {result["newest_data"].date()}')
            ...
            print(f'  ✓ Dollar volume: ${avg_dollar_vol:,.0f} >= ${min_dollar_volume:,} : {"PASS" if dollar_vol_pass else "FAIL"}')
            
            price_pass = min_price <= avg_price <= max_price
            print(f'  ✓ Price range: ${min_price} <= ${avg_price:.2f} <= ${max_price} : {"PASS" if price_pass else "FAIL"}')
            
            data_points_pass = result["data_points"] >= min_data_points
            print(f'  ✓ Data points: {result["data_points"]} >= {min_data_points} : {"PASS" if data_points_pass else "FAIL"}')
