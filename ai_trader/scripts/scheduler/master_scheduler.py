#!/usr/bin/env python3
"""
AI Trading System - Master Job Scheduler

Enterprise-grade job scheduler that reads job_definitions.yaml and manages
the execution of all scheduled trading operations with proper dependency
handling, error recovery, and logging integration.
...
class JobScheduler:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.config_path = config_path or os.path.join(
            os.path.dirname(__file__), "job_definitions.yaml"
        )
        ...
        # Setup logging
        log_dir = Path(self.global_config.get('log_directory', 'logs/scheduled_jobs'))
    ...
    def _check_dependencies(self, job_id: str) -> bool:
        ...
        for dep_job_id in dependencies:
            ...
            recent_executions = [
                ex for ex in self.job_history
                if ex.job_id == dep_job_id and 
                ex.start_time and
                ex.start_time.date() == datetime.now().date() and
    ...
    def _execute_job(self, job_id: str, job_config: Dict[str, Any]) -> JobExecution:
        ...
        try:
            ...
            timeout_minutes = job_config.get('timeout_minutes', 
                                           self.global_config.get('max_job_runtime_minutes', 120))
            ...
            python_path = self.global_config.get('python_path', 'python')
            ...
            if execution.pid:
                try:
                    ...
                    execution.resource_usage = {
                        ...
                        'runtime_seconds': (execution.end_time - execution.start_time).total_seconds()
                    }
    ...
    def _retry_job(self, job_id: str, job_config: Dict[str, Any], 
                   failed_execution: JobExecution) -> Optional[JobExecution]:
    ...
    def _send_job_alert(self, job_id: str, job_config: Dict[str, Any], 
                       execution: JobExecution):
        """Send alerts for job failures or timeouts."""
        alert_config = job_config.get('alerts', {})
        
        should_alert = (
            (execution.status == JobStatus.FAILED and alert_config.get('on_failure', False)) or
            (execution.status == JobStatus.TIMEOUT and alert_config.get('on_timeout', False))
        )
    ...
    def _report_scheduler_health(self):
        ...
        try:
            ...
            recent_failures = len([
                ex for ex in self.job_history[-100:]  # Last 100 executions
                if ex.status == JobStatus.FAILED and
                ex.start_time and
                (datetime.now(self.timezone) - ex.start_time) < timedelta(hours=1)
            ])
        ...
        if job_id:
            ...
            return {
                ...
                "recent_executions": len([
                    ex for ex in self.job_history
                    if ex.start_time and
                    (datetime.now(self.timezone) - ex.start_time) < timedelta(hours=24)
                ])
            }
