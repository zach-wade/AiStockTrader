...
from src.main.scanners.scanner_pipeline import ScannerPipeline, PipelineResult
...
Examples:
  ...
  python scripts/run_scanner_pipeline.py --no-save
    ...
    parser.add_argument(
        '--test', 
        action='store_true',
        help='Run in test mode with limited symbols'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        help='Specific symbols to scan (overrides Layer 0)'
    )
...
def load_custom_config(config_path: Optional[str]) -> Optional[DictConfig]:
...
def merge_configs(base_config: DictConfig, custom_config: Optional[DictConfig], 
...
async def save_results(result: PipelineResult, format_type: str, output_dir: Optional[str]):
