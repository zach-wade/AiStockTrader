...
from risk_management.integration.trading_engine_integration import (
    TradingEngineRiskIntegration, TradingMode, RiskSystemStatus
)

# Import trading components (mocked for demo)
from trading_engine.core.unified_position_manager import UnifiedPositionManager, OrderSide
...
async def demonstrate_risk_system():
    ...
    for mode in trading_modes:
        ...
        success = await risk_integration.initialize_system(
            ...
            base_order_manager=order_manager
        )
        ...
        print(f"🔧 Components Active: {status['components']['risk_monitor']}, "
        ...
        if risk_order_manager:
            ...
            for symbol, side, quantity, description in test_orders:
                ...
                if order_id:
                    ...
                    print(f"      🚫 Order blocked by risk system")
        ...
        if risk_order_manager:
            ...
            print(f"   Emergency Halt Active: {'⚠️ YES' if stats['emergency_halt_active'] else '✅ NO'}")
