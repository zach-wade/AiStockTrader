...
@click.group()
@click.option('--config', default=None, help='Path to configuration file (optional, uses modular config by default)')
@click.option('--log-level', default='INFO', type=click.Choice(['DEBUG', 'INFO', 'WARNING', 'ERROR']))
@click.option('--environment', default=None, help='Environment override (paper, live, backtest, training)')
@click.pass_context
def cli(ctx, config, log_level, environment):
    ...
    print(f"DEBUG: ctx={ctx}, config={config}, log_level={log_level}, environment={environment}", file=sys.stderr)
    ...
    try:
        ...
        if config:
            ...
            logger.warning(f"Custom config file '{config}' specified but using modular config system")
        ...
        logger.info(f"AI Trader initialized with modular config system (environment: {app_config.system.environment})")
...
@cli.command()
@click.option('--mode', type=click.Choice(['live', 'paper', 'backtest', 'dev']), 
...
@click.option('--enable-system-dashboard', is_flag=True, help='Enable system health dashboard (port 8052)')
...
@click.pass_context
def trade(ctx, mode, symbols, strategy, enable_ml, disable_monitoring, disable_streaming, 
          dashboard_port, websocket_port, enable_system_dashboard, trading_dashboard_only, system_dashboard_only):
    ...
    if strategy:
        ...
        # If no specific strategy is selected, use all enabled strategies
        if not hasattr(config.strategies, 'active') or not config.strategies.active:
    ...
    async def run_integrated_system():
        ...
        try:
            ...
            print(f"🔍 DEBUG: About to enter managed_app_context at {datetime.now()}", file=sys.stderr)
            
            print(f"🔍 DEBUG: Creating managed_app_context with components: ['database', 'dual_storage', 'data_sources', 'ingestion', 'processing']", file=sys.stderr)
            async with managed_app_context(
                ...
                if enable_monitoring:
                    logger.info(f"Initializing V2 monitoring dashboards...")
                ...
                logger.info(f"   ML Trading: {'Enabled' if config.get('ml_trading', {}).get('enabled', False) else 'Disabled'}")
                logger.info(f"   Streaming: {'Enabled' if enable_streaming else 'Disabled'}")
                logger.info(f"   Dashboard: {'http://localhost:' + str(dashboard_port) if enable_monitoring else 'Disabled'}")
                logger.info("="*60)
    ...
    try:
        ...
        print(f"🔍 DEBUG: About to call asyncio.run(run_integrated_system()) at {datetime.now()}", file=sys.stderr)
        asyncio.run(run_integrated_system())
        print(f"🔍 DEBUG: asyncio.run(run_integrated_system()) completed at {datetime.now()}", file=sys.stderr)
...
@cli.command()
@click.option('--stage', callback=validate_stage, 
              default='all', help='Data collection stage (use --list-stages to see available options)')
@click.option('--list-stages', is_flag=True, help='List available backfill stages')
@click.option('--symbols', help='Comma-separated symbols, "universe" for all, or "layer0/layer1/layer2/layer3" for scanner-qualified symbols')
@click.option('--days', type=int, default=1825, help='Number of days to backfill (default: 5 years)')
...
def backfill(ctx, stage, list_stages, symbols, days, force, test_mode, limit):
    ...
    try:
        ...
        if result['status'] == 'success':
            ...
            logger.info(f"Processed {result['symbols_processed']} symbols in {result['duration_seconds']:.2f}s")
        else:
...
def train(ctx, symbols, models, lookback_days, test_size):
    """Train ML models"""
    from main.models.training.training_orchestrator import ModelTrainingOrchestrator as TrainingOrchestrator
    
    config = ctx.obj['config']
    
    # Prepare training parameters
    symbol_list = symbols.split(',') if symbols else config.get('universe.symbols', ['AAPL', 'MSFT'])  # Default fallback
    ...
    try:
        logger.info(f"Starting model training: symbols={symbol_list}, models={model_types}, fast_mode={fast_mode}")
...
@click.option('--features', help='Comma-separated list of feature sets (technical, fundamental, sentiment, all)')
...
def features(ctx, symbols, features, start_date, end_date, output):
    ...
    feature_config = {
        ...
        'feature_sets': features.split(',') if features else ['technical', 'fundamental', 'sentiment'],
...
def events(ctx, symbols, events, duration, output):
    ...
    event_config = {
        'symbols': symbols.split(',') if symbols else config.get('universe.symbols', []),
        'event_types': events.split(',') if events else ['earnings', 'news', 'price_movement', 'volume_spike'],
...
@click.option('--model-type', type=click.Choice(['xgboost', 'lightgbm', 'random_forest']), 
...
@click.option('--compare', multiple=True, help='Compare multiple models (can be used multiple times)')
@click.option('--output', help='Save results to file (JSON format)')
@click.pass_context
def backtest(ctx, model_path, model_type, symbols, start_date, end_date, 
             initial_cash, list_models, compare, output):
    """Run backtest on saved ML models"""
    from main.app.run_backtest import BacktestRunner, find_latest_model, find_and_list_models
    from datetime import datetime
    import json
    ...
    try:
        if len(model_paths) > 1:
            ...
            results = asyncio.run(runner.run_multiple_backtests(
            ...
            results = asyncio.run(runner.run_model_backtest(
...
def _print_comparison_results(results: Dict):
    ...
    if summary:
        ...
        # Select key columns
        display_cols = ['model_type', 'total_return', 'sharpe_ratio', 
...
@click.option('--layer', type=click.Choice(['0', '1', '2', '3']), default='0', help='Layer to query')
...
def universe(ctx, populate, stats, health, layer, limit):
    ...
    try:
        if populate:
            ...
            if result['success']:
                logger.info(f"✅ Universe populated successfully")
            ...
            if not health_data['healthy']:
                logger.warning("⚠️ Consider running 'python ai_trader.py universe --populate' to populate the universe")
...
def scan(ctx, pipeline, layer, catalyst, symbols, dry_run, show_alerts):
    ...
    try:
        if pipeline:
            ...
            logger.info(f"Symbols qualified: {len(results) if isinstance(results, list) else 'N/A'}")
            ...
            if not scanner:
                ...
                logger.info("Available scanners: volume, technical, news, earnings, options, insider, sector, social, intermarket")
                sys.exit(1)
            ...
            if show_alerts and alerts:
                ...
                for alert in alerts[:10]:  # Show first 10
                    logger.info(f"  {alert.symbol}: {alert.alert_type.value} (confidence: {alert.confidence:.2f})")
                if len(alerts) > 10:
...
@click.option('--layer', type=click.Choice(['0', '1', '2', '3', 'all']), default='all', 
