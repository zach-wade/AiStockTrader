    ...
    def verify_transition_recorded(self, symbol: str, from_layer: int, to_layer: int) -> bool:
        ...
        for transition in self.layer_transitions:
            if (transition['symbol'] == symbol and 
                transition['from_layer'] == from_layer and 
                transition['to_layer'] == to_layer):
                return True
        return False
...
async def test_full_e2e_flow():
    ...
    result = await repo.update_layer(
        symbol=symbol,
        layer=1,
        reason="Scanner qualification: High volume"
    )
    assert result.success
