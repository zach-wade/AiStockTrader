...
class AITraderConfig(BaseModel):
    ...
    database: Optional[Dict[str, Any]] = Field(default=None, description="Database configuration")
    ...
    universe: UniverseMainConfig = Field(default_factory=UniverseMainConfig, description="Universe configuration")
    paths: PathsConfig = Field(default_factory=PathsConfig, description="Paths configuration")
    orchestrator: OrchestratorConfig = Field(default_factory=OrchestratorConfig, description="Orchestrator configuration")
    
    # Environment-specific overrides
    environments: Optional[Dict[str, EnvironmentOverrides]] = Field(default=None, description="Environment overrides")
    ...
    def validate_risk_consistency(self):
        ...
        if trading_max > risk_max_dollars:
            logger.warning(f"Trading max position size (${trading_max}) exceeds risk limit (${risk_max_dollars})")
