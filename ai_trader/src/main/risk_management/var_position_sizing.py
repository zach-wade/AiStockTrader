...
class VaRPositionSizer:
    ...
    async def calculate_position_size(self,
                                    ...
                                    target_var_contribution: Optional[float] = None) -> PositionSizeRecommendation:
        """
        ...
        if method == PositionSizingMethod.MAX_VAR:
            ...
            size_recommendation = await self._size_by_equal_var(
                symbol, current_price, asset_var_pct, portfolio_value,
                current_portfolio, target_var_contribution
            )
        ...
        return PositionSizeRecommendation(
            ...
            warnings=size_recommendation.get('warnings', []) + portfolio_var_check.get('warnings', [])
        )
    ...
    async def _calculate_historical_var(self, symbol: str) -> float:
        ...
        returns = await self.risk_calculator.get_historical_returns(
            symbol, 
            lookback_days=self.constraints.lookback_days
        )
    ...
    async def _size_by_max_var(self,
                              symbol: str,
                              current_price: float,
                              asset_var_pct: float,
    ...
    async def _size_by_equal_var(self,
                                symbol: str,
                                current_price: float,
                                asset_var_pct: float,
                                portfolio_value: float,
                                current_portfolio: List[Position],
                                target_var_contribution: Optional[float]) -> Dict[str, Any]:
    ...
    async def _size_by_risk_parity(self,
                                  symbol: str,
                                  current_price: float,
                                  asset_var_pct: float,
                                  portfolio_value: float,
                                  current_portfolio: List[Position]) -> Dict[str, Any]:
    ...
    async def _size_by_kelly_var(self,
                                symbol: str,
                                current_price: float,
                                asset_var_pct: float,
                                portfolio_value: float) -> Dict[str, Any]:
        ...
        max_var_position_value = (self.constraints.max_position_var * portfolio_value) / asset_var_pct
    ...
    async def _check_portfolio_var_impact(self,
                                        symbol: str,
                                        shares: int,
                                        current_portfolio: List[Position],
                                        portfolio_value: float) -> Dict[str, Any]:
        ...
        if not constraints_met:
            warnings.append(f"Portfolio VaR ({portfolio_var_pct:.2%}) exceeds limit ({self.constraints.max_portfolio_var:.2%})")
    ...
    async def calculate_var_efficient_portfolio(self,
                                              candidate_symbols: List[str],
                                              portfolio_value: float,
                                              target_var: float) -> List[PositionSizeRecommendation]:
        """
        ...
        for i, symbol in enumerate(symbols):
            if best_weights[i] > 0.01:  # At least 1% allocation
                ...
                if shares > 0:
                    ...
                    recommendations.append(PositionSizeRecommendation(
                        symbol=symbol,
    ...
    def adjust_for_correlation(self,
                             ...
                             symbol: str,
                             current_portfolio: List[Position],
                             correlation_threshold: float = 0.7) -> int:
        """
