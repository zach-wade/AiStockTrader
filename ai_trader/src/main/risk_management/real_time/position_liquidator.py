...
class MarketImpactModel:
    ...
    def estimate_impact(self, 
        ...
        total_impact = (permanent_impact + temporary_impact + spread_impact) * participation_adjustment
...
class PositionLiquidator:
    ...
    def __init__(self, 
        ...
        # Risk thresholds
        self.max_portfolio_liquidation_pct = config.get('max_portfolio_liquidation_pct', 0.50)  # 50%
        self.emergency_liquidation_threshold = config.get('emergency_liquidation_threshold', 0.15)  # 15%
    ...
    async def create_liquidation_plan(self,
                                    positions: Dict[str, Any],
                                    liquidation_percentage: float,
                                    urgency: LiquidationUrgency = LiquidationUrgency.MEDIUM,
                                    target_symbols: Optional[List[str]] = None) -> LiquidationPlan:
        """
        ...
        logger.info(f"Creating liquidation plan {plan_id}: {liquidation_percentage:.1%} of portfolio (${total_liquidation_value:,.2f})")
        ...
        logger.info(f"Liquidation plan created: {len(targets)} positions, estimated duration {estimated_duration} minutes")
    ...
    async def execute_liquidation_plan(self, plan: LiquidationPlan) -> bool:
        ...
        logger.critical(f"🔴 EXECUTING LIQUIDATION PLAN {plan.plan_id}: {plan.liquidation_percentage:.1%} of portfolio")
    ...
    async def emergency_liquidate_all(self, 
                                    positions: Dict[str, Any],
                                    reason: str = "Emergency liquidation") -> LiquidationPlan:
        """
    ...
    async def partial_liquidate_position(self,
                                       symbol: str,
                                       reduction_percentage: float,
                                       urgency: LiquidationUrgency = LiquidationUrgency.MEDIUM) -> bool:
        """
    ...
    async def _execute_liquidation_plan_async(self, plan: LiquidationPlan):
        ...
        try:
            ...
            plan.completion_time = datetime.now(timezone.utc)
            plan.actual_duration_minutes = (plan.completion_time - plan.start_time).total_seconds() / 60
            ...
            if plan.rebalancing_required and plan.status in [LiquidationStatus.COMPLETED, LiquidationStatus.PARTIAL]:
                await self._perform_post_liquidation_rebalancing(plan)
            
            logger.info(f"Liquidation plan {plan.plan_id} completed with status {plan.status.value}")
    ...
    async def _execute_single_target(self, target: LiquidationTarget):
        ...
        logger.info(f"Executing liquidation: {target.symbol} {target.liquidation_quantity} shares ({target.strategy.value})")
        ...
        try:
            ...
            if target.executed_quantity >= target.liquidation_quantity * Decimal('0.95'):  # 95% threshold
    ...
    async def _execute_immediate_liquidation(self, target: LiquidationTarget):
        ...
        order = await self.order_manager.submit_market_order(
            ...
            side=order_side,
            quantity=abs(float(target.liquidation_quantity))
        )
    ...
    async def _execute_twap_liquidation(self, target: LiquidationTarget):
        ...
        logger.info(f"TWAP liquidation: {target.symbol} in {num_slices} slices over {target.time_horizon_minutes} minutes")
    ...
    async def _select_liquidation_targets(self,
                                        positions: Dict[str, Any],
                                        liquidation_percentage: float,
                                        urgency: LiquidationUrgency,
                                        target_symbols: Optional[List[str]]) -> Dict[str, Dict]:
    ...
    async def _calculate_liquidation_priority_score(self, position, portfolio_value: Decimal) -> float:
    ...
    async def _calculate_position_risk_metrics(self, symbol: str, position, portfolio_value: Decimal) -> Dict[str, float]:
    ...
    def _prioritize_liquidation_targets(self, targets: List[LiquidationTarget]) -> List[LiquidationTarget]:
    ...
    def _calculate_execution_quality(self, plan: LiquidationPlan) -> float:
        ...
        for target in plan.targets:
            ...
            completion_rate = float(target.executed_quantity / target.liquidation_quantity) if target.liquidation_quantity > 0 else 0
            
            # Market impact penalty
            impact_penalty = min(50, target.market_impact_bps / 10)  # 10 bps = 5 point penalty
            
            # Speed bonus/penalty
            time_ratio = plan.actual_duration_minutes / plan.estimated_duration_minutes if plan.estimated_duration_minutes > 0 else 1
    ...
    async def get_liquidation_status(self, plan_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'progress_percentage': float(plan.executed_value / plan.total_liquidation_value * 100) if plan.total_liquidation_value > 0 else 0,
            'start_time': plan.start_time.isoformat() if plan.start_time else None,
            'estimated_completion': (plan.start_time + timedelta(minutes=plan.estimated_duration_minutes)).isoformat() if plan.start_time else None,
            'targets_completed': len([t for t in plan.targets if t.status == LiquidationStatus.COMPLETED]),
