...
class PositionLimitBreaker(BaseBreaker):
    ...
    def __init__(self, breaker_type: BreakerType, config: BreakerConfig):
        ...
        self.max_sector_concentration = config.config.get('max_sector_concentration', 0.30)  # 30% per sector
        ...
        logger.info(f"Position limit breaker initialized - max positions: {self.max_positions}, max size: {self.max_position_size:.2%}")
    ...
    async def check(self, 
        ...
        if len(positions) > self.max_positions:
            self.logger.error(f"Position count limit exceeded: {len(positions)} > {self.max_positions}")
            return True
    ...
    async def _check_position_concentration(self, positions: Dict[str, Any], portfolio_value: float) -> bool:
        ...
        for symbol, pos in positions.items():
            ...
            if position_pct > self.max_position_size:
                ...
                self.logger.error(f"Position concentration limit exceeded for {symbol}: {position_pct:.2%} > {self.max_position_size:.2%}")
                return True
        
        return False
    
    async def _check_sector_concentration(self, positions: Dict[str, Any], portfolio_value: float) -> bool:
        ...
        for sector, exposure in sector_exposure.items():
            ...
            if sector_pct > self.max_sector_concentration:
                self.logger.error(f"Sector concentration limit exceeded for {sector}: {sector_pct:.2%} > {self.max_sector_concentration:.2%}")
                return True
        
        return False
    
    async def _check_exposure_limits(self, positions: Dict[str, Any], portfolio_value: float) -> bool:
        ...
        if long_exposure_pct > self.max_long_exposure:
            self.logger.error(f"Long exposure limit exceeded: {long_exposure_pct:.2%} > {self.max_long_exposure:.2%}")
            return True
        
        # Check short exposure limit
        if short_exposure_pct > self.max_short_exposure:
            self.logger.error(f"Short exposure limit exceeded: {short_exposure_pct:.2%} > {self.max_short_exposure:.2%}")
            return True
        
        return False
    
    def _calculate_max_concentration(self, positions: Dict[str, Any], portfolio_value: float) -> float:
    ...
    def get_metrics(self) -> BreakerMetrics:
        ...
        if self.position_history:
            ...
            if latest['positions']:
                total_value = sum(abs(pos.get('market_value', 0)) for pos in latest['positions'].values())
                if total_value > 0:
                    max_pos_value = max(abs(pos.get('market_value', 0)) for pos in latest['positions'].values())
    ...
    def get_position_statistics(self) -> Dict[str, Any]:
        ...
        if positions:
            ...
            concentrations = []
            if total_value > 0:
                concentrations = [abs(pos.get('market_value', 0)) / total_value for pos in positions.values()]
        else:
            concentrations = []
        ...
        # Exposure analysis
        long_exposure = sum(pos.get('market_value', 0) for pos in positions.values() if pos.get('market_value', 0) > 0)
        short_exposure = sum(abs(pos.get('market_value', 0)) for pos in positions.values() if pos.get('market_value', 0) < 0)
    ...
    def get_risk_contribution_analysis(self) -> Dict[str, Any]:
        ...
        sorted_risks = sorted(risk_contributions.items(), key=lambda x: x[1]['risk_contribution'], reverse=True)
            ...
            'top_risk_contributors': [
                {
                    ...
                    'risk_percentage': rc['risk_contribution'] / total_risk * 100 if total_risk > 0 else 0,
                    'weight': rc['weight'],
                    'volatility': rc['volatility']
                }
                for symbol, rc in top_contributors
            ],
            'risk_concentration': max(rc['risk_contribution'] for _, rc in risk_contributions.items()) / total_risk if total_risk > 0 else 0
        }
