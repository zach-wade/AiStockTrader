...
class LossRateBreaker(BaseBreaker):
    ...
    def __init__(self, breaker_type: BreakerType, config: BreakerConfig):
        ...
        logger.info(f"Loss rate breaker initialized - threshold: {self.loss_rate_threshold:.2%} in {self.loss_rate_window}")
    ...
    async def check(self, 
        ...
        self.loss_history = deque(
            (t, v) for t, v in self.loss_history if t > cutoff_time
        )
        
        # Check rapid loss rate
        current_loss_rate = self._calculate_loss_rate()
        if current_loss_rate > self.loss_rate_threshold:
            self.logger.error(f"Rapid loss rate detected: {current_loss_rate:.2%} > {self.loss_rate_threshold:.2%}")
            return True
    ...
    async def _check_loss_acceleration(self) -> bool:
        ...
        recent_loss_rate = (recent_values[0] - recent_values[-1]) / recent_values[0] if recent_values[0] > 0 else 0
        
        # Earlier period (first 1/3 of window)
        earlier_values = values[:recent_period]
        earlier_loss_rate = (earlier_values[0] - earlier_values[-1]) / earlier_values[0] if earlier_values[0] > 0 else 0
        
        # Check if loss rate is accelerating
        if recent_loss_rate > earlier_loss_rate * 1.5:  # 50% acceleration
            self.logger.warning(f"Loss rate accelerating: {recent_loss_rate:.2%} vs {earlier_loss_rate:.2%}")
            return True
    ...
    def get_loss_pattern_analysis(self) -> Dict[str, Any]:
        ...
        # Time-based analysis
        recent_events = [le for le in loss_events if (datetime.now() - le['timestamp']).total_seconds() < 3600]  # Last hour
