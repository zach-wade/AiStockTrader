...
class DrawdownController:
    ...
    def check_drawdown(self) -> DrawdownState:
        ...
        if hasattr(self, '_last_level') and self._last_level != self.current_state.level:
            logger.warning(f"Drawdown level changed: {self._last_level.value} -> "
    ...
    def _handle_drawdown_breach(self):
        ...
        logger.warning(f"Drawdown breach: {self.current_state.current_drawdown:.1%} "
                      f"(Level: {level.value})")
    ...
    def get_risk_metrics(self) -> Dict:
        ...
        recovery_factor = (total_return / self.current_state.max_drawdown 
                          if self.current_state.max_drawdown > 0 else 0)
        ...
        return {
            ...
            'drawdown_frequency': np.sum(drawdowns > 0.05) / len(drawdowns),  # % time in 5%+ drawdown
