...
class ExposureLimitsChecker(ErrorHandlingMixin):
    ...
    async def _get_factor_loadings(self, symbol: str) -> Dict[str, float]:
        ...
        if loadings is None:
            ...
            np.random.seed(hash(symbol) % 1000)  # Consistent randomness per symbol (non-security-critical)
            loadings = {
    ...
    def _check_sector_limits(self, exposures: Dict) -> List[ExposureCheck]:
        ...
        for sector, exposure in exposures['sectors'].items():
            if sector in self.sector_limits:
                ...
                adjusted_limit = limit.max_exposure * self.market_stress_multiplier * self.regime_adjustment
    ...
    async def _check_concentration_limits(self, order: Order, exposures: Dict) -> List[ExposureCheck]:
        """Check position concentration limits."""
        checks = []
        
        # Single position limit
        max_position_size = self.config.get('risk_management.exposure_limits.max_position_size', 0.05)
        ...
        sorted_positions = sorted(exposures['concentration'].items(), key=lambda x: x[1], reverse=True)
    ...
    def _map_country_to_region(self, country: str) -> str:
        ...
        region_mapping = {
            ...
            'JP': 'Japan',
            'CN': 'Asia', 'HK': 'Asia', 'SG': 'Asia', 'KR': 'Asia', 'TW': 'Asia',
            'BR': 'Emerging Markets', 'IN': 'Emerging Markets', 'MX': 'Emerging Markets',
            'AU': 'Asia', 'NZ': 'Asia'
        }
