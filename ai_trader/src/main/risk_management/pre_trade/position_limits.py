...
class PositionLimitChecker:
    ...
    async def check_pre_trade_limits(self,
                                   symbol: str,
                                   quantity: int,
                                   price: float,
                                   side: str,
                                   portfolio_value: float,
                                   current_positions: Dict[str, Any]) -> Tuple[bool, List[PositionLimit]]:
        ...
        async with self._lock:
            ...
            correlation_limit = await self._check_correlation_exposure(
            ...
            # 6. Liquidity check
            liquidity_limit = await self._check_liquidity(
                symbol, quantity
            )
            ...
            for limit in limits_checked:
                if limit.utilization > 80:
                    logger.warning(f"Limit approaching: {limit.limit_type.value} at {limit.utilization:.1f}%")
            
            return all_passed, limits_checked
    
    async def _check_position_size(self, symbol: str, quantity: int, 
                                 price: float, portfolio_value: float) -> PositionLimit:
    ...
    async def _check_sector_exposure(self, symbol: str, quantity: int, price: float,
                                   portfolio_value: float, 
                                   current_positions: Dict[str, Any]) -> PositionLimit:
    ...
    async def _check_correlation_exposure(self, symbol: str,
                                        current_positions: Dict[str, Any]) -> PositionLimit:
    ...
    async def _check_beta_exposure(self, symbol: str, quantity: int, price: float,
                                 portfolio_value: float,
                                 current_positions: Dict[str, Any]) -> PositionLimit:
    ...
    async def _check_concentration(self, symbol: str, quantity: int, price: float,
                                 portfolio_value: float,
                                 current_positions: Dict[str, Any]) -> PositionLimit:
    ...
    async def _check_volatility_adjusted_size(self, symbol: str, quantity: int,
                                            price: float, portfolio_value: float) -> PositionLimit:
    ...
    def adjust_position_size(self, proposed_quantity: int, limits: List[PositionLimit]) -> int:
        ...
        if adjustment_factors:
            ...
            adjusted_quantity = int(proposed_quantity * min_factor * 0.95)  # 95% to ensure compliance
    ...
    async def get_limit_summary(self, portfolio_value: float,
        ...
        for symbol, position in current_positions.items():
            sector = self.sector_map.get(symbol, 'Other')
            sector_exposures[sector] = sector_exposures.get(sector, 0) + position.market_value / portfolio_value
