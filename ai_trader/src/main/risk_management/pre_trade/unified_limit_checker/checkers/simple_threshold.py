...
class SimpleThresholdChecker(LimitChecker):
    ...
    def check_limit(self, limit: LimitDefinition, 
        ...
        # Perform comparison
        passed = self._evaluate_condition(current_value, limit.operator, threshold, limit.upper_threshold)
        ...
        if not passed:
            ...
            if limit.soft_threshold is not None:
                soft_passed = self._evaluate_condition(current_value, limit.operator, limit.soft_threshold)
                if not soft_passed:
    ...
    def _evaluate_condition(self, value: float, operator: ComparisonOperator, 
                          threshold: float, upper_threshold: Optional[float] = None) -> bool:
    ...
    def _generate_message(self, limit: LimitDefinition, current_value: float, 
                         threshold: float, passed: bool) -> str:
        ...
        if limit.operator in [ComparisonOperator.BETWEEN, ComparisonOperator.OUTSIDE]:
            return (f"Limit '{limit.name}' {status}: {current_value:.4f} {op_str} "
