...
class LimitTemplates:
    ...
    def create_position_size_limit(
        ...
        return LimitDefinition(
            ...
            description=f"Maximum position size as percentage of portfolio",
    ...
    def create_volatility_limit(
        ...
        return LimitDefinition(
            ...
            description=f"Maximum annualized volatility",
...
def create_comprehensive_portfolio_limits() -> List[LimitDefinition]:
    ...
    limits.extend([
        ...
        LimitTemplates.create_position_size_limit("pos_size_10_warn", 10.0, 
    ...
    limits.extend([
        ...
        LimitTemplates.create_concentration_limit("currency_conc_40", 40.0,
                                                 scope=LimitScope.CURRENCY,
                                                 group_by='currency'),
        LimitTemplates.create_concentration_limit("country_conc_30", 30.0,
                                                 scope=LimitScope.GEOGRAPHIC,
                                                 group_by='country')
    ])
...
def validate_limit_definition(limit: LimitDefinition) -> List[str]:
    ...
    if limit.soft_threshold is not None:
        if limit.operator in [ComparisonOperator.LESS_THAN, ComparisonOperator.LESS_EQUAL]:
            ...
            if limit.soft_threshold <= limit.threshold_value:
                errors.append("soft_threshold must be greater than threshold_value for GREATER operators")
...
def format_violation_summary(violation: LimitViolation) -> str:
    ...
    parts = [
        ...
        f"Breach: {violation.breach_magnitude:.1%}" if violation.breach_magnitude else "Breach: N/A"
    ]
...
def get_violations_by_limit_type(
    violations: List[LimitViolation],
    limit_types: Dict[str, LimitType]
...
def calculate_violation_duration_stats(
    ...
    durations = [
        v.duration_seconds for v in violations
        if v.resolved and v.duration_seconds is not None
    ]
...
async def setup_basic_portfolio_limits(
