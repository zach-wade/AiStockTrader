...
from main.risk_management.real_time.live_risk_monitor import (
    LiveRiskMonitor, PortfolioRiskSnapshot, RiskDecision, RiskDecisionType
)
from main.risk_management.types import RiskAlert, RiskAlertLevel
from main.risk_management.real_time.circuit_breaker import BreakerEvent, BreakerStatus
...
class AlertConfiguration:
    ...
    # Alert channels
    enabled_channels: List[AlertChannel] = field(default_factory=lambda: [AlertChannel.LOG, AlertChannel.TERMINAL])
...
"""
        ...
        if self.current_value is not None and self.threshold_value is not None:
            self.formatted_message += f"Current: {self.current_value:.2f} | Threshold: {self.threshold_value:.2f}\n"
...
"""
        ...
        if self.current_value is not None and self.threshold_value is not None:
            self.html_message += f"            <strong>Current Value:</strong> {self.current_value:.2f}<br>\n"
            self.html_message += f"            <strong>Threshold:</strong> {self.threshold_value:.2f}<br>\n"
...
class LiveRiskDashboard:
    ...
    def __init__(self,
        ...
        self.hour_reset_time = datetime.now(timezone.utc).replace(minute=0, second=0, microsecond=0)
    ...
    def _cleanup_old_alerts(self):
        ...
        cutoff_time = datetime.now(timezone.utc) - timedelta(days=self.alert_config.alert_history_days)
    ...
    def get_current_dashboard_data(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'alert_summary': {
                ...
                'critical': len([a for a in self.active_alerts.values() if a.severity == RiskAlertLevel.CRITICAL]),
                'high': len([a for a in self.active_alerts.values() if a.severity == RiskAlertLevel.HIGH]),
                'medium': len([a for a in self.active_alerts.values() if a.severity == RiskAlertLevel.MEDIUM])
            },
            'system_status': {
    ...
    def get_dashboard_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'alerts': {
                ...
                'alert_frequency_limit_minutes': self.alert_config.min_alert_interval.total_seconds() / 60,
