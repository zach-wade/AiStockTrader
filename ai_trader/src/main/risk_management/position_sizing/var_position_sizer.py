...
from main.trading_engine.core.position_manager import (
    PositionManager as UnifiedPositionManager
)
from main.models.common import Position, OrderSide
...
class PositionSizeRecommendation:
    ...
    conservative_size: Optional[Decimal] = None
    aggressive_size: Optional[Decimal] = None
...
class VaRPositionSizer:
    ...
    def __init__(self,
        ...
        self.current_risk_budget: Optional[PortfolioRiskBudget] = None
    ...
    async def calculate_position_size(self,
                                    symbol: str,
                                    side: OrderSide,
                                    target_price: Union[float, Decimal],
                                    strategy_signal_strength: float = 1.0) -> PositionSizeRecommendation:
        """
        ...
        try:
            ...
            correlation_data = await self._calculate_correlation_adjustments(symbol, current_positions)
            ...
            recommendation = PositionSizeRecommendation(
                ...
                portfolio_weight=float((final_size * target_price) / portfolio_value) 
            ...
            return PositionSizeRecommendation(
                ...
                constraint_violations=[f"Calculation error: {str(e)}"],
                confidence_score=0.0
            )
    
    async def _update_risk_budget(self, 
        ...
        self.current_risk_budget = PortfolioRiskBudget(
            ...
            budget_utilization_pct=float(total_allocated_var / total_var_budget * 100) 
                                  if total_var_budget > 0 else 0,
    ...
    def _calculate_asset_volatility(self, returns: np.ndarray) -> float:
        ...
        vol = max(self.config.volatility_floor, 
                 min(self.config.volatility_cap, annualized_vol))
        
        return vol
    
    async def _calculate_correlation_adjustments(self,
                                               symbol: str,
                                               current_positions: Dict[str, Position]) -> Dict[str, Any]:
        ...
        if correlations:
            ...
            if max_correlation > self.config.correlation_threshold:
                adjustment_factor = 1.0 - (max_correlation - self.config.correlation_threshold) * 0.5
    ...
    async def _calculate_base_position_size(self,
                                          ...
                                          portfolio_value: Decimal,
                                          asset_volatility: float) -> Decimal:
        ...
        z_score = stats.norm.ppf(self.config.var_confidence_levels[0])  # Use primary confidence level
    ...
    def _apply_sizing_adjustments(self,
                                base_size: Decimal,
                                asset_volatility: float,
                                correlation_data: Dict[str, Any],
                                signal_strength: float) -> Decimal:
    ...
    def _validate_position_constraints(self,
                                     ...
                                     price: Decimal,
                                     portfolio_value: Decimal,
                                     symbol: str) -> Tuple[Decimal, List[str], List[str]]:
        ...
        if position_value < self.config.min_position_size:
            if self.config.min_position_size / price <= size * Decimal('2'):
                ...
                warnings.append(f"Position size increased to minimum: ${self.config.min_position_size}")
            else:
        ...
        if portfolio_weight < self.config.min_portfolio_weight and final_size > 0:
            ...
            if min_value <= self.config.max_position_size:
                ...
                warnings.append(f"Position size increased to minimum weight: {self.config.min_portfolio_weight:.1%}")
    ...
    async def _calculate_position_risk_metrics(self,
                                             symbol: str,
                                             quantity: Decimal,
                                             price: Decimal,
    ...
    def _generate_alternative_sizes(self,
                                  base_size: Decimal,
                                  risk_metrics: Dict[str, Any],
                                  portfolio_value: Decimal) -> Tuple[Optional[Decimal], Optional[Decimal]]:
        ...
        if aggressive_size and risk_metrics.get('var_95', Decimal('0')) > max_weight_value * Decimal('0.02'):
    ...
    def get_current_risk_budget(self) -> Optional[PortfolioRiskBudget]:
        ...
        for symbol, position in current_positions.items():
            if not position.is_flat:
                try:
                    ...
                    size_change_pct = float(abs(recommended_size - current_size) / current_size) if current_size > 0 else 0
