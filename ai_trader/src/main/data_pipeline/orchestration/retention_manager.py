...
from main.utils.core import get_logger, ensure_utc, get_trading_days_between, AsyncCircuitBreaker, process_in_batches
...
from main.data_pipeline.core.enums import DataLayer, DataType, RetentionPolicy
from main.data_pipeline.core.exceptions import StorageError, convert_exception
...
class RetentionManager(IRetentionManager):
    ...
    async def get_retention_policy(self, layer: DataLayer, data_type: DataType) -> Dict[str, Any]:
        ...
        try:
            ...
            base_policy = {
                ...
                'hot_storage_days': retention_config.get('hot_storage_days', layer.hot_storage_days),
                'total_retention_days': retention_config.get('total_retention_days', layer.retention_days),
                'cold_storage_days': retention_config.get('total_retention_days', layer.retention_days) - retention_config.get('hot_storage_days', layer.hot_storage_days),
                'retention_policy': retention_config.get('retention_policy', 'hot_cold'),
                'archive_after_days': retention_config.get('total_retention_days', layer.retention_days),
            ...
            if data_type == DataType.NEWS:
                ...
                base_policy['total_retention_days'] = max(layer.retention_days, 90)  # Min 3 months total
                # Calculate actual trading days
                end_date = ensure_utc(datetime.now(timezone.utc))
                start_date = end_date - timedelta(days=base_policy['total_retention_days'])
                base_policy['trading_days_retained'] = len(get_trading_days_between(start_date, end_date))
                ...
                base_policy['total_retention_days'] = min(layer.retention_days, 30)  # Max 30 days total
            ...
            base_policy.update({
                ...
                'created_at': ensure_utc(datetime.now(timezone.utc)).isoformat()
            })
        ...
        try:
            ...
            for data_type in DataType:
                if await self._has_data_for_type(symbol, data_type):
                    type_results = await self._apply_retention_for_data_type(symbol, layer, data_type)
            ...
            results['end_time'] = ensure_utc(datetime.now(timezone.utc))
    ...
    async def cleanup_expired_data(self, layer: DataLayer) -> Dict[str, Any]:
        ...
        try:
            ...
            async def process_symbol_batch(batch):
                ...
                for symbol in batch:
                    for data_type in DataType:
                        ...
                        cutoff_date = ensure_utc(datetime.now(timezone.utc)) - timedelta(days=cleanup_days)
                        deleted_count = await self._cleanup_expired_records(symbol, data_type, cutoff_date)
            ...
            cleanup_results['end_time'] = ensure_utc(datetime.now(timezone.utc))
    ...
    async def move_to_cold_storage(self, symbol: str, layer: DataLayer) -> bool:
        ...
        try:
            cutoff_date = ensure_utc(datetime.now(timezone.utc)) - timedelta(days=layer.hot_storage_days)
            ...
            """.format(table_name=f"{self.hot_storage_table_prefix}_1h")
    ...
    async def archive_old_data(self, symbol: str, layer: DataLayer) -> bool:
        ...
        async def _archive_with_breaker():
            archive_cutoff = ensure_utc(datetime.now(timezone.utc)) - timedelta(days=layer.retention_days)
    ...
    def _get_cleanup_days(self, layer: DataLayer, data_type: DataType) -> Optional[int]:
