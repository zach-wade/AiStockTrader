...
class LayerManager(ILayerManager):
    ...
    async def get_layer_config(self, layer: DataLayer) -> Dict[str, Any]:
        ...
        try:
            with timer(f"get_layer_config({layer.name})"):
                ...
                base_config = self.config_manager.load_config('layer_configs').get(f'layer_{layer}', {})
    ...
    async def get_symbols_for_layer(self, layer: DataLayer) -> List[str]:
        ...
        try:
            ...
            self._cache_expiry[cache_key] = ensure_utc(datetime.now(timezone.utc))
    ...
    async def promote_symbol(
        ...
        try:
            ...
            history_entry = f"{from_layer.value}→{to_layer.value}@{ensure_utc(datetime.now(timezone.utc)).isoformat()}"
            ...
            self.logger.info(f"Promoted symbol {symbol} from layer {from_layer.value} to {to_layer.value}")
            return True
    ...
    async def demote_symbol(
        ...
        try:
            ...
            self.logger.info(f"Demoted symbol {symbol} from layer {from_layer.value} to {to_layer.value}")
            return True
            ...
            with timer("get_layer_statistics"):
                ...
                for layer in DataLayer:
                    ...
                    stats[layer.name.lower()] = {
                        ...
                        'max_symbols': layer.max_symbols,
                        'capacity_used_percent': safe_divide(len(symbols) * 100, layer.max_symbols, default=0.0),
                        'retention_days': layer.retention_days,
                        'hot_storage_days': layer.hot_storage_days,
                        'supported_intervals': layer.supported_intervals,
                ...
                return {
                    ...
                    'total_symbols': sum(len(await self.get_symbols_for_layer(layer)) for layer in DataLayer),
