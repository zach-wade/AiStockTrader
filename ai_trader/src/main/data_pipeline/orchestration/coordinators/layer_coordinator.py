...
class LayerCoordinator:
    ...
    async def get_layer_symbols(
        self,
        max_per_layer: Optional[Dict[DataLayer, int]] = None
    ) -> LayerSymbols:
        """
        ...
        basic = await self._get_symbols_for_layer(DataLayer.BASIC, max_per_layer.get(DataLayer.BASIC, 10000))
        liquid = await self._get_symbols_for_layer(DataLayer.LIQUID, max_per_layer.get(DataLayer.LIQUID, 2000))
        catalyst = await self._get_symbols_for_layer(DataLayer.CATALYST, max_per_layer.get(DataLayer.CATALYST, 500))
        active = await self._get_symbols_for_layer(DataLayer.ACTIVE, max_per_layer.get(DataLayer.ACTIVE, 50))
    ...
    async def check_symbol_qualification(self, symbol: str) -> Optional[int]:
        """
