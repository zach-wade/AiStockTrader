...
from main.utils.core import get_logger, ensure_utc, RateLimiter, AsyncCircuitBreaker, async_retry, gather_with_exceptions
from main.interfaces.events import IEventBus, EventType
from main.interfaces.events.event_types import (
    SymbolQualifiedEvent, SymbolPromotedEvent, DataGapDetectedEvent,
    BackfillScheduledEvent
)
from main.interfaces.data_pipeline.orchestration import IEventCoordinator, ILayerManager, IRetentionManager
...
class EventCoordinator(IEventCoordinator):
    ...
    def __init__(
        ...
        try:
            rate_limit_config = self.config_manager.get_config().get('rate_limits', {}).get('events', {})
        except (AttributeError, KeyError, TypeError) as e:
        ...
        self.backfill_rate_limiter = RateLimiter(
            rate=rate_limit_config.get('backfill_rate_per_minute', 10),
            per=60  # per minute
        )
    ...
    async def get_event_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'generated_at': ensure_utc(datetime.now(timezone.utc)).isoformat()
        }
    ...
    async def _get_symbol_layer(self, symbol: str) -> Optional[DataLayer]:
    ...
    async def _schedule_enhancement_backfill(
        ...
        backfill_id = f"enhancement_{symbol}_{layer.value}_{int(ensure_utc(datetime.now(timezone.utc)).timestamp())}"
        ...
        layer_config = get_layer_config(layer)
        retention_days = layer_config.get('retention', {}).get('total_retention_days', layer.retention_days)
        ...
        backfill_event = {
            ...
            'scheduled_at': ensure_utc(datetime.now(timezone.utc)).isoformat()
        }
    ...
    async def _schedule_gap_fill_backfill(
        ...
        backfill_id = f"gap_fill_{symbol}_{data_type.value}_{int(ensure_utc(datetime.now(timezone.utc)).timestamp())}"
    ...
    async def _publish_promotion_completed_event(
        ...
        event_data = {
            ...
            'from_layer': from_layer.value,
            'to_layer': to_layer.value,
            'completed_at': ensure_utc(datetime.now(timezone.utc)).isoformat()
        }
