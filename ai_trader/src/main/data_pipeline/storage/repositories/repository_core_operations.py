...
class RepositoryCoreOperations:
    ...
    async def get_by_id(self, record_id: Any) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            duration = time.time() - start_time
            await self._record_operation_metric_if_enabled(
                'get_by_id', duration, success=False
            )
            raise
    ...
    async def update(self, record_id: Any, data: Dict[str, Any]) -> OperationResult:
        ...
        try:
            ...
            # Execute update
            affected = await self.db_adapter.execute_query(
                query_data['sql'], *query_data['params']
            )
            
            # Invalidate cache
            await self._invalidate_cache_if_enabled(
                f"{self.table_name}_id", id=record_id
            )
    ...
    async def _get_from_cache_if_enabled(self, prefix: str, **kwargs) -> Optional[Any]:
