...
class SentimentRepository(BaseRepository):
    ...
    def __init__(self, db_adapter: IAsyncDatabase, config: Optional[RepositoryConfig] = None):
        super().__init__(
            db_adapter,
            type('Sentiment', (), {'__tablename__': 'sentiment_data'}),
            config
        )
...
class RatingsRepository(BaseRepository):
    ...
    def __init__(self, db_adapter: IAsyncDatabase, config: Optional[RepositoryConfig] = None):
        super().__init__(
            db_adapter,
            type('Rating', (), {'__tablename__': 'analyst_ratings'}),
            config
        )
        ...
        try:
            ...
            if result and result['total_ratings'] > 0:
                return {
                    ...
                    'avg_price_target': float(result['avg_price_target']) if result['avg_price_target'] else None,
...
class DividendsRepository(BaseRepository):
    ...
    def __init__(self, db_adapter: IAsyncDatabase, config: Optional[RepositoryConfig] = None):
        super().__init__(
            db_adapter,
            type('Dividend', (), {'__tablename__': 'dividends'}),
            config
        )
...
class SocialSentimentRepository(BaseRepository):
    ...
    def __init__(self, db_adapter: IAsyncDatabase, config: Optional[RepositoryConfig] = None):
        super().__init__(
            db_adapter,
            type('SocialSentiment', (), {'__tablename__': 'social_sentiment'}),
            config
        )
...
class GuidanceRepository(BaseRepository):
    ...
    def __init__(self, db_adapter: IAsyncDatabase, config: Optional[RepositoryConfig] = None):
        super().__init__(
            db_adapter,
            type('Guidance', (), {'__tablename__': 'earnings_guidance'}),
            config
        )
    ...
    async def compare_guidance_to_actual(
        self,
        symbol: str,
        period_end: datetime
    ) -> Dict[str, Any]:
            ...
            for guidance in guidance_results:
                ...
                if actual_val is not None:
                    comparison['metrics'][metric] = {
                        ...
                        'beat_percentage': ((actual_val - guidance_val) / guidance_val * 100) if guidance_val else 0
                    }
