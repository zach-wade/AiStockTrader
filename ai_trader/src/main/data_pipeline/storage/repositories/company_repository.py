...
class CompanyRepository(BaseRepository, ICompanyRepository):
        ...
        try:
            if layer not in [0, 1, 2, 3]:
                return OperationResult(
                    success=False,
                    error=f"Invalid layer: {layer}. Must be 0, 1, 2, or 3"
                )
            ...
            if result:
                ...
                await self._invalidate_cache(f"layer_*")
    ...
    async def get_symbols_by_layer(
        self, 
        layer: int,
        is_active: bool = True
    ) -> List[str]:
    ...
    async def get_symbols_above_layer(
        self,
        min_layer: int,
        is_active: bool = True
    ) -> List[str]:
    ...
    async def _record_layer_transition(
        ...
        try:
            ...
            logger.debug(
                f"Recorded layer transition: {symbol} from {from_layer} to {to_layer}"
            )
    ...
    async def update_layer_qualification(
        ...
        if qualified:
            ...
            return await self.update_layer(
                symbol, 
                target_layer, 
                reason or f"Disqualified from layer {layer}"
            )
