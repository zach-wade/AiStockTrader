...
class FinancialsRepository(BaseRepository, IFinancialsRepository):
    ...
    def __init__(
        ...
        super().__init__(
            db_adapter,
            type('Financials', (), {'__tablename__': 'financials_data'}),
            config
        )
    ...
    async def get_financials(
        ...
        try:
            ...
            await self.metrics.record_operation(
                'get_financials', duration, True, len(df)
            )
            ...
            await self.metrics.record_operation(
                'get_financials', time.time() - start_time, False
            )
            return pd.DataFrame()
    ...
    async def cleanup_old_financials(
        self,
        ...
        try:
            cutoff_date = datetime.now(timezone.utc).replace(
                year=datetime.now().year - years_to_keep
            )
    ...
    async def _upsert_financial(self, record: Dict[str, Any]) -> None:
        ...
        update_fields = [f"{f} = EXCLUDED.{f}" for f in fields if f not in ['symbol', 'period_end', 'period_type']]
