...
class PolygonMarketClient(BaseIngestionClient[List[Dict[str, Any]]]):
    ...
    async def parse_response(self, response) -> List[Dict[str, Any]]:
        ...
        for record in results:
            parsed_results.append({
                'timestamp': datetime.fromtimestamp(record['t'] / 1000, tz=timezone.utc),
                'open': float(record['o']),
                'high': float(record['h']),
                'low': float(record['l']),
    ...
    async def fetch_aggregates(
        ...
        with timer("polygon.market_data.fetch", tags={"symbol": symbol, "interval": interval.value}):
        ...
        if result.success and result.data:
            ...
            record_metric("polygon.market_data.records", len(result.data), MetricType.COUNTER,
                         tags={"symbol": symbol, "interval": interval.value})
        else:
            # Track API errors
            record_metric("polygon.api.errors", 1, MetricType.COUNTER,
                         tags={"data_type": "market_data", "symbol": symbol, 
    ...
    async def fetch_latest_price(self, symbol: str) -> FetchResult[Dict[str, Any]]:
        ...
        if result.success and result.data:
            if len(result.data) > 0:
                ...
                record_metric("polygon.latest_price.success", 1, MetricType.COUNTER,
                             tags={"symbol": symbol})
                
                # Track price as metric
                if 'close' in latest:
                    record_metric("polygon.latest_price.value", latest['close'],
                                 MetricType.GAUGE, tags={"symbol": symbol})
        ...
        # Track failures
        record_metric("polygon.latest_price.failure", 1, MetricType.COUNTER,
    ...
    async def fetch_multiple_symbols(
        ...
        async def fetch_symbol(symbol: str) -> FetchResult:
            return await self.fetch_aggregates(
                symbol, interval, start_date, end_date
            )
        
        # Track batch operation
        batch_start = datetime.now()
        record_metric("polygon.batch.symbols_requested", len(symbols),
                     MetricType.GAUGE, tags={"interval": interval.value})
        ...
        record_metric("polygon.batch.duration", batch_duration, MetricType.HISTOGRAM,
                     tags={"interval": interval.value, "symbols": len(symbols)})
        record_metric("polygon.batch.success_rate", successful / len(symbols) if symbols else 0,
                     MetricType.GAUGE, tags={"interval": interval.value})
        
        if failed > 0:
            record_metric("polygon.batch.failures", failed, MetricType.COUNTER,
