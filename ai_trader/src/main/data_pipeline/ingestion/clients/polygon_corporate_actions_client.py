...
class PolygonCorporateActionsClient(BaseIngestionClient[List[Dict[str, Any]]]):
    ...
    async def fetch_dividends(
        ...
        if result.success and result.data:
            ...
            record_metric("polygon.corporate_actions.dividends", len(result.data), MetricType.COUNTER,
                         tags={"symbol": symbol})
            
            # Track dividend yield if available
            if result.data:
                cash_amounts = [r.get('cash_amount', 0) for r in result.data if r.get('cash_amount')]
                if cash_amounts:
                    avg_dividend = sum(cash_amounts) / len(cash_amounts)
                    gauge("polygon.corporate_actions.avg_dividend", avg_dividend,
                          tags={"symbol": symbol})
        else:
            # Track API errors
            record_metric("polygon.api.errors", 1, MetricType.COUNTER,
                         tags={"data_type": "dividends", "symbol": symbol,
    ...
    async def fetch_splits(
        ...
        if result.success and result.data:
            ...
            record_metric("polygon.corporate_actions.splits", len(result.data), MetricType.COUNTER,
                         tags={"symbol": symbol})
            
            # Track split ratios
            for split in result.data:
                if split.get('split_from') and split.get('split_to'):
                    ratio = split['split_to'] / split['split_from'] if split['split_from'] > 0 else 0
                    record_metric("polygon.corporate_actions.split_ratio", ratio, MetricType.HISTOGRAM,
                                 tags={"symbol": symbol})
        else:
            # Track API errors
            record_metric("polygon.api.errors", 1, MetricType.COUNTER,
                         tags={"data_type": "splits", "symbol": symbol,
                               "error": result.error or "unknown"})
    ...
    async def fetch_corporate_actions(
        ...
        gauge("polygon.corporate_actions.total_actions", len(all_actions),
              tags={"symbol": symbol})
        gauge("polygon.corporate_actions.dividend_count", dividend_count,
              tags={"symbol": symbol})
        gauge("polygon.corporate_actions.split_count", split_count,
              tags={"symbol": symbol})
        
        if errors:
            record_metric("polygon.corporate_actions.errors", len(errors), MetricType.COUNTER,
                         tags={"symbol": symbol})
    ...
    async def fetch_multiple_symbols(
        ...
        record_metric("polygon.corporate_actions.batch_duration", batch_duration, MetricType.HISTOGRAM,
                     tags={"symbols": len(symbols)})
        record_metric("polygon.corporate_actions.batch_success_rate", successful / len(symbols) if symbols else 0,
                     MetricType.GAUGE)
        record_metric("polygon.corporate_actions.batch_total_actions", total_actions, MetricType.COUNTER)
        
        if len(symbols) - successful > 0:
            self.logger.warning(f"Corporate actions batch fetch failed for {len(symbols) - successful}/{len(symbols)} symbols")
