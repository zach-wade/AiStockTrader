...
from typing import List, Dict, Any, Optional, Set, Generic, TypeVar
...
from main.interfaces.ingestion import IBulkLoader, BulkLoadConfig, BulkLoadResult
from main.utils.core import get_logger, timer, AsyncCircuitBreaker
...
class BaseBulkLoader(IBulkLoader, Generic[T], ABC):
    ...
    def __init__(
        ...
        logger.info(f"{self.__class__.__name__} initialized for {data_type} with config: {self.config}")
    ...
    def _add_to_buffer(self, records: List[T], symbol: Optional[str] = None):
    ...
    def _should_flush(self) -> bool:
        ...
        if memory_mb >= self.config.max_memory_mb:
            logger.info(f"Buffer memory {memory_mb:.1f}MB reached limit {self.config.max_memory_mb}MB")
            return True
        
        # Check timeout
        time_since_flush = (datetime.now(timezone.utc) - self._last_flush_time).total_seconds()
    ...
    async def _flush_buffer(self) -> BulkLoadResult:
        ...
        with timer("bulk_load_flush") as t:
            try:
                ...
                # Load to hot storage
                try:
                    logger.info(f"{self.__class__.__name__} flushing {len(self._buffer)} records to database")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        records_per_second = (self._total_records_loaded / self._total_load_time) if self._total_load_time > 0 else 0
