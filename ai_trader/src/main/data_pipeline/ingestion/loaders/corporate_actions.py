...
from main.data_pipeline.services.processing import (
...
class CorporateActionsBulkLoader(BaseBulkLoader[Dict[str, Any]]):
    ...
    def __init__(
        ...
        actions_service: Optional[CorporateActionsService] = None,
        ...
        **kwargs
        ...
        try:
            ...
            if not prepared_records:
                ...
                result.success = True
                result.skip_reason = f"No valid records after processing ({unprocessed} unprocessed)"
                return result
    ...
    async def _archive_records(self, records: List[Dict[str, Any]]) -> None:
        ...
        for record in records:
            ...
            if record.get('ex_date'):
                date = record['ex_date'].date() if isinstance(record['ex_date'], datetime) else record['ex_date']
