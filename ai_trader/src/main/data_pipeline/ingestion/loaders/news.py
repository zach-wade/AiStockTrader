...
class NewsBulkLoader(BaseBulkLoader[Dict[str, Any]]):
    ...
    def _extract_first_image(self, images: List[Any]) -> Optional[str]:
    ...
    async def _load_to_database(self, records: List[Dict[str, Any]]) -> int:
        ...
        columns = [
            'news_id', 'headline', 'summary', 'content', 'published_at',
            'created_at', 'updated_at', 'source', 'publisher', 'author',
            'url', 'image_url', 'amp_url', 'symbols', 'keywords',
            'sentiment_score', 'sentiment_label', 'sentiment_magnitude',
            'insights', 'relevance_score', 'raw_data',
        ...
        async with self.db_adapter.acquire() as conn:
            try:
                ...
                await conn.execute(
                    "CREATE TEMP TABLE temp_news (LIKE news_data INCLUDING ALL)"
                )
