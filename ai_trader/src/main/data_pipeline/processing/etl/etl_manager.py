...
class ETLManager(ErrorHandlingMixin):
    ...
    async def load_data(
        ...
        """
        with timer("etl.load", tags={
            "symbol": symbol,
            "data_type": data_type.value if hasattr(data_type, 'value') else str(data_type),
            "layer": layer.name
        }):
            try:
                self.logger.info(f"Starting ETL for {symbol} {data_type} from {start_date} to {end_date}")
                ...
                # Record metrics
                record_metric("etl.records_loaded", result.records_loaded, MetricType.COUNTER,
                             tags={"symbol": symbol, "data_type": str(data_type)})
                
                if result.records_failed > 0:
                    record_metric("etl.records_failed", result.records_failed, MetricType.COUNTER,
                                 tags={"symbol": symbol, "data_type": str(data_type)})
    ...
    async def _perform_etl(
        ...
        transformed_data = await self._transform_data(
            raw_records, data_type, interval
        )
        ...
        if not transformed_data:
            ...
            return ETLResult(
                success=True,
                records_loaded=0,
                warnings=["No data after transformation"]
            )
    ...
    async def _extract_from_archive(
        ...
        data_type_str = data_type.value.lower() if hasattr(data_type, 'value') else str(data_type).lower()
    ...
    async def _transform_data(
        ...
        self.logger.debug(f"Transformed {len(raw_records)} raw records into {len(transformed)} data records")
        return transformed
    ...
    async def _load_to_database(
        ...
        try:
            ...
            if loader is None:
                return ETLResult(
                    success=False,
                    errors=[f"No loader available for {data_type}"]
                )
            
            # Prepare load parameters based on data type
            load_params = {
                'data': data,
                'source': source or 'polygon'
            }
