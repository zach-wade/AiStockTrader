...
from main.utils.data import ValidationUtils, get_global_validator, ValidationLevel
...
class PipelineValidator(ErrorHandlingMixin):
    ...
    async def validate_data(
        ...
        """
        with timer("validate.process", tags={
            ...
            # Record metrics
            record_metric("validate.errors", len(result.errors), MetricType.COUNTER,
                         tags={"data_type": str(data_type), "layer": layer.name})
            record_metric("validate.warnings", len(result.warnings), MetricType.COUNTER,
                         tags={"data_type": str(data_type), "layer": layer.name})
        ...
        if layer >= DataLayer.LIQUID:
            ...
            if 'close' in data.columns and len(data) > 1:
                ...
                if len(extreme_changes) > 0:
                    result.warnings.append(f"Found {len(extreme_changes)} extreme price changes (>50%)")
        
        # Check volume validity
        if 'volume' in data.columns:
            zero_volume = (data['volume'] == 0).sum()
            if zero_volume > len(data) * 0.1:  # >10% zero volume
                result.warnings.append(f"High proportion of zero volume: {zero_volume/len(data):.1%}")
    ...
    async def _apply_strict_validation(
        self,
        data: pd.DataFrame,
        result: ValidationResult
    ) -> None:
