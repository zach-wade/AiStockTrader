...
from main.utils.data import ProcessingUtils, get_global_processor
...
class DataStandardizer(IDataStandardizer, ErrorHandlingMixin):
    ...
    COLUMN_MAPPINGS = {
        'polygon': {
            ...
            'v': 'volume', 't': 'timestamp', 'vw': 'vwap', 'n': 'trades'
        },
        'alpaca': {
    ...
    async def standardize(
        ...
        """
        with timer("standardize.process", tags={
            "source": source,
            "layer": layer.name,
            ...
            record_metric("standardize.rows", len(df), MetricType.COUNTER,
        ...
        if data_type == DataType.MARKET_DATA:
            ...
            if existing_critical:
                ...
                if len(df) < before_len:
                    self.logger.info(f"Dropped {before_len - len(df)} rows with missing critical data")
