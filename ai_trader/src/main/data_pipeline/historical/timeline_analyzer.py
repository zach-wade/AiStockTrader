...
from main.utils.core import get_logger, ensure_utc, get_trading_days_between
...
class TimelineAnalyzer:
    ...
    def get_expected_data_points(
        ...
        if data_type == DataType.MARKET_DATA:
            return self._get_market_data_timeline(
                symbol, interval, start_date, end_date
            )
        elif data_type == DataType.NEWS:
            return self._get_news_timeline(
                symbol, start_date, end_date
            )
        else:
            return self._get_generic_timeline(
    ...
    def _get_market_data_timeline(
        ...
        if interval == TimeInterval.ONE_DAY:
            ...
            for day in trading_days:
                day_timeline = self._get_trading_day_timeline(
                    day, interval_minutes, interval.value
                )
