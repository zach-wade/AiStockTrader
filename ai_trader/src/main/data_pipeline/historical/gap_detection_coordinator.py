...
from .timeline_analyzer import TimelineAnalyzer, DataPointInfo
from .data_existence_checker import DataExistenceChecker, DataExistenceInfo
from .gap_analyzer import GapAnalyzer, GapSegment
...
class GapDetectionCoordinator:
    ...
    async def detect_gaps_batch(
        self,
        requests: List[GapDetectionRequest]
        ...
        for i, request in enumerate(requests):
            try:
                ...
                results.append(GapDetectionResult(
                    symbol=request.symbol,
                    data_type=request.data_type,
                    interval=request.interval,
                    gaps=[],
    ...
    async def get_gap_summary(
        ...
        for data_type in data_types:
            for interval in intervals:
                ...
                requests.append(GapDetectionRequest(
                    symbol=symbol,
                    data_type=data_type,
                    interval=interval,
                    start_date=start_date,
        ...
        for result in results:
            ...
            summary['overall']['total_missing_points'] += result.statistics.get('total_missing_points', 0)
