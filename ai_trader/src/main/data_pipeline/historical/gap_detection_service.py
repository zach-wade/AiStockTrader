...
from main.utils.core import get_logger, timer, ensure_utc, get_trading_days_between
...
from main.data_pipeline.types import GapInfo, DataType, TimeInterval
...
class GapDetectionService:
    ...
    def __init__(
        ...
        priority_calculator: Optional[GapPriorityCalculator] = None
    ):
        """
    ...
    async def _get_expected_data_points(
        ...
        elif interval in [TimeInterval.ONE_MINUTE, TimeInterval.FIVE_MINUTES,
                         TimeInterval.FIFTEEN_MINUTES, TimeInterval.THIRTY_MINUTES,
                ...
                if news_items:
                    for item in news_items:
                        if 'published_at' in item:
                            points.add(ensure_utc(item['published_at']).replace(
    ...
    async def _check_news_for_date(
        self,
        symbol: str,
        date: datetime
    ) -> bool:
