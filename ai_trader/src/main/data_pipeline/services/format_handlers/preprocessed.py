...
class PreProcessedFormatHandler(BaseFormatHandler):
    ...
    def process(
        ...
        logger.debug(f"Processing {len(data)} pre-processed financial records for symbols: {symbols}")
        ...
        for item in data:
            try:
                ...
                if self.metric_extractor.validate_metrics(item):
                    ...
                    invalid_skipped += 1
                    logger.debug(f"Skipping pre-processed record with insufficient metrics: {symbol} {year} {period}")
    ...
    def _validate_year(self, year_value: Any) -> Optional[int]:
        """
