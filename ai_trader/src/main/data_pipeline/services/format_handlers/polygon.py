...
class PolygonFormatHandler(BaseFormatHandler):
        ...
        for item in data:
            try:
                ...
                if self.metric_extractor.validate_metrics(record):
                    ...
                    logger.debug(f"Skipping Polygon record with insufficient metrics: {symbol} {fiscal_year} {period}")
    ...
    def _extract_fiscal_year(self, item: Dict[str, Any]) -> Optional[int]:
        """
    ...
    def _extract_filing_date(self, item: Dict[str, Any]) -> Optional[Any]:
        """
