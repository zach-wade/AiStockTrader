...
from main.utils.core import get_logger, ensure_utc, AsyncCircuitBreaker, RateLimiter, async_lru_cache
...
class BaseService(ABC):
    ...
    def __init__(
        ...
        self._service_stats = {
            ...
            'failed_requests': 0,
            'creation_time': ensure_utc(datetime.now(timezone.utc)),
            'last_request': None
        }
        
        # Health status
        self._health_status = "healthy"
        self._last_health_check = ensure_utc(datetime.now(timezone.utc))
    ...
    async def execute(
        ...
        """
        context = context or {}
        request_id = context.get('request_id', f"{self.service_name}_{ensure_utc(datetime.now(timezone.utc)).isoformat()}")
    ...
    def _update_stats(self, success: bool) -> None:
        ...
        self._service_stats['last_request'] = ensure_utc(datetime.now(timezone.utc))
        self._last_health_check = ensure_utc(datetime.now(timezone.utc))
