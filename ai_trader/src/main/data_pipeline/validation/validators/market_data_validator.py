...
from main.interfaces.validation.validators import IMarketDataValidator
...
class MarketDataValidator:
    ...
    def __init__(self, config: Optional[Union[Dict[str, Any], DataPipelineConfig.ValidationConfig]] = None):
        ...
        if isinstance(config, DataPipelineConfig.ValidationConfig):
            ...
            self.required_ohlcv_fields = config.get('required_ohlcv_fields', ['open', 'high', 'low', 'close', 'volume'])
    ...
    async def validate_ohlcv_data(
        ...
        try:
            ...
            missing_fields = [field for field in self.required_ohlcv_fields if field not in df.columns]
            if missing_fields:
    ...
    async def validate_trading_hours(
        ...
        try:
            ...
            warnings.append(f"Trading hours validation error: {str(e)}")
    ...
    async def validate_corporate_actions(
        ...
        try:
            ...
            warnings.append(f"Corporate actions validation error: {str(e)}")
        ...
        try:
            ...
            for price, name in zip(prices, ohlc_fields):
                if not self._check_price_precision(price):
                    errors.append(f"{row_prefix}{name.capitalize()} price has excessive precision: {price}")
