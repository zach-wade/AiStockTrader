...
from main.interfaces.validation.validators import IFeatureValidator
...
class FeatureValidator:
    ...
    def __init__(self, config: Optional[Union[Dict[str, Any], DataPipelineConfig.ValidationConfig]] = None):
        ...
        if isinstance(config, DataPipelineConfig.ValidationConfig):
            ...
            self.correlation_thresholds = {
                'max_correlation': config.features.max_correlation
            }
    ...
    async def is_applicable(
        ...
        return context.data_type in [DataType.FEATURES, DataType.MARKET_DATA] and \
               context.layer in [DataLayer.FEATURE, DataLayer.PROCESSED]
    ...
    async def validate_feature_dataframe(
        ...
        try:
            ...
            if features.empty:
                ...
                return self._create_result(context.stage, False, errors, warnings, metrics, start_time)
            ...
            if nan_metrics['nan_ratio'] > self.max_nan_ratio:
                errors.append(f"NaN ratio {nan_metrics['nan_ratio']:.2%} exceeds threshold {self.max_nan_ratio:.2%}")
            ...
            if inf_metrics['inf_count'] > self.max_inf_count:
                errors.append(f"Infinite values count {inf_metrics['inf_count']} exceeds threshold {self.max_inf_count}")
            ...
            if constant_metrics['constant_columns']:
                warnings.append(f"Found {len(constant_metrics['constant_column_names'])} constant features")
            ...
            if coverage_score < self.min_feature_coverage:
                errors.append(f"Feature coverage {coverage_score:.2%} below threshold {self.min_feature_coverage:.2%}")
    ...
    async def detect_feature_drift(
        ...
        try:
            ...
            drift_results['summary'] = {
                ...
                'max_drift_score': max(drift_results['drift_scores'].values()) if drift_results['drift_scores'] else 0
            }
    ...
    async def validate_feature_engineering(
        ...
        try:
            ...
            if hasattr(source_data, 'columns'):
                ...
                if overlapping_cols:
                    warnings.append(f"Feature names overlap with source data: {list(overlapping_cols)}")
