...
class DataCoverageAnalyzer:
    ...
    async def analyze_symbol_coverage(
        ...
        if expected_symbols:
            ...
            coverage_score = len(actual_symbols & expected_set) / len(expected_set) if expected_set else 1.0
        else:
            missing_symbols = set()
            extra_symbols = set()
    ...
    async def analyze_field_coverage(
        ...
        if isinstance(data, pd.DataFrame):
            ...
            field_completeness = {
                field: (field_counts.get(field, 0) / len(data))
                for field in actual_fields
            }
        else:
            actual_fields = set()
        ...
        if required_fields:
            ...
            coverage_score = len(actual_fields & required_set) / len(required_set) if required_set else 1.0
        else:
            missing_fields = set()
            extra_fields = set()
        ...
        avg_completeness = (
            ...
            if field_completeness else 0.0
        )
    ...
    async def analyze_multi_symbol_coverage(
        ...
        for symbol in symbols:
            ...
            for interval in intervals:
                ...
                interval_coverage = await self._analyze_symbol_interval_coverage(
                    symbol, interval
                )
    ...
    async def _analyze_time_coverage(
        ...
        # Calculate metrics
        coverage_score = (expected_points - len(missing_dates)) / expected_points if expected_points > 0 else 0.0
    ...
    async def _analyze_symbol_interval_coverage(
        self,
        symbol: str,
        interval: str
    ) -> Dict[str, Any]:
