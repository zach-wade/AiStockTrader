...
class QualityDataCleaner:
    ...
    async def handle_missing_values(
        ...
        if strategy == "drop":
            ...
            numeric_cols = data.select_dtypes(include=[np.number]).columns
            data[numeric_cols] = data[numeric_cols].interpolate(method='linear', limit=self.fill_limit)
            return data
        else:
    ...
    async def standardize_dataframe(
        ...
        if data.empty:
            logger.warning(f"Empty DataFrame provided for standardization{' for ' + symbol if symbol else ''}")
            return data
    ...
    async def clean_features(
        ...
        if effective_aggressive:
            logger.debug(f"Applying aggressive cleaning for features")
    ...
    async def _ensure_datetime_index(
        ...
        if date_col_found:
            try:
                ...
                logger.warning(f"Failed to set datetime index using '{date_col_found}' for {symbol}: {e}")
        else:
