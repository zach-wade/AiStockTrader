...
from .rule_definitions import ValidationRule, RuleProfile, FailureAction
...
class RuleRegistry:
    ...
    def get_rules_for_stage(
        ...
        if profile:
            rules = [r for r in rules if profile in r.applies_to_profiles or "all" in r.applies_to_profiles]
    ...
    def get_rules_for_profile(
        ...
        if profile:
            if "all" not in profile.enabled_checks:
                rules = [r for r in rules if r.name in profile.enabled_checks or 
                        r.severity.value in profile.enabled_checks]
    ...
    def get_rule_count(self) -> Dict[str, int]:
        ...
        return {
            ...
            "by_severity": {
                severity.value: len(rules)
                for severity, rules in self.rules_by_severity.items()
            },
            "by_stage": {
                stage.value: len(rules)
                for stage, rules in self.rules_by_stage.items()
            },
            "by_data_type": {
                data_type: len(rules)
                for data_type, rules in self.rules_by_data_type.items()
            },
