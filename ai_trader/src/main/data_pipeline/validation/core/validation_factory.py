...
class ValidationFactory:
    ...
    def __init__(
        self,
        profile_manager: Optional[IValidationProfileManager] = None,
        ...
        if use_config_manager:
            try:
                ...
                if hasattr(config, 'orchestrator') and hasattr(config.orchestrator, 'data_pipeline'):
    ...
    async def _get_validation_config(
        ...
        """Get validation configuration."""
        profile_name = config.get('profile', self.default_profile) if config else self.default_profile
