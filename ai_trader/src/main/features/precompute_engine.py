...
from main.utils.cache import features_key, scanner_key
...
class FeaturePrecomputeEngine:
    ...
    async def precompute_features(self, symbols: List[str], 
    ...
    async def _compute_features(self, market_data: pd.DataFrame, 
                               symbol: str, feature_type: str) -> pd.DataFrame:
    ...
    def _compute_technical_indicators(self, market_data: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_cols = [col for col in df.columns if col not in ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'vwap']]
    ...
    def _compute_momentum_features(self, market_data: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_cols = [col for col in df.columns if col.startswith(('momentum_', 'roc_', 'price_accel'))]
        result = df[['timestamp'] + feature_cols].copy()
    ...
    def _compute_volatility_features(self, market_data: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_cols = [col for col in df.columns if col.startswith(('true_range', 'atr', 'volatility_', 'rel_vol_', 'vol_ratio_'))]
        result = df[['timestamp'] + feature_cols].copy()
    ...
    def _compute_volume_features(self, market_data: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_cols = [col for col in df.columns if col.startswith(('vol_', 'rvol_', 'vpt', 'obv'))]
        result = df[['timestamp'] + feature_cols].copy()
    ...
    def _compute_price_action_features(self, market_data: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_cols = [col for col in df.columns if col.startswith(('body_', 'upper_', 'lower_', 'gap_', 'daily_', 'range_', 'price_vs_', 'intraday_'))]
        result = df[['timestamp'] + feature_cols].copy()
        
        return result.dropna()
    
    async def _cache_features(self, symbol: str, feature_type: str, 
    ...
    async def _store_features_async(self, symbol: str, feature_type: str, 
                                   features_df: pd.DataFrame):
        ...
        try:
            ...
            success = await asyncio.to_thread(
                ...
                symbol, features_df, feature_type
            )
    ...
    async def _metrics_collector(self):
        ...
        while self.is_running:
            try:
                ...
                await self.cache.set(CacheType.CUSTOM,
                    "precompute:metrics", 
                    metrics_data, 
                    ttl=300
                )
