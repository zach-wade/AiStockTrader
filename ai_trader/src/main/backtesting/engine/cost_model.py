...
class SlippageModel:
    ...
    def calculate(self, quantity: int, price: float, order_side: OrderSide,
...
class FixedSlippage(SlippageModel):
    ...
    def calculate(self, quantity: int, price: float, order_side: OrderSide,
                 spread: float = 0.01, volatility: float = 0.02) -> float:
...
class CostModel(ErrorHandlingMixin):
    ...
    def __init__(self,
                 ...
                 regulatory_fee_rate: float = 0.00002):  # 0.2 bps
        """
    ...
    def calculate_trade_cost(self,
                           ...
                           holding_days: int = 0) -> CostComponents:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cost_breakdown': {
                'commission_pct': self.total_costs.commission / self.total_costs.total_cost * 100
                if self.total_costs.total_cost > 0 else 0,
                'slippage_pct': self.total_costs.slippage / self.total_costs.total_cost * 100
                if self.total_costs.total_cost > 0 else 0,
                'market_impact_pct': self.total_costs.market_impact / self.total_costs.total_cost * 100
                if self.total_costs.total_cost > 0 else 0,
            }
        }
