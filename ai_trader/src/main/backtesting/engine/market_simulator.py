...
from main.models.common import Order, OrderType, OrderSide, OrderStatus, TimeInForce
from main.interfaces.events import OrderEvent
from main.events.types import FillEvent
from .cost_model import CostModel, CostComponents
...
class MarketSimulator(ErrorHandlingMixin):
    ...
    def __init__(self,
                 ...
                 default_spread_bps: float = 10.0):  # 10 basis points
        """
    ...
    def update_market_data(self, symbol: str, bid: float, ask: float,
                          bid_size: int, ask_size: int, last_price: float,
                          timestamp: datetime):
    ...
    def submit_order(self, order: Order, timestamp: datetime) -> OrderEvent:
        ...
        logger.debug(f"Order submitted: {order.order_id} - {order.symbol} "
    ...
    def _process_symbol_orders(self, symbol: str, order_book: SimulatedOrderBook,
                              timestamp: datetime) -> List[FillEvent]:
    ...
    def _try_fill_order(self, order: Order, order_book: SimulatedOrderBook,
                       timestamp: datetime) -> Optional[FillEvent]:
    ...
    def _get_execution_price(self, order: Order, order_book: SimulatedOrderBook) -> Optional[float]:
        ...
        if order.order_type == OrderType.MARKET:
            ...
            if order.side == OrderSide.BUY:
                ...
                # Sell limit can execute at bid or better
                if limit_price <= order_book.bid:
                    if self.execution_mode == ExecutionMode.OPTIMISTIC:
                        return limit_price  # Fill at limit
                    else:
    ...
    def _should_fill(self, order: Order, execution_price: float,
                    order_book: SimulatedOrderBook) -> bool:
        ...
        if self.execution_mode == ExecutionMode.REALISTIC:
            available_size = (order_book.ask_size if order.side == OrderSide.BUY
                            else order_book.bid_size)
    ...
    def _get_fill_quantity(self, order: Order, order_book: SimulatedOrderBook) -> int:
        ...
        available = (order_book.ask_size if order.side == OrderSide.BUY
                    else order_book.bid_size)
    ...
    def _calculate_costs(self, order: Order, execution_price: float,
                        fill_quantity: int, order_book: SimulatedOrderBook) -> CostComponents:
    ...
    def _create_rejection_event(self, order: Order, timestamp: datetime,
                               reason: str) -> OrderEvent:
    ...
    def cancel_order(self, order_id: str, timestamp: datetime) -> Optional[OrderEvent]:
        ...
        if symbol in self.order_queues:
            self.order_queues[symbol] = [
                p for p in self.order_queues[symbol]
