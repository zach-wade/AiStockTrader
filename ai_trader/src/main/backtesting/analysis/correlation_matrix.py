...
class CorrelationMatrix:
    ...
    def _detect_correlation_clusters(self, returns: pd.DataFrame):
        ...
        try:
            ...
            for cluster_id in range(n_clusters):
                cluster_assets = [returns.columns[i] for i, c in enumerate(clusters) if c == cluster_id]
    ...
    def get_correlation_pairs(self, 
                            ...
                            lookback_period: int = 60) -> List[CorrelationPair]:
        ...
        for i in range(len(correlation_matrix.columns)):
            for j in range(i + 1, len(correlation_matrix.columns)):
                ...
                if abs(correlation) >= min_correlation:
                    ...
                    z_score = (correlation - rolling_mean) / (rolling_std + 1e-6) if rolling_std > 0 else 0
