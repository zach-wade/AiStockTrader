...
class IEventCoordinator(ABC):
    ...
    @abstractmethod
    async def handle_symbol_promoted(self, symbol: str, from_layer: DataLayer, to_layer: DataLayer) -> None:
        """Handle symbol promotion event."""
        pass
    
    @abstractmethod
    async def handle_data_gap_detected(self, symbol: str, data_type: DataType, gap_info: Dict[str, Any]) -> None:
