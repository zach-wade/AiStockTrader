...
class SystemDashboardV2:
    ...
    def render_system_health(self):
        ...
        # Current metrics
        latest_metrics = self.system_metrics_history[-1] if self.system_metrics_history else {
            'cpu': 0, 'memory': 0, 'disk': 0, 'memory_available': 0, 'disk_free': 0
        }
        
        # Metrics cards
        metrics_row = html.Div([
            self._create_metric_card("CPU Usage", f"{latest_metrics['cpu']:.1f}%", 
                                   latest_metrics['cpu'] > 80),
            self._create_metric_card("Memory Usage", f"{latest_metrics['memory']:.1f}%", 
                                   latest_metrics['memory'] > 80),
            self._create_metric_card("Disk Usage", f"{latest_metrics['disk']:.1f}%", 
                                   latest_metrics['disk'] > 90),
            self._create_metric_card("Available Memory", f"{latest_metrics['memory_available']:.1f} GB", 
        ...
        return html.Div([
            metrics_row,
            html.H3("Resource Usage Trends", style={'color': '#00ff88', 'marginBottom': '15px'}),
            resource_chart,
            html.Br(),
            html.Div([
                html.Div([
                    html.H3("Component Status", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    component_grid
                ], style={'width': '48%', 'display': 'inline-block'}),
                html.Div([
                    html.H3("Process Information", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    process_info
    ...
    def render_data_pipeline(self):
        ...
        feeds_grid = html.Div(feed_cards, style={
            'display': 'grid',
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.H3("Data Quality Metrics", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    quality_metrics
                ], style={'width': '48%', 'display': 'inline-block'}),
                html.Div([
                    html.H3("Gap Detection", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    gap_analysis
                ], style={'width': '48%', 'display': 'inline-block', 'marginLeft': '4%'})
            ]),
            html.Br(),
    ...
    def render_infrastructure(self):
        ...
        return html.Div([
            html.Div([
                html.Div([
                    html.H3("Database Metrics", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    db_metrics
                ], style={'width': '48%', 'display': 'inline-block'}),
                html.Div([
                    html.H3("API Client Status", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    api_status
                ], style={'width': '48%', 'display': 'inline-block', 'marginLeft': '4%'})
            ]),
            html.Br(),
    ...
    def render_analytics(self):
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.H3("Feature Engine Status", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    feature_status
                ], style={'width': '48%', 'display': 'inline-block'}),
                html.Div([
                    html.H3("ML Model Performance", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    ml_performance
                ], style={'width': '48%', 'display': 'inline-block', 'marginLeft': '4%'})
            ]),
            html.Br(),
            html.H3("Recent Errors & Warnings", style={'color': '#00ff88', 'marginBottom': '15px'}),
            error_log_table
        ])
    ...
    def _create_resource_usage_chart(self):
        ...
        fig.add_trace(go.Scatter(
            x=df['timestamp'], y=df['cpu'], name='CPU %',
            line=dict(color='#00ff88', width=2)
        ))
        fig.add_trace(go.Scatter(
            x=df['timestamp'], y=df['memory'], name='Memory %',
            line=dict(color='#00aaff', width=2)
        ))
        fig.add_trace(go.Scatter(
            x=df['timestamp'], y=df['disk'], name='Disk %',
    ...
    def _create_component_status_grid(self):
        ...
        for comp_name, status in self.component_status.items():
            ...
            component = html.Div([
                ...
                html.Div(status.get('message', ''), style={'fontSize': '11px', 'color': '#666'})
            ], style={
                'backgroundColor': '#1a1a1a',
                'padding': '15px',
                'borderRadius': '8px',
    ...
    def _create_process_info(self):
        ...
        return html.Div(info_items, style={
            'backgroundColor': '#1a1a1a',
            'padding': '20px',
            'borderRadius': '8px'
        })
    ...
    def _create_throughput_chart(self):
        ...
        for feed_name in self.data_feed_metrics.keys():
            ...
            fig.add_trace(go.Scatter(
                x=times, y=throughput, name=feed_name.capitalize(),
                mode='lines'
            ))
    ...
    def _create_data_quality_metrics(self):
        ...
        return html.Div(metric_items, style={
            'backgroundColor': '#1a1a1a',
            'padding': '20px',
            'borderRadius': '8px'
        })
    ...
    def _create_gap_analysis(self):
        ...
        for gap in gaps:
            ...
            gap_items.append(html.Div([
                html.Span(f"{gap['symbol']}: ", style={'fontWeight': 'bold'}),
                html.Span(f"{gap['gaps']} gaps", style={'color': color}),
                html.Span(f" ({gap['duration']})", style={'color': '#888', 'fontSize': '12px'})
            ], style={'marginBottom': '8px'}))
        
        return html.Div([
            html.Div("Data gaps in last hour:", style={'marginBottom': '15px', 'color': '#888'}),
            html.Div(gap_items)
        ], style={
            'backgroundColor': '#1a1a1a',
            'padding': '20px',
    ...
    def _create_dual_storage_metrics(self):
        ...
        success_cards = html.Div([
            html.Div([
                ...
                html.Div(f"{metrics['hot_success_rate']:.1%}", style={
                    'fontSize': '36px',
                    'fontWeight': 'bold',
                    'color': '#00ff88' if metrics['hot_success_rate'] > 0.99 else '#ffaa00'
                }),
                html.Div(f"{metrics['hot_writes_success']:,} successful", style={'color': '#888', 'fontSize': '14px'}),
                html.Div(f"{metrics['hot_writes_failed']} failed", style={'color': '#ff4444', 'fontSize': '14px'}),
                html.Div([
                    html.Span("Circuit Breaker: ", style={'color': '#888'}),
                    html.Span(metrics['hot_circuit_breaker_state'].upper(), style={
                        'color': '#00ff88' if metrics['hot_circuit_breaker_state'] == 'closed' else '#ff4444',
                        'fontWeight': 'bold'
                    })
            ...
            html.Div([
                ...
                html.Div(f"{metrics['cold_success_rate']:.1%}", style={
                    'fontSize': '36px',
                    'fontWeight': 'bold',
                    'color': '#00ff88' if metrics['cold_success_rate'] > 0.95 else '#ffaa00'
                }),
                html.Div(f"{metrics['cold_writes_success']:,} successful", style={'color': '#888', 'fontSize': '14px'}),
                html.Div(f"{metrics['cold_writes_failed']} failed", style={'color': '#ff4444', 'fontSize': '14px'}),
                html.Div([
                    html.Span("Circuit Breaker: ", style={'color': '#888'}),
                    html.Span(metrics['cold_circuit_breaker_state'].upper(), style={
                        'color': '#00ff88' if metrics['cold_circuit_breaker_state'] == 'closed' else '#ff4444',
                        'fontWeight': 'bold'
                    })
                ], style={'marginTop': '10px'})
            ], style={
        ...
        event_metrics = html.Div([
            html.Div([
                html.Div("Event Publishing", style={'color': '#888', 'marginBottom': '10px'}),
                html.Div([
                    html.Span(f"{metrics['events_published']:,} published", style={'color': '#00ff88'}),
                    html.Span(" | ", style={'color': '#444'}),
                    html.Span(f"{metrics['events_failed']} failed", style={'color': '#ff4444'})
                ])
            ], style={'marginBottom': '15px'}),
            html.Div([
                html.Div("Cold Storage Lag", style={'color': '#888', 'marginBottom': '10px'}),
                html.Div([
                    html.Span(f"{metrics['cold_storage_lag_seconds']}s", style={
                        'fontSize': '24px',
                        'fontWeight': 'bold',
                        'color': '#00ff88' if metrics['cold_storage_lag_seconds'] < 60 else '#ffaa00'
                    }),
                    html.Span(f" ({metrics['queue_depth']} items in queue)", style={
                        'color': '#888',
                        'fontSize': '14px',
                        'marginLeft': '10px'
                    })
    ...
    def _create_api_client_status(self):
        ...
        clients = [
            {'name': 'Alpaca Trading', 'status': 'connected', 'requests': '1.2k/min', 'errors': '0.1%'},
            {'name': 'Polygon.io', 'status': 'connected', 'requests': '800/min', 'errors': '0.0%'},
            {'name': 'Benzinga News', 'status': 'connected', 'requests': '50/min', 'errors': '0.5%'},
            {'name': 'Reddit API', 'status': 'rate_limited', 'requests': '60/min', 'errors': '2.1%'}
        ]
        ...
        for client in clients:
            ...
            client_items.append(html.Div([
                ...
                html.Div([
                    html.Span(f"Requests: {client['requests']}", style={'fontSize': '12px', 'marginRight': '10px'}),
                    html.Span(f"Errors: {client['errors']}", style={'fontSize': '12px', 'color': '#888'})
                ], style={'marginTop': '5px'})
            ], style={'marginBottom': '15px'}))
        
        return html.Div(client_items, style={
            'backgroundColor': '#1a1a1a',
            'padding': '20px',
            'borderRadius': '8px'
        })
    ...
    def _create_network_io_chart(self):
        ...
        for i in range(1, len(self.system_metrics_history)):
            ...
            if time_diff > 0:
                rates.append({
                    ...
                    'sent_rate': (curr['network_sent'] - prev['network_sent']) / time_diff / 1024,  # KB/s
                    'recv_rate': (curr['network_recv'] - prev['network_recv']) / time_diff / 1024   # KB/s
                })
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=df['timestamp'], y=df['sent_rate'], name='Sent',
            line=dict(color='#00ff88', width=2)
        ))
        fig.add_trace(go.Scatter(
            x=df['timestamp'], y=df['recv_rate'], name='Received',
            line=dict(color='#00aaff', width=2)
        ))
    ...
    def _create_sentiment_overview(self):
        ...
        sentiment_color = '#00ff88' if overall_sentiment > 0.1 else '#ff4444' if overall_sentiment < -0.1 else '#ffaa00'
        
        overview = html.Div([
            html.Div([
                html.Span("Overall Sentiment: ", style={'color': '#888'}),
                html.Span(f"{overall_sentiment:+.3f}", 
        ...
        for symbol, data in self.sentiment_data.get('symbols', {}).items():
            sentiment_color = '#00ff88' if data['sentiment'] > 0.1 else '#ff4444' if data['sentiment'] < -0.1 else '#ffaa00'
            trend_icon = '↗' if data['trend'] == 'up' else '↘' if data['trend'] == 'down' else '→'
            
            symbols.append(html.Div([
                html.Span(f"{symbol}: ", style={'fontWeight': 'bold'}),
                html.Span(f"{data['sentiment']:+.2f}", style={'color': sentiment_color}),
                html.Span(f" ({data['posts']} posts) ", style={'color': '#888', 'fontSize': '12px'}),
    ...
    def _create_feature_engine_status(self):
        ...
        features = [
            {'name': 'Price Features', 'status': 'active', 'latency': '12ms', 'success_rate': '99.8%'},
            {'name': 'Volume Features', 'status': 'active', 'latency': '8ms', 'success_rate': '99.9%'},
            {'name': 'Technical Indicators', 'status': 'active', 'latency': '45ms', 'success_rate': '98.5%'},
            {'name': 'Sentiment Features', 'status': 'delayed', 'latency': '250ms', 'success_rate': '95.2%'},
            {'name': 'Market Features', 'status': 'active', 'latency': '23ms', 'success_rate': '99.1%'}
        ]
        ...
        for feature in features:
            ...
            feature_items.append(html.Div([
                ...
                html.Div([
                    html.Span(f"Latency: {feature['latency']}", style={'fontSize': '12px', 'marginRight': '10px'}),
                    html.Span(f"Success: {feature['success_rate']}", style={'fontSize': '12px', 'color': '#888'})
                ], style={'marginTop': '5px'})
            ], style={'marginBottom': '15px'}))
        
        return html.Div(feature_items, style={
            'backgroundColor': '#1a1a1a',
            'padding': '20px',
            'borderRadius': '8px'
        })
    ...
    def _create_ml_performance_metrics(self):
        ...
        return html.Div(model_items, style={
            'backgroundColor': '#1a1a1a',
            'padding': '20px',
            'borderRadius': '8px'
        })
    ...
    def _create_error_log_table(self):
        ...
        style_conditions = [
            ...
            {
                'if': {'column_id': 'level', 'filter_query': '{level} = "INFO"'},
                'color': '#00aaff'
            }
        ]
