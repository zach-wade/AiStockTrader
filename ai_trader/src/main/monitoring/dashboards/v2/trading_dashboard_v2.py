...
class TradingDashboardV2:
            ...
            for row in rows:
                if row['quantity'] and row['current_price']:
                    ...
                    positions.append({
                        ...
                        'portfolio_percent': (market_value / total_value * 100) if total_value > 0 else 0,
        ...
        async with self.db_pool.acquire() as conn:
            ...
            if len(vix_rows) >= 1:
                ...
                vix_data = {
                    ...
                    'change_pct': ((current_vix - prev_vix) / prev_vix * 100) if prev_vix > 0 else 0,
    ...
    def render_trading_overview(self):
        ...
        metrics_row = html.Div([
            ...
            self._create_metric_card("Unrealized P&L", f"${unrealized_pnl:,.2f}", unrealized_pnl < 0),
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.H3("Positions Heatmap", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    positions_chart
                ], style={'width': '48%', 'display': 'inline-block'}),
                html.Div([
                    html.H3("P&L History", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    pnl_chart
                ], style={'width': '48%', 'display': 'inline-block', 'marginLeft': '4%'})
            ]),
            html.Div([
                html.Div([
                    html.H3("Recent Activity", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    activity_feed
                ], style={'width': '70%', 'display': 'inline-block'}),
                html.Div([
                    html.H3("Risk Status", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    risk_gauge
    ...
    def render_market_analysis(self):
        ...
        # VIX card
        vix_card = html.Div([
            html.H3("VIX - Volatility Index", style={'color': '#00ff88', 'marginBottom': '15px'}),
            html.Div([
                html.Div([
                    html.Span("Current: ", style={'color': '#888'}),
                    html.Span(f"{vix_data.get('current', 0):.2f}", style={'fontSize': '24px', 'fontWeight': 'bold'})
                ]),
                html.Div([
                    html.Span("Change: ", style={'color': '#888'}),
                    html.Span(
                        f"{vix_data.get('change', 0):+.2f} ({vix_data.get('change_pct', 0):+.1f}%)",
                        style={'color': '#00ff88' if vix_data.get('change', 0) < 0 else '#ff4444'}
                    )
                ]),
                html.Div([
                    html.Span("Level: ", style={'color': '#888'}),
                    html.Span(vix_data.get('level', 'Unknown'), style={'fontWeight': 'bold'})
                ])
    ...
    def render_portfolio_analytics(self):
        ...
        if not positions_df.empty:
            ...
            positions_table = html.Div("No positions", style={'textAlign': 'center', 'padding': '50px'})
        ...
        risk_cards = html.Div([
            ...
            self._create_metric_card("Max Concentration", f"{risk_metrics['max_concentration']:.1f}%", 
                                   risk_metrics['max_concentration'] > 30),
            self._create_metric_card("Leverage", f"{risk_metrics['leverage']:.2f}x", 
                                   risk_metrics['leverage'] > 2)
        ], style={'display': 'grid', 'gridTemplateColumns': 'repeat(4, 1fr)', 'gap': '20px', 'marginBottom': '30px'})
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.H3("Sector Exposure", style={'color': '#00ff88', 'marginBottom': '15px'}),
                    dcc.Graph(figure=sector_fig)
                ], style={'width': '48%', 'display': 'inline-block'}),
                html.Div([
                    html.H3("Performance Attribution", style={'color': '#00ff88', 'marginBottom': '15px'}),
    ...
    def render_alerts_activity(self):
        ...
        for alert in self.alerts:
            ...
            card = html.Div([
                html.Div([
                    html.Span(alert['timestamp'].strftime('%H:%M:%S'), style={'marginRight': '10px'}),
                    html.Span(alert['type'].upper(), style={'color': severity_color, 'fontWeight': 'bold'})
                ], style={'marginBottom': '5px'}),
                html.Div(alert['message'], style={'fontSize': '14px'}),
                html.Div(f"Symbol: {alert['symbol']}", style={'fontSize': '12px', 'color': '#888'})
            ], style={
        ...
        alerts_section = html.Div([
            ...
            html.Div(alert_cards) if alert_cards else html.Div("No active alerts", 
        ...
        if logs_data:
            ...
            logs_table = html.Div("No trading activity", style={'textAlign': 'center', 'padding': '50px'})
    ...
    def _create_positions_heatmap(self):
        ...
        positions = sorted(self.positions_data, key=lambda x: abs(x['total_pnl']), reverse=True)[:20]
        ...
        fig = go.Figure(go.Treemap(
            ...
            values=sizes,
            parents=[""] * len(symbols),
            text=[f"{s}<br>${pnl:.2f}<br>{pnl_pct:.1f}%" 
                  for s, pnl, pnl_pct in zip(symbols, pnl_values, [p['pnl_percent'] for p in positions])],
    ...
    def _create_activity_feed(self):
        ...
        for activity in self.activity_log[:10]:  # Show last 10
            ...
            item = html.Div([
                html.Div([
                    html.Span(activity['timestamp'].strftime('%H:%M:%S'), style={'marginRight': '10px', 'color': '#888'}),
                    html.Span(activity['symbol'], style={'fontWeight': 'bold', 'marginRight': '10px'}),
                    html.Span(activity['side'].upper(), style={'color': side_color, 'marginRight': '10px'}),
                    html.Span(f"{activity['quantity']} @ ${activity['price']:.2f}" if activity['price'] > 0 else f"{activity['quantity']} shares"),
                    html.Span(f" - {activity['status']}", style={'color': '#888', 'fontSize': '12px'})
                ])
            ], style={'padding': '10px', 'borderBottom': '1px solid #333'})
            
            activity_items.append(item)
        
        return html.Div(activity_items, style={
            'backgroundColor': '#1a1a1a',
            'borderRadius': '8px',
            'maxHeight': '300px',
            'overflowY': 'auto'
    ...
    def _create_risk_gauge(self):
        ...
        position_concentration = max([p['portfolio_percent'] for p in self.positions_data]) if self.positions_data else 0
    ...
    def _create_index_card(self, index_data):
        ...
        return html.Div([
            ...
            html.Div(f"${float(index_data.get('last_price', 0)):.2f}", style={'fontSize': '20px'}),
    ...
    def _create_positions_table(self, df):
        ...
        columns_to_show = ['symbol', 'side', 'quantity', 'avg_price', 'current_price', 'P&L', 'P&L %', 'Value', 'Portfolio %']
    ...
    def _calculate_risk_metrics(self):
        ...
        # Max concentration
        max_concentration = max([p['portfolio_percent'] for p in self.positions_data]) if self.positions_data else 0
    ...
    def _create_performance_attribution(self):
        ...
        top_positions = sorted(self.positions_data, key=lambda x: abs(x['total_pnl']), reverse=True)[:10]
