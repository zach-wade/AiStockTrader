...
from main.monitoring.alerts.unified_alerts import AlertLevel, AlertCategory
...
class UnifiedHealthReporter(ErrorHandlingMixin):
    ...
    def __init__(
        ...
        # Reporting schedule
        self.reporting_schedule = self.config.get('schedule', {
    ...
    async def generate_monthly_report(self) -> None:
        ...
        try:
            ...
            self._last_monthly_report = datetime.now(timezone.utc)
    ...
    async def _capture_performance_snapshot(self):
        ...
        # Store in history
        history_key = datetime.now(timezone.utc).strftime('%Y-%m-%d')
    ...
    def _should_generate_weekly_report(self, now: datetime) -> bool:
        ...
        return (now.weekday() == self.reporting_schedule['weekly']['day'] and
                now.hour == self.reporting_schedule['weekly']['hour'])
    ...
    def _should_generate_monthly_report(self, now: datetime) -> bool:
        ...
        return (now.day == self.reporting_schedule['monthly']['day'] and
                now.hour == self.reporting_schedule['monthly']['hour'])
    ...
    def _identify_top_issues(
        ...
        issues.sort(key=lambda x: (
            ...
            1 if x.get('severity') == 'high' else
            2 if x.get('severity') == 'warning' else
            3
        ))
    ...
    async def _analyze_weekly_trends(self) -> Dict[str, Any]:
        ...
        for metric_name in ['system.cpu.usage_percent', 'system.memory.usage_percent',
                           'trading.orders.executed', 'trading.pnl.unrealized']:
            series = await self.unified_metrics.get_metric_series(
                metric_name,
                period_minutes=7 * 24 * 60
            )
    ...
    async def _analyze_monthly_performance(self) -> Dict[str, Any]:
        ...
        for alert in alert_history:
            ...
            analysis['alert_frequency'][category] = \
                analysis['alert_frequency'].get(category, 0) + 1
    ...
    def _clean_old_history(self):
        ...
        cutoff_date = (datetime.now(timezone.utc) - 
                      timedelta(days=self.max_history_days)).strftime('%Y-%m-%d')
