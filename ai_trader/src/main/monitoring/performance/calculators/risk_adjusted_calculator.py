...
class RiskAdjustedCalculator:
    ...
    @staticmethod
    def sortino_ratio(returns: List[float], target_return: float = 0.0, risk_free_rate: float = 0.02) -> float:
        ...
        avg_excess = np.mean(excess_returns)
        downside_vol = RiskCalculator.downside_volatility(returns, target_return/252, annualize=True)
    ...
    def information_ratio(portfolio_returns: List[float], benchmark_returns: List[float]) -> float:
        ...
        return safe_divide(avg_excess, tracking_error, default_value=0.0) * np.sqrt(252)  # Annualized
