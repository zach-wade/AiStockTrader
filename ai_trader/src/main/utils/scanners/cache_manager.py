...
class ScannerCacheManager(IScannerCache):
                    ...
                    if value is not None:
                        ...
                        if self.enable_memory_cache:
                            await self._add_to_memory_cache(
                                full_key, value, scanner_name, symbol
                            )
    ...
    async def cache_result(
        ...
        if self.enable_memory_cache:
            await self._add_to_memory_cache(
                full_key, result, scanner_name, symbol, ttl_seconds
            )
    ...
    def _get_smart_ttl(self, cache_key: str, result: Any) -> int:
        ...
        if isinstance(result, dict):
            ...
            if 'timestamp' in result:
                data_age = datetime.now(timezone.utc) - result.get('timestamp', datetime.now(timezone.utc))
