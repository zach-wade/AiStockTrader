...
class SyntaxChecker:
    ...
    def __init__(self, project_root: Path):
        ...
        self.async_functions = {
            'fetch_one', 'fetch_all', 'execute', 'execute_query',
        ...
        class AsyncVisitor(ast.NodeVisitor):
            ...
            def visit_Call(self, node):
                if self.in_async:
                    ...
                    if func_name and any(async_func in func_name.lower() 
                                        for async_func in ['async', 'await'] + 
                                        list(self.async_functions)):
    ...
    def check_directory(self, directory: Path, exclude_patterns: Optional[List[str]] = None) -> List[SyntaxIssue]:
        ...
        """
        exclude_patterns = exclude_patterns or ['__pycache__', '.git', 'venv', '.pytest_cache', 'tests']
        all_issues = []
