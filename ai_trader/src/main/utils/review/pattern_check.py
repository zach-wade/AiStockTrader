...
class PatternChecker:
                                    ...
                                    if hasattr(parent, 'orelse') and stmt in ast.walk(parent):
                                        if isinstance(parent, ast.Try):
                                            issues.append(Issue(
                                                ...
                                                code_snippet=lines[stmt.lineno - 1] if stmt.lineno <= len(lines) else None,
    ...
    def check_directory(self, directory: Path, exclude_patterns: Optional[List[str]] = None) -> List[Issue]:
    ...
    def generate_report(self, issues: List[Issue]) -> str:
        ...
        # Report by severity
        for severity_list, severity_name in [(critical, "CRITICAL"), (major, "MAJOR"), (minor, "MINOR")]:
            if severity_list:
...
def main():
    ...
    parser.add_argument("--severity", choices=["critical", "major", "minor", "all"], 
    ...
    if args.severity != "all":
        ...
        if min_severity == IssueSeverity.CRITICAL:
            ...
            issues = [i for i in issues if i.severity in [IssueSeverity.CRITICAL, IssueSeverity.MAJOR]]
