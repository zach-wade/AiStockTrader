...
from .types import UniverseSnapshot
...
class UniverseAnalyzer:
    ...
    def compare_universes(self, 
        ...
        return {
            ...
            'overlap_percentage': len(intersection) / min(len(symbols1), len(symbols2)) * 100 if min(len(symbols1), len(symbols2)) > 0 else 0,
        ...
        for i in range(1, len(recent_history)):
            ...
            if added or removed:
                changes.append({
                    ...
                    'turnover_rate': (len(added) + len(removed)) / len(prev_snapshot.symbols) * 100 if prev_snapshot.symbols else 0
                })
    ...
    def get_universe_statistics(self, name: str) -> Dict[str, Any]:
        ...
        if len(history) > 1:
            ...
            stats['symbol_count_cv'] = stats['symbol_count_std'] / stats['avg_symbol_count'] if stats['avg_symbol_count'] > 0 else 0
    ...
    def analyze_universe_stability(self, name: str, window_days: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            'stability_trend': 'increasing' if stability_scores[-1] > stability_scores[0] else 'decreasing' if stability_scores[-1] < stability_scores[0] else 'stable',
    ...
    def find_unique_symbols(self, target_universe: str, comparison_universes: List[str]) -> Dict[str, Any]:
    ...
    def get_sector_analysis(self, name: str) -> Dict[str, Any]:
        ...
        sector_percentages = {
            sector: (count / total_symbols) * 100 
        ...
        return {
            ...
            'dominant_sector_percentage': sector_percentages.get(sorted_sectors[0][0], 0) if sorted_sectors else 0,
            'minor_sectors': [sector for sector, count in sorted_sectors if count < total_symbols * 0.05],  # < 5%
