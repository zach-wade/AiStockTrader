...
@asynccontextmanager
async def managed_aiohttp_session(connector_kwargs: Optional[Dict[str, Any]] = None, 
                                  **session_kwargs) -> AsyncGenerator:
    """
...
async def cleanup_orphaned_sessions():
    ...
    """
    if _active_sessions:
        logger.warning(f"Found {len(_active_sessions)} potentially unclosed sessions: {_active_sessions}")
