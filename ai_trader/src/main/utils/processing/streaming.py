...
import pandas as pd
from typing import (
    AsyncIterator, Callable, Dict, Any, List, Optional, Union, 
...
class DataFrameStreamer:
    ...
    async def stream_chunks(
        self, 
        data_source: Union[pd.DataFrame, str, Callable[[], pd.DataFrame]]
    ...
    async def process_stream(
        ...
        try:
            async for chunk in self.stream_chunks(data_source):
                ...
                # Process chunk
                with self.memory_monitor.memory_context(f"process_chunk_{self.stats.chunks_processed}"):
                    processed_chunk = await self._process_chunk_safely(chunk, processor)
                    
                    if processed_chunk is not None and not processed_chunk.empty:
                        if output_path:
                            # Write incrementally to file
                            await self._append_to_file(processed_chunk, output_path, self.stats.chunks_processed == 0)
                        else:
                            ...
                            if current_memory > self.config.max_memory_mb:
                                logger.warning(f"Memory usage {current_memory:.1f}MB exceeds limit, flushing buffer")
                                if chunk_buffer:
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(processor):
                ...
                result = await asyncio.wait_for(
                    loop.run_in_executor(self._executor, processor, chunk),
                    timeout=timeout
                )
    ...
    def _log_final_stats(self):
        ...
        logger.info(f"  Avg time per chunk: {self.stats.processing_time/max(self.stats.chunks_processed, 1):.3f}s")
...
class StreamingJoiner:
    ...
    async def streaming_join(
        self,
        left_source: Union[pd.DataFrame, str],
        right_source: Union[pd.DataFrame, str],
