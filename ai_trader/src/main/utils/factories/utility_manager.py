...
from main.utils.cache import CacheBackend, MemoryBackend
...
class UtilityManager:
    ...
    def get_cache(self, config_dict: Optional[Dict[str, Any]] = None):
        """
    ...
    def get_circuit_breaker_stats(self, name: Optional[str] = None) -> Dict[str, Any]:
        """
...
_utility_manager: Optional[UtilityManager] = None
...
def get_resilience_manager(name: str, config: Optional[Dict[str, Any]] = None) -> ResilienceStrategies:
    """Convenience function to get a resilience manager."""
    return get_utility_manager().get_resilience_manager(name, config)


def get_cache(config_dict: Optional[Dict[str, Any]] = None):
