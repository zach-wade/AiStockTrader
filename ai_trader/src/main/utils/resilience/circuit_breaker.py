...
class CircuitBreaker:
    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.state == CircuitBreakerState.OPEN:
            raise CircuitBreakerError(f"Circuit breaker is open. Last failure: {self.metrics.last_failure_time}")
        ...
        try:
            ...
            raise CircuitBreakerTimeoutError(f"Operation timed out after {self.config.timeout_seconds} seconds")
    ...
    async def _record_failure(self):
        ...
        if self.state == CircuitBreakerState.CLOSED:
            if (len(self.metrics.recent_requests) >= self.config.minimum_throughput and
                self.metrics.failure_rate >= (self.config.failure_threshold / self.config.window_size)):
                self.state = CircuitBreakerState.OPEN
                self.metrics.circuit_opened_count += 1
                logger.warning(f"Circuit breaker opened due to high failure rate: {self.metrics.failure_rate:.2%}")
