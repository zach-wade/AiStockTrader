...
from .types import MetricType, AlertLevel, PerformanceMetric, Alert
...
class EnhancedMonitor:
    ...
    def __init__(
        ...
        alert_manager: Optional[Any] = None,  # Type: AlertManager
    ...
    async def get_aggregated_metrics(
        ...
        aggregation: str = "avg",
        period_minutes: int = 60,
        group_by_interval: Optional[int] = None  # minutes
    ...
    def _auto_register_metric(self, name: str, metric_type: Optional[MetricType] = None):
    ...
    def _check_thresholds(self, name: str, value: float, definition: EnhancedMetricDefinition):
        ...
        if definition.critical_threshold is not None:
            if self._compare_threshold(value, definition.critical_threshold, definition.threshold_operator):
                alert_level = AlertLevel.CRITICAL
                threshold = definition.critical_threshold
        # Then warning threshold
        elif definition.warning_threshold is not None:
            if self._compare_threshold(value, definition.warning_threshold, definition.threshold_operator):
    ...
    async def _persistence_loop(self):
        ...
        while self._is_running:
            try:
                ...
                while datetime.now(timezone.utc) < deadline and len(batch) < 1000:
                    try:
                ...
                if batch or (datetime.now(timezone.utc) - last_flush).seconds > 10:
                    if batch:
    ...
    async def _create_metric_aggregation(self, conn, name: str, definition: EnhancedMetricDefinition):
        ...
        last_end = row['last_end'] if row and row['last_end'] else \
            ...
            if rows:
                ...
                for agg_type, value in aggregations.items():
                    await conn.execute(
                        ...
                        VALUES ($1, $2, $3, $4, $5, $6)
                        """,
                        name, agg_type, value, period_start, period_end, len(values)
                    )
    ...
    async def _get_time_buckets(
        ...
        while bucket_start < now:
            ...
            if values:
                ...
                buckets.append(AggregatedMetric(
                    name=name,
                    value=value,
                    aggregation_type=aggregation,
    ...
    def cleanup_old_metrics(self, retention_hours: Optional[int] = None):
        """Clean up old metrics from memory and database."""
        cutoff = datetime.now(timezone.utc) - timedelta(hours=retention_hours or 168)
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                deleted_agg = await conn.execute(
                    "DELETE FROM metric_aggregations WHERE period_end < $1",
                    cutoff
                )
                
                logger.info(
                    f"Cleaned up old metrics: {deleted} metrics, "
                    f"{deleted_agg} aggregations"
                )
