...
from main.utils.monitoring import (
    record_metric, timer, start_monitoring, stop_monitoring,
    get_system_summary, MetricType
)
from main.utils.monitoring.dashboard_adapters import (
    create_dashboard_adapter, DashboardHealthReporter
)
...
async def basic_monitoring_example():
    ...
    record_metric("example.api_latency", 150, MetricType.TIMER, 
...
async def enhanced_monitoring_example(db_pool=None):
    ...
    avg_latency = await monitor.get_metric_value(
        "api.request_duration", "avg", period_minutes=1
    )
