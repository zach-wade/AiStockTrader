...
from .types import PerformanceMetric, MetricType, AlertLevel
...
class PerformanceMonitor:
    ...
    def export_metrics(self, format: str = 'json') -> str:
        ...
        if format == 'json':
            ...
            # System metrics
            writer.writerow(['Metric Type', 'Timestamp', 'CPU %', 'Memory %', 'Disk %', 'Network MB/s'])
            ...
            html_parts.append('.alert-warning { background-color: #fff3cd; border: 1px solid #ffeaa7; }')
            html_parts.append('.alert-critical { background-color: #f8d7da; border: 1px solid #f5c6cb; }')
            html_parts.append('</style></head><body>')
            
            html_parts.append(f'<h1>AI Trader Metrics Report</h1>')
            html_parts.append(f'<p>Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>')
            
            # System metrics
            html_parts.append('<h2>System Metrics</h2>')
            html_parts.append('<table>')
            html_parts.append('<tr><th>Timestamp</th><th>CPU %</th><th>Memory %</th><th>Disk %</th></tr>')
            ...
            if func_summary:
                ...
                html_parts.append('<table>')
                html_parts.append('<tr><th>Function</th><th>Calls</th><th>Avg Time (s)</th><th>Total Time (s)</th></tr>')
            ...
            if recent_alerts:
                ...
                for alert in recent_alerts:
                    alert_class = 'alert-critical' if 'critical' in alert.severity.lower() else 'alert-warning'
                    html_parts.append(f'<div class="alert {alert_class}">')
                    html_parts.append(f'<strong>{alert.timestamp.strftime("%Y-%m-%d %H:%M:%S")}</strong> - ')
            ...
            raise NotImplementedError(f"Export format '{format}' not implemented. Supported formats: json, csv, html")
