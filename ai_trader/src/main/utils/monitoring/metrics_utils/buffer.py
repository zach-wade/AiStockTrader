...
class MetricsBuffer:
    ...
    def add_counter(self, name: str, value: float = 1.0, tags: Optional[Dict] = None):
    ...
    def add_histogram(self, name: str, value: float, tags: Optional[Dict] = None):
    ...
    def add_timer(self, name: str, duration: float, tags: Optional[Dict] = None):
        """Add a timer metric."""
        with self._lock:
            key = self._create_key(name, tags)
    ...
    def add_event(self, name: str, data: Dict, tags: Optional[Dict] = None):
        """Add an event."""
        with self._lock:
            event = {
                'name': name,
    ...
    def _create_key(self, name: str, tags: Optional[Dict]) -> str:
    ...
    def _check_flush(self):
        ...
        total_size = (len(self._counters) + len(self._gauges) + 
...
def buffer_counter(name: str, value: float = 1.0, tags: Optional[Dict] = None):
    """Buffer a counter metric."""
    get_global_buffer().add_counter(name, value, tags)


def buffer_gauge(name: str, value: float, tags: Optional[Dict] = None):
    """Buffer a gauge metric."""
    get_global_buffer().set_gauge(name, value, tags)


def buffer_histogram(name: str, value: float, tags: Optional[Dict] = None):
    """Buffer a histogram value."""
    get_global_buffer().add_histogram(name, value, tags)


def buffer_timer(name: str, duration: float, tags: Optional[Dict] = None):
    """Buffer a timer metric."""
    get_global_buffer().add_timer(name, duration, tags)


def buffer_event(name: str, data: Dict, tags: Optional[Dict] = None):
    """Buffer an event."""
