...
from main.utils.auth import CredentialValidator, CredentialType, validate_credential
        ...
        try:
            ...
            for component_name, validator_func in components:
                try:
                    ...
                    result.add_error(f"Failed to validate {component_name}: {str(e)}")
                    self.handle_error(e, f"validating {component_name}")
            
            # Overall validation summary
            if result.passed:
                self.logger.info(f"✅ Configuration validation passed for {len(result.validated_components)} components")
            else:
