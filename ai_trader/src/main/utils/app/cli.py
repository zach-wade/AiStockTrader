...
from main.utils.core import setup_logging, get_logger, ErrorHandlingMixin
from main.utils.monitoring import record_metric, get_global_monitor
...
class StandardCLIHandler(ErrorHandlingMixin):
    ...
    def __init__(self, config: CLIAppConfig):
        ...
        self.context: Optional[StandardAppContext] = None
...
async def _show_status(config: CLIAppConfig):
    ...
    try:
        async with managed_app_context(config.name, config.context_components) as context:
            ...
            table.add_row("Database", "✅ Connected" if status['database_pool_active'] else "❌ Disconnected", "")
            table.add_row("Data Sources", "✅ Active" if status['data_source_manager_active'] else "❌ Inactive", "")
            table.add_row("Ingestion", "✅ Ready" if status['ingestion_orchestrator_active'] else "❌ Not Ready", "")
            table.add_row("Processing", "✅ Ready" if status['processing_manager_active'] else "❌ Not Ready", "")
            table.add_row("Errors", "⚠️ Issues" if status['error_count'] > 0 else "✅ None", f"Count: {status['error_count']}")
...
def async_command(
    ...
    def decorator(func: Callable):
        ...
        def wrapper(*args, **kwargs):
            return asyncio.run(_execute_async_command(
    ...
    **kwargs
    ...
    try:
        async with cli_handler.managed_execution():
            ...
            console.print(f"✅ {operation_name} completed successfully in {duration:.2f}s", style="green")
...
def warning_message(message: str, details: Optional[str] = None):
...
def info_message(message: str, details: Optional[str] = None):
