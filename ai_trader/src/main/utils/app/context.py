...
class StandardAppContext(ErrorHandlingMixin):
    ...
    def __init__(self, app_name: str, config: Optional[Any] = None):
        """
        ...
        self.data_source_manager: Optional[Any] = None
        self.ingestion_orchestrator: Optional[Any] = None
        self.processing_manager: Optional[Any] = None
        
        # Dual storage components
        self.event_bus: Optional[Any] = None
        self.cold_storage: Optional[Any] = None
        ...
        self.register_error_callback(
            ...
            lambda error, context: record_metric(
                "app_error", 
                1, 
                tags={"app": self.app_name, "context": context}
            )
        )
    
    async def initialize(self, components: Optional[List[str]] = None) -> 'StandardAppContext':
        """
    ...
    async def _create_yahoo_clients(self) -> Dict[str, Any]:
        ...
        try:
            ...
            from main.data_pipeline.ingestion.yahoo_corporate_actions_client import YahooCorporateActionsClient
    ...
    async def _initialize_ingestion(self):
        ...
        try:
            ...
            if not self.data_source_manager:
                raise AppContextError("Data source manager must be initialized before ingestion orchestrator")
    ...
    async def _initialize_dual_storage(self):
        ...
        try:
            ...
            enable_dual_storage = self.config.get('data_pipeline', {}).get('storage', {}).get('enable_dual_storage', True)
    ...
    async def _initialize_processing(self):
        ...
        try:
            ...
            from main.interfaces.database import IDatabaseFactory
            from main.data_pipeline.storage.database_factory import DatabaseFactory
...
async def create_app_context(app_name: str, components: Optional[List[str]] = None) -> StandardAppContext:
    """
...
@asynccontextmanager
async def managed_app_context(app_name: str, components: Optional[List[str]] = None):
    """
