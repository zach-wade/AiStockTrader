...
class ErrorLogger:
    ...
    def __init__(self, config: Dict[str, Any], alert_system=None):
        ...
        self.rate_counters: Dict[ErrorSeverity, deque] = {
            severity: deque(maxlen=limit)
            for severity, limit in self.rate_limits.items()
        }
    ...
    def log_error(self,
                  ...
                  recovery_action: str = "Manual intervention required") -> str:
...
"""
    ...
    def _check_error_patterns(self, error_event: ErrorEvent):
        ...
        pattern_key = f"{error_event.category.value}:{error_event.exception_type}:{error_event.module}"
        self.error_patterns[pattern_key].append(error_event)
        
        # Check for rapid occurrence
        recent_errors = [e for e in self.error_patterns[pattern_key]
                        if e.timestamp > datetime.now() - timedelta(minutes=5)]
        
        if len(recent_errors) > 5:
            # Pattern detected
            logger.warning(f"Error pattern detected: {pattern_key} occurred {len(recent_errors)} times in 5 minutes")
    ...
    async def _analyze_error_trends(self):
        ...
        while self._running:
            try:
                ...
                # Save report
                report_file = self.log_dir / f'error_report_{datetime.now().strftime("%Y%m%d_%H")}.json'
    ...
    def get_recent_errors(self, 
                         ...
                         category: Optional[ErrorCategory] = None) -> List[ErrorEvent]:
    ...
    def generate_error_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'error_rate': {
                ...
                'average': np.mean([r[1] for r in self.metrics['error_rate']]) if self.metrics['error_rate'] else 0,
                'max': max([r[1] for r in self.metrics['error_rate']]) if self.metrics['error_rate'] else 0
            },
            'top_errors': [],
            'critical_errors': [],
            'patterns': []
    ...
    def log_data_error(self, message: str, symbol: Optional[str] = None, **kwargs):
    ...
    def log_trading_error(self, message: str, order_id: Optional[str] = None, **kwargs):
