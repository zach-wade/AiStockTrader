...
from .types import BufferConfig, WebSocketMessage, MessagePriority
...
class MessageBuffer:
    ...
    def add_message(self, message: WebSocketMessage) -> bool:
        ...
        with self._lock:
            ...
            if self.bytes_received > self.config.memory_limit_mb * 1024 * 1024:
                logger.warning(f"Buffer memory limit exceeded")
            ...
            if message.priority in [MessagePriority.CRITICAL, MessagePriority.HIGH]:
                if len(self.priority_queue) < self.config.priority_queue_size:
                    ...
                    # Priority queue full, drop lowest priority message
                    if self.priority_queue and message.priority.value < self.priority_queue[0].priority.value:
    ...
    async def get_batch(self, max_size: Optional[int] = None) -> List[WebSocketMessage]:
        """
                ...
                with self._lock:
                    should_flush = (
                        ...
                        (self.batch_start_time and 
                         time.time() - self.batch_start_time > self.config.flush_interval_ms / 1000.0)
                    )
