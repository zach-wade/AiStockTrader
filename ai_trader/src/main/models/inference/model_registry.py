...
from main.models.inference.model_registry_helpers.registry_storage_manager import RegistryStorageManager
...
from main.models.inference.model_registry_helpers.model_comparison_analyzer import ModelComparisonAnalyzer
...
class ModelRegistry:
    ...
    def __init__(self, models_dir: Path, prediction_engine: PredictionEngine, config: Optional[Dict[str, Any]] = None):
        """
        ...
        self.prediction_engine = prediction_engine # Shared dependency for deployment manager helper
        ...
        self._model_exporter = ModelExporter(models_base_dir=self.models_dir) # Explicitly initialize exporter

        # Registry state: Core data of the ModelRegistry
        self.versions: Dict[str, List[ModelVersion]] = defaultdict(list) # model_id -> List[ModelVersion]
        self.active_deployments: Dict[str, ModelVersion] = {} # model_id -> ModelVersion (actively serving traffic)
        
        self._load_registry_initial() # Load initial state on startup
        
        logger.info(f"ModelRegistry initialized. Loaded {sum(len(v) for v in self.versions.values())} model versions from disk.")
        ...
        for model_id, version_obj in self.active_deployments.items():
            if version_obj.deployment_pct > 0:
                try:
                    ...
                    model_obj, features, _ = self._model_file_manager.load_model(model_id, version_obj.version)
                    ...
                    logger.debug(f"Hot-loaded active model {model_id} v{version_obj.version} ({version_obj.deployment_pct:.1f}%) into PredictionEngine on startup.")

                except Exception as e:
                    logger.error(f"Failed to hot-load active model {model_id} v{version_obj.version} on startup: {e}. Marking as failed.", exc_info=True)
    ...
    def register_model(self, model: Any, model_id: str, 
                      model_type: str, 
        ...
        if self.get_model_version(model_id, version_str):
            logger.warning(f"Model {model_id} version {version_str} already registered. Consider updating or generating a new version.")
        ...
        try:
            ...
            logger.error(f"Failed to save model file for {model_id} v{version_str}. Model will not be registered.", exc_info=True)
    ...
    def get_latest_version(self, model_id: str, status: Optional[str] = None) -> Optional[ModelVersion]:
        """
    ...
    def list_models(self, model_id: Optional[str] = None, status: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        target_versions = self.versions.get(model_id, []) if model_id else [v for sublist in self.versions.values() for v in sublist]
