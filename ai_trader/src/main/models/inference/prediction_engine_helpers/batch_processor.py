...
class BatchProcessor:
    ...
    async def process_batch(self,
        ...
        logger.info(f"Batch processing completed: {self.progress['processed_items']}/{self.progress['total_items']} items processed")
    ...
    async def _process_single_batch(self,
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                logger.error(f"Batch {batch_idx} failed (attempt {attempt + 1}/{self.retry_count}): {e}")
...
async def process_predictions_batch(predictions: List[Dict],
                                  model_func: Callable,
