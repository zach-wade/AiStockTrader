...
class DeploymentManager(ErrorHandlingMixin):
    ...
    async def deploy_model(
        ...
        with self._handle_error("deploying model"):
            ...
            logger.info(
                f"Started deployment {deployment_id} for "
    ...
    async def _deploy_blue_green(
        ...
        try:
            ...
            if config.health_check_enabled:
                ...
                healthy = await self._health_check_instances(
                    green_instances, config
                )
    ...
    async def _deploy_canary(
        ...
        try:
            ...
            canary_count = max(1, int(config.replica_count * 
                                     config.canary_percentage / 100))
            
            status.status = "deploying_canary"
            await self._update_deployment_status(deployment_id, 10.0)
            
            canary_instances = await self._provision_instances(
                config, "canary", count=canary_count
            )
            ...
            canary_healthy = await self._monitor_canary(
                deployment_id,
                config,
                config.canary_duration_minutes
            )
            ...
            remaining_instances = await self._provision_instances(
                config, "production",
                count=config.replica_count - canary_count
            )
    ...
    async def _deploy_rolling(
        ...
        try:
            ...
            for batch_idx in range(num_batches):
                ...
                if config.health_check_enabled:
                    healthy = await self._health_check_instances(
                        batch_instances, config
                    )
                    
                    if not healthy:
                        raise Exception(
                            f"Batch {batch_idx} failed health check"
                        )
    ...
    async def _deploy_immediate(
        ...
        try:
            ...
            instances = await self._provision_instances(
                config, "production"
            )
    ...
    async def _health_check_instances(
        ...
        logger.info(
            f"Health check: {healthy_count}/{len(instances)} healthy "
    ...
    async def _switch_traffic(
        ...
        logger.info(
            f"Switched traffic for {model_name} in {environment} to {target}"
        )
    ...
    async def _route_canary_traffic(
        ...
        logger.info(
            f"Routing {percentage}% of traffic for {model_name} "
            f"in {environment} to canary"
        )
    
    async def _decommission_instances(
        self,
        tag: str
    ) -> None:
    ...
    async def cancel_deployment(
        ...
        await self._record_deployment_event(
            ...
            {'cancelled_at': status.progress_percentage}
        )
