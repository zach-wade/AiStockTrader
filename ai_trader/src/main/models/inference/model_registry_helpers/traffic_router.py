...
class TrafficRouter(ErrorHandlingMixin):
            ...
            for model_key, endpoints in self._endpoints.items():
                for idx, ep in enumerate(endpoints):
                    if ep.endpoint_id == endpoint_id:
                        ...
                        logger.info(
                            f"Unregistered endpoint {endpoint_id}"
                        )
    ...
    async def route_request(
        ...
        with self._handle_error("routing request"):
            ...
            if session_id and routing_config.strategy == RoutingStrategy.STICKY:
                endpoint = self._check_session_affinity(
                    session_id, endpoints, routing_config
                )
                if endpoint:
            ...
            if routing_config.strategy == RoutingStrategy.ROUND_ROBIN:
                ...
                endpoint = self._route_weighted_round_robin(
                    endpoints, routing_config
                )
                ...
                endpoint = self._route_canary(
                    endpoints, routing_config, request_id
                )
            elif routing_config.strategy == RoutingStrategy.AB_TEST:
                endpoint = self._route_ab_test(
                    endpoints, routing_config, request_id
                )
            else:
                # Default to random
            ...
            if session_id and routing_config.strategy == RoutingStrategy.STICKY:
                self._update_session_affinity(
                    session_id, endpoint, routing_config
                )
            
            # Record routing decision
            await self._record_routing_decision(
                request_id, endpoint, routing_config.strategy
            )
    ...
    def _route_canary(
        ...
        canary_endpoints = [
            ...
            if ep.metadata.get('deployment_type') == 'canary'
        ]
        stable_endpoints = [
            ep for ep in endpoints 
            if ep.metadata.get('deployment_type') != 'canary'
    ...
    def _route_ab_test(
        ...
        a_endpoints = [
            ep for ep in endpoints 
            if ep.metadata.get('ab_variant') == 'A'
        ]
        b_endpoints = [
            ep for ep in endpoints 
            if ep.metadata.get('ab_variant') == 'B'
        ]
    ...
    def _check_session_affinity(
        self,
        session_id: str,
        available_endpoints: List[ModelEndpoint],
        config: RoutingConfig
    ...
    def _cleanup_old_sessions(
        ...
        cutoff = datetime.utcnow() - timedelta(
            minutes=config.sticky_session_duration_minutes
        )
        
        to_remove = [
            session_id for session_id, (_, timestamp) in 
            self._session_affinity.items()
            if timestamp < cutoff
        ]
    ...
    def _update_routing_metrics(
        ...
        if endpoint.endpoint_id not in self._routing_metrics:
            self._routing_metrics[endpoint.endpoint_id] = {
                'total_requests': 0,
                'by_strategy': {}
            }
    ...
    async def _health_check_loop(
        ...
        while True:
            try:
                ...
                if not healthy:
                    logger.warning(
                        f"Endpoint {endpoint.endpoint_id} marked unhealthy"
                    )
                ...
                logger.error(
                    f"Health check error for {endpoint.endpoint_id}: {e}"
                )
    ...
    async def update_endpoint_weight(
        self,
        endpoint_id: str,
        weight: float
    ) -> bool:
        ...
        for endpoints in self._endpoints.values():
            for ep in endpoints:
                if ep.endpoint_id == endpoint_id:
                    ep.weight = weight
                    logger.info(
                        f"Updated weight for {endpoint_id} to {weight}"
                    )
                    return True
