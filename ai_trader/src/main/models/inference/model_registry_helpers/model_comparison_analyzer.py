...
class ModelComparisonAnalyzer:
    ...
    def compare_model_versions(self, version1_obj: ModelVersion, version2_obj: ModelVersion) -> Dict[str, Any]:
        """
        ...
        if version1_obj.model_id != version2_obj.model_id:
            logger.warning(f"Comparing different model_ids: {version1_obj.model_id} vs {version2_obj.model_id}. Comparison might be less meaningful.")
        ...
        comparison = {
            ...
            'metrics_diff': {},
            'features_diff': {
                'added_in_v2': sorted(list(set(version2_obj.features) - set(version1_obj.features))),
                'removed_in_v2': sorted(list(set(version1_obj.features) - set(version2_obj.features)))
            },
        ...
        for metric in all_metrics_keys:
            ...
            v1_val_num = float(v1_val) if (isinstance(v1_val, (int, float)) and pd.notna(v1_val)) else np.nan
            v2_val_num = float(v2_val) if (isinstance(v2_val, (int, float)) and pd.notna(v2_val)) else np.nan
            ...
            if np.isclose(v1_val_num, v2_val_num, equal_nan=True): # equal_nan handles np.nan == np.nan
                continue 

            change_pct = np.nan
            if pd.notna(v1_val_num) and v1_val_num != 0:
                change_pct = ((v2_val_num - v1_val_num) / v1_val_num) * 100
            elif pd.notna(v2_val_num) and (pd.isna(v1_val_num) or v1_val_num == 0): # From missing/zero to non-zero
        ...
        all_hparam_keys = set(version1_obj.hyperparameters.keys()) | set(version2_obj.hyperparameters.keys())
        ...
        logger.debug(f"Compared model '{version1_obj.model_id}' versions '{version1_obj.version}' and '{version2_obj.version}'.")
        return comparison
