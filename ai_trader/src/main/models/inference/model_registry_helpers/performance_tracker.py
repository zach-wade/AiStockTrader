...
class PerformanceTracker:
    ...
    def update_model_performance(self, model_version: ModelVersion, performance_data: Dict[str, float]):
        """
        ...
        for metric_name, new_value in performance_data.items():
            if not isinstance(new_value, (int, float)) or np.isnan(new_value) or np.isinf(new_value):
                logger.warning(f"Skipping non-numeric or invalid value for metric '{metric_name}': {new_value}.")
                continue
            ...
            logger.debug(f"Updated '{model_version.model_id}' v'{model_version.version}' metric '{metric_name}' to {model_version.metrics[metric_name]:.4f}.")
        ...
        #     **performance_data
        # })
        logger.info(f"Performance updated for model '{model_version.model_id}' version '{model_version.version}'.")
