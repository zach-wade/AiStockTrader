    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'FeatureSetDefinition':
        ...
        for f_data in data.get('features', []):
            feature = FeatureDefinition(
                ...
                transforms=[
                    FeatureTransform(t) for t in f_data.get('transforms', [])
                ],
...
class FeatureSetManager(ErrorHandlingMixin):
    ...
    def get_feature_set(
        ...
        # Get latest version
        matching = [
            (k, v) for k, v in self._feature_sets.items()
        ...
        for feature_def in feature_set.features:
            if feature_def.name in features:
                ...
                if feature_def.min_value is not None and value < feature_def.min_value:
                    errors.append(
                        f"{feature_def.name} below minimum: "
                        f"{value} < {feature_def.min_value}"
                    )
                
                if feature_def.max_value is not None and value > feature_def.max_value:
                    errors.append(
                        f"{feature_def.name} above maximum: "
                        f"{value} > {feature_def.max_value}"
                    )
                
                # Allowed values validation
                if feature_def.allowed_values and value not in feature_def.allowed_values:
                    errors.append(
                        f"{feature_def.name} not in allowed values: {value}"
                    )
    ...
    def _register_default_validators(self):
        ...
        def cross_feature_validator(features: Dict[str, Any], rule: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        def sum_constraint_validator(features: Dict[str, Any], rule: Dict[str, Any]) -> Tuple[bool, str]:
