...
class Order:
    ...
    metadata: Dict[str, Any] = field(default_factory=dict) # Use default_factory for mutable defaults
    ...
    def is_complete(self) -> bool:
        ...
        return self.status in [OrderStatus.FILLED, OrderStatus.CANCELLED, OrderStatus.REJECTED, OrderStatus.FAILED, OrderStatus.EXPIRED]
    ...
    def with_status(self, status: OrderStatus, timestamp: Optional[datetime] = None, message: Optional[str] = None) -> 'Order':
    ...
    def with_fill(self, fill_qty: float, fill_price: float, timestamp: Optional[datetime] = None, commission: float = 0.0) -> 'Order':
...
class Strategy:
    ...
    def generate_signals(self, timestamp: datetime, market_data: Dict[str, MarketData]) -> List[Signal]:
        """
    ...
    def _apply_risk_management(self, signals: List[Signal]) -> List[Signal]:
        ...
        if not self.check_risk_limits():
            ...
            signals = [s for s in signals if s.action == 'SELL' or 
                      (s.symbol in self.positions and s.action == 'BUY' and self.positions[s.symbol].side == 'short')]
