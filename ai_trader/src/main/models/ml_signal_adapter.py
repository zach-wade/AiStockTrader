...
from main.trading_engine.signals.unified_signal import (
    UnifiedSignal, SignalSource, SignalPriority
)
...
class MLSignalAdapter:
    ...
    def convert_prediction_to_signal(self,
                                   prediction: MLPrediction,
                                   model_config: Optional[Dict[str, Any]] = None) -> Optional[UnifiedSignal]:
        """
        ...
        try:
            ...
            if prediction.confidence < config.min_confidence:
                logger.debug(f"Prediction confidence {prediction.confidence:.2f} below threshold {config.min_confidence}")
                return None
            ...
            logger.info(f"Created ML signal: {signal.symbol} {signal.side.value} "
    ...
    def _get_signal_config(self, model_config: Optional[Dict[str, Any]]) -> MLSignalConfig:
        ...
        if self.config:
            ...
            config.use_dynamic_sizing = self.config.get('use_dynamic_sizing', config.use_dynamic_sizing)
            config.max_position_size_pct = self.config.get('max_position_size', config.max_position_size_pct)
    ...
    def _calculate_position_size(self, prediction: MLPrediction, config: MLSignalConfig) -> float:
        ...
        if config.use_dynamic_sizing:
            ...
            confidence_normalized = (prediction.confidence - config.min_confidence) / confidence_range
    ...
    def _create_signal_metadata(self, prediction: MLPrediction, config: MLSignalConfig) -> Dict[str, Any]:
        """Create comprehensive signal metadata."""
        metadata = {
            'ml_model_id': prediction.model_id,
            'ml_confidence': prediction.confidence,
            'ml_prediction_time': prediction.timestamp.isoformat() if prediction.timestamp else None,
    ...
    def batch_convert_predictions(self, 
                                predictions: List[MLPrediction],
                                model_configs: Optional[Dict[str, Dict[str, Any]]] = None) -> List[UnifiedSignal]:
        """
