...
class EntryPriceDeterminer(ErrorHandlingMixin):
    ...
    async def determine_entry_price(
        ...
        with self._handle_error("determining entry price"):
            ...
            price_result = await self._calculate_entry_price(
                context, method
            )
            ...
            for i in range(0, len(requests), max_concurrent):
                ...
                for request, result in zip(batch, batch_results):
                    if isinstance(result, Exception):
                        logger.error(
                            f"Error determining price for {request[0]}: {result}"
                        )
                    else:
                        results.append(result)
            
            logger.info(
                f"Determined entry prices for {len(results)}/{len(requests)} requests"
            )
    ...
    async def _get_price_context(
        ...
        async with self.db_pool.acquire() as conn:
            ...
            book_row = await conn.fetchrow(
                book_query, symbol, window_start, window_end
            )
    ...
    async def _calculate_derived_context(
        self,
        context: PriceContext,
        conn
    ) -> None:
    ...
    async def _select_pricing_method(
        self,
        context: PriceContext
    ) -> PricingMethod:
        """Select optimal pricing method based on context."""
        # High liquidity and normal hours -> use midpoint or VWAP
        if (context.liquidity_score and context.liquidity_score > 0.7 and 
            context.market_hours and context.bid_price and context.ask_price):
        ...
        if (context.order_size and context.volume and 
            context.order_size > 0.05 * context.volume):
    ...
    async def _calculate_close_price(
        self,
        context: PriceContext
    ) -> EntryPriceResult:
    ...
    async def _calculate_open_price(
        self,
        context: PriceContext
    ) -> EntryPriceResult:
    ...
    async def _calculate_midpoint_price(
        self,
        context: PriceContext
    ) -> EntryPriceResult:
    ...
    async def _calculate_vwap_price(
        self,
        context: PriceContext
    ) -> EntryPriceResult:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            return EntryPriceResult(
                ...
                market_conditions={
                    'vwap_periods': len(rows),
                    'total_volume': total_volume
                }
            )
    
    async def _calculate_twap_price(
        self,
        context: PriceContext
    ) -> EntryPriceResult:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            return EntryPriceResult(
                ...
                base_price=twap,
                market_conditions={
                    'twap_periods': len(prices)
                }
            )
    
    async def _calculate_arrival_price(
        self,
        context: PriceContext
    ) -> EntryPriceResult:
    ...
    async def _validate_price_result(
        self,
        ...
        if context.high_price and context.low_price:
            if not (context.low_price <= result.entry_price <= context.high_price):
                ...
                if (result.entry_price < context.low_price - max_deviation or
                    result.entry_price > context.high_price + max_deviation):
    ...
    def configure_impact_model(
        ...
        logger.info(
            f"Configured impact model: {impact_model}, "
            f"spread factor: {spread_impact_factor}"
        )