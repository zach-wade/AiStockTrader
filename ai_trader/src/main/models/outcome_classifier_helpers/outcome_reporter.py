...
class OutcomeReporter:
    ...
    def generate_classification_report(self, outcomes: List[OutcomeMetrics]) -> Dict[str, Any]:
        ...
        label_percentages = {
            k: (v / total_valid_outcomes) * 100 
            for k, v in label_counts.items()
        } if total_valid_outcomes > 0 else {}

        # 2. Performance Metrics (e.g., average return for successful breakouts)
        successful_breakouts = [o for o in valid_outcomes if o.outcome_label == OutcomeLabel.SUCCESSFUL_BREAKOUT]
        # Filter out None values before calculating mean
        successful_returns_3d = [o.return_3d for o in successful_breakouts if o.return_3d is not None]
        ...
        report = {
            ...
            'classification_success_rate_pct': (total_valid_outcomes / total_outcomes) * 100 if total_outcomes > 0 else 0.0,
            ...
            'performance_metrics': {
                ...
                'successful_breakout_rate_of_valid': (len(successful_breakouts) / total_valid_outcomes) * 100 if total_valid_outcomes > 0 else 0.0,
            ...
            'data_quality_issues_in_classification': {
                'no_data_count': sum(1 for o in outcomes if o.outcome_label == OutcomeLabel.NO_DATA),
                'calculation_error_count': sum(1 for o in outcomes if o.outcome_label == OutcomeLabel.CALCULATION_ERROR)
            },
    ...
    def _get_confidence_bins(self, confidences: List[float]) -> Dict[str, int]:
        ...
        for conf in confidences:
            if conf >= 0.8: bins['0.8-1.0'] += 1
            elif conf >= 0.6: bins['0.6-0.8'] += 1
            elif conf >= 0.4: bins['0.4-0.6'] += 1
            elif conf >= 0.2: bins['0.2-0.4'] += 1
            else: bins['0.0-0.2'] += 1
