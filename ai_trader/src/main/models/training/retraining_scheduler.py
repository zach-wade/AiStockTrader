...
class RetrainingScheduler:
    ...
    async def initialize_components(self, model_registry: ModelRegistry, 
        ...
        self.feature_orchestrator = FeatureOrchestrator(
            data_collector=self.data_orchestrator.collector,
            config=self.config
        )
    ...
    def get_scheduler_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_trained': {
                ...
                for model_id, timestamp in self.last_trained.items()
            },
            'training_in_progress': list(
                model_id for model_id, in_progress in self.training_in_progress.items()
                if in_progress
            )
        }
