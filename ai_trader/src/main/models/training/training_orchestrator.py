...
from main.interfaces.backtesting import (
    IBacktestEngineFactory, BacktestConfig, BacktestMode, BacktestResult
)
...
class ModelTrainingOrchestrator:
    ...
    async def run_full_workflow(self, symbols: List[str], model_types: List[str], fast_mode: bool = False) -> Dict:
        """
    ...
    async def run_hyperparameter_optimization(self, symbols: List[str], model_types: List[str]) -> Dict[str, Dict]:
    ...
    async def run_training(self, symbols: List[str], model_types: List[str], best_params: Dict) -> Dict:
        ...
        target_cols = [col for col in combined_df.columns if col.startswith(('next_', 'up_down_', 'directional_'))]
        ...
        for col in combined_df.columns:
            if col not in non_feature_cols:
                ...
                if pd.api.types.is_numeric_dtype(combined_df[col]):
                    ...
                    logger.debug(f"Excluding non-numeric column: {col} (dtype: {combined_df[col].dtype})")
        ...
        default_target = 'next_1d_return' if 'next_1d_return' in target_cols else target_cols[0] if target_cols else None
        ...
        for model_type in model_types:
            params = best_params.get(model_type, {}) # Use optimized params or empty dict for defaults
    ...
    async def run_backtest_validation(self, models: Dict[str, Any], symbols: List[str], lookback_days: int = 90) -> Dict[str, Dict]:
        """
        ...
        for model_type, model in models.items():
            ...
            try:
                ...
                if self.backtest_factory is None:
                    logger.warning(f"No backtest factory provided, skipping backtest for {model_type}")
                    continue
                ...
                logger.info(f"Backtest completed for {model_type}: Return={backtest_results[model_type]['total_return']:.2%}")
    ...
    async def save_trained_models(self, training_results: Dict[str, Dict], symbols: List[str]) -> None:
        """
