...
class HyperparameterSearch:
    ...
    def _create_objective(self, model_type: str, X: pd.DataFrame, y: pd.Series, metric: str) -> Callable:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            for fold, (train_idx, val_idx) in enumerate(self.cv_tool.purged_kfold_split(X)):
                ...
                if model_type in ['xgboost', 'lightgbm']:
                    ...
                    fit_params['callbacks'] = [optuna.integration.LightGBMPruningCallback(trial, 'binary_logloss')]
    ...
    def _calculate_strategy_returns(self, predictions: np.ndarray, features: pd.DataFrame) -> pd.Series:
    ...
    def run_study(self, model_type: str, X_train: pd.DataFrame, y_train: pd.Series, study_name: Optional[str] = None, metric: str = 'f1') -> optuna.Study:
        """
        ...
        logger.info(f"Starting Optuna study '{study_name}' for model '{model_type}' optimizing for '{metric}'")
    ...
    def grid_search_timeframes(self, model_type: str, 
                             timeframes: List[str] = ['1h', '1d', '1w'],
                             lookback_periods: List[int] = [20, 50, 100]) -> Dict[str, Any]:
        """
        ...
        for timeframe in timeframes:
            for lookback in lookback_periods:
                ...
                X = features.drop(['target', 'returns', 'symbol', 'timestamp'], axis=1, errors='ignore')
                y = features['target']
    ...
    def ensemble_optimization(self, model_types: List[str],
        ...
        def ensemble_objective(trial):
            ...
            ensemble_pred = np.sum([w * p for w, p in zip(normalized_weights, predictions.values())], axis=0)
        ...
        optimal_weights = [w / total_weight for w in best_weights_raw] if total_weight > 0 else [1.0/len(model_types)]*len(model_types)
