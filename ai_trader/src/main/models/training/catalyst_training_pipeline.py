...
class CatalystTrainingPipeline:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.training_config = CatalystTrainingConfig(**self.global_config.get('ml', {}).get('catalyst_training', {}))
    ...
    async def run_training(self, start_date: str, end_date: str, force_regenerate_data: bool = False) -> PipelineResults:
        ...
        try:
            ...
            dataset = await self._prepare_catalyst_dataset(start_date, end_date, force_regenerate_data)
            if dataset is None or len(dataset) < self.training_config.min_training_samples:
                raise ValueError(f"Insufficient training data: {len(dataset) if dataset is not None else 0} samples.")
    ...
    async def _prepare_catalyst_dataset(self, start: str, end: str, force: bool) -> Optional[pd.DataFrame]:
        ...
        if 'error' in stats: raise ValueError(stats['error'])
        return pd.read_parquet(dataset_file)
    ...
    def _prepare_features_and_targets(self, dataset: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
        available_features = [col for col in self.training_config.feature_columns if col in dataset.columns]
    ...
    def _run_centralized_cv(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        ...
        validation_model = RandomForestClassifier(n_estimators=50, random_state=42, class_weight='balanced')
    ...
    def _analyze_feature_importance(self) -> Dict[str, Any]:
        ...
        for specialist in self.ensemble.specialists.values():
            if specialist.is_trained and hasattr(specialist.model, 'feature_importances_'):
                for feature, importance in zip(specialist.feature_columns, specialist.model.feature_importances_):
    ...
    async def _evaluate_final_ensemble(self, dataset: pd.DataFrame) -> Dict[str, Any]:
        ...
        prediction_tasks = [
            self.ensemble.predict_catalyst_outcome(record)
        ...
        if not y_true: return {"error": "No predictions made on test sample."}
