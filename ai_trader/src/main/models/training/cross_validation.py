...
class TimeSeriesCV:
    ...
    def split_by_date(self, X: pd.DataFrame, y: pd.Series,
                     start_date: Optional[datetime] = None,
                     end_date: Optional[datetime] = None) -> List[Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]]:
        """
    ...
    def expanding_window_split(self, X: pd.DataFrame, y: pd.Series,
                             min_train_size: int = None) -> List[Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]]:
        """
        ...
        while True:
            ...
            if len(X_train) > 0 and len(X_test) > 0:
                ...
                logger.info(f"Expanding split {split_num+1}: "
    ...
    def purged_kfold_split(self, X: pd.DataFrame, y: pd.Series,
                          n_folds: int = None) -> List[Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]]:
        """
        ...
        for i in range(n_folds):
            ...
            logger.info(f"Purged fold {i+1}: Train size: {len(X_train)}, "
                       f"Test size: {len(X_test)}")
        
        return splits
    
    def combinatorial_cv(self, X: pd.DataFrame, y: pd.Series,
                        n_splits: int = None,
                        n_test_groups: int = 2) -> List[Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]]:
        """
    ...
    def validate_model(self, model, X: pd.DataFrame, y: pd.Series,
        ...
        for i, (X_train, y_train, X_test, y_test) in enumerate(splits):
            ...
            # Calculate metrics
            fold_results = self._calculate_metrics(
                y_test, y_pred, y_proba, X_test, metrics
            )
    ...
    def _calculate_metrics(self, y_true, y_pred, y_proba, X_test,
