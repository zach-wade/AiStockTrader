...
class DynamicEnsemble(BaseEstimator, ClassifierMixin):
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        ...
        for name, model in self.fitted_models_.items():
            proba = model.predict_proba(X) if hasattr(model, 'predict_proba') else self._to_proba(model.predict(X))
    ...
    def _optimize_initial_weights(self, X: pd.DataFrame, y: pd.Series):
        ...
        for name, model in self.base_models.items():
            ...
            for train_idx, val_idx in self.cv_tool.purged_kfold_split(X):
                ...
                proba = fold_model.predict_proba(X_val) if hasattr(fold_model, 'predict_proba') else self._to_proba(fold_model.predict(X_val))
        ...
        def objective(weights):
            ...
            ensemble_pred = np.sum([w * p for w, p in zip(weights, oos_predictions.values())], axis=0)
        ...
        if result.success:
            ...
            logger.info(f"Optimized initial weights: { {k: f'{v:.2%}' for k, v in self.weights_.items()} }")
        else:
...
class StackingEnsemble(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, base_models: Dict[str, Any], cv_tool: TimeSeriesCV, meta_learner: Optional[Any] = None, **kwargs):
        self.base_models = base_models
...
class BayesianEnsemble(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, base_models: Dict[str, Any], **kwargs):
        ...
        self.weights_ = prior_weights if prior_weights else {name: 1.0 / n_models for name in base_models}
        self.fitted_models_ = {}
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        ...
        ensemble_proba = np.sum([self.weights_[name] * proba for name, proba in predictions.items()], axis=0)
        return ensemble_proba
...
class EnsembleBuilder:
    ...
    def _create_base_models(self) -> Dict[str, Any]:
        ...
        models = {
            ...
            'lightgbm': lgb.LGBMClassifier(random_state=42, verbose=-1, **self.config.get('lightgbm_params', {})),
            'random_forest': RandomForestClassifier(random_state=42, **self.config.get('rf_params', {}))
        }
        return models
