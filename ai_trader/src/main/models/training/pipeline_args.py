...
class PipelineArgs:
    ...
    feature_sets: List[str] = field(default_factory=lambda: ['technical', 'sentiment', 'microstructure'])
    ...
    def validate(self) -> List[str]:
        ...
        valid_features = ['technical', 'sentiment', 'news', 'microstructure', 'cross_sectional', 'options']
...
def create_parser() -> argparse.ArgumentParser:
    ...
    control_group.add_argument('--skip-download', action='store_true',
                              help='Skip data download stage')
    control_group.add_argument('--skip-features', action='store_true',
                              help='Skip feature engineering stage')
    control_group.add_argument('--skip-hyperopt', action='store_true',
                              help='Skip hyperparameter optimization')
    control_group.add_argument('--skip-training', action='store_true',
                              help='Skip model training stage')
    control_group.add_argument('--skip-validation', action='store_true',
                              help='Skip model validation stage')
    control_group.add_argument('--skip-deployment', action='store_true',
                              help='Skip model deployment stage')
    
    # Data parameters
    data_group = parser.add_argument_group('Data Parameters')
    data_group.add_argument('--symbols', type=str, default='AAPL,GOOGL,MSFT,AMZN,TSLA',
                           help='Comma-separated list of symbols')
    data_group.add_argument('--start-date', type=str,
                           help='Start date (YYYY-MM-DD)')
    data_group.add_argument('--end-date', type=str,
                           help='End date (YYYY-MM-DD)')
    data_group.add_argument('--data-sources', type=str, default='yahoo,alpaca',
                           help='Comma-separated list of data sources')
    
    # Feature parameters
    feature_group = parser.add_argument_group('Feature Engineering')
    feature_group.add_argument('--feature-sets', type=str, 
                              default='technical,sentiment,microstructure',
                              help='Comma-separated list of feature sets')
    feature_group.add_argument('--feature-lookback', type=int, default=20,
                              help='Lookback period for features')
    feature_group.add_argument('--no-feature-cache', dest='feature_cache',
                              action='store_false', help='Disable feature caching')
    
    # Model parameters
    model_group = parser.add_argument_group('Model Training')
    model_group.add_argument('--models', type=str, default='xgboost,lightgbm,random_forest',
                            help='Comma-separated list of models to train')
    model_group.add_argument('--target', type=str, default='next_day_return',
                            help='Target variable for prediction')
    model_group.add_argument('--train-split', type=float, default=0.8,
                            help='Training data split ratio')
    model_group.add_argument('--validation-split', type=float, default=0.1,
                            help='Validation data split ratio')
    model_group.add_argument('--random-seed', type=int, default=42,
                            help='Random seed for reproducibility')
    
    # Hyperparameter optimization
    hyperopt_group = parser.add_argument_group('Hyperparameter Optimization')
    hyperopt_group.add_argument('--hyperopt-trials', type=int, default=100,
                               help='Number of hyperparameter optimization trials')
    hyperopt_group.add_argument('--hyperopt-algorithm', type=str, default='tpe',
                               choices=['tpe', 'random', 'anneal'],
                               help='Hyperparameter optimization algorithm')
    hyperopt_group.add_argument('--hyperopt-metric', type=str, default='sharpe_ratio',
                               help='Metric to optimize')
    hyperopt_group.add_argument('--hyperopt-direction', type=str, default='maximize',
                               choices=['maximize', 'minimize'],
                               help='Optimization direction')
    
    # Advanced options
    advanced_group = parser.add_argument_group('Advanced Options')
    advanced_group.add_argument('--parallel-jobs', type=int, default=-1,
                               help='Number of parallel jobs (-1 for all cores)')
    advanced_group.add_argument('--memory-limit', type=int,
                               help='Memory limit in MB')
    advanced_group.add_argument('--gpu', dest='gpu_enabled', action='store_true',
                               help='Enable GPU acceleration')
    advanced_group.add_argument('--debug', dest='debug_mode', action='store_true',
                               help='Enable debug mode with verbose output')
    
    # Output configuration
    output_group = parser.add_argument_group('Output Configuration')
    output_group.add_argument('--output-dir', type=str, default='models/training_runs',
                             help='Output directory for results')
    output_group.add_argument('--experiment-name', type=str,
                             help='Name for this experiment')
    output_group.add_argument('--no-artifacts', dest='save_artifacts',
                             action='store_false', help='Do not save training artifacts')
    
    # Ensemble configuration
    ensemble_group = parser.add_argument_group('Ensemble Configuration')
    ensemble_group.add_argument('--no-ensemble', dest='ensemble_enabled',
                               action='store_false', help='Disable ensemble models')
    ensemble_group.add_argument('--ensemble-method', type=str, default='voting',
                               choices=['voting', 'stacking', 'blending'],
                               help='Ensemble method to use')
    ensemble_group.add_argument('--ensemble-weights', type=str,
                               help='JSON string of model weights for ensemble')
