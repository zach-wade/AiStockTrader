...
class CatalystSpecialistEnsemble:
    ...
    async def predict_catalyst_outcome(self, catalyst_features: Dict[str, Any]) -> Optional[EnsemblePrediction]:
        ...
        try:
            ...
            specialist_predictions = {
                pred.specialist_type: pred for pred in results if pred and pred.confidence >= self.confidence_threshold
            }
            ...
            ensemble_probability = self._calculate_weighted_probability(specialist_predictions, ensemble_weights)
            ...
            recommendation = self._generate_recommendation(ensemble_probability, ensemble_confidence)
    ...
    def _calculate_ensemble_weights(self, preds: Dict[str, CatalystPrediction]) -> Dict[str, float]:
        ...
        return {name: w / total_weight for name, w in raw_weights.items()} if total_weight > 0 else {}
    
    def _calculate_weighted_probability(self, preds: Dict[str, CatalystPrediction], weights: Dict[str, float]) -> float:
    ...
    def _extract_catalyst_types(self, features: Dict[str, Any]) -> List[str]:
        ...
        return [
            name for name in self.specialists.keys() 
            if features.get(f'has_{name}_catalyst', False)
        ]
    ...
    def train_all_specialists(self, training_data: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        ...
        results = {
            name: spec.train(training_data) for name, spec in self.specialists.items()
        }
    ...
    def get_ensemble_status(self) -> Dict[str, Any]:
        ...
        spec_status = {name: {'is_trained': s.is_trained, 'model_version': s.model_version} 
