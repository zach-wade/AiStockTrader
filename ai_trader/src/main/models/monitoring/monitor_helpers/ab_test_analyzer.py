...
class ABTestAnalyzer:
    ...
    def __init__(self, model_registry: "ModelRegistry", performance_calculator: PerformanceCalculator, alert_manager: AlertManager, config: Dict[str, Any]):
        """
        ...
        self.min_predictions_for_ab_test = self.config.get('ml', {}).get('monitoring', {}).get('min_predictions_for_ab_test', 500)
        self.ab_test_improvement_threshold = self.config.get('ml', {}).get('monitoring', {}).get('ab_test_improvement_threshold_pct', 10.0) # 10% MAE improvement
    ...
    async def check_ab_test_results(self):
        ...
        for model_id, candidates in models_with_candidates.items():
            ...
            if not production_model:
                logger.warning(f"No production model found for {model_id} while candidates are active. Skipping A/B test comparison.")
                continue
            
            for candidate in candidates:
                logger.debug(f"Comparing candidate {model_id} v{candidate.version} against production v{production_model.version}.")
                ...
                if await self._compare_models_for_promotion(candidate, production_model):
                    ...
                    await self.model_registry.promote_model(
                        model_id, candidate.version
                    )
                    logger.info(f"Promoted candidate {candidate.version} to production for model {model_id} (won A/B test).")
                    ...
                    logger.debug(f"Candidate {model_id} v{candidate.version} did not outperform production v{production_model.version} sufficiently.")

    async def _compare_models_for_promotion(self, candidate: "ModelVersion", production: "ModelVersion") -> bool:
        """
        ...
        if (candidate_metrics.get('count', 0) < self.min_predictions_for_ab_test or
            production_metrics.get('count', 0) < self.min_predictions_for_ab_test):
            logger.info(f"Insufficient prediction data for A/B test comparison: Candidate has {candidate_metrics.get('count',0)}/{self.min_predictions_for_ab_test}, Production has {production_metrics.get('count',0)}/{self.min_predictions_for_ab_test}.")
            return False
        ...
        if not np.isfinite(candidate_mae) or not np.isfinite(production_mae) or production_mae <= 0:
            logger.warning(f"Invalid MAE values for A/B comparison. Candidate: {candidate_mae}, Production: {production_mae}. Skipping comparison.")
            return False
        ...
        logger.debug(f"A/B Test: Model {candidate.model_id} (v{candidate.version} MAE: {candidate_mae:.4f}) vs Production (v{production.version} MAE: {production_mae:.4f}). Improvement: {improvement_pct:.2f}%. Threshold: {self.ab_test_improvement_threshold:.2f}%. Candidate wins: {is_winner}.")
