...
class DriftDetector:
    ...
    def calculate_prediction_drift(self, prediction_history: List[float], min_predictions_for_drift: int) -> Tuple[float, Optional[float]]:
        """
        ...
        """
        if len(prediction_history) < min_predictions_for_drift * 2: # Need at least two windows of min_predictions
            logger.debug(f"Insufficient prediction history ({len(prediction_history)} < {min_predictions_for_drift * 2}) for drift calculation.")
            return 0.0, None
        ...
        if not reference_data or not current_data or \
           (np.std(reference_data) == 0 and np.std(current_data) == 0):
            logger.warning("Prediction history for drift is empty or constant. Cannot perform KS test.")
            return 0.0, None

        try:
            # Kolmogorov-Smirnov test for distribution drift
            ks_statistic, p_value = stats.ks_2samp(reference_data, current_data)
            logger.debug(f"Prediction drift KS statistic: {ks_statistic:.4f}, p-value: {p_value:.4f}.")
    ...
    def check_feature_drift(self, model_id: str, feature_history_data: Dict[str, List[Any]], 
                            model_features_list: List[str], min_points_for_drift: int) -> Dict[str, float]:
        """
        ...
        for feature_name in model_features_list:
            ...
            history = feature_history_data.get(feature_key, []) # Get feature history for this specific feature

            if len(history) < min_points_for_drift * 2:
                logger.debug(f"Insufficient history ({len(history)}) for feature '{feature_name}' drift check.")
                continue
            ...
            if not reference_data or not current_data or \
               (isinstance(reference_data[0], (int, float)) and np.std(reference_data) == 0 and np.std(current_data) == 0):
                logger.debug(f"Feature '{feature_name}' data is empty or constant. Skipping drift check.")
                continue
            ...
            try:
                ...
                if ks_statistic > self.drift_threshold:
                    ...
                    logger.warning(f"Drift detected for feature '{feature_name}' of model {model_id}: KS={ks_statistic:.4f}, p={p_value:.4f}")
            except Exception as e:
                logger.error(f"Error calculating drift for feature '{feature_name}' of model {model_id}: {e}", exc_info=True)
