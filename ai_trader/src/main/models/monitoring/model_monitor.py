...
class ModelMonitor(ErrorHandlingMixin):
    ...
    def __init__(self, config: Any, model_registry: ModelRegistry, 
                 prediction_engine: PredictionEngine, alert_manager: AlertManager):
        """
        ...
        self.prediction_engine = prediction_engine # Not used directly in loop, but for recording predictions
        self.alert_manager = alert_manager
        
        # Monitoring configuration values
        self.monitoring_interval = self.config.get('ml', {}).get('monitoring', {}).get('interval_seconds', 300)
        self.drift_threshold = self.config.get('ml', {}).get('monitoring', {}).get('drift_threshold', 0.05)
        self.performance_window_hours = self.config.get('ml', {}).get('monitoring', {}).get('performance_window_hours', 24)
        self.min_predictions_for_analysis = self.config.get('ml', {}).get('monitoring', {}).get('min_predictions', 100)
        
        # Initialize helper components
        self._data_collector = PredictionDataCollector(
            history_maxlen=self.config.get('ml', {}).get('monitoring', {}).get('prediction_history_maxlen', 10000),
            feature_history_maxlen=self.config.get('ml', {}).get('monitoring', {}).get('feature_history_maxlen', 1000)
        )
    ...
    async def _monitoring_loop(self):
        ...
        logger.info(f"Model monitoring loop started with interval: {self.monitoring_interval} seconds.")
        while self._running:
            try:
                ...
                self.handle_error(f"Critical error in model monitoring loop: {e}", level="critical", exc_info=True)
                await asyncio.sleep(60) # Wait before retrying loop
    
    async def _monitor_single_model_workflow(self, model_id: str, model_version: Any): # Use Any for ModelVersion type
        """
        ...
        try:
            ...
            if not recent_predictions_for_perf or not all(p.get('actual') is not None for p in recent_predictions_for_perf):
                logger.debug(f"Insufficient recent predictions with actuals for performance monitoring for {model_id}.")
                return # Skip if not enough data with actuals

            current_performance = self._performance_calculator.calculate_metrics(recent_predictions_for_perf)
            
            # Update ModelVersion metrics (EMA is handled by PerformanceTracker helper)
            self.model_registry.update_performance(model_id, model_version.version, current_performance)
            
            # 2. Check for Performance Degradation
            baseline_mae = model_version.metrics.get('mae', float('inf')) # Use ModelVersion's EMA-updated MAE as baseline
            current_mae = current_performance.get('mae', float('inf'))
            
            if self._performance_calculator.check_performance_degradation(current_mae, baseline_mae, self.config.get('ml.monitoring.degradation_threshold_mae_pct', 0.20)):
                await self._mlops_action_manager.handle_performance_degradation(model_id, current_performance)
            
            # 3. Check for Prediction Drift
            prediction_values_history = [p['prediction'] for p in self._data_collector.prediction_history[model_id] if p.get('prediction') is not None]
            drift_score_ks, _p_value = self._drift_detector.calculate_prediction_drift(prediction_values_history, self.min_predictions_for_analysis)
            ...
            if drift_score_ks > self.drift_threshold:
                ...
                await self._mlops_action_manager.handle_prediction_drift_detected(model_id, drift_score_ks)
            ...
            drifted_features = self._drift_detector.check_feature_drift(
                model_id, self._data_collector.feature_history, model_features_list, self.min_predictions_for_analysis
            )
            ...
            if drifted_features:
                ...
                self.model_registry._save_registry_state() # Persist metadata update
                await self._mlops_action_manager.handle_feature_drift_detected(model_id, drifted_features)
            
        except Exception as e:
            self.handle_error(f"Error during monitoring workflow for model {model_id}: {e}", level="error", exc_info=True)
