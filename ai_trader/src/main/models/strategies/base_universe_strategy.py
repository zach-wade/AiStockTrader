...
class BaseUniverseStrategy(BaseStrategy):
    ...
    async def generate_signals_for_universe(
        self,
        ranked_universe: Dict[str, float]
    ) -> List[Signal]:
        """
    ...
    async def _update_universe(self, timestamp: datetime) -> None:
        ...
        # Apply filters
        filtered_universe, stats = await self._apply_universe_filters(
            all_symbols,
            timestamp
        )
        ...
        if self.metrics:
            self.metrics.gauge(
                f"strategy.{self.name}.universe_size",
                stats.filtered_symbols
            )
        ...
        for symbol in symbols:
            ...
            try:
                ...
                if ref_data:
                    ...
                    if market_cap:
                        if self.universe_filter.min_market_cap and market_cap < self.universe_filter.min_market_cap:
                            filter_reasons['mcap_too_low'] = filter_reasons.get('mcap_too_low', 0) + 1
                            continue
                            
                        if self.universe_filter.max_market_cap and market_cap > self.universe_filter.max_market_cap:
                            filter_reasons['mcap_too_high'] = filter_reasons.get('mcap_too_high', 0) + 1
                            continue
                    ...
                    if self.universe_filter.exchanges and exchange not in self.universe_filter.exchanges:
                        filter_reasons['wrong_exchange'] = filter_reasons.get('wrong_exchange', 0) + 1
                        continue
                        
                    # Sector filter
                    sector = ref_data.get('sector')
                    if sector:
                        if self.universe_filter.sectors and sector not in self.universe_filter.sectors:
                            filter_reasons['wrong_sector'] = filter_reasons.get('wrong_sector', 0) + 1
                            continue
                            
                        if self.universe_filter.exclude_sectors and sector in self.universe_filter.exclude_sectors:
                            filter_reasons['excluded_sector'] = filter_reasons.get('excluded_sector', 0) + 1
                            continue
    ...
    def _track_execution(self, timestamp: datetime, signal_count: int) -> None:
        ...
        self.event_tracker.track("execution", {
            "timestamp": timestamp.isoformat(),
            "universe_size": len(self._current_universe),
            "signal_count": signal_count,
            "top_symbol": self.get_top_ranked_symbols(1)[0][0] if self._universe_ranks else None
        })
        
        if self.metrics:
            self.metrics.increment(
                f"strategy.{self.name}.executions",
                tags={"signal_count": str(signal_count)}
            )