...
class PairsTradingStrategy(BaseUniverseStrategy):
    ...
    def __init__(self, config: Dict[str, Any], feature_engine: UnifiedFeatureEngine):
        ...
        logger.info(f"PairsTradingStrategy initialized with {len(self.tradable_pairs)} pre-calculated pairs.")
    ...
    async def generate_universe_signals(self, market_features: Dict[str, pd.DataFrame], portfolio_state: Dict) -> List[Signal]:
        ...
        for pair_info in self.tradable_pairs:
            ...
            if len(price_series1) < self.lookback_period or len(price_series2) < self.lookback_period:
