...
class MLModelStrategy(BaseStrategy):
    ...
    def __init__(self, model_path: str, config: Dict[str, Any], feature_engine=None):
        ...
        self.min_confidence = config.get('ml_strategy', {}).get('min_confidence', 0.3)  # Lower for testing
        self.max_position_size = config.get('ml_strategy', {}).get('max_position_size', 100)  # 100 shares for testing
    ...
    def get_required_feature_sets(self) -> List[str]:
        ...
        if self.feature_columns:
            ...
            if any('fundamental' in col.lower() or 'financial' in col.lower() 
    ...
    def _prediction_to_signal(self, symbol: str, prediction: Dict[str, float], 
    ...
    async def on_market_data(self, event) -> List[Order]:
        ...
        for signal in signals:
            if signal.action in ['buy', 'sell']:
                ...
                orders.append(order)
                logger.info(f"Created {signal.action} order for {signal.quantity} shares of {symbol}")
