...
class FinalSentimentStrategy(BaseStrategy):
    ...
    async def generate_signals(self, symbol: str, features: pd.DataFrame, current_position: Optional[Dict]) -> List[Signal]:
        ...
        # Bullish Case
        if blended_sentiment > self.sentiment_threshold and volume_ratio > self.volume_spike_threshold:
        ...
        # Bearish Case
        elif blended_sentiment < -self.sentiment_threshold and volume_ratio > self.volume_spike_threshold:
