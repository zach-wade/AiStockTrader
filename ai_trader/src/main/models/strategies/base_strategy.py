...
class BaseStrategy(ABC):
    ...
    async def execute(self, symbol: str, data: pd.DataFrame, current_position: Optional[Dict] = None) -> List[Signal]:
        """
        ...
        try:
            ...
            logger.error(f"FATAL: Error executing strategy {self.name} for {symbol}: {e}", exc_info=True)
            return []
    ...
    def _prepare_features(self, symbol: str, data: pd.DataFrame) -> pd.DataFrame:
        ...
        return self.feature_engine.calculate_features(
            data=data,
            symbol=symbol,
            calculators=required_calculators,
            use_cache=True
        )
