...
class BreakoutStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any], feature_engine: UnifiedFeatureEngine):
        ...
        self.consolidation_range_max_pct = strategy_conf.get('consolidation_range_max_pct', 0.05) # Refactored
    ...
    async def generate_signals(self, symbol: str, features: pd.DataFrame, current_position: Optional[Dict]) -> List[Signal]:
        ...
        try:
            ...
            if is_price_breakout_up and is_volume_breakout and (not current_position or current_position.get('direction') != 'long'):
            ...
            if is_price_breakout_down and is_volume_breakout and (not current_position or current_position.get('direction') != 'short'):
                confidence = min(1.0, (current_volume / avg_volume - 1.0) * 0.5 + 0.5)
                ...
                logger.info(f"Generated downward breakout signal for {symbol} at {current_price:.2f}")
                return [signal]
