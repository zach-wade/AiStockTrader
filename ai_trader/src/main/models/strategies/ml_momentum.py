...
class MLMomentumStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any], feature_engine: UnifiedFeatureEngine):
        ...
        self.momentum_features = strategy_conf.get('momentum_features', [
            'price_momentum', 'volume_momentum', 'rsi_divergence'
        ])
    ...
    def _load_model(self):
        ...
        try:
            ...
            if model_info:
                ...
                model_path = Path(self.config.get('models', {}).get('path', 'models')) / f"{self.model_name}.pkl"
    ...
    def _prepare_ml_features(self, features: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            for period in self.lookback_periods:
                if len(features) > period:
                    ...
                    if f'rsi_{period}' in features.columns:
                        price_change = features['close'].iloc[-1] / features['close'].iloc[-period-1] - 1
                        rsi_change = features[f'rsi_{period}'].iloc[-1] - features[f'rsi_{period}'].iloc[-period-1]
                        latest_features[f'rsi_divergence_{period}'] = price_change - (rsi_change / 100)
            ...
            if 'market_regime' in features.columns:
                latest_features['regime_bullish'] = int(features['market_regime'].iloc[-1] == 'bullish')
                latest_features['regime_bearish'] = int(features['market_regime'].iloc[-1] == 'bearish')
