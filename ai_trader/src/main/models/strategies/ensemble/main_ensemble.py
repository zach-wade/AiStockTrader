...
class AdvancedStrategyEnsemble(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Advanced Strategy Ensemble initialized with {len(self.strategies)} strategies.")
    ...
    async def generate_signals(self, symbol: str, features: pd.DataFrame) -> List[Signal]:
        ...
        try:
            ...
            raw_signals = {
                name: res for name, res in zip(self.strategies.keys(), signal_results)
                if isinstance(res, list)
            }

            # 4. Delegate aggregation to the SignalAggregator
            ensemble_signals = self.aggregator.aggregate(raw_signals, self.strategy_weights, symbol, regime)
