...
class ScannerOrchestrator(IScannerOrchestrator):
    ...
    def __init__(
        ...
        self._cache_cleanup_time = datetime.now(timezone.utc)
    ...
    async def register_scanner(
        ...
        if self.metrics:
            self.metrics.increment(
                "orchestrator.scanners_registered",
    ...
    async def unregister_scanner(self, scanner_name: str) -> None:
        ...
        if scanner_name in self._scanners:
            ...
            if self.metrics:
                self.metrics.increment(
                    "orchestrator.scanners_unregistered",
                    tags={"scanner": scanner_name}
                )
        else:
        ...
        try:
            with timer() as t:
                ...
                if self.metrics:
                    ...
                    self.metrics.record(
                        "orchestrator.alerts_generated",
                        len(aggregated_alerts)
                    )
    ...
    async def get_scanner_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, registration in self._scanners.items():
            ...
            status[name] = {
                ...
                "last_scan_time": (
                    registration.last_scan_time.isoformat() 
                    if registration.last_scan_time else None
                ),
                "config": {
    ...
    def _get_scanners_to_use(
        ...
        if scanner_names:
            ...
            scanners = [
                reg for name, reg in self._scanners.items()
                if name in scanner_names and reg.enabled
            ]
        else:
            # Use all enabled scanners
            scanners = [
                reg for reg in self._scanners.values()
                if reg.enabled
            ]
    ...
    async def _scan_parallel(
        ...
        for name, reg in self._scanners.items():
            if name in scanner_results:
                reg.success_count += 1
                reg.last_scan_time = datetime.now(timezone.utc)
        ...
        for registration in scanners:
            ...
            try:
                ...
                alerts = await asyncio.wait_for(
                    registration.scanner.scan(universe),
                ...
                registration.last_scan_time = datetime.now(timezone.utc)
    ...
    async def _cleanup_scanner(self, registration: 'ScannerRegistration') -> None:
        ...
        try:
            ...
            logger.error(
                f"Error cleaning up scanner '{registration.config.name}': {e}"
            )