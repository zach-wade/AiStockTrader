...
class PipelineMonitor:
    ...
    def record_layer_start(self, layer_name: str, input_count: int):
        ...
        self.layer_times[f"{layer_name}_start"] = datetime.now(timezone.utc).timestamp()
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'layer_durations': {k: v for k, v in self.layer_times.items() if k.endswith('_duration')},
...
class DataQualityChecker:
    ...
    @staticmethod
    async def check_market_data_freshness(db_adapter: IAsyncDatabase, 
                                        symbols: List[str],
...
<body>
    ...
    {errors_section}
        ...
        # Calculate metrics
        initial_symbols = pipeline_result['layer_results'][0]['output_count'] if pipeline_result.get('layer_results') else 0
        ...
        # Save to file
        output_file = output_path / f"pipeline_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
...
def estimate_remaining_pipeline_time(completed_layers: List[str],
                                   historical_times: Dict[str, float]) -> float:
    """
