...
class Layer0StaticUniverseScanner:
    ...
    def __init__(self, config: DictConfig, event_bus: IEventBus = None):
        ...
        self.company_repository: ICompanyRepository = repo_factory.create_company_repository(self.db_adapter)
        ...
        self.excluded_exchanges = self.filter_config.get('excluded_exchanges', ['OTC', 'OTCBB', 'PINK', 'GREY'])
        self.excluded_patterns = self.filter_config.get('excluded_patterns', ['W', 'RT', 'CV', 'Q']) # Warrants, Rights, Convertibles, Bankrupt
        self.included_asset_classes = self.filter_config.get('included_asset_classes', ['us_equity'])
    ...
    async def run(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Database population result: {companies_result.processed_records} companies processed, success: {companies_result.success}")
            ...
            logger.info(f"✅ Layer 0 complete in {duration:.2f}s. Found {len(final_symbols)} tradable symbols.")
    ...
    async def _populate_companies_table(self, assets_data: List[Dict[str, Any]]):
        ...
        return await self.company_repository.bulk_upsert(
            ...
            update_fields=['name', 'exchange', 'quote_type', 'is_active', 'layer']  # Update these fields on conflict
        )
        ...
        try:
            ...
            for symbol in symbols:
                result = await self.company_repository.update_layer(
                    ...
                    metadata={'source': 'layer0_scanner', 'timestamp': datetime.now(timezone.utc).isoformat()}
                )
