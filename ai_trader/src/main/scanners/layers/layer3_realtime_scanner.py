...
from main.scanners.layers.realtime_websocket_stream import (
    WebSocketDataStream, RealtimeQuote, RealtimeTrade
)
...
class Layer3RealtimeScanner:
    ...
    async def scan(self, symbols: List[str]) -> List[RealtimeOpportunity]:
        ...
        try:
            ...
            logger.info(f"Real-time scan completed in {duration:.3f}s - {len(qualified)} opportunities")
    ...
    async def _load_previous_closes(self, symbols: List[str]):
        ...
        for symbol in symbols:
            ...
            # Cache the result
            await self.cache.set(CacheType.CUSTOM, f"prev_close:{symbol}", close_price, 86400)  # 24 hours
    ...
    async def _identify_opportunities(self) -> List[RealtimeOpportunity]:
        ...
        for symbol in self.tracked_symbols:
            try:
                ...
                catalyst_score = await self.cache.get(CacheType.SIGNALS, f"catalyst_score:{symbol}") or 0
    ...
    def _calculate_opportunity_score(self, rvol: float, price_change_pct: float,
                                   spread_bps: float, momentum_score: float,
    ...
    def _rank_opportunities(self, opportunities: List[RealtimeOpportunity]) -> List[RealtimeOpportunity]:
        ...
        while True:
            try:
                ...
                for symbol in list(self.quote_buffer.keys()):
                    # Remove old quotes
                    while self.quote_buffer[symbol] and self.quote_buffer[symbol][0].timestamp < cutoff:
                        self.quote_buffer[symbol].popleft()
                
                for symbol in list(self.trade_buffer.keys()):
                    # Remove old trades
                    while self.trade_buffer[symbol] and self.trade_buffer[symbol][0].timestamp < cutoff:
