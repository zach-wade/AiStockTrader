...
from main.utils.scanners import ScannerMetricsCollector, ScannerCacheManager

# Import event system
from main.events.core import EventBusFactory
from main.interfaces.events import ScannerAlertEvent, IEventBus
from main.events.handlers import ScannerFeatureBridge
from main.events.types import ScanAlert, AlertType
from main.scanners.scanner_adapter import ScannerAdapter
from main.data_pipeline.validation.core.validation_pipeline import ValidationPipeline
from main.data_pipeline.validation.core.validation_types import ValidationStage
...
class Layer2CatalystOrchestrator:
    ...
    def __init__(self, config: DictConfig, db_adapter=None, event_bus: IEventBus = None):
        ...
        self.event_bus = event_bus or (EventBusFactory.create(self.config) if 'event_bus' in self.config else EventBusFactory.create_test_instance())
        ...
        logger.info(f"Layer2CatalystOrchestrator initialized with scanner factory and validation pipeline.")
    ...
    async def run(self, input_universe: List[str]) -> Dict[str, Any]:
        ...
        qualified_symbols = {
            symbol: data for symbol, data in aggregated_signals.items()
            if data.get('final_score', 0) >= self.min_final_score
        }
        
        # 4. Sort by score and prepare final output
        sorted_symbols = sorted(qualified_symbols.items(), key=lambda item: item[1]['final_score'], reverse=True)
        ...
        logger.info(f"✅ Layer 2 Scan complete. Found {len(final_output['symbols'])} symbols with significant catalysts.")
    ...
    def _aggregate_scanner_results(self, scanner_results: List[Any]) -> Dict[str, Any]:
        ...
        for idx, result_set in enumerate(scanner_results):
            ...
            for alert in alerts:
                ...
                alert_score = (alert.score * self.config.get('orchestrator.catalyst_scoring.scaling.alert_score_multiplier', 5.0)) if alert.score else 1.0  # Convert 0-1 to 0-5 scale
        ...
        for symbol, data in sorted_symbols:
            try:
                ...
                alert_event = ScannerAlertEvent(
                    ...
                    score=data['final_score'] / self.config.get('orchestrator.catalyst_scoring.scaling.score_normalization_divisor', 10.0),  # Normalize to 0-1
                ...
                logger.debug(f"Published catalyst event for {symbol} (score: {data['final_score']})")
    ...
    async def process_alerts(self, layer1_alerts: List[ScanAlert]) -> List[ScanAlert]:
        ...
        logger.info(f"Processing {len(layer1_alerts)} Layer 1 alerts through Layer 2 catalyst analysis")
        ...
        try:
            ...
            if alerts_data:
                ...
                if validation_result.has_warnings:
                    logger.warning(f"Layer 1 alerts validation warnings: {validation_result.warnings}")
        ...
        for symbol in catalyst_results['symbols']:
            ...
            alert = ScanAlert(
                ...
                metadata={
                    ...
                    'sub_scores': {
                        k: v for k, v in catalyst_data.items() 
                        if k.endswith('_details') or k == 'final_score'
                    },
    ...
    def _determine_primary_catalyst(self, catalyst_data: Dict[str, Any]) -> str:
        ...
        # Options activity
        if any('options' in reason.lower() or 'unusual flow' in reason.lower() for reason in reasons):
        ...
        if any('coordinated' in reason.lower() or 'correlated' in reason.lower() for reason in reasons):
            return AlertType.COORDINATED_ACTIVITY
        
        # Default to momentum if we have strong signals
        if catalyst_data.get('final_score', 0) >= self.config.get('orchestrator.catalyst_scoring.thresholds.strong_signal_threshold', 5.0):
            return AlertType.MOMENTUM
        
        # Otherwise, potential breakout
        return AlertType.BREAKOUT
    
    def _calculate_combined_confidence(self, catalyst_data: Dict[str, Any], 
                                     layer1_alerts: List[Dict[str, Any]]) -> float:
        """
        ...
        layer2_confidence = min(catalyst_data.get('final_score', 0) / self.config.get('orchestrator.catalyst_scoring.scaling.score_normalization_divisor', 10.0), 1.0)
        
        # Average Layer 1 scores
        if layer1_alerts:
            layer1_scores = [alert.get('score', 0.5) for alert in layer1_alerts if alert.get('score')]
        ...
        combined = (self.config.get('orchestrator.catalyst_scoring.weighting.layer2_confidence_weight', 0.7) * layer2_confidence) + (self.config.get('orchestrator.catalyst_scoring.weighting.layer1_confidence_weight', 0.3) * layer1_confidence)
        
        # Boost for multiple confirming signals
        signal_count = len(catalyst_data.get('reasons', [])) + len(layer1_alerts)
        if signal_count >= self.config.get('orchestrator.catalyst_scoring.thresholds.signal_boost_threshold_high', 5):
            combined = min(combined * self.config.get('orchestrator.catalyst_scoring.weighting.high_signal_boost_factor', 1.2), 1.0)
        elif signal_count >= self.config.get('orchestrator.catalyst_scoring.thresholds.signal_boost_threshold_medium', 3):
            combined = min(combined * self.config.get('orchestrator.catalyst_scoring.weighting.medium_signal_boost_factor', 1.1), 1.0)
        
        return round(combined, 3)
    
    async def _check_emerging_catalysts(self, symbols: List[str], 
                                      catalyst_results: Dict[str, Any],
                                      layer1_context: Dict[str, List[Dict[str, Any]]]) -> List[ScanAlert]:
        """
        ...
        for symbol in symbols:
            ...
            if (catalyst_data.get('final_score', 0) >= self.config.get('orchestrator.catalyst_scoring.thresholds.emerging_catalyst_threshold', 1.5) and  # Some catalyst activity
                len(layer1_alerts) >= self.config.get('orchestrator.catalyst_scoring.thresholds.min_layer1_signals', 2)):  # Multiple Layer 1 signals
                
                alert = ScanAlert(
                    symbol=symbol,
                    alert_type=AlertType.CONSOLIDATION,  # Emerging catalyst
                    timestamp=datetime.now(),
                    score=self.config.get('orchestrator.catalyst_scoring.scaling.emerging_score_base', 0.4) + (catalyst_data.get('final_score', 0) / self.config.get('orchestrator.catalyst_scoring.scaling.emerging_score_divisor', 20.0)),  # Lower scores
    ...
    def _initialize_dependencies(self):
        ...
        self.metrics_collector = ScannerMetricsCollector() if self.config.get('scanners.global.enable_metrics', True) else None
        self.cache_manager = ScannerCacheManager(
            default_ttl_seconds=self.config.get('scanners.global.cache_ttl_seconds', 60)
        ) if self.config.get('scanners.global.enable_caching', True) else None
    ...
    async def _async_initialize_scanners(self):
        ...
        if self.scanners and self.config.get('scanners.global.use_priority_ordering', True):
            self.scanners.sort(
                key=lambda s: self.config.get(f'scanners.{s.name.lower().replace("scanner", "")}.priority', 5),
                reverse=True
            )
