...
class Layer1LiquidityFilter:
    ...
    async def run(self, input_symbols: List[str]) -> List[str]:
        ...
        try:
            ...
            if not liquidity_data:
                ...
                logger.error("  1. The market_data view doesn't exist (run create_market_data_views.sql)")
                logger.error("  2. No market data has been loaded yet (run backfill)")
                logger.error("  3. The date range has no data")
                raise ValueError("Cannot run Layer 1 scanner without market data. Please ensure market data is loaded.")
            
            logger.info(f"Successfully calculated liquidity metrics for {len(liquidity_data)} symbols.")
            ...
            sorted_symbols_data = sorted(filtered_symbols_data, key=lambda x: x.get('liquidity_score', 0), reverse=True)
            ...
            logger.info(f"✅ Layer 1 complete in {duration:.2f}s. Final universe size: {len(final_symbols)}")
        ...
        try:
            ...
            for row in results:
                liquidity_data.append({
                    'symbol': row['symbol'],
                    'avg_dollar_volume': float(row['avg_dollar_volume']) if row['avg_dollar_volume'] else 0,
                    'avg_price': float(row['avg_price']) if row['avg_price'] else 0,
                    'avg_volume': float(row['avg_volume']) if row['avg_volume'] else 0,
                    'data_points': row['data_points'],
                    'max_dollar_volume': float(row['max_dollar_volume']) if row['max_dollar_volume'] else 0,
            ...
            if "relation \"market_data\" does not exist" in str(e):
                logger.error("The market_data view is missing. Please run: create_market_data_views.sql")
            elif "column" in str(e).lower():
                logger.error("Database schema mismatch. The market_data view may be outdated.")
            return []
    
    
    def _apply_liquidity_filters(self, liquidity_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _update_company_qualifications(
        ...
        try:
            ...
            company_repo: ICompanyRepository = repo_factory.create_company_repository(self.db_adapter)
