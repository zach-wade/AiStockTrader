...
from main.utils.core import (
    create_event_tracker, create_task_safely, chunk_list,
    async_retry
)
...
class ParallelScannerEngine:
    ...
    async def scan_symbols(
        self,
        symbols: List[str],
        scanner_filter: Optional[Callable[[str], bool]] = None,
        **kwargs
    ) -> List[ScanAlert]:
        """
        ...
        active_scanners = [
            config for name, config in self._scanners.items()
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                if result.error:
                    self._update_metrics(result.scanner_name, success=False, error=str(result.error))
                else:
        ...
        if self.metrics:
            ...
            if duration is not None:
                self.metrics.histogram(
                    "scanner_engine.scan_duration",
                    duration,
                    tags={"scanner": scanner_name}
                )
                
    def get_scanner_metrics(self, scanner_name: Optional[str] = None) -> Dict[str, ScannerMetrics]:
    ...
    def _track_execution(
        ...
        self.event_tracker.track("parallel_scan", {
