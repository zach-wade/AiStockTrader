...
class ScannerPipeline:
    ...
    def __init__(self, config: Optional[DictConfig] = None, test_mode: bool = False):
        """
        ...
        if self.test_mode:
            ...
            logger.info(f"Running in TEST MODE - limiting to {self.test_symbols_limit} symbols per layer")
    ...
    async def _run_layer0(self) -> LayerResult:
        ...
        try:
            ...
            if self.test_mode and len(symbols) > self.test_symbols_limit:
                logger.info(f"Test mode: Limiting Layer 0 output from {len(symbols)} to {self.test_symbols_limit}")
    ...
    async def _run_layer1(self, input_symbols: List[str]) -> LayerResult:
        ...
        try:
            ...
            # Apply test mode limit
            if self.test_mode and len(symbols) > self.test_symbols_limit:
                logger.info(f"Test mode: Limiting Layer 1 output from {len(symbols)} to {self.test_symbols_limit}")
                symbols = symbols[:self.test_symbols_limit]
    ...
    async def _run_layer1_5(self, input_symbols: List[str]) -> LayerResult:
        ...
        try:
            ...
            # Apply test mode limit
            if self.test_mode and len(symbols) > self.test_symbols_limit:
                logger.info(f"Test mode: Limiting Layer 1.5 output from {len(symbols)} to {self.test_symbols_limit}")
                symbols = symbols[:self.test_symbols_limit]
    ...
    async def _run_layer2(self, input_symbols: List[str]) -> LayerResult:
        ...
        try:
            ...
            # Apply test mode limit
            if self.test_mode and len(symbols) > self.test_symbols_limit:
                logger.info(f"Test mode: Limiting Layer 2 output from {len(symbols)} to {self.test_symbols_limit}")
                symbols = symbols[:self.test_symbols_limit]
    ...
    async def _save_results(self, result: PipelineResult):
        ...
        for layer in result.layer_results:
            result_dict['layer_results'].append({
                ...
                'errors': layer.errors,
                'symbols': layer.symbols[:10] if self.test_mode else layer.symbols  # Limit symbols in test mode
            })
