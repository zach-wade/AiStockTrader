...
class SectorScanner(CatalystScannerBase):
    ...
    def __init__(
        ...
        self.rotation_threshold = self.params.get('rotation_threshold', 2.0)  # 2% relative performance
        self.relative_strength_threshold = self.params.get('relative_strength_threshold', 1.0)  # 1% vs SPY
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    ...
                    if cached_alerts is not None:
                        logger.info(f"Using cached results for sector scan")
                ...
                logger.info(
                    f"✅ Sector Scanner: Found {len(alerts)} signals "
        ...
        for alert in alerts:
            signal = {
                ...
                'metadata': {
                    ...
                    'economic_cycle_alignment': alert.metadata.get('economic_cycle_alignment', False)
                }
            }
    ...
    async def _process_symbol_sector_data(
        ...
        if rotation_type != 'neutral' and self._is_sector_aligned_with_rotation(sector, rotation_type):
            score += 0.2
