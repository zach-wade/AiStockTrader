...
class InsiderScanner(CatalystScannerBase):
    ...
    async def scan(self, symbols: List[str], **kwargs) -> List[ScanAlert]:
        ...
        with timer() as t:
            logger.info(f"👤 Insider Scanner: Analyzing {len(symbols)} symbols for insider activity...")
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    cache_key = f"insider_scan:{','.join(sorted(symbols[:10]))}:{self.lookback_days}"
                    ...
                    if cached_alerts is not None:
                        logger.info(f"Using cached results for insider scan")
                ...
                for symbol, transactions in insider_data.items():
                    ...
                    symbol_alerts = await self._process_symbol_insider_data(
                        symbol,
                        transactions
                    )
    ...
    def _analyze_transactions(self, transactions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        sorted_transactions = sorted(
            transactions,
            key=lambda x: x.get('transaction_date', datetime.min),
            reverse=True
        )
        ...
        for txn in sorted_transactions:
            ...
            if txn_date and (datetime.now(timezone.utc) - txn_date).days <= 30:
