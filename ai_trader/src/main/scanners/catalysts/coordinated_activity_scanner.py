...
class CoordinatedActivityScanner(CatalystScannerBase):
    ...
    async def initialize(self) -> None:
        ...
        if not SKLEARN_AVAILABLE:
            logger.warning("Scikit-learn not installed. CoordinatedActivityScanner requires sklearn for clustering.")
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    ...
                    if cached_alerts is not None:
                        logger.info(f"Using cached results for coordinated activity scan")
    ...
    async def run(self, universe: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for alert in alerts:
            catalyst_signals[alert.symbol] = {
                ...
                'metadata': {
                    'cluster_authors': alert.metadata.get('cluster_authors', [])
                }
            }
        
        return catalyst_signals

    def _convert_social_data_to_df(self, social_data: Dict[str, List[Dict[str, Any]]]) -> pd.DataFrame:
    ...
    def _build_author_network(self, posts_df: pd.DataFrame) -> tuple[nx.Graph, pd.DataFrame]:
        ...
        author_features = posts_df.groupby('author').agg({
    ...
    def _get_promoted_symbols(self, clusters: List[List[str]], posts_df: pd.DataFrame) -> Dict[str, Dict]:
        ...
        author_to_cluster = {author: i for i, member_list in enumerate(clusters) for author in member_list}
        ...
        symbol_counts = cluster_mentions.groupby(['symbol', 'cluster_id']).size().reset_index(name='mention_count')

        # Vectorized scoring calculation
        symbol_counts['cluster_size'] = symbol_counts['cluster_id'].apply(lambda x: len(clusters[x]))
        symbol_counts['score'] = (symbol_counts['cluster_size'] * 0.5) + (symbol_counts['mention_count'] * 0.2)
