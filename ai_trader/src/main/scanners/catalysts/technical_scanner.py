...
class TechnicalScanner(CatalystScannerBase):
        ...
        with timer() as t:
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    cache_key = f"technical_scan:{','.join(sorted(symbols[:10]))}:{self.lookback_days}"
                    ...
                    if cached_alerts is not None:
                        logger.info(f"Using cached results for technical scan")
    ...
    async def _check_breakout(self, symbol: str, df: pd.DataFrame, volume_stats: Optional[Dict]) -> List[ScanAlert]:
