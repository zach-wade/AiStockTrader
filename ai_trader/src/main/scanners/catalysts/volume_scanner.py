        ...
        with timer() as t:
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    cached_alerts = await self.cache.get(
                        self.name,
                        symbols,
                        {'key': cache_key}
                    )
                    if cached_alerts is not None:
                        logger.info(f"Using cached results for volume scan")
                ...
                async def process_symbol(symbol: str) -> Optional[ScanAlert]:
                ...
                await self.record_scan_metrics(
                    scan_start if self.metrics else datetime.now(timezone.utc),
                    len(symbols),
                    len(alerts)
                )
