        ...
        with timer() as t:
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    cache_key = f"options_scan:{','.join(sorted(symbols[:10]))}:{self.lookback_days}"
                    ...
                    if cached_alerts is not None:
                        logger.info(f"Using cached results for options scan")
                ...
                logger.info(
                    f"✅ Options Scanner: Found {len(alerts)} signals "
