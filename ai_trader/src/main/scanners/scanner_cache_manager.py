...
class ScannerCacheManager:
    ...
    def __init__(self, ttl_seconds: int = 300, max_entries: int = 1000):
        ...
        logger.info(f"ScannerCacheManager initialized with TTL={ttl_seconds}s, max_entries={max_entries}")
    ...
    async def get(self, scanner_name: str, symbols: List[str], params: Dict[str, Any]) -> Optional[Any]:
        """
    ...
    async def set(self, scanner_name: str, symbols: List[str], params: Dict[str, Any], result: Any) -> None:
        """
        ...
        async with self._lock:
            ...
            if len(self._cache) >= self.max_entries:
                ...
                oldest_keys = sorted(self._cache.keys(), 
                                   key=lambda k: self._cache[k][1])[:len(self._cache) - self.max_entries + 1]
    ...
    async def invalidate(self, scanner_name: Optional[str] = None) -> None:
        """
