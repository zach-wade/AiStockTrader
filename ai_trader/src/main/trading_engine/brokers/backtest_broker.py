...
class BacktestBroker(BrokerInterface, ErrorHandlingMixin):
    ...
    def load_historical_data(self, symbol: str, data: pd.DataFrame) -> None:
        ...
        logger.info(f"Loaded {len(data)} bars for {symbol} from {data.index[0]} to {data.index[-1]}")
    ...
    async def submit_order(self, order: Order) -> Order:
        ...
        try:
            ...
            # Set submission time
            order.submitted_at = self.current_timestamp or datetime.now(timezone.utc)
            ...
            logger.info(f"Order submitted: {order.broker_order_id} - {order.side.value} "
    ...
    def _check_order_fill(self, order: Order, market_state: Dict) -> Tuple[bool, Optional[float]]:
        """
        ...
        if order.order_type == OrderType.MARKET:
            ...
            if order.side == OrderSide.BUY:
                ...
                # Sell stop fills if price drops to or below stop
    ...
    async def _execute_order(self, order: Order, base_fill_price: float) -> None:
        ...
        # Update positions
        self._update_position(order.symbol, fill_quantity if order.side == OrderSide.BUY else -fill_quantity, fill_price)
        ...
        logger.info(f"Order filled: {order.broker_order_id} - {fill_quantity} @ ${fill_price:.2f} "
    ...
    def _update_position(self, symbol: str, quantity_change: float, price: float) -> None:
        ...
        if total_quantity == 0:
            ...
            total_value = sum(lot['quantity'] * lot['price'] for lot in self._position_details[symbol])
    ...
    async def get_account_info(self) -> AccountInfo:
        ...
        return AccountInfo(
            ...
            daytrade_count=0,
            balance_asof=self.current_timestamp or datetime.now(timezone.utc)
        )
