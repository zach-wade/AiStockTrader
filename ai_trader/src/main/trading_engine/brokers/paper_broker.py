...
from main.models.common import AccountInfo, Position, Order, OrderStatus, OrderType, OrderSide, TimeInForce, MarketData
...
class PaperBroker(BrokerInterface):
    ...
    async def submit_order(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if order['type'].value == 'market':
            # Execute order
            fill_price = price * (1.0001 if order['side'].value == 'buy' else 0.9999)  # Simulate slippage
            ...
            logger.info(f"Order created: {order['side'].value} {order['quantity']} {order['symbol']} (pending)")
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        total_value = self.buying_power + sum(pos['market_value'] for pos in self.positions.values())
    ...
    async def modify_order(self, order_id: str, 
        ...
        if order['status'] not in ['pending', 'submitted']:
            raise ValueError(f"Cannot modify order with status '{order['status']}'. Only pending orders can be modified.")
    ...
    async def get_position(self, symbol: str) -> Optional[Position]:
        ...
        if symbol in self.positions:
            ...
            return Position(
                ...
                unrealized_pnl_pct=((pos['market_value'] / pos['cost_basis']) - 1) * 100 if pos['cost_basis'] > 0 else 0,
                realized_pnl=0.0,
    ...
    async def get_order(self, order_id: str) -> Optional[Order]:
        ...
        if order_id in self.orders:
            ...
            return Order(
                ...
                created_at=order.get('filled_at', datetime.now(timezone.utc))
            )
        return None
            ...
            for order_id, order_data in list(self.orders.items()):
                ...
                if order_data['status'] == 'pending':
                    ...
                    if order_data['type'] == 'limit':
                        if order_data['side'] == 'buy' and current_price <= order_data.get('limit_price', float('inf')):
                            should_fill = True
                        elif order_data['side'] == 'sell' and current_price >= order_data.get('limit_price', 0):
                            should_fill = True
                    ...
                    if should_fill:
                        ...
                        order_data['filled_at'] = datetime.now(timezone.utc)
                        ...
                        if order_data['side'] == 'buy':
                            ...
                            if symbol in self.positions:
                                pos = self.positions[symbol]
                                total_cost = (pos['quantity'] * pos['entry_price']) + (quantity * current_price)
