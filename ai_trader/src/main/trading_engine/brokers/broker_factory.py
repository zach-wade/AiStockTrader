...
class BrokerFactory:
    ...
    def create_broker(cls, broker_type: str, config: Any) -> BrokerInterface:
        ...
        if not broker_class:
            raise ConfigurationError(
                f"No implementation registered for broker type: {broker_type}"
            )
    ...
    def create_from_config(cls, config: Any) -> BrokerInterface:
        ...
        if hasattr(config, 'broker'):
            ...
            raise ConfigurationError(
                "Configuration must have a 'broker' section with 'type' field"
            )
    ...
    def register_broker(cls, broker_type: BrokerType, broker_class: Type[BrokerInterface]):
        ...
        if not issubclass(broker_class, BrokerInterface):
            raise ValueError(
                f"{broker_class.__name__} must inherit from BrokerInterface"
            )
...
def create_broker(broker_type: Optional[str] = None, config: Optional[Any] = None) -> BrokerInterface:
    """
...
def create_paper_broker(config: Optional[Any] = None) -> PaperBroker:
    """Create a paper trading broker."""
    return create_broker('paper', config)


def create_alpaca_broker(config: Optional[Any] = None) -> AlpacaBroker:
    """Create an Alpaca broker."""
    return create_broker('alpaca', config)


def create_mock_broker(config: Optional[Any] = None) -> MockBroker:
    """Create a mock broker for testing."""
    return create_broker('mock', config)


def create_backtest_broker(config: Optional[Any] = None) -> BacktestBroker:
