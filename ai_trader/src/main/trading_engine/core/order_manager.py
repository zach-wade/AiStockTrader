...
from main.utils.core import (
    AITraderException, create_event_tracker, async_retry,
    create_task_safely
)
...
class OrderManager:
    ...
    async def create_order(
        ...
        # Record event
        self._record_event(order, "created", None, OrderStatus.PENDING_NEW, {
    ...
    async def submit_order(self, order: Order) -> str:
        ...
        try:
            ...
            # Record event
            self._record_event(order, "submitted", OrderStatus.PENDING_SUBMIT, order.status, {
            ...
            # Track metrics
            if self.metrics:
                self.metrics.increment("order_manager.orders_submitted", tags={
                    "symbol": order.symbol,
            ...
            self._record_event(order, "rejected", OrderStatus.PENDING_SUBMIT, order.status, {
                "reason": str(e)
            })
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        if order.status not in [OrderStatus.NEW, OrderStatus.PARTIALLY_FILLED, OrderStatus.PENDING_NEW]:
        ...
        try:
            ...
            if order.order_id:  # Has broker ID
                ...
                if success:
                    ...
                    self._record_event(order, "cancel_failed", OrderStatus.PENDING_CANCEL, old_status)
            ...
            if self.metrics and success:
                self.metrics.increment("order_manager.orders_canceled", tags={
                    "symbol": order.symbol
                })
    ...
    async def _handle_broker_update(self, broker_order: Order) -> None:
        ...
        # Record event
        self._record_event(our_order, "status_update", old_status, our_order.status, {
            "filled_qty": float(broker_order.filled_quantity) if broker_order.filled_quantity else 0,
            "avg_price": float(broker_order.avg_fill_price) if broker_order.avg_fill_price else None
        })
        ...
        # Track metrics
        if self.metrics:
            self.metrics.increment("order_manager.status_updates", tags={
                "old_status": old_status.value,
                "new_status": our_order.status.value
            })
    ...
    def _record_event(
        ...
        # Track event
        self.event_tracker.track("order_event", {
            "order_id": order.client_order_id,
            "event_type": event_type,
            "symbol": order.symbol,
            "side": order.side.value,
