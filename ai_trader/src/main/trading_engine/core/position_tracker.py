...
class PositionTracker:
    ...
    async def update_position(self, position: Position) -> Optional[Position]:
        """
    ...
    async def update_positions(self, positions: Dict[str, Position]) -> Dict[str, Optional[Position]]:
        """
            ...
            for symbol, new_price in price_updates.items():
                if symbol in self.positions:
                    ...
                    updated_position = Position(
                        ...
                        unrealized_pnl=(float(new_price) - old_position.avg_entry_price) * old_position.quantity,
                        unrealized_pnl_pct=((float(new_price) / old_position.avg_entry_price) - 1) * 100 if old_position.avg_entry_price > 0 else 0,
    ...
    def _add_to_history(self, position: Position, old_position: Optional[Position]):
