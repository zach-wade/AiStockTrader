...
class ExecutionData:
    ...
    urgency: Optional[str] = "NORMAL"  # URGENT, NORMAL, PASSIVE
...
class TransactionCostAnalyzer:
    ...
    def analyze_execution(self, 
                         ...
                         market_data: Optional[pd.DataFrame] = None) -> TCAMetrics:
        """
        ...
        market_impact_bps = (market_impact / (execution.intended_price * execution.executed_quantity)) * 10000
        ...
        if market_data is not None and len(market_data) > 0:
            ...
            vwap = self._calculate_vwap(market_data, execution.timestamp, execution.timestamp + timedelta(minutes=5))
            
            # Timing cost: difference between execution and arrival price
            if execution.side == OrderSide.BUY:
                timing_cost = (execution.execution_price - arrival_price) * execution.executed_quantity
            else:
                timing_cost = (arrival_price - execution.execution_price) * execution.executed_quantity
        ...
        opportunity_cost_bps = (opportunity_cost / (mid_price * execution.intended_quantity)) * 10000 if execution.intended_quantity > 0 else 0
        
        # Calculate total cost
        total_cost = spread_cost + market_impact + timing_cost + opportunity_cost
        total_cost_bps = spread_cost_bps + market_impact_bps + timing_cost_bps + opportunity_cost_bps
        
        # Calculate execution shortfall
        if execution.side == OrderSide.BUY:
            execution_shortfall = (execution.execution_price - execution.intended_price) * execution.executed_quantity
        else:
            execution_shortfall = (execution.intended_price - execution.execution_price) * execution.executed_quantity
        ...
        participation_rate = execution.executed_quantity / execution.market_volume if execution.market_volume > 0 else 0
    ...
    def _calculate_vwap(self, market_data: pd.DataFrame, start_time: datetime, end_time: datetime) -> float:
        ...
        if 'price' in market_data.columns and 'volume' in market_data.columns:
            ...
            if len(data_slice) > 0:
                return (data_slice['price'] * data_slice['volume']).sum() / data_slice['volume'].sum()
    ...
    def get_summary_statistics(self, 
        ...
        summary = {
            ...
            'cost_breakdown': {
                'spread': np.mean(spread_costs) / np.mean(total_costs) * 100 if np.mean(total_costs) > 0 else 0,
                'market_impact': np.mean(market_impacts) / np.mean(total_costs) * 100 if np.mean(total_costs) > 0 else 0,
            },
    ...
    def identify_cost_drivers(self) -> Dict:
        ...
        analysis = {
            ...
            'recommendations': self._generate_recommendations(venue_costs, time_costs, size_costs, urgency_costs)
        }
    ...
    def _generate_recommendations(self, venue_costs, time_costs, size_costs, urgency_costs) -> List[str]:
    ...
    def calculate_slippage_metrics(self, 
                                  intended_trades: List[Dict],
                                  actual_trades: List[Dict]) -> Dict:
        """
        ...
        return {
            ...
            'avg_slippage_bps': np.mean([s for sublist in slippage_by_symbol.values() for s in sublist]),
            'slippage_by_symbol': {
                symbol: {
    ...
    def optimize_execution_schedule(self,
        ...
        for i, time_str in enumerate(best_times):
            ...
            schedule.append({
                ...
                'quantity': quantity * weight / sum(1/(j+1) for j in range(len(best_times))),
                'type': 'LIMIT',
                'urgency': 'NORMAL'
            })
