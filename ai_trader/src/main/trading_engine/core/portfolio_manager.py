...
from main.utils.cache import CacheType, MemoryBackend, get_global_cache

# NEW: Import common models (Position, AccountInfo) and enums
from main.models.common import Position as CommonPosition, AccountInfo as CommonAccountInfo, OrderSide
...
class Portfolio:
    ...
    def total_value(self) -> float:
        ...
        return self.cash + sum(pos.market_value for pos in self.positions.values() if pos.market_value is not None)
    
    @property
    def invested_value(self) -> float:
        """Calculates the total market value of all open positions."""
        return sum(pos.market_value for pos in self.positions.values() if pos.market_value is not None)
...
class PortfolioManager:
    ...
    async def _update_cache(self, portfolio: Portfolio):
        ...
        try:
            await self.cache.set(CacheType.PORTFOLIO, "current_portfolio", portfolio, self.cache_ttl_seconds)
    ...
    async def initialize_portfolio_from_broker(self):
        ...
        async with self._timed_lock(self._update_lock, timeout=15.0):
            try:
                ...
                logger.info(f"Portfolio initialized/synchronized with broker. Cash: ${self.portfolio.cash:,.2f}, Positions: {self.portfolio.position_count}.")
            except asyncio.TimeoutError:
    ...
    async def _update_portfolio_internal(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"Portfolio state synchronized with broker successfully. Cash: {self.portfolio.cash}, Equity: {self.portfolio.total_value}")
            return True
    ...
    async def get_position_size(self, symbol: str, price: float, risk_pct: float = 0.02) -> float:
        ...
        if portfolio.total_value <= 0:
            logger.warning("Portfolio total value is zero or negative. Cannot calculate position size.")
            return 0.0
    ...
    async def open_position(self, symbol: str, quantity: float, price: float, side: OrderSide, strategy: str = "manual") -> bool:
        """
        ...
        async with self._timed_lock(self._position_lock, timeout=10.0):
            ...
            if symbol in new_positions:
                ...
                new_positions[symbol] = CommonPosition(
                    ...
                    cost_basis=new_total_quantity * (new_total_cost / new_total_quantity), # Update cost basis
                    unrealized_pnl= (price - (new_total_cost / new_total_quantity)) * new_total_quantity,
                    unrealized_pnl_pct= (price / (new_total_cost / new_total_quantity) - 1) * 100 if (new_total_cost / new_total_quantity) else 0.0,
                    realized_pnl=existing_pos.realized_pnl # Realized PnL carries over
                )
                logger.info(f"Updated existing position for {symbol}: new quantity {new_total_quantity}, avg price {new_positions[symbol].avg_entry_price:.2f}")
            else:
                # Open a new position
                new_positions[symbol] = CommonPosition(
                    symbol=symbol,
    ...
    async def close_position(self, symbol: str, quantity: float, price: float) -> Optional[float]:
        """
        ...
        async with self._timed_lock(self._position_lock, timeout=10.0):
            ...
            if quantity > position_to_close.quantity:
                logger.warning(f"Attempted to close {quantity} of {symbol}, but only {position_to_close.quantity} held. Closing all available.")
            ...
            if quantity_to_close_actual == position_to_close.quantity:
                ...
                logger.info(f"Fully closed position: {symbol} at ${price:.2f}, Realized P&L: ${pnl_realized:.2f}")
            else:
                ...
                new_positions[symbol] = CommonPosition(
                    ...
                    unrealized_pnl_pct=(price / position_to_close.avg_entry_price - 1) * 100 if position_to_close.avg_entry_price else 0.0,
                    realized_pnl=position_to_close.realized_pnl + pnl_realized # Accumulate realized PnL
                )
                logger.info(f"Partially closed position: {symbol} {quantity_to_close_actual} shares at ${price:.2f}. Remaining: {new_positions[symbol].quantity}.")
            ...
            self.trade_history.append({
                ...
                'strategy': position_to_close.strategy if hasattr(position_to_close, 'strategy') else "manual_close"
            })
            ...
            for symbol, price in prices.items():
                if symbol in new_positions:
                    ...
                    new_positions[symbol] = CommonPosition(
                        ...
                        unrealized_pnl=(price - existing_pos.avg_entry_price) * existing_pos.quantity, # Recalculate P&L
                        unrealized_pnl_pct=(price / existing_pos.avg_entry_price - 1) * 100 if existing_pos.avg_entry_price else 0.0,
                    )
                    updated_count += 1
    ...
    async def get_position_by_symbol(self, symbol: str) -> Optional[CommonPosition]:
        ...
        return {
            ...
            'positions': {
                symbol: {
                    ...
                    'unrealized_pnl_pct': pos.pnl_pct if hasattr(pos, 'pnl_pct') else pos.unrealized_pnl_pct,
