...
from main.models.common import Position, Order, MarketData, PositionSide
...
class PositionRiskValidator:
        ...
        try:
            ...
            for result in stress_results:
                if not result.passed:
                    errors.append(f"Failed stress test '{result.scenario_name}': loss {result.portfolio_loss:.1%}")
    ...
    async def _calculate_portfolio_metrics(
        ...
        portfolio_var_95 = Decimal(str(abs(np.percentile(portfolio_returns, 5) * float(total_value))))
        portfolio_var_99 = Decimal(str(abs(np.percentile(portfolio_returns, 1) * float(total_value))))
        ...
        portfolio_beta = Decimal(str(np.cov(portfolio_returns, market_returns)[0, 1] / np.var(market_returns)))
        
        # Sharpe ratio
        portfolio_sharpe = Decimal(str(np.mean(portfolio_returns) / np.std(portfolio_returns) * np.sqrt(252)))
    ...
    async def _run_stress_tests(
        ...
        for scenario in self.limits.stress_test_scenarios:
            if scenario == "market_crash_10pct":
                ...
                portfolio_value = sum(p.market_value for p in positions) + (new_quantity * new_price)
