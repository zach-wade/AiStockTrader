...
from main.models.common import (
    Order, Position, Signal, AccountInfo, OrderSide, PositionSide,
    MarketData, OrderStatus
)
from main.trading_engine.core.position_validator import (
    PositionValidator, PositionLimits, ValidationContext
)
from main.trading_engine.core.position_risk_validator import (
    PositionRiskValidator, RiskLimits, RiskMetrics
)
from main.trading_engine.brokers.broker_interface import BrokerInterface
from main.utils.core import (
    AITraderException, ValidationResult, create_event_tracker,
    create_task_safely
)
...
class RiskManager:
    ...
    async def handle_emergency_stop(self) -> None:
        ...
        try:
            ...
            # Track event
            self.event_tracker.track("emergency_stop", {
                "positions_closed": len(order_ids),
                "daily_pnl": float(self._daily_pnl)
            })
    ...
    async def get_risk_state(self) -> Optional[RiskState]:
        ...
        try:
            ...
            for fill in fills:
                if fill.side == OrderSide.SELL:
                    ...
                    cost_basis = await self.db.get_position_cost_basis(
                        fill.symbol,
                        fill.timestamp
                    )
                    if cost_basis:
    ...
    async def _process_alert(self, alert: RiskAlert) -> None:
        ...
        # Track event
        self.event_tracker.track("risk_alert", {
    ...
    def _track_validation(
        ...
        self.event_tracker.track("order_validation", {
            "symbol": order.symbol,
