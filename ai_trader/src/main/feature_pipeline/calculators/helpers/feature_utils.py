    ...
    for col in normalized.columns:
        if normalized[col].dtype in [np.float64, np.float32, np.int64, np.int32]:
            ...
            if max_val > min_val:
                ...
                normalized[col] = normalized[col] * (feature_range[1] - feature_range[0]) + feature_range[0]
            else:
        ...
        for col in columns:
            if group_by and group_by in features.columns:
                ...
                lagged_features[f'{col}_lag_{lag}'] = \
                    features.groupby(group_by)[col].shift(lag)
            else:
                # Simple lagging
            ...
            if group_by and group_by in features.columns:
                ...
                if 'mean' in operations:
                    rolling_features[f'{col}_rolling_mean_{window}'] = \
                        grouped.rolling(window=window, min_periods=min_per).mean().reset_index(level=0, drop=True)
                
                if 'std' in operations:
                    rolling_features[f'{col}_rolling_std_{window}'] = \
                        grouped.rolling(window=window, min_periods=min_per).std().reset_index(level=0, drop=True)
                
                if 'min' in operations:
                    rolling_features[f'{col}_rolling_min_{window}'] = \
                        grouped.rolling(window=window, min_periods=min_per).min().reset_index(level=0, drop=True)
                
                if 'max' in operations:
                    rolling_features[f'{col}_rolling_max_{window}'] = \
                        grouped.rolling(window=window, min_periods=min_per).max().reset_index(level=0, drop=True)
            else:
                # Simple rolling
                if 'mean' in operations:
                    rolling_features[f'{col}_rolling_mean_{window}'] = \
