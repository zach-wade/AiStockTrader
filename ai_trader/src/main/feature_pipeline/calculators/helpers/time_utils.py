...
from main.utils.core import (
    get_logger, ensure_utc, is_market_open, get_market_hours,
    is_trading_day, get_next_trading_day, get_last_us_trading_day
)
...
def calculate_time_decay(
    base_value: float,
    elapsed_time: Union[timedelta, float],
    half_life: Union[timedelta, float],
...
def create_time_windows(
    ...
    windows: List[Union[timedelta, int]],
...
def calculate_time_weighted_average(
    ...
    try:
        ...
        for i, elapsed in enumerate(time_diffs):
            weights.iloc[i] = calculate_time_decay(
                1.0, elapsed, half_life_hours, decay_type
            )
    ...
    try:
        ...
        if reference_time is not None:
            ...
            features['days_since_reference'] = \
                (timestamps - reference_time).dt.total_seconds() / 86400
