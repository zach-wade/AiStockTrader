...
class MarketRegimeCalculator(BaseFeatureCalculator):
    ...
    def _calculate_hmm_regimes(self, features: pd.DataFrame, 
                              data: pd.DataFrame) -> pd.DataFrame:
        """Calculate Hidden Markov Model based regimes."""
        # Prepare market-wide features for HMM
        market_data = data.groupby('timestamp').agg({
            'returns': 'mean',
            'volume': 'sum',
            'close': 'mean'
        }).sort_index()
        ...
        try:
            ...
            warnings.warn(f"HMM regime detection failed: {str(e)}")
            features['hmm_regime'] = 0
            features['hmm_confidence'] = 0.5
        
        return features
        
    def _calculate_volatility_regimes(self, features: pd.DataFrame, 
                                    data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [20, 60, 120]:
            ...
            features[f'{vol_col}_percentile'] = (
                data.groupby('timestamp')[vol_col].rank(pct=True)
            )
        ...
        features['vol_term_structure'] = (
            data['volatility_20d'] / data['volatility_60d']
        ).fillna(1)
    ...
    def _calculate_garch_regimes(self, features: pd.DataFrame, 
                                data: pd.DataFrame) -> pd.DataFrame:
        ...
        high_vol_streaks = high_vol_periods.groupby(
            (~high_vol_periods).cumsum()
        ).cumsum()
    ...
    def _calculate_trend_regimes(self, features: pd.DataFrame, 
                               data: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in data.columns for col in ['high', 'low', 'close']):
            ...
            adx = talib.ADX(market_high.values, market_low.values, market_close.values, timeperiod=14)
        ...
        # Trend consistency
        trend_changes = market_close.pct_change(20).rolling(60).apply(
    ...
    def _calculate_correlation_regimes(self, features: pd.DataFrame, 
                                     data: pd.DataFrame) -> pd.DataFrame:
        ...
        corr_mean_series = pd.Series(
            rolling_corr_mean,
            index=returns_pivot.index[window:]
        )
        corr_std_series = pd.Series(
            rolling_corr_std,
            index=returns_pivot.index[window:]
        )
    ...
    def _calculate_liquidity_regimes(self, features: pd.DataFrame, 
                                   data: pd.DataFrame) -> pd.DataFrame:
        ...
        liquidity_score = StandardScaler().fit_transform(
            features[['volume_zscore', 'volume_concentration']].fillna(0)
        ).mean(axis=1)
    ...
    def _calculate_regime_transitions(self, features: pd.DataFrame, 
                                    data: pd.DataFrame) -> pd.DataFrame:
        ...
        for regime_type in ['hmm_regime', 'vol_regime', 'trend_regime']:
            ...
            regime_changes = features[regime_type] != features.groupby('symbol')[regime_type].shift(1)
    ...
    def _calculate_regime_stability(self, features: pd.DataFrame, 
                                  data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'hmm_confidence' in features.columns:
            features['regime_stability_score'] = (
                ...
                (1 - features['hmm_regime_change_freq_20d'] / 20)
            ).clip(0, 1)
        ...
        if len(regime_cols) > 1:
            ...
            features['multi_regime_agreement'] = regime_agreement / (len(regime_cols) * (len(regime_cols) - 1) / 2)
        
        return features
        
    def _calculate_conditional_analytics(self, features: pd.DataFrame, 
                                       data: pd.DataFrame) -> pd.DataFrame:
        ...
        for regime_type in ['hmm_regime', 'vol_regime', 'trend_regime']:
            ...
            features[f'{regime_type}_expected_return'] = features[regime_type].map(regime_forward_returns)
            
            # Conditional volatility by regime
            regime_volatility = data.groupby([data['timestamp'], features[regime_type]])['returns'].std()
        ...
        if 'vol_regime' in features.columns and 'trend_regime' in features.columns:
            ...
            features['combined_regime'] = (
                features['vol_regime'].astype(str) + '_' +
                features['trend_regime'].astype(str)
            )
        ...
        try:
            ...
            if hasattr(self, '_regime_cache') and hasattr(self, '_cache_timestamp'):
                ...
                if (self._cache_timestamp and 
                    datetime.now(timezone.utc) - self._cache_timestamp < cache_duration):
    ...
    def _analyze_single_benchmark_regime(self, market_data: List[Dict], symbol: str) -> str:
        ...
        try:
            ...
            # Recent performance
            price_change_1m = (current_price - df['close'].iloc[-20]) / df['close'].iloc[-20] if len(df) >= 20 else 0
            ...
            if is_high_vol and abs(price_change_1m) > 0.1:  # High vol + big moves
                ...
                # Mixed signals - use overall trend
                if price_change_3m > 0.02:
                    return 'bull'
