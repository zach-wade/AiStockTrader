...
class TimeseriesCalculator(BaseStatisticalCalculator):
    ...
    def get_feature_names(self) -> List[str]:
        ...
        for window in self.stationarity_windows:
            feature_names.extend([
                f'adf_pvalue_{window}', f'adf_statistic_{window}',
                f'kpss_pvalue_{window}', f'kpss_statistic_{window}',
                f'pp_pvalue_{window}'  # Phillips-Perron
            ])
        
        # Serial correlation tests
        feature_names.extend([
            'ljungbox_pvalue', 'ljungbox_statistic', 'portmanteau_stat',
            'autocorr_1', 'autocorr_5', 'autocorr_10'
        ])
        
        # Long memory tests
        feature_names.extend([
            'rs_statistic', 'hurst_rs', 'variance_ratio_2', 'variance_ratio_4',
            'variance_ratio_8', 'variance_ratio_16', 'long_memory_test'
        ])
        
        # Change point detection
        feature_names.extend([
            'cusum_stat', 'changepoint_prob', 'structural_break_test',
            'regime_probability', 'regime_persistence', 'regime_volatility'
        ])
        
        # Unit root tests
        feature_names.extend([
            'unit_root_prob', 'trend_strength', 'seasonal_strength',
            'residual_autocorr', 'heteroskedasticity_test'
        ])
    ...
    def _calculate_stationarity_tests(self, prices: pd.Series, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in self.stationarity_windows:
            ...
            features[f'pp_pvalue_{window}'] = self.rolling_apply_safe(
                prices, window, pp_test_func
            )
        
        return features
    
    def _calculate_serial_correlation_tests(self, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate serial correlation tests."""
        ...
        features['autocorr_1'] = self.rolling_apply_safe(
            returns, 30, lambda x: autocorr_func(x, 1)
        )
        features['autocorr_5'] = self.rolling_apply_safe(
            returns, 30, lambda x: autocorr_func(x, 5)
        )
    ...
    def _calculate_long_memory_tests(self, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        features['rs_statistic'] = self.rolling_apply_safe(
            returns, self.rs_window, rs_func
        )
        ...
        features['hurst_rs'] = self.rolling_apply_safe(
            returns, self.rs_window, hurst_rs_func
        )
        ...
        features['long_memory_test'] = self.rolling_apply_safe(
            returns, 100, long_memory_func
        )
        
        return features
    
    def _calculate_changepoint_features(self, prices: pd.Series, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        features['cusum_stat'] = self.rolling_apply_safe(
            returns, self.cusum_window, cusum_func
        )
    ...
    def _calculate_regime_features(self, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        features['regime_probability'] = self.rolling_apply_safe(
            returns, 60, regime_prob_func
        )
        ...
        features['regime_volatility'] = self.rolling_apply_safe(
            returns, 60, regime_volatility_func
        )
        
        return features
    
    def _calculate_diagnostic_tests(self, prices: pd.Series, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        features['unit_root_prob'] = self.rolling_apply_safe(
            prices, 60, unit_root_func
        )
        ...
        features['trend_strength'] = self.rolling_apply_safe(
            prices, 60, trend_strength_func
        )
        ...
        features['seasonal_strength'] = self.rolling_apply_safe(
            returns, 60, seasonal_strength_func
        )
        ...
        features['residual_autocorr'] = self.rolling_apply_safe(
            returns, 40, residual_autocorr_func
        )
        ...
        features['heteroskedasticity_test'] = self.rolling_apply_safe(
            returns, 60, arch_test_func
        )
    ...
    def _adf_test(self, data: np.ndarray) -> Tuple[float, float]:
        ...
        try:
            result = adfuller(data, maxlag=self.adf_maxlag, regression=self.adf_regression, autolag='AIC')
    ...
    def _portmanteau_test(self, data: np.ndarray) -> float:
        ...
        try:
            ...
            # Ljung-Box statistic
            lb_stat = n * (n + 2) * sum(
                corr**2 / (n - i - 1) for i, corr in enumerate(autocorrs)
            )
