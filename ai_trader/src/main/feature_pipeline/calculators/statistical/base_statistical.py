...
from ..helpers import (
    calculate_moving_average, calculate_rolling_std, calculate_rolling_quantile,
    remove_outliers, fit_distribution, normalize_series, standardize_series,
    calculate_entropy, calculate_hurst_exponent, safe_divide
)
...
class BaseStatisticalCalculator(BaseFeatureCalculator):
    ...
    def calculate_rolling_statistics(
        ...
        for window in windows:
            if 'mean' in operations:
                features[f'rolling_mean_{window}'] = calculate_moving_average(
                    series, window
                )
            
            if 'std' in operations:
                features[f'rolling_std_{window}'] = calculate_rolling_std(
                    series, window
                )
    ...
    def calculate_entropy_features(
        ...
        for window in windows:
            features[f'shannon_entropy_{window}'] = series.rolling(
                ...
                lambda x: calculate_entropy(pd.Series(x), bins=self.entropy_bins),
                raw=True
            )
    ...
    def calculate_tail_features(
        ...
        series: pd.Series,
        windows: Optional[List[int]] = None,
        tail_percentiles: Optional[List[float]] = None
    ) -> pd.DataFrame:
        """
    ...
    def calculate_complexity_features(
        ...
        for window in windows:
            if window >= 50:  # Minimum required for reliable Hurst
                features[f'hurst_exponent_{window}'] = series.rolling(
                    ...
                    lambda x: calculate_hurst_exponent(pd.Series(x)),
                    raw=True
                )
        ...
        for window in windows:
            features[f'approx_entropy_{window}'] = series.rolling(
                ...
                lambda x: self._calculate_approx_entropy(x),
                raw=True
            )
