...
class StatisticalConfig:
    ...
    distribution_types: List[str] = field(
        default_factory=lambda: ["normal", "t", "lognormal"]
    )
    ...
    entropy_bins: int = 10
    entropy_methods: List[str] = field(
        default_factory=lambda: ["shannon", "normalized"]
    )
    
    # Autocorrelation parameters
    autocorr_lags: List[int] = field(
...
class EntropyConfig:
    ...
    multiscale_factors: List[int] = field(
...
class MomentConfig:
    ...
    l_moment_orders: List[int] = field(
