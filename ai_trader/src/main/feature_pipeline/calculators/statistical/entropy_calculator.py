...
from ..helpers import (
    create_feature_dataframe, safe_divide, normalize_series,
    calculate_entropy as basic_entropy
)
...
class EntropyCalculator(BaseStatisticalCalculator):
    ...
    def get_feature_names(self) -> List[str]:
        ...
        if self.entropy_config.permutation:
            for window in self.lookback_periods:
                features.extend([
                    f'permutation_entropy_{window}',
                    f'weighted_permutation_entropy_{window}'
                ])
    ...
    def _calculate_shannon_entropy(self, returns: pd.Series) -> pd.DataFrame:
        ...
        for window in self.lookback_periods:
            ...
            features[f'shannon_entropy_{window}'] = returns.rolling(
                ...
                lambda x: basic_entropy(pd.Series(x), bins=self.entropy_config.bins),
                raw=True
            )
            ...
            if window > 1:
                features[f'entropy_rate_{window}'] = \
                    features[f'shannon_entropy_{window}'].diff()
        
        return features
    
    def _calculate_renyi_entropy(
        self, returns: pd.Series, alpha: float = 2.0
    ) -> pd.DataFrame:
        ...
        for window in self.lookback_periods:
            features[f'renyi_entropy_{window}'] = returns.rolling(
                ...
                lambda x: renyi_entropy(x, alpha, self.entropy_config.bins),
                raw=True
            )
        
        return features
    
    def _calculate_tsallis_entropy(
        self, returns: pd.Series, q: float = 2.0
    ) -> pd.DataFrame:
        ...
        for window in self.lookback_periods:
            features[f'tsallis_entropy_{window}'] = returns.rolling(
                ...
                lambda x: tsallis_entropy(x, q, self.entropy_config.bins),
                raw=True
            )
        ...
        for window in self.lookback_periods:
            if window >= order:
                ...
                features[f'permutation_entropy_{window}'] = returns.rolling(
                    ...
                    lambda x: self._permutation_entropy(x, order),
                    raw=True
                )
                ...
                features[f'weighted_permutation_entropy_{window}'] = returns.rolling(
                    ...
                    lambda x: self._weighted_permutation_entropy(x, order),
                    raw=True
                )
    ...
    def _calculate_multiscale_entropy(self, returns: pd.Series) -> pd.DataFrame:
        ...
        for scale in self.entropy_config.multiscale_factors:
            ...
            features[f'multiscale_entropy_scale_{scale}'] = self._calculate_sample_entropy(
                coarse_series
            ).iloc[:, 0]  # Take first column (smallest window)
    ...
    def _calculate_spectral_entropy(self, returns: pd.Series) -> pd.DataFrame:
        ...
        for window in windows:
            features[f'spectral_entropy_{window}'] = returns.rolling(
                ...
                lambda x: self._spectral_entropy(x),
                raw=True
            )
    ...
    def _calculate_differential_entropy(self, returns: pd.Series) -> pd.DataFrame:
        ...
        for window in self.lookback_periods:
            features[f'differential_entropy_{window}'] = returns.rolling(
                ...
                lambda x: self._differential_entropy(x),
                raw=True
            )
    ...
    def _calculate_cross_entropy(self, returns: pd.Series) -> pd.DataFrame:
        ...
        if len(clean_returns) > 30:
            ...
            features['relative_entropy_vs_normal'] = self._relative_entropy_vs_normal(
                clean_returns
            )
            ...
            features['kullback_leibler_divergence'] = self._calculate_kl_divergence(
                returns
            )
