...
class MultivariateCalculator(BaseStatisticalCalculator):
    ...
    def get_feature_names(self) -> List[str]:
        ...
        for window in self.evt_windows:
            feature_names.extend([
                f'evt_location_{window}', f'evt_scale_{window}',
                f'var_95_{window}', f'es_95_{window}', f'tail_index_{window}'
            ])
    ...
    def _calculate_component_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_pca_features(self, return_matrix: pd.DataFrame, features: pd.DataFrame, 
        ...
        try:
            ...
            logger.debug(f"PCA: {n_components} components explain {total_explained_var:.3f} of variance")
    ...
    def _calculate_ica_features(self, return_matrix: pd.DataFrame, features: pd.DataFrame,
                               original_index: pd.Index) -> pd.DataFrame:
    ...
    def _calculate_extreme_value_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_wavelet_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate wavelet decomposition features."""
        close_prices = data['close']
        
        try:
            # Perform wavelet decomposition on price series
            coeffs = pywt.wavedec(close_prices.values, self.wavelet_family, level=self.wavelet_levels)
            ...
            features['wavelet_approx_energy'] = approx_series.reindex(close_prices.index).fillna(method='ffill')
            
            # Energy at detail levels
            for i, detail_coeffs in enumerate(coeffs[1:], 1):
                detail_energy = detail_coeffs**2
                detail_series = pd.Series(detail_energy, index=close_prices.index[:len(detail_energy)])
                features[f'wavelet_detail_{i}_energy'] = detail_series.reindex(close_prices.index).fillna(method='ffill')
