...
from ..helpers import (
    ...
    calculate_rolling_std, normalize_series, calculate_entropy,
...
class OptionsSentimentCalculator(BaseOptionsCalculator):
    ...
    def _calculate_pcr_features(
        ...
        for window in self.sentiment_windows:
            features[f'pcr_volume_{window}d'] = rolling_features[f'pcr_volume_rolling_mean_{window}']
            features[f'pcr_oi_{window}d'] = rolling_features[f'pcr_oi_rolling_mean_{window}']
            features[f'pcr_dollar_volume_{window}d'] = rolling_features[f'pcr_dollar_rolling_mean_{window}']
    ...
    def _calculate_skew_features(
        ...
        # Calculate moneyness
        options_data['moneyness'] = safe_divide(options_data['strike'], options_data['underlying_price'], default_value=1.0)
        ...
        for date in options_data.index.unique():
            ...
            put_skew = self._calculate_single_skew(
                exp_data[exp_data['option_type'] == 'put']
            )
            call_skew = self._calculate_single_skew(
                exp_data[exp_data['option_type'] == 'call']
            )
    ...
    def _calculate_flow_sentiment(
        ...
        for date in options_data.index.unique():
            ...
            smart_money_sent = self._calculate_smart_money_sentiment(
                options_data.loc[date]
            )
    ...
    def _calculate_volume_weighted_sentiment(
        ...
        for date in options_data.index.unique():
            ...
            features.loc[date, 'dollar_weighted_pcr'] = safe_divide(
    ...
    def _calculate_term_structure_sentiment(
        self,
        options_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        for date in options_data.index.unique():
            ...
            features.loc[date, 'term_structure_slope'] = (
                features.loc[date, 'far_term_sentiment'] -
                features.loc[date, 'near_term_sentiment']
            )
    ...
    def _calculate_positioning_indicators(
        self,
        options_data: pd.DataFrame
    ) -> pd.DataFrame:
        """Calculate positioning indicators."""
    ...
    def _calculate_composite_sentiment(
        ...
        smart_retail_div = abs(
            features.get('smart_money_sentiment', 0) -
            features.get('retail_sentiment', 0)
        )
        term_div = abs(features.get('term_structure_slope', 0))
        composite['sentiment_divergence'] = safe_divide((smart_retail_div + term_div), 2, default_value=0.0)
        
        # Sentiment strength
        sentiment_abs = abs(composite['options_sentiment_composite'])
        flow_strength = abs(features.get('net_flow_sentiment', 0))
        composite['sentiment_strength'] = safe_divide((sentiment_abs + flow_strength), 2, default_value=0.0)
    ...
    def _calculate_butterfly(
        ...
        otm_calls = options[(options['option_type'] == 'call') & 
                           (options['delta'].abs() - delta).abs() < 0.05]
        otm_puts = options[(options['option_type'] == 'put') & 
                          (options['delta'].abs() - delta).abs() < 0.05]
        ...
        wing_iv = safe_divide(
            ...
            otm_puts['implied_volatility'].mean()), 2, default_value=0.0
        )
        
        return wing_iv - atm_iv
    
    def _calculate_smile_shape(
        self,
        options: pd.DataFrame
    ) -> Tuple[float, float]:
    ...
    def _calculate_smart_money_sentiment(
        self,
        options: pd.DataFrame
    ) -> float:
    ...
    def _calculate_vwap_sentiment(self, options: pd.DataFrame) -> float:
        ...
        # VWAP
        vwap = safe_divide((options['last_price'] * options['volume']).sum(), options['volume'].sum(), default_value=0.0)
    ...
    def _calculate_calendar_sentiment(
        self,
        options: pd.DataFrame,
        expirations: List
    ) -> float:
    ...
    def _calculate_positioning_concentration(
        self,
        options: pd.DataFrame
    ) -> float:
    ...
    def _calculate_max_pain(self, options: pd.DataFrame) -> float:
        ...
        for strike in strikes:
            ...
            ]['open_interest'].sum() * (strike - options['strike'])
            
            put_pain = options[
                (options['option_type'] == 'put') & 
                (options['strike'] > strike)
            ]['open_interest'].sum() * (options['strike'] - strike)
