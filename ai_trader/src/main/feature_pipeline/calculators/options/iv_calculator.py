...
class ImpliedVolatilityCalculator(BaseOptionsCalculator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.debug(f"Initialized ImpliedVolatilityCalculator with {len(self.term_structure_expiries)} term structure points")
    ...
    def _calculate_basic_iv_metrics(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(iv_series) >= self.percentile_window:
                def calculate_percentile(series):
                    ...
                    return safe_divide(stats.percentileofscore(historical_vals, current_val), 100, default_value=0.5)
    ...
    def _get_iv_data(self, data: pd.DataFrame) -> Optional[pd.Series]:
        ...
        try:
            ...
            if self.historical_iv is not None and not self.historical_iv.empty:
                return self.historical_iv.iloc[:, 0] if isinstance(self.historical_iv, pd.DataFrame) else self.historical_iv
            ...
            if self.options_chain is not None and not self.options_chain.empty:
                if 'impliedVolatility' in self.options_chain.columns:
                    ...
                    if not atm_options.empty:
                        ...
                        if weights.sum() > 0:
                            weighted_iv = safe_divide((atm_options['impliedVolatility'] * weights).sum(), weights.sum(), default_value=0.2)
    ...
    def _calculate_iv_hv_comparison(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for window in [20, 60]:
                ...
                if iv_data is not None:
                    # Current IV value
                    current_iv = iv_data.iloc[-1] if len(iv_data) > 0 else self.options_config.default_volatility
                    ...
                    features[f'iv_premium_{window}d'] = hist_vol.iloc[-1] * 0.2 if len(hist_vol) > 0 else 0.04
    ...
    def _calculate_term_structure_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _estimate_term_structure(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            base_iv = self.estimate_implied_volatility(data['close'], window=20).iloc[-1] if len(data) > 0 else 0.2
    ...
    def _calculate_advanced_iv_metrics(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate advanced IV metrics."""
        try:
            iv_data = self._get_iv_data(data)
            ...
            if len(iv_series) >= self.rank_window:
                iv_percentile = safe_divide((iv_series.iloc[-self.rank_window:] < current_iv).sum(), self.rank_window, default_value=0.5)
                ...
                iv_momentum = features.get('iv_momentum', pd.Series([0.0])).iloc[-1] if len(features) > 0 else 0.0
            ...
            if len(iv_series) >= 30:
                ...
                iv_persistence = iv_series.rolling(window=30, min_periods=10).apply(calculate_autocorr)
