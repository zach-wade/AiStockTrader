...
from ..helpers import safe_divide, create_rolling_features
...
class VolumeFlowCalculator(BaseOptionsCalculator):
    ...
    def _calculate_basic_volume_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_flow_metrics(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(features) >= self.momentum_window:
                ...
                volume_trend = volume_series.rolling(window=self.momentum_window, min_periods=3).apply(calculate_trend)
                ...
                features['volume_volatility'] = rolling_features[f'volume_rolling_std_{self.momentum_window}']
    ...
    def _calculate_block_trade_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate block trade features."""
        try:
            if self.options_chain is None or self.options_chain.empty:
                # Estimate block trades based on total volume
                total_volume = features.get('total_option_volume', pd.Series([0])).iloc[-1] if len(features) > 0 else 0
                
                # Estimate that 10% of volume comes from block trades
                estimated_block_volume = total_volume * 0.1
                estimated_block_count = max(1, int(safe_divide(estimated_block_volume, self.volume_config['block_size'], default_value=0.0)))
                ...
                estimated_large_count = max(1, int(safe_divide(estimated_large_volume, self.volume_config.get('large_size', 500), default_value=0.0)))
            ...
            features['block_trade_ratio'] = safe_divide(block_volume, total_volume, default_value=0.0)
    ...
    def _calculate_premium_flow_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate premium flow features."""
        try:
            if self.options_chain is None or self.options_chain.empty or 'lastPrice' not in self.options_chain.columns:
                ...
                estimated_premium_flow = total_volume * estimated_avg_premium * 100  # Contract multiplier
    ...
    def _calculate_term_structure_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            if self.options_chain is None or self.options_chain.empty or 'daysToExpiration' not in self.options_chain.columns:
    ...
    def _calculate_advanced_flow_metrics(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate advanced flow metrics."""
        try:
            ...
            if 'total_option_volume' in features.columns and 'net_premium_flow' in features.columns:
                ...
                premium_flow = abs(features['net_premium_flow'].iloc[-1]) if len(features) > 0 else 1
