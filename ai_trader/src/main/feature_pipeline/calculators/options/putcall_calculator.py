...
from ..helpers import safe_divide, safe_sqrt, create_rolling_features
...
class PutCallAnalysisCalculator(BaseOptionsCalculator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.debug(f"Initialized PutCallAnalysisCalculator with extremes: {self.sentiment_config['extreme_low']:.2f}-{self.sentiment_config['extreme_high']:.2f}")
    ...
    def _calculate_basic_pc_ratios(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if total_volume > 0 and total_oi > 0:
                volume_weight = safe_divide(total_volume, (total_volume + total_oi), default_value=0.5)
    ...
    def _calculate_pc_extremes(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            neutral_zone = (
                current_pc >= self.sentiment_config['extreme_low'] * 1.1 and
                current_pc <= self.sentiment_config['extreme_high'] * 0.9
            )
            ...
            if len(features) >= self.percentile_window:
                ...
                def calculate_percentile(series):
                    ...
                    percentile = safe_divide((historical_values < current_value).sum(), len(historical_values), default_value=0.5)
                    return percentile
                
                pc_percentile = pc_series.rolling(
                    window=min(self.percentile_window, len(pc_series)),
                    min_periods=10
    ...
    def _calculate_pc_trends(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            features['pc_ratio_ma5'] = rolling_features[f'pc_ratio_rolling_mean_{self.short_window}']
            features['pc_ratio_ma20'] = rolling_features[f'pc_ratio_rolling_mean_{self.long_window}']
        ...
        try:
            if self.options_chain is None or self.options_chain.empty:
                ...
                if 'pc_volume_ratio' in features.columns:
                    ...
                    # Institutional bias
                    institutional_bias = safe_divide((smart_money_pc - retail_pc), retail_pc, default_value=0.0)
            ...
            if 'smart_money_pc_ratio' in features.columns and 'retail_pc_ratio' in features.columns:
                ...
                institutional_bias = safe_divide((smart_pc - retail_pc), retail_pc, default_value=0.0)
                features['institutional_bias'] = institutional_bias
    ...
    def _calculate_moneyness_pc_ratios(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate moneyness-based P/C ratios."""
        try:
            if self.options_chain is None or self.options_chain.empty:
                # Estimate moneyness P/C ratios
                base_pc = features.get('pc_volume_ratio', pd.Series([1.0])).iloc[-1] if len(features) > 0 else 1.0
                ...
                # Moneyness spread
                features['pc_moneyness_spread'] = features['otm_pc_ratio'] - features['itm_pc_ratio']
            ...
            # Moneyness spread
            itm_pc = features.get('itm_pc_ratio', pd.Series([1.0])).iloc[-1] if len(features) > 0 else 1.0
            otm_pc = features.get('otm_pc_ratio', pd.Series([1.0])).iloc[-1] if len(features) > 0 else 1.0
    ...
    def _calculate_advanced_pc_metrics(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(pc_series) >= self.long_window:
                ...
                features['pc_ratio_volatility'] = rolling_features[f'pc_ratio_rolling_std_{self.long_window}']
            else:
            ...
            if len(pc_series) >= self.long_window:
                ...
                pc_skewness = pc_series.rolling(window=self.long_window, min_periods=3).apply(calculate_skewness)
            ...
            if current_pc > neutral_pc:
                # Bearish territory
                sentiment_score = safe_divide((current_pc - neutral_pc), (max_pc - neutral_pc), default_value=0.0)
                sentiment_score = min(sentiment_score, 1.0)
            else:
                # Bullish territory
                sentiment_score = safe_divide((current_pc - neutral_pc), (neutral_pc - min_pc), default_value=0.0)
