...
class OptionsAnalyticsFacade:
            ...
            for calc in self.calculators:
                try:
                    ...
                    if not calc_features.empty:
                        ...
                        logger.debug(f"{calc.__class__.__name__} calculated {len(calc_features.columns)} features")
                    else:
        ...
        try:
            ...
            for calc in self.calculators:
                try:
                    ...
                    logger.warning(f"Error getting feature names from {calc.__class__.__name__}: {e}")
                    continue
    ...
    def get_options_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'data_status': {
                'has_options_chain': self.options_chain is not None and not self.options_chain.empty,
                'has_historical_iv': self.historical_iv is not None and not self.historical_iv.empty,
                'options_count': len(self.options_chain) if self.options_chain is not None else 0,
                'iv_history_length': len(self.historical_iv) if self.historical_iv is not None else 0
            },
