...
from ..helpers import (
    ...
    calculate_rolling_std, aggregate_features
)
...
class MonetaryImpactCalculator(BaseNewsCalculator):
    ...
    def calculate(self, news_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            analyst_features = self._calculate_analyst_features(
                validated_data, monetary_data
            )
    ...
    def _calculate_price_target_features(
        self,
    ...
    def _calculate_estimate_features(
        self,
        monetary_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        if earnings_values:
            ...
            features['earnings_estimate_change'] = self._calculate_estimate_change(
                earnings_values
            )
            features['earnings_revision_direction'] = np.sign(
                features['earnings_estimate_change']
            )
            features['earnings_revision_magnitude'] = abs(
                features['earnings_estimate_change']
            )
        else:
        ...
        if revenue_values:
            ...
            features['revenue_estimate_change'] = self._calculate_estimate_change(
                revenue_values
            )
            features['revenue_revision_direction'] = np.sign(
                features['revenue_estimate_change']
            )
        else:
    ...
    def _calculate_corporate_features(
        self,
        monetary_data: pd.DataFrame
    ) -> pd.DataFrame:
        """Calculate deal and corporate action features."""
    ...
    def _calculate_policy_features(
        self,
        news_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        features['rate_hike_probability'] = safe_divide(
            rate_hike_count, total_policy_mentions
        )
        features['rate_cut_probability'] = safe_divide(
            rate_cut_count, total_policy_mentions
        )
