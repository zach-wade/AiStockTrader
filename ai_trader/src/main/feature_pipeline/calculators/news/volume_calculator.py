...
from ..helpers import (
    ...
    calculate_rolling_std, calculate_entropy, aggregate_features
)
...
class VolumeCalculator(BaseNewsCalculator):
    ...
    def calculate(self, news_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for window in self.volume_windows:
                window_features = self._calculate_window_volumes(
                    validated_data, window
                )
    ...
    def _calculate_window_volumes(
        ...
        if not window_data.empty:
            ...
            features[f'news_frequency_{window_suffix}'] = safe_divide(
                features[f'news_count_{window_suffix}'],
                window_hours,
                default_value=0.0
            )
            ...
            features[f'avg_articles_per_source_{window_suffix}'] = safe_divide(
                features[f'news_count_{window_suffix}'],
                features[f'unique_sources_{window_suffix}']
            )
            ...
            # Fill with zeros
            for col in [f'news_count_{window_suffix}',
                       f'news_frequency_{window_suffix}',
                       f'unique_sources_{window_suffix}',
                       f'avg_articles_per_source_{window_suffix}',
                       f'news_density_{window_suffix}']:
    ...
    def _calculate_velocity_metrics(
        ...
        for window in self.velocity_windows:
            ...
            features[f'velocity_ratio_{window_suffix}'] = safe_divide(
                current_count,
                prev_count + 1  # Add 1 to avoid division by zero
            )
    ...
    def _calculate_burst_metrics(
        ...
        if not baseline_data.empty:
            hourly_counts = baseline_data.groupby(
                pd.Grouper(freq='1H')
            ).size()
    ...
    def _calculate_coverage_metrics(
        ...
        if not window_data.empty:
            ...
            features['exclusive_coverage_ratio'] = self._calculate_exclusive_ratio(
                window_data
            )
            
            # Redundancy score
            features['redundancy_score'] = self._calculate_redundancy_score(
                window_data
            )
        else:
            # Fill with defaults
    ...
    def _calculate_flow_patterns(
        ...
        features['news_lifecycle_stage'] = self._determine_lifecycle_stage(
            news_data, features
        )
    ...
    def _calculate_time_since_burst(
        self,
        burst_indicator: pd.Series
    ) -> pd.Series:
    ...
    def _calculate_coverage_depth(
                ...
                for i in range(len(idx_headlines)):
                    for j in range(i + 1, len(idx_headlines)):
                        ...
                        if words1 and words2:
                            overlap = safe_divide(len(words1 & words2), min(len(words1), len(words2)), default_value=0.0)
        ...
        return pd.Series(
            depth_scores,
            index=window_data.index.unique()
        ).reindex(window_data.index, method='ffill')
    
    def _calculate_exclusive_ratio(
        self,
        window_data: pd.DataFrame
    ) -> pd.Series:
    ...
    def _calculate_redundancy_score(
        ...
        def calc_redundancy(group):
            ...
            return safe_divide(redundant_count, (len(group) * (len(group) - 1) / 2), default_value=0.0)
    ...
    def _calculate_decay_rate(
        ...
        for i in range(1, len(hourly_counts) - 1):
            if (hourly_counts.iloc[i] > hourly_counts.iloc[i-1] and 
