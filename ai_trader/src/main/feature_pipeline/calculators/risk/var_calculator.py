...
class VaRResult:
    ...
    # Model validation
    exceptions: Optional[int] = None            # VaR breaches in backtest
    exception_rate: Optional[float] = None      # Actual vs expected breach rate
    kupiec_test_pvalue: Optional[float] = None  # Kupiec test p-value
...
class VaRCalculator(BaseRiskCalculator):
    ...
    def _calculate_basic_var_metrics(self, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for confidence in self.confidence_levels:
                ...
                for horizon in self.time_horizons:
                    ...
                    features[f'expected_shortfall_{conf_str}_{horizon_str}'] = var_result.expected_shortfall
    ...
    def _calculate_method_specific_var(self, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_var_single(self, returns: pd.Series, confidence: float, 
                            horizon: int, method: VaRMethod) -> VaRResult:
        ...
        try:
            ...
            if method == VaRMethod.HISTORICAL:
                ...
                expected_shortfall = self._monte_carlo_expected_shortfall(scaled_returns, confidence)
                
            elif method == VaRMethod.EXTREME_VALUE:
                var_value = self._extreme_value_var(scaled_returns, confidence)
                expected_shortfall = self._extreme_value_expected_shortfall(scaled_returns, confidence)
    ...
    def _calculate_var_validation(self, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(var_values) > 0:
                ...
                if exceptions > 0:
                    kupiec_lr = -2 * np.log(
                        (expected_rate ** exceptions) * ((1 - expected_rate) ** (len(actual_returns) - exceptions))
                    )
