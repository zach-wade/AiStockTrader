...
class DrawdownCalculator(BaseRiskCalculator):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            features = self._calculate_drawdown_adjusted_performance(cumulative_returns, returns, features)
            features = self._calculate_drawdown_risk_metrics(cumulative_returns, returns, features)
            features = self._calculate_advanced_drawdown_metrics(cumulative_returns, returns, features)
    ...
    def _calculate_basic_drawdown_metrics(self, cumulative_returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if underwater_periods:
                ...
                recovery_times = [period['recovery_time'] for period in underwater_periods 
    ...
    def _calculate_drawdown_distribution(self, cumulative_returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(non_zero_drawdowns) > 0:
                ...
                # Percentiles
                features['drawdown_95_percentile'] = abs(non_zero_drawdowns.quantile(0.05))  # 5th percentile
                features['drawdown_99_percentile'] = abs(non_zero_drawdowns.quantile(0.01))  # 1st percentile
    ...
    def _calculate_recovery_analysis(self, cumulative_returns: pd.Series, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            completed_recoveries = len([p for p in underwater_periods if p['recovery_time'] is not None])
    ...
    def _calculate_rolling_drawdown_metrics(self, cumulative_returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_drawdown_adjusted_performance(self, cumulative_returns: pd.Series, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_drawdown_risk_metrics(self, cumulative_returns: pd.Series, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_advanced_drawdown_metrics(self, cumulative_returns: pd.Series, returns: pd.Series, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(drawdown) > 20:
                ...
                try:
                    ...
                    features['drawdown_persistence'] = persistence if not np.isnan(persistence) else 0
                except (ValueError, TypeError):
            ...
            if len(drawdown) > 60:
                ...
                if rolling_expected_dd and len(rolling_expected_dd) > 0:
                    ...
                    surprises = (actual_dd < pd.Series(rolling_expected_dd, index=actual_dd.index)).sum()
