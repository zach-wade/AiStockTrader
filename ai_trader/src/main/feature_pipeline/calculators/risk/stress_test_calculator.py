...
class StressTestCalculator(BaseRiskCalculator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.stress_factors = config.get('stress_factors', {
    ...
    def get_feature_names(self) -> List[str]:
        ...
        for scenario in self.historical_scenarios:
            features.extend([
                f'{scenario}_impact',
                f'{scenario}_var_impact',
                f'{scenario}_recovery_time'
            ])
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            aggregate_features = self._calculate_aggregate_metrics(
                features, returns
            )
    ...
    def _calculate_historical_scenarios(
        self,
        returns: pd.Series
    ) -> pd.DataFrame:
    ...
    def _calculate_parametric_stress(
        self,
        returns: pd.Series
    ) -> pd.DataFrame:
        ...
        if market_impacts:
            features['market_shock_sensitivity'] = pd.Series(
                [np.mean([abs(imp) for imp in market_impacts])] * len(returns),
                index=returns.index
            )
        ...
        if vol_impacts:
            features['volatility_shock_impact'] = pd.concat(
                vol_impacts, axis=1
            ).mean(axis=1)
    ...
    def _calculate_monte_carlo_stress(
        self,
        returns: pd.Series
    ) -> pd.DataFrame:
        """Run Monte Carlo stress simulations."""
        ...
        simulated_returns = secure_numpy_normal(
            stressed_mean,
            stressed_vol,
            (self.n_simulations, self.simulation_horizon)
        )
        ...
        features['mc_expected_shortfall_stressed'] = np.mean(tail_returns) if len(tail_returns) > 0 else var_threshold
    ...
    def _calculate_factor_stress(
        ...
        rolling_beta = returns.rolling(window=60).apply(
            lambda x: np.corrcoef(x, market_proxy[-len(x):])[0, 1] * (x.std() / market_proxy[-len(x):].std()),
            raw=False
        )
    ...
    def _calculate_aggregate_metrics(
        ...
        if scenario_impacts:
            aggregate['stress_test_score'] = pd.concat(
                scenario_impacts, axis=1
            ).mean(axis=1)
        
        # Resilience indicator (inverse of stress score)
        aggregate['resilience_indicator'] = 1 / (
            1 + aggregate.get('stress_test_score', 0)
        )
