...
from .risk_config import RiskConfig, VaRMethod, VolatilityMethod
...
class BaseRiskCalculator(BaseFeatureCalculator, ABC):
    ...
    def validate_input_data(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if len(data) < self.risk_config.var_min_observations:
                logger.warning(f"Insufficient data: {len(data)} < {self.risk_config.var_min_observations}")
                return False
    ...
    def calculate_rolling_statistics(self, returns: pd.Series, window: int) -> pd.DataFrame:
        ...
        try:
            if len(returns) < window:
                logger.warning(f"Insufficient data for rolling statistics: {len(returns)} < {window}")
                return pd.DataFrame()
    ...
    def calculate_exponential_statistics(self, returns: pd.Series, decay: float = 0.94) -> pd.Series:
        """
        ...
        try:
            ...
            if distribution == 'normal':
                ...
                return {
                    ...
                    'bic': len(returns) * np.log(2 * np.pi * sigma**2) + len(returns) + 2 * np.log(len(returns))
                }
                ...
                return {
                    ...
                    'bic': -2 * np.sum(stats.t.logpdf(returns, df, loc, scale)) + 3 * np.log(len(returns))
                }
    ...
    def annualize_metric(self, metric: Union[float, pd.Series], 
