...
class LeadLagCalculator(BaseCorrelationCalculator):
    ...
    def _calculate_cross_asset_timing(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_timing_dynamics(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if lead_features:
                lead_values = [features[col].iloc[0] if len(features) > 0 else 0.0 for col in lead_features]
            ...
            if 'optimal_timing_strength' in features.columns:
                ...
                timing_strength = features['optimal_timing_strength'].iloc[0] if len(features) > 0 else 0.0
            ...
            if 'optimal_timing_strength' in features.columns and 'timing_consistency_score' in features.columns:
                strength = features['optimal_timing_strength'].iloc[0] if len(features) > 0 else 0.0
                consistency = features['timing_consistency_score'].iloc[0] if len(features) > 0 else 0.0
    ...
    def _calculate_advanced_timing(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate advanced timing features."""
        try:
            ...
            if not returns_pivot.empty:
                ...
                if 'timestamp' in data.columns:
                    ...
                    data_with_dow['day_of_week'] = pd.to_datetime(data_with_dow['timestamp']).dt.dayofweek
            ...
            if all_leadlag_corrs:
                ...
                diversification = 1.0 - concentration if 'lead_lag_concentration' in features.columns else 0.0
