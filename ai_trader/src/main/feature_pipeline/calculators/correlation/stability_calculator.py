...
from ..helpers import (
    create_feature_dataframe, safe_divide, calculate_rolling_std,
    calculate_entropy, calculate_hurst_exponent
)
...
class StabilityAnalysisCalculator(BaseCorrelationCalculator):
    ...
    def get_feature_names(self) -> List[str]:
        ...
        features.extend([
            'cross_corr_stability',
            'lead_lag_stability',
            'correlation_network_stability'
        ])
        ...
        try:
            ...
            if self.asset_data:
                ...
                if self.benchmark_data is not None:
                    ...
                    base_correlation = returns.rolling(
                ...
                base_correlation = returns.rolling(
                    window=20, min_periods=10
                ).apply(lambda x: x.autocorr(lag=1), raw=True)
    ...
    def _calculate_stability_metrics(
        ...
        for window in self.stability_windows:
            # Rolling statistics
            rolling_mean = correlation_series.rolling(
                window=window, min_periods=window//2
            ).mean()
            
            rolling_std = correlation_series.rolling(
                window=window, min_periods=window//2
            ).std()
    ...
    def _calculate_persistence_metrics(
        ...
        features['corr_half_life'] = correlation_series.rolling(
            window=50, min_periods=25
        ).apply(self._calculate_half_life, raw=True)
    ...
    def _detect_correlation_regimes(
        self,
        correlation_series: pd.Series
    ) -> pd.DataFrame:
        ...
        features['current_regime_stability'] = correlation_series.rolling(
            window=20, min_periods=10
        ).std().rolling(window=20).mean()
        ...
        # Regime entropy
        features['regime_entropy'] = regime.rolling(
            window=50, min_periods=25
        ).apply(lambda x: calculate_entropy(pd.Series(x), bins=3), raw=True)
        ...
        for i, idx in enumerate(correlation_series.index):
            if len(break_times) > 0:
                ...
                if len(recent_breaks) > 0:
                    ...
                    features.loc[idx, 'time_since_break'] = \
                        len(pd.bdate_range(last_break, idx))
        
        # Break magnitude
        features['break_magnitude'] = cusum.abs()
        
        # Break confidence (based on statistical significance)
        features['break_confidence'] = self._calculate_break_confidence(
            correlation_series, cusum
        )
    ...
    def _calculate_confidence_intervals(
        ...
        for confidence in [0.90, 0.95]:
            ...
            features[f'corr_ci_lower_{int(confidence*100)}'] = \
                rolling_mean - z_score * rolling_std
            features[f'corr_ci_upper_{int(confidence*100)}'] = \
                rolling_mean + z_score * rolling_std
            features[f'corr_ci_width_{int(confidence*100)}'] = \
                2 * z_score * rolling_std
        
        return features
    
    def _generate_stability_forecasts(
        self,
        correlation_series: pd.Series
    ) -> pd.DataFrame:
    ...
    def _calculate_multi_asset_stability(self) -> pd.DataFrame:
        ...
        for i in range(n_assets):
            for j in range(i + 1, n_assets):
                ...
                corr = returns_df[asset1].rolling(
                    window=20, min_periods=10
        ...
        if stability_scores:
            features['cross_corr_stability'] = pd.concat(
                stability_scores, axis=1
            ).mean(axis=1)
        ...
        features['correlation_network_stability'] = self._calculate_network_stability(
            returns_df
        )
