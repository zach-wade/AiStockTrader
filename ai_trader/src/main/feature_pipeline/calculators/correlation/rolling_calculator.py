...
class RollingCorrelationCalculator(BaseCorrelationCalculator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.debug(f"Initialized RollingCorrelationCalculator with {len(self.benchmark_symbols)} benchmarks")
    ...
    def get_feature_names(self) -> List[str]:
        ...
        for benchmark in self.benchmark_symbols:
            feature_names.extend([
                f'ewm_corr_{benchmark}',
                f'ewm_corr_{benchmark}_change'
            ])
    ...
    def _calculate_benchmark_correlations(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
                    ...
                    for symbol in returns_pivot.columns:
                        ...
                        for timestamp in rolling_corr.index:
                            if timestamp in features.index:
                                ...
                                features.loc[timestamp, f'corr_{benchmark}_{window}d'] = rolling_corr.loc[timestamp]
                                features.loc[timestamp, f'corr_{benchmark}_{window}d_std'] = corr_std.loc[timestamp]
                                features.loc[timestamp, f'corr_{benchmark}_{window}d_trend'] = corr_trend.loc[timestamp]
    ...
    def _calculate_ewm_correlations(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate exponentially weighted correlations."""
        try:
            returns_pivot = self.pivot_returns_data(data)
            ...
            for benchmark in self.benchmark_symbols:
                ...
                if ewm_correlations:
                    ...
                    for timestamp in avg_ewm_corr.index:
                        if timestamp in features.index:
                            features.loc[timestamp, f'ewm_corr_{benchmark}'] = avg_ewm_corr.loc[timestamp]
                            features.loc[timestamp, f'ewm_corr_{benchmark}_change'] = ewm_change.loc[timestamp]
    ...
    def _calculate_cross_sectional_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """Calculate cross-sectional correlation features."""
        try:
            returns_pivot = self.pivot_returns_data(data)
            ...
            for window in [20, 60, 120]:
                ...
                for i in range(window, len(returns_pivot)):
                    ...
                    if timestamp in features.index:
                        # Average correlation
                        features.loc[timestamp, f'avg_benchmark_corr_{window}d'] = upper_triangle.mean()
                        
                        if window == 20:  # Only calculate these for 20d window
                            # Max and min correlations
                            features.loc[timestamp, f'max_benchmark_corr_{window}d'] = upper_triangle.max()
                            features.loc[timestamp, f'min_benchmark_corr_{window}d'] = upper_triangle.min()
                            
                            # Correlation dispersion
                            features.loc[timestamp, f'corr_dispersion_{window}d'] = upper_triangle.std()
                            ...
                            if len(self.benchmark_symbols) > 0:
                                ...
                                for benchmark in self.benchmark_symbols:
                                    if benchmark in corr_matrix.columns:
                                        benchmark_avg_corr = corr_matrix[benchmark].drop(benchmark).abs().mean()
                                        benchmark_corrs.append(benchmark_avg_corr)
                                
                                if benchmark_corrs:
                                    features.loc[timestamp, 'dominant_benchmark_corr'] = max(benchmark_corrs)
    ...
    def _calculate_correlation_dynamics(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            mean_reversion = (corr_series - rolling_mean) / (rolling_mean + self.numerical_tolerance)
