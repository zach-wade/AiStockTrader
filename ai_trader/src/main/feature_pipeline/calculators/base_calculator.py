...
from main.utils.core import ErrorHandlingMixin, get_logger, AsyncCircuitBreaker
from .helpers import (
    validate_ohlcv_data, preprocess_data, check_data_quality,
    create_feature_dataframe, postprocess_features
)
...
class BaseFeatureCalculator(ErrorHandlingMixin, ABC):
    ...
    def validate_data(self, data: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        if 'open' in required_columns and 'high' in required_columns:
            ...
            is_valid, ohlcv_errors = validate_ohlcv_data(
                ...
                min_rows=self.min_required_rows
            )
    ...
    def _create_empty_features(self, index: Optional[pd.Index] = None) -> pd.DataFrame:
        """
    ...
    def _generate_cache_key(self, data: pd.DataFrame) -> Optional[str]:
        """
