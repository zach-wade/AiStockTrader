...
class TrendIndicatorsCalculator(BaseTechnicalCalculator):
    ...
    def _calculate_moving_averages(
        ...
        for ma_type in self.ma_types:
            for period in self.ma_periods:
                ...
                # Price to MA ratio
                features[f'price_to_{ma_type}_{period}_ratio'] = safe_divide(
                    close, ma
                )
        ...
        if 'sma_50' in ma_dict and 'sma_200' in ma_dict:
            ...
            features['ma_50_200_spread'] = safe_divide(
                ma_dict['sma_50'] - ma_dict['sma_200'],
                ma_dict['sma_200']
            )
    ...
    def _calculate_aroon_features(
        ...
        high_period = data['high'].rolling(window=period + 1).apply(
            lambda x: period - x.argmax() if len(x) == period + 1 else np.nan,
            raw=True
        )
        features['aroon_up'] = (high_period / period) * 100
        
        # Aroon Down: (period - periods since lowest low) / period * 100
        low_period = data['low'].rolling(window=period + 1).apply(
            lambda x: period - x.argmin() if len(x) == period + 1 else np.nan,
            raw=True
        )
    ...
    def _calculate_psar_features(
        ...
        features['psar_distance'] = safe_divide(
            data['close'] - psar,
            data['close']
        )
    ...
    def _calculate_supertrend_features(
        self,
        data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        features['supertrend_distance'] = safe_divide(
    ...
    def _calculate_ichimoku_features(
        ...
        features['ichimoku_span_b'] = (
            (span_b_high + span_b_low) / 2
        ).shift(params['displacement'])
    ...
    def _calculate_channel_features(
        self,
        data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        features['channel_position'] = safe_divide(
            data['close'] - features['donchian_lower'],
            channel_range
        )
        
        # Channel width (volatility measure)
        features['channel_width'] = safe_divide(
            channel_range,
            features['donchian_middle']
        )
