...
class VolatilityIndicatorsCalculator(BaseTechnicalCalculator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.annualization_factor = self.config.get('annualization_factor', 252)  # trading days per year
    ...
    def get_feature_names(self) -> List[str]:
        ...
        for period in self.atr_periods:
            feature_names.extend([
                f'atr_{period}', f'atr_{period}_pct'
            ])
        ...
        for period in self.bb_periods:
            feature_names.extend([
                ...
                f'bb_width_{period}', f'bb_position_{period}', f'bb_squeeze_{period}'
            ])
    ...
    def _calculate_atr_indicators(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
        """Calculate ATR indicators."""
        # ATR (Average True Range)
        for period in self.atr_periods:
            atr = talib.ATR(
    ...
    def _calculate_bollinger_bands(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
    ...
    def _calculate_keltner_channels(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
    ...
    def _calculate_historical_volatility(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
        ...
        for period in self.hist_vol_periods:
            ...
            hist_vol = returns.rolling(period, min_periods=10).std() * np.sqrt(self.annualization_factor)
            features.loc[mask, f'hist_volatility_{period}'] = hist_vol
        
        return features
    
    def _calculate_volatility_ratios(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
        ...
        if len(self.hist_vol_periods) >= 2:
            ...
            if short_vol_col in features.columns and long_vol_col in features.columns:
                volatility_ratio = (
                    features.loc[mask, short_vol_col] / 
                    features.loc[mask, long_vol_col]
                ).fillna(1.0)
