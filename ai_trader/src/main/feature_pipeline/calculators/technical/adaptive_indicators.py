...
class AdaptiveIndicatorsCalculator(BaseTechnicalCalculator):
    ...
    def get_feature_names(self) -> List[str]:
        ...
        feature_names.extend([
            'kama', 'price_to_kama', 'adaptive_rsi', 'vma', 'frama',
            'market_efficiency_ratio', 'adaptive_momentum'
        ])
    ...
    def _calculate_kama_indicators(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
        ...
        def calculate_kama(prices, period=10, fast_ema=2, slow_ema=30):
            ...
            for i in range(period + 1, len(prices)):
                if not np.isnan(smoothing_constant.iloc[i]):
                    kama[i] = kama[i-1] + smoothing_constant.iloc[i] * (prices.iloc[i] - kama[i-1])
                else:
                    kama[i] = kama[i-1]
            
            return pd.Series(kama, index=prices.index)
        
        kama = calculate_kama(symbol_data['close'], self.kama_period, self.kama_fast_ema, self.kama_slow_ema)
    ...
    def _calculate_adaptive_rsi(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
        ...
        volatility_rank = atr_series.rolling(self.adaptive_rsi_volatility_window, min_periods=10).rank(pct=True)
        ...
        adaptive_rsi = talib.RSI(
            symbol_data['close'].values,
            timeperiod=adaptive_period
        )
        features.loc[mask, 'adaptive_rsi'] = adaptive_rsi
        
        return features
    
    def _calculate_vma_indicators(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
    ...
    def _calculate_frama_indicators(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
        ...
        def calculate_frama(prices, period=20):
            ...
            for i in range(n2, len(prices)):
                ...
                if highest != lowest and n1_high != n1_low and n2_high != n2_low:
                    try:
                        d = (np.log(np.abs(n1_high - n1_low) + np.abs(n2_high - n2_low)) -
                             np.log(highest - lowest)) / np.log(2)
    ...
    def _calculate_efficiency_indicators(self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
