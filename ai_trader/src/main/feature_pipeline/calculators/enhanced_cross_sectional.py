...
class EnhancedCrossSectionalCalculator(BaseFeatureCalculator):
    ...
    def _calculate_factor_features(
        ...
        for window in self.factor_windows:
            # PCA-based factors
            pca_features = self._calculate_pca_factors(
                returns_data, asset, window, asset_index
            )
    ...
    def _calculate_pca_factors(
        ...
        for factor_num in range(self.n_factors):
            ...
            scores_series = pd.Series(
                [scores[factor_num] for scores in factor_scores],
                index=returns_data.index[window:]
            )
    ...
    def _calculate_regime_features(
        ...
        if not dispersion.empty:
            ...
            regime_indicator = safe_divide(
                dispersion - dispersion_ma,
                dispersion_std
            )
            features['regime_indicator'] = regime_indicator.reindex(
                asset_index, method='ffill'
            )
            
            # Regime momentum score
            regime_momentum = regime_indicator.rolling(window=20).mean()
            features['regime_momentum_score'] = regime_momentum.reindex(
                asset_index, method='ffill'
            )
        ...
        base_rank = returns_data.rolling(window=20).apply(
            lambda x: stats.percentileofscore(x.iloc[-1], x.mean()),
            raw=False, axis=1
        )[asset]
        ...
        features['regime_adjusted_rank'] = regime_adjusted_rank.reindex(
            asset_index, method='ffill'
        )
        
        # Regime transition score
        regime_changes = features.get('regime_indicator', pd.Series()).diff().abs()
        features['regime_transition_score'] = regime_changes.reindex(
            asset_index, method='ffill'
        )
    ...
    def _calculate_advanced_momentum(
        ...
        exp_momentum = asset_returns.rolling(
            window=self.momentum_halflife
        ).apply(
            lambda x: np.average(x, weights=weights[:len(x)]),
            raw=True
        )
        features['exponential_momentum_score'] = exp_momentum.reindex(
            asset_index, method='ffill'
        )
        ...
        features['momentum_acceleration'] = momentum_accel.reindex(
            asset_index, method='ffill'
        )
        ...
        if not market_return.empty:
            ...
            beta = asset_returns.rolling(window=60).cov(market_return) / market_return.rolling(window=60).var()
    ...
    def _calculate_risk_decomposition(
        ...
        if not market_return.empty:
            ...
            rolling_beta = asset_returns.rolling(window=60).cov(market_return) / market_return.rolling(window=60).var()
            features['systematic_risk_loading'] = rolling_beta.reindex(
                asset_index, method='ffill'
            )
        ...
        # Risk-adjusted score
        risk_adjusted = safe_divide(
            features.get('exponential_momentum_score', 0),
            asset_vol
        )
    ...
    def _calculate_sector_features(
        ...
        if len(similar_assets) > 1:
            ...
            # Rank within sector
            sector_rank = returns_data[similar_assets].rolling(
                window=self.sector_rotation_window
            ).apply(
                lambda x: stats.percentileofscore(x.iloc[-1], asset_returns.iloc[x.index[-1]]),
                raw=False, axis=1
            )[asset]
            
            features['sector_momentum_rank'] = sector_rank.reindex(
                asset_index, method='ffill'
            ) / 100
    ...
    def _calculate_dynamic_features(
        ...
        features['adaptive_ranking_score'] = adaptive_ranking.reindex(
            asset_index, method='ffill'
        )
        ...
        if not market_return.empty:
            ...
            for i in range(60, len(asset_returns)):
                ...
                weighted_var = np.average(
                    (recent_market - recent_market.mean()) ** 2,
                    weights=weights
                )
            ...
            features['time_varying_beta'] = tv_beta_series.reindex(
                asset_index, method='ffill'
            )
        ...
        features['dynamic_correlation_score'] = dynamic_corr.reindex(
            asset_index, method='ffill'
        )
    ...
    def _calculate_ml_features(
        ...
        features['ml_momentum_probability'] = ml_momentum.reindex(
            asset_index, method='ffill'
        )
        ...
        for i in range(60, len(asset_returns)):
            ...
            current_position = (window_returns.iloc[-1] - window_returns.mean()) / window_returns.std()
        ...
        features['ml_reversion_probability'] = ml_reversion.reindex(
            asset_index, method='ffill'
        )
        ...
        for i in range(60, len(asset_returns)):
            window_data = returns_data.iloc[i-60:i]
            if asset in window_data.columns:
                asset_zscore = (window_data[asset].iloc[-1] - window_data[asset].mean()) / window_data[asset].std()
        ...
        features['anomaly_detection_score'] = anomaly_series.reindex(
            asset_index, method='ffill'
        )
        ...
        features['pattern_recognition_signal'] = pattern_series.reindex(
            asset_index, method='ffill'
        )
