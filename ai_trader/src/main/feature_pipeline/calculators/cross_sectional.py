...
class CrossSectionalCalculator(BaseFeatureCalculator):
    ...
    def calculate(self, data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        try:
            ...
            for asset in data.keys():
                ...
                relative_features = self._calculate_relative_performance(
                    cs_data, asset, asset_data
                )
    ...
    def _calculate_ranking_features(
        ...
        for window in self.ranking_windows:
            ...
            asset_zscore = safe_divide(
                rolling_returns[asset] - mean_return,
                std_return
    ...
    def _calculate_momentum_features(
        ...
        for window in self.momentum_windows:
            ...
            asset_momentum_rank = momentum_rank[asset].reindex(
            ...
            momentum_strength = safe_divide(
                momentum[asset] - cs_mean,
                cs_mean.abs()
            ).reindex(asset_index, method='ffill')
            features[f'momentum_strength_{window}d'] = momentum_strength
            
            # Momentum persistence (consistency of ranking)
            momentum_consistency = momentum_rank[asset].rolling(
                window=window
            ).std().reindex(asset_index, method='ffill')
    ...
    def _calculate_relative_performance(
        ...
        for period in rsi_periods:
            ...
            features['relative_strength_index'] = rsi.reindex(
        ...
        features['beta_adjusted_return'] = beta_adjusted.reindex(
            asset_data.index, method='ffill'
        )
        
        # Alpha estimate
        features['alpha_estimate'] = beta_adjusted.rolling(
            window=window
        ).mean().reindex(asset_data.index, method='ffill')
    ...
    def _calculate_volatility_features(
        ...
        features['relative_volatility_rank'] = vol_rank[asset].reindex(
            asset_index, method='ffill'
        )
        ...
        vol_zscore = safe_divide(
            volatilities[asset] - vol_mean,
            vol_std
        ).reindex(asset_index, method='ffill')
        ...
        features['vol_adjusted_return'] = vol_adjusted.reindex(
            asset_index, method='ffill'
        )
        ...
        features['sharpe_rank'] = sharpe_rank[asset].reindex(
            asset_index, method='ffill'
        )
    ...
    def _calculate_dispersion_features(
        ...
        features['cross_sectional_dispersion'] = cs_std.reindex(
            asset_index, method='ffill'
        )
        ...
        idiosyncratic_series = pd.Series(
            idiosyncratic_var,
            index=cs_data.index[window:]
        )
        features['idiosyncratic_risk'] = idiosyncratic_series.reindex(
            asset_index, method='ffill'
        )
    ...
    def _calculate_composite_scores(
        ...
        composite['cross_momentum_signal'] = (
