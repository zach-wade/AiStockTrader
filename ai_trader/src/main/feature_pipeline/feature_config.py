...
class FeatureConfig:
    ...
    def _load_calculator_configs(self) -> Dict[str, CalculatorConfig]:
        ...
        for calc_name, default_config in default_calculators.items():
            ...
            calculator_configs[calc_name] = CalculatorConfig(
                name=calc_name,
                **merged_config
            )
    ...
    def _load_feature_set_configs(self) -> Dict[str, FeatureSetConfig]:
        ...
        default_sets = {
            ...
            'full_feature_set': {
                ...
                'calculators': [
                    'technical_indicators', 'advanced_statistical', 'cross_asset',
                    'sentiment_features', 'market_regime', 'microstructure'
                ],
                'required_data_types': ['market_data', 'news', 'social']
            }
        }
        ...
        for set_name, default_config in default_sets.items():
            ...
            feature_set_configs[set_name] = FeatureSetConfig(
                name=set_name,
                **merged_config
            )
    ...
    def get_calculator_config(self, calculator_name: str) -> Optional[CalculatorConfig]:
    ...
    def get_enabled_calculators(self) -> List[str]:
        ...
        return [
            name for name, config in self.calculator_configs.items()
            if config.enabled
        ]
    ...
    def get_calculators_by_priority(self) -> List[str]:
        ...
        enabled_calculators = [
            (name, config) for name, config in self.calculator_configs.items()
            if config.enabled
        ]
    ...
    def resolve_calculator_order(self, requested_calculators: Optional[List[str]] = None) -> List[str]:
        """
        ...
        requested_calculators = [
            name for name in requested_calculators
    ...
    def get_feature_set_config(self, set_name: str) -> Optional[FeatureSetConfig]:
    ...
    def validate_configuration(self) -> Dict[str, List[str]]:
        ...
        for calc_name, config in self.calculator_configs.items():
            ...
            if config.priority < 1 or config.priority > 10:
                errors['calculators'].append(f"Calculator {calc_name} invalid priority: {config.priority}")
        ...
        for set_name, config in self.feature_set_configs.items():
            ...
            for calc_name in config.calculators:
                if calc_name not in self.calculator_configs:
                    errors['feature_sets'].append(f"Feature set {set_name} references unknown calculator: {calc_name}")
        
        # Validate dependencies
        for calc_name, config in self.calculator_configs.items():
            for dep in config.dependencies:
                if dep not in self.calculator_configs:
                    errors['dependencies'].append(f"Calculator {calc_name} depends on unknown calculator: {dep}")
