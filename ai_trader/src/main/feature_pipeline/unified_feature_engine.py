...
class UnifiedFeatureEngine:
    ...
    def __init__(self, config: DictConfig):
        ...
        logger.info(f"✅ UnifiedFeatureEngine initialized with {len(self.calculators)} calculators.")
    ...
    def calculate_features(self, data: pd.DataFrame, symbol: str, 
        ...
        if calculators is None:
            ...
            calculators_to_use = {k: v for k, v in self.calculators.items() 
                                if k in calculators}
            
        logger.debug(f"Calculating features for {symbol} using calculators: {list(calculators_to_use.keys())}")
        ...
        for name, calculator in calculators_to_use.items():
            try:
                ...
                if hasattr(calculator, 'validate_input_data') and callable(getattr(calculator, 'validate_input_data')):
                ...
                elif hasattr(calculator, 'validate_inputs') and callable(getattr(calculator, 'validate_inputs')):
