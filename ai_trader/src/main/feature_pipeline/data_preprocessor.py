...
class PreprocessorConfig:
    ...
    imputation_method: str = 'forward_fill'  # 'forward_fill', 'backward_fill', 'mean', 'median', 'knn'
...
class DataPreprocessor:
    ...
    def preprocess_feature_data(self, data: pd.DataFrame, feature_type: str = 'general') -> pd.DataFrame:
        """
    ...
    def _handle_missing_data(self, data: pd.DataFrame, data_type: str) -> pd.DataFrame:
        ...
        if columns_to_drop:
            logger.warning(f"Dropping columns with >{self.config.missing_threshold*100}% missing data: {columns_to_drop}")
        ...
        for col in volume_cols:
            if col in data.columns:
                ...
                if low_volume_mask.any():
                    logger.debug(f"Found {low_volume_mask.sum()} rows with volume < {self.config.min_volume_threshold}")
    ...
    def _validate_data_quality(self, data: pd.DataFrame, symbol: str = None) -> pd.DataFrame:
        ...
        if len(data) < self.config.min_data_points:
            logger.warning(f"Insufficient data points for {symbol}: {len(data)} < {self.config.min_data_points}")
        ...
        if 'timestamp' in data.columns:
            ...
            if large_gaps.any():
                ...
                logger.warning(f"Found {gap_count} gaps > {self.config.max_gap_days} days in {symbol}")
        ...
        if 'close' in data.columns:
            ...
            if extreme_moves.any():
                ...
                logger.warning(f"Found {extreme_count} extreme price movements (>{self.config.price_change_threshold*100}%) in {symbol}")
    ...
    def _add_basic_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Add typical price
        if all(col in data.columns for col in ['high', 'low', 'close']) and 'typical_price' not in data.columns:
    ...
    def _scale_features(self, data: pd.DataFrame, feature_type: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.debug(f"Applied {self.config.scaling_method} scaling to {len(scale_columns)} features")
        except Exception as e:
