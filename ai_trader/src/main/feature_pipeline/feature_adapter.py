...
class FeatureAdapter(ErrorHandlingMixin):
    ...
    async def get_features(
        ...
        with self._handle_error("getting features"):
            ...
            if request.transformations:
                features = self._apply_transformations(
                    features,
    ...
    async def get_latest_features(
        ...
        available_features = [
            ...
            if f in feature_set.features.columns
        ]
    ...
    def list_available_features(
        ...
        if group:
            features = [
                f for f in features
                if self._feature_metadata[f].get('group') == group
            ]
    ...
    def _apply_filters(
        ...
        for key, value in filters.items():
            if key == 'min_date' and 'date' in result.index.names:
                ...
                result = result[
                    result.index.get_level_values('symbol').isin(value)
                ]
...
def create_feature_adapter(config: Optional[Dict[str, Any]] = None) -> FeatureAdapter:
