...
class FeatureStore:
    ...
    def __init__(self, feature_path: Optional[str] = None, config: Optional[Dict] = None):
        """
        ...
        logger.info(f"FeatureStore compatibility wrapper initialized with path: {self.features_path}")
    ...
    def save_features(self, symbol: str, features_df: pd.DataFrame, 
    ...
    def load_features(self, symbol: str, feature_type: str = "technical_indicators",
    ...
    def get_latest_features(self, symbol: str, 
                          feature_type: str = "technical_indicators") -> Optional[pd.DataFrame]:
        """
    ...
    def list_available_features(self, symbol: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    async def get_features(self, request: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    if combined_file.exists():
                        logger.info(f"Loading pre-computed features with targets from {combined_file}")
                        df = pd.read_csv(combined_file, index_col=0, parse_dates=True)
                        logger.info(f"Loaded {len(df)} rows with {len(df.columns)} columns for {symbol}")
                        ...
                        for feature_set in feature_sets:
                            if feature_set == 'all' or feature_set in ['technical', 'features', 'combined']:
                                if feature_set not in results:
                                    results[feature_set] = df.copy()
                                else:
                                    results[feature_set] = pd.concat([results[feature_set], df], ignore_index=False)
                                ...
                                for feature_set in feature_sets:
                                    if feature_set == 'all' or feature_set in ['technical', 'features', 'combined']:
                                        if feature_set not in results:
                                            ...
                                            results[feature_set] = pd.concat([results[feature_set], combined_df], ignore_index=False)
                                            
                                logger.info(f"Generated features and targets for {symbol}: {len(combined_df)} rows")
                            else:
                                logger.warning(f"No features loaded for {symbol} from HDF5")
                        else:
                            logger.warning(f"No HDF5 features file found for {symbol} at {hdf5_path}")
            ...
            for feature_set, df in results.items():
                logger.info(f"Final result for '{feature_set}': {len(df)} rows, {len(df.columns)} columns")
    ...
    async def get_historical_data(self, symbol: str, start_date: datetime, 
        ...
        try:
            ...
            market_path = data_lake_path / 'processed' / 'market_data' / f'symbol={symbol}' / f'interval={timeframe}'
