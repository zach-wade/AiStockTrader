            ...
            if results['status'] == 'success':
                ...
                if not dry_run and records_archived > 0:
                    ...
                    for repo_name, repo_status in status_after.get('repository_status', {}).items():
                        if 'error' not in repo_status:
                            before_eligible = status_before.get('repository_status', {}).get(
                                repo_name, {}
...
async def run_full_archival(dry_run: bool = False) -> Dict[str, Any]:
    ...
    with timer("full_archival") as job_timer:
        try:
            info_message(
                "Starting full repository archival",
                f"Mode: {'DRY RUN' if dry_run else 'LIVE'}"
            )
            ...
            if results['status'] == 'success':
                ...
                for repo_name, repo_result in results.get('repository_results', {}).items():
                    if repo_result.get('status') == 'success':
                        ...
                        warning_message(
                            f"{repo_name} error",
                            repo_result.get('message', 'Unknown error')
                        )
...
def main():
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Simulate the rotation without making changes'
    )
    parser.add_argument(
        '--force',
        action='store_true',
        help='Run even if disabled in configuration'
    )
    parser.add_argument(
        '--full',
        action='store_true',
        help='Archive all repository types, not just market_data'
    )
    ...
    try:
        if args.full:
            ...
            results = asyncio.run(run_storage_rotation(
