...
@click.option('--force', is_flag=True, 
              help='Force repopulation even if universe exists')
@click.option('--source', type=click.Choice(['alpaca', 'polygon']), 
              default='alpaca', help='Data source for universe')
...
@click.option('--layer', type=click.Choice(['0', '1', '2', '3', 'all']), 
              default='all', help='Layer to show statistics for')
...
@click.option('--check-all', is_flag=True, 
              help='Check health of all layers')
@click.option('--layer', type=click.Choice(['0', '1', '2', '3']), 
              help='Check specific layer health')
@click.option('--fix', is_flag=True, 
              help='Attempt to fix issues found')
@click.pass_context
def health(ctx, check_all: bool, layer: Optional[str], fix: bool):
        ...
        if check_all:
            ...
            for layer_value in range(4):  # Layers 0-3
                ...
                layer_health = {
                    ...
                    'count': len(companies),
                    'checks': {
                        'has_symbols': {'passed': len(companies) > 0, 'message': f'{len(companies)} symbols'},
...
@click.option('--to-layer', type=click.Choice(['1', '2', '3']), 
              required=True, help='Target layer')
@click.option('--reason', help='Reason for promotion')
@click.pass_context
def promote(ctx, symbol: str, to_layer: str, reason: Optional[str]):
    ...
    try:
        ...
        # Update layer
        result = asyncio.run(company_repo.update_layer(
            symbol=symbol,
            layer=layer_enum,
            metadata={'reason': reason or 'Manual promotion', 'promoted_at': datetime.now().isoformat()}
        ))
        
        if result.success:
            print(f"✅ Successfully promoted {symbol} from Layer {current_layer.value} to Layer {to_layer}")
            
            # Publish promotion event
            event_bus = EventBusFactory.create(config)
            publisher = ScannerEventPublisher(event_bus)
            asyncio.run(publisher.publish_symbol_promoted(
                symbol=symbol,
...
@universe.command()
@click.option('--layer', type=click.Choice(['0', '1', '2', '3']), 
              help='Export specific layer')
@click.option('--format', type=click.Choice(['csv', 'json', 'txt']), 
              default='csv', help='Export format')
@click.option('--output', required=True, help='Output file path')
@click.pass_context
def export(ctx, layer: Optional[str], format: str, output: str):
...
async def _get_all_layer_stats(company_repo) -> Dict:
    ...
    for layer_value in range(4):  # Layers 0-3
        ...
        stats[layer_value] = {
            ...
            'avg_liquidity_score': sum(c.get('liquidity_score', 0) for c in companies) / len(companies) if companies else 0,
            'avg_catalyst_score': sum(c.get('catalyst_score', 0) for c in companies) / len(companies) if companies else 0,
        }
...
async def _get_layer_stats(company_repo, layer: DataLayer) -> Dict:
    ...
    return {
        ...
        'avg_liquidity_score': sum(c.get('liquidity_score', 0) for c in companies) / len(companies) if companies else 0,
        'avg_catalyst_score': sum(c.get('catalyst_score', 0) for c in companies) / len(companies) if companies else 0,
        'avg_premarket_score': sum(c.get('premarket_score', 0) for c in companies) / len(companies) if companies else 0,
        'top_symbols': sorted(companies, key=lambda x: x.get('liquidity_score', 0), reverse=True)[:10]
    }
