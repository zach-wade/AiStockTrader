...
@click.option('--disable-monitoring', is_flag=True, 
              help='Disable monitoring dashboard')
@click.option('--disable-streaming', is_flag=True, 
              help='Disable real-time streaming')
@click.option('--dashboard-port', type=int, default=None, 
              help='Port for monitoring dashboard')
@click.option('--risk-limit', type=float, default=None, 
              help='Maximum risk per trade (percentage)')
@click.option('--position-limit', type=int, default=None, 
              help='Maximum number of concurrent positions')
@click.option('--capital', type=float, default=None, 
              help='Trading capital to use')
@click.option('--log-trades', is_flag=True, 
              help='Log all trades to database')
@click.option('--dry-run', is_flag=True, 
              help='Run without placing actual orders')
@click.pass_context
def trade(ctx, mode: str, symbols: Optional[str], strategy: Optional[str],
          enable_ml: bool, disable_monitoring: bool, disable_streaming: bool,
          dashboard_port: Optional[int], risk_limit: Optional[float],
          position_limit: Optional[int], capital: Optional[float],
          log_trades: bool, dry_run: bool):
...
@click.option('--model-type', type=click.Choice(['xgboost', 'lstm', 'ensemble']), 
...
@click.pass_context
def backtest(ctx, model_path: Optional[str], model_type: Optional[str],
             symbols: Optional[str], start_date: Optional[str],
             end_date: Optional[str], initial_capital: float,
             commission: float, slippage: float, strategy: Optional[str],
             compare: bool, output: Optional[str]):
    ...
    try:
        ...
        if compare:
            # Compare multiple models
            results = asyncio.run(engine.compare_models(
...
@click.option('--period', type=click.Choice(['1d', '1w', '1m', '3m', 'ytd', 'all']), 
              default='1d', help='Time period for positions')
@click.option('--format', type=click.Choice(['table', 'json', 'csv']), 
              default='table', help='Output format')
@click.pass_context
def positions(ctx, symbols: Optional[str], period: str, format: str):
...
@trading.command()
@click.option('--period', type=click.Choice(['1d', '1w', '1m', '3m', 'ytd', 'all']), 
              default='1d', help='Time period for performance')
