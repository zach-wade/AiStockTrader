...
from main.models.utils.model_loader import ModelLoader, find_latest_model, list_available_models
from main.backtesting.engine.backtest_engine import BacktestEngine, BacktestConfig, BacktestMode
...
from main.interfaces.backtesting import BacktestResult
...
class BacktestRunner:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.default_lookback_days = self.config.get('backtesting', {}).get('default_lookback_days', 365)
    ...
    async def run_model_backtest(
        ...
        try:
            ...
            logger.info(f"Loaded {model_info['model_type']} model with {model_info['features_count']} features")
    ...
    def _process_backtest_results(self, result: Any, model_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        formatted_results = {
            ...
            'trade_metrics': {
                'total_trades': metrics.get('total_trades', len(result.trades) if hasattr(result, 'trades') else 0),
            ...
            'execution_info': {
                'events_processed': result.events_processed if hasattr(result, 'events_processed') else 0,
