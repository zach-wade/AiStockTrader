...
class EventDrivenEngine(ErrorHandlingMixin):
    ...
    def __init__(self, config: Dict):
        ...
        self.connection_recovery_manager = ErrorRecoveryManager(RetryConfig(
            max_attempts=5,
            base_delay=2.0,
            max_delay=30.0,
        ...
        with timer() as engine_timer:
            try:
                ...
                record_metric(
                    ...
                    1,
                    tags={"strategies_count": len(self.strategies)}
                )
                ...
                if any(isinstance(s, NewsAnalyticsStrategy) for s in self.strategies):
                    if self.news_client:
                        ...
                        task = asyncio.create_task(
                            self._start_resilient_news_stream()
                        )
    ...
    async def _start_resilient_news_stream(self):
        ...
        try:
            ...
            await self._execute_with_resilience(
                "streaming",
                self.news_client.stream_news,
                callback=resilient_callback
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        record_metric("event_driven_engine.strategies_active", 
                     len(self.strategies), metric_type="gauge")
        record_metric("event_driven_engine.tasks_active", 
                     active_tasks, metric_type="gauge")
        record_metric("event_driven_engine.total_events_processed", 
                     self.events_processed, metric_type="gauge")
...
app = create_event_driven_app(
    "event_engine",
    "AI Trader Event-Driven Engine"
)
...
def run():
    ...
    async def _run_engine(handler: StandardCLIHandler):
        try:
            ...
            logger.error(f"Engine failed to run: {str(e)}")
            raise
