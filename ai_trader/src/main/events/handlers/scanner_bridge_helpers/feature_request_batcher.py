...
class FeatureRequestBatcher(ErrorHandlingMixin):
    ...
    def add_to_pending(self, symbol: str, features: List[str], 
                       priority: int, scanner_source: str, correlation_id: Optional[str] = None) -> Optional[FeatureRequestBatch]:
        """
        ...
        if symbol not in request_batch.symbols and len(request_batch.symbols) >= self._batch_size:
            ...
            new_batch_key = f"p{priority}_{features_key}_overflow_{datetime.now(timezone.utc).isoformat()}" # Unique key for overflow
            ...
            logger.debug(f"Batch '{batch_key}' completed by symbol limit. Returning. New batch created: '{new_batch_key}'.")
        ...
        logger.debug(f"Added symbol '{symbol}' to batch '{batch_key}'. Current symbols: {len(request_batch.symbols)}/{self._batch_size}.")
