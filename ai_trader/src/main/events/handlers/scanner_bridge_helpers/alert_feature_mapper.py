...
class AlertFeatureMapper(ErrorHandlingMixin):
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            ...
            config_path = os.path.join(base_dir, 'src', 'config', 'events', 'alert_feature_mappings.yaml')
    ...
    def get_features_for_alert_type(self, alert_type: str) -> List[str]:
        ...
        record_metric("alert_feature_mapper.lookup", 1, tags={
        ...
        if 'all_features' in features:
            logger.debug(f"Alert type '{alert_type}' requested 'all_features'. Expanding to full list.")
            record_metric("alert_feature_mapper.all_features_requested", 1, tags={
                "alert_type": alert_type
            })
    ...
    def get_supported_alert_types(self) -> List[str]:
        ...
        return [alert_type for alert_type in self._alert_feature_map.keys() if alert_type != 'default']
