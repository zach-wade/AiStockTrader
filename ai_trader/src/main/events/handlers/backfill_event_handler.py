...
class BackfillEventHandler(ErrorHandlingMixin):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.event_bus.subscribe(
                'BackfillRequested',
                self.handle_backfill_requested
            )
    ...
    async def handle_backfill_requested(self, event) -> None:
        ...
        try:
            ...
            if self._is_duplicate(task):
                ...
                self.logger.debug(
                    f"Skipping duplicate backfill for {task.symbol} "
                    f"(layer={task.layer})"
                )
                return
    ...
    async def _execute_backfill_with_control(self, task: BackfillTask) -> None:
        ...
        async with self._semaphore:
            ...
            if self._circuit_breaker.state == 'open':
                self.logger.warning(
                    f"Circuit breaker open, skipping backfill for {task.symbol}"
                )
                return
    ...
    async def _execute_backfill_with_retry(self, task: BackfillTask) -> None:
        ...
        if task.attempt > 1:
            ...
            self.logger.info(
                f"Retrying backfill for {task.symbol} "
    ...
    async def _execute_backfill(self, task: BackfillTask) -> None:
        ...
        self.logger.info(
            f"Executing backfill: {task.backfill_id} "
            f"for {task.symbol} (layer={task.layer})"
        )
    ...
    def _cleanup_completed_tasks(self) -> None:
        ...
        keys_to_remove = [
            key for key, timestamp in self._completed_tasks.items()
