...
class EventBus(ErrorHandlingMixin, IEventBus):
    ...
    def subscribe(
        ...
        try:
            ...
            self._stats_tracker.update_subscriber_count(
                event_type_enum, len(handlers)
            )
    ...
    def unsubscribe(self, event_type: str, handler: Callable[[Any], asyncio.Future]) -> None:
        ...
        try:
            ...
            if handler_to_remove:
                ...
                event_type_str = event_type_enum.value if hasattr(event_type_enum, 'value') else str(event_type_enum)
                logger.debug(
                    f"Unsubscribed handler {handler.__name__} from {event_type_str}"
                )
        except Exception as e:
    ...
    async def publish(self, event: Event) -> None:
        ...
        if hasattr(self, '_enable_validation') and self._enable_validation:
            ...
            event_type_str = event.event_type if isinstance(event.event_type, str) else str(event.event_type)
        ...
        try:
            ...
            record_metric("event_bus.queue_full", 1, tags={
                "event_type": event_type_str
            })
            ...
            record_metric("event_bus.publish_error", 1, tags={
                "event_type": event_type_str,
    ...
    async def _dispatch_event(self, event: Event):
        ...
        if not handlers:
            ...
            record_metric("event_bus.no_handlers", 1, tags={
                "event_type": event_type_str
            })
            return
        
        # Record number of handlers
        record_metric("event_bus.handler_count", len(handlers), 
        ...
        if success_count > 0:
            ...
            record_metric("event_bus.handlers_succeeded", success_count, tags={
                "event_type": event_type_str
            })
        
        if failure_count > 0:
            self._stats_tracker.increment_failed()
            record_metric("event_bus.handlers_failed", failure_count, tags={
                "event_type": event_type_str
            })
            logger.warning(
                f"Event {event_type_str} had {failure_count} handler failures"
            )
        ...
        try:
            ...
            if self._dlq_manager:
                await self._dlq_manager.add_event(
                    event,
                    f"handler_error: {handler.__name__} - {str(e)}"
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        history_size = (
            self._history_manager.get_event_count()
            if self._history_manager else 0
        )
        dlq_size = (
            self._dlq_manager.get_queue_size()
            if self._dlq_manager else 0
        )
        ...
        record_metric("event_bus.circuit_breaker_state", cb_state_value, 
                     metric_type="gauge")
    ...
    async def replay_events(
        ...
        if not self._history_manager:
            ...
            return {
                'status': 'error',
                'message': 'Event history not enabled',
                'events_replayed': 0
            }
        ...
        if dry_run:
            ...
            for event in events:
                event_type_str = event.event_type.value if hasattr(event.event_type, 'value') else str(event.event_type)
                stats['event_types'][event_type_str] = stats['event_types'].get(event_type_str, 0) + 1
        ...
        stats['replay_end'] = datetime.now(timezone.utc)
        stats['replay_duration_seconds'] = (stats['replay_end'] - stats['replay_start']).total_seconds()
    ...
    async def process_dlq(self, max_retries: int = 3) -> Dict[str, int]:
        ...
        results = await self._dlq_manager.process_events(
            self.publish,
