...
from main.events.core.event_bus_factory import EventBusFactory, EventBusConfig
...
class EventBusRegistry(IEventBusProvider):
    ...
    def get_event_bus(self, name: Optional[str] = None) -> IEventBus:
        """
    ...
    def has_event_bus(self, name: Optional[str] = None) -> bool:
        """
    ...
    def unregister_event_bus(self, name: Optional[str] = None) -> None:
        """
        ...
        with self._lock:
            if name not in self._instances:
                raise EventBusNotFoundError(
                    f"Event bus '{name or 'default'}' not found"
                )
    ...
    def get_all_names(self) -> Set[Optional[str]]:
