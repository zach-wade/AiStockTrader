...
class EventHistoryManager:
    ...
    def add_event(self, event: Event):
        ...
        logger.debug(f"Added event to history: {event_type_str}. Current history size: {len(self._history)}")
        
        # Record metrics
        record_metric("event_history.event_added", 1, tags={
    ...
    def get_history(self, event_type: Optional[EventType] = None,
        ...
        if event_type:
            ...
            record_metric("event_history.query_filtered", 1, tags={
                "event_type": event_type_str,
                "results": len(history_list)
            })
        else:
            record_metric("event_history.query_all", 1, tags={
                "results": len(history_list)
            })
    ...
    def get_events_for_replay(self, event_type: Optional[EventType] = None, 
        ...
        record_metric("event_history.replay_query", 1, tags={
            "event_type": event_type_str,
