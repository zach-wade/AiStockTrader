    ...
    try:
        ...
        if hasattr(config, 'keys'):
            ...
            if 'data' in config:
                ...
                print(f"📊 Data config keys: {list(data_config.keys()) if hasattr(data_config, 'keys') else 'Not a dict'}")
                
                if hasattr(data_config, 'keys') and 'backfill' in data_config:
                    backfill_config = data_config['backfill']
                    print(f"🔄 Backfill config keys: {list(backfill_config.keys()) if hasattr(backfill_config, 'keys') else 'Not a dict'}")
...
def check_config_file():
    ...
    try:
        from main.config.config_manager import ModularConfigManager
