...
class PerformanceBenchmark:
    ...
    def benchmark_instantiation(self, iterations: int = 1000):
        ...
        print(
            f"Nested Pydantic: {nested_time*1000:.2f}ms total, {nested_time/iterations*1000:.4f}ms per instance"
        )
        print(
            f"Flat Config:     {flat_time*1000:.2f}ms total, {flat_time/iterations*1000:.4f}ms per instance"
        )
    ...
    def benchmark_deep_access(self, iterations: int = 100000):
        ...
        print(
            f"Nested Pydantic: {nested_time*1000:.2f}ms total, {nested_time/iterations*1000000:.2f}ns per access"
        )
        print(
            f"Flat Config:     {flat_time*1000:.2f}ms total, {flat_time/iterations*1000000:.2f}ns per access"
        )
        print(f"Speedup:         {nested_time/flat_time:.2f}x faster")
    ...
    def benchmark_memory_usage(self, instances: int = 100):
        ...
        print(
            f"Nested Pydantic: {nested_memory:.2f}MB for {instances} instances ({nested_memory/instances*1000:.2f}KB per instance)"
        )
        print(
            f"Flat Config:     {flat_memory:.2f}MB for {instances} instances ({flat_memory/instances*1000:.2f}KB per instance)"
        )
    ...
    def benchmark_serialization(self, iterations: int = 1000):
        ...
        print(
            f"Nested Pydantic: {nested_time*1000:.2f}ms total, {nested_time/iterations*1000:.4f}ms per serialization"
        )
        print(
            f"Flat Config:     {flat_time*1000:.2f}ms total, {flat_time/iterations*1000:.4f}ms per serialization"
        )
        print(f"Speedup:         {nested_time/flat_time:.2f}x faster")
    ...
    def benchmark_concurrent_access(self, threads: int = 10, iterations: int = 10000):
        ...
        def flat_worker():
            for _ in range(iterations):
                value = flat_config.get(
                    "data_pipeline.validation.quality_thresholds.min_quality_score"
                )
