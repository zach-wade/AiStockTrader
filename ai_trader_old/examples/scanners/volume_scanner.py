...
class VolumeScanner(BaseScanner):
    ...
    def __init__(self, config, volume_multiplier: float = 2.0, lookback_days: int = 20):
    ...
    async def scan(self, symbols: list[str]) -> dict[str, Any]:
        ...
        logger.info(
            "Starting volume scan", symbol_count=len(symbols), multiplier=self.volume_multiplier
        )
    ...
    async def _analyze_volume(self, symbol: str) -> dict[str, Any]:
        ...
        # Volume patterns
        accumulation_score = self._calculate_accumulation_score(volumes, closes)
        distribution_score = self._calculate_distribution_score(volumes, closes)

        # Volume dryup detection
        days_below_average = sum(1 for v in volumes[-5:] if v < avg_volume * 0.5)
    ...
    def _calculate_accumulation_score(self, volumes: list[float], closes: list[float]) -> float:
    ...
    def _calculate_distribution_score(self, volumes: list[float], closes: list[float]) -> float:
    ...
    def _identify_pattern(self, volumes: list[float], closes: list[float]) -> str:
...
async def main():
    ...
    # Create scanner
    scanner = VolumeScanner(config, volume_multiplier=2.5, lookback_days=20)
    ...
    if results["volume_spikes"]:
        ...
        for spike in results["volume_spikes"]:
            print(
                f"  {spike['symbol']}: {spike['volume_ratio']:.1f}x average "
