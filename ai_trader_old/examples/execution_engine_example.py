...
from main.trading_engine.core.execution_engine import ExecutionMode, create_execution_engine
...
async def basic_execution_example():
    ...
    config = {
        ...
        "risk_management": {"max_drawdown": 0.1, "max_daily_loss": 0.05},
    }
    ...
    try:
        ...
        engine = await create_execution_engine(
            config=config, trading_mode=TradingMode.PAPER, execution_mode=ExecutionMode.SEMI_AUTO
        )
...
async def advanced_execution_example():
    ...
    config = {
        "brokers": {
            "alpaca": {"enabled": True, "api_key": "your_api_key", "api_secret": "your_api_secret"}
        },
        "risk_management": {
            "circuit_breaker": {
                "enabled": True,
            ...
            "drawdown_control": {"enabled": True, "max_drawdown": 0.1, "reduction_factor": 0.5},
        },
    }
    ...
    try:
        ...
        engine = await create_execution_engine(
            config=config, trading_mode=TradingMode.PAPER, execution_mode=ExecutionMode.FULL_AUTO
        )
