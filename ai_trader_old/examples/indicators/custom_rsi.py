...
class CustomRSI(BaseIndicator):
    ...
    async def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Detect divergences
        result["bullish_divergence"] = self._detect_divergence(data["close"], rsi, "bullish")
        result["bearish_divergence"] = self._detect_divergence(data["close"], rsi, "bearish")
    ...
    def _detect_divergence(
        self, price: pd.Series, rsi: pd.Series, divergence_type: str, lookback: int = 20
    ) -> pd.Series:
...
async def main():
    ...
    data = pd.DataFrame(
        {"close": prices, "volume": np.secure_randint(1000000, 5000000, 100)}, index=dates
    )
