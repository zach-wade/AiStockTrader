...
# Local imports
from main.data_pipeline.orchestrator import DataFlowConfig, PipelineMode
...
async def targeted_data_ingestion_example():
    ...
    try:
        ...
        news_result = await pipeline_manager.run_ingestion(
            data_type="news", symbols=symbols, start_date=start_date, end_date=end_date
        )
...
async def real_time_streaming_example():
    ...
    flow_config = DataFlowConfig(
        mode=PipelineMode.REAL_TIME, batch_size=10, interval_seconds=1.0, max_workers=5
    )
...
async def data_quality_monitoring_example():
    ...
    try:
        ...
        logger.info(
            f"Health status: {'Healthy' if health_status['healthy'] else 'Issues detected'}"
        )
...
async def selective_pipeline_stages_example():
    ...
    class MockOrchestrator:
        def __init__(self):
            self.config = {
                "data_pipeline": {"enabled_stages": ["ingestion", "processing", "historical"]}
            }
...
async def integration_with_trading_system_example():
    ...
    logger.info(
        f"Hot storage window: {integrated_config['data_pipeline']['storage']['hot_storage_days']} days"
    )
