...
FEATURE_REGISTRY_PATH = (
    PROJECT_ROOT / "data_pipeline/processing/features/metadata/feature_sets.json"
)
...
class FeatureDiscoveryEngine:
    ...
    def run(self):
        ...
        all_registered_features = {
            feature for f_set in registered_sets.values() for feature in f_set["features"]
        }
        logger.info(
            f"Found {len(all_registered_features)} features registered in feature_sets.json."
        )
    ...
    def _generate_report(self, new_features: set[str], features_by_file: dict[str, set[str]]):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"⚠️ **Action Required:** Found **{len(new_features)}** new, unregistered features in the codebase.\n\n"
            )
            f.write(
                "Please review these features and add them to `data_pipeline/processing/features/metadata/feature_sets.json` if they are ready for production use.\n\n"
            )
        ...
        logger.warning(
            f"ACTION REQUIRED: Found {len(new_features)} new features. Report saved to: {report_path}"
        )
