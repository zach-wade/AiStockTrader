...
class DataLakeLoader:
    ...
    async def load_market_data(
        ...
        market_loader = MarketDataSplitBulkLoader(db_adapter=self.db_adapter, config=bulk_config)
        ...
        for symbol in symbols:
            ...
            try:
                ...
                logger.info(
                    f"  Found data for intervals: {sorted(records_by_interval.keys())} ({sum(len(v) for v in records_by_interval.values())} total records)"
                )
                ...
                for interval, interval_records in records_by_interval.items():
                    if interval == "unknown":
                        logger.warning(
                            f"  Skipping {len(interval_records)} records with unknown interval"
                        )
                        continue

                    # Process records for this interval
                    logger.info(
                        f"  Processing {len(interval_records)} {interval} records for {symbol}"
                    )
                    total_records = 0
                        ...
                        if record.data and "data" in record.data:
                            ...
                            if isinstance(market_data, list) and market_data:
                                ...
                                load_result = await market_loader.load(
                                    data=df, symbol=symbol, interval=interval, source="polygon"
                                )
                                ...
                                if load_result.success:
                                    if not load_result.skipped:
                                        ...
                                        logger.debug(
                                            f"  Skipped loading {interval} data: {load_result.skip_reason}"
                                        )
                                else:
                                    logger.error(
                                        f"  Failed to load {interval} data: {'; '.join(load_result.errors)}"
                                    )
                                    self.stats["errors"] += 1
                    ...
                    if total_records > 0:
                        ...
                        logger.info(
                            f"  Processed {len(interval_records)} {interval} files but no new records (likely duplicates)"
                        )
    ...
    async def load_news_data(
        ...
        news_loader = NewsBulkLoader(db_adapter=self.db_adapter, config=bulk_config)
    ...
    async def load_financials_data(
        ...
        financials_loader = FundamentalsBulkLoader(db_adapter=self.db_adapter, config=bulk_config)
    ...
    async def get_symbols_by_layer(self, layer: str) -> list[str]:
        ...
        if layer == "layer0":
            ...
            query = (
                "SELECT symbol FROM companies WHERE layer1_qualified = TRUE AND is_active = TRUE"
            )
        elif layer == "layer2":
            query = (
                "SELECT symbol FROM companies WHERE layer2_qualified = TRUE AND is_active = TRUE"
            )
        elif layer == "layer3":
            query = (
                "SELECT symbol FROM companies WHERE layer3_qualified = TRUE AND is_active = TRUE"
            )
        elif layer == "all":
...
async def main():
    ...
    # Date range
    parser.add_argument(
        "--days", type=int, help="Number of days to load (from today backwards)", default=30
    )
    parser.add_argument(
        "--start",
    ...
    try:
        ...
        if args.dry_run:
            ...
            if "market" in args.data_types:
                logger.info("  Market data: All available intervals will be loaded")
            return
