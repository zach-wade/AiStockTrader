...
# Local imports
from src.main.monitoring.alerts.unified_alerts import AlertCategory, AlertLevel, UnifiedAlertSystem
...
async def test_webhook_alerts(webhook_url: str):
    ...
    await alert_system.alert_trade_executed(
        symbol="MSFT", side="SELL", quantity=50, price=350.25, order_id="TEST-001"
    )
...
def main():
    ...
    parser.add_argument(
        "--webhook-url", required=True, help="Webhook URL (e.g., Slack, Discord, Teams)"
    )
