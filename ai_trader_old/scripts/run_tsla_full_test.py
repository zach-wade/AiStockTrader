...
def main():
    ...
    step = {"name": "Validate Database Empty", "command": "python scripts/validate_db_empty.py"}
    success = run_command(f"cd {project_dir} && {step['command']}", step["name"])
    ...
    step = {"name": "Validate TSLA Backfill", "command": "python scripts/validate_tsla_backfill.py"}
    success = run_command(f"cd {project_dir} && {step['command']}", step["name"])
    step["success"] = success
    results["steps"].append(step)
