...
async def validate_tsla_data():
    ...
    try:
        ...
        for row in market_results:
            ...
            print(
                f"  Date Range: {row['earliest'].date()} to {row['latest'].date()} ({days_span} days)"
            )
        ...
        if news_result and news_result["total_articles"] > 0:
            results["validations"]["news_data"] = {
                ...
                "avg_sentiment": (
                    float(news_result["avg_sentiment"]) if news_result["avg_sentiment"] else None
                ),
            }
            ...
            print(
                f"Average Sentiment: {news_result['avg_sentiment']:.3f}"
                if news_result["avg_sentiment"]
                else "Average Sentiment: N/A"
            )
        else:
        ...
        market_files = list(
            data_lake_path.glob("market_data/symbol=TSLA/interval=*/date=*/*.parquet")
        )
