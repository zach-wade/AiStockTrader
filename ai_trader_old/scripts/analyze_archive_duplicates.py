        ...
        for interval, interval_records in sorted(records_by_interval.items()):
            ...
            if all_timestamps:
                ...
                for i, info in enumerate(file_info[:5]):  # Show first 5
                    ...
                    print(
                        f"      Date range: {info['min_date'].date()} to {info['max_date'].date()} ({info['date_range_days']} days)"
                    )
                ...
                for i in range(len(file_info)):
                    for j in range(i + 1, len(file_info)):
                        ...
                        if (
                            file1["min_date"] <= file2["max_date"]
                            and file1["max_date"] >= file2["min_date"]
                        ):
                            overlaps += 1
                ...
                if interval == "1day":
                    print("  Actual records in DB: 1,256")
