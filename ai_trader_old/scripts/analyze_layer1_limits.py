...
async def analyze_layer1_issue():
    ...
    try:
        ...
        print("Target universe size: 2000")
        print("Actual qualified: ~1450")
        ...
        print(
            f'1. Total symbols with ANY data in last {lookback_days} days: {result1["total_symbols"]}'
        )
        ...
        result2 = await db_adapter.fetch_one(
            query2, {"start": start_date, "end": end_date, "min_points": min_data_points}
        )
        print(
            f'2. Symbols with >= {min_data_points} data points: {result2["symbols_with_enough_data"]}'
        )
        ...
        print(
            f'3. Symbols meeting ${min_dollar_volume:,} volume requirement: {result3["symbols_meeting_volume"]}'
        )
        ...
        print(
            f'4. Symbols in price range ${min_price}-${max_price}: {result3b["symbols_in_price_range"]}'
        )
        ...
        print("- The scanner cannot select 2000 symbols because only ~1450-1500 qualify")
        print(
            "- AA is ranked #1914 by liquidity, but since only ~1450 qualify, it doesn't make the cut"
        )
