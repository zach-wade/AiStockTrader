    ...
    for sector, symbols in universe.items():
        ...
        for symbol1, symbol2 in combinations(symbols, 2):
            ...
            if p_value < COINT_PVALUE_THRESHOLD:
                ...
                half_life = (
                    -np.log(2) / hl_model.params[0] if hl_model.params[0] < 0 else float("inf")
                )
                ...
                logger.info(
                    f"✓ Found valid pair: {symbol1}-{symbol2} (p-value: {p_value:.4f}, half-life: {half_life:.2f} days)"
                )
...
def main():
    ...
    class MockDataProvider:
        def get_bulk_daily_data(self, symbols, days):
            ...
            return {
                s: pd.DataFrame(np.random.randn(days, 1) + (i * 10), index=dates, columns=["close"])
