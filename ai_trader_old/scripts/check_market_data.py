...
async def check_market_data(symbol: str = "AA"):
    ...
    try:
        ...
        if rows:
            ...
            print("  ... (showing first 10 of potentially more)\n")
        else:
...
async def check_specific_dates(symbol: str = "AA", start_date: str = None, end_date: str = None):
    ...
    try:
        ...
        if start_date:
            ...
            params["start_date"] = datetime.strptime(start_date, "%Y-%m-%d").replace(tzinfo=UTC)
        if end_date:
            date_filter += f" AND timestamp <= ${len(params) + 1}"
            params["end_date"] = datetime.strptime(end_date, "%Y-%m-%d").replace(tzinfo=UTC)
