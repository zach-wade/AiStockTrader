...
async def list_available_models(model_registry: ModelRegistry):
    ...
    for model in models:
        ...
        logger.info(
            f"{model['model_id']:<20} {model['version']:<10} {model['status']:<15} {created:<20} {metrics_str}"
        )
...
async def main():
    ...
    model_registry = ModelRegistry(
        models_dir=models_dir, prediction_engine=prediction_engine, config=config
    )
    ...
    if args.list:
        ...
        if aapl_model_path.exists():
            ...
            aapl_models = [
                m for m in model_registry.list_models() if m["model_id"] == "aapl_xgboost"
            ]

            if not aapl_models:
                logger.info("Registering AAPL model...")
                model_version = await deploy_model_from_file(
                    model_registry, aapl_model_path, "aapl_xgboost"
                )
                if model_version:
                    await deploy_model_to_production(
                        model_registry, "aapl_xgboost", model_version.version
                    )
            else:
                ...
                if not production_models:
                    ...
                    await deploy_model_to_production(
                        model_registry, "aapl_xgboost", latest["version"]
                    )
                else:
