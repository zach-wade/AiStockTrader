    ...
    try:
        ...
        if aa_result:
            ...
            debug_result = await db_adapter.fetch_one(
                debug_query, {"start": start_date, "end": end_date}
            )
            print("\nDebug info:")
        ...
        for row in all_liquidity:
            if (
                row["avg_dollar_volume"] >= min_dollar_volume
                and min_price <= row["avg_price"] <= max_price
            ):
                filtered.append(row)
        ...
        if aa_position:
            ...
            print(
                f"  Makes the cut for top {target_size}: {'YES' if aa_position <= target_size else 'NO'}"
            )
        else:
