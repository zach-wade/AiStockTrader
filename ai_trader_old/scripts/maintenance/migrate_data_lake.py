...
def main():
    ...
    migrator = DataLakeMigrator(
        data_lake_path=str(data_lake_path), dry_run=True, max_workers=4, backup_enabled=True
    )
    ...
    try:
        ...
        for data_type, info in data_types.items():
            if info["directories"]:
                print(
                    f"   • {data_type}: {len(info['directories'])} directories ({info['migration_complexity']} complexity)"
                )
        ...
        if stats.errors:
            ...
            print("✅ No issues detected - migration should be safe")

        print("\nTo execute the actual migration:")
        print("   python migrate_data_lake.py --execute")
        print("\nTo create backup before migration:")
        print("   python migrate_data_lake.py --execute --backup")
...
def execute_migration():
    ...
    migrator = DataLakeMigrator(
        data_lake_path=str(data_lake_path), dry_run=False, max_workers=4, backup_enabled=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--execute", action="store_true", help="Execute actual migration (not dry-run)"
    )
