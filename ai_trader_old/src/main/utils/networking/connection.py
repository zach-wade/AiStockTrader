...
class WebSocketConnection:
    ...
    def __init__(
        self, config: ConnectionConfig, buffer: MessageBuffer, name: str = "ws_connection"
    ):
        self.config = config
        self.buffer = buffer
        self.name = name
            ...
            while self.websocket and not self.websocket.closed:
                try:
                    ...
                    self.latency_metrics.add_measurement(
                        rtt=network_time, network_time=network_time
                    )
    ...
    def _determine_message_priority(self, message: str | bytes) -> MessagePriority:
                ...
                if self.websocket and not self.websocket.closed:
                    try:
                        ...
                        await asyncio.wait_for(
                            self.websocket.ping(), timeout=self.config.ping_timeout
                        )
    ...
    async def send_message(
        self, data: str | dict[str, Any], priority: MessagePriority = MessagePriority.NORMAL
    ) -> bool:
        """
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            and self.latency_metrics.avg_rtt * 1000 < self.latency_metrics.rtt_critical_ms
        )
