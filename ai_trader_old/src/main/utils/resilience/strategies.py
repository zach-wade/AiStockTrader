...
class ResilienceStrategies:
    ...
    def __init__(self, config: dict[str, Any] | ResilienceConfig | Any | None = None):
        """
        ...
        logger.info(
            f"ResilienceStrategies initialized with max_retries={self.max_retries}, "
        ...
        try:
            ...
            if hasattr(config, "resilience"):
                ...
                if hasattr(resilience_section, "_content") or isinstance(resilience_section, dict):
                    ...
                    resilience_dict = (
                        dict(resilience_section) if hasattr(resilience_section, "items") else {}
                    )
                    ...
                    if "rate_limiting" in resilience_dict:
                        ...
                        if isinstance(rate_config, dict):
                            ...
                            if "calls_per_period" in rate_config:
                                resilience_config["rate_limit_calls"] = rate_config[
                                    "calls_per_period"
                                ]
                            if "period_seconds" in rate_config:
                                resilience_config["rate_limit_period"] = rate_config[
                                    "period_seconds"
                                ]
    ...
    def _check_rate_limit(self) -> bool:
        ...
        self._call_times = [
            t for t in self._call_times if current_time - t < self.rate_limit_period
        ]
    ...
    async def execute_with_resilience(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        try:
            ...
            result = await self.error_recovery.execute_with_retry(
                wrapped, retry_config=retry_config
            )
    ...
    def with_resilience(self, func: Callable[..., T]) -> Callable[..., T]:
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            return loop.run_until_complete(self.execute_with_resilience(func, *args, **kwargs))
...
class ResilienceStrategiesFactory:
    ...
    @staticmethod
    def create_from_config(config: ResilienceConfig | dict[str, Any] | Any) -> ResilienceStrategies:
    ...
    def create_from_profile(profile_name: str, config: Any = None) -> ResilienceStrategies:
        ...
        try:
            ...
            logger.warning(
                f"Could not find profile '{profile_name}' in configuration, using defaults"
            )
...
def create_resilience_strategies(
    profile: str | None = None, config: ResilienceConfig | dict[str, Any] | Any | None = None
