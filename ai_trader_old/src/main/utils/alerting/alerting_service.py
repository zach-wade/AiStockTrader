            ...
            try:
                if channel == AlertChannel.SLACK and self.slack_webhook_url:
                    results["slack"] = await self._send_slack_alert(
                        title, message, priority, context
                    )
                    ...
                    results["pagerduty"] = await self._send_pagerduty_alert(
                        title, message, priority, context
                    )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.slack_webhook_url, json=slack_message, timeout=10
                ) as response:
        ...
        try:
            ...
            event = {
                ...
                "payload": {
                    ...
                    "custom_details": {"message": message, **(context or {})},
                },
            }
...
def get_alerting_service(config: dict[str, Any] | None = None) -> AlertingService:
    """
