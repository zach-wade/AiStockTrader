...
class MemoryTracker:
            ...
            if growth_rate > self.thresholds.growth_critical:
                alerts.append(
                    MemoryAlert(
                        ...
                        growth_rate,
                        self.thresholds.growth_critical,
                    )
                )
                ...
                alerts.append(
                    MemoryAlert(
                        ...
                        growth_rate,
                        self.thresholds.growth_warning,
                    )
                )
    ...
    def record_function_memory(self, function_name: str, memory_usage: float):
        ...
        with self._lock:
            self.function_memory[function_name].append(
                {"timestamp": datetime.now(), "memory_mb": memory_usage}
            )
    ...
    def force_garbage_collection(self) -> dict[str, int]:
        ...
        logger.info(
            f"Garbage collection completed: {collected} objects collected in {collection_time:.3f}s"
        )
...
class MemoryMonitor:
        ...
        while self._monitoring:
            try:
                ...
                if (
                    ...
                    and snapshot.rss_mb > self.auto_gc_threshold
                    and time.time() - self._last_gc_time > self.gc_cooldown
                ):

                    logger.info(
                        f"Auto-triggering garbage collection (memory: {snapshot.rss_mb:.1f}MB)"
                    )
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    if abs(memory_delta) > 10:  # 10MB threshold
                        logger.info(
                            f"Function {func.__name__} memory impact: {memory_delta:+.1f}MB "
    ...
    def memory_context(self, operation_name: str, gc_before: bool = False, gc_after: bool = False):
        ...
        try:
            ...
            if gc_after:
                ...
                logger.info(
                    f"Post-operation GC for {operation_name}: {gc_stats['total_objects_collected']} objects"
                )
            ...
            logger.info(
                f"Memory context {operation_name}: {memory_delta:+.1f}MB "
                f"(execution time: {execution_time:.3f}s)"
            )
    ...
    def optimize_dataframe_memory(self, df: pd.DataFrame, deep: bool = True) -> pd.DataFrame:
        ...
        if memory_saved > 0.1:  # Log if significant savings
            logger.info(
                f"DataFrame memory optimized: {memory_saved:.1f}MB saved "
...
def optimize_memory_usage():
    ...
    logger.info(
        f"Memory optimization completed: {gc_stats['total_objects_collected']} objects collected, "
        f"current usage: {snapshot.rss_mb:.1f}MB"
    )

    return {"gc_stats": gc_stats, "current_memory": snapshot.rss_mb, "timestamp": datetime.now()}
