...
from .types import FunctionMetrics, MetricType
...
class FunctionTracker:
    ...
    def _record_function_metrics(self, func_name: str, duration: float, success: bool):
        ...
        logger.debug(
            f"Recorded function metrics for {func_name}: {duration:.4f}s (success: {success})"
        )
    ...
    def get_function_summary(self) -> dict[str, Any]:
        ...
        for func_name, metrics in self.function_metrics.items():
            summary[func_name] = {
                ...
                "min_duration": round(
                    metrics.min_duration if metrics.min_duration != float("inf") else 0, 4
                ),
    ...
    def get_function_metrics(self, func_name: str) -> FunctionMetrics | None:
    ...
    def get_top_functions_by_calls(self, limit: int = 10) -> dict[str, Any]:
        ...
        sorted_functions = sorted(
            self.function_metrics.items(), key=lambda x: x[1].total_calls, reverse=True
        )
    ...
    def get_top_functions_by_duration(self, limit: int = 10) -> dict[str, Any]:
        ...
        sorted_functions = sorted(
            self.function_metrics.items(), key=lambda x: x[1].total_duration, reverse=True
        )
    ...
    def get_slowest_functions(self, limit: int = 10) -> dict[str, Any]:
        ...
        sorted_functions = sorted(
            self.function_metrics.items(), key=lambda x: x[1].avg_duration, reverse=True
        )
