...
class DashboardMetricsAdapter:
    ...
    def __init__(self, monitor: MigrationMonitor | None = None):
        """
    ...
    async def get_metric_value(
        ...
        if hasattr(self.monitor, "get_metric_value"):
            return await self.monitor.get_metric_value(name, aggregation, period_minutes, tags)
        else:
            # Fallback for basic monitor
    ...
    async def get_metric_series(
        ...
        if hasattr(self.monitor, "get_metric_series"):
            ...
            history = self.monitor.get_metric_history(name, minutes=period_minutes)
            return [
                {"timestamp": m.timestamp.isoformat(), "value": m.value, "tags": {}}
                for m in history
            ]
        ...
        if hasattr(self.monitor, "enhanced_monitor") and self.monitor.enhanced_monitor:
            ...
            return {
                ...
                "metrics_processed": sum(
                    len(metrics) for metrics in self.monitor.enhanced_monitor._metrics.values()
                ),
...
class DashboardHealthReporter:
    ...
    def _generate_recommendations(
        self, health_score: dict[str, Any], alerts: list[Alert]
    ) -> list[str]:
...
class DashboardPerformanceTracker:
    ...
    async def get_performance_metrics(self, timeframe: str = "1h") -> dict[str, Any]:
        ...
        timeframe_map = {"5m": 5, "15m": 15, "1h": 60, "4h": 240, "1d": 1440, "1w": 10080}
        ...
        # API performance
        api_latency = await self.adapter.get_metric_value("api.request_duration", "avg", minutes)
        if api_latency:
            metrics["api_latency_ms"] = api_latency

        api_requests = await self.adapter.get_metric_value("api.request_count", "sum", minutes)
        if api_requests:
            metrics["api_requests_total"] = api_requests

        api_errors = await self.adapter.get_metric_value("api.error_count", "sum", minutes)
        if api_errors:
        ...
        # System performance
        cpu_avg = await self.adapter.get_metric_value("system.cpu_usage", "avg", minutes)
        if cpu_avg:
            metrics["cpu_usage_avg"] = cpu_avg

        memory_avg = await self.adapter.get_metric_value("system.memory_usage", "avg", minutes)
        if memory_avg:
...
def create_dashboard_adapter(db_pool: DatabasePool | None = None) -> DashboardMetricsAdapter:
    """
