...
from .metrics import MetricRecord, MetricType, PipelineStatus, ValidationMetric
from .types import Alert, AlertLevel, FunctionMetrics, PerformanceMetric, SystemResources
...
try:
    ...
    class MetricsCollector:
        ...
        def __init__(self, storage_backend=None, retention_hours=24, aggregation_intervals=None):
            """
        ...
        def record_metric(self, name, value, tags=None, timestamp=None):
            ...
            with self._lock:
                ...
                if self.storage_backend:
                    try:
                        self.storage_backend.store_metric(name, value, tags, timestamp)
                    except Exception:
        ...
        def collect_metrics(self, time_range=None, metric_names=None, tags=None):
            ...
            """
            with self._lock:
                collected = {"metrics": {}, "gauges": {}, "counters": {}, "histograms": {}}
                ...
                for key, values in self._metrics.items():
                    ...
                    if time_range:
                        ...
                        filtered_values = [
                            v for v in values if start_time <= v["timestamp"] <= end_time
                        ]
                    else:
        ...
        def get_metric_stats(self, metric_name, tags=None, hours=1):
            ...
            with self._lock:
                ...
                recent_values = [
                    v["value"] for v in self._metrics[metric_key] if v["timestamp"] >= cutoff_time
                ]
        ...
        def get_metric_history(self, metric_name, tags=None, hours=1):
            ...
            with self._lock:
                ...
                cutoff_time = datetime.utcnow() - timedelta(hours=hours)
                return [v for v in self._metrics[metric_key] if v["timestamp"] >= cutoff_time]

        def aggregate_metrics(
            self, metric_name, interval_seconds=60, aggregation="mean", tags=None
        ):
            ...
            with self._lock:
                ...
                for bucket_time, values in sorted(buckets.items()):
                    if values:
                        ...
                        aggregated.append(
                            {"timestamp": bucket_time, "value": agg_value, "count": len(values)}
                        )
