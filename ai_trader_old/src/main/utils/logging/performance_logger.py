...
class PerformanceLogger(ErrorHandlingMixin):
    ...
    def _setup_file_handlers(self) -> dict[str, logging.Handler]:
        ...
        metrics_handler = logging.handlers.RotatingFileHandler(
            self.log_dir / "metrics.log", maxBytes=100 * 1024 * 1024, backupCount=10  # 100MB
        )
        ...
        strategy_handler.setFormatter(logging.Formatter("%(asctime)s - %(message)s"))
        handlers["strategy"] = strategy_handler

        # JSON log for analysis
        json_handler = logging.handlers.RotatingFileHandler(
            self.log_dir / "performance.json", maxBytes=100 * 1024 * 1024, backupCount=5
        )
    ...
    def log_metric(
        ...
        with self._handle_error("logging metric"):
            ...
            self._metric_history[key] = [
                (ts, val) for ts, val in self._metric_history[key] if ts > cutoff
            ]
    ...
    async def log_benchmark_comparison(
        ...
        with self._handle_error("logging benchmark comparison"):
            ...
            comparison = self._calculate_comparison_metrics(strategy_data, benchmark_data)
    ...
    def log_risk_metrics(
        ...
        with self._handle_error("logging risk metrics"):
            ...
            self.log_metric(
                MetricType.DRAWDOWN, max_drawdown, strategy=strategy, metadata={"type": "max"}
            )
            ...
            entry = PerformanceLogEntry(
                ...
                category="risk",
                message=(f"Risk Metrics - " f"VaR95: {var_95:.2%}, " f"MaxDD: {max_drawdown:.2%}"),
    ...
    def log_portfolio_analytics(
        ...
        with self._handle_error("logging portfolio analytics"):
            ...
            self.log_metric(MetricType.EXPOSURE, net_exposure, metadata={"type": "net"})
            self.log_metric(MetricType.TURNOVER, turnover, metadata={"period": "daily"})

    async def generate_performance_report(
        self, strategy: str | None = None, period_days: int = 30
    ) -> dict[str, Any]:
        ...
        with self._handle_error("generating performance report"):
            ...
            if strategy:
                metrics = await self._calculate_strategy_metrics(strategy, start_date, end_date)
            else:
                metrics = await self._calculate_portfolio_metrics(start_date, end_date)
    ...
    async def _calculate_all_metrics(self):
        ...
        try:
            ...
            if portfolio_metrics:
                # Log key metrics
                self.log_metric(MetricType.RETURN, portfolio_metrics.get("daily_return", 0))
                self.log_metric(MetricType.SHARPE, portfolio_metrics.get("sharpe_ratio", 0))

            # Calculate strategy metrics
            strategies = await self._get_active_strategies()
            for strategy in strategies:
                strategy_metrics = await self._calculate_strategy_metrics(
                    strategy, datetime.utcnow() - timedelta(days=1), datetime.utcnow()
                )
    ...
    def _calculate_comparison_metrics(
        self, strategy_data: list[dict[str, float]], benchmark_data: list[dict[str, float]]
    ) -> dict[str, float]:
        ...
        common_dates = sorted(set(strategy_returns.keys()) & set(benchmark_returns.keys()))
        ...
        # Information ratio
        if tracking_error > 0:
            information_ratio = np.mean(strat_ret - bench_ret) * 252 / tracking_error
        else:
            information_ratio = 0
    ...
    def _calculate_statistics(self, metrics: dict[str, float]) -> dict[str, Any]:
    ...
    async def _get_benchmark_returns(
        self, benchmark_name: str, period: str
    ) -> list[dict[str, float]]:
