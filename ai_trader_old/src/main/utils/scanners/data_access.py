...
from main.utils.core import create_task_safely, timer
...
class ScannerDataAccess:
    ...
    async def get_scanner_data_batch(
        self, symbols: list[str], data_types: list[str], lookback_hours: int = 24
        ...
        with timer() as t:
            ...
            query_filter = QueryFilter(
                symbols=symbols,
                start_date=datetime.now(UTC) - timedelta(hours=lookback_hours),
            ...
            routing_decision = self.storage_router.route_query(
                query_filter, query_type, prefer_performance=self.config.prefer_hot_storage
            )

            # Handle both enum and string types for primary_tier
            tier_name = (
                routing_decision.primary_tier.value
                if hasattr(routing_decision.primary_tier, "value")
                else str(routing_decision.primary_tier)
            )
            logger.info(
            ...
            for data_type in data_types:
                task = create_task_safely(self._fetch_data_type(symbols, data_type, query_filter))
            ...
            for data_type, task in tasks:
                try:
                    data = await asyncio.wait_for(task, timeout=self.config.timeout_seconds)
    ...
    async def get_cached_or_fetch(self, cache_key: str, fetch_func, *args, **kwargs) -> Any:
        ...
        if self.cache and self.config.enable_caching:
            cached_data = await self.cache.get(cache_key, cache_type=CacheType.SCANNER_RESULTS)
    ...
    async def get_market_snapshot(self, symbols: list[str]) -> dict[str, dict[str, Any]]:
        """
    ...
    async def get_technical_data(
        self, symbols: list[str], indicators: list[str], lookback_days: int = 30
        ...
        market_data = await self.repository.get_market_data(
            symbols, query_filter, columns=required_columns
        )

        return market_data

    async def _fetch_data_type(
        self, symbols: list[str], data_type: str, query_filter: QueryFilter
    ) -> dict[str, Any]:
