...
class ScannerMetricsCollector(IScannerMetrics):
    ...
    def __init__(
        ...
        self.time_series_metrics: dict[str, deque] = defaultdict(lambda: deque(maxlen=max_metrics))

        # Recent scan durations for percentile calculations
        self.recent_durations: dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))
    ...
    def record_scan_duration(
        ...
        perf.last_scan_time = datetime.now(UTC)
        ...
        if self.global_metrics:
            self.global_metrics.record_gauge(
                "scanner.scan_duration_ms", duration_ms, tags={"scanner": scanner_name}
            )
            self.global_metrics.record_counter(
                "scanner.symbols_scanned", symbol_count, tags={"scanner": scanner_name}
            )
    ...
    def record_alert_generated(
        ...
        metric = ScannerMetric(
            ...
            timestamp=datetime.now(UTC),
            value=confidence,
            tags={"alert_type": alert_type, "symbol": symbol},
        )
        self._record_metric(metric)
        ...
        if self.global_metrics:
            self.global_metrics.record_counter(
                ...
                1,
                tags={"scanner": scanner_name, "alert_type": alert_type, "symbol": symbol},
            )
            self.global_metrics.record_gauge(
                "scanner.alert_confidence",
                confidence,
                tags={"scanner": scanner_name, "alert_type": alert_type},
            )
    ...
    def record_scan_error(self, scanner_name: str, error_type: str, error_message: str) -> None:
        ...
        if self.global_metrics:
            self.global_metrics.record_counter(
                "scanner.errors", 1, tags={"scanner": scanner_name, "error_type": error_type}
            )
    ...
    def get_time_series_metrics(
        self, scanner_name: str, metric_type: str, minutes: int = 30
    ) -> list[dict[str, Any]]:
        """
    ...
    def get_alert_statistics(
        self, scanner_name: str | None = None, hours: int = 24
    ) -> dict[str, Any]:
        """
        ...
        if scanner_name:
            ...
            stats = {name: dict(counts) for name, counts in self.alert_counts.items()}
    ...
    async def _aggregation_loop(self) -> None:
        ...
        while True:
            try:
                ...
                # Clean old metrics
                cutoff_time = datetime.now(UTC) - timedelta(minutes=self.retention_minutes)
    ...
    def _get_or_create_performance(self, scanner_name: str) -> ScannerPerformance:
        ...
        if scanner_name not in self.scanner_performance:
            self.scanner_performance[scanner_name] = ScannerPerformance(scanner_name=scanner_name)
    ...
    def _format_performance_summary(self, perf: ScannerPerformance) -> dict[str, Any]:
        ...
        return {
            ...
            "last_scan_time": (perf.last_scan_time.isoformat() if perf.last_scan_time else None),
            "alerts_per_scan": (
                round(perf.total_alerts_generated / perf.total_scans, 2)
                if perf.total_scans > 0
                else 0
            ),
            "symbols_per_scan": (
