...
def safe_log(
    value: float | np.ndarray | pd.Series, base: float | None = None, default_value: float = 0.0
) -> float | np.ndarray | pd.Series:
    """
    ...
    try:
        ...
        if isinstance(value, (pd.Series, np.ndarray)):
            ...
            result = pd.Series(
                index=value.index if isinstance(value, pd.Series) else None, data=default_value
            )
...
def safe_sqrt(
    value: float | np.ndarray | pd.Series, default_value: float = 0.0
) -> float | np.ndarray | pd.Series:
    """
...
def calculate_growth_rate(
    current_value: float, previous_value: float, time_hours: float, default_rate: float = 0.0
) -> float:
    """
