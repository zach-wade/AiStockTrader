...
from main.utils.monitoring import MetricType, record_metric
...
class LayerMetricsCollector:
    ...
    def record_api_call(
        ...
        record_metric("layer.api_calls", 1, MetricType.COUNTER, tags=tags)

        # Record duration if provided
        if duration_ms is not None:
            record_metric(
                "layer.api_call_duration_ms", duration_ms, MetricType.HISTOGRAM, tags=tags
            )

        # Log for debugging
        logger.debug(
            f"API call recorded - Layer: {layer.name}, " f"Endpoint: {endpoint}, Success: {success}"
        )

    @staticmethod
    def record_rate_limit_usage(
        layer: DataLayer, usage_percent: float, calls_made: int, calls_limit: int
    ) -> None:
        """
        ...
        """
        tags = {"layer": layer.name, "layer_value": str(layer.value)}

        # Record usage percentage as gauge
        record_metric("layer.rate_limit_usage_percent", usage_percent, MetricType.GAUGE, tags=tags)

        # Record actual vs limit
        record_metric("layer.rate_limit_calls_made", calls_made, MetricType.GAUGE, tags=tags)

        record_metric("layer.rate_limit_calls_limit", calls_limit, MetricType.GAUGE, tags=tags)
    ...
    def record_layer_promotion(
        ...
        record_metric("layer.promotions", 1, MetricType.COUNTER, tags=tags)

        # Record layer change magnitude
        layer_change = to_layer.value - from_layer.value
        record_metric("layer.promotion_magnitude", layer_change, MetricType.HISTOGRAM, tags=tags)
    ...
    def record_layer_demotion(
        ...
        record_metric("layer.demotions", 1, MetricType.COUNTER, tags=tags)

        # Record layer change magnitude (negative for demotion)
        layer_change = from_layer.value - to_layer.value
        record_metric("layer.demotion_magnitude", layer_change, MetricType.HISTOGRAM, tags=tags)
    ...
    def record_layer_capacity(layer: DataLayer, current_symbols: int) -> None:
        ...
        record_metric("layer.symbol_count", current_symbols, MetricType.GAUGE, tags=tags)

        # Record max capacity
        record_metric("layer.symbol_capacity", max_symbols, MetricType.GAUGE, tags=tags)

        # Record usage percentage
        record_metric("layer.capacity_usage_percent", usage_percent, MetricType.GAUGE, tags=tags)
    ...
    def record_data_processing(
        ...
        record_metric("layer.records_processed", records_processed, MetricType.COUNTER, tags=tags)

        # Record processing time
        record_metric("layer.processing_duration_ms", duration_ms, MetricType.HISTOGRAM, tags=tags)

        # Calculate and record throughput
        if duration_ms > 0:
            throughput = (records_processed / duration_ms) * 1000  # records per second
            record_metric("layer.processing_throughput", throughput, MetricType.GAUGE, tags=tags)
