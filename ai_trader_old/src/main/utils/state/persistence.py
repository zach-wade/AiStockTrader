...
class StatePersistence:
    ...
    async def restore(self, checkpoint_id: str, namespace: str = "default") -> bool:
        ...
        try:
            ...
            checkpoint_data = await self.state_manager.get(
                f"_checkpoint_{checkpoint_id}", namespace="_system", backend=StorageBackend.FILE
            )
    ...
    async def list_checkpoints(self, namespace: str | None = None) -> list[StateCheckpoint]:
        """
    ...
    def get_checkpoint_stats(self) -> dict[str, int]:
        ...
        return {
            ...
            "checkpoints_by_namespace": len(set(cp.namespace for cp in self._checkpoints.values())),
        }
