...
class QueryTracker:
    ...
    def track_query(
        ...
        if execution_time >= self.slow_query_threshold:
            ...
            logger.warning(
                f"Slow query detected: {execution_time:.3f}s - {normalized_query[:100]}..."
            )
    ...
    def _analyze_query_issues(self, stats: QueryStats, analysis: QueryAnalysis):
        ...
        if len(stats.recent_executions) > 50:
            ...
            if recent_avg > stats.avg_time * 1.5:
                analysis.issues.append(
                    f"Performance degradation: recent avg {recent_avg:.3f}s vs overall {stats.avg_time:.3f}s"
                )
        ...
        if stats.query_type in [QueryType.UPDATE, QueryType.DELETE] and "WHERE" not in query_upper:
        ...
        return [
            {
                ...
                "last_execution": (
                    stats.last_execution.isoformat() if stats.last_execution else None
                ),
            }
            for stats in sorted_stats[:limit]
        ]

    def get_query_analyses(self, priority: QueryPriority | None = None) -> list[dict[str, Any]]:
    ...
    def export_analysis_report(self) -> str:
        ...
        for i, stats in enumerate(self.get_query_stats(10), 1):
            ...
            report.append(
                f"   Avg Time: {stats['avg_time']:.4f}s, Count: {stats['execution_count']}"
            )
