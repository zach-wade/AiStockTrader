...
class PoolHealthMonitor:
    ...
    def assess_health(self, pool_info: dict[str, Any]) -> ConnectionHealthStatus:
        ...
        avg_response_time = (
            metrics["performance_metrics"]["recent_avg_time"] * 1000
        )  # Convert to ms
    ...
    def check_connection_leaks(self, pool_info: dict[str, Any]) -> dict[str, Any]:
        ...
        if overflow > max_overflow * 0.5:
            leak_indicators.append(f"High overflow usage: {overflow}/{max_overflow}")
    ...
    def generate_optimization_recommendations(self) -> list[str]:
        ...
        if avg_response_time > 0.2:  # 200ms
            recommendations.append("High average response time. Consider connection pool tuning.")
        ...
        if error_metrics["pool_exhaustions"] > 5:
            recommendations.append("Pool exhaustion detected. Consider increasing pool size.")

        # Usage-based recommendations
        pool_status = metrics["pool_status"]
        if pool_status["overflow_connections"] > 0:
            recommendations.append("Overflow connections in use. Monitor for traffic spikes.")
