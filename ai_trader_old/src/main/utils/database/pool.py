...
from .helpers import ConnectionHealthStatus, PoolHealthMonitor, QueryPerformanceTracker
...
class DatabasePool:
    ...
    def initialize(
        ...
        config: dict[str, Any] | DictConfig | AITraderConfig | None = None,
    ):
        """
        ...
        logger.info(
            f"Database connection pool initialized: {self._mask_password(self._database_url)}"
        )
    ...
    def _build_url_from_env(self) -> str | None:
    ...
    def get_health_status(self) -> ConnectionHealthStatus:
        ...
        pool = self._engine.pool
        pool_info = {"pool_size": pool.size(), "max_overflow": getattr(pool, "_max_overflow", 0)}
