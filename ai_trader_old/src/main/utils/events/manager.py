...
class CallbackManager:
    ...
    def register_global_callback(
        ...
        **metadata,
        ...
        """
        callback_info = CallbackInfo(
            callback=callback, priority=priority, event_filter=event_filter, metadata=metadata
        )
    ...
    def unregister_callback(self, event_type: str, callback: Callable) -> bool:
        ...
        self.callbacks[event_type] = [
            cb for cb in self.callbacks[event_type] if cb.get_callback() != callback
        ]
    ...
    async def emit_event(
        ...
        **metadata,
        ...
        # Create event
        event = Event(
            event_type=event_type, data=data, source=source, event_id=event_id, metadata=metadata
        )
        ...
        if event_type in self.event_filters:
            if not self.event_filters[event_type](event):
                ...
                return EventResult(event=event, status=EventStatus.CANCELLED)
    ...
    async def _process_event(self, event: Event) -> EventResult:
        ...
        result = EventResult(event=event, status=EventStatus.PROCESSING)
    ...
    async def _execute_callbacks(
        self, callbacks: list[CallbackInfo], event: Event, result: EventResult
    ):
    ...
    async def _execute_single_callback(self, callback_info: CallbackInfo, event: Event) -> Any:
        ...
        for attempt in range(callback_info.max_retries + 1):
            try:
                ...
                if attempt < callback_info.max_retries:
                    ...
                    logger.error(
                        f"Callback failed after {callback_info.max_retries + 1} attempts: {e}"
                    )
                    raise
