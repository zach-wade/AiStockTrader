...
from main.events.handlers.feature_pipeline_helpers.feature_handler_stats_tracker import (
...
from main.feature_pipeline.feature_orchestrator import (
    FeatureOrchestrator,  # Feature computation logic
)
...
class FeaturePipelineHandler:
    ...
    async def start(self):
        ...
        await self.event_bus.subscribe(
            EventType.FEATURE_REQUEST, self._handle_feature_request_event  # Pass the handler method
        )
        ...
        for i in range(self._num_workers):
            ...
            worker_task = asyncio.create_task(self._run_worker_loop(worker_instance))
    ...
    async def stop(self):
        ...
        await self.event_bus.unsubscribe(
            EventType.FEATURE_REQUEST, self._handle_feature_request_event
        )
        ...
        if self._workers:
            ...
            )  # return_exceptions=True so one cancelled worker doesn't stop others
    ...
    async def _handle_feature_request_event(self, event: Event):
        ...
        try:
            ...
            await self._request_queue_manager.add_request(
                symbols=symbols, features=features, event=event, priority=priority
            )
            self._stats_tracker.increment_requests_received()  # Update stats

        except Exception as e:
            logger.error(
                f"Error in _handle_feature_request_event for event {event.correlation_id}: {e}",
                exc_info=True,
            )
            record_metric(
                "feature_pipeline.request_error", 1, tags={"error_type": type(e).__name__}
            )
    ...
    async def _run_worker_loop(self, worker_instance: FeatureComputationWorker):
        ...
        while True:
            try:
                ...
                _priority, _timestamp, request_data = (
                    await self._request_queue_manager.get_next_request()
                )

                # Process the request using the dedicated worker instance
                record_metric(
                    "feature_pipeline.worker_processing",
                    1,
                    tags={
                ...
                logger.error(
                    f"Unhandled exception in worker loop {worker_instance.worker_id}: {e}",
                    exc_info=True,
                )
                record_metric(
                    "feature_pipeline.worker_error",
                    1,
                    tags={"worker_id": worker_instance.worker_id, "error_type": type(e).__name__},
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return self._stats_tracker.get_stats(queue_size=queue_size, active_workers=active_workers)
