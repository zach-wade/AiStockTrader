...
from main.events.handlers.scanner_bridge_helpers.feature_request_batcher import (
    FeatureRequestBatch,  # For type hinting batch
)
    ...
    async def send_feature_request_batch(self, request_batch: FeatureRequestBatch):
        ...
        try:
            ...
            feature_event.metadata = {
                ...
                "request_age_seconds": (
                    datetime.now(UTC) - request_batch.created_at
                ).total_seconds(),
                "correlation_ids": (
                    list(request_batch.correlation_ids) if request_batch.correlation_ids else []
                ),
            }
            ...
            logger.info(
                f"Dispatched FeatureRequestEvent: {len(request_batch.symbols)} symbols, "
                f"{len(features_list)} features, priority {request_batch.priority}."
            )
        except Exception as e:
            self.handle_error(e, "dispatching feature request batch")
