...
from main.events.types import Event, EventType, FeatureRequestEvent, ScanAlert, ScannerAlertEvent
from main.interfaces.events import IEventBus
from main.utils.core import ErrorHandlingMixin, RateLimiter, ensure_utc, get_logger
...
class ScannerFeatureBridge(ErrorHandlingMixin):
    ...
    def __init__(
        ...
        # Rate limiting
        self.rate_limiter = RateLimiter(rate=rate_limit_per_second, per=1.0)  # per second
    ...
    async def start(self):
        ...
        self.event_bus.subscribe(
            EventType.SCANNER_ALERT, self._handle_scanner_alert, priority=100  # High priority
        )
    ...
    async def stop(self):
        ...
        self.event_bus.unsubscribe(EventType.SCANNER_ALERT, self._handle_scanner_alert)
    ...
    async def _is_duplicate(self, symbol: str) -> bool:
        ...
        async with self._dedup_lock:
            ...
            # Clean old entries
            self._recent_symbols = {s: t for s, t in self._recent_symbols.items() if t > cutoff}
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        stats = self.stats_tracker.get_stats(
            pending_batches_count=pending_batches, pending_symbols_count=pending_symbols
        )
        ...
        record_metric(
            "scanner_bridge.dedup_cache_size", len(self._recent_symbols), metric_type="gauge"
        )
    ...
    async def process_manual_request(
        self, symbols: list[str], feature_groups: list[str], priority: int = 0
    ):
        """
