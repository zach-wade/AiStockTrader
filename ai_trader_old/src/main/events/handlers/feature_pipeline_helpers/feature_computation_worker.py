...
from main.events.core.event_bus_helpers.event_bus_stats_tracker import (
    EventBusStatsTracker,  # To update stats
)
...
class FeatureComputationWorker(ErrorHandlingMixin):
    ...
    async def process_request(self, request: dict[str, Any]):
        ...
        features_list = request[
            "features"
        ]  # Renamed to avoid confusion with `features` param in `_compute_features`
        original_event = request["event"]  # Renamed for clarity

        logger.info(
            f"Worker {self.worker_id} processing: {len(symbols)} symbols, {len(features_list)} features."
        )
        ...
        try:
            ...
            computation_time_seconds = (datetime.now(UTC) - start_time).total_seconds()
            ...
            self.stats_tracker.increment_features_computed(
                len(results) * len(features_list)
            )  # Rough estimate

            logger.info(
                f"Worker {self.worker_id} completed request for {len(symbols)} symbols in {computation_time_seconds:.2f}s."
            )

        except Exception as e:
            logger.error(
                f"Worker {self.worker_id} failed to process feature request for symbols {symbols}: {e}",
                exc_info=True,
            )
    ...
    async def _compute_features_for_group(
        self, symbol: str, feature_group_name: str, features_in_group: list[str]
    ) -> pd.DataFrame | None:
        """
        ...
        try:
            ...
            if feature_group_name in group_mappings:
                ...
                logger.warning(
                    f"Unknown feature group '{feature_group_name}'. No feature sets configured for it."
                )
                return None  # No features to compute for this unknown group

            if not feature_sets_to_request:
                logger.debug(
                    f"No specific feature sets defined for group '{feature_group_name}'. Skipping computation."
                )
                return None
            ...
            if computed_df is not None:
                record_metric(
                    "feature_worker.features_computed",
                    len(computed_df.columns) if hasattr(computed_df, "columns") else 0,
                    tags={"symbol": symbol, "feature_group": feature_group_name},
                )
            ...
            logger.error(
                f"Error computing {feature_group_name} features for {symbol}: {e}", exc_info=True
            )
            record_metric(
                "feature_worker.computation_error",
                1,
                tags={
                    "worker_id": self.worker_id,
                    "symbol": symbol,
