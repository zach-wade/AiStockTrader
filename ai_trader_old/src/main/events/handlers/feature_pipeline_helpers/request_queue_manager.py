...
class RequestQueueManager(ErrorHandlingMixin):
    ...
    def __init__(
        ...
        self._dedup_tracker = DeduplicationTracker(dedup_window_seconds=dedup_window_seconds)
    ...
    @timer
    async def enqueue_request(self, request: FeatureRequest, request_id: str | None = None) -> bool:
        """
        ...
        async with self._lock:
            try:
                ...
                queued = QueuedRequest(request=request, request_id=request_id)
                ...
                record_metric(
                    ...
                    1,
                    tags={"symbol": request.symbol, "priority": request.priority},
                )
    ...
    async def dequeue_request(self, worker_id: int | None = None) -> QueuedRequest | None:
        """
        ...
        async with self._lock:
            try:
                ...
                while self._queue:
                    ...
                    # Check symbol limit
                    if (
                        self._active_requests_by_symbol[queued.request.symbol]
                        >= self.max_requests_per_symbol
                    ):
                        # Put back in queue
                        heapq.heappush(self._queue, queued)
        ...
        async with self._lock:
            if request_id in self._request_map:
                ...
                record_metric(
                    ...
                    1,
                    tags={"success": success, "symbol": queued.request.symbol},
                )

                logger.debug(f"Completed request {request_id} " f"(success={success})")
    ...
    def _is_expired(self, queued: QueuedRequest) -> bool:
        ...
        age = (datetime.now(UTC) - queued.queued_at).seconds
    ...
    def _handle_expired_request(self, queued: QueuedRequest) -> None:
        ...
        # Record metric
        record_metric("request_queue.expired", 1, tags={"symbol": queued.request.symbol})

        logger.warning(f"Request {queued.request_id} expired after " f"{self.request_ttl_seconds}s")

    def _find_alternative_request(self) -> QueuedRequest | None:
        ...
        while self._queue and not alternative:
            ...
            if (
                self._active_requests_by_symbol[candidate.request.symbol]
                < self.max_requests_per_symbol
            ):
