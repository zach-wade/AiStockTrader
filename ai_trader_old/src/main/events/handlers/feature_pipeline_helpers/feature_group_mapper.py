...
class FeatureGroupMapper(ErrorHandlingMixin):
    ...
    def map_alert_to_features(self, alert: ScanAlert) -> FeatureRequest:
        ...
        try:
            ...
            base_groups = self.alert_mappings.get(
                alert.alert_type, self.alert_mappings[AlertType.UNKNOWN]
            )

            # Record alert mapping metrics
            record_metric(
                "feature_group_mapper.alert_mapped",
                1,
                tags={
            ...
            record_metric(
                "feature_group_mapper.groups_selected",
                len(all_groups),
    ...
    def get_required_data_types(self, feature_groups: list[FeatureGroup]) -> set[str]:
        ...
        record_metric(
            "feature_group_mapper.data_types_query",
            1,
            tags={"group_count": len(feature_groups), "data_type_count": len(data_types)},
        )
    ...
    def prioritize_requests(self, requests: list[FeatureRequest]) -> list[FeatureRequest]:
        ...
        """
        return sorted(requests, key=lambda r: (r.priority, -r.created_at.timestamp()), reverse=True)
    ...
    def _get_conditional_groups(self, alert: ScanAlert) -> list[FeatureGroup]:
        ...
        # Volume spike alerts
        if (
            alert.data.get("volume_multiplier", 1.0)
            > self.conditional_rules["volume_spike_multiplier"]
        ):
    ...
    def _calculate_priority(self, alert: ScanAlert) -> int:
        ...
        base_priority = self.priority_rules["base_priority_map"].get(alert.alert_type, 0)
        ...
        priority = max(
            self.priority_rules["min_priority"], min(priority, self.priority_rules["max_priority"])
        )
    ...
    def get_computation_params(self, feature_groups: list[FeatureGroup]) -> dict[str, Any]:
        """
