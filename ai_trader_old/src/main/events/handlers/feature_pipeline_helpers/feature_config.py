...
def initialize_group_configs() -> dict[FeatureGroup, FeatureGroupConfig]:
    ...
    return {
        ...
        FeatureGroup.PRICE: FeatureGroupConfig(
            ...
            computation_params={"lookback_periods": [5, 10, 20, 50], "return_periods": [1, 5, 20]},
        ),
        FeatureGroup.VOLUME: FeatureGroupConfig(
            ...
            computation_params={"volume_windows": [5, 10, 30], "vwap_periods": [5, 15, 60]},
        ),
        FeatureGroup.VOLATILITY: FeatureGroupConfig(
            ...
            computation_params={"volatility_windows": [10, 20, 50], "ewm_spans": [10, 20]},
        ),
        FeatureGroup.MOMENTUM: FeatureGroupConfig(
            ...
            required_data=["prices"],
            dependencies=[FeatureGroup.PRICE],
            computation_params={"rsi_periods": [9, 14, 21], "macd_params": [(12, 26, 9)]},
        ),
        FeatureGroup.TREND: FeatureGroupConfig(
            ...
            computation_params={"ma_periods": [10, 20, 50, 200], "channel_periods": [20, 50]},
        ),
        ...
        FeatureGroup.TECHNICAL_BASIC: FeatureGroupConfig(
            ...
            computation_params={"bb_periods": [20], "bb_stds": [2], "stoch_periods": [(14, 3)]},
        ),
        ...
        FeatureGroup.MICROSTRUCTURE: FeatureGroupConfig(
            ...
            computation_params={"depth_levels": [5, 10], "imbalance_windows": [1, 5, 15]},
            priority_boost=2,
        ),
        FeatureGroup.ORDER_FLOW: FeatureGroupConfig(
            ...
            computation_params={"flow_windows": [5, 15, 60], "aggressor_side": True},
            priority_boost=2,
        ),
        # Sentiment features
        FeatureGroup.NEWS_SENTIMENT: FeatureGroupConfig(
            ...
            required_data=["news"],
            computation_params={"lookback_hours": [1, 6, 24], "sentiment_model": "finbert"},
        ),
        FeatureGroup.SOCIAL_SENTIMENT: FeatureGroupConfig(
            ...
            computation_params={"platforms": ["twitter", "reddit"], "lookback_hours": [1, 4, 12]},
        ),
        # Risk features
        FeatureGroup.RISK_METRICS: FeatureGroupConfig(
            ...
            computation_params={"var_confidence": [0.95, 0.99], "cvar_confidence": [0.95]},
        ),
        ...
        FeatureGroup.EARNINGS: FeatureGroupConfig(
            ...
            computation_params={"lookback_quarters": 4, "forward_estimates": True},
        ),
        FeatureGroup.CORPORATE_ACTIONS: FeatureGroupConfig(
            ...
            computation_params={"action_types": ["dividend", "split", "merger"]},
        ),
        # ML features
        FeatureGroup.ML_SIGNALS: FeatureGroupConfig(
            ...
            dependencies=[FeatureGroup.PRICE, FeatureGroup.VOLUME, FeatureGroup.TECHNICAL_BASIC],
            computation_params={"models": ["momentum_classifier", "mean_reversion_regressor"]},
            priority_boost=3,
        ),
        FeatureGroup.EMBEDDINGS: FeatureGroupConfig(
            ...
            computation_params={"embedding_dims": 64, "model_version": "v2.0"},
            priority_boost=3,
        ),
    }
...
def initialize_alert_mappings() -> dict[AlertType, list[FeatureGroup]]:
    ...
    return {
        ...
        # Default mapping
        AlertType.UNKNOWN: [FeatureGroup.PRICE, FeatureGroup.VOLUME],
    }
...
def get_conditional_group_rules() -> dict[str, Any]:
    ...
    return {
        ...
        "high_score_additions": [FeatureGroup.ML_SIGNALS, FeatureGroup.TECHNICAL_ADVANCED],
        "volume_spike_multiplier": 3.0,
        "volume_spike_additions": [FeatureGroup.ORDER_FLOW, FeatureGroup.MICROSTRUCTURE],
...
def get_priority_calculation_rules() -> dict[str, Any]:
    ...
    return {
        ...
        "volatility_boost": {"low": 0, "normal": 0, "high": 1, "extreme": 2},
    }
