...
from main.utils.core import ErrorHandlingMixin, get_logger, secure_dumps, secure_loads
from main.utils.database import DatabasePool, batch_upsert, execute_with_retry, transaction_context
...
class DeadLetterQueueManager(ErrorHandlingMixin):
    ...
    async def add_event(
        self, event: Event, error_message: str, error_type: str | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            if event_id in self._event_index:
                ...
                failed_event = FailedEvent(
                    event=event, error_message=error_message, error_type=error_type or "unknown"
                )
            ...
            record_metric(
                "dlq.event_added",
                1,
                tags={"event_type": event.event_type.value, "error_type": error_type or "unknown"},
            )

            logger.warning(f"Added event to DLQ: {event.event_type.value} - {error_message}")
    ...
    async def process_events(
        ...
        event_filter: Callable[[FailedEvent], bool] | None = None,
    ) -> dict[str, int]:
        """
        ...
        events_to_process = [
            ...
            if (not event_filter or event_filter(fe)) and fe.failure_count <= max_retries
        ]
        ...
        for failed_event in events_to_process:
            ...
            try:
                ...
                # Update failure info
                await self.add_event(failed_event.event, str(e), type(e).__name__)

                logger.error(f"Failed to reprocess event from DLQ: {e}", exc_info=True)
    ...
    def get_failed_events(
        ...
        if error_type:
            filtered = [fe for fe in filtered if fe.error_type == error_type]
    ...
    def get_failure_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "oldest_failure": (
                min(fe.first_failure for fe in self._queue) if self._queue else None
            ),
            "newest_failure": (max(fe.last_failure for fe in self._queue) if self._queue else None),
        }
    ...
    async def cleanup_expired(self) -> int:
        ...
        expired = [fe for fe in self._queue if fe.last_failure < cutoff_time]
    ...
    async def schedule_retry(
        self, failed_event: FailedEvent, publish_func: Callable[[Event], asyncio.Future]
    ) -> None:
        """
    ...
    def _get_event_id(self, event: Event) -> str:
        ...
        key_parts = [event.event_type.value, str(event.timestamp), str(event.data.get("id", ""))]
    ...
    async def _evict_oldest(self) -> None:
        ...
        if self._queue:
            ...
            logger.warning(f"Evicted oldest event from DLQ: {oldest.event.event_type.value}")
    ...
    async def _remove_persisted_event(self, event_id: str) -> None:
        ...
        async def delete_operation(conn):
            return await conn.execute("DELETE FROM event_dlq WHERE event_id = $1", event_id)

        await execute_with_retry(self.db_pool, delete_operation, max_retries=3)
