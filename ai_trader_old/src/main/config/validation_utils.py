...
class ConfigValidator:
        ...
        try:
            ...
            try:
                ...
                raise ConfigValidationError(
                    ...
                    [f"YAML parsing error: {e!s}"],
                )
            except Exception as e:
                raise ConfigValidationError(
                    f"Error reading configuration file: {config_path}", [f"File read error: {e!s}"]
                )
    ...
    def validate_dict(
        self, config_dict: dict[str, Any], source: str = "configuration"
    ) -> AITraderConfig:
        """
        ...
        try:
            ...
            raise ConfigValidationError(
                f"Configuration validation failed for {source}", validation_errors
            )
        ...
        if self.warnings:
            if self.strict_mode:
                raise ConfigValidationError(
                    f"Configuration validation warnings in strict mode for {source}", self.warnings
                )
            else:
        ...
        if self.errors:
            raise ConfigValidationError(
                f"Configuration validation errors for {source}", self.errors
            )
    ...
    def _post_validate_config(self, config: AITraderConfig) -> None:
        ...
        if config.system.environment.value == "live":
            ...
            if config.risk.position_sizing.max_position_size > 2.0:
                self.warnings.append(
                    f"High position size limit for live trading: {config.risk.position_sizing.max_position_size}%"
                )

            if config.risk.circuit_breaker.daily_loss_limit > 5.0:
                self.warnings.append(
                    f"High daily loss limit for live trading: {config.risk.circuit_breaker.daily_loss_limit}%"
                )
        ...
        if config.data.backfill.max_parallel > 30:
            self.warnings.append(
                f"High parallel backfill setting may cause rate limiting: {config.data.backfill.max_parallel}"
            )

        # Check data source availability
        if len(config.data.sources) == 1 and config.data.sources[0].value == "alpaca":
            self.warnings.append(
                "Only Alpaca data source configured - consider adding backup sources"
            )
    ...
    def _format_validation_errors(self, validation_error: ValidationError) -> list[str]:
        ...
        for error in validation_error.errors():
            ...
            if error_type == "value_error.missing":
                ...
                errors.append(
                    f"Value must be greater than {error.get('ctx', {}).get('limit_value', 0)}: {field_path}"
                )
            elif error_type == "value_error.number.not_ge":
                errors.append(
                    f"Value must be greater than or equal to {error.get('ctx', {}).get('limit_value', 0)}: {field_path}"
                )
            elif error_type == "value_error.number.not_le":
                errors.append(
                    f"Value must be less than or equal to {error.get('ctx', {}).get('limit_value', 0)}: {field_path}"
                )
    ...
    def check_system_requirements(self) -> list[str]:
        ...
        required_packages = ["pydantic", "pandas", "numpy", "alpaca-py", "pyyaml", "python-dotenv"]
...
def validate_startup_config(config_path: str | None = None) -> AITraderConfig:
    """
...
def validate_config_and_warn(config_path: str) -> tuple[AITraderConfig | None, list[str]]:
    """
