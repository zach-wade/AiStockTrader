...
class SystemConfig(BaseModel):
    ...
    debug: dict[str, Any] | None = Field(default_factory=dict, description="Debug settings")
...
class PositionSizingConfig(BaseModel):
    ...
    method: PositionSizeType = Field(
        default=PositionSizeType.EQUAL_WEIGHT, description="Position sizing method"
    )
    default_position_size: PositiveFloat = Field(
        default=5000.0, description="Default position size in dollars"
    )
...
class ExecutionConfig(BaseModel):
    ...
    algorithm: ExecutionAlgorithm = Field(
        default=ExecutionAlgorithm.MARKET, description="Execution algorithm"
    )
    slippage_bps: NonNegativeFloat = Field(
        default=5.0, le=100.0, description="Slippage in basis points"
    )
...
class TradingConfig(BaseModel):
    ...
    position_sizing: PositionSizingConfig = Field(
        default_factory=PositionSizingConfig, description="Position sizing config"
    )
    execution: ExecutionConfig = Field(
        default_factory=ExecutionConfig, description="Execution config"
    )
    close_positions_on_shutdown: bool = Field(
        default=False, description="Close positions on shutdown"
    )
...
class RiskPositionSizingConfig(BaseModel):
    ...
    method: PositionSizeType = Field(
        default=PositionSizeType.KELLY, description="Risk position sizing method"
    )
    max_position_size: PositionSize = Field(
        default=20.0, description="Maximum position size as percentage of portfolio"
    )
    max_portfolio_leverage: PositiveFloat = Field(
        default=1.0, le=4.0, description="Maximum portfolio leverage"
    )
...
class RiskLimitsConfig(BaseModel):
    ...
    max_positions: PositiveInt = Field(
        default=20, le=100, description="Maximum concurrent positions"
    )
    max_sector_exposure: Percentage = Field(
        default=30.0, description="Maximum sector exposure percentage"
    )
...
class StopLossConfig(BaseModel):
    ...
    default_percentage: RiskPercentage = Field(
...
class RiskConfig(BaseModel):
    ...
    position_sizing: RiskPositionSizingConfig = Field(
        default_factory=RiskPositionSizingConfig, description="Position sizing config"
    )
    limits: RiskLimitsConfig = Field(
        default_factory=RiskLimitsConfig, description="Risk limits config"
    )
    stop_loss: StopLossConfig = Field(
        default_factory=StopLossConfig, description="Stop loss config"
    )
...
class StrategyWeights(BaseModel):
    ...
    default_weight: NonNegativeFloat = Field(
        default=0.5, le=1.0, description="Default strategy weight"
    )
...
class StrategiesConfig(BaseModel):
    ...
    weights: dict[str, StrategyWeights] = Field(
        default_factory=dict, description="Strategy weights"
    )
    lookback_periods: dict[str, int] = Field(
        default_factory=dict, description="Strategy lookback periods"
    )
