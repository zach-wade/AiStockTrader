...
class UniverseConfig(BaseModel):
    ...
    provider: DataProvider = Field(
        default=DataProvider.ALPACA, description="Universe data provider"
    )
    ...
    max_symbols: PositiveInt = Field(
        default=2000, le=10000, description="Maximum symbols in universe"
    )
...
class SourceLimitsConfig(BaseModel):
    ...
    market_data: dict[TimeFrame, NonNegativeInt] = Field(
        default_factory=dict, description="Market data limits by timeframe"
    )
    news: NonNegativeInt = Field(default=0, description="News data limit in days")
    corporate_actions: NonNegativeInt = Field(
        default=0, description="Corporate actions limit in days"
    )
...
class BackfillConfig(BaseModel):
    ...
    max_parallel: PositiveInt = Field(
        default=20, le=50, description="Maximum parallel backfill jobs"
    )
    source_limits: dict[str, SourceLimitsConfig] = Field(
        default_factory=dict, description="Source-specific limits"
    )
...
class DataConfig(BaseModel):
    ...
    universe: UniverseConfig = Field(
        default_factory=UniverseConfig, description="Universe configuration"
    )
    sources: list[DataProvider] = Field(
        default_factory=lambda: [DataProvider.ALPACA], description="Data sources"
    )
    streaming: dict[str, Any] = Field(default_factory=dict, description="Streaming configuration")
    backfill: BackfillConfig = Field(
        default_factory=BackfillConfig, description="Backfill configuration"
    )
...
class TechnicalIndicatorsConfig(BaseModel):
    ...
    bb_std: PositiveFloat = Field(
        default=2.0, ge=1.0, le=3.0, description="Bollinger Bands standard deviation"
    )
...
class FeaturesConfig(BaseModel):
    ...
    technical_indicators: TechnicalIndicatorsConfig = Field(
        default_factory=TechnicalIndicatorsConfig, description="Technical indicators config"
    )
    microstructure: dict[str, bool] = Field(
        default_factory=lambda: {"enabled": True}, description="Microstructure features"
    )
    cross_sectional: dict[str, bool] = Field(
        default_factory=lambda: {"enabled": True}, description="Cross-sectional features"
    )
    advanced_statistical: dict[str, bool] = Field(
        default_factory=lambda: {"enabled": True}, description="Advanced statistical features"
    )
...
class TrainingConfig(BaseModel):
    ...
    top_n_symbols_for_training: PositiveInt = Field(
        default=500, le=2000, description="Top N symbols for training"
    )
    ...
    max_symbols_per_sector: PositiveInt = Field(
        default=50, description="Maximum symbols per sector"
    )
    feature_lookback_days: PositiveInt = Field(default=730, description="Feature lookback days")
    models: list[str] = Field(
        default_factory=lambda: ["xgboost", "lightgbm", "random_forest", "ensemble"],
...
class UniverseMainConfig(BaseModel):
    ...
    filters: UniverseFiltersConfig = Field(
        default_factory=UniverseFiltersConfig, description="Universe filters"
    )
...
class PathsConfig(BaseModel):
    ...
    feature_cache: str = Field(
        default="data_pipeline/storage/feature_cache", description="Feature cache path"
    )
