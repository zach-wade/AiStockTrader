...
class ConfigCache:
    ...
    def get(self, key: str) -> DictConfig | None:
        """
...
class ConfigManager:
    ...
    def _load_yaml_config(
        self, config_path: Path, overrides: list[str] | None = None
    ) -> DictConfig:
        """
    ...
    def _create_cache_key(self, config_name: str, overrides: list[str] | None) -> str:
        """
    ...
    def _load_unified_config(
        self, overrides: list[str] | None = None, force_reload: bool = False
    ) -> DictConfig:
        """
        ...
        try:
                ...
                cfg.trading = OmegaConf.create(
                    {
                        ...
                        "strategy": "default",
                        "capital": 10000,
                        "risk_management": {"max_risk_per_trade": 0.02, "max_positions": 10},
                        "position_sizing": {"max_position_size": 1000},
                        "starting_cash": 10000,
                    }
                )

            if "monitoring" not in cfg:
                cfg.monitoring = OmegaConf.create({"dashboard": {"port": 8050}})
            ...
            if "broker" not in cfg:
                cfg.broker = OmegaConf.create({"paper_trading": True})
            ...
            return OmegaConf.create(
                {
                    ...
                    "api_keys": {"alpaca": {"key": "", "secret": ""}, "polygon": {"key": ""}},
    ...
    def _validate_config(self, cfg: DictConfig, source: str) -> None:
        ...
        try:
            ...
            if self.strict_validation:
                ...
                logger.warning(
                    "Continuing with unvalidated configuration - consider fixing validation errors"
                )
    ...
    def load_environment_config(
        self, env: str, base_config: DictConfig | None = None
    ) -> DictConfig:
        """
        ...
        try:
            ...
            try:
                ...
                for key, value in env_config.items():
                    if (
                        key in merged
                        and OmegaConf.is_config(merged[key])
    ...
    def clear_cache(self, config_name: str | None = None) -> None:
        """
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        with self._cache._lock:
            ...
            expired_count = sum(
                1
                for _, timestamp in self._cache._cache.values()
...
def get_production_config_manager(config_dir: str | None = None) -> ConfigManager:
    """
    ...
    """
    return ConfigManager(
        config_dir=config_dir, use_validation=True, strict_validation=True, enable_caching=True
    )
...
def get_config(
    ...
    manager = get_config_manager(
        config_dir=config_dir, use_validation=False, strict_validation=False
    )
