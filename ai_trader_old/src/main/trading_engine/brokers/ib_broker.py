...
class IBBroker(BrokerInterface, ErrorHandlingMixin):
    ...
    def __init__(self, config: Any = None):
        ...
        if not IB_AVAILABLE:
            raise ImportError(
                "ib_insync package is required for IB broker. Install with: pip install ib_insync"
            )
    ...
    async def _create_contract(self, symbol: str) -> Contract:
        ...
        await self.cache.set(
            CacheType.CUSTOM, f"ib_contract:{symbol}", contract, 3600
        )  # Cache for 1 hour
        return contract
    ...
    def _create_ib_order(self, order: Order) -> IBOrder:
        ...
        ib_order.algoParams = [("adaptivePriority", "Normal")]
    ...
    def _on_order_status(self, trade):
        ...
        if our_order.status == OrderStatus.FILLED:
            our_order.filled_at = datetime.now(UTC)
        elif our_order.status == OrderStatus.CANCELLED:
            our_order.cancelled_at = datetime.now(UTC)
    ...
    def _on_position(self, position):
        ...
        # Cache the position
        asyncio.create_task(
            self.cache.set(CacheType.POSITIONS, position.contract.symbol, pos, 300)
        )  # Cache for 5 minutes
        ...
        for ticker in tickers:
            if ticker.contract.symbol in self._subscribed_symbols:
                ...
                if cached_pos:
                    ...
                    cached_pos.unrealized_pnl = (
                        market_data.last - cached_pos.average_price
                    ) * cached_pos.quantity
                    # Update the cache with modified position
                    await self.cache.set(
                        CacheType.POSITIONS, ticker.contract.symbol, cached_pos, 300
                    )
    ...
    async def modify_order(
        ...
        our_order.modified_at = datetime.now(UTC)
