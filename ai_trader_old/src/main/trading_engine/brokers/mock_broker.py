...
# Local imports
from main.models.common import AccountInfo, MarketData, Order, OrderStatus, Position
...
class MockBroker(BrokerInterface):
    ...
    def __init__(self, config=None):
        ...
        self._order_fill_callback: Callable | None = None
    ...
    def get_call_history(self, method_name: str | None = None) -> list[dict[str, Any]]:
    ...
    async def _track_call(self, method_name: str, **kwargs):
        ...
        self._call_history.append(
            {"method": method_name, "timestamp": datetime.now(UTC), "args": kwargs}
        )
    ...
    async def submit_order(self, order: Order) -> Order:
        ...
        if self._order_fill_callback:
            ...
            if fill_result:
                ...
                submitted_order.filled_at = datetime.now(UTC)
    ...
    async def get_historical_data(
        ...
        """Get historical data."""
        result = await self._track_call(
            "get_historical_data", symbol=symbol, start=start, end=end, timeframe=timeframe
        )
        if result is not None:
            return result
