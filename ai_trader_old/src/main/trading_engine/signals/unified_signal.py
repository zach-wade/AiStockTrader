...
class UnifiedSignal:
    ...
    correlation_group: str | None = None  # For deduplication
...
class SignalAggregator:
    ...
    def add_signals(self, signals: list[UnifiedSignal]) -> list[UnifiedSignal]:
        ...
        for signal in signals:
            if self._is_duplicate(signal):
                logger.debug(
                    f"Duplicate signal filtered: {signal.signal.symbol} {signal.signal.direction}"
                )
                continue
    ...
    def _is_duplicate(self, signal: UnifiedSignal) -> bool:
        ...
        for recent in self.recent_signals:
            ...
            if (
                recent.signal.symbol == signal.signal.symbol
                and recent.signal.direction == signal.signal.direction
                and abs((signal.timestamp - recent.timestamp).total_seconds())
                < self.dedup_window.total_seconds()
            ):
    ...
    def _clean_expired_signals(self, current_time: datetime):
        ...
        for group in list(self.signal_groups.keys()):
            self.signal_groups[group] = [
                s for s in self.signal_groups[group] if s.timestamp > cutoff_time
            ]
...
class SignalRouter:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Routing rules
        self.priority_routing = self.routing_config.get(
            "priority_routing",
            {
        ...
        self.size_thresholds = self.routing_config.get(
            "size_thresholds",
            {
    ...
    def _get_execution_path(self, signal: UnifiedSignal) -> str:
        ...
        return self.priority_routing.get(signal.priority.name, "standard")
    ...
    def _calculate_urgency(self, signal: UnifiedSignal) -> float:
        ...
        if signal.expiry:
            time_remaining = (signal.expiry - datetime.now(UTC)).total_seconds()
...
class UnifiedSignalHandler:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Metrics
        self.signal_metrics = {"received": 0, "duplicates": 0, "processed": 0, "failed": 0}
    ...
    async def process_signals(self, portfolio_value: float) -> list[dict[str, Any]]:
        ...
        async with self.processing_lock:
            ...
            while self.signal_queue:
                ...
                # Check expiry
                if signal.expiry and signal.expiry < datetime.now(UTC):
                    logger.warning(
                        f"Signal expired: {signal.signal.symbol} {signal.signal.direction}"
                    )
                    continue
