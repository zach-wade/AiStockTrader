...
class TWAPAlgorithm:
    ...
    def __init__(self, broker: BrokerInterface, config: dict[str, Any] | None = None):
        self.broker = broker
        ...
        self.randomize_size = self.config.get(
            "randomize_size", True
        )  # Add randomness to hide pattern
    ...
    async def execute(
        ...
        schedule = self._create_twap_schedule(
            total_quantity, start_time, end_time, duration_minutes
        )
        ...
        logger.info(
            f"Symbol: {symbol}, Quantity: {total_quantity}, Duration: {duration_minutes}min"
        )
        logger.info(
            f"Schedule: {len(schedule.time_slices)} slices, ~{total_quantity/len(schedule.time_slices):.0f} shares/slice"
        )

        try:
            # Execute according to TWAP schedule
            await self._execute_twap_schedule(
                execution_id, symbol, side, schedule, limit_price, participation_rate
            )
    ...
    def _create_twap_schedule(
        self, total_quantity: int, start_time: datetime, end_time: datetime, duration_minutes: int
    ) -> TWAPSchedule:
        """
        ...
        return TWAPSchedule(
            time_slices=time_slices, quantities=quantities, slice_duration=slice_duration
        )

    async def _execute_twap_schedule(
        self,
        execution_id: str,
        symbol: str,
        side: str,
        ...
        for i, (slice_time, slice_quantity) in enumerate(
            zip(schedule.time_slices, schedule.quantities)
        ):
            ...
            if current_time < slice_time:
                ...
                if wait_seconds > 0:
                    logger.debug(
                        f"Waiting {wait_seconds:.1f}s until slice {i+1}/{len(schedule.time_slices)}"
                    )
            ...
            if i < len(schedule.time_slices) - 1:  # Not the last slice
                ...
                if datetime.now(UTC) > next_slice_time:
            ...
            if limit_price:
                ...
                if not self._check_price_condition(market_price, limit_price, side):
                    logger.warning(
                        f"Skipping slice {i+1} due to price condition: {market_price} vs {limit_price}"
                    )
                    continue
            ...
            logger.info(
                f"TWAP progress: {progress:.1f}% ({execution['executed_quantity']}/{execution['total_quantity']})"
            )
    ...
    async def _submit_child_order(
        ...
        try:
            ...
            if order_id:
                logger.info(
                    f"TWAP child order submitted: {order_id} - Slice {slice_number}/{total_slices} - {quantity} shares"
                )
        ...
        if fills:
            ...
            if execution["side"] == "BUY":
                execution["price_improvement"] = (
                    arrival_price - execution["vwap_price"]
                ) / arrival_price
            else:
                execution["price_improvement"] = (
                    execution["vwap_price"] - arrival_price
                ) / arrival_price
    ...
    def _get_execution_summary(self, execution_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "duration": (datetime.now(UTC) - execution["start_time"]).total_seconds() / 60,
    ...
    async def modify_execution(
        ...
        new_participation_rate: float | None = None,
    ) -> bool:
        """
        ...
        if execution_id in self.active_executions:
            ...
            if execution["status"] == "ACTIVE":
                ...
                if new_participation_rate is not None:
                    ...
                    logger.info(
                        f"Updated participation rate for {execution_id} to {new_participation_rate}"
                    )
