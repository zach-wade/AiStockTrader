...
class VWAPAlgorithm:
    ...
    def __init__(self, broker: BrokerInterface, config: dict[str, Any] | None = None):
        ...
        self.max_participation_rate = self.config.get(
    ...
    def _create_typical_volume_profile(self, duration_minutes: int) -> VolumeProfile:
        ...
        time_buckets = [
            current_time + timedelta(minutes=i * bucket_duration) for i in range(num_buckets)
        ]
    ...
    def _create_uniform_volume_profile(self, duration_minutes: int) -> VolumeProfile:
        ...
        # Calculate cumulative volume
        cumulative_volume = [(i + 1) * volume_per_bucket for i in range(num_buckets)]
    ...
    async def _execute_vwap_schedule(
        self,
        execution_id: str,
        symbol: str,
        total_quantity: int,
            ...
            if bucket_quantity > 0:
                ...
                if limit_price:
                    ...
                    if not self._check_price_condition(market_price, limit_price, side):
                        logger.warning(
                            f"Skipping bucket due to price condition: {market_price} vs {limit_price}"
                        )
                        continue

                # Submit child order
                await self._submit_child_order(
                    execution_id, symbol, bucket_quantity, side, limit_price
                )
        ...
        if remaining_quantity > 0:
            ...
            await self._submit_child_order(
                execution_id, symbol, remaining_quantity, side, limit_price
            )

    async def _submit_child_order(
        self, execution_id: str, symbol: str, quantity: int, side: str, limit_price: float | None
    ):
