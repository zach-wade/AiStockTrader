...
class ExecutionParameters:
    ...
    emergency_stop_loss: float | None = None
...
class BaseAlgorithm(ABC):
    ...
    async def execute(
        self, symbol: str, total_quantity: int, side: str | OrderSide, **kwargs
    ) -> ExecutionSummary:
        """
        ...
        params = ExecutionParameters(
            symbol=symbol, total_quantity=total_quantity, side=side, **kwargs
        )
        ...
        execution_state = ExecutionState(
            execution_id=execution_id, algorithm_name=self.algorithm_name, parameters=params
        )
    ...
    async def get_execution_status(self, execution_id: str) -> ExecutionSummary | None:
        """Get current status of an execution."""
        execution_state = self.active_executions.get(execution_id) or self.completed_executions.get(
            execution_id
        )
    ...
    @abstractmethod
    def _calculate_slice_size(self, execution_state: ExecutionState, current_slice: int) -> int:
        """
    ...
    @abstractmethod
    def _get_next_delay(self, execution_state: ExecutionState, current_slice: int) -> float:
        """
    ...
    async def _validate_parameters(self, params: ExecutionParameters) -> str | None:
        """
    ...
    async def _validate_algorithm_parameters(self, params: ExecutionParameters) -> str | None:
    ...
    async def _submit_child_order(
        self, execution_state: ExecutionState, slice_size: int, price: float | None = None
    ) -> ChildOrder:
        """
    ...
    async def _update_execution_progress(
        self, execution_state: ExecutionState, fill_quantity: int, fill_price: float
    ) -> None:
        ...
        logger.debug(
            f"Execution {execution_state.execution_id}: "
    ...
    def _add_randomization(
        self, base_value: int | float, randomization_factor: float = 0.1
    ) -> int | float:
    ...
    async def _calculate_participation_rate(self, execution_state: ExecutionState) -> float:
        ...
        try:
            ...
            volume_profile = await self._get_volume_profile(
                execution_state.parameters.symbol, days=5  # Use 5-day average
            )
    ...
    async def on_slice_submitted(
        self, execution_state: ExecutionState, child_order: ChildOrder
    ) -> None:
        """
    ...
    async def on_execution_complete(self, execution_state: ExecutionState) -> None:
        ...
        fill_rate = (
            execution_state.executed_quantity / execution_state.parameters.total_quantity
        ) * 100
        logger.info(
    ...
    async def _get_volume_profile(self, symbol: str, days: int = 30) -> pd.DataFrame | None:
