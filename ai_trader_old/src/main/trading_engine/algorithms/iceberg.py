...
class IcebergAlgorithm:
    ...
    async def _submit_slice_order(
        self, execution_id: str, symbol: str, quantity: int, side: str, price_limit: float | None
    ):
        ...
        try:
            ...
            if order_id:
                ...
                self.active_executions[execution_id]["fills"].append(
                    {"order_id": order_id, "quantity": quantity, "timestamp": datetime.now()}
                )
    ...
    def _get_execution_summary(self, execution_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_slice_size": (
                execution["executed_quantity"] / len(execution["fills"])
                if execution["fills"]
                else 0
            ),
