...
class ExecutionEngine:
    ...
    def __init__(
        ...
        self.global_position_manager: PositionManager | None = None
        ...
        logger.info(
            f"Execution engine initialized: {trading_mode.value} mode, {execution_mode.value} execution"
        )
    ...
    async def _create_trading_system(
        self, broker_name: str, broker_config: dict[str, Any]
    ) -> TradingSystem | None:
        ...
        try:
            ...
            trading_system = TradingSystem(
                broker_interface=broker_interface, config=self.config, mode=self.trading_mode
            )
    ...
    async def _perform_health_checks(self) -> bool:
        ...
        try:
            ...
            await self.cache.set(
                CacheType.METRICS, "execution_engine_health", health_status, 300  # 5 minute cache
            )
    ...
    async def submit_cross_system_order(
        self, order: Order, preferred_broker: str | None = None
    ) -> str | None:
        """
        ...
        async with self._order_coordination_lock:
            try:
                ...
                if broker_order_id:
                    ...
                    logger.info(
                        f"Cross-system order submitted: {order.symbol} via {selected_broker}"
                    )
    ...
    async def _select_optimal_broker(
        self, order: Order, preferred_broker: str | None
    ) -> str | None:
    ...
    async def _system_monitor_task(self):
        ...
        while self.status not in [ExecutionEngineStatus.STOPPED, ExecutionEngineStatus.ERROR]:
            try:
                ...
                # Update heartbeat
                self.session_metrics["last_heartbeat"] = datetime.now(UTC).isoformat()
    ...
    async def get_comprehensive_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.session_start_time:
                uptime = (datetime.now(UTC) - self.session_start_time).total_seconds()
...
async def create_execution_engine(
    ...
    engine = ExecutionEngine(
        config=config, trading_mode=trading_mode, execution_mode=execution_mode
    )
