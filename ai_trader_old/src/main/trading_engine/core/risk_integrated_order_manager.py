...
from main.trading_engine.core.unified_position_manager import OrderSide, UnifiedPositionManager
...
class RiskIntegratedOrderManager:
    ...
    async def submit_order_with_risk_check(
        self,
        symbol: str,
        side: OrderSide,
        ...
        if self.emergency_halt_active and not force_approval:
            logger.error(
                f"Order submission blocked due to emergency halt: {self.emergency_halt_reason}"
            )
            return None
        ...
        try:
            ...
            price = self._get_order_price_for_risk_calc(symbol, order_type, limit_price, stop_price)
            ...
            risk_decision = await self.risk_monitor.evaluate_trade_risk(
                symbol=symbol, side=side, quantity=quantity, price=Decimal(str(price))
            )
            ...
            if modified_params:
                ...
                self.risk_modified_orders[order_id] = {
                    "original_quantity": float(
                        Decimal(str(quantity)) / Decimal(str(modified_params.get("quantity", 1)))
                    ),
            ...
            if submitted_order_id:
                ...
                self.risk_decisions[submitted_order_id] = self.risk_decisions.pop(
                    order_id, risk_decision
                )
    ...
    async def modify_order_with_risk_check(
        self,
        order_id: str,
        ...
        try:
            ...
            if quantity and quantity != current_order.quantity:
                ...
                price = (
                    limit_price
                    or current_order.limit_price
                    or stop_price
                    or current_order.stop_price
                )
                if not price:
                    logger.error(
                        "Cannot determine price for risk calculation in order modification"
                    )
                    return False
            ...
            success = await self.base_order_manager.modify_order(
                order_id=order_id, quantity=quantity, limit_price=limit_price, stop_price=stop_price
            )
    ...
    def _get_order_price_for_risk_calc(
        self,
        symbol: str,
    ...
    def get_risk_decision(self, order_id: str) -> RiskDecision | None:
    ...
    def get_risk_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "block_rate_pct": (
                (self.risk_blocked_count / self.total_order_requests * 100)
                if self.total_order_requests > 0
                else 0
            ),
            "modification_rate_pct": (
                (self.risk_modified_count / self.total_order_requests * 100)
                if self.total_order_requests > 0
                else 0
            ),
    ...
    def get_risk_audit_trail(self, order_id: str | None = None) -> list[dict[str, Any]]:
    ...
    async def get_order_summary(self) -> dict[str, Any]:
        ...
        return {**base_summary, "risk_integration": risk_stats}
...
def create_risk_integrated_order_manager(
    ...
    """
    # Create risk monitor
    risk_monitor = LiveRiskMonitor(position_manager=position_manager, risk_profile=risk_profile)
