...
class FastExecutionPath:
    ...
    def __init__(self, broker: BrokerInterface, config: dict[str, Any]):
        ...
        self.max_position_size = config.get("execution", {}).get(
            "fast_path_max_position_size", 10000
        )
    ...
    def _get_order_from_pool(self) -> Order | None:
    ...
    async def execute_fast(
        ...
        try:
            ...
            if spread_bps < 5:  # Tight spread - use market order
                ...
                logger.debug(
                    f"Using LIMIT order for {symbol} @ {order.limit_price} (spread: {spread_bps:.1f} bps)"
                )
            ...
            logger.info(
                f"Order submitted in {signal_to_order_ms:.1f}ms: {symbol} {side.value} {quantity}"
            )
            ...
            if filled_order and filled_order.status == OrderStatus.FILLED:
                metrics = FastExecutionMetrics(
                    ...
                    order_to_fill_ms=(filled_order.filled_at.timestamp() - order_submit_time)
                    * 1000,
                    total_execution_ms=total_ms,
                    slippage_bps=self._calculate_slippage_bps(
                        current_price, filled_order.avg_fill_price
                    ),
    ...
    async def _monitor_order_aggressive(self, order: Order, intended_price: float) -> Order | None:
        """
        ...
        while attempts < max_attempts:
            try:
                ...
                if order.filled_qty > 0 and order.avg_fill_price:
                    slippage_bps = self._calculate_slippage_bps(
                        intended_price, order.avg_fill_price
                    )
                    if slippage_bps > self.max_slippage_bps:
                        logger.warning(
                            f"Excessive slippage {slippage_bps:.1f} bps, cancelling order"
                        )
                ...
                if (
                    order.order_type == OrderType.LIMIT
                    and attempts > 10  # After 1 second
                    ...
                    if market_data:
                        ...
                        logger.debug(
                            f"Adjusting limit price from {order.limit_price} to {new_price}"
                        )
    ...
    def _calculate_slippage_bps(self, intended_price: float, execution_price: float) -> float:
    ...
    def get_execution_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_signal_to_order_ms": sum(m.signal_to_order_ms for m in metrics_list)
            / len(metrics_list),
            "avg_order_to_fill_ms": sum(m.order_to_fill_ms for m in metrics_list)
            / len(metrics_list),
            "avg_total_execution_ms": sum(m.total_execution_ms for m in metrics_list)
            / len(metrics_list),
