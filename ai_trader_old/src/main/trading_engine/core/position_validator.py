...
# Local imports
from main.models.common import AccountInfo, Order, OrderSide, Position, PositionSide
...
class PositionValidator:
    ...
    def __init__(
        self, position_limits: PositionLimits, metrics_collector: MetricsCollector | None = None
    ):
        """
    ...
    def validate_new_position(
        ...
        if quantity > self.limits.max_position_size:
            errors.append(
                f"Position size {quantity} exceeds maximum {self.limits.max_position_size}"
            )
        ...
        if position_value > self.limits.max_position_value:
            errors.append(
                f"Position value {position_value} exceeds maximum {self.limits.max_position_value}"
            )
        ...
        if position_value > context.account_info.buying_power:
            errors.append(
                f"Insufficient buying power: need {position_value}, have {context.account_info.buying_power}"
            )
        ...
        if portfolio_value > 0:
            if concentration_pct > self.limits.max_concentration_pct * Decimal("0.8"):
                warnings.append(
                    f"Position concentration {concentration_pct:.1f}% approaching limit"
                )
    ...
    def validate_position_modification(
        self, position: Position, new_quantity: Decimal, context: ValidationContext
    ) -> ValidationResult:
        """
        ...
        if new_quantity > 0 and new_quantity < self.limits.min_position_size:
            errors.append(
                f"Modified position size {new_quantity} below minimum {self.limits.min_position_size}"
            )
    ...
    def validate_order(self, order: Order, context: ValidationContext) -> ValidationResult:
        """
        ...
        existing_position = next(
            (p for p in context.existing_positions if p.symbol == order.symbol), None
        )
            ...
            if (existing_position.side == PositionSide.LONG and order.side == OrderSide.SELL) or (
                ...
                if remaining_qty < 0:
                    ...
                    result = self.validate_new_position(
                        ...
                        new_position_qty,
                        order.limit_price
                        or existing_position.current_price
                        or existing_position.avg_entry_price,
                        context,
                    )
                    errors.extend(result.errors)
                    warnings.extend(result.warnings)
                ...
                result = self.validate_position_modification(
                    existing_position, new_quantity, context
                )
                errors.extend(result.errors)
                warnings.extend(result.warnings)
        else:
        ...
        duplicate_orders = [
            o for o in context.pending_orders if o.symbol == order.symbol and o.side == order.side
        ]
        if duplicate_orders:
            warnings.append(
                f"Found {len(duplicate_orders)} pending orders for {order.symbol} {order.side.value}"
            )
    ...
    def validate_portfolio(self, context: ValidationContext) -> ValidationResult:
        ...
        if len(context.existing_positions) > self.limits.max_total_positions:
            errors.append(
                f"Total positions {len(context.existing_positions)} exceeds limit {self.limits.max_total_positions}"
            )
        ...
        if leverage > self.limits.max_leverage:
            errors.append(
                f"Portfolio leverage {leverage:.2f}x exceeds maximum {self.limits.max_leverage}x"
            )
    ...
    def _calculate_leverage(self, context: ValidationContext, additional_value: Decimal) -> Decimal:
        ...
        total_position_value = (
            sum(p.market_value for p in context.existing_positions) + additional_value
        )
    ...
    def _track_validation(self, validation_type: str, symbol: str, is_valid: bool) -> None:
        ...
        self.event_tracker.track(
            "validation", {"type": validation_type, "symbol": symbol, "is_valid": is_valid}
        )

        if self.metrics:
            self.metrics.increment(
                "position_validator.validation",
                tags={"type": validation_type, "result": "valid" if is_valid else "invalid"},
            )
