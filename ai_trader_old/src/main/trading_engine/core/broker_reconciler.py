...
class ReconciliationResult:
    ...
    def success_rate(self) -> float:
        ...
        if self.total_positions_checked == 0:
            return 100.0
        return (
            (self.total_positions_checked - self.discrepancies_found) / self.total_positions_checked
        ) * 100
...
class BrokerReconciler:
    ...
    async def reconcile_positions(self, auto_correct: bool = False) -> ReconciliationResult:
        ...
        try:
            ...
            if discrepancies:
                logger.warning(
                    f"Reconciliation found {len(discrepancies)} discrepancies, {auto_corrected} auto-corrected"
                )
                for disc in discrepancies:
                    logger.warning(
                        f"  {disc.symbol}: {disc.discrepancy_type} ({disc.severity.value})"
                    )
            else:
                logger.info(
                    "Position reconciliation completed successfully - no discrepancies found"
                )
    ...
    def _analyze_discrepancy(
        self, local_pos: Position | None, broker_pos: Position | None
        ...
        if local_pos and broker_pos:
            ...
            value_diff = abs(
                Decimal(str(local_pos.market_value)) - Decimal(str(broker_pos.market_value))
            )
            ...
            if qty_diff > self.quantity_tolerance or value_diff > self.value_tolerance:
                ...
                return PositionDiscrepancy(
                    ...
                    broker_position=broker_pos,
                    quantity_difference=Decimal(str(broker_pos.quantity))
                    - Decimal(str(local_pos.quantity)),
                    value_difference=Decimal(str(broker_pos.market_value))
                    - Decimal(str(local_pos.market_value)),
                    severity=severity,
    ...
    def _assess_discrepancy_severity(
        self, qty_diff: Decimal, value_diff: Decimal
    ) -> tuple[DiscrepancySeverity, bool]:
    ...
    def get_discrepancy_summary(self) -> dict[str, any]:
        ...
        for severity in DiscrepancySeverity:
            by_severity[severity.value] = sum(
                1 for d in recent_discrepancies if d.severity == severity
            )
