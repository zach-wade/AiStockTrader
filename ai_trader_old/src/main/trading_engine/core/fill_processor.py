...
# Local imports
from main.models.common import Order, OrderSide, Position
...
class FillProcessor:
    ...
    async def process_fill(
        ...
        try:
            ...
            logger.info(
                f"Processed fill for {symbol}: {fill_quantity} @ {fill_price}, P&L: {realized_pnl}"
            )
        ...
        try:
            ...
            if not current_position:
                ...
                new_cost_basis = old_cost_basis + (
                    abs(signed_fill_quantity) * fill_price + commission + fees
                )
                ...
                unrealized_pnl_pct = (
                    ((current_price / new_avg_entry_price) - 1) * 100
