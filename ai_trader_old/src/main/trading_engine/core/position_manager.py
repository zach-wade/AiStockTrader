        ...
        async with position_lock:
            ...
            try:
                await self.cache.set(
                    CacheType.POSITIONS, f"position:{symbol}", position, self.cache_ttl
                )
            except Exception as e:
                logger.debug(f"Cache update failed for {symbol}: {e}")

            logger.debug(
                f"Updated position for {symbol}: {position.quantity} @ {position.avg_entry_price}"
            )
...
class PositionManager:
        ...
        try:
            ...
            if current_position:
                ...
                if self.portfolio_manager:
                    await self.portfolio_manager.open_position(symbol, quantity, entry_price, side)
            ...
            logger.info(
                f"Opened/updated position: {symbol} {side.value} {quantity} @ {entry_price}"
            )
            return new_position
    ...
    async def close_position(
        ...
        try:
            ...
            if remaining_quantity <= 0.001:  # Essentially zero
                ...
                new_quantity = (
                    remaining_quantity if current_position.quantity > 0 else -remaining_quantity
                )
                ...
                logger.info(
                    f"Partially closed position: {symbol} {close_quantity} @ {exit_price}, remaining: {new_quantity}"
                )
    ...
    async def _add_to_position(
        self, current_position: Position, add_quantity: float, add_price: float, side: OrderSide
    ) -> Position:
    ...
    def _calculate_realized_pnl(
        self, position: Position, close_quantity: float, exit_price: float
    ) -> Decimal:
