...
class OutcomeDataFetcher(ErrorHandlingMixin):
    ...
    @timer
    async def fetch_outcome_data(self, request: OutcomeRequest) -> List[OutcomeData]:
        """
        ...
        with self._handle_error("fetching outcome data"):
            ...
            for idx, row in price_data.iterrows():
                ...
                if outcomes:
                    # Add market context
                    context = await self._get_market_context(request.symbol, timestamp)
            ...
            record_metric(
                ...
                len(outcome_data),
                tags={"symbol": request.symbol, "horizons": len(request.outcome_horizons)},
            )
        ...
        with self._handle_error("fetching batch outcome data"):
            ...
            for i in range(0, len(requests), max_concurrent):
                ...
                batch_results = await asyncio.gather(
                    *[self.fetch_outcome_data(req) for req in batch], return_exceptions=True
                )

                # Collect results
                for request, result in zip(batch, batch_results):
                    if isinstance(result, Exception):
                        logger.error(f"Error fetching data for {request.symbol}: {result}")
    ...
    async def _calculate_outcomes(
        ...
        outcomes = {"prices": {}, "returns": {}, "directions": {}, "magnitudes": {}}
        ...
        for horizon in horizons:
            ...
            exit_price = await self._get_price_at_time(symbol, target_timestamp)
            ...
            # Classify direction
            outcomes["directions"][horizon] = self._classify_direction(price_return)

            # Classify magnitude
            outcomes["magnitudes"][horizon] = self._classify_magnitude(abs(price_return))
    ...
    async def _get_market_context(self, symbol: str, timestamp: datetime) -> Dict[str, Any]:
    ...
    def _classify_direction(self, price_return: float) -> str:
    ...
    def _classify_magnitude(self, abs_return: float) -> str:
    ...
    async def get_outcome_statistics(
        self, symbol: str, start_date: datetime, end_date: datetime, horizon: int
    ) -> Dict[str, Any]:
        """Get statistics about outcomes for a symbol and horizon."""
        request = OutcomeRequest(
            symbol=symbol, start_date=start_date, end_date=end_date, outcome_horizons=[horizon]
        )
    ...
    async def export_outcome_data_to_csv(
        self, outcome_data: List[OutcomeData], file_path: str
    ) -> None:
