...
class OutcomeMetricsCalculator(ErrorHandlingMixin):
    ...
    async def calculate_comprehensive_metrics(
        ...
        with self._handle_error("calculating comprehensive metrics"):
            ...
            classification_metrics = self._calculate_classification_metrics(y_true, y_pred, y_proba)
            ...
            # Assess data quality
            data_quality_score = self._assess_data_quality(y_true, y_pred, returns)
            ...
            result = OutcomeMetricsResult(
                ...
                or (datetime.utcnow() - timedelta(days=30), datetime.utcnow()),
    ...
    def _calculate_classification_metrics(
        self, y_true: List[str], y_pred: List[str], y_proba: Optional[List[List[float]]] = None
    ...
    def _calculate_trading_metrics(
        self, y_true: List[str], y_pred: List[str], returns: List[float]
    ) -> TradingMetrics:
        ...
        # Sharpe ratio
        sharpe_ratio = (
            (annualized_return - self.risk_free_rate) / volatility if volatility > 0 else 0.0
        )
    ...
    def _predictions_to_signals(self, y_pred: List[str]) -> List[float]:
        """Convert predictions to trading signals."""
        signals = []
    ...
    def _calculate_transaction_costs(self, signals: List[float]) -> np.ndarray:
    ...
    def _assess_data_quality(
        self, y_true: List[str], y_pred: List[str], returns: Optional[List[float]]
    ) -> float:
    ...
    def _generate_quality_notes(self, result: OutcomeMetricsResult) -> List[str]:
        ...
        if not result.statistical_tests.returns_significant:
            notes.append(
                "Returns are not statistically significant. " "Model may not have predictive power."
            )
    ...
    async def compare_models(self, results: List[OutcomeMetricsResult]) -> Dict[str, Any]:
        """Compare multiple model results."""
        if len(results) < 2:
            return {"error": "Need at least 2 models to compare"}

        comparison = {"models": [r.model_name for r in results], "metrics_comparison": {}}
        ...
        if len(results) == 2:
            ...
            comparison["statistical_comparison"] = {
                ...
                "sharpe_difference": (
                    model_b.trading_metrics.sharpe_ratio - model_a.trading_metrics.sharpe_ratio
                ),
            }

        return comparison

    def export_metrics_to_dict(self, result: OutcomeMetricsResult) -> Dict[str, Any]:
