...
class OutcomeLabeler:
    ...
    def classify_based_on_metrics(self, metrics: Dict[str, Any]) -> Tuple[OutcomeLabel, float]:
        ...
        if any(
            metrics.get(k) is None or np.isnan(metrics.get(k))
            for k in required_classification_metrics
        ):
            logger.warning(
                f"Missing or NaN critical classification metrics. Returning NO_DATA. Metrics: {metrics.keys()}"
            )
        ...
        if (
            return_3d >= thresh_sb.get("min_return_3d", 0.05)
            and max_favorable >= thresh_sb.get("min_max_favorable", 0.08)
            and momentum_score >= thresh_sb.get("min_follow_through", 0.3)
        ):

            confidence = min(
                (return_3d / thresh_sb.get("min_return_3d", 0.05) * 0.4)  # Return component
                + (
                    max_favorable / thresh_sb.get("min_max_favorable", 0.08) * 0.3
                )  # Peak move component
                + (momentum_score * 0.3),  # Consistency component
                1.0,
            )
            logger.debug(
                f"Classified as SUCCESSFUL_BREAKOUT for return_3d={return_3d:.2f}, max_fav={max_favorable:.2f}, momentum={momentum_score:.2f}"
            )
        ...
        if return_3d <= thresh_fb.get("max_return_3d", -0.03) and max_adverse <= thresh_fb.get(
            ...
            logger.debug(
                f"Classified as FAILED_BREAKDOWN for return_3d={return_3d:.2f}, max_adv={max_adverse:.2f}"
            )
        ...
        if (
            reversal_strength > thresh_rev.get("min_reversal_strength", 0.4)
            and max_favorable > thresh_rev.get("min_initial_move", 0.03)  # Initial upward move
            and abs(return_3d) <= thresh_rev.get("max_final_return", 0.01)
        ):  # Final close to start

            confidence = min(
                reversal_strength * 0.7
                + (max_favorable / thresh_rev.get("min_initial_move", 0.03)) * 0.3,
                1.0,
            )
            logger.debug(
                f"Classified as REVERSAL_PATTERN for reversal={reversal_strength:.2f}, max_fav={max_favorable:.2f}"
            )
        ...
        if (
            thresh_mg.get("min_return_3d", 0.02)
            <= return_3d
        ...
        if (
            abs(return_3d) <= thresh_sf.get("max_abs_return_3d", 0.02)
            and volatility <= thresh_sf.get("max_volatility", 0.03)  # Volatility must be low
            and max_favorable
            <= thresh_sf.get("max_max_move", 0.04)  # Overall range must be limited
            and abs(max_adverse) <= thresh_sf.get("max_max_move", 0.04)
        ):

            confidence = 0.8  # High confidence for clear sideways action
            logger.debug(
                f"Classified as SIDEWAYS_FIZZLE for return_3d={return_3d:.2f}, volatility={volatility:.2f}"
            )
