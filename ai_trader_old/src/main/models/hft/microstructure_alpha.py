...
class MicrostructureAlphaStrategy(BaseHFTStrategy):
    ...
    def __init__(self, config: Dict, strategy_specific_config: Dict):
        ...
        self.holding_periods = self.params.get(
            "holding_periods", {"imbalance": 30, "sweep": 10, "iceberg": 60, "spread": 20}
        )
        ...
        self.order_books: Dict[str, Deque[OrderBookSnapshot]] = defaultdict(
    ...
    async def _check_order_imbalance(
        self, symbol: str, snapshots: List[OrderBookSnapshot]
    ) -> Optional[MicrostructureSignal]:
        current = snapshots[-1]
        weighted_imbalance = self._calculate_weighted_imbalance(current)

        if abs(weighted_imbalance) > self.min_imbalance and self._check_persistent_imbalance(
            snapshots[-30:]
        ):
            return MicrostructureSignal(
                symbol=symbol,
                action="BUY" if weighted_imbalance > 0 else "SELL",
                ...
                time_horizon=self.holding_periods["imbalance"],
    ...
    def _calculate_weighted_imbalance(self, snapshot: OrderBookSnapshot) -> float:
        ...
        return (
            (weighted_bid_volume - weighted_ask_volume) / total_volume if total_volume > 0 else 0.0
        )

    def _check_persistent_imbalance(
        self, snapshots: List[OrderBookSnapshot], threshold: float = 0.5
    ) -> bool:
        if len(snapshots) < 10:
            return False
    ...
    async def _check_sweep_detection(
        self, symbol: str, snapshots: List[OrderBookSnapshot]
    ) -> Optional[MicrostructureSignal]:
        if len(snapshots) < 2:
            return None
        ...
        bid_sweep = (
            previous.bid_depth > 0
            and (previous.bid_depth - current.bid_depth) / previous.bid_depth > 0.5
        )
        ask_sweep = (
            previous.ask_depth > 0
            and (previous.ask_depth - current.ask_depth) / previous.ask_depth > 0.5
        )
        ...
        if (bid_sweep or ask_sweep) and price_move > 0.0005:
            ...
            if len(recent_trades) > 10:
                ...
                if any(
                    t["size"] > avg_size * self.sweep_size_multiplier for t in recent_trades[-10:]
                ):
                    return MicrostructureSignal(
                        symbol=symbol,
                        action="SELL" if bid_sweep else "BUY",
                        signal_type="sweep",
                        confidence=0.8,
                        expected_move=price_move * 10000,
                        time_horizon=self.holding_periods["sweep"],
                        entry_price=current.mid_price,
                        metadata={"price_impact_bps": price_move * 10000},
                    )
        return None

    async def _check_iceberg_orders(
        self, symbol: str, snapshots: List[OrderBookSnapshot]
    ) -> Optional[MicrostructureSignal]:
        ...
        for snapshot in snapshots[-50:]:
            for price, size in snapshot.bids[:3]:
                price_levels[("bid", price)].append(size)
            for price, size in snapshot.asks[:3]:
                price_levels[("ask", price)].append(size)

        for (side, price), sizes in price_levels.items():
            if len(sizes) > 10:
                changes = np.diff(sizes)
                reloads = sum(
                    1 for i in range(len(changes) - 1) if changes[i] < 0 and changes[i + 1] > 0
                )
                if reloads > len(changes) * 0.3:
                    return MicrostructureSignal(
                        ...
                        confidence=0.75,
                        expected_move=10,
                        time_horizon=self.holding_periods["iceberg"],
                        entry_price=snapshots[-1].mid_price,
                        metadata={"iceberg_price": price},
                    )
        return None

    async def _check_spread_dynamics(
        self, symbol: str, snapshots: List[OrderBookSnapshot]
    ) -> Optional[MicrostructureSignal]:
        # This logic for liquidity provision is preserved as is.
        if len(snapshots) < 20 or not snapshots[-1].bids or not snapshots[-1].asks:
            return None
        current = snapshots[-1]
        spread_bps = (current.asks[0][0] - current.bids[0][0]) / current.mid_price * 10000
        historical_spreads = [
            s.spread / s.mid_price * 10000 for s in snapshots[-20:] if s.mid_price > 0
        ]
        ...
        if spread_std > 0 and spread_bps > avg_spread + 2 * spread_std:
            return MicrostructureSignal(
                ...
                confidence=0.6,
                expected_move=spread_bps / 2,
                time_horizon=self.holding_periods["spread"],
                entry_price=current.mid_price,
                metadata={"spread_zscore": (spread_bps - avg_spread) / spread_std},
            )
        return None
