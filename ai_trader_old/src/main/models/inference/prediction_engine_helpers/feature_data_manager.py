    ...
    async def get_features_for_prediction(
        ...
        if use_cache and self.use_cache:
            ...
            try:
                ...
                logger.error(
                    f"Error retrieving features from cache for {cache_key}: {e}", exc_info=True
                )
        ...
        try:
            ...
            if features_df is None or features_df.empty:
                logger.warning(
                    f"FeatureStore returned no data for {symbol} with model {model_name}."
                )
                return None
            ...
            if use_cache and self.use_cache:
                try:
                    ...
                    await self.cache.set(
                        CacheType.FEATURES, cache_key, features_dict_list, self._cache_ttl_seconds
                    )
            ...
            logger.error(
                f"Failed to get features for {symbol} from FeatureStore for model {model_name}: {e}",
                exc_info=True,
            )
            return None
