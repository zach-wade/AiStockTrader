...
class PredictionCalculator:
    ...
    def calculate_prediction(
        self, model_obj: Any, features_df: pd.DataFrame, model_metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if expected_feature_names:
            try:
                ...
                if missing_cols:
                    logger.warning(
                        f"Missing {len(missing_cols)} features for prediction. Filling with 0.0: {missing_cols}"
                    )
                    # Create a new DataFrame to ensure correct order and fill missing
                    features_df = features_df.reindex(
                        columns=expected_feature_names, fill_value=0.0
                    )
                ...
                prediction_result["error"] = (
                    f"Feature mismatch: {e}. Check expected_feature_names in model metadata."
                )
                logger.error(
                    f"Feature mismatch: {e} during reordering for prediction.", exc_info=True
                )
                return prediction_result
        ...
        try:
            ...
            prediction = model_obj.predict(features_df)[
                0
            ]  # Assuming single prediction for single row DF
            prediction_result["prediction"] = int(prediction)  # Ensure integer output

            # Get probability if model supports it
            if hasattr(model_obj, "predict_proba"):
                probabilities = model_obj.predict_proba(features_df)[
                    0
                ]  # Assuming probabilities for single row
                if len(probabilities) == 2:  # Binary classification
                    probability = probabilities[1]  # Probability of the positive class
                elif prediction_result["prediction"] is not None and prediction_result[
                    "prediction"
                ] < len(probabilities):
                ...
                prediction_result["confidence"] = (
                    abs(probability - 0.5) * 2 if len(probabilities) == 2 else float(probability)
                )
            else:
