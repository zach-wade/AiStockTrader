...
class RequestValidator:
        ...
        for feature, expected_type in self.feature_types.items():
            ...
            # Check string types
                ...
                if not pd.api.types.is_object_dtype(actual_type):
                    warnings.append(
                        f"Feature '{feature}' should be string/object, got {actual_type}"
                    )
    ...
    def _validate_security(self, df: pd.DataFrame, errors: List[str], warnings: List[str]):
        ...
        for col in numeric_cols:
            ...
            if max_abs > 1e10:  # Arbitrary large number
                warnings.append(
                    f"Feature '{col}' has suspiciously large values (max abs: {max_abs:.2e})"
                )
        ...
        for col in string_cols:
            for pattern in dangerous_patterns:
                if df[col].astype(str).str.contains(pattern, case=False, na=False).any():
                    errors.append(
                        f"Potential injection detected in feature '{col}' (pattern: {pattern})"
                    )
    ...
    def _validate_rate_limits(
        ...
        self._request_history = [
            t for t in self._request_history[-self._request_history_limit :] if t > cutoff
        ]

        # Check rate limits
        requests_per_minute = sum(
            1 for t in self._request_history if t > now - timedelta(minutes=1)
        )
        ...
        if requests_per_hour > hour_limit:
            errors.append(
                f"Rate limit exceeded: {requests_per_hour} requests/hour (limit: {hour_limit})"
            )
...
def create_strict_validator(required_features: List[str], **kwargs) -> RequestValidator:
    ...
    return RequestValidator(
        required_features=required_features, validation_level=ValidationLevel.STRICT, **kwargs
    )


def create_lenient_validator(
    required_features: Optional[List[str]] = None, **kwargs
) -> RequestValidator:
    """Create a lenient validator."""
    return RequestValidator(
        required_features=required_features, validation_level=ValidationLevel.LENIENT, **kwargs
    )
