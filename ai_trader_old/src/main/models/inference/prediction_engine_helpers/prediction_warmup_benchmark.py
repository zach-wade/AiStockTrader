...
class PredictionWarmupBenchmark:
    ...
    def __init__(
        self,
        get_features_func: Callable[
            [str, str, List[str], int, bool], Awaitable[Optional[pd.DataFrame]]
        ],
    ...
    async def warmup_engine(
        ...
        logger.info(
            f"Warming up prediction engine for {len(symbols)} symbols and {len(model_names)} models..."
        )
        ...
        for model_name in model_names:
            ...
            required_features = required_feature_names_map.get(
                model_name, ["close", "volume"]
            )  # Fallback features
        ...
        if warmup_tasks:
            ...
            for i, res in enumerate(results):
                if isinstance(res, Exception):
                    logger.warning(
                        f"Warmup task {i} failed: {res}", exc_info=False
                    )  # Log errors but don't stop warmup
    ...
    async def benchmark_engine(
        ...
        if not model_name:
            logger.warning(
                "No specific model name provided for benchmark. Attempting to use a default loaded model."
            )
        ...
        successful_predictions = [
            r for r in results if not isinstance(r, Exception) and r.get("prediction") is not None
        ]

        # Extract latencies from individual prediction results
        prediction_times = [
            r["prediction_time"] for r in successful_predictions if "prediction_time" in r
        ]
        ...
        benchmark_results = {
            ...
            "avg_time_per_prediction_seconds": (
                total_time / n_predictions if n_predictions > 0 else 0.0
            ),
        ...
        logger.info(
            f"Benchmark complete. Avg time: {benchmark_results['avg_time_per_prediction_seconds']:.4f}s/pred."
        )
