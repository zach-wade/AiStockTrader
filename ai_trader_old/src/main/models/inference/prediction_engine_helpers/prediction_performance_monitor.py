...
class PredictionPerformanceMonitor:
    ...
    def __init__(self, max_history_size: int = 1000):
        ...
        """
        self.prediction_times: deque[float] = deque(
            maxlen=max_history_size
        )  # Stores prediction latencies in seconds
        self.feature_times: deque[float] = deque(
            maxlen=max_history_size
        )  # Stores feature retrieval latencies in seconds
        logger.debug(
            f"PredictionPerformanceMonitor initialized with max_history_size: {max_history_size}"
        )
    ...
    def get_performance_stats(
        self, current_cache_size: int = 0, models_loaded_count: int = 0
    ) -> Dict[str, Any]:
        """
