    ...
    def load_model_object(self, model_name: str) -> Tuple[Any, Dict[str, Any]]:
        ...
        if not model_path:
            raise FileNotFoundError(
                f"Model file not found for '{model_name}' in {self.models_base_dir}"
            )
        ...
        try:
            ...
            if isinstance(model_data, dict):
                ...
                metadata = {}
                logger.warning(
                    f"Model '{model_name}' loaded in legacy format. No explicit metadata found."
                )

            if model_obj is None:
                raise ValueError(
                    f"Model object is None after loading from {model_path}. File might be corrupted."
                )
            ...
            logger.error(
                f"Failed to load model '{model_name}' from {model_path}: {e}", exc_info=True
            )
            raise IOError(f"Could not load model file for '{model_name}': {e}")

    def update_model_object_in_cache(
        self, model_name: str, model_obj: Any, metadata: Optional[Dict[str, Any]] = None
    ):
        """
        ...
        try:
            model_data_to_save = {"model": model_obj, "metadata": self._model_metadata[model_name]}
            ...
            logger.error(
                f"Failed to save updated model '{model_name}' to disk at {model_path}: {e}",
                exc_info=True,
            )
