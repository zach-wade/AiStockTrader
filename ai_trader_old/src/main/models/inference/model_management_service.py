...
class ModelManagementService:
    ...
    async def deploy_model(self, model_id: str, version: str, deployment_pct: float = 0.0) -> bool:
        ...
        if not version_obj:
            logger.error(
                f"Model {model_id} version {version} not found in registry. Cannot deploy."
            )
            return False
        ...
        if success:
            ...
            if version_obj.deployment_pct > 0:
                ...
                model_id in self.model_registry.active_deployments
                and self.model_registry.active_deployments[model_id].version == version
            ):
    ...
    async def archive_old_models(self, days: int = 30) -> int:
        ...
        all_versions_flat: List[ModelVersion] = [
            v for sublist in self.model_registry.versions.values() for v in sublist
        ]
    ...
    def update_model_performance(
        self, model_id: str, version: str, performance_data: Dict[str, float]
    ):
        """
        ...
        if not version_obj:
            logger.warning(
                f"Model {model_id} version {version} not found for performance update. Skipping."
            )
            return
