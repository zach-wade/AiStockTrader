...
class ModelVersionManager(ErrorHandlingMixin):
        ...
        with self._handle_error("creating model version"):
            async with self.db_pool.acquire() as conn:
                ...
                version_id = await conn.fetchval(
                    query, model_name, version, json.dumps(metadata), datetime.utcnow(), "created"
                )
    ...
    async def promote_version(
        ...
        with self._handle_error("promoting model version"):
            async with self.db_pool.acquire() as conn:
                ...
                await conn.execute(query, datetime.utcnow(), environment, model_name, version)
                ...
                await conn.execute(deactivate_query, model_name, environment, version)

                record_metric(
                    "model_registry.version_promoted",
                    1,
                    tags={"model": model_name, "version": version, "environment": environment},
                )

                logger.info(f"Promoted {model_name} v{version} to {environment}")
    ...
    async def rollback_version(
        ...
        with self._handle_error("rolling back model version"):
            ...
            success = await self.promote_version(model_name, target_version, environment)
            ...
            if success:
                ...
                logger.info(
                    f"Rolled back {model_name} from v{current.version} " f"to v{target_version}"
                )
    ...
    async def compare_versions(
        self, model_name: str, version_a: str, version_b: str, metric_names: List[str]
    ) -> ModelComparison:
        """Compare two model versions."""
        with self._handle_error("comparing model versions"):
            # Get metrics for both versions
            metrics_a = await self._get_version_metrics(model_name, version_a, metric_names)
            metrics_b = await self._get_version_metrics(model_name, version_b, metric_names)
            ...
            for metric in metric_names:
                if metric in metrics_a and metric in metrics_b:
                    comparisons[metric] = {
                        ...
                        "improvement_pct": (
                            (metrics_b[metric] - metrics_a[metric]) / metrics_a[metric] * 100
            ...
            # Determine winner
            improvements = sum(1 for m in comparisons.values() if m["difference"] > 0)
    ...
    async def _get_version_metrics(
        self, model_name: str, version: str, metric_names: List[str]
    ) -> Dict[str, float]:
...
class ModelMetricsTracker(ErrorHandlingMixin):
    ...
    async def track_metric(
        ...
        if (
            len(self._metric_buffer) >= self._buffer_size
            or (datetime.utcnow() - self._last_flush).seconds > 60
        ):
    ...
    async def get_metric_history(
        self, model_name: str, version: str, metric_name: str, hours: int = 24
        ...
        async with self.db_pool.acquire() as conn:
            ...
            rows = await conn.fetch(query, model_name, version, metric_name, cutoff)

            return [(row["timestamp"], row["metric_value"]) for row in rows]

    async def get_metric_summary(
        self, model_name: str, version: str, hours: int = 24
...
class ModelRegistryEnhancements(ErrorHandlingMixin):
    ...
    async def route_request(self, test_name: str, request_id: str) -> str:
    ...
    async def analyze_ab_test(self, test_name: str) -> Dict[str, Any]:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            rows = await conn.fetch(query, test_name, config.success_metrics)
        ...
        if len(results) == 2:
            ...
            if (
                model_a_stats.get("request_count", 0) > 30
                and model_b_stats.get("request_count", 0) > 30
            ):

                # Calculate test statistic
                mean_diff = model_b_stats.get("avg_metric", 0) - model_a_stats.get("avg_metric", 0)

                pooled_std = (
                    model_a_stats.get("std_metric", 1) ** 2 / model_a_stats.get("request_count", 1)
                    + model_b_stats.get("std_metric", 1) ** 2
                    / model_b_stats.get("request_count", 1)
                ) ** 0.5
        ...
        return {
            ...
            "p_value": p_value,
            "duration_hours": (
                datetime.utcnow() - self._get_test_start_time(test_name)
            ).total_seconds()
            / 3600,
        }
