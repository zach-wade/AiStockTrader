...
class ModelArchiver:
    ...
    def archive_old_model_versions(
        self, all_versions: List[ModelVersion], days_to_keep: int
    ) -> int:
        """
        ...
        for version in all_versions:
            ...
            if (
                ...
                and version.status not in ["production", "candidate"]
                ...
                archived_count += 1
                logger.info(
                    f"Archived model '{version.model_id}' version '{version.version}' (created: {version.created_at.isoformat()})."
                )
