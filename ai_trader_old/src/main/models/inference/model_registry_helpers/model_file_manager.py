...
class ModelFileManager:
    ...
    def save_model(self, model_obj: Any, version_info: ModelVersion) -> Path:
        ...
        try:
            ...
            logger.info(
                f"Saved model '{version_info.model_id}' version '{version_info.version}' to {model_path}"
            )
            return model_path
        except Exception as e:
            logger.error(
                f"Failed to save model {version_info.model_id} v{version_info.version} to {model_path}: {e}",
                exc_info=True,
            )
    ...
    def load_model(self, model_id: str, version: str) -> Tuple[Any, List[str], Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to load model {model_id} v{version} from {model_path}: {e}", exc_info=True
            )
