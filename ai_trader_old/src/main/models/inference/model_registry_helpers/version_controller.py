...
# Local imports
from main.models.inference.model_registry_types import DeploymentStatus, ModelVersion
...
class SemanticVersion:
    ...
    def __init__(self, version: str):
        ...
        self.major, self.minor, self.patch, self.prerelease, self.build = self._parse_version(
            version
        )
    ...
    def increment(
        self, version_type: VersionType, prerelease: Optional[str] = None
    ) -> "SemanticVersion":
...
class VersionController(ErrorHandlingMixin):
    ...
    async def create_version(
        ...
        with self._handle_error("creating model version"):
            ...
            # Store in database
            version_id = await self._store_version(model_version, dependencies)
            ...
            record_metric(
                ...
                1,
                tags={"model": model_name, "version": new_version, "type": version_type.value},
            )
    ...
    async def promote_version(
        ...
        with self._handle_error("promoting version"):
            ...
            if not self._is_valid_promotion(current_status, target_status):
                raise ValueError(
                    f"Invalid promotion from {current_status.value} " f"to {target_status.value}"
                )

            # Check dependencies if promoting to production
            if target_status == VersionStatus.PRODUCTION:
                await self._validate_production_requirements(model_name, version)

            # Update version status
            await self._update_version_status(model_name, version, target_status)

            # Update lifecycle
            await self._update_lifecycle(model_name, version, target_status, validation_results)
    ...
    async def rollback_version(
        self, model_name: str, target_version: str, reason: str = "manual_rollback"
    ) -> bool:
        """
        ...
        with self._handle_error("rolling back version"):
            ...
            if target.status not in [VersionStatus.PRODUCTION.value, VersionStatus.STAGED.value]:
                raise ValueError(f"Cannot rollback to version with status {target.status}")
            ...
            # Check compatibility
            is_compatible = await self._check_rollback_compatibility(current, target)
            ...
            await self._update_version_status(model_name, current.version, VersionStatus.STAGED)

            # 2. Promote target version
            await self._update_version_status(model_name, target_version, VersionStatus.PRODUCTION)

            # 3. Record rollback event
            await self._record_rollback(model_name, current.version, target_version, reason)
    ...
    async def get_version(self, model_name: str, version: str) -> Optional[ModelVersion]:
    ...
    async def get_latest_version(
        self, model_name: str, status: Optional[VersionStatus] = None
        ...
        version_pairs = [(SemanticVersion(v.version), v) for v in versions]
    ...
    async def get_production_version(self, model_name: str) -> Optional[ModelVersion]:
        ...
        return await self.get_latest_version(model_name, VersionStatus.PRODUCTION)

    async def list_versions(
        self, model_name: str, limit: Optional[int] = None
    ) -> List[ModelVersion]:
    ...
    async def check_compatibility(
        ...
        breaking_changes.extend(ver_b.metadata.get("breaking_changes", []))

        deprecation_warnings = ver_b.metadata.get("deprecation_warnings", [])
    ...
    def _is_valid_promotion(self, current: VersionStatus, target: VersionStatus) -> bool:
    ...
    async def _validate_dependencies(self, dependencies: List[VersionDependency]) -> None:
        ...
        for dep in dependencies:
            if dep.required:
                ...
                if not versions:
                    raise ValueError(f"Required dependency {dep.model_name} not found")
    ...
    async def _validate_production_requirements(self, model_name: str, version: str) -> None:
        ...
        if not lifecycle or lifecycle.status not in [VersionStatus.STAGED, VersionStatus.TESTING]:
            raise ValueError(
                f"Version {version} must be staged or tested " f"before production promotion"
            )
    ...
    async def _store_version(
        self, version: ModelVersion, dependencies: Optional[List[VersionDependency]] = None
    ) -> int:
    ...
    async def _initialize_lifecycle(self, model_name: str, version: str) -> None:
    ...
    async def _update_version_status(
        self, model_name: str, version: str, status: VersionStatus
    ) -> None:
        """Update version status."""
        async with self.db_pool.acquire() as conn:
            await conn.execute(
    ...
    async def _get_lifecycle(self, model_name: str, version: str) -> Optional[VersionLifecycle]:
    ...
    async def _handle_deprecation(self, model_name: str, version: str) -> None:
    ...
    async def _handle_archival(self, model_name: str, version: str) -> None:
    ...
    async def _check_rollback_compatibility(
        self, current: ModelVersion, target: ModelVersion
    ) -> bool:
    ...
    def _clear_version_cache(self, model_name: str) -> None:
        ...
        to_remove = [
            key for key in self._lifecycle_cache.keys() if key.startswith(f"{model_name}:")
        ]
