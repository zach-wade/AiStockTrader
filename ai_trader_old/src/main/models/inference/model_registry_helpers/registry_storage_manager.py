...
# Local imports
from main.utils.core import ErrorHandlingMixin, get_logger, run_in_executor, timer
...
class RegistryStorageManager(ErrorHandlingMixin):
    ...
    def __init__(self, db_pool: DatabasePool, storage_path: str, max_storage_gb: float = 100.0):
        ...
        with self._handle_error("storing model artifact"):
            ...
            if existing:
                ...
                artifact = await self._store_new_artifact(
                    model_name, version, artifact_type, data, checksum, metadata, compress
                )
    ...
    async def retrieve_model_artifact(
        self, model_name: str, version: str, artifact_type: str
    ) -> Optional[bytes]:
        """
        ...
        with self._handle_error("retrieving model artifact"):
            ...
            artifact = await self._get_artifact_metadata(model_name, version, artifact_type)

            if not artifact:
                logger.warning(f"Artifact not found: {model_name} v{version} {artifact_type}")
            ...
            record_metric(
                ...
                1,
                tags={"model": model_name, "version": version, "type": artifact_type},
            )

            return data

    async def delete_model_artifacts(self, model_name: str, version: Optional[str] = None) -> int:
        """
        ...
        with self._handle_error("deleting model artifacts"):
            ...
            artifacts = await self._list_artifacts(model_name, version)
            ...
            for artifact in artifacts:
                ...
                refs = await self._count_artifact_references(artifact.checksum)
            ...
            record_metric(
                ...
                deleted_count,
                tags={"model": model_name, "bytes_freed": freed_bytes},
            )
    ...
    async def get_storage_stats(self) -> StorageStats:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            if compression_row and compression_row["original_size"]:
                compression_ratio = (
                    compression_row["compressed_size"] / compression_row["original_size"]
                )
            else:
                compression_ratio = 1.0

            # Calculate deduplication ratio
            dedup_ratio = self._storage_metrics["dedup_savings_bytes"] / max(
                1, row["total_size"] + self._storage_metrics["dedup_savings_bytes"]
            )
    ...
    async def cleanup_old_artifacts(
        self, retention_days: int = 90, keep_latest_versions: int = 3
    ) -> int:
        """
        ...
        with self._handle_error("cleaning up old artifacts"):
            ...
            async with self.db_pool.acquire() as conn:
                ...
                rows = await conn.fetch(query, keep_latest_versions, cutoff_date)

                # Delete artifacts
                deleted_count = 0
                for row in rows:
                    await self.delete_model_artifacts(row["model_name"], row["version"])
                    deleted_count += 1

            logger.info(
                f"Cleaned up {deleted_count} old artifacts " f"(retention: {retention_days} days)"
            )
            ...
            for artifact in uncompressed:
                try:
                    ...
                    data = await self.retrieve_model_artifact(
                        artifact.model_name, artifact.version, artifact.artifact_type
                    )
                    ...
                    if data:
                        ...
                        bytes_saved = artifact.file_size_bytes - new_artifact.file_size_bytes

                        results["compressed_count"] += 1
                        results["bytes_saved"] += bytes_saved

                except Exception as e:
                    results["errors"].append({"artifact_id": artifact.artifact_id, "error": str(e)})
    ...
    async def _find_artifact_by_checksum(self, checksum: str) -> Optional[ModelArtifact]:
    ...
    async def _count_artifact_references(self, checksum: str) -> int:
    ...
    async def _delete_artifact_record(self, artifact_id: str) -> None:
        """Delete artifact record."""
        async with self.db_pool.acquire() as conn:
            await conn.execute("DELETE FROM model_artifacts WHERE artifact_id = $1", artifact_id)
