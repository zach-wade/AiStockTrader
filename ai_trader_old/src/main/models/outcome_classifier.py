...
from main.models.outcome_classifier_helpers.outcome_metrics_calculator import (
...
class OutcomeClassifier:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.thresholds = {
            "successful_breakout": {
                ...
                "min_follow_through": 0.3,
            },
            "failed_breakdown": {
                "max_return_3d": -0.03,
                "max_max_favorable": 0.02,
                "min_adverse_move": -0.05,
            },
            "sideways_fizzle": {
                "max_abs_return_3d": 0.02,
                "max_volatility": 0.03,
                "max_max_move": 0.04,
            },
            "modest_gain": {"min_return_3d": 0.02, "max_return_3d": 0.05, "min_consistency": 0.2},
            "reversal_pattern": {
                "min_initial_move": 0.03,
                "min_reversal_strength": 0.4,
                "max_final_return": 0.01,
            },
        }

        # Evaluation periods for returns calculation
        self.evaluation_periods = self.config.get("outcome_classifier", {}).get(
            "evaluation_periods", [1, 3, 5, 7, 14]
        )
        self.primary_period = self.config.get("outcome_classifier", {}).get(
            "primary_classification_period", 3
        )
        ...
        self._metrics_calculator = OutcomeMetricsCalculator(
    ...
    async def classify_outcome(
        ...
        """
        logger.debug(
            f"Classifying outcome for {symbol} on {entry_date} (Eval days: {evaluation_days})"
        )
        ...
        try:
            ...
            if price_data is None or price_data.empty:
                logger.warning(
                    f"No sufficient price data found for {symbol} around {entry_date}. Skipping classification."
                )
            ...
            entry_price, data_quality_note = self._entry_price_determiner.determine_entry_price(
                price_data,
                entry_date,
                entry_time,  # entry_time is currently for intraday consideration
            )

            if entry_price <= 0:
                logger.warning(
                    f"Invalid entry price ({entry_price}) determined for {symbol} on {entry_date}."
                )
                return OutcomeMetrics(
                    entry_price=0.0,
            ...
            if not calculated_metrics or "calculation_error" in calculated_metrics:
                logger.warning(
                    f"Error calculating metrics for {symbol} on {entry_date}. Details: {calculated_metrics.get('calculation_error', 'unknown error')}"
                )
                return OutcomeMetrics(
            ...
            outcome_label, confidence = self._outcome_labeler.classify_based_on_metrics(
                calculated_metrics
            )
            ...
            logger.debug(
                f"{symbol} {entry_date}: Classified as {outcome_label.value} (Confidence: {confidence:.2f}, 3d Return: {final_metrics.return_3d:.2%})"
            )
            return final_metrics

        except Exception as e:
            logger.error(
                f"Error classifying outcome for {symbol} on {entry_date}: {e}", exc_info=True
            )
            return OutcomeMetrics(
                entry_price=0.0,
                outcome_label=OutcomeLabel.CALCULATION_ERROR,
    ...
    async def classify_batch(
        ...
        """
        logger.info(
            f"Classifying outcomes for {len(symbols_and_dates)} symbol-date pairs in batches."
        )
        ...
        for i in range(0, len(symbols_and_dates), batch_size):
            ...
            for symbol, date_val in batch:
                ...
                tasks.append(
                    self.classify_outcome(symbol, date_val, evaluation_days=evaluation_days)
                )
            ...
            for (symbol, date_val), outcome_metric in zip(batch, batch_results):
                if isinstance(outcome_metric, Exception):
                    logger.error(
                        f"Error processing {symbol} {date_val} in batch: {outcome_metric}",
                        exc_info=True,
                    )
            ...
            if i + batch_size < len(symbols_and_dates):  # Don't sleep after last batch
                await asyncio.sleep(
                    self.config.get("outcome_classifier", {}).get("batch_rate_limit_seconds", 0.5)
        ...
        logger.info(
            f"Successfully classified {success_count}/{len(symbols_and_dates)} outcomes in total."
        )
        return results
