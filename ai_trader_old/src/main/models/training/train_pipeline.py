...
class ModelTrainingPipeline:
    ...
    def train_model(
        ...
        try:
            ...
            logger.info(
                f"Time-series split: {len(X_train)} train samples, {len(X_test)} test samples"
            )
            ...
            logger.info(
                f"Training complete for {model_type}. Accuracy: {metrics['accuracy']:.4f}, F1: {metrics['f1_score']:.4f}"
            )
