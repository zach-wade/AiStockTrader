        ...
        for model_file in model_files:
            try:
                ...
                if not metadata_file.exists():
                    logger.warning(
                        f"Skipping model {model_file.name}: Missing metadata file {metadata_file.name}"
                    )
                    continue
                ...
                if not all(key in metadata for key in required_keys):
                    logger.warning(
                        f"Skipping model {model_file.name}: Metadata is missing required keys."
                    )
                    continue
                ...
                logger.info(
                    f"✅ Registered model: {metadata['model_id']} (AUC: {metadata['metrics'].get('auc', 'N/A'):.4f})"
                )
                registered_count += 1
