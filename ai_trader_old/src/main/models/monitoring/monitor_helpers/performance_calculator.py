...
class PerformanceCalculator:
    ...
    def calculate_metrics(self, recent_predictions: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        valid_data = [
            ...
            and np.isfinite(r.get("prediction"))
            and r.get("actual") is not None
            and np.isfinite(r.get("actual"))
        ]
        ...
        if (
            len(predictions) < 2
        ):  # Need at least 2 points for some metrics like directional accuracy consistency
            logger.debug(
                "Too few valid data points for comprehensive performance metrics. Returning basic stats."
            )
        ...
        logger.debug(
            f"Calculated performance metrics: MAE={metrics['mae']:.4f}, RMSE={metrics['rmse']:.4f}."
        )
        return metrics

    def check_performance_degradation(
        self, current_mae: float, baseline_mae: float, degradation_threshold: float = 0.20
    ) -> bool:
        """
        ...
        if not np.isfinite(current_mae) or not np.isfinite(baseline_mae) or baseline_mae <= 0:
            logger.warning(
                f"Invalid MAE values for degradation check: current={current_mae}, baseline={baseline_mae}. Cannot determine degradation."
            )
            return False
        ...
        logger.debug(
            f"Performance degradation check: current MAE={current_mae:.4f}, baseline MAE={baseline_mae:.4f}, degradation={degradation:.2%}. Degraded: {is_degraded}."
        )
        return is_degraded
