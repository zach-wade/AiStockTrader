...
class MLOpsActionManager:
    ...
    def __init__(
        ...
        self.rollback_threshold = (
            self.config.get("ml", {}).get("monitoring", {}).get("rollback_threshold_mae", 0.5)
        )  # MAE value
        self.retrain_threshold_drift = (
            self.config.get("ml", {}).get("monitoring", {}).get("retrain_threshold_drift", 0.1)
        )  # KS statistic

        logger.debug("MLOpsActionManager initialized.")

    async def handle_performance_degradation(
        self, model_id: str, current_performance_metrics: Dict[str, float]
    ):
        """
        ...
        """
        logger.warning(
            f"Performance degradation detected for model {model_id}. Current MAE: {current_performance_metrics.get('mae', 'N/A'):.4f}"
        )
        ...
        if current_performance_metrics.get("mae", float("inf")) > self.rollback_threshold:
            logger.critical(
                f"MAE ({current_performance_metrics.get('mae'):.4f}) for model {model_id} exceeds rollback threshold ({self.rollback_threshold:.4f}). Initiating rollback."
            )
            await self._rollback_model(model_id)
        else:
            logger.info(
                f"Performance degradation for {model_id} noted, but not severe enough for automatic rollback."
            )
    ...
    async def handle_prediction_drift_detected(self, model_id: str, drift_score: float):
        ...
        if drift_score > self.retrain_threshold_drift:
            logger.info(
                f"Drift score ({drift_score:.4f}) for model {model_id} exceeds retraining threshold ({self.retrain_threshold_drift:.4f}). Triggering retraining."
            )
            await self._trigger_retraining(model_id)
        else:
            logger.info(
                f"Prediction drift for {model_id} noted, but not severe enough for automatic retraining."
            )

    async def handle_feature_drift_detected(
        self, model_id: str, drifted_features: Dict[str, float]
    ):
        """
        ...
        """
        logger.warning(
            f"Feature drift detected for model {model_id}. Features: {list(drifted_features.keys())}"
        )
        ...
        await self.alert_manager.send_alert(
            level="info", title=f"Model Feature Drift Detected: {model_id}", message=alert_message
        )
    ...
    async def _rollback_model(self, model_id: str):
        ...
        """
        logger.critical(
            f"Attempting automatic rollback for model {model_id} due to severe degradation."
        )
        ...
        for v in sorted(versions, key=lambda mv: mv.created_at, reverse=True):
            ...
            if (
                v.status in ["production", "candidate", "archived"]
                and v.metrics.get("mae", float("inf")) < self.rollback_threshold
            ):
        ...
        if previous_stable_version:
            success = await self.model_registry.rollback_model(
                model_id, previous_stable_version.version
            )
            if success:
                await self.alert_manager.send_alert(
                    level="critical",
                ...
                logger.info(
                    f"Successfully rolled back model {model_id} to version {previous_stable_version.version}."
                )
            else:
                await self.alert_manager.send_alert(
                    level="error",
                ...
                logger.error(
                    f"Failed to rollback model {model_id} to version {previous_stable_version.version}."
                )
        else:
        ...
        try:
            ...
            try:
                ...
                if hasattr(self, "training_orchestrator") and self.training_orchestrator:
                    ...
                    logger.warning(
                        f"No training orchestrator available for model {model_id} - manual intervention required"
                    )
