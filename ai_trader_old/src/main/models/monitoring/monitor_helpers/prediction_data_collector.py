...
class PredictionDataCollector:
    ...
    def __init__(self, history_maxlen: int = 10000, feature_history_maxlen: int = 1000):
        ...
        self.feature_history: Dict[str, deque[Any]] = defaultdict(
            lambda: deque(maxlen=feature_history_maxlen)
        )

        logger.debug(
            f"PredictionDataCollector initialized. Prediction history maxlen: {history_maxlen}, Feature history maxlen: {feature_history_maxlen}"
        )
    ...
    def record_prediction(
        ...
        logger.debug(
            f"Recorded prediction for {model_id}. History size: {len(self.prediction_history[model_id])}"
        )

    def get_recent_predictions(
        self, model_id: str, window_hours: int, min_predictions: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        )  # Convert deque to list for slicing/filtering

        if len(history) < min_predictions:
            logger.debug(
                f"Insufficient prediction history ({len(history)} < {min_predictions}) for model {model_id}."
            )
            return []
        ...
        if len(recent_predictions) < min_predictions:
            logger.debug(
                f"Insufficient recent predictions ({len(recent_predictions)} < {min_predictions}) for model {model_id}."
            )
            return []
    ...
    def get_feature_history_for_drift(
        self, model_id: str, feature_name: str, min_points: int
    ) -> List[Any]:
        """
        ...
        if len(history) < min_points:
            logger.debug(
                f"Insufficient feature history ({len(history)} < {min_points}) for feature {feature_name} of model {model_id}."
            )
            return []
    ...
    def clean_old_data(self, cutoff_days: int = 7):
        ...
        logger.debug(
            f"Cleaned old monitoring data older than {cutoff_days} days (for prediction_history)."
        )
