...
class MonitorReporter(ErrorHandlingMixin):
    ...
    async def generate_comprehensive_report(
        self, models: Optional[List[Tuple[str, str]]] = None, period_hours: int = 24
    ) -> MonitoringReport:
        """
        ...
        with self._handle_error("generating comprehensive report"):
            ...
            healthy_models = sum(1 for h in health_statuses if h.overall_health == "healthy")
            models_with_warnings = sum(1 for h in health_statuses if h.overall_health == "warning")
            critical_models = sum(1 for h in health_statuses if h.overall_health == "critical")
            ...
            logger.info(f"Generated monitoring report {report_id} for " f"{total_models} models")
    ...
    async def generate_model_health_report(
        ...
        with self._handle_error("generating model health report"):
            ...
            metrics = await self._collect_model_metrics(model_name, version)
    ...
    async def generate_performance_report(
        self, model_name: str, version: str, period_hours: int = 24
    ) -> PerformanceReport:
        ...
        with self._handle_error("generating performance report"):
            ...
            async with self.db_pool.acquire() as conn:
                ...
                drift_row = await conn.fetchrow(drift_query, model_name, version)
    ...
    async def generate_drift_report(self, model_name: str, version: str) -> Optional[DriftReport]:
        ...
        with self._handle_error("generating drift report"):
            async with self.db_pool.acquire() as conn:
                ...
                ks_test_results = json.loads(row["ks_test_results"] or "{}")
                chi_square_results = json.loads(row["chi_square_results"] or "{}")
    ...
    async def _collect_health_statuses(
        ...
        for model_name, version in models:
            health_status = await self.generate_model_health_report(model_name, version)
            health_statuses.append(health_status)

        return health_statuses

    async def _collect_performance_reports(
        self, models: List[Tuple[str, str]], start_time: datetime, end_time: datetime
        ...
        for model_name, version in models:
            report = await self.generate_performance_report(model_name, version, period_hours)
            performance_reports.append(report)

        return performance_reports

    async def _collect_drift_reports(self, models: List[Tuple[str, str]]) -> List[DriftReport]:
        """Collect drift reports for models."""
        drift_reports = []

        for model_name, version in models:
            drift_report = await self.generate_drift_report(model_name, version)
            if drift_report:
                drift_reports.append(drift_report)

        return drift_reports

    async def _collect_model_metrics(self, model_name: str, version: str) -> Dict[str, float]:
    ...
    def _assess_model_health(self, metrics: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _generate_action_items(
        ...
        critical_models = [h for h in health_statuses if h.overall_health == "critical"]
        ...
        poor_performance = [p for p in performance_reports if p.accuracy and p.accuracy < 0.7]
        ...
        # High error rates
        high_error_models = [p for p in performance_reports if p.error_rate > 0.1]
    ...
    def _generate_drift_recommendations(
        self, drift_score: float, drifting_features: List[str]
    ) -> List[str]:
        ...
        if len(drifting_features) > 5:
            recommendations.append("Consider feature selection - many features drifting")
    ...
    async def _store_report(self, report: MonitoringReport) -> None:
    ...
    def export_report_to_html(self, report: MonitoringReport) -> str:
        ...
        action_items_html = "\n".join([f"<li>{item}</li>" for item in report.action_items])
