...
class MLRegressionStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        model_path: str,
        config: Dict[str, Any],
        feature_engine: Optional[UnifiedFeatureEngine] = None,
    ):
        """
    ...
    async def generate_signals(
        ...
        try:
            ...
            signal = self._generate_signal_from_prediction(
                symbol, predicted_return, prediction_std, position_size, current_position
            )
    ...
    async def _calculate_realtime_features(
    ...
    def _calculate_position_size(
        self, predicted_return: float, prediction_std: float, features: pd.DataFrame
    ) -> float:
        """
        ...
        # Volatility adjustment
        vol_scalar = (
            min(self.vol_target / (current_vol * np.sqrt(252)), 2.0) if current_vol > 0 else 1.0
        )
        ...
        if self.use_kelly_sizing:
            ...
            position_size = min(self.base_position_size * return_multiple, self.max_position_size)
    ...
    def _generate_signal_from_prediction(
        ...
        if current_position and current_position.get("quantity", 0) != 0:
            ...
            if (current_side == "buy" and predicted_return < -self.min_predicted_return) or (
                ...
                logger.info(
                    f"Exit signal for {symbol}: reversing from {current_side} to {direction}"
                )
    ...
    def _update_prediction_tracking(self, symbol: str, predicted_return: float):
        ...
        self.recent_predictions[symbol].append(
            {"timestamp": datetime.now(), "prediction": predicted_return}
        )

        # Keep only recent predictions
        cutoff_time = datetime.now() - timedelta(hours=24)
        self.recent_predictions[symbol] = [
            p for p in self.recent_predictions[symbol] if p["timestamp"] > cutoff_time
