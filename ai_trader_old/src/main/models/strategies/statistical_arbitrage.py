...
class StatisticalArbitrageStrategy(BaseUniverseStrategy):
    ...
    def __init__(self, config: Dict[str, Any], feature_engine: UnifiedFeatureEngine):
        ...
        logger.info(
            f"StatisticalArbitrageStrategy initialized with {len(self.tradable_pairs)} pre-analyzed pairs."
        )
    ...
    def _load_tradable_pairs(self) -> List[Dict]:
        ...
        if not ANALYSIS_RESULTS_PATH.exists():
            logger.warning(
                f"Analysis results not found at {ANALYSIS_RESULTS_PATH}. Strategy will be inactive."
            )
            return []
        try:
    ...
    async def generate_universe_signals(
        ...
        for pair_info in self.tradable_pairs:
            ...
            z_score = (
                (current_spread - pair_info["spread_mean"]) / pair_info["spread_std"]
                if pair_info["spread_std"] > 0
                else 0
            )
            ...
            if not position_info:  # No open position, look for entry
                ...
                if (
                    position_info["side"] == "long_spread" and z_score > -self.zscore_exit_threshold
                ) or (
                    ...
                    signals.append(
                        Signal(
                            ...
                            direction="close",
                            confidence=1.0,
                            metadata={"pair_key": pair_key, "reason": "mean_reversion"},
                        )
                    )
                    signals.append(
                        Signal(
                ...
                # Exit condition 2: Stop loss
                    ...
                    signals.append(
                        Signal(
                            ...
                            direction="close",
                            confidence=1.0,
                            metadata={"pair_key": pair_key, "reason": "stop_loss"},
                        )
                    )
                    signals.append(
                        Signal(
