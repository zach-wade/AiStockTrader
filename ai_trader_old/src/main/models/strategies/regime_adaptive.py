...
class RegimeAdaptiveStrategy(BaseStrategy):
    ...
    async def generate_signals(
        ...
        try:
            ...
            signal = self._apply_regime_strategy(
                symbol, features, self.current_regime, regime_strategy, current_position
            )
        ...
        if len(self.regime_history) >= self.transition_buffer:
            ...
            if all(r == recent_regimes[0] for r in recent_regimes):
                if self.current_regime != recent_regimes[0]:
                    logger.info(
                        f"Regime change detected: {self.current_regime} -> {recent_regimes[0]}"
                    )
    ...
    def _handle_regime_transition(
        self, symbol: str, current_position: Optional[Dict]
    ) -> List[Signal]:
        ...
        if current_position.get("direction") == "long":
            return [
                Signal(
                    ...
                    metadata={"reason": "regime_transition", "strategy": self.name},
                )
            ]
    ...
    def _apply_regime_strategy(
        ...
        if strategy_type == "momentum":
            ...
            return self._mean_reversion_signal(symbol, features, strategy_params, current_position)
            ...
            return self._range_trading_signal(symbol, features, strategy_params, current_position)
