...
class CorrelationStrategy(BaseUniverseStrategy):
    ...
    def __init__(self, config: Dict, feature_engine: UnifiedFeatureEngine):
        ...
        self.correlation_matrix = CorrelationMatrix(
            strategy_conf.get("correlation_matrix_config", {})
        )
    ...
    async def generate_universe_signals(
        self, market_features: Dict[str, pd.DataFrame]
    ) -> List[Signal]:
        """
        ...
        try:
            ...
            correlation_events = self.correlation_matrix.analyze_latest_correlations(
                market_features
            )
        ...
        # Example for a 'risk_off' regime shift event
            ...
            for asset in event.affected_assets:
                if asset in risky_assets:
                    signals.append(
                        Signal(symbol=asset, direction="sell", confidence=event.strength)
                    )
