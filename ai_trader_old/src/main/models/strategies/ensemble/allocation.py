...
class EnsembleAllocator:
    ...
    def __init__(
        self, config: AllocationConfig, metrics_collector: Optional[MetricsCollector] = None
    ):
        """
    ...
    def _risk_parity_allocation(
        self, symbol_signals: Dict[str, List[Signal]], market_data: Dict[str, pd.DataFrame]
    ) -> AllocationResult:
        """
    ...
    def _mean_variance_allocation(
        ...
        if self.config.target_volatility:
            constraints.append(
                {
                    "type": "eq",
                    "fun": lambda x: np.sqrt(x @ cov_matrix.values @ x)
    ...
    def _signal_strength_allocation(
        self, symbol_signals: Dict[str, List[Signal]]
    ) -> AllocationResult:
        """
        ...
        if total_strength > 0:
            weights = {
                symbol: strength / total_strength for symbol, strength in signal_strengths.items()
            }
        else:
            weights = {}
    ...
    def _equal_weight_allocation(self, symbol_signals: Dict[str, List[Signal]]) -> AllocationResult:
        """
    ...
    def _adjust_weights_by_signals(
        self, weights: Dict[str, float], symbol_signals: Dict[str, List[Signal]]
    ) -> Dict[str, float]:
        """Adjust weights based on signal strength."""
        adjusted_weights = {}
    ...
    def _apply_constraints(
        self, result: AllocationResult, current_weights: Dict[str, float]
    ) -> AllocationResult:
    ...
    def _calculate_portfolio_metrics(
        self, result: AllocationResult, market_data: Dict[str, pd.DataFrame]
    ) -> AllocationResult:
        ...
        # Get returns
        returns_df = pd.DataFrame({s: self._returns_cache.get(s, pd.Series()) for s in symbols})
    ...
    def _create_result_from_current(
        self, current_weights: Dict[str, float], market_data: Dict[str, pd.DataFrame]
    ) -> AllocationResult:
    ...
    def _track_allocation(self, result: AllocationResult, signal_count: int) -> None:
        ...
        if self.metrics:
            self.metrics.gauge("allocator.portfolio_sharpe", result.sharpe_ratio)
            self.metrics.gauge("allocator.effective_positions", result.effective_n)
