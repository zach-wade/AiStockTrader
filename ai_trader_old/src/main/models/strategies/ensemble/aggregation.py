...
class SignalAggregator:
    ...
    def aggregate(
        ...
        # Add other aggregation methods like consensus voting here
            ...
            logger.warning(
                f"Unknown aggregation method: {self.aggregation_method}. Defaulting to weighted average."
            )
            return self._weighted_average_aggregation(
                signals_by_symbol[symbol], strategy_weights, symbol, regime
            )
    ...
    def _weighted_average_aggregation(
        ...
        for name, signal in signal_pairs:
            ...
            if weight > 0:
                ...
                dir_val = (
                    1 if signal.direction == "buy" else -1 if signal.direction == "sell" else 0
                )
        ...
        if total_weight > 0:
            ...
            return [
                EnsembleSignal(
                    ...
                    time_horizon=int(
                        np.mean([s.metadata.get("time_horizon", 60) for _, s in signal_pairs])
                    ),
