...
class StrategyPerformance:
    ...
    recent_returns: Deque[float] = field(
        default_factory=lambda: deque(maxlen=252)
    )  # Store ~1 year of daily returns
...
class PerformanceTracker:
    ...
    def __init__(self, strategy_names: list[str]):
        self.strategy_performance: Dict[str, StrategyPerformance] = {
            name: StrategyPerformance(strategy_name=name) for name in strategy_names
        }
    ...
    def update(self, trade_result: Dict[str, Any]):
        ...
        for strategy_name in contributing_strategies:
            if strategy_name not in self.strategy_performance:
                logger.warning(
                    f"Performance update for unknown strategy '{strategy_name}'. Skipping."
                )
                continue
            ...
            logger.debug(
                f"Updated performance for {strategy_name}: Sharpe={perf.sharpe_ratio:.2f}, Drawdown={perf.max_drawdown:.2%}"
            )
    ...
    def _calculate_sharpe_ratio(
        self, perf: StrategyPerformance, risk_free_rate: float = 0.0
    ) -> float:
