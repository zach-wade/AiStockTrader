...
class BaseCatalystSpecialist(ABC):
    ...
    def train(
        self, training_data: pd.DataFrame, target_column: str = "successful_outcome"
    ) -> Dict[str, float]:
        ...
        try:
            ...
            if len(relevant_data) < self.min_training_samples:
                logger.warning(
                    f"{self.specialist_type}: insufficient training data ({len(relevant_data)} samples)"
                )
    ...
    def _prepare_feature_vector(
        self, specialist_features: Dict[str, float]
    ) -> Optional[np.ndarray]:
        """## REFACTOR: Stricter feature vector preparation."""
        if not self.feature_columns:
            logger.error(
                f"{self.specialist_type} model has no feature_columns. Must be trained first."
            )
            return None
        ...
        try:
            ...
            if not np.all(np.isfinite(feature_array)):
                logger.warning(
                    f"NaN/inf found in feature vector for {self.specialist_type}. Replacing with 0."
                )
    ...
    def _save_model(self):
        ...
        try:
            model_file = (
                self.model_dir / f"{self.specialist_type}_specialist_{self.model_version}.joblib"
            )
