...
class FeatureCalculationEngine:
    ...
    async def run(self, feature_config: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            feature_sets = feature_config.get(
                "feature_sets", ["technical", "fundamental", "sentiment"]
            )
            ...
            logger.info(
                f"Calculating features for {len(symbols)} symbols from {start_date} to {end_date}"
            )
            ...
            for symbol in symbols:
                try:
                    ...
                    # Calculate features
                    features_df = self._calculate_features(market_data, symbol, feature_sets)
                    ...
                    # Store features
                    success = await self._store_features(symbol, features_df, feature_sets)

                    if success:
                        results["symbols"].append(symbol)
                        results["features_calculated"] += len(features_df)
                        logger.info(
                            f"Successfully calculated {len(features_df)} features for {symbol}"
                        )
                    else:
    ...
    async def _fetch_market_data(
        ...
        try:
            ...
            logger.info(
                f"Combined {len(all_data)} files with {len(combined_data)} total rows for {symbol}"
            )

            # Check if timestamp is in index or columns
            if combined_data.index.name == "timestamp" or isinstance(
                combined_data.index, pd.DatetimeIndex
            ):
                ...
                filtered_data = combined_data.loc[
                    (combined_data.index >= start_date) & (combined_data.index <= end_date)
                ].copy()
                logger.info(
                    f"Filtered to {len(filtered_data)} rows within date range {start_date.date()} to {end_date.date()}"
                )
            ...
            if missing_columns:
                logger.warning(
                    f"Missing required columns: {missing_columns}. Available: {filtered_data.columns.tolist()}"
                )
                return pd.DataFrame()
    ...
    def _calculate_features(
        self, market_data: pd.DataFrame, symbol: str, feature_sets: list[str]
    ) -> pd.DataFrame:
        ...
        try:
            ...
            features_df = self.feature_engine.calculate_features(
                data=market_data, symbol=symbol, calculators=calculators, use_cache=False
            )
    ...
    async def _store_features(
        self, symbol: str, features_df: pd.DataFrame, feature_sets: list[str]
    ) -> bool:
    ...
    async def _save_results_to_file(self, results: dict[str, Any], output_file: str):
