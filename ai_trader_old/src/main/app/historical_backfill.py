...
from main.data_pipeline.types import BackfillParams, DataSource, DataType, TimeInterval
...
from main.utils.resilience import NETWORK_RETRY_CONFIG, ErrorRecoveryManager
...
async def create_historical_manager(config) -> HistoricalManager:
    ...
    enable_dual_storage = (
        config.get("data_pipeline", {}).get("storage", {}).get("enable_dual_storage", True)
    )
    ...
    if bulk_config.get("enabled", True):
        ...
        bulk_loader = BulkDataLoader(
            db_adapter=db_adapter, archive=archive, config=bulk_load_config
        )

        info_message(
            "Bulk loader enabled",
            f"Using optimized bulk loading with accumulation size: {bulk_load_config.accumulation_size}",
        )
    ...
    rate_limiter = RateLimiter(rate=calls_per_minute, per=60.0)  # per 60 seconds (1 minute)
    # Local imports
    ...
    processing_manager = ProcessingManager(config=config, db_adapter=db_adapter)
...
def convert_cli_params_to_backfill_params(
                ...
                if stage_config:
                    ...
                    if stage_data_type:
                        try:
                            ...
                            data_type_enum = data_type_mapping.get(
                                stage_data_type, DataType.MARKET_DATA
                            )
    ...
    if not sources or len(sources) > 3:  # If no sources or too many accumulated
        ...
        logger.info(
            f"Using smart source selection: {[s.value for s in smart_sources]} for data types: {[dt.value for dt in data_types]}"
        )
    ...
    logger.info(
        f"Backfill parameters - Intervals: {[i.value for i in intervals]}, "
...
async def run_historical_backfill(backfill_config: dict) -> dict[str, Any]:
    ...
    with timer("historical_backfill") as backfill_timer:
        try:
            ...
            info_message("Backfill configuration", f"Stages: {stages}, Days: {lookback_days}")
            ...
            if not symbols:
                ...
                if not symbols:
                    warning_message(
                        "No universe symbols found", "Consider running universe population first"
                    )
                    ...
                    if original_count != len(symbols):
                        info_message(
                            "Filtered symbols",
                            f"Using {len(symbols)} valid symbols (excluded {original_count - len(symbols)} invalid)",
                        )
                    else:
            ...
            # Handle layer-specific symbol loading
                ...
                try:
                    ...
                    info_message(
                        f"Loading Layer {layer_num} symbols",
                        "Fetching from scanner qualifications...",
                    )
                    ...
                    if symbols:
                        ...
                        symbols = filter_valid_symbols(symbols)
                        info_message(
                            f"Loaded Layer {layer_num} symbols",
                            f"Using {len(symbols)} valid symbols"
                            + (
                                f" (excluded {original_count - len(symbols)} invalid)"
                                if original_count != len(symbols)
                                else ""
                            ),
                        )
                        ...
                        warning_message(
                            f"No symbols found for Layer {layer_num}",
                            "Check if scanner pipeline has been run",
                        )
                        return {
                            "status": "failed",
                            "error": f"No symbols found for Layer {layer_num}",
                        }

                except ValueError:
                    error_message(
                        "Invalid layer specification",
                        "Use 'layer0', 'layer1', 'layer2', or 'layer3'",
                    )
            ...
            params = convert_cli_params_to_backfill_params(
                stages=stages, symbols=symbols, lookback_days=lookback_days, config=old_config
            )
            ...
            if test_mode:
                ...
                info_message(
                    "Limiting symbols", f"Processing first {limit} of {len(symbols)} symbols"
                )
            ...
            if structured_config.backfill_orchestration.use_symbol_tiers and len(symbols) > 100:
                info_message(
                    "Using backfill orchestrator",
                    f"Processing {len(symbols)} symbols with intelligent tiering",
                )
                ...
                if not backfill_stages:
                    backfill_stages = [
                        BackfillStage.MARKET_DATA,
                        BackfillStage.NEWS,
                        BackfillStage.CORPORATE_ACTIONS,
                    ]
                    logger.info(
                        f"No valid stages found, using defaults: {[s.value for s in backfill_stages]}"
                    )
                ...
                logger.info(
                    f"Symbol source detected: {symbol_source if 'symbol_source' in locals() else 'unknown'}"
                )
                if "symbol_source" in locals() and symbol_source == "layer1":
                    use_tiers = False
                    logger.info(
                        "🎯 Disabling symbol tier filtering for Layer 1 backfill to process all qualified symbols"
                    )
                else:
                ...
                logger.info(
                    f"Starting orchestrator with data_types={[dt.value for dt in params.data_types] if params.data_types else 'All'}"
                )
                ...
                logger.info(
                    f"Orchestrator completed: {orch_result.symbols_completed} symbols, {orch_result.records_loaded} records"
                )
                ...
                results = {
                    ...
                    "market_data_records": orch_result.stage_results.get("market_data", {}).get(
                        "records_loaded", 0
                    ),
                    "news_records": orch_result.stage_results.get("news", {}).get(
                        "records_loaded", 0
                    ),
                    "corporate_actions_records": orch_result.stage_results.get(
                        "corporate_actions", {}
                    ).get("records_loaded", 0),
                    "fundamentals_records": orch_result.stage_results.get("fundamentals", {}).get(
                        "records_loaded", 0
                    ),
                    "financial_records": orch_result.stage_results.get("fundamentals", {}).get(
                        "records_loaded", 0
            ...
            if old_config.get("storage", {}).get("lifecycle", {}).get("archive_on_backfill", True):
                ...
                try:
                    ...
                    if archival_results.get("status") == "success":
                        ...
                        warning_message(
                            "Data archival issue", archival_results.get("message", "Unknown error")
                        )
            ...
            if (
                "corporate_actions" in data_types_processed
                or results.get("corporate_actions_records", 0) > 0
            ):
                info_message(
                    f"  Corporate actions: {results.get('corporate_actions_records', 0)} records"
                )
