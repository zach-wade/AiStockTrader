        ...
        try:
            ...
            if sample_symbols:
                ...
                if not market_data:
                    ...
                    self.logger.info(
                        f"✅ Market data validated ({len(market_data)} records for {symbol})"
                    )
            ...
            errors.append(f"Data pipeline validation error: {e!s}")
    ...
    async def _validate_feature_pipeline(self):
        ...
        try:
            ...
            if not table_check or not table_check.get("exists"):
                ...
                # Check feature count
                feature_count = await db_adapter.fetch_one("SELECT COUNT(*) as count FROM features")
            ...
            for calc_type in calculators:
                try:
                    ...
                    errors.append(f"Calculator {calc_type} error: {e!s}")

            await db_adapter.close()

        except Exception as e:
            errors.append(f"Feature pipeline validation error: {e!s}")
        ...
        try:
            ...
            if production_models:
                try:
                    ...
                    errors.append(f"Model loading error: {e!s}")

        except Exception as e:
            errors.append(f"Model validation error: {e!s}")
    ...
    async def _validate_trading(self):
        ...
        try:
            ...
            try:
                ...
                errors.append(f"Broker initialization error: {e!s}")
            ...
            errors.append(f"Trading validation error: {e!s}")
