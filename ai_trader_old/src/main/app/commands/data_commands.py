...
@click.option("--days", type=int, default=30, help="Number of days to backfill (default: 30)")
@click.option("--force", is_flag=True, help="Force backfill even if data exists")
...
@click.option(
    "--component",
    type=click.Choice(["all", "data", "features", "models"]),
    default="all",
...
@click.pass_context
def process_raw(ctx, data_type: str, limit: int | None):
...
@data.command()
@click.option("--older-than", type=int, default=90, help="Archive data older than N days")
    ...
    try:
        ...
        if dry_run:
            ...
            results = {
                ...
                "total_size_mb": 0,
                "tables": [],
                "archive_path": str(archive.local_path),
            }
...
@click.pass_context
def stats(ctx, table: str | None, detailed: bool):
...
@click.option(
    "--interval",
    type=click.Choice(["1min", "5min", "1hour", "1day"]),
    default="1day",
    help="Data interval to check",
)
@click.option("--days", type=int, default=30, help="Number of days to check")
@click.option("--fill", is_flag=True, help="Automatically fill detected gaps")
@click.pass_context
def gaps(ctx, symbols: str | None, interval: str, days: int, fill: bool):
    ...
    try:
        ...
        if not gaps_found:
            ...
            for gap in gaps_found[:10]:  # Show first 10
                print(
                    f"  {gap['symbol']}: {gap['start']} to {gap['end']} ({gap['missing']} records)"
                )
...
def _print_validation_results(results: dict):
    ...
    for component, component_results in results.get("components", {}).items():
        ...
        print(
            f"  Checks passed: {component_results.get('passed_checks', 0)}/{component_results.get('total_checks', 0)}"
        )
...
def _print_overall_stats(stats: dict, detailed: bool):
    ...
    for table_info in stats.get("top_tables_by_size", [])[:5]:
        print(
            f"  {table_info['name']}: {table_info['size_mb']:.2f} MB ({table_info['row_count']:,} rows)"
        )
