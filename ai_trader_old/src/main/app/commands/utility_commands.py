...
@click.option("--lookback-days", type=int, default=365, help="Days of historical data for training")
@click.option("--test-size", type=float, default=0.2, help="Test set size (0-1)")
@click.pass_context
def train(ctx, symbols: str | None, models: str | None, lookback_days: int, test_size: float):
...
def features(
    ...
    try:
        ...
        feature_config = FeatureConfig(
            symbols=symbol_list, features=feature_list, start_date=start, end_date=end
        )
...
@click.option("--duration", type=int, default=60, help="Duration to run event monitor (seconds)")
@click.option("--output", help="Output file for events (JSON)")
@click.pass_context
def events(ctx, symbols: str | None, events: str | None, duration: int, output: str | None):
    ...
    try:
        ...
        collected_events = asyncio.run(
            monitor.monitor(duration=duration, symbols=symbol_list, event_types=event_list)
        )
...
@click.option("--timeout", type=int, default=30, help="Timeout for graceful shutdown (seconds)")
@click.pass_context
