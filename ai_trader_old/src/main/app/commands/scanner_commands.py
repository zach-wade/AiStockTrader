...
@click.option("--layer", type=click.Choice(["0", "1", "2", "3"]), help="Run specific layer scanner")
@click.option("--catalyst", is_flag=True, help="Run catalyst scanner (earnings, news, insider)")
...
def scan(
    ...
    try:
        if pipeline:
            ...
            alerts = asyncio.run(
                catalyst_scanner.scan_catalysts(symbols=symbol_list, dry_run=dry_run)
            )
...
@click.option(
    ...
    default="all",
    help="Layer to check status",
)
@click.option("--detailed", is_flag=True, help="Show detailed scanner metrics")
@click.pass_context
...
@click.option("--min-score", type=float, help="Minimum alert score to display")
@click.option(
    "--type",
    type=click.Choice(["earnings", "news", "insider", "premarket", "all"]),
    default="all",
    help="Alert type to display",
)
@click.option(
    "--format", type=click.Choice(["table", "json", "csv"]), default="table", help="Output format"
)
@click.pass_context
def alerts(ctx, hours: int, min_score: float | None, type: str, format: str):
...
@scanner.command()
@click.option(
    "--layer", type=click.Choice(["0", "1", "2", "3"]), required=True, help="Layer to configure"
)
@click.option("--threshold", type=float, help="Score threshold for layer qualification")
@click.option("--max-symbols", type=int, help="Maximum symbols for this layer")
@click.option("--scan-interval", type=int, help="Scan interval in minutes")
...
@scanner.command()
@click.option("--clear-all", is_flag=True, help="Clear all scanner cache")
@click.option(
    "--layer", type=click.Choice(["0", "1", "2", "3"]), help="Clear cache for specific layer"
)
@click.option("--older-than", type=int, help="Clear cache older than N hours")
@click.pass_context
def cache(ctx, clear_all: bool, layer: str | None, older_than: int | None):
    ...
    try:
        ...
        if clear_all:
            ...
            print(
                f"✅ Cleared cache older than {older_than} hours: {result['cleared_entries']} entries"
            )
    ...
    for alert_type, type_alerts in by_type.items():
        ...
        if show_alerts:
            for alert in type_alerts[:5]:
                print(
                    f"  {alert['symbol']}: {alert['message']} (score: {alert.get('score', 0):.1f})"
                )
...
def _print_cache_stats(stats: dict):
    ...
    if stats.get("by_layer"):
        ...
        for layer, layer_stats in stats["by_layer"].items():
            print(
                f"  Layer {layer}: {layer_stats['entries']} entries ({layer_stats['size_mb']:.2f} MB)"
            )
