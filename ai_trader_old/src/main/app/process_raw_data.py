...
class RawDataProcessor:
    ...
    def __init__(self, config: DictConfig):
        ...
        self.data_lake_path = Path(
            config.get(
                "data_pipeline.storage.archive.local_path",
        ...
        logger.info(
            f"RawDataProcessor initialized with standardizer and transformer at: {self.data_lake_path}"
        )

    def find_unprocessed_files(
        self, data_type: str = "all", limit: int = 1000, symbol: str = None
    ) -> list[Path]:
        """
                ...
                for pattern in ["*.json", "*.parquet"]:
                    for raw_file in subdir.rglob(pattern):
                        ...
                        if raw_file.suffix == ".json":
                            ...
                            processed_file = self.processed_path / relative_path.with_suffix(
                                ".parquet"
                            )
                        else:
    ...
    async def transform_market_data_file(self, raw_file: Path) -> pd.DataFrame | None:
        """
        ...
        try:
                ...
                # Check if it has a direct 'data' key
                    ...
                    if isinstance(data_content, dict) and "data" in data_content:
                        ...
                        if (
                            ...
                            and "timestamp" not in data_content
                            and "timestamp" in raw_data
                        ):
            ...
            transformed_df = await self.transformer.transform_market_data(
                standardized_df, source, symbol
            )
    ...
    async def process_parquet_file(self, raw_file: Path) -> pd.DataFrame | None:
        """
    ...
    def transform_news_file(self, raw_file: Path) -> list[dict] | None:
        """
    ...
    async def process_files(
        self, data_type: str = "all", limit: int = 100, symbol: str = None
    ) -> dict[str, int]:
        """
        ...
        success_message(
            "Raw data processing completed", f"Processed: {processed_count}, Failed: {failed_count}"
        )

        return {"files_processed": processed_count, "files_failed": failed_count}
...
def run(
    ...
    async def _process_raw(handler: StandardCLIHandler, data_type: str, limit: int, symbol: str):
        ...
        try:
            ...
            error_message("Raw data processing failed", f"Error: {e!s}")
            raise
...
def status():
    ...
    async def _show_status(handler: StandardCLIHandler):
        ...
        for data_type in ["market_data", "news", "social_sentiment", "financials"]:
            ...
            processed_count = (
                len(list(processed_dir.rglob("*.parquet"))) if processed_dir.exists() else 0
            )
