...
class RiskAnalyzer:
    ...
    def calculate_risk_metrics(
        ...
        sortino = safe_divide(
            metrics["annual_return"] - self.risk_free_rate, downside_std, default_value=0.0
        )
        metrics["sortino_ratio"] = sortino

        # Calmar ratio
        calmar = safe_divide(
            metrics["annual_return"], abs(metrics["max_drawdown"]), default_value=0.0
        )
        ...
        if benchmark_returns is not None and len(benchmark_returns) == len(returns):
            ...
            information_ratio = safe_divide(
                excess_returns.mean() * 252, tracking_error, default_value=0.0
            )
            ...
            alpha = metrics["annual_return"] - (
                self.risk_free_rate + beta * (benchmark_returns.mean() * 252 - self.risk_free_rate)
            )
            metrics["alpha"] = alpha

        # Tail ratio
        metrics["tail_ratio"] = abs(
            safe_divide(np.percentile(returns, 95), np.percentile(returns, 5), default_value=0.0)
        )
        ...
        metrics["profit_factor"] = safe_divide(
            winning_returns.sum(), abs(losing_returns.sum()), default_value=np.inf
        )
    ...
    def stress_test(
        self, portfolio_returns: pd.Series, portfolio_positions: pd.DataFrame, scenario: str
    ) -> dict[str, any]:
        """
        ...
        for _ in range(scenario_params["duration_days"]):
            # Apply equity shock
            base_return = safe_divide(
                scenario_params["equity_shock"], scenario_params["duration_days"], default_value=0.0
            )

            # Add increased volatility
            vol_shock = secure_numpy_normal(
                0,
                safe_divide(
                    current_volatility * scenario_params["volatility_multiplier"],
                    np.sqrt(252),
                    default_value=0.0,
                ),
            )
    ...
    def run_all_stress_tests(
        self, portfolio_returns: pd.Series, portfolio_positions: pd.DataFrame
    ) -> dict[str, dict]:
    ...
    def monte_carlo_var(
        ...
        for _ in range(n_simulations):
            ...
            random_returns = np.random.multivariate_normal(
                mean_returns * time_horizon, cov_matrix * time_horizon, size=1
            )[0]
    ...
    def calculate_correlation_risk(self, returns: pd.DataFrame) -> dict[str, float]:
        ...
        normal_risk = np.sqrt(
            np.dot(equal_weights, np.dot(returns.cov(), equal_weights))
        ) * np.sqrt(252)

        stressed_cov = returns.std() * stressed_correlation * returns.std().values.reshape(-1, 1)
        stressed_risk = np.sqrt(
            np.dot(equal_weights, np.dot(stressed_cov, equal_weights))
        ) * np.sqrt(252)

        correlation_risk_increase = safe_divide(
            stressed_risk - normal_risk, normal_risk, default_value=0.0
        )
