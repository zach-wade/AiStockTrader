...
class StrategyValidationSuite:
    ...
    async def run_walk_forward_analysis(
        self, strategy: BaseStrategy, symbol: str, features: pd.DataFrame
        ...
        for i, (train_start, train_end, test_start, test_end) in enumerate(periods):
            logger.info(
                f"Processing Period {i+1}/{len(periods)}: Test from {test_start.date()} to {test_end.date()}"
            )
            ...
            out_of_sample_metrics = self.performance_analyzer.calculate_all_metrics(
            ...
            logger.info(
                f"  In-Sample Sharpe: {in_sample_metrics.get('sharpe_ratio', 0):.2f} -> "
    ...
    def _log_walk_forward_summary(self, results: list[WalkForwardResult]):
        ...
        degradation = [
            (is_sharpe - oos_sharpe) / abs(is_sharpe) if abs(is_sharpe) > 0.1 else 0
