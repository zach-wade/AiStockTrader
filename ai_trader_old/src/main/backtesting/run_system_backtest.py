...
class SystemBacktestRunner:
    ...
    def __init__(self, config: DictConfig):
        ...
        self.data_fetcher = DataFetcher(config=config, db_adapter=self.db_adapter, clients=clients)
    ...
    async def run_all_backtests(
        self, broad_universe_symbols: list[str], start_date: datetime, end_date: datetime
    ):
        """
        ...
        historical_data_map = await self.data_provider.get_bulk_daily_data(
            broad_universe_symbols, start_date, end_date
        )

        symbol_metrics = await self.symbol_selector.analyze_symbols(
            symbols=broad_universe_symbols, market_data=historical_data_map
        )
        ...
        logger.info(
            f"Universe selection complete. Proceeding to backtest {len(tradable_symbols)} high-quality symbols."
        )
        ...
        for symbol in tradable_symbols:
            ...
            for name, strategy in self.strategies.items():
                try:
                    ...
                    logger.info(
                        f"✓ {name}: Sharpe={metrics.get('sharpe_ratio', 0):.2f}, Return={metrics.get('total_return_pct', 0):.2%}"
                    )

                except Exception as e:
                    logger.error(
                        f"Backtest failed for strategy '{name}' on symbol '{symbol}': {e}",
                        exc_info=True,
                    )
    ...
    def _generate_summary_report(self, all_results: dict[str, list[dict]]) -> pd.DataFrame | None:
    ...
    async def _run_deep_validation_on_best_strategy(
        self, summary_df: pd.DataFrame, historical_data_map: dict[str, pd.DataFrame]
    ):
        ...
        if validation_symbol not in historical_data_map:
            logger.error(
                f"Validation symbol '{validation_symbol}' not found. Cannot run deep validation."
            )
            return

        logger.info("\n" + "=" * 80)
        logger.info(
            f"        PHASE 3: DEEP DIVE VALIDATION on '{best_strategy_name}' using '{validation_symbol}'"
        )
        logger.info("=" * 80)
        ...
        await self.validation_suite.run_walk_forward_analysis(
            strategy=best_strategy_obj, symbol=validation_symbol, features=validation_features
        )
...
async def main():
    ...
    broad_universe = backtest_config.get(
        "broad_universe", ["AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "TSLA", "JPM", "V", "SPY"]
    )
