...
from main.interfaces.events import Event, EventType, MarketEvent, OrderEvent
from main.models.common import Order, OrderSide, OrderType, Strategy
from main.utils.core import AsyncCircuitBreaker, ErrorHandlingMixin, ensure_utc, get_logger

from .bar_aggregator import BarAggregator
from .cost_model import CostModel, create_default_cost_model
from .market_simulator import ExecutionMode, MarketSimulator
...
class BacktestConfig:
    ...
    commission_model: str | None = "percentage"
    slippage_model: str | None = "spread"
...
class BacktestEngine(ErrorHandlingMixin):
    ...
    def __init__(
        ...
        self.portfolio = Portfolio(
            initial_cash=config.initial_cash, margin_ratio=config.margin_ratio
        )

        self.market_simulator = MarketSimulator(
            cost_model=self.cost_model, execution_mode=config.execution_mode
        )
        ...
        self.circuit_breaker = AsyncCircuitBreaker(failure_threshold=10, recovery_timeout=60)
    ...
    async def _setup_event_handlers(self):
        ...
        self.event_bus.subscribe(EventType.MARKET_DATA, self._handle_market_event)
        ...
        self.event_bus.subscribe(EventType.ORDER_FILLED, self._handle_fill_event)
    ...
    async def _handle_market_event(self, event: MarketEvent):
        ...
        if hasattr(self.strategy, "on_market_data"):
            orders = await self.circuit_breaker.call(self.strategy.on_market_data, event)
    ...
    def _generate_results(self) -> BacktestResult:
        ...
        # Create equity curve
        equity_curve = (
            portfolio_history["total_equity"] if not portfolio_history.empty else pd.Series()
        )

        # Calculate drawdown
        drawdown_curve = (
            self._calculate_drawdown(equity_curve) if not equity_curve.empty else pd.Series()
        )
...
def create_backtest_engine(
    config: BacktestConfig, strategy: Strategy, data_source: Any
) -> BacktestEngine:
