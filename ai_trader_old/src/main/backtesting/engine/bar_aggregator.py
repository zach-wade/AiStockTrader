...
class BarAggregator:
    ...
    def process_minute_bar(
        self, symbol: str, timestamp: datetime, ohlcv: dict[str, float]
    ) -> list[MarketEvent]:
        """
        ...
        for timeframe, minutes in self.timeframe_minutes.items():
            ...
            if completed_bar:
                ...
                logger.debug(
                    f"Completed {timeframe} bar for {symbol} at {completed_bar['timestamp']}"
                )
        ...
        for symbol in self.incomplete_bars:
            for timeframe in self.incomplete_bars[symbol]:
                for bar_key, bar in list(self.incomplete_bars[symbol][timeframe].items()):
                    ...
                    event = MarketEvent(
                        timestamp=completed_bar["timestamp"],
                        data={"symbol": symbol, "timeframe": timeframe, "ohlcv": completed_bar},
                    )
                    completed_events.append(event)
