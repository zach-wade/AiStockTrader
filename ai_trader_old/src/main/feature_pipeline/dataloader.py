...
from main.data_pipeline.storage.storage_router import QueryType, StorageRouter

# KeyManager was refactored away
# from main.data_pipeline.storage.key_manager import KeyManager
from main.feature_pipeline.feature_store import FeatureStoreRepository, FeatureStoreV2
...
class MarketDataSource(DataSource):
    ...
    async def _load_symbol_data(
        ...
        # Create query filter
        query_filter = QueryFilter(symbols=[symbol], start_date=start_date, end_date=end_date)
    ...
    async def _load_from_data_lake_processed(
        self, symbol: str, start_date: datetime, end_date: datetime, interval: str
    ) -> pd.DataFrame:
        ...
        try:
            ...
            while current_date <= end_month:
                ...
                if monthly_data is not None and not monthly_data.empty:
                    ...
                    logger.debug(
                        f"Loaded {len(monthly_data)} records from data lake for {symbol} {current_date.strftime('%Y-%m')}"
                    )
            ...
            if all_data:
                combined_data = pd.concat(all_data, ignore_index=True)
                logger.debug(
                    f"Loaded {len(combined_data)} total records from data lake for {symbol}"
                )
                return combined_data
            else:
    ...
    async def _load_from_data_lake_raw(
        self, symbol: str, start_date: datetime, end_date: datetime, interval: str
    ) -> pd.DataFrame:
            ...
            for obj_metadata in raw_objects:
                ...
                if key_info and "date" in key_info["partitions"]:
                    ...
                    if start_date.date() <= file_date <= end_date.date():
                        ...
                        if (
                            raw_data
                            and "raw_response" in raw_data
                            and "data" in raw_data["raw_response"]
                        ):
    ...
    def get_data_range(self, symbol: str) -> tuple[datetime, datetime]:
        ...
        try:
            # Access market_data_repo through storage_router
            return (datetime.now(UTC) - timedelta(days=30), datetime.now(UTC))  # Default range
        except Exception as e:
...
class FeatureDataSource(DataSource):
    ...
    async def _load_symbol_features(
        self, symbol: str, start_date: datetime, end_date: datetime, feature_names: list[str] | None
    ) -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning(
                f"StorageRouter query failed for features {symbol}: {e}, falling back to direct methods"
            )
    ...
    async def _load_from_hdf5_storage(
        self, symbol: str, start_date: datetime, end_date: datetime, feature_names: list[str] | None
    ) -> pd.DataFrame:
        ...
        try:
            ...
            while current_date <= end_month:
                for feature_type in feature_types:
                    ...
                    if monthly_features is not None and not monthly_features.empty:
                        # Filter to requested date range
                        if (
                            monthly_features.index.name == "timestamp"
                            or "timestamp" in monthly_features.columns
                        ):
                        ...
                        logger.debug(
                            f"Loaded {len(monthly_features)} features from data lake for {symbol} {feature_type} {current_date.strftime('%Y-%m')}"
                        )
            ...
            if all_features:
                combined_features = pd.concat(all_features, ignore_index=True)
                logger.debug(
                    f"Loaded {len(combined_features)} total features from data lake for {symbol}"
                )
                return combined_features
            else:
...
class AlternativeDataSource(DataSource):
    ...
    def __init__(self, source_type: str, config: dict[str, Any] | None = None):
        ...
        try:
            ...
            class EconomicDataRepository(BaseRepository):
                ...
                async def get_economic_data(
                    self, indicators: list[str], start_date: datetime, end_date: datetime
                ):
    ...
    async def _load_news_data(self, request: DataRequest) -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning(
                f"StorageRouter query failed for news: {e}, falling back to direct query"
            )
        ...
        try:
            ...
            if result is not None:
                ...
                if not social_df.empty:
                    logger.info(
                        f"Loaded {len(social_df)} social sentiment records via StorageRouter"
                    )
                    return social_df
            ...
            logger.warning(
                f"StorageRouter query failed for social sentiment: {e}, falling back to direct query"
            )
...
class DataLoader:
    ...
    async def load_data(self, request: DataRequest) -> dict[str, pd.DataFrame]:
        ...
        for data_type in request.data_types:
            if data_type in self.sources:
                task = asyncio.create_task(
                    self._load_from_source(data_type, request), name=f"load_{data_type}"
                )
        ...
        if request.include_features or request.feature_names:
            task = asyncio.create_task(
                self._load_from_source("features", request), name="load_features"
            )
            ...
            try:
                if data_type == "market_data":
                    ...
                    for symbol in symbols:
                        ...
                        processed_symbol_data = self.preprocessor.preprocess_market_data(
                            symbol_data, symbol
                        )
                    ...
                    processed_data[data_type] = self.preprocessor.preprocess_feature_data(
                        data, "general"
                    )

                elif data_type in ["news", "social", "economic"]:
                    # Apply alternative data preprocessing
                    processed_data[data_type] = self.preprocessor.preprocess_alternative_data(
                        data, data_type
                    )
                ...
                logger.debug(
                    f"Preprocessed {data_type}: {data.shape} -> {processed_data[data_type].shape}"
                )
    ...
    async def load_combined_data(
        ...
        if market_data.empty:
            ...
            combined = pd.merge(market_data, features, on=["symbol", "timestamp"], how="left")
            return combined
    ...
    def get_data_range(
        self, symbol: str, data_type: str = "market_data"
...
async def load_combined_data(
    ...
    loader = DataLoader(config)
    try:
        return await loader.load_combined_data(
            symbols, start_date, end_date, interval, feature_names
        )
    finally:
        await loader.close()
