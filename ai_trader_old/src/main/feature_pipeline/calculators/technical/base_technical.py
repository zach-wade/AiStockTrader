    ...
    def preprocess_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if invalid_ohlc.any():
                ...
                mask = invalid_ohlc
                processed_data.loc[mask, "high"] = processed_data.loc[
                    mask, ["high", "open", "close"]
                ].max(axis=1)
                processed_data.loc[mask, "low"] = processed_data.loc[
                    mask, ["low", "open", "close"]
                ].min(axis=1)
    ...
    def postprocess_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            ma_cols = [
                ...
                if any(ma in col for ma in ["sma_", "ema_", "wma_"])
            ]
            for col in ma_cols:
            ...
            ratio_cols = [
                col for col in processed_features.columns if "price_to_" in col or "_ratio" in col
            ]
            for col in ratio_cols:
                if col in processed_features.columns:
                    if "price_to_" in col:
                        processed_features[col] = processed_features[col].clip(
                            0.1, 10
                        )  # Price ratios
                    else:
                        processed_features[col] = processed_features[col].clip(
            ...
            for col in rsi_cols:
                if col in processed_features.columns and not col.endswith(
            ...
            bool_cols = [
                ...
                if any(
                    suffix in col
                    for suffix in ["_oversold", "_overbought", "_cross", "_above_", "_below_"]
                )
            ]
