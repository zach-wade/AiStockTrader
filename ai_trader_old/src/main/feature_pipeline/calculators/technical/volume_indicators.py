...
class VolumeIndicatorsCalculator(BaseTechnicalCalculator):
    ...
    def get_feature_names(self) -> list[str]:
        ...
        for period in self.volume_ma_periods:
            feature_names.extend([f"volume_ma_{period}", f"relative_volume_{period}"])

        # Force Index
        feature_names.extend(["force_index", "force_index_ema"])
    ...
    def _calculate_obv_indicators(
        ...
        """Calculate OBV indicators."""
        # OBV (On Balance Volume)
        obv = talib.OBV(symbol_data["close"].values, symbol_data["volume"].values)
    ...
    def _calculate_vwap_indicators(
        self, features: pd.DataFrame, symbol_data: pd.DataFrame, mask: pd.Series
    ) -> pd.DataFrame:
        """Calculate VWAP indicators."""
