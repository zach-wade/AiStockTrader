...
class MomentumIndicatorsCalculator(BaseTechnicalCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.macd_configs = config.get(
            "macd_configs",
            [(12, 26, 9), (5, 35, 5), (19, 39, 9)],  # Standard  # Aggressive  # Conservative
        )
    ...
    def get_feature_names(self) -> list[str]:
        ...
        for period in self.momentum_periods:
            features.extend(
                [f"momentum_{period}", f"roc_{period}", f"momentum_acceleration_{period}"]
            )
    ...
    def _calculate_rsi_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in self.rsi_periods:
            ...
            # RSI moving average
            features[f"rsi_{period}_sma"] = rsi.rolling(window=period, min_periods=1).mean()
    ...
    def _calculate_macd_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for fast, slow, signal in self.macd_configs:
            ...
            features[f"{prefix}_cross"] = self._detect_crossover(macd_line, signal_line)

            # MACD divergence
            divergence = self._detect_divergence(close, histogram, self.divergence_lookback)
            features[f"{prefix}_divergence"] = divergence

        return features

    def _calculate_stochastic_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in self.stoch_periods:
            ...
            features[f"stoch_cross_{period}"] = self._detect_crossover(k_percent, d_percent)
    ...
    def _calculate_other_oscillators(self, data: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_composite_indicators(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if oscillators:
            composite["momentum_composite"] = pd.concat(oscillators, axis=1).mean(axis=1)
        else:
        ...
        osc_cols = [
            ...
            if any(ind in col for ind in ["rsi", "stoch_k", "cci", "mfi"])
            and not any(
    ...
    def _calculate_stochastic(
        ...
        raw_k = safe_divide(close - lowest_low, highest_high - lowest_low) * 100
    ...
    def _calculate_williams_r(self, data: pd.DataFrame, period: int) -> pd.Series:
        ...
        williams_r = safe_divide(highest_high - data["close"], highest_high - lowest_low) * -100
    ...
    def _detect_divergence(
        self, price: pd.Series, indicator: pd.Series, lookback: int
    ) -> pd.Series:
        ...
        for i in range(lookback, len(price)):
            ...
            if len(price_peaks) >= 2 and len(ind_peaks) >= 2:
                if (
                    price_window.iloc[price_peaks[-1]] > price_window.iloc[price_peaks[-2]]
                    and ind_window.iloc[ind_peaks[-1]] < ind_window.iloc[ind_peaks[-2]]
                ):
                    divergence.iloc[i] = -1
            ...
            if len(price_troughs) >= 2 and len(ind_troughs) >= 2:
                if (
                    price_window.iloc[price_troughs[-1]] < price_window.iloc[price_troughs[-2]]
                    and ind_window.iloc[ind_troughs[-1]] > ind_window.iloc[ind_troughs[-2]]
                ):
                    divergence.iloc[i] = 1
    ...
    def _find_peaks(self, series: pd.Series, min_distance: int = 2) -> list[int]:
        ...
        for i in range(min_distance, len(series) - min_distance):
            if (
                ...
                and series.iloc[i] > series.iloc[i + min_distance]
            ):
                peaks.append(i)
