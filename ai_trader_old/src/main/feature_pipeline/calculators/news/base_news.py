...
from ..helpers import ensure_datetime_index, validate_news_data
...
class BaseNewsCalculator(BaseFeatureCalculator, ErrorHandlingMixin):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.news_config = NewsConfig(**config.get("news", {}) if config else {})
        ...
        self.tfidf = TfidfVectorizer(
            max_features=100, stop_words="english", ngram_range=(1, 2), min_df=1, max_df=0.95
        )
    ...
    def get_news_in_window(self, end_time: pd.Timestamp, window_hours: int) -> pd.DataFrame:
        ...
        try:
            ...
            mask = (self.news_data["timestamp"] >= window_start) & (
    ...
    def calculate_news_weights(
        self, news_df: pd.DataFrame, reference_time: pd.Timestamp
    ) -> pd.Series:
        """
        ...
        try:
            ...
            if "timestamp" in news_df.columns:
                time_diff = (
                    reference_time - news_df["timestamp"]
    ...
    def calculate_rolling_news_metric(
        self, data: pd.DataFrame, window_hours: int, metric_func, default_value: float = 0.0
    ) -> pd.Series:
        """
