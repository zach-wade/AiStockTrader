...
class NewsFeatureFacade(BaseNewsCalculator):
    ...
    def calculate(self, news_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            signal_features = self._calculate_advanced_signals(calc_features, validated_data)
    ...
    def _calculate_parallel(self, news_data: pd.DataFrame) -> dict[str, pd.DataFrame]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_calc = {
                executor.submit(self._safe_calculate, calc_name, calculator, news_data): calc_name
    ...
    def _calculate_sequential(self, news_data: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    def _safe_calculate(
        self, calc_name: str, calculator: BaseNewsCalculator, news_data: pd.DataFrame
    ) -> pd.DataFrame | None:
    ...
    def _calculate_interactions(self, calc_features: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    def _calculate_composite_scores(self, calc_features: dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        sentiment_abs = abs(calc_features.get("sentiment", {}).get("sentiment_composite_score", 0))
        volume_score = calc_features.get("volume", {}).get("news_intensity_score", 0)

        features["news_signal_strength"] = (sentiment_abs + volume_score) / 2

        # News divergence score (disagreement across sources)
        features["news_divergence_score"] = self._calculate_divergence_score(calc_features)
        ...
        features["information_quality_index"] = np.mean([c for c in quality_components if c != 0])
    ...
    def _calculate_advanced_signals(
        self, calc_features: dict[str, pd.DataFrame], news_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        features["news_momentum_indicator"] = np.mean([c for c in momentum_components if c != 0])

        # News alpha signal (predictive power)
        alpha_components = []

        # High sentiment with low volume (contrarian signal)
        sent_score = abs(calc_features.get("sentiment", {}).get("sentiment_composite_score", 0))
        ...
        mon_impact = calc_features.get("monetary", {}).get("monetary_impact_score", 0)
        cred_score = calc_features.get("credibility", {}).get("credibility_weighted_score", 1)
        ...
        features["information_edge_score"] = np.mean([c for c in edge_components if c != 0])

        return features

    def _calculate_divergence_score(self, calc_features: dict[str, pd.DataFrame]) -> pd.Series:
