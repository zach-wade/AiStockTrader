...
class NewsTopicCalculator(BaseNewsCalculator):
    ...
    def _calculate_dynamic_topic_features(
        ...
        for i, topic_words in enumerate(self.topics[: self.news_config.n_topics]):
            ...
            def topic_intensity_func(timestamp):
                return self._calculate_topic_intensity(
                    timestamp, topic_words, self.topic_score_window
                )
    ...
    def _calculate_topic_aggregation_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        topic_score_cols = [
            col for col in features.columns if "topic_" in col and "_score_24h" in col
        ]
        ...
        if topic_score_cols:
            ...
            for col in topic_score_cols:
                ...
                rolling_features_calc = create_rolling_features(
                    series_df, columns=[col], windows=[6], operations=["mean"], min_periods=2
                )
            ...
            if emergence_scores:
                features["news_topic_emergence_24h"] = pd.concat(emergence_scores, axis=1).max(
                    axis=1
                )
    ...
    def _calculate_advanced_topic_metrics(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_topic_score(
        ...
        try:
            ...
            if len(weights) > 0 and weights.sum() > 0:
                weighted_score = (
                    sum(score * weight for score, weight in zip(topic_scores, weights))
                    / weights.sum()
                )
            else:
    ...
    def _calculate_topic_intensity(
        self, timestamp: pd.Timestamp, keywords: list[str], window_hours: int
    ) -> float:
        ...
        try:
            ...
            intensity = sum(
                score * weight for score, weight in zip(topic_scores, sentiment_weights)
            )
