...
class NewsConfig:
    ...
    time_windows: list[int] = field(
        default_factory=lambda: [1, 6, 24, 72, 168]
    )  # Hours: 1h, 6h, 1d, 3d, 1w
    ...
    stop_words_custom: list[str] = field(
        default_factory=lambda: [
            "stock",
            "stocks",
            "share",
    ...
    feature_lag_periods: list[int] = field(
        default_factory=lambda: [0, 1, 6, 24]
    )  # Hours for lagged features
    rolling_statistics_windows: list[int] = field(
        default_factory=lambda: [6, 24, 72]
    )  # Hours for rolling stats
    percentile_levels: list[float] = field(
    ...
    def __post_init__(self):
        ...
        if (
            not -1.0
            <= self.sentiment_threshold_negative
            <= self.sentiment_threshold_positive
            <= 1.0
        ):
            raise ValueError(
                "Sentiment thresholds must be between -1 and 1, with negative <= positive"
            )
