...
class SentimentCalculator(BaseNewsCalculator):
    ...
    def calculate(self, news_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for window in self.time_windows:
                window_features = self._calculate_window_sentiment(sentiment_data, window)
            ...
            distribution_features = self._calculate_sentiment_distribution(sentiment_data)
            features = pd.concat([features, distribution_features], axis=1)

            # Calculate composite scores
            composite_features = self._calculate_composite_scores(sentiment_data, validated_data)
            features = pd.concat([features, composite_features], axis=1)

            # Calculate finance-specific sentiment
            finance_features = self._calculate_finance_sentiment(validated_data, sentiment_data)
    ...
    def _calculate_base_sentiment(self, news_data: pd.DataFrame) -> pd.DataFrame:
        ...
        sentiment_data["time_weight"] = self.calculate_time_decay(sentiment_data["timestamp"])
    ...
    def _calculate_window_sentiment(
        self, sentiment_data: pd.DataFrame, window_hours: int
    ) -> pd.DataFrame:
        ...
        if not window_data.empty:
            # Basic statistics
            features[f"sentiment_mean_{window_suffix}"] = window_data.groupby(window_data.index)[
                "sentiment"
            ].transform("mean")
            ...
            features[f"sentiment_std_{window_suffix}"] = (
                window_data.groupby(window_data.index)["sentiment"].transform("std").fillna(0)
            )

            # Sentiment skewness
            features[f"sentiment_skew_{window_suffix}"] = (
                window_data.groupby(window_data.index)["sentiment"].transform("skew").fillna(0)
            )

            # Sentiment ratios
            features[f"sentiment_positive_ratio_{window_suffix}"] = self._calculate_sentiment_ratio(
                window_data, "positive"
            )
            features[f"sentiment_negative_ratio_{window_suffix}"] = self._calculate_sentiment_ratio(
                window_data, "negative"
            )
            features[f"sentiment_neutral_ratio_{window_suffix}"] = self._calculate_sentiment_ratio(
                window_data, "neutral"
            )

            # Extreme sentiment ratio
            features[f"sentiment_extreme_ratio_{window_suffix}"] = self._calculate_extreme_ratio(
                window_data
            )

            # Subjectivity statistics
            features[f"subjectivity_mean_{window_suffix}"] = window_data.groupby(window_data.index)[
                "subjectivity"
            ].transform("mean")

            features[f"subjectivity_std_{window_suffix}"] = (
                window_data.groupby(window_data.index)["subjectivity"].transform("std").fillna(0)
            )

        return features.fillna(0)

    def _calculate_sentiment_dynamics(self, sentiment_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Sentiment momentum
        features["sentiment_momentum_1h_6h"] = safe_divide(sent_1h - sent_6h, sent_6h.abs() + 0.1)
        ...
        features["sentiment_acceleration"] = (
            features["sentiment_momentum_1h_6h"] - features["sentiment_momentum_6h_24h"]
        )

        # Sentiment volatility (24h rolling)
        # Create DataFrame for rolling features
        sent_df = pd.DataFrame(
            {"sentiment": sentiment_data["sentiment"]}, index=sentiment_data.index
        )
        rolling_features = create_rolling_features(
            sent_df, columns=["sentiment"], windows=["24H"], operations=["std"], min_periods=1
        )
        features["sentiment_volatility_24h"] = rolling_features["sentiment_rolling_std_24H"].fillna(
            0
        )

        # Sentiment trend strength
        features["sentiment_trend_strength"] = self._calculate_trend_strength(sentiment_data)

        return features

    def _calculate_sentiment_distribution(self, sentiment_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Sentiment range
        features["sentiment_range"] = grouped.transform(lambda x: x.max() - x.min()).fillna(0)
        ...
        features["sentiment_entropy"] = grouped.transform(self._calculate_sentiment_entropy).fillna(
            0
        )
    ...
    def _calculate_composite_scores(
        self, sentiment_data: pd.DataFrame, news_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        avg_credibility = sentiment_data.groupby(sentiment_data.index)[
            "credibility_weight"
        ].transform("mean")
        ...
        sentiment_std = (
            sentiment_data.groupby(sentiment_data.index)["sentiment"].transform("std").fillna(0)
        )

        features["sentiment_agreement_score"] = safe_divide(
            1, (1 + sentiment_std), default_value=1.0
        )

        # Intensity score (absolute sentiment * volume)
        abs_sentiment = sentiment_data.groupby(sentiment_data.index)["sentiment"].transform(
            lambda x: x.abs().mean()
        )
    ...
    def _calculate_finance_sentiment(
        self, news_data: pd.DataFrame, sentiment_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        features["finance_adjusted_sentiment"] = sentiment_data.groupby(sentiment_data.index)[
            "sentiment"
        ].transform("mean")

        # Earnings-related sentiment
        earnings_mask = news_data["headline"].str.contains(
            "earnings|revenue|profit|loss|guidance", case=False, na=False
        )
        ...
        analyst_mask = news_data["headline"].str.contains(
            "analyst|upgrade|downgrade|rating|target", case=False, na=False
        )
        ...
        for term in market_terms:
            ...
            if term_mask.any():
                term_sent = (
                    sentiment_data[term_mask]
                    .groupby(sentiment_data[term_mask].index)["sentiment"]
                    .transform("mean")
                )
                market_sentiment += term_sent.reindex(features.index, fill_value=0)

        features["market_sentiment_alignment"] = safe_divide(
            market_sentiment, len(market_terms), default_value=0.0
        )

        return features

    def _adjust_finance_sentiment(self, text: str, base_sentiment: float) -> float:
    ...
    def _calculate_weighted_sentiment(self, window_data: pd.DataFrame) -> pd.Series:
    ...
    def _calculate_sentiment_ratio(
        self, window_data: pd.DataFrame, sentiment_type: str
    ) -> pd.Series:
        ...
        def calc_ratio(group):
            ...
            if sentiment_type == "positive":
                ...
                count = (
                    (group["sentiment"] >= self.neutral_range[0])
                    & (group["sentiment"] <= self.neutral_range[1])
                ).sum()
    ...
    def _calculate_extreme_ratio(self, window_data: pd.DataFrame) -> pd.Series:
    ...
    def _calculate_trend_strength(self, sentiment_data: pd.DataFrame) -> pd.Series:
        ...
        rolling_features = create_rolling_features(
            sent_df, columns=["sentiment"], windows=["24H"], operations=["mean"], min_periods=1
        )
    ...
    def _calculate_sentiment_concentration(self, sentiments: pd.Series) -> float:
    ...
    def _calculate_category_sentiment(
        self, sentiment_data: pd.DataFrame, category_mask: pd.Series
    ) -> pd.Series:
        ...
        if category_mask.any():
            ...
            grouped_sentiment = category_data.groupby(category_data.index)["sentiment"].mean()
            result.update(grouped_sentiment)

        return result.fillna(0)

    def _get_window_average(self, data: pd.DataFrame, window_hours: int, column: str) -> pd.Series:
