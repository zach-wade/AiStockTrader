...
class NewsCredibilityCalculator(BaseNewsCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.mainstream_sources = self.config.get(
            "mainstream_sources", ["reuters", "bloomberg", "wsj", "marketwatch", "cnbc", "ap", "ft"]
        )

        self.alternative_sources = self.config.get(
            "alternative_sources",
            ["reddit", "twitter", "seekingalpha", "stocktwits", "benzinga", "yahoo"],
        )

        self.premium_sources = self.config.get(
            "premium_sources", ["reuters", "bloomberg", "wsj", "ft"]
        )
    ...
    def _calculate_source_diversity_features(
    ...
    def _calculate_source_type_features(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        """Calculate source type ratio features."""

        def mainstream_ratio_func(timestamp):
            return self._calculate_source_type_ratio(
                timestamp, self.mainstream_sources, self.diversity_window
            )

        def alternative_ratio_func(timestamp):
            return self._calculate_source_type_ratio(
                timestamp, self.alternative_sources, self.diversity_window
            )

        def premium_ratio_func(timestamp):
            return self._calculate_source_type_ratio(
                timestamp, self.premium_sources, self.diversity_window
            )
        ...
        features["news_mainstream_vs_alternative"] = mainstream_series / (
            alternative_series + self.numerical_tolerance
        )
    ...
    def _calculate_coverage_features(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        """Calculate coverage analysis features."""
    ...
    def _calculate_advanced_credibility_features(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_source_type_ratio(
        self, timestamp: pd.Timestamp, source_list: list[str], window_hours: int
    ) -> float:
        ...
        try:
            ...
            source_count = (
                window_news["source"].str.lower().isin([s.lower() for s in source_list]).sum()
            )
    ...
    def _calculate_weighted_credibility(self, timestamp: pd.Timestamp, window_hours: int) -> float:
        ...
        try:
            ...
            credibility_scores = sources_lower.map(
                lambda x: self.news_config.source_weights.get(x, 0.5)
            ).values
            ...
            weighted_credibility = (
                sum(c * w for c, w in zip(credibility_scores, weights)) / weights.sum()
            )
    ...
    def _calculate_source_reliability_score(
        ...
        try:
            ...
            mainstream_ratio = self._calculate_source_type_ratio(
                timestamp, self.mainstream_sources, window_hours
            )
            premium_ratio = self._calculate_source_type_ratio(
                timestamp, self.premium_sources, window_hours
            )
    ...
    def _calculate_cross_source_validation(
        self, timestamp: pd.Timestamp, window_hours: int
    ) -> float:
    ...
    def _calculate_source_agreement_ratio(
        self, timestamp: pd.Timestamp, window_hours: int
    ) -> float:
    ...
    def _calculate_exclusive_coverage_ratio(
        self, timestamp: pd.Timestamp, window_hours: int
    ) -> float:
    ...
    def _calculate_redundant_coverage_ratio(
        self, timestamp: pd.Timestamp, window_hours: int
    ) -> float:
    ...
    def _calculate_source_weight_variance(
        self, timestamp: pd.Timestamp, window_hours: int
    ) -> float:
    ...
    def _calculate_credibility_confidence(
        self, timestamp: pd.Timestamp, window_hours: int
    ) -> float:
    ...
    def _calculate_information_quality_score(
        self, timestamp: pd.Timestamp, window_hours: int
    ) -> float:
        ...
        try:
            ...
            info_quality = (
                source_quality * 0.3 + trust_score * 0.3 + confidence * 0.2 + network_effect * 0.2
            )
