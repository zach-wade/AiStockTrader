...
class MoneynessCalculator(BaseOptionsCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        logger.debug(
            f"Initialized MoneynessCalculator with ATM threshold: ±{self.atm_threshold:.1%}"
        )
    ...
    def _calculate_volume_by_moneyness(
        ...
        try:
            ...
            for option_type in ["call", "put"]:
                ...
                volume_by_moneyness[f"itm_{option_type}"] = self.options_chain.loc[
                    itm_mask & type_mask, "volume"
                ].sum()
                volume_by_moneyness[f"atm_{option_type}"] = self.options_chain.loc[
                    atm_mask & type_mask, "volume"
                ].sum()
                volume_by_moneyness[f"otm_{option_type}"] = self.options_chain.loc[
                    otm_mask & type_mask, "volume"
                ].sum()
    ...
    def _estimate_volume_by_moneyness(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_oi_by_moneyness(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if (
                ...
                or "openInterest" not in self.options_chain.columns
            ):
            ...
            for option_type in ["call", "put"]:
                ...
                oi_by_moneyness[f"itm_{option_type}"] = self.options_chain.loc[
                    itm_mask & type_mask, "openInterest"
                ].sum()
                oi_by_moneyness[f"atm_{option_type}"] = self.options_chain.loc[
                    atm_mask & type_mask, "openInterest"
                ].sum()
                oi_by_moneyness[f"otm_{option_type}"] = self.options_chain.loc[
                    otm_mask & type_mask, "openInterest"
                ].sum()
    ...
    def _calculate_pc_ratios_by_moneyness(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_concentration_metrics(
        ...
        try:
            ...
            for moneyness in ["itm", "atm", "otm"]:
                call_volume = (
                    features.get(f"{moneyness}_call_volume", pd.Series([0])).iloc[-1]
                    if len(features) > 0
                    else 0
                )
                put_volume = (
                    features.get(f"{moneyness}_put_volume", pd.Series([0])).iloc[-1]
                    if len(features) > 0
                    else 0
                )
            ...
            if total_volume > 0:
                ...
                concentrations = [
                    safe_divide(v, total_volume, default_value=0.0)
                    for v in moneyness_volumes.values()
                ]
    ...
    def _calculate_strike_distribution(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        """Calculate strike distribution analysis."""
        try:
            ...
            if len(strikes) > 0 and sum(volumes) > 0:
                ...
                range_coverage = safe_divide(
                    (max_strike - min_strike), current_price, default_value=0.0
                )
                ...
                weighted_strike = safe_divide(
                    sum(strike * volume for strike, volume in zip(strikes, volumes)),
                    total_volume,
    ...
    def _calculate_advanced_moneyness_metrics(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if all(f"{m}_call_volume" in features.columns for m in ["itm", "atm", "otm"]):
                itm_volume = (
                    (features["itm_call_volume"].iloc[-1] + features["itm_put_volume"].iloc[-1])
                    if len(features) > 0
                    else 0
                )
                otm_volume = (
                    (features["otm_call_volume"].iloc[-1] + features["otm_put_volume"].iloc[-1])
                    if len(features) > 0
                    else 0
                )
                total_volume = itm_volume + otm_volume

                moneyness_momentum = safe_divide(
                    (otm_volume - itm_volume), total_volume, default_value=0.0
                )
            ...
            if "volume_concentration_atm" in features.columns:
                atm_concentration = (
                    features["volume_concentration_atm"].iloc[-1] if len(features) > 0 else 0.33
                )
            ...
            otm_call_vol = (
                features.get("otm_call_volume", pd.Series([0])).iloc[-1] if len(features) > 0 else 0
            )
            otm_put_vol = (
                features.get("otm_put_volume", pd.Series([0])).iloc[-1] if len(features) > 0 else 0
            )
            ...
            total_volume = sum(
                [
                    (
                        features.get(f"{m}_{t}_volume", pd.Series([0])).iloc[-1]
                        if len(features) > 0
                        else 0
                    )
            ...
            itm_call_vol = (
                features.get("itm_call_volume", pd.Series([0])).iloc[-1] if len(features) > 0 else 0
            )
            itm_put_vol = (
                features.get("itm_put_volume", pd.Series([0])).iloc[-1] if len(features) > 0 else 0
            )
            ...
            if "moneyness_concentration_index" in features.columns:
                concentration_index = (
                    features["moneyness_concentration_index"].iloc[-1]
                    if len(features) > 0
                    else 0.33
                )
