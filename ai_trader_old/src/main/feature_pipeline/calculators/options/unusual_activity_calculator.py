...
class UnusualActivityCalculator(BaseOptionsCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        logger.debug(
            f"Initialized UnusualActivityCalculator with {self.unusual_threshold}x threshold"
        )
    ...
    def _calculate_volume_anomalies(
        ...
        try:
            ...
            if has_avg_volume:
                ...
                volume_ratios = safe_divide(
                    self.options_chain["volume"], avg_volume, default_value=0.0
                )
    ...
    def _estimate_volume_anomalies(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_directional_unusual_activity(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            if self.options_chain is None or self.options_chain.empty:
                ...
                total_unusual = (
                    features.get("unusual_option_volume", pd.Series([0])).iloc[-1]
                    if len(features) > 0
                    else 0
                )
            ...
            if has_avg_volume:
                ...
                volume_oi_ratios = safe_divide(self.options_chain["volume"], oi, default_value=0.0)
                unusual_mask = (volume_oi_ratios >= self.oi_ratio_threshold) | (
                    self.options_chain["volume"] >= self.block_size
                )
            ...
            unusual_sentiment = safe_divide(
                (unusual_call_volume - unusual_put_volume), total_unusual_volume, default_value=0.0
            )
    ...
    def _calculate_block_trade_analysis(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            if self.options_chain is None or self.options_chain.empty:
                ...
                total_volume = (
                    features.get("unusual_option_volume", pd.Series([1000])).iloc[-1]
                    if len(features) > 0
                    else 1000
                )
                ...
                features["mega_trade_count"] = max(
                    0, int(total_volume / (self.large_size * 2) * 0.02)
                )
    ...
    def _calculate_premium_flow_anomalies(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            unusual_volume_threshold = self.options_chain["volume"].quantile(
                0.9
            )  # Top 10% by volume
            ...
            if not high_volume_options.empty:
                ...
                concentration = safe_divide(
                    top_premium_options["premium_value"].sum(), total_premium, default_value=0.0
                )
        ...
        try:
            ...
            if total_unusual_volume > 0:
                ...
                if has_avg_volume:
                    ...
                    oi = self.options_chain["openInterest"].fillna(0)
                    volume_oi_ratios = safe_divide(
                        self.options_chain["volume"], oi, default_value=0.0
                    )
                    unusual_mask = (volume_oi_ratios >= self.oi_ratio_threshold) | (
                        self.options_chain["volume"] >= self.block_size
                    )
                ...
                large_unusual_volume = unusual_options[
                    unusual_options["volume"] >= self.block_size
                ]["volume"].sum()
                small_unusual_volume = unusual_options[unusual_options["volume"] < self.block_size][
                    "volume"
                ].sum()

                # Smart money ratio
                smart_money_ratio = safe_divide(
                    large_unusual_volume, total_unusual_volume, default_value=0.0
                )
    ...
    def _calculate_advanced_anomaly_metrics(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            unusual_count = (
                features.get("unusual_option_count", pd.Series([0])).iloc[-1]
                if len(features) > 0
                else 0
            )
            unusual_volume = (
            ...
            if unusual_count > 0:
                burst_intensity = min(
                    unusual_count * safe_log(1 + safe_divide(avg_unusual_volume, 1000)), 10.0
                )
            ...
            volume_spike_score = (
                features.get("volume_spike_score", pd.Series([1.0])).iloc[-1]
                if len(features) > 0
                else 1.0
            )
            ...
            unusual_ratio = (
                features.get("unusual_volume_ratio", pd.Series([0])).iloc[-1]
                if len(features) > 0
                else 0
            )
            block_ratio = (
                features.get("block_trade_ratio", pd.Series([0])).iloc[-1]
                if len(features) > 0
                else 0
            )
