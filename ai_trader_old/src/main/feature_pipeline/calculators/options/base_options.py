...
from ..helpers import safe_divide, safe_log, safe_sqrt, validate_options_data
...
class BaseOptionsCalculator(BaseFeatureCalculator, ErrorHandlingMixin, ABC):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        logger.debug(
            f"Initialized {self.__class__.__name__} with {len(self.options_config.expiry_windows)} expiry windows"
        )
    ...
    def validate_options_chain(self, options_chain: pd.DataFrame) -> pd.DataFrame:
        ...
        is_valid, errors = validate_options_data(
            options_chain, min_rows=self.options_config.min_options_required
        )
        if not is_valid:
        ...
        if all(col in cleaned_chain.columns for col in ["volume", "openInterest"]):
            valid_mask = (cleaned_chain["volume"] >= self.options_config.min_volume) | (
        ...
        if "lastPrice" in cleaned_chain.columns:
            price_mask = (cleaned_chain["lastPrice"] >= self.options_config.min_option_price) & (
    ...
    def calculate_time_to_expiry(
        self, expiry_date: str | datetime, current_date: datetime | None = None
    ) -> float:
        """
    ...
    def estimate_implied_volatility(self, price_data: pd.Series, window: int = 20) -> pd.Series:
        ...
        return estimated_iv.clip(
            self.options_config.min_volatility, self.options_config.max_volatility
        )
    ...
    def black_scholes_price(
        ...
        try:
            d1 = safe_divide(
                (safe_log(safe_divide(S, K)) + (r + 0.5 * sigma**2) * T), (sigma * safe_sqrt(T))
            )
