...
class BlackScholesCalculator(BaseOptionsCalculator):
    ...
    def get_feature_names(self) -> list[str]:
        ...
        features.extend(
            ["implied_volatility", "iv_premium", "iv_rank", "iv_percentile", "realized_iv_spread"]
        )

        # Sensitivity analysis
        features.extend(
            ["spot_sensitivity", "vol_sensitivity", "time_sensitivity", "convexity_measure"]
        )
    ...
    def calculate(self, options_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            sensitivity_features = self._calculate_sensitivities(validated_data, greeks_features)
    ...
    def _calculate_theoretical_prices(self, options_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "call_price" in options_data.columns:
            ...
            features["call_mispricing_pct"] = (
                safe_divide(features["call_mispricing"], bs_call) * 100
            )

        if "put_price" in options_data.columns:
            features["put_mispricing"] = options_data["put_price"] - bs_put
            features["put_mispricing_pct"] = safe_divide(features["put_mispricing"], bs_put) * 100

        # Average mispricing
        if "call_mispricing_pct" in features and "put_mispricing_pct" in features:
            features["avg_mispricing"] = safe_divide(
                (features["call_mispricing_pct"].abs() + features["put_mispricing_pct"].abs()),
                2,
                default_value=0.0,
            )
    ...
    def _calculate_greeks(self, options_data: pd.DataFrame) -> pd.DataFrame:
        ...
        features["gamma"] = self._calculate_gamma(spot, strike, time_to_expiry, volatility, d1)
        ...
        features["call_rho"] = safe_divide(
            self._calculate_call_rho(strike, time_to_expiry, risk_free_rate, d2),
            100,
            default_value=0.0,
        )  # Per 1% change in rate
        features["put_rho"] = safe_divide(
            self._calculate_put_rho(strike, time_to_expiry, risk_free_rate, d2),
            100,
            default_value=0.0,
        )
        ...
        # Dollar Greeks
        features["dollar_gamma"] = safe_divide(
            features["gamma"] * spot * spot, 100, default_value=0.0
        )
    ...
    def _calculate_higher_order_greeks(self, options_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Vanna (dDelta/dVol)
        features["vanna"] = self._calculate_vanna(spot, time_to_expiry, volatility, d1)
        ...
        # Vomma (dVega/dVol)
        features["vomma"] = self._calculate_vomma(spot, time_to_expiry, volatility, d1)

        # Speed (dGamma/dSpot)
        features["speed"] = self._calculate_speed(spot, strike, time_to_expiry, volatility, d1)

        # Zomma (dGamma/dVol)
        features["zomma"] = self._calculate_zomma(spot, time_to_expiry, volatility, d1)
    ...
    def _calculate_iv_metrics(
        self, options_data: pd.DataFrame, price_features: pd.DataFrame
    ) -> pd.DataFrame:
        """Calculate implied volatility metrics."""
        features = pd.DataFrame(index=options_data.index)
        ...
        if "call_price" in options_data.columns:
            ...
            features["implied_volatility"] = self._calculate_implied_volatility(options_data, "put")
        else:
        ...
        # IV percentile
        features["iv_percentile"] = self._calculate_iv_percentile(features["implied_volatility"])
    ...
    def _calculate_sensitivities(
        self, options_data: pd.DataFrame, greeks_features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        features["spot_sensitivity"] = safe_divide(
            spot_sens, len(self.spot_bumps), default_value=0.0
        )
        ...
        features["time_sensitivity"] = safe_divide(
            time_sens, len(self.time_horizons), default_value=0.0
        )
    ...
    def _calculate_d1(
        ...
        return safe_divide(
            (
                safe_log(safe_divide(spot, strike, default_value=1.0))
                + (rate + 0.5 * volatility**2) * time
            ),
            (volatility * safe_sqrt(time)),
            default_value=0.0,
        )
    ...
    def _calculate_gamma(
        ...
        """Calculate gamma."""
        return safe_divide(
            stats.norm.pdf(d1), (spot * volatility * safe_sqrt(time)), default_value=0.0
        )
    ...
    def _calculate_vega(self, spot: pd.Series, time: pd.Series, d1: pd.Series) -> pd.Series:
        """Calculate vega."""
        return spot * stats.norm.pdf(d1) * safe_sqrt(time)

    def _calculate_call_rho(
        self, strike: pd.Series, time: pd.Series, rate: float, d2: pd.Series
    ) -> pd.Series:
    ...
    def _calculate_vanna(
        self, spot: pd.Series, time: pd.Series, volatility: pd.Series, d1: pd.Series
    ) -> pd.Series:
    ...
    def _calculate_charm(
        ...
        charm = safe_divide(
            ...
            (2 * time * volatility * safe_sqrt(time)),
            default_value=0.0,
        )
    ...
    def _calculate_speed(
        ...
        return safe_divide(
            ...
            (spot * volatility * safe_sqrt(time)),
            default_value=0.0,
        )
    ...
    def _calculate_color(
        ...
        color = safe_divide(
            -stats.norm.pdf(d1)
            * (2 * rate * time - d2 * volatility * safe_sqrt(time) - d1 * (1 + d1 * d2)),
            (2 * spot * time * volatility * safe_sqrt(time)),
            default_value=0.0,
        )
    ...
    def _calculate_implied_volatility(
        self, options_data: pd.DataFrame, option_type: str
    ) -> pd.Series:
        ...
        for idx, row in options_data.iterrows():
            ...
            for _ in range(self.iv_iterations):
                ...
                iv += safe_divide(
                    price_diff, (vega + 1e-10), default_value=0.0
    ...
    def _calculate_iv_rank(self, iv: pd.Series) -> pd.Series:
        ...
        rolling_features = create_rolling_features(
            iv_df, columns=["iv"], windows=[window], operations=["min", "max"], min_periods=20
        )
    ...
    def _calculate_iv_percentile(self, iv: pd.Series) -> pd.Series:
        ...
        iv_percentile = iv.rolling(window=window, min_periods=20).apply(percentile_rank, raw=False)
