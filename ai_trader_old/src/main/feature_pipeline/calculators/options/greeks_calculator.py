...
class GreeksCalculator(BaseOptionsCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        logger.debug(
            f"Initialized GreeksCalculator with pin risk range: ±{self.pin_risk_range:.1%}"
        )
    ...
    def _calculate_aggregate_greeks(
        ...
        try:
            ...
            has_greeks = all(
                col in self.options_chain.columns for col in ["delta", "gamma", "theta", "vega"]
            )
    ...
    def _calculate_from_chain_greeks(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            total_gamma_exposure = (
                self.options_chain["gamma"] * self.options_chain["openInterest"]
            ).sum()
            ...
            features["positive_gamma_exposure"] = (
                positive_gamma["gamma"] * positive_gamma["openInterest"]
            ).sum()
            features["negative_gamma_exposure"] = (
                negative_gamma["gamma"] * negative_gamma["openInterest"]
            ).sum()
            ...
            total_theta_exposure = (
                self.options_chain["theta"] * self.options_chain["openInterest"]
            ).sum()
    ...
    def _calculate_estimated_greeks(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if self.options_chain is not None and not self.options_chain.empty:
                try:
                    ...
                    time_to_expiry = safe_divide(
                        self.options_chain.get("daysToExpiration", 30).fillna(30),
                        365.25,
                        default_value=0.0,
                    )
                    ...
                    delta_values = self.options_chain.apply(
                        lambda row: self.calculate_delta(
                            current_price,
                            row["strike"],
                            time_to_expiry[row.name],
                            self.greeks_config["risk_free_rate"],
                            estimated_iv,
                            row["optionType"],
                        ),
                        axis=1,
    ...
    def _estimate_aggregate_greeks(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_pin_risk_analysis(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if "gamma" in self.options_chain.columns:
                pin_risk_gamma = (
                    near_money_options["gamma"] * near_money_options["openInterest"]
                ).sum()
                ...
                gamma_concentration = safe_divide(
                    abs(pin_risk_gamma), abs(total_gamma), default_value=0.0
                )
            ...
            if total_volume > 0:
                pin_risk_level = safe_divide(
                    total_volume_near_money, total_volume, default_value=0.0
                )
    ...
    def _calculate_individual_greeks_analysis(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if "net_delta_exposure" in features.columns:
                ...
                features["delta_hedge_ratio"] = safe_divide(
                    abs(net_delta), current_price, default_value=0.0
                )
            ...
            if "total_gamma_exposure" in features.columns:
                ...
                normalized_gamma = min(
                    safe_divide(abs(total_gamma), self.max_gamma_exposure, default_value=0.0), 1.0
                )
            ...
            if "total_vega_exposure" in features.columns:
                ...
                normalized_vega = min(
                    safe_divide(abs(total_vega), self.max_vega_exposure, default_value=0.0), 1.0
                )
    ...
    def _calculate_advanced_greeks_metrics(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            delta_exposure = (
                features.get("net_delta_exposure", pd.Series([0])).iloc[-1]
                if len(features) > 0
                else 0
            )
            theta_exposure = (
                features.get("total_theta_exposure", pd.Series([0])).iloc[-1]
                if len(features) > 0
                else 0
            )
            ...
            vega_exposure = (
                features.get("total_vega_exposure", pd.Series([0])).iloc[-1]
                if len(features) > 0
                else 0
            )
            ...
            gamma_exposure = (
                features.get("total_gamma_exposure", pd.Series([0])).iloc[-1]
                if len(features) > 0
                else 0
            )
    ...
    def _calculate_greeks_ratios(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if sum(greeks_values) > 0:
                ...
                normalized_greeks = [
                    safe_divide(g, sum(greeks_values), default_value=0.0) for g in greeks_values
                ]
            ...
            if (
                ...
                and "lastPrice" in self.options_chain.columns
            ):
                total_premium = (
                    self.options_chain["lastPrice"] * self.options_chain["openInterest"] * 100
                ).sum()
