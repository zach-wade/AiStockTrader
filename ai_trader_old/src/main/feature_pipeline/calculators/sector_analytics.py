...
class SectorAnalyticsCalculator(BaseFeatureCalculator):
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.sector_config = SectorConfig(**config.get("sector", {}) if config else {})
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Successfully calculated {total_features} sector features across {len(feature_counts)} categories"
        )
                ...
                if sector_df is not None and not sector_df.empty:
                    ...
                    if not data.empty:
                        start_date = data.index.min() - timedelta(
                            days=max(self.sector_config.momentum_windows)
                        )
                        end_date = data.index.max()

                        sector_df = sector_df[
                            (sector_df.index >= start_date) & (sector_df.index <= end_date)
                        ].copy()
    ...
    def postprocess_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        ratio_columns = [
            ...
            if any(
                keyword in col
                for keyword in ["ratio", "correlation", "relative_strength", "vs_market"]
            )
        ]
        ...
        breadth_columns = [
            ...
            if any(
                keyword in col
                for keyword in ["breadth", "momentum", "advance_decline", "dispersion"]
            )
        ]
        ...
        if self.sector_data:
            ...
            if rs_scores:
                ...
                features["rs_percentile"] = (
                    features["avg_relative_strength"].rolling(252).rank(pct=True)
                )
    ...
    def _add_sector_momentum(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for sector, sector_df in self.sector_data.items():
            ...
            if mom_scores:
                ...
                # Momentum rank
                features[f"{sector}_momentum_rank"] = (
                    features[f"{sector}_momentum"].rolling(60).rank(pct=True)
                )
        ...
        momentum_cols = [
            col
            for col in features.columns
            if col.endswith("_momentum") and not col.endswith("_rank")
        ]
        if momentum_cols:
            ...
            features["strong_momentum_sectors"] = (
                momentum_df > momentum_df.quantile(0.8, axis=1).values[:, None]
            ).sum(axis=1)
    ...
    def _add_economic_cycle(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if cyclical_returns and defensive_returns:
            ...
            # Relative performance
            features["cyclical_defensive_ratio"] = (1 + avg_cyclical.rolling(20).mean()) / (
            ...
            features["sector_risk_on"] = (
                ...
                > features["cyclical_defensive_ratio"].rolling(20).mean()
            ).astype(int)
        ...
        if "technology_rotation_score" in features and "financials_rotation_score" in features:
            features["growth_value_tilt"] = (
                features["technology_rotation_score"] - features["financials_rotation_score"]
            )
    ...
    def _add_sector_correlation(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for sector, sector_df in self.sector_data.items():
            ...
            # Rolling correlation
            corr = aligned_symbol.rolling(self.sector_config.correlation_window).corr(
                aligned_sector
            )
        ...
        if correlations:
            ...
            features["decorrelated"] = (features["highest_correlation"] < 0.5).astype(int)
    ...
    def _add_industry_analysis(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if industry_momentum:
            ...
            features["industry_concentration"] = momentum_df.abs().max(
                axis=1
            ) / momentum_df.abs().mean(axis=1)
    ...
    def _add_sector_divergence(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if divergence_scores:
            # Maximum divergence
            features["max_sector_divergence"] = (
                pd.concat(divergence_scores, axis=1).abs().max(axis=1)
            )

            # Divergence direction
            features["positive_divergence"] = (
                (pd.concat(divergence_scores, axis=1) > 0.1).any(axis=1).astype(int)
            )
    ...
    def _add_intersector_correlations(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for sector1, sector2 in relationships:
            if sector1 in self.sector_data and sector2 in self.sector_data:
                if (
                    "close" in self.sector_data[sector1].columns
                    and "close" in self.sector_data[sector2].columns
                ):
    ...
    def _create_empty_features(self, index: pd.Index) -> pd.DataFrame:
        ...
        for col in feature_names:
            if any(keyword in col for keyword in ["ratio", "correlation", "relative_strength"]):
                ...
                keyword in col for keyword in ["rotation", "thrust", "early_cycle", "late_cycle"]
            ):
    ...
    def _validate_output_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in features.columns:
            if (
                features[col].replace([np.inf, -np.inf], np.nan).isna().sum()
                != features[col].isna().sum()
            ):
                inf_cols.append(col)
    ...
    def _get_sector_for_industry(self, industry: str) -> str | None:
