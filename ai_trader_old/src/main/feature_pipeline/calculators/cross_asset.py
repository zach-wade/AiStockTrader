...
class CrossAssetCalculator(BaseFeatureCalculator):
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.cross_config = CrossAssetConfig(**config.get("cross_asset", {}) if config else {})
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                ...
                market_count = (
                    len(self.cross_config.market_indices)
                    * (len(self.cross_config.corr_windows) * 2 + 1)
                    + 2
                )
            ...
            if len(self.market_data) >= self.cross_config.n_components:
                try:
                    ...
                    pca_count = (
                        self.cross_config.n_components + 2
                    )  # components + explained variance + loading
                ...
                logger.debug(
                    f"Insufficient market data for PCA: {len(self.market_data)} < {self.cross_config.n_components}"
                )
    ...
    def _create_empty_features(self, index: pd.Index) -> pd.DataFrame:
        ...
        try:
            ...
            empty_features = pd.DataFrame(
                data=0, index=index, columns=feature_names  # Fill with zeros instead of NaN
            )
            logger.warning(
                f"Created empty cross-asset features DataFrame with {len(feature_names)} columns"
            )
    ...
    def validate_inputs(self, data: pd.DataFrame) -> bool:
        ...
        if len(data) < max(self.cross_config.corr_windows):
            logger.warning("Insufficient data for cross-asset calculation")
            return False
    ...
    def get_feature_names(self) -> list[str]:
        ...
        for index_symbol in self.cross_config.market_indices:
            ...
            for window in self.cross_config.corr_windows:
                feature_names.extend(
                    [f"corr_{index_symbol}_{window}d", f"beta_{index_symbol}_{window}d"]
                )
        ...
        feature_names.extend(
            [
                "best_pair_symbol",
                "best_pair_correlation",
                "pair_spread",
                "pair_spread_zscore",
                "pair_trade_long",
                "pair_trade_short",
                "pair_cointegration_pvalue",
                "is_cointegrated",
            ]
        )
    ...
    def preprocess_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(processed_data) < min_required:
                logger.warning(
                    f"Insufficient data for cross-asset analysis: {len(processed_data)} < {min_required}"
                )
                return pd.DataFrame()
            ...
            if available_ohlc:
                processed_data[available_ohlc] = processed_data[available_ohlc].fillna(
                    method="ffill"
                )
            ...
            for symbol, market_df in self.market_data.items():
                if market_df is not None and not market_df.empty and "close" in market_df.columns:
                    ...
                    if len(clean_market) > 10:  # Minimum data requirement
                        ...
                        logger.debug(
                            f"Insufficient data for {symbol}, excluding from cross-asset analysis"
                        )
                else:
            ...
            logger.info(
                f"Preprocessed cross-asset data: {len(processed_data)} rows, {len(self.market_data)} market assets"
            )
    ...
    def postprocess_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            corr_cols = [
                col for col in processed_features.columns if "corr" in col and "pvalue" not in col
            ]
            ...
            rel_strength_cols = [
                col for col in processed_features.columns if col.startswith("rel_strength_")
            ]
            ...
            spread_cols = [
                col for col in processed_features.columns if "spread" in col and "zscore" not in col
            ]
            ...
            if "pca_explained_variance" in processed_features.columns:
                processed_features["pca_explained_variance"] = processed_features[
            ...
            if "market_factor_loading" in processed_features.columns:
                processed_features["market_factor_loading"] = processed_features[
                    "market_factor_loading"
                ].clip(-2, 2)
            ...
            for col in categorical_cols:
                if col in processed_features.columns:
                    ...
                    try:
                        processed_features[col] = pd.Categorical(
                            processed_features[col].astype(str)
                        ).codes
            ...
            for col in binary_cols:
                if col in processed_features.columns:
                    processed_features[col] = (
                        processed_features[col].fillna(0).astype(int).clip(0, 1)
                    )
            ...
            corr_default_cols = corr_cols + [
                "avg_market_correlation",
                "market_correlation_dispersion",
            ]
    ...
    def _add_sector_analysis(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if sector_correlations:
            ...
            features["sector_rotation_score"] = (
                features["sector_dispersion"] * features["max_sector_corr"]
            )
            ...
            defensive_corr = pd.DataFrame(
                {s: sector_correlations.get(s, 0) for s in defensive}
            ).mean(axis=1)
            cyclical_corr = pd.DataFrame({s: sector_correlations.get(s, 0) for s in cyclical}).mean(
                axis=1
            )
    ...
    def _add_risk_sentiment(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if "VIX" in self.market_data:
            ...
            features["vix_correlation"] = (
                aligned_returns.rolling(20).corr(aligned_vix).reindex(data.index)
            )
            features["fear_gauge"] = (
                features["vix_correlation"] * -1
            )  # Negative correlation is fearful
    ...
    def _add_intermarket_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if "UUP" in self.market_data:
            ...
            features["dollar_correlation"] = (
                aligned_returns.rolling(20).corr(aligned_dollar).reindex(data.index)
            )
        ...
        if "EEM" in self.market_data:
            ...
            features["emerging_markets_corr"] = (
                aligned_returns.rolling(20).corr(aligned_eem).reindex(data.index)
            )
    ...
    def _add_divergence_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if "SPY" in self.market_data:
            ...
            features["return_divergence"] = (rolling_return - rolling_spy_return).reindex(
                data.index
            )

            # Divergence direction
            features["positive_divergence"] = (
                ((rolling_return > 0) & (rolling_spy_return < 0)).astype(int).reindex(data.index)
            )
        ...
        if len(self.market_data) > 10:
            ...
            features["breadth_divergence"] = (
                (price_direction != breadth_direction).astype(int).reindex(data.index)
            )
    ...
    def _add_pair_trading_features(
