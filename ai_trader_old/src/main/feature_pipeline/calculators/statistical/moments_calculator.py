...
from scipy.stats import anderson, jarque_bera, normaltest
...
class MomentsCalculator(BaseStatisticalCalculator):
        ...
        for window in self.stat_config.moment_windows:
            feature_names.extend(
                [
                    ...
                    f"skew_{window}",
                    f"kurt_{window}",
                    f"jarque_bera_{window}",
                    f"tail_ratio_{window}",
                ]
            )
    ...
    def _calculate_distribution_tests(
        ...
        for window in self.distribution_test_windows:
            ...
            features[f"ks_stat_{window}"] = self.rolling_apply_safe(returns, window, ks_test_func)
    ...
    def _calculate_derived_moments(
        ...
        features["volatility_clustering"] = squared_returns.rolling(20).corr(
        ...
        features["tail_asymmetry"] = self.rolling_apply_safe(returns, 60, tail_asymmetry_func)
