...
class FractalCalculator(BaseStatisticalCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.multifractal_q_values = self.config.get(
            "multifractal_q_values", [-5, -3, -1, 0, 1, 3, 5]
        )
    ...
    def _calculate_hurst_exponents(
        ...
        if "rs" in self.stat_config.hurst_methods:
            ...
            features["hurst_rs"] = self.rolling_apply_safe(returns, window, hurst_rs_func)
        ...
        if "dma" in self.stat_config.hurst_methods:
            ...
            features["hurst_dma"] = self.rolling_apply_safe(returns, window, hurst_dma_func)
        ...
        if "peng" in self.stat_config.hurst_methods:
            ...
            features["hurst_peng"] = self.rolling_apply_safe(returns, window, hurst_peng_func)

        return features

    def _calculate_dfa_features(
        self, returns: pd.Series, features: pd.DataFrame, window: int
    ) -> pd.DataFrame:
        ...
        features["dfa_alpha"] = self.rolling_apply_safe(returns, window, dfa_func)

        return features

    def _calculate_multifractal_features(
        self, returns: pd.Series, features: pd.DataFrame, window: int
    ) -> pd.DataFrame:
        ...
        features["multifractal_width"] = self.rolling_apply_safe(returns, window, multifractal_func)

        return features

    def _calculate_similarity_measures(
        self, returns: pd.Series, features: pd.DataFrame, window: int
    ) -> pd.DataFrame:
    ...
    def _calculate_derived_fractal_features(
        ...
        features["cyclical_strength"] = self.rolling_apply_safe(returns, 60, cyclical_strength_func)
        ...
        features["roughness_index"] = self.rolling_apply_safe(returns, 50, roughness_func)
    ...
    def _calculate_hurst_rs(self, data: np.ndarray) -> float:
        ...
        try:
            ...
            valid_rs = [
                (lag, rs) for lag, rs in zip(lags, rs_values) if not np.isnan(rs) and rs > 0
            ]
    ...
    def _calculate_hurst_dma(self, data: np.ndarray) -> float:
        ...
        try:
            ...
            valid_data = [
                (lag, fluct)
                for lag, fluct in zip(lags, fluctuations)
