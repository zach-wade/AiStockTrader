...
class NonlinearCalculator(BaseStatisticalCalculator):
    ...
    def get_feature_names(self) -> list[str]:
        ...
        # Attractor analysis
        feature_names.extend(["attractor_dimension", "predictability", "complexity_lmc"])
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            features = self._calculate_correlation_dimension_features(
                returns, features, nonlinear_window
            )
    ...
    def _calculate_lyapunov_features(
        self, returns: pd.Series, features: pd.DataFrame, window: int
    ) -> pd.DataFrame:
        ...
        features["lyapunov_wolf"] = self.rolling_apply_safe(returns, window, lyapunov_wolf_func)
        ...
        features["lyapunov_kantz"] = self.rolling_apply_safe(returns, window, lyapunov_kantz_func)

        return features

    def _calculate_correlation_dimension_features(
        self, returns: pd.Series, features: pd.DataFrame, window: int
    ) -> pd.DataFrame:
    ...
    def _calculate_rqa_features(
        ...
        features["recurrence_rate"] = rqa_results.apply(
            lambda x: extract_rqa_measure(x, 0) if not pd.isna(x) else np.nan
        )
        features["determinism"] = rqa_results.apply(
            lambda x: extract_rqa_measure(x, 1) if not pd.isna(x) else np.nan
        )
        features["laminarity"] = rqa_results.apply(
            lambda x: extract_rqa_measure(x, 2) if not pd.isna(x) else np.nan
        )
        features["entropy_rqa"] = rqa_results.apply(
            lambda x: extract_rqa_measure(x, 3) if not pd.isna(x) else np.nan
        )
        features["max_line_length"] = rqa_results.apply(
            lambda x: extract_rqa_measure(x, 4) if not pd.isna(x) else np.nan
        )
        features["divergence"] = rqa_results.apply(
            lambda x: extract_rqa_measure(x, 5) if not pd.isna(x) else np.nan
        )
        features["trend_rqa"] = rqa_results.apply(
            lambda x: extract_rqa_measure(x, 6) if not pd.isna(x) else np.nan
        )

        return features

    def _calculate_chaos_detection(
        self, returns: pd.Series, features: pd.DataFrame, window: int
    ) -> pd.DataFrame:
        """Calculate chaos detection measures."""
        ...
        features["chaos_test_01"] = self.rolling_apply_safe(returns, window, chaos_01_func)
        ...
        features["false_nearest_neighbors"] = self.rolling_apply_safe(returns, window, fnn_func)

        return features

    def _calculate_attractor_features(
        self, returns: pd.Series, features: pd.DataFrame, window: int
    ) -> pd.DataFrame:
        ...
        features["predictability"] = self.rolling_apply_safe(returns, window, predictability_func)

        # LMC complexity
        def lmc_complexity_func(x):
            return self._lmc_complexity(x)

        features["complexity_lmc"] = self.rolling_apply_safe(returns, window, lmc_complexity_func)
    ...
    def _estimate_lyapunov_largest(self, data: np.ndarray) -> float:
        ...
        try:
            ...
            for i in range(len(embedded) - self.lyapunov_k):
                ...
                # Track evolution
                if (
                    min_idx + self.lyapunov_k < len(embedded)
                    and i + self.lyapunov_k < len(embedded)
                    and initial_dist > self.numerical_tolerance
                ):
                        ...
                        for neighbor_idx in neighbors:
                            if neighbor_idx + evolution_time < len(embedded):
                                final_distance = np.linalg.norm(
                                    embedded[i + evolution_time]
                                    - embedded[neighbor_idx + evolution_time]
                                )
    ...
    def _calculate_laminarity(self, recurrence_matrix: np.ndarray) -> float:
        ...
        return (
            points_in_vertical_lines / total_recurrent_points
