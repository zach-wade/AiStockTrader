...
class AdvancedStatisticalCalculator(BaseFeatureCalculator):
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.stat_config = StatisticalConfig(**config.get("statistical", {}) if config else {})
        ...
        logger.debug(
            f"Initialized AdvancedStatisticalCalculator with {len(self.calculators)} specialized calculators"
        )

    def _initialize_calculators(self, config: dict | None = None) -> dict[str, object]:
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for calc_name, calculator in self.calculators.items():
                ...
                try:
                    ...
                    if features is not None and not features.empty:
                        ...
                        logger.debug(
                            f"{calc_name} calculator: {len(features.columns)} features in {calc_time:.3f}s"
                        )
                    else:
            ...
            logger.info(
                f"Statistical calculation completed: {len(combined_features.columns)} features in {total_time:.3f}s"
            )
    ...
    def _create_empty_features(self, index: pd.Index) -> pd.DataFrame:
        ...
        try:
            ...
            empty_features = pd.DataFrame(index=index, columns=all_feature_names, dtype=float)
