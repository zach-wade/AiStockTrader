...
from .helpers import create_feature_dataframe, postprocess_features, safe_divide
...
class InsiderAnalyticsCalculator(BaseFeatureCalculator):
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.insider_config = InsiderConfig(**config.get("insider", {}) if config else {})
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if self.insider_data is not None and not self.insider_data.empty:
                ...
                logger.info(
                    f"Calculating insider features for {len(data)} periods "
    ...
    def get_feature_names(self) -> list[str]:
        ...
        # Clustering features
        feature_names.extend(
            ["insider_cluster_score", "insider_cluster_buy", "insider_cluster_sell"]
        )
        ...
        # Conviction features
        feature_names.extend(
            ["insider_conviction_ratio", "high_conviction_trade", "unanimous_insider_action"]
        )
    ...
    def preprocess_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.insider_data is not None:
            ...
            if "transaction_date" in self.insider_data.columns:
                ...
                if not data.empty:
                    start_date = data.index.min() - timedelta(
                        days=max(self.insider_config.lookback_windows)
                    )
                    ...
                    logger.info(
                        f"Filtered insider data to {len(self.insider_data)} transactions "
    ...
    def _postprocess_insider_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if "insider_sentiment_strength" in features.columns:
            features["insider_sentiment_strength"] = features["insider_sentiment_strength"].clip(
                0, 1
            )
    ...
    def _add_transaction_metrics(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        significant_trades["transaction_date"] = pd.to_datetime(
            significant_trades["transaction_date"]
        )
        ...
        for window in self.insider_config.lookback_windows:
            ...
            for idx in data.index:
                ...
                buys = window_trades[
                    window_trades["transaction_type"].str.contains("Buy", case=False, na=False)
                ]
                sells = window_trades[
                    window_trades["transaction_type"].str.contains("Sell", case=False, na=False)
                ]
            ...
            features[f"insider_net_value_{window}d"] = np.array(total_buy_value) - np.array(
                total_sell_value
            )
    ...
    def _add_insider_type_features(
        ...
        exec_mask = self.insider_data["title"].str.contains(
            "|".join(self.insider_config.executive_titles), case=False, na=False
        )

        director_mask = self.insider_data["title"].str.contains(
            "|".join(self.insider_config.director_titles), case=False, na=False
        )
        ...
        for window in [30, 90]:
            ...
            for idx in data.index:
                ...
                exec_buys = window_exec[
                    window_exec["transaction_type"].str.contains("Buy", case=False, na=False)
                ]
                exec_sells = window_exec[
                    window_exec["transaction_type"].str.contains("Sell", case=False, na=False)
                ]
                ...
                dir_buys = window_dir[
                    window_dir["transaction_type"].str.contains("Buy", case=False, na=False)
                ]
                dir_sells = window_dir[
                    window_dir["transaction_type"].str.contains("Sell", case=False, na=False)
                ]

                director_buy_count.append(len(dir_buys))
                director_sell_count.append(len(dir_sells))

            features[f"executive_net_trades_{window}d"] = np.array(exec_buy_count) - np.array(
                exec_sell_count
            )
            features[f"director_net_trades_{window}d"] = np.array(director_buy_count) - np.array(
                director_sell_count
            )
    ...
    def _add_transaction_patterns(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in data.index:
            ...
            if not window_trades.empty:
                ...
                if len(window_trades) >= 3:
                    trade_times = (
                        pd.to_datetime(window_trades["transaction_date"]) - window_start
                    ).dt.days
        ...
        features["insider_increasing_positions"] = (np.array(transaction_size_trends) > 0).astype(
            int
        )
    ...
    def _add_performance_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in data.index:
            ...
            if not historical_trades.empty and idx - perf_window >= data.index[0]:
                ...
                entry_price = (
                    data.loc[trade_date:, "close"].iloc[0]
                ...
                buys = historical_trades[
                    historical_trades["transaction_type"].str.contains("Buy", case=False, na=False)
                ]
                sells = historical_trades[
                    historical_trades["transaction_type"].str.contains("Sell", case=False, na=False)
                ]
    ...
    def _add_conviction_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if "insider_avg_transaction_size" in features.columns:
            ...
            features["high_conviction_trade"] = (features["insider_conviction_ratio"] > 3).astype(
                int
            )
    ...
    def _add_timing_patterns(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in data.index:
            ...
            days_to_quarter_end = min(abs(current_month - m) * 30 for m in quarter_end_months)
    ...
    def _add_top_insider_features(self, data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in data.index:
            ...
            if not ceo_cfo_trades.empty:
                buys = ceo_cfo_trades[
                    ceo_cfo_trades["transaction_type"].str.contains("Buy", case=False, na=False)
                ]
                sells = ceo_cfo_trades[
                    ceo_cfo_trades["transaction_type"].str.contains("Sell", case=False, na=False)
                ]
    ...
    def _add_moving_averages(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if "insider_sentiment_score" in features.columns:
            features["insider_sentiment_ma_30"] = (
                features["insider_sentiment_score"].rolling(30).mean()
            )
            features["insider_sentiment_ma_90"] = (
                features["insider_sentiment_score"].rolling(90).mean()
            )

        # Moving averages of transaction activity
        if "insider_net_count_30d" in features.columns:
            features["insider_activity_ma_30"] = (
                features["insider_net_count_30d"].rolling(30).mean()
            )

        # Moving averages of conviction ratio
        if "insider_conviction_ratio" in features.columns:
            features["insider_conviction_ma_30"] = (
                features["insider_conviction_ratio"].rolling(30).mean()
            )

        # Trend indicators
        if (
            "insider_sentiment_ma_30" in features.columns
            and "insider_sentiment_ma_90" in features.columns
        ):
    ...
    def _add_feature_interactions(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            "insider_sentiment_score" in features.columns
            and "insider_conviction_ratio" in features.columns
        ):
            features["sentiment_conviction_product"] = features[
                "insider_sentiment_score"
            ] * np.log1p(features["insider_conviction_ratio"].abs())

        # Executive activity × Sentiment
        if (
            "executive_net_trades_30d" in features.columns
            and "insider_sentiment_score" in features.columns
        ):
        ...
        if (
            "insider_cluster_score" in features.columns
            and "insider_performance_score" in features.columns
        ):
