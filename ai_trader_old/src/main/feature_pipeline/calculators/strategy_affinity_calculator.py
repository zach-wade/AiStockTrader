...
class StrategyAffinityCalculator:
    ...
    async def _calculate_symbol_affinities(self, symbol: str) -> dict[str, float]:
        ...
        try:
            ...
            if not market_data or len(market_data) < self.min_data_points:
                logger.warning(
                    f"Insufficient data for {symbol}: {len(market_data) if market_data else 0} points"
                )
    ...
    def _calculate_momentum_affinity(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            volume_score = (
                min(volume_confirmation / 1.5, 1.0) if not pd.isna(volume_confirmation) else 0.5
            )
    ...
    def _calculate_mean_reversion_affinity(self, df: pd.DataFrame) -> float:
        ...
        try:
                ...
                for idx in df[high_vol_periods].index:
                    if idx + 5 < len(df):
                        ...
                        if not pd.isna(sma_value):
                            if any(
                                abs(price - sma_value) / sma_value < 0.02 for price in future_prices
                            ):
                                reversals += 1
            ...
            trending_strength = (
                abs(df["close"].iloc[-1] - df["close"].iloc[0]) / df["close"].iloc[0]
            )
    ...
    def _calculate_breakout_affinity(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            vol_trend = np.polyfit(
                range(len(df["volatility"].dropna())), df["volatility"].dropna(), 1
            )[0]
            ...
            for i in range(self.breakout_period, len(df)):
                if (
                    df.iloc[i]["high"] > rolling_high.iloc[i - 1]
                    and df.iloc[i]["volume_ratio"] > 1.5
                ):
                    high_breakouts += 1
                elif (
                    df.iloc[i]["low"] < rolling_low.iloc[i - 1] and df.iloc[i]["volume_ratio"] > 1.5
                ):
                    low_breakouts += 1
            ...
            for i in range(window, len(df) - window):
                ...
                if pre_vol < df["volatility"].quantile(0.3) and post_vol > df[
                    "volatility"
                ].quantile(0.7):
            ...
            for idx in df[spike_days].index:
                if idx + 3 < len(df):
                    ...
                    if len(future_returns) > 0:
                        ...
                        if (initial_return > 0 and avg_return > 0) or (
                            initial_return < 0 and avg_return < 0
                        ):
                            momentum_after_spikes += 1

            momentum_score = (
                momentum_after_spikes / spike_days.sum() if spike_days.sum() > 0 else 0.0
            )
