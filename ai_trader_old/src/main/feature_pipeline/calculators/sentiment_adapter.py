...
# Local imports
from main.interfaces.calculators import CalculatorConfig, FeatureResult, ISentimentCalculator
...
class SentimentCalculatorAdapter(ISentimentCalculator):
    ...
    def __init__(self, config: dict[str, Any] | CalculatorConfig):
        ...
        if isinstance(config, CalculatorConfig):
            ...
            self._config = CalculatorConfig(
                name="sentiment_features", enabled=True, parameters=config
            )
    ...
    def calculate_sentiment(
        self, text_data: pd.DataFrame | list[str], sentiment_config: dict[str, Any] | None = None
    ) -> pd.DataFrame:
        """
