...
class MicrostructureCalculator(BaseFeatureCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.rate_limiter = RateLimiter(max_calls=200, time_window=60)
    ...
    def _prepare_microstructure_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "volume" in micro_data.columns:
            ...
            micro_data["relative_volume"] = safe_divide(micro_data["volume"], avg_volume)
        ...
        if isinstance(micro_data.index, pd.DatetimeIndex):
            micro_data["time_between_trades"] = (
                micro_data.index.to_series().diff().dt.total_seconds()
            )
            micro_data["trade_intensity"] = safe_divide(1, micro_data["time_between_trades"])

        return micro_data.fillna(method="ffill").dropna()

    def _calculate_spread_features(self, micro_data: pd.DataFrame, index: pd.Index) -> pd.DataFrame:
        ...
        for window in self.spread_windows:
            ...
            features[f"bid_ask_spread_{window}min"] = avg_spread.reindex(index, method="ffill")

            # Relative spread
            rel_spread = safe_divide(spread, mid_price)
            avg_rel_spread = rel_spread.rolling(window=window).mean()
            features[f"relative_spread_{window}min"] = avg_rel_spread.reindex(index, method="ffill")
            ...
            features[f"spread_volatility_{window}min"] = spread_vol.reindex(index, method="ffill")

        return features

    def _calculate_order_flow_features(
        self, micro_data: pd.DataFrame, index: pd.Index
    ) -> pd.DataFrame:
        ...
        order_flow_imb = safe_divide(rolling_buy - rolling_sell, rolling_buy + rolling_sell)
        features["order_flow_imbalance"] = order_flow_imb.reindex(index, method="ffill")

        # Trade direction persistence
        direction_changes = direction.diff() != 0
        persistence = 1 - direction_changes.rolling(window=window).mean()
        features["trade_direction_persistence"] = persistence.reindex(index, method="ffill")

        # Volume imbalance ratio
        vol_imbalance = safe_divide(
            buy_volume.rolling(window=window).sum(), sell_volume.rolling(window=window).sum()
        )
        features["volume_imbalance_ratio"] = vol_imbalance.reindex(index, method="ffill")
        ...
        features["buy_sell_pressure_ratio"] = pressure_ratio.reindex(index, method="ffill")

        # Aggressive trade ratio
        if "aggressive_trade" in micro_data.columns:
            aggressive_ratio = micro_data["aggressive_trade"].rolling(window=window).mean()
            features["aggressive_trade_ratio"] = aggressive_ratio.reindex(index, method="ffill")
    ...
    def _calculate_trade_size_features(
        ...
        features["large_trade_frequency"] = large_trades.reindex(index, method="ffill")
        features["small_trade_frequency"] = small_trades.reindex(index, method="ffill")

        # Trade size skewness
        size_skew = trade_size.rolling(window=window).apply(
            lambda x: stats.skew(x) if len(x) > 3 else 0, raw=True
        )
        features["trade_size_skewness"] = size_skew.reindex(index, method="ffill")
        ...
        features["block_trade_indicator"] = block_trades.reindex(index, method="ffill")
    ...
    def _calculate_market_impact_features(
        ...
        for window in self.impact_windows:
            ...
            features[f"temporary_impact_{window}min"] = temp_impact.reindex(index, method="ffill")

            # Permanent impact
            perm_impact = price_change.rolling(window=window * 2).mean()
            features[f"permanent_impact_{window}min"] = perm_impact.reindex(index, method="ffill")

            # Price impact normalized by volume
            impact_per_volume = safe_divide(price_change.abs(), volume_window)
            features[f"price_impact_{window}min"] = impact_per_volume.reindex(index, method="ffill")
    ...
    def _calculate_liquidity_features(
        ...
        if "estimated_spread" in micro_data.columns:
            ...
            features["quoted_spread"] = quoted_spread.reindex(index, method="ffill")
        ...
        if "volume" in micro_data.columns:
            ...
            features["depth_imbalance"] = depth_imbalance.reindex(index, method="ffill")
        ...
        if all(col in micro_data.columns for col in ["volume", "close"]):
            ...
            features["liquidity_ratio"] = liquidity_ratio.reindex(index, method="ffill")
        ...
        if "close" in micro_data.columns:
            ...
            features["resilience_score"] = resilience_score.reindex(index, method="ffill")

        # Market depth change
        if "volume" in micro_data.columns:
            volume = micro_data["volume"]
            depth_change = volume.pct_change().rolling(window=20).std()
            features["market_depth_change"] = depth_change.reindex(index, method="ffill")

        # Turnover velocity
        if all(col in micro_data.columns for col in ["volume", "close"]):
            turnover = micro_data["volume"] * micro_data["close"]
            velocity = turnover / turnover.rolling(window=20).mean()
            features["turnover_velocity"] = velocity.reindex(index, method="ffill")
    ...
    def _calculate_price_discovery_features(
        ...
        features["price_efficiency_ratio"] = efficiency_ratio.reindex(index, method="ffill")
        ...
        if "volume" in micro_data.columns:
            ...
            features["information_share"] = info_share.reindex(index, method="ffill")
        ...
        features["autocorrelation_decay"] = autocorr_series.reindex(index, method="ffill")

        # Realized spread
        if "estimated_spread" in micro_data.columns:
            spread = micro_data["estimated_spread"]
            realized_spread = spread * returns.abs()
            features["realized_spread"] = realized_spread.reindex(index, method="ffill")
    ...
    def _calculate_volume_profile_features(
        ...
        features["volume_participation_rate"] = participation_rate.reindex(index, method="ffill")
        ...
        if isinstance(micro_data.index, pd.DatetimeIndex):
            ...
            features["time_weighted_volume"] = tw_volume.reindex(index, method="ffill")
        ...
        features["volume_synchronicity"] = synchronicity.reindex(index, method="ffill")
        ...
        features["volume_clustering"] = volume_clustering.reindex(index, method="ffill")

        return features

    def _calculate_noise_features(self, micro_data: pd.DataFrame, index: pd.Index) -> pd.DataFrame:
        """Calculate microstructure noise features."""
        features = pd.DataFrame(index=index)
        ...
        features["microstructure_noise"] = noise_variance.reindex(index, method="ffill")
        ...
        features["roll_spread_estimate"] = roll_spread.reindex(index, method="ffill")
        ...
        features["hasbrouck_noise"] = hasbrouck_noise.reindex(index, method="ffill")
        ...
        effective_tick = (
            ...
            .apply(lambda x: np.percentile(x[x > 0], 10) if (x > 0).any() else min_tick)
        )
        features["effective_tick_size"] = effective_tick.reindex(index, method="ffill")
    ...
    def _calculate_high_frequency_features(
        ...
        if isinstance(micro_data.index, pd.DatetimeIndex):
            ...
            features["tick_frequency"] = tick_frequency.reindex(index, method="ffill")
        ...
        if "estimated_spread" in micro_data.columns:
            ...
            features["quote_update_frequency"] = quote_updates.reindex(index, method="ffill")
        ...
        if "trade_intensity" in micro_data.columns:
            ...
            features["trade_arrival_intensity"] = intensity.reindex(index, method="ffill")
        ...
        if "volume" in micro_data.columns:
            ...
            features["market_fragmentation"] = fragmentation.reindex(index, method="ffill")

        return features

    def _calculate_composite_measures(self, features: pd.DataFrame) -> pd.DataFrame:
