...
class EnhancedCorrelationCalculator(BaseCorrelationCalculator):
    ...
    def __init__(self, config: dict | None = None):
        ...
        logger.info(
            f"EnhancedCorrelationCalculator facade initialized with {len(self.calculators)} specialized calculators"
        )
            ...
            for calc_name, calculator in self.calculators.items():
                try:
                    ...
                    if calc_features is not None and not calc_features.empty:
                        ...
                        for column in calc_features.columns:
                            if column not in combined_features.columns:
                                ...
                                if len(calc_features) == len(combined_features):
                                    ...
                                    try:
                                        aligned_values = calc_features[column].reindex(
                                            combined_features.index, fill_value=0.0
                                        )
                                        combined_features[column] = aligned_values
                                    except (ValueError, TypeError, KeyError):
                                        # Fallback: use first value or zero
                                        fill_value = (
                                            calc_features[column].iloc[0]
                                            if len(calc_features) > 0
                                            else 0.0
                                        )
                                        combined_features[column] = fill_value
                            else:
                                logger.warning(
                                    f"Duplicate feature name '{column}' from {calc_name} calculator"
                                )
                        ...
                        logger.debug(
                            f"{calc_name} calculator: {len(calc_features.columns)} features calculated"
                        )
                    else:
                        logger.warning(f"{calc_name} calculator returned empty features")
                        calculation_results[calc_name] = {"features_count": 0, "status": "empty"}

                except Exception as e:
                    logger.error(f"Error calculating {calc_name} features: {e}")
                    calculation_results[calc_name] = {"error": str(e), "status": "failed"}

            # Log summary
            total_features = len(combined_features.columns)
            successful_calcs = sum(
                1 for result in calculation_results.values() if result["status"] == "success"
            )

            logger.info(
                f"Feature calculation complete: {total_features} total features from {successful_calcs}/{len(self.calculators)} calculators"
            )
    ...
    def _validate_feature_completeness(self, features: pd.DataFrame, results: dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Feature count validation: {actual_total}/{total_expected} expected features"
            )
            ...
            for calc_name, expected_count in expected_counts.items():
                if calc_name in results and results[calc_name]["status"] == "success":
                    ...
                    if actual_count != expected_count:
                        logger.warning(
                            f"{calc_name} calculator: {actual_count}/{expected_count} features (expected {expected_count})"
                        )
                else:
    ...
    def get_calculator_status(self) -> dict[str, str]:
        ...
        for calc_name, calculator in self.calculators.items():
            try:
                ...
                status[calc_name] = f"error: {e!s}"

        return status

    def validate_configuration(self) -> dict[str, Any]:
        """Validate configuration across all calculators."""
        validation_results = {"facade": {"status": "valid"}, "calculators": {}}
