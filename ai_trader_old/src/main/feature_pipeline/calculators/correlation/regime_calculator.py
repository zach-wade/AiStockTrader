...
class RegimeCorrelationCalculator(BaseCorrelationCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        logger.debug(
            f"Initialized RegimeCorrelationCalculator with {self.volatility_lookback}d lookback"
        )
    ...
    def _calculate_volatility_regime_correlations(
    ...
    def _calculate_trend_regime_correlations(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_crisis_correlations(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_regime_transitions(
        ...
        try:
            ...
            if len(transition_indices) > 1:
                transition_gaps = [
                    (transition_indices[i] - transition_indices[i - 1]).days
    ...
    def _calculate_advanced_regime_features(
        self, data: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            for symbol in returns_pivot.columns:
                ...
                if extreme_down_mask.any():
                    down_tail_corr = self.safe_correlation(
                        symbol_returns[extreme_down_mask], market_returns[extreme_down_mask]
                    )
            ...
            if (
                "corr_uptrend" in features.columns
                and "corr_downtrend" in features.columns
                and "corr_high_vol" in features.columns
                and "corr_low_vol" in features.columns
            ):

                trend_asymmetry = abs(
                    features["corr_uptrend"].iloc[0] - features["corr_downtrend"].iloc[0]
                )
                vol_asymmetry = abs(
                    features["corr_high_vol"].iloc[0] - features["corr_low_vol"].iloc[0]
                )
            ...
            if (
                "corr_crisis_periods" in features.columns
                and "corr_normal_periods" in features.columns
            ):
