...
class BetaAnalysisCalculator(BaseCorrelationCalculator):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.beta_lookback_periods = self.beta_config.get(
            "beta_lookback_periods", [20, 60, 120, 252]
        )
        self.regime_threshold = self.beta_config.get("regime_threshold", 0.02)

        logger.debug(
            f"Initialized BetaAnalysisCalculator with {len(self.beta_lookback_periods)} lookback periods"
        )
    ...
    def get_feature_names(self) -> list[str]:
        ...
        for window in self.beta_lookback_periods:
            feature_names.extend(
                [f"beta_{window}d", f"beta_{window}d_std", f"beta_{window}d_change"]
            )

        # Regime-dependent betas
        feature_names.extend(
            ["beta_bull_market", "beta_bear_market", "beta_asymmetry", "beta_regime_sensitivity"]
        )
    ...
    def _calculate_rolling_betas(
        self, data: pd.DataFrame, features: pd.DataFrame, market_returns: pd.Series
    ) -> pd.DataFrame:
                ...
                if beta_values:
                    ...
                    for timestamp in median_beta.index:
                        if timestamp in features.index:
                            ...
                            features.loc[timestamp, f"beta_{window}d_std"] = median_beta_std.loc[
                                timestamp
                            ]
                            features.loc[timestamp, f"beta_{window}d_change"] = (
                                median_beta_change.loc[timestamp]
                            )
    ...
    def _calculate_beta_dynamics(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            mean_reversion = (beta_series - rolling_mean) / (
    ...
    def _calculate_risk_decomposition(
        ...
        try:
            ...
            if symbol_risks:
                ...
                # Risk ratios
                systematic_ratio = systematic_risk_median / (
                    total_risk_median + self.numerical_tolerance
                )
                idiosyncratic_ratio = idiosyncratic_risk_median / (
                    total_risk_median + self.numerical_tolerance
                )
    ...
    def _calculate_beta_percentiles(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            downside_beta = (
                pd.Series(downside_proxy, index=beta_series.index).rolling(window=20).mean()
            )
