...
class PCACorrelationCalculator(BaseCorrelationCalculator):
    ...
    def get_feature_names(self) -> list[str]:
        ...
        for i in range(1, self.pca_components + 1):
            feature_names.extend([f"pc{i}_loading", f"pc{i}_exposure"])
        ...
        feature_names.extend(
            ["pc1_trend_30d", "pc1_volatility_30d", "factor_rotation_speed", "pca_regime_indicator"]
        )
    ...
    def _calculate_rolling_pca(
        self, returns_pivot: pd.DataFrame, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_factor_analysis(
        ...
        try:
            ...
            if len(eigenvalues) > 1:
                eigenvalue_dispersion = np.std(eigenvalues[:5]) / (
                    np.mean(eigenvalues[:5]) + self.numerical_tolerance
                )
            ...
            if len(eigenvalues) > 1:
                ...
                stability_score = np.max(eigenvalue_gaps) / (
                    eigenvalues[0] + self.numerical_tolerance
                )
