...
class VolatilityCalculator(BaseRiskCalculator):
    ...
    def _calculate_basic_volatility(
        ...
        try:
            ...
            if len(ewma_vol) > 10:
                try:
                    ...
                    features["ewma_persistence"] = (
                        ewma_persistence if not np.isnan(ewma_persistence) else 0.0
                    )
                except Exception as e:
            ...
            if prev_close > 0:
                ...
                if features["overnight_volatility"] != 0:
                    features["volatility_ratio"] = (
                        features["intraday_volatility"] / features["overnight_volatility"]
                    )
        ...
        try:
            ...
            if "volatility_20d" in features.columns and "volatility_60d" in features.columns:
                if features["volatility_60d"] != 0:
                    features["volatility_ratio_20_60"] = (
            ...
            if "volatility_60d" in features.columns and "volatility_252d" in features.columns:
                if features["volatility_252d"] != 0:
                    features["volatility_ratio_60_252"] = (
                        features["volatility_60d"] / features["volatility_252d"]
                    )
                    ...
                    if len(rolling_vol) > 10:
                        ...
                        if len(recent_vol) > 1:
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"Error calculating volatility trend for {window}d: {e}"
                                )
        ...
        try:
            ...
            if len(squared_returns) > 10:
                try:
                    ...
                    features["volatility_clustering"] = (
                        clustering if not np.isnan(clustering) else 0.0
                    )
                except Exception as e:
            ...
            for lag in autocorr_lags:
                if len(squared_returns) > lag + 5:
                    try:
                        ...
                        features[f"volatility_autocorr_{lag}"] = (
            ...
            if len(returns) > 60:
                ...
                if len(rolling_vol) > 40:
                    ...
                    try:
                        ...
                        features["volatility_mean_reversion"] = (
                            mean_reversion if not np.isnan(mean_reversion) else 0.0
                        )
                    except Exception as e:
    ...
    def _calculate_volatility_distribution(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_volatility_forecasting(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if len(returns) >= 30:
                ...
                for horizon in horizons:
                    ...
                    forecast_vol = long_term_vol + (current_vol - long_term_vol) * (
                        persistence**horizon
                    )
        ...
        try:
            ...
            if has_arch and len(returns) >= 100:
                try:
                    ...
                    features["garch_unconditional_vol"] = (
                        np.sqrt(
                            fitted_model.params["omega"]
                            / (1 - fitted_model.params["alpha[1]"] - fitted_model.params["beta[1]"])
                        )
                        / 100
                    )
                    ...
                    # GARCH persistence
                    garch_persistence = (
                        fitted_model.params["alpha[1]"] + fitted_model.params["beta[1]"]
                    )
    ...
    def _calculate_volatility_risk_metrics(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
