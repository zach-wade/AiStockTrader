...
class TailRiskCalculator(BaseRiskCalculator):
        ...
        try:
            ...
            if len(lower_tail) > 0:
                ...
                if len(log_ratios) > 0:
                    ...
                    goodness_of_fit = 1 - safe_divide(
                        standard_error, hill_estimator, default_value=0.0
                    )
    ...
    def _calculate_peak_over_threshold(
        ...
        try:
            ...
            features["pot_exceedances_rate"] = safe_divide(
                len(exceedances), len(returns), default_value=0.0
            )
    ...
    def _calculate_extreme_quantiles(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if hill_index > 0:
                ...
                for confidence in [0.95, 0.99]:
                    ...
                    extrapolated_quantile = threshold * (
                        safe_divide(n, k, default_value=1.0) * (1 - confidence)
                    ) ** (-hill_index)
    ...
    def _calculate_tail_dependence(
        ...
        try:
            ...
            joint_exceedances = (
                (aligned_returns > threshold_value) & (lagged_returns > threshold_value)
            ).sum()
            marginal_exceedances = (aligned_returns > threshold_value).sum()

            if marginal_exceedances > 0:
                upper_tail_dependence = safe_divide(
                    joint_exceedances, marginal_exceedances, default_value=0.0
                )
            ...
            if marginal_exceedances > 0:
                lower_tail_dependence = safe_divide(
                    joint_exceedances, marginal_exceedances, default_value=0.0
                )
            ...
            if len(tail_data) > 10:
                ...
                features["tail_correlation"] = (
                    tail_correlation if not np.isnan(tail_correlation) else 0
                )
    ...
    def _calculate_extreme_expected_shortfall(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            for level, label in zip(extreme_levels, extreme_labels):
                ...
                if len(tail_losses) > 0:
                    ...
                    expected_shortfall = mean_return - std_return * stats.norm.pdf(
                        stats.norm.ppf(1 - level)
                    ) / (1 - level)
    ...
    def _calculate_evt_diagnostics(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if len(exceedances) > 10:
                ...
                features["evt_qq_plot_correlation"] = (
                    qq_correlation if not np.isnan(qq_correlation) else 0
                )

                # PP plot correlation
                features["evt_pp_plot_correlation"] = (
                    qq_correlation  # Same for this simplified version
                )
    ...
    def _calculate_tail_risk_forecasting(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if len(returns) > 60:
                ...
                if len(rolling_tail_risk) > 10:
                    ...
                    features["tail_risk_persistence"] = (
                    ...
                    for horizon in [1, 5, 21]:
                        ...
                        forecast = long_term_avg + (rolling_series.iloc[-1] - long_term_avg) * (
                            persistence**horizon
                        )
    ...
    def _calculate_advanced_tail_metrics(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if len(returns) > 50:
                ...
                weights = np.array(
                    [
                        safe_divide(i, n, default_value=0.0) ** alpha
                        - safe_divide(i - 1, n, default_value=0.0) ** alpha
            ...
            if len(returns) > 100:
                ...
                if len(tail_estimates) > 0:
                    cv = safe_divide(
                        np.std(tail_estimates), np.mean(tail_estimates), default_value=0.0
                    )
