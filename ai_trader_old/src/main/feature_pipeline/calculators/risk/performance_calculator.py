...
class PerformanceCalculator(BaseRiskCalculator):
    ...
    def _create_benchmark_returns(self, returns: pd.Series) -> pd.Series:
        ...
        try:
            ...
            benchmark_returns = pd.Series(
                secure_numpy_normal(
                    daily_benchmark_return, benchmark_volatility * 0.5, len(returns)
                ),
                index=returns.index,
            )
    ...
    def _calculate_sortino_metrics(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if len(upside_returns) > 0:
                ...
                if features.get("downside_deviation", 0) > 0:
                    upside_downside_ratio = safe_divide(
                        upside_deviation, features["downside_deviation"]
                    )
    ...
    def _calculate_information_metrics(
        self, returns: pd.Series, benchmark_returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_treynor_metrics(
        self, returns: pd.Series, benchmark_returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _calculate_performance_consistency(
        ...
        try:
            ...
            if len(returns) >= 60:
                ...
                consistency_ratio = safe_divide(
                    len(rolling_returns[rolling_returns > 0]), len(rolling_returns.dropna())
                )
    ...
    def _calculate_advanced_performance(
        self, returns: pd.Series, features: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if len(returns) > 30:
                ...
                if len(positive_returns) > 5:
                    ...
                    if positive_returns.std() > 0:
                        conditional_sharpe = safe_divide(
                            excess_positive.mean(), positive_returns.std()
                        )
            ...
            if len(returns) > 30:
                ...
                if len(positive_returns) > 0:
                    capture_up = (
                        safe_divide(positive_returns.mean(), returns.mean())
                        if returns.mean() > 0
                        else 0
                    )
                ...
                if len(negative_returns) > 0:
                    capture_down = (
                        safe_divide(negative_returns.mean(), returns.mean())
                        if returns.mean() < 0
                        else 0
                    )
                ...
                if features.get("capture_ratio_down", 0) > 0:
                    total_capture = safe_divide(
                        features.get("capture_ratio_up", 0), features["capture_ratio_down"]
                    )
            ...
            if len(returns) > 60:
                ...
                if len(first_half_cumulative) > 0 and len(second_half_cumulative) > 0:
                    ...
                    persistence = (
                        ...
                        or (first_return < 0 and second_return < 0)
                        else 0
                    )
