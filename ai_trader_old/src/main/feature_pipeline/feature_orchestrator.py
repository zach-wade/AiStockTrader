...
from main.utils.data import StreamingConfig, streaming_context
...
class FeatureOrchestrator:
    ...
    def __init__(self, config=None, event_bus: IEventBus | None = None):
        """
        ...
        streaming_memory_limit = self.config.get(
            "orchestrator.features.streaming.max_memory_mb", 500.0
        )
        ...
        self.streaming_threshold_rows = self.config.get(
            "orchestrator.features.streaming.threshold_rows", 50000
        )

        logger.info(
            f"FeatureOrchestrator initialized with {max_workers} parallel workers, "
    ...
    def set_data_clients(self, clients: dict[str, Any]):
        ...
        if clients:
            ...
            self.data_fetch_service = DataFetchService(clients=clients, archive=self.data_archive)
    ...
    async def calculate_features(
        ...
        logger.info(
            f"Calculating features for {len(symbols_to_calculate)} symbols ({len(cached_features)} cached)"
        )

        # Create parallel tasks for feature calculation
        tasks = []
        for symbol in symbols_to_calculate:
            task = self._calculate_single_symbol_features(
                symbol, start_time, end_time, feature_sets
            )
            tasks.append(task)

        # Execute tasks concurrently with progress tracking
        calculated_features = await self._execute_parallel_feature_calculation(
            tasks, symbols_to_calculate
        )
        ...
        self.calculation_stats["last_calculation_time"] = datetime.now(UTC)

        calc_duration = time.time() - start_calc_time
        logger.info(
            f"Feature calculation completed in {calc_duration:.2f}s - {len(all_features)} symbols total "
    ...
    async def _calculate_single_symbol_features(
        ...
        try:
            ...
            market_data = await self.market_data_repo.get_data_for_symbols_and_range(
                symbols=[symbol], start_time=start_time, end_time=end_time
            )
            ...
            if validation_result.has_warnings:
                logger.warning(
                    f"Feature validation warnings for {symbol}: {validation_result.warnings}"
                )
            ...
            if len(symbol_data) > self.streaming_threshold_rows:
                ...
                feature_df = await loop.run_in_executor(
                    self.executor, self._calculate_basic_features, symbol_data
                )
    ...
    async def _execute_parallel_feature_calculation(
        self, tasks: list[Any], symbols: list[str]
    ) -> dict[str, pd.DataFrame]:
        """
        ...
        max_concurrent = self.config.get(
            "orchestrator.features.parallel_processing.max_concurrent_tasks", 10
        )
        ...
        for coro in asyncio.as_completed(bounded_tasks):
            ...
            if completed_count % 5 == 0 or completed_count == len(tasks):
                logger.info(
                    f"Feature calculation progress: {completed_count}/{len(tasks)} symbols completed"
                )
    ...
    async def _store_features(
        self, symbol: str, feature_df: pd.DataFrame, start_time: datetime, end_time: datetime
    ):
        """
        Store calculated features in both hot and cold storage.
        ...
        try:
            ...
            if not feature_df.empty:
                ...
                if batch_result.success:
                    logger.debug(
                        f"Stored {len(feature_records)} feature records for {symbol} in PostgreSQL"
                    )
                else:
                    logger.error(
                        f"Failed to store features for {symbol} in PostgreSQL: {batch_result.errors}"
                    )
    ...
    async def _calculate_features_streaming(
        self, data: pd.DataFrame, feature_sets: list[str] | None = None
    ) -> pd.DataFrame:
        """
        ...
        async with streaming_context(self.streaming_config) as streamer:
            ...
            logger.info(
                f"Streaming feature calculation completed: "
    ...
    def _calculate_basic_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "close" in data.columns:
            features["sma_10"] = (
                ...
                .rolling(
                    window=self.config.get(
                        "orchestrator.features.technical_indicators.sma_short_window", 10
                    )
                )
                .mean()
            )
            features["sma_20"] = (
                ...
                .rolling(
                    window=self.config.get(
                        "orchestrator.features.technical_indicators.sma_long_window", 20
                    )
                )
                .mean()
            )
        ...
        if "volume" in data.columns:
            features["volume_sma"] = (
                ...
                .rolling(
                    window=self.config.get(
                        "orchestrator.features.technical_indicators.volume_sma_window", 10
                    )
                )
                .mean()
            )
    ...
    async def _on_scanner_alert(self, event: ScannerAlertEvent):
        ...
        try:
            ...
            required_features = self._alert_feature_mapping.get(
                alert_type, self._alert_feature_mapping["default"]
            )

            # Calculate fresh features for the alerted symbol
            end_time = datetime.now(UTC)
            start_time = end_time - pd.Timedelta(
                days=self.config.get("orchestrator.lookback_periods.feature_calculation_days", 30)
            )
    ...
    async def _on_feature_request(self, event: FeatureRequestEvent):
        ...
        try:
            ...
            logger.info(
                f"Received feature request: {len(symbols)} symbols, {len(features)} features, priority {priority}"
            )
            ...
            if priority >= 8:
                ...
                await self.calculate_features(
                    symbols=symbols, start_time=start_time, end_time=end_time, feature_sets=features
                )
            else:
    ...
    async def _process_batch_queue(self):
        ...
        batch_interval = self.config.get(
            "orchestrator.features.batch_processing.interval_seconds", 5
        )
        ...
        while True:
            try:
                ...
                for request in pending_requests:
                    end_time = datetime.now(UTC)
                    start_time = end_time - pd.Timedelta(
                        days=self.config.get(
                            "orchestrator.lookback_periods.feature_calculation_days", 30
                        )
                    )
                    ...
                    logger.debug(
                        f"Processed batch request: {len(request['symbols'])} symbols, priority {request['priority']}"
                    )
    ...
    async def run_batch_calculation(self, symbols: list[str]):
        ...
        with self.memory_monitor.memory_context(
            "batch_feature_calculation", gc_before=True, gc_after=True
        ):
            # Process in batches to avoid overwhelming the system
            batch_size = self.config.get(
                "orchestrator.features.batch_processing.default_batch_size", 50
            )
            ...
            for i in range(0, len(symbols), batch_size):
                ...
                with self.memory_monitor.memory_context(f"batch_{batch_num}", gc_before=True):
                    ...
                    batch_features = await self.calculate_features(
                        symbols=batch, start_time=start_time, end_time=end_time
                    )

                    batch_duration = time.time() - batch_start_time

                    logger.info(
                        f"Batch {batch_num}/{total_batches} completed in {batch_duration:.2f}s "
                ...
                await asyncio.sleep(
                    self.config.get(
                        "orchestrator.features.batch_processing.processing_delay_seconds", 1
                    )
                )
    ...
    def _log_calculation_stats(self):
        ...
        logger.info("Feature calculation statistics:")
    ...
    async def calculate_streaming_aggregations(
        self, symbols: list[str], aggregation_type: str = "market_summary", lookback_days: int = 30
    ) -> pd.DataFrame:
        """
        ...
        """
        logger.info(
            f"Starting streaming aggregation for {len(symbols)} symbols: {aggregation_type}"
        )
        ...
        if aggregation_type == "market_summary":
            ...
            aggregations = {"close": "last", "high": "max", "low": "min", "volume": "sum"}
        else:
        ...
        if len(all_market_data) > self.streaming_threshold_rows:
            ...
            result = await aggregator.aggregate_streaming(
                data_source=all_market_data, group_by=group_by, aggregations=aggregations
            )
        else:
    ...
    async def process_large_dataset_streaming(
        ...
        async with streaming_context(self.streaming_config) as streamer:
            ...
            result = await streamer.process_stream(data_source, enhanced_processor, output_path)
