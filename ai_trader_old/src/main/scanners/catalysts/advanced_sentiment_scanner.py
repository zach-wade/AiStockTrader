...
class AdvancedSentimentScanner(CatalystScannerBase):
    ...
    def __init__(
        ...
        self.finbert: Pipeline | None = None
        self.intent_classifier: Pipeline | None = None
    ...
    def _initialize_transformer_models(self) -> tuple[Optional["Pipeline"], Optional["Pipeline"]]:
        ...
        if not TRANSFORMERS_AVAILABLE:
            logger.warning(
                "Transformers library not installed. Advanced sentiment analysis is disabled."
            )
            return None, None
    ...
    async def scan(self, symbols: list[str], **kwargs) -> list[ScanAlert]:
        ...
        if not self.finbert or not self.intent_classifier:
            logger.warning(
                "Transformer models not available. Skipping advanced sentiment analysis."
            )
            return []
        ...
        with timer() as t:
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    cache_key = (
                        f"advanced_sentiment:{','.join(sorted(symbols[:10]))}:{self.lookback_hours}"
                    )
                    cached_alerts = await self.cache.get_cached_result(
                        self.name, "batch", cache_key
                    )
                    if cached_alerts is not None:
                        logger.info("Using cached results for advanced sentiment scan")
    ...
    async def run(self, universe: list[str]) -> dict[str, dict[str, Any]]:
        ...
        for alert in alerts:
            catalyst_signals[alert.symbol] = {
                ...
                "metadata": {"avg_sentiment": alert.metadata.get("avg_sentiment", 0)},
            }

        return catalyst_signals

    def _combine_textual_data(
        self, news_data: dict[str, list[dict]], social_data: dict[str, list[dict]]
    ) -> pd.DataFrame:
    ...
    def _analyze_content(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        candidate_labels = self.params.get(
            "intent_labels", ["positive catalyst", "negative catalyst", "general discussion"]
        )
    ...
    def _generate_alerts(self, df: pd.DataFrame) -> list[ScanAlert]:
        ...
        sentiment_by_symbol = (
            df.groupby("symbol")["sentiment_score"].agg(["mean", "count"]).to_dict("index")
        )
        ...
        for symbol, stats in sentiment_by_symbol.items():
            ...
            if abs(avg_sentiment) >= self.min_sentiment_threshold:
                ...
                alert_type = (
                    AlertType.SENTIMENT_SURGE if avg_sentiment > 0 else AlertType.SENTIMENT_SURGE
                )

                # Get dominant intent for this symbol
                symbol_df = df[df["symbol"] == symbol]
                dominant_intent = (
                    symbol_df.groupby("intent_label").size().idxmax()
                ...
                if self.metrics:
                    self.metrics.record_alert_generated(
                        self.name, alert_type, symbol, normalized_score
                    )
