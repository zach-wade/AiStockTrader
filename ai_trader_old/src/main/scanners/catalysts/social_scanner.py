...
class SocialScanner(CatalystScannerBase):
    ...
    def __init__(
        ...
        """
        super().__init__(
            "SocialScanner", config, repository, event_bus, metrics_collector, cache_manager
        )
        ...
        self.platform_settings = self.params.get(
            "platforms",
            {
        ...
        self.alert_thresholds = self.params.get(
            "alert_thresholds", {"extreme": 0.9, "high": 0.7, "moderate": 0.5}
        )
        ...
        with timer() as t:
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    cache_key = (
                        f"social_scan:{','.join(sorted(symbols[:10]))}:{self.lookback_hours}"
                    )
                    cached_alerts = await self.cache.get_cached_result(
                        self.name, "batch", cache_key
                    )
                    if cached_alerts is not None:
                        logger.info("Using cached results for social scan")
                ...
                logger.info(
                    f"✅ Social Scanner: Found {len(alerts)} alerts " f"in {t.elapsed * 1000:.2f}ms"
                )
    ...
    async def _process_symbol_social_data(
        self, symbol: str, social_data: list[dict[str, Any]]
    ) -> list[ScanAlert]:
        """
    ...
    def _calculate_social_metrics(self, data: dict[str, Any]) -> dict[str, float]:
        ...
        if data["sentiment_scores"]:
            ...
            metrics["sentiment_trend"] = self._calculate_trend(data["sentiment_scores"])
        ...
        if data["engagement_metrics"]:
            ...
            viral_posts = sum(
                1 for e in data["engagement_metrics"] if e > high_engagement_threshold
            )
    ...
    def _check_sentiment_extreme(self, symbol: str, metrics: dict[str, float]) -> ScanAlert | None:
        """
    ...
    def _check_volume_spike(
        self, symbol: str, metrics: dict[str, float], data: dict[str, Any]
    ) -> ScanAlert | None:
        """
        ...
        if volume_spike_ratio > self.volume_spike_threshold:
            ...
            score = min((volume_spike_ratio - 1) / (self.volume_spike_threshold * 2), 1.0)
            ...
            if score > self.alert_thresholds["moderate"]:
                ...
                if self.metrics:
                    self.metrics.record_alert_generated(
                        self.name, AlertType.SOCIAL_VOLUME, symbol, score
                    )
    ...
    def _check_viral_pattern(
        ...
        if viral_coefficient > self.viral_threshold:
            ...
            score = min(viral_coefficient * 0.6 + min(engagement_ratio / 10, 0.4), 1.0)
            ...
            if score > self.alert_thresholds["moderate"]:
                ...
                if self.metrics:
                    self.metrics.record_alert_generated(
                        self.name, AlertType.SOCIAL_VIRAL, symbol, score
                    )
    ...
    def _check_coordinated_activity(self, symbol: str, data: dict[str, Any]) -> ScanAlert | None:
        """
        ...
        if burst_ratio > 0.3:  # 30% of posts within 1 minute of each other
            ...
            if self.metrics:
                self.metrics.record_alert_generated(
                    self.name, AlertType.UNUSUAL_ACTIVITY, symbol, score
                )
