...
class MarketValidationScanner(CatalystScannerBase):
                ...
                if catalyst_signals:
                    ...
                    if self.cache and self.use_cache:
                        ...
                        if cached_alerts is not None:
                            logger.info("Using cached results for market validation")
                        ...
                        if score > 0:
                            ...
                            if self.metrics:
                                self.metrics.record_alert_generated(
                                    self.name, AlertType.MOMENTUM, symbol, normalized_score
                                )
    ...
    async def _validate_catalyst_signals(
        self, catalyst_signals: dict[str, dict[str, Any]]
        ...
        logger.info(
            f"📈 Market Validation Scanner: Validating {len(symbols_to_validate)} catalyst signals..."
        )
        ...
        for symbol in symbols_to_validate:
            ...
            # Calculate validation score
            validation_score, validation_reasons = self._calculate_validation_score(
                symbol, price_data
            )
            ...
            if symbol in validated_signals:
                ...
                validated_signals[symbol]["final_score"] = (original_score * 0.6) + (
    ...
    def _convert_validated_signals_to_alerts(
