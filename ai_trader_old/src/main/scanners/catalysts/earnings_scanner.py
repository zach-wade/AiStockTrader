...
class EarningsScanner(CatalystScannerBase):
    ...
    def __init__(
        ...
        """
        super().__init__(
            "EarningsScanner", config, repository, event_bus, metrics_collector, cache_manager
        )
        ...
        self.catalyst_weights = self.params.get("weights", {"today": 5.0, "upcoming": 3.0})
    ...
    async def scan(self, symbols: list[str], **kwargs) -> list[ScanAlert]:
        ...
        with timer() as t:
            logger.info(
                f"📈 Earnings Scanner: Checking {len(symbols)} symbols for upcoming earnings..."
            )
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    ...
                    if cached_alerts is not None:
                        logger.info("Using cached results for earnings scan")
                ...
                query_filter = QueryFilter(
                    ...
                    end_date=datetime.now(UTC) + timedelta(days=self.days_ahead_threshold),
                )
                ...
                for symbol, earnings_info in earnings_data.items():
                    ...
                    symbol_alerts = await self._process_symbol_earnings(symbol, events)
    ...
    async def _process_symbol_earnings(
        self, symbol: str, earnings_events: list[dict[str, Any]]
    ) -> list[ScanAlert]:
        """
        ...
        for event in earnings_events:
            ...
            if self.metrics:
                self.metrics.record_alert_generated(
                    self.name, AlertType.EARNINGS_ANNOUNCEMENT, symbol, normalized_score
                )
