...
class IntermarketScanner(CatalystScannerBase):
    ...
    def __init__(
        ...
        """
        super().__init__(
            "IntermarketScanner", config, repository, event_bus, metrics_collector, cache_manager
        )
        ...
        self.market_indicators = self.params.get(
            "market_indicators",
            {
                "SPY": "S&P 500",
                "QQQ": "NASDAQ",
        ...
        self.sector_etfs = self.params.get(
            "sector_etfs",
            {
                "XLK": "Technology",
                "XLF": "Financials",
        ...
        # Alert thresholds
        self.alert_thresholds = self.params.get(
            "alert_thresholds",
            {
        ...
        with timer() as t:
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    cache_key = (
                        f"intermarket_scan:{','.join(sorted(symbols[:10]))}:{self.lookback_days}"
                    )
                    cached_alerts = await self.cache.get_cached_result(
                        self.name, "batch", cache_key
                    )
                    if cached_alerts is not None:
                        logger.info("Using cached results for intermarket scan")
                        return cached_alerts

                # Build query filter
                query_filter = QueryFilter(
                    symbols=symbols
                    + list(self.market_indicators.keys())
                ...
                for i in range(0, len(symbols), batch_size):
                    ...
                    batch_alerts = await self._analyze_symbol_batch(batch_symbols, symbol_data)
                ...
                if self.cache and self.use_cache and alerts:
                    await self.cache.cache_result(
                        self.name, "batch", cache_key, alerts, ttl_seconds=1800  # 30 minute TTL
                    )
        ...
        for alert in alerts:
            signal = {
                ...
                "metadata": {
                    ...
                    "correlation": alert.metadata.get(
                        "recent_correlation", alert.metadata.get("correlation")
                    ),
    ...
    async def _analyze_symbol_batch(
        self, symbols: list[str], symbol_data: dict[str, pd.DataFrame]
    ) -> list[ScanAlert]:
        """
        ...
        async def analyze_single_symbol(symbol: str) -> list[ScanAlert]:
            ...
            correlation_alerts = self._check_correlation_patterns(
                symbol, symbol_data[symbol], symbol_data
            )
            symbol_alerts.extend(correlation_alerts)

            # Check for divergences
            divergence_alerts = self._check_divergences(symbol, symbol_data[symbol], symbol_data)
    ...
    def _check_correlation_patterns(
        self, symbol: str, symbol_df: pd.DataFrame, market_data: dict[str, pd.DataFrame]
    ) -> list[ScanAlert]:
        """
        ...
        for indicator, indicator_name in self.market_indicators.items():
            ...
            historical_correlation = symbol_returns.loc[
                common_dates[: -self.correlation_window]
            ].corr(indicator_returns.loc[common_dates[: -self.correlation_window]])
            ...
            if correlation_change > 0.5 and abs(historical_correlation) > self.min_correlation:
                score = (
                    min(correlation_change / 0.5, 1.0) * self.alert_thresholds["correlation_break"]
                )
                ...
                if self.metrics:
                    self.metrics.record_alert_generated(
                        self.name, AlertType.CORRELATION_ANOMALY, symbol, score
                    )

            # Check for new strong correlations
            elif (
                abs(recent_correlation) > self.min_correlation and abs(historical_correlation) < 0.3
            ):
    ...
    def _check_divergences(
        ...
        if abs(correlation) > self.min_correlation:
            ...
            for i in range(20, 60):
                hist_symbol_perf = (
                    symbol_df["close"].iloc[-i] / symbol_df["close"].iloc[-i - 20] - 1
                )
            ...
            if std_diff > 0:
                ...
                if abs(z_score) > self.divergence_threshold:
                    ...
                    if self.metrics:
                        self.metrics.record_alert_generated(
                            self.name, AlertType.DIVERGENCE, symbol, score
                        )

        return alerts

    def _check_sector_relationships(
        self, symbol: str, symbol_df: pd.DataFrame, market_data: dict[str, pd.DataFrame]
    ) -> list[ScanAlert]:
        """
        ...
        for sector_etf, sector_name in self.sector_etfs.items():
            if sector_etf in market_data:
                ...
                if len(common_dates) >= 60:
                    correlation = symbol_returns.loc[common_dates].corr(
        ...
        if abs(sector_info["correlation"]) > 0.6:
            ...
            recent_sector_perf = (
                sector_data["close"].iloc[-5:].mean() / sector_data["close"].iloc[-10:-5].mean() - 1
            )

            historical_sector_perf = (
                sector_data["close"].iloc[-30:-20].mean()
                / sector_data["close"].iloc[-40:-30].mean()
                - 1
            )
        ...
        if "VIX" in market_data:
            ...
            if len(vix_data) >= 20:
                ...
                if current_vix > vix_ma * 1.5:
                    score = (
                        min((current_vix / vix_ma - 1), 1.0)
                        * self.alert_thresholds["regime_change"]
                    )
                    ...
                    if self.metrics:
                        self.metrics.record_alert_generated(
                            self.name, AlertType.REGIME_CHANGE, "MARKET", score
                        )
