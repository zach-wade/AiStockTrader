...
class NewsScanner(CatalystScannerBase):
    ...
    def __init__(
        ...
        """
        super().__init__(
            "NewsScanner", config, repository, event_bus, metrics_collector, cache_manager
        )
        ...
        # News impact weights
        self.impact_weights = self.params.get(
            "impact_weights",
            {
                "upgrade": 0.9,
                "downgrade": 0.8,
        ...
        self.classification_keywords = self.params.get(
    ...
    async def scan(self, symbols: list[str], **kwargs) -> list[ScanAlert]:
        ...
        with timer() as t:
            logger.info(
                f"📰 News Scanner: Analyzing {len(symbols)} symbols for news from the last {self.lookback_hours} hours..."
            )
            ...
            try:
                ...
                if self.cache and self.use_cache:
                    ...
                    if cached_alerts is not None:
                        logger.info("Using cached results for news scan")
                ...
                for symbol, news_items in news_sentiment_data.items():
                    ...
                    symbol_alerts = await self._process_symbol_news(symbol, news_items)
    ...
    async def _process_symbol_news(
        ...
        for item in news_items:
            ...
            published_at = item.get("published_at", datetime.now(UTC))

            # Skip if sentiment too low
            if (
                sentiment_score < self.min_sentiment_score
                and sentiment_score > -self.min_sentiment_score
            ):
                continue
            ...
            # Record metric
            if self.metrics:
                self.metrics.record_alert_generated(self.name, alert_type, symbol, final_score)
