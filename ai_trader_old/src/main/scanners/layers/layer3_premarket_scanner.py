...
class Layer3PreMarketScanner:
    ...
    def __init__(self, config: Any = None, event_bus: IEventBus = None):
        ...
        self.company_repo: ICompanyRepository = repo_factory.create_company_repository(
            self.db_adapter
        )
    ...
    async def scan_premarket(self, input_symbols: list[str] | None = None) -> dict[str, Any]:
        """
        ...
        for symbol in symbols:
            try:
                ...
                bars_df = await self.polygon_client.fetch_market_data(
                    symbol=symbol, start=start, end=now, timeframe="1Min"
                )
    ...
    async def _calculate_rvol(
        self, symbol: str, current_volume: float, current_time: datetime
    ) -> float:
    ...
    async def _apply_premarket_filters(self, premarket_data: dict[str, dict]) -> list[dict]:
        ...
        for symbol, data in premarket_data.items():
            ...
            if (
                rvol >= self.min_rvol
                and abs(data["price_change"]) >= self.min_price_change
                and data["volume"] >= self.min_premarket_volume
            ):
    ...
    async def _update_database(self, qualified_symbols: list[dict]):
        ...
        try:
            ...
            premarket_scores = {
                s["symbol"]: s.get("premarket_score", 0.0) for s in qualified_symbols
            }
        ...
        def execute_updates(session):
            ...
            for item in qualified_symbols:
                session.execute(
                    update_query,
                    {"symbol": item["symbol"], "score": item["final_score"], "rvol": item["rvol"]},
                )
    ...
    def _generate_report(
        ...
        start_time: datetime,
        universe: list[str],
        premarket_results: dict,
        qualified_symbols: list[dict],
    ) -> dict:
        ...
        report = {
            ...
            "statistics": {
                ...
                "average_price_change": (
                    np.mean([abs(p) for p in price_changes]) if price_changes else 0
                ),
