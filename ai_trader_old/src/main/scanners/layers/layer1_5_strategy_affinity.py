...
from main.feature_pipeline.calculators.strategy_affinity_calculator import (
    StrategyAffinityCalculator,
)
...
class Layer1_5_StrategyAffinityFilter:
    ...
    def __init__(self, config: Any, db_adapter: IAsyncDatabase, event_bus: IEventBus = None):
        ...
        self.regime_strategy_preferences = self.params.get(
    ...
    async def run(self, input_symbols: list[str]) -> list[str]:
        ...
        """
        logger.info(
            f"🎯 Starting Layer 1.5 Strategy Affinity Filter for {len(input_symbols)} symbols..."
        )
        ...
        try:
            ...
            symbols_without_scores = [
                symbol for symbol in input_symbols if symbol not in symbols_with_scores
            ]

            if symbols_without_scores:
                logger.info(
                    f"📊 Found {len(symbols_without_scores)} symbols without affinity scores"
                )
                if self.auto_calculate_affinities:
                    # Calculate missing affinities
                    calculated_affinities = await self._calculate_missing_affinities(
                        symbols_without_scores
                    )
            ...
            logger.info(
                f"✅ Layer 1.5 Filter: {len(input_symbols)} -> {len(final_symbols)} symbols in {duration:.2f}s"
            )
    ...
    async def _update_layer_metadata(
        self, symbols: list[str], affinity_scores: dict[str, dict[str, Any]]
    ):
        ...
        try:
            for symbol in symbols:
                ...
                result = await self.company_repository.update_company_metadata(
                    symbol=symbol, metadata={"scanner_metadata": json.dumps(existing_metadata)}
                )
            ...
            for row in rows:
                ...
                if symbol not in affinity_data:  # First (most recent) record for this symbol
                    affinity_data[symbol] = {
                        ...
                        "composite_scores": (
                            json.loads(row["composite_scores_json"])
                            if row["composite_scores_json"]
                            else {}
                        ),
    ...
    async def _calculate_missing_affinities(
        self, symbols_without_scores: list[str]
        ...
        logger.info(
            f"🔄 Calculating strategy affinities for {len(symbols_without_scores)} symbols..."
        )
        ...
        for i in range(0, len(symbols_without_scores), self.max_batch_size):
            ...
            try:
                ...
                batch_affinities = (
                    await self.strategy_affinity_calculator.calculate_affinities_batch(batch)
                )

                if batch_affinities:
                    # Save to database for future use
                    await self.strategy_affinity_calculator.save_affinities_to_database(
                        batch_affinities
                    )
                    ...
                    for symbol, scores in batch_affinities.items():
                        calculated_affinities[symbol] = {
                            ...
                            "best_strategy": max(scores.items(), key=lambda x: x[1])[0].replace(
                                "_affinity", ""
                            ),
                    ...
                    logger.info(
                        f"   ✅ Successfully calculated affinities for {len(batch_affinities)} symbols"
                    )
                else:
                ...
                logger.error(
                    f"   ❌ Error calculating affinities for batch starting at index {i}: {e}"
                )
                continue

        logger.info(
            f"🔄 Completed affinity calculation: {len(calculated_affinities)} symbols processed"
        )
        return calculated_affinities

    def _apply_strategy_affinity_filter(
        self, universe: list[str], current_regime: str, affinity_scores: dict[str, dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def _calculate_regime_compatibility(
        self, symbol_affinities: dict[str, Any], preferred_strategies: list[str]
    ) -> dict[str, Any]:
    ...
    def _rank_and_limit_symbols(self, qualified_symbols: list[dict[str, Any]]) -> list[str]:
    ...
    async def _save_layer1_5_output(
        ...
        timestamped_file = (
            self.output_dir / f'layer1_5_universe_{timestamp.strftime("%Y%m%d_%H%M%S")}.json'
        )
    ...
    def _generate_filter_statistics(
        self, input_universe: list[str], output_symbols: list[str], current_regime: str
    ) -> dict[str, Any]:
