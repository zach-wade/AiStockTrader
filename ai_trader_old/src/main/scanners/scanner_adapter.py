...
class AlertToSignalConfig:
    ...
    alert_weights: dict[AlertType, float] = field(
...
class ScannerAdapter(IScannerAdapter):
    ...
    def __init__(
        ...
        if engine is not None:
            ...
            self.engine = ParallelScannerEngine(config.engine_config, metrics_collector)
    ...
    async def scan_once(self, symbols: list[str] | None = None) -> list[Signal]:
        """
    ...
    def _calculate_alert_score(self, alert: ScanAlert) -> float:
        ...
        base_weight = self.config.alert_to_signal_config.alert_weights.get(alert.alert_type, 0.5)

        # Adjust by confidence
        score = base_weight * alert.confidence

        # Adjust by severity
        severity_multiplier = {"low": 0.7, "medium": 1.0, "high": 1.3, "critical": 1.5}
    ...
    async def _publish_alerts_to_event_bus(self, alerts: list[ScanAlert]) -> None:
        ...
        try:
            ...
            # Local imports
            from main.events.types import EventType, ScannerAlertEvent
    ...
    def _track_signal_generation(self, signal: Signal, agg_alert: AggregatedAlert) -> None:
        ...
        if self.metrics:
            self.metrics.increment(
                ...
                tags={"signal_type": signal.signal_type.value, "symbol": signal.symbol},
            )
    ...
    def get_alert_buffer_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_alerts_per_symbol": (
                sum(a.alert_count for a in self._alert_buffer.values()) / len(self._alert_buffer)
