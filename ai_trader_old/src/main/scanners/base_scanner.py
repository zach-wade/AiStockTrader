...
from main.interfaces.events import EventType, IEventBus, ScannerAlertEvent
...
class BaseScanner(ABC):
    ...
    def filter_alerts_by_score(self, alerts: list[ScanAlert], min_score: float) -> list[ScanAlert]:
        ...
        """
        return [alert for alert in alerts if alert.score is not None and alert.score >= min_score]
    ...
    async def publish_alerts_to_event_bus(
        self, alerts: list[ScanAlert], event_bus: IEventBus | None
    ) -> None:
        """
