...
class PerformanceTracker:
    ...
    def add_portfolio_value(self, value: float, timestamp: datetime | None = None):
    ...
    def calculate_metrics(
        self,
        timeframe: TimeFrame = TimeFrame.INCEPTION,
        ...
        self._cache_expiry[cache_key] = datetime.now(UTC) + timedelta(minutes=5)

        return metrics

    def _calculate_metrics(
        self, timeframe: TimeFrame, start_date: datetime | None, end_date: datetime | None
    ) -> PerformanceMetrics:
        ...
        period_days = (
            (filtered_timestamps[-1] - filtered_timestamps[0]).days
        ...
        sortino_ratio = RiskAdjustedCalculator.sortino_ratio(
            daily_returns, 0.0, self.risk_free_rate
        )
        ...
        return PerformanceMetrics(
            start_date=filtered_timestamps[0] if filtered_timestamps else datetime.now(UTC),
            end_date=filtered_timestamps[-1] if filtered_timestamps else datetime.now(UTC),
            period_days=period_days,
            total_return=total_return,
            annualized_return=annualized_return,
            cagr=ReturnCalculator.cagr(
                self.initial_capital, filtered_values[-1], period_days / 365.25
            ),
    ...
    def _filter_trades(
        ...
        return [trade for trade in self.trade_records if start_date <= trade.entry_time <= end_date]
    ...
    def _create_empty_metrics(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            start_date=now, end_date=now, period_days=0, risk_free_rate=self.risk_free_rate
        )
...
def create_performance_tracker(
    initial_capital: float = 100000.0, risk_free_rate: float = 0.02, enable_alerts: bool = True
) -> PerformanceTracker:
