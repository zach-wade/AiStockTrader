...
class ArchiveWidget:
    ...
    def __init__(
        self,
        metrics_collector: Any | None = None,  # ArchiveMetricsCollector when available
        config: dict[str, Any] | None = None,
        unified_metrics: Any | None = None,  # UnifiedMetrics when available
    ):
        """
    ...
    def _create_storage_panel(self) -> dict[str, Any]:
        ...
        for file_type, stats in storage.get("by_type", {}).items():
            by_type_data.append(
                {"name": file_type, "value": stats.get("size_mb", 0), "y": stats.get("size_mb", 0)}
            )
    ...
    def _create_health_panel(self) -> dict[str, Any]:
        ...
        storage_status = (
            "OK" if storage_mb < 10000 else "WARNING" if storage_mb < 50000 else "CRITICAL"
        )
        health_items.append(
            {
                "name": "Storage",
        ...
        compression_status = (
            "OK" if compression > 1.5 else "WARNING" if compression > 1.0 else "CRITICAL"
        )
        health_items.append(
            {
                "name": "Compression",
    ...
    def _get_layout_config(self) -> dict[str, Any]:
        ...
        return {
            "grid": {"columns": 24, "rows": "auto", "gap": 1},
            "responsive": True,
    ...
    def _get_status_color(self, status: str) -> str:
        ...
        colors = {"OK": "green", "WARNING": "yellow", "CRITICAL": "red", "UNKNOWN": "gray"}
    ...
    async def _enrich_with_unified_metrics(self):
        ...
        try:
            ...
            archive_alerts = [
                alert for alert in active_alerts if "archive" in alert.metric_name.lower()
            ]
