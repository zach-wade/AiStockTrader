...
from .events import CircuitBreakerEvent, CircuitBreakerEventBuilder
from .registry import BaseBreaker, BreakerRegistry
from .types import BreakerMetrics, BreakerStatus, BreakerType, MarketConditions
...
class CircuitBreakerFacade(ErrorHandlingMixin):
    ...
    async def check_conditions(
        self, metrics: BreakerMetrics, market_conditions: MarketConditions
    ) -> SystemStatus:
    ...
    async def _handle_breaker_trip(
        self, breaker_name: str, breaker: BaseBreaker, metrics: BreakerMetrics
    ):
    ...
    async def _emit_warning(self, breaker_name: str, breaker: BaseBreaker, metrics: BreakerMetrics):
    ...
    def is_trading_allowed(self) -> bool:
        ...
        return (
            self._system_enabled and not self._emergency_stop and len(self._tripped_breakers) == 0
        )
