...
class VolatilityBreaker(BaseBreaker):
    ...
    def __init__(self, breaker_type: BreakerType, config: BreakerConfig):
        ...
        self.volatility_acceleration_threshold = (
            self.volatility_threshold / 10
        )  # 10% of main threshold
    ...
    async def check(
        ...
        if market_conditions.volatility > self.volatility_threshold:
            self.logger.warning(
                f"High volatility detected: {market_conditions.volatility:.2%} > {self.volatility_threshold:.2%}"
            )
            return True
    ...
    async def check_warning_conditions(
        ...
        if market_conditions.volatility > self.warning_threshold:
            self.logger.warning(
                f"Approaching volatility threshold: {market_conditions.volatility:.2%}"
            )
            return True
    ...
    async def _check_volatility_breakout(self, current_volatility: float) -> bool:
        ...
        if std_vol > 0:
            ...
            if z_score > 3.0:  # 3 standard deviations
                self.logger.warning(
                    f"Volatility breakout detected: {z_score:.2f} standard deviations"
                )
                return True
    ...
    def get_metrics(self) -> BreakerMetrics:
        ...
        if self.volatility_history:
            ...
            if len(self.volatility_history) >= 2:
                ...
                metrics.recent_volatility = (
                    np.mean(historical_vols[-10:])
                    if len(historical_vols) >= 10
                    else np.mean(historical_vols)
                )
