...
class DrawdownBreaker(BaseBreaker):
    ...
    def __init__(self, breaker_type: BreakerType, config: BreakerConfig):
        ...
        self.drawdown_acceleration_threshold = (
            self.max_drawdown / 20
        )  # 5% of max drawdown per period
    ...
    async def check_warning_conditions(
        ...
        if len(self.drawdown_history) >= 5:
            ...
            if len(recent_drawdowns) >= 2:
                drawdown_acceleration = (recent_drawdowns[-1] - recent_drawdowns[0]) / len(
                    recent_drawdowns
                )
                if drawdown_acceleration > self.drawdown_acceleration_threshold / 2:
                    self.logger.warning(
                        f"Drawdown accelerating: {drawdown_acceleration:.4%}/period"
                    )
                    return True
    ...
    async def _check_underwater_duration(self) -> bool:
        ...
        current_drawdown = self._calculate_drawdown(
            self.portfolio_value_history[-1][1] if self.portfolio_value_history else 0
        )

        if underwater_duration > 24 and current_drawdown > 0.05:  # 5% drawdown for 24+ hours
            self.logger.warning(
                f"Extended underwater period: {underwater_duration:.1f} hours with {current_drawdown:.2%} drawdown"
            )
            return True
    ...
    def get_recovery_analysis(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recovery_success_rate": len(
                [rp for rp in recovery_periods if rp["recovery_amount"] > 0.01]
            )
