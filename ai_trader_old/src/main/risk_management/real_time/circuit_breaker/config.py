...
from .types import BreakerConfiguration, BreakerType
...
class BreakerConfig:
    ...
    def get_all_breaker_configs(self) -> dict[BreakerType, BreakerConfiguration]:
        ...
        return {breaker_type: self.get_breaker_config(breaker_type) for breaker_type in BreakerType}
    ...
    def validate_limits(self) -> dict[str, str]:
        ...
        if self.volatility_threshold > 0.10:
            warnings["volatility_threshold"] = (
                f"High volatility threshold: {self.volatility_threshold:.2%}"
            )

        if self.loss_rate_threshold > 0.05:
            warnings["loss_rate_threshold"] = (
                f"High loss rate threshold: {self.loss_rate_threshold:.2%}"
            )

        if self.max_position_size > 0.20:
            warnings["max_position_size"] = (
                f"High position size limit: {self.max_position_size:.2%}"
            )
