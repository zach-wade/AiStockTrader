...
class BaseBreaker(ABC):
    ...
    def update_threshold(self, new_threshold: float):
        ...
        self.logger.info(
            f"{self.breaker_type.value} threshold updated: {old_threshold} -> {new_threshold}"
        )
...
class BreakerRegistry:
    ...
    def register_breaker_class(self, breaker_type: BreakerType, breaker_class: type[BaseBreaker]):
        ...
        self.logger.info(
            f"Registered breaker class: {breaker_type.value} -> {breaker_class.__name__}"
        )
    ...
    async def check_all_breakers(
        ...
        for breaker_type, breaker in self.breakers.items():
            ...
            try:
                ...
                if not is_tripped:
                    is_warning = await breaker.check_warning_conditions(
    ...
    def get_breaker(self, breaker_type: BreakerType) -> BaseBreaker | None:
    ...
    async def update_breaker_threshold(self, breaker_type: BreakerType, new_threshold: float):
        ...
        if breaker_type in self.breakers:
            ...
            self.logger.warning(
                f"Cannot update threshold for {breaker_type.value} breaker: not found"
            )
