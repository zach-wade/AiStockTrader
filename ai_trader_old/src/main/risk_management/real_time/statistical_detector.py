...
from .anomaly_models import AnomalyEvent
...
class StatisticalAnomalyDetector(ErrorHandlingMixin):
    ...
    def detect_price_anomalies(
        self, price_history: list[float], current_price: float, symbol: str
    ) -> list[AnomalyEvent]:
        ...
        with self._handle_error("detecting price anomalies"):
            ...
            z_score_anomaly = self._detect_zscore_anomaly(
                historical_returns, current_return, "price_return"
            )

            if z_score_anomaly:
                anomaly_type = (
                    AnomalyType.PRICE_SPIKE if current_return > 0 else AnomalyType.PRICE_CRASH
                )
            ...
            if self.config.use_rolling_stats and len(historical_returns) >= 50:
                iso_anomaly = self._detect_isolation_forest_anomaly(
                    historical_returns, current_return, symbol, "price"
                )
    ...
    def detect_volume_anomalies(
        self, volume_history: list[int], current_volume: int, symbol: str
    ) -> list[AnomalyEvent]:
        ...
        with self._handle_error("detecting volume anomalies"):
            ...
            z_score_anomaly = self._detect_zscore_anomaly(
                historical_log_volumes, current_log_volume, "volume", use_robust=True
            )
    ...
    def detect_volatility_anomalies(
        ...
        with self._handle_error("detecting volatility anomalies"):
            ...
            z_score_anomaly = self._detect_zscore_anomaly(
                np.array(historical_vols), current_vol, "volatility"
            )
    ...
    def _detect_isolation_forest_anomaly(
        self, historical_data: np.ndarray, current_value: float, symbol: str, data_type: str
    ) -> dict[str, Any] | None:
        ...
        if key not in self._isolation_forests:
            self._isolation_forests[key] = IsolationForest(
                contamination=self.config.outlier_fraction, random_state=42, n_estimators=100
            )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "detection_rate": (
                self._anomaly_count / self._detection_count if self._detection_count > 0 else 0
            ),
