...
class MarketRegimeDetector:
    ...
    def __init__(self, config: dict[str, Any] = None):
        ...
        logger.info(
            f"Regime detector initialized with volatility thresholds: {self.volatility_thresholds}"
        )

    def detect_regime_change(
        self, price_history: list[float], volume_history: list[int] = None
    ) -> list[AnomalyEvent]:
        """Detect market regime changes."""
        ...
        if self.current_regime and self._is_regime_change(self.current_regime, current_regime):
            ...
            anomaly = AnomalyEvent(
                ...
                severity=self._determine_regime_change_severity(
                ...
                market_context={
                    ...
                    "volatility_change": current_regime.avg_volatility
                    - self.current_regime.avg_volatility,
                },
                contributing_factors=self._identify_regime_change_factors(
                    self.current_regime, current_regime
                ),
    ...
    def get_regime_statistics(self) -> dict[str, Any]:
        ...
        for i in range(1, len(self.regime_history)):
            duration = (
                self.regime_history[i].start_time - self.regime_history[i - 1].start_time
            ).total_seconds()
        ...
        return {
            ...
            "current_volatility": (
                self.current_regime.avg_volatility if self.current_regime else None
            ),
        }

    def _calculate_current_regime(
        self, price_history: list[float], volume_history: list[int]
    ) -> MarketRegime:
    ...
    def _is_regime_change(self, old_regime: MarketRegime, new_regime: MarketRegime) -> bool:
        ...
        return (
            volatility_change > self.volatility_change_threshold  # Significant volatility change
            or trend_change  # Trend direction change
            or risk_change
        )  # Risk level change

    def _determine_regime_change_severity(
    ...
    def _identify_regime_change_factors(
        self, old_regime: MarketRegime, new_regime: MarketRegime
    ) -> list[str]:
        ...
        if old_regime.trend_direction != new_regime.trend_direction:
            factors.append(
                f"Trend change: {old_regime.trend_direction} -> {new_regime.trend_direction}"
            )
    ...
    def _calculate_regime_risk_score(self, regime: MarketRegime) -> float:
        ...
        risk_scores = {"low": 20, "medium": 40, "high": 70, "extreme": 95}
