...
class MarketRegime:
    ...
    def is_high_risk(self) -> bool:
        ...
        return self.regime_type in [RegimeType.BEAR_VOLATILE, RegimeType.CRISIS]
...
class CorrelationMatrix:
    ...
    baseline_correlation: np.ndarray | None = None
    correlation_change: np.ndarray | None = None
...
class AnomalyDetectionConfig:
    ...
    lookback_periods: dict[str, int] = field(
        default_factory=lambda: {"price": 100, "volume": 50, "volatility": 20, "correlation": 60}
    )
    ...
    # Severity mapping
    severity_thresholds: dict[AnomalySeverity, float] = field(
...
class AnomalyContext:
    ...
    typical_resolution_time: float | None = None  # Hours
