...
class LiveRiskMonitor:
    ...
    async def _update_positions(self):
        ...
        try:
            ...
            self._last_position_update = datetime.now(UTC)
    ...
    async def _check_position_risks(self):
        ...
        for symbol, position in self._position_cache.items():
            ...
            volatility = await self.risk_calculator.calculate_volatility(symbol, lookback_days=20)
    ...
    async def _check_portfolio_risks(self):
        ...
        if not positions_df.empty:
            ...
            await self._check_limit(
                RiskMetricType.PORTFOLIO_VAR, var_pct, metadata={"var_amount": portfolio_var}
            )

        # Check concentration
        concentration = self._calculate_concentration()
        await self._check_limit(RiskMetricType.CONCENTRATION, concentration)

        # Check leverage
        leverage = await self._calculate_leverage()
        await self._check_limit(RiskMetricType.LEVERAGE, leverage)
        ...
        if daily_pnl_pct < 0:  # Only check losses
            await self._check_limit(
                RiskMetricType.DAILY_LOSS, abs(daily_pnl_pct), metadata={"pnl_amount": daily_pnl}
            )

        # Check margin usage
        margin_usage = await self._calculate_margin_usage()
        await self._check_limit(RiskMetricType.MARGIN_USAGE, margin_usage)
    ...
    async def _create_alert(
        ...
        # Log alert
        logger.warning(
            f"Risk Alert: {alert.message} (Value: {current_value:.4f}, Threshold: {threshold:.4f})"
        )
    ...
    async def _create_risk_snapshot(self):
        ...
        snapshot = RiskSnapshot(
            ...
            total_exposure=sum(
                abs(p.quantity * p.current_price) for p in self._position_cache.values()
            ),
            portfolio_var=portfolio_var,
        ...
        self.risk_snapshots = [s for s in self.risk_snapshots if s.timestamp > cutoff_time]
    ...
    def _calculate_concentration(self) -> float:
        ...
        total_value = sum(abs(p.quantity * p.current_price) for p in self._position_cache.values())
        ...
        hhi = sum(
            (abs(p.quantity * p.current_price) / total_value) ** 2
    ...
    def get_active_alerts(
        self, level: RiskAlertLevel | None = None, metric_type: RiskMetricType | None = None
    ) -> list[RiskAlert]:
