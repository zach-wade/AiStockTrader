...
from main.risk_management.types import PortfolioRisk, RiskEvent, RiskEventType, RiskLevel
...
class TradingEngineRiskIntegration(ErrorHandlingMixin):
    ...
    def __init__(self, config: TradingEngineConfig | None = None):
        """Initialize trading engine risk integration."""
        super().__init__()
        self.config = config or TradingEngineConfig()

        # Risk components
        self.limit_checker: UnifiedLimitChecker | None = None
        self.risk_monitor: LiveRiskMonitor | None = None
        self.circuit_breakers: CircuitBreakerFacade | None = None
        self.stop_loss_manager: DynamicStopLossManager | None = None
        self.drawdown_controller: DrawdownController | None = None
        self.position_sizer: Any | None = None  # BasePositionSizer
        ...
        with self._handle_error("checking order risk"):
            ...
            if self.risk_monitor:
                ...
                if portfolio_risk.var_95 > self.config.max_portfolio_var:
                    ...
                    risk_scores.append(portfolio_risk.var_95 / self.config.max_portfolio_var * 100)
            ...
            if checks_failed:
                if any(
                    check in ["system_operational", "max_order_value"] for check in checks_failed
                ):
                    action = OrderAction.REJECT
                    ...
                    action = (
                        OrderAction.MODIFY if modified_quantity != quantity else OrderAction.HOLD
                    )
            ...
            record_metric(
                "risk.order_checks",
                1,
                tags={"action": action.value, "symbol": symbol, "risk_score": f"{risk_score:.0f}"},
            )
    ...
    async def register_trade_execution(
        self,
        trade_id: str,
        symbol: str,
        side: str,
        ...
        with self._handle_error("registering trade execution"):
            ...
            if self.config.enable_stop_loss and self.stop_loss_manager:
                await self.stop_loss_manager.register_position(
                    symbol=symbol, entry_price=price, quantity=quantity, side=side
                )
            ...
            # Record metric
            record_metric("risk.trade_registered", 1, tags={"symbol": symbol, "side": side})
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "rejection_rate": (
                self._orders_rejected / self._checks_performed if self._checks_performed > 0 else 0
            ),
