...
class LiquidityChecker:
    ...
    async def check_liquidity(
        ...
        if not liquidity_data:
            return {"passed": False, "reason": "No liquidity data available", "score": 0}
        ...
        # Get failure reasons
        failure_reasons = [
            f"{name}: {check['reason']}" for name, check in checks.items() if not check["passed"]
        ]
    ...
    async def _get_liquidity_data(self, symbol: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if volume_data and latest_quote:
                liquidity_data = {
                    ...
                    "spread": (
                        latest_quote["ask"] - latest_quote["bid"]
                        if latest_quote["ask"] and latest_quote["bid"]
                        else 0
                    ),
    ...
    def _check_market_depth(self, quantity: int, liquidity_data: dict, side: str) -> dict[str, Any]:
    ...
    def _calculate_liquidity_score(self, checks: dict, liquidity_data: dict) -> float:
    ...
    def _get_recommendations(self, quantity: int, liquidity_data: dict, checks: dict) -> list[str]:
    ...
    def estimate_execution_time(
        self, symbol: str, quantity: int, liquidity_data: dict
    ) -> dict[str, Any]:
