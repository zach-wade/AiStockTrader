...
from main.risk_management.types import RiskCheckResult, RiskMetric
...
class DrawdownChecker(LimitChecker, ErrorHandlingMixin):
    ...
    def __init__(
        self,
        checker_id: str = "drawdown_checker",
        limit_config=None,
    ...
    async def calculate_current_value(
        self, limit: LimitDefinition, context: dict[str, Any]
    ) -> float:
    ...
    async def check(
        self, symbol: str, quantity: float, price: float, side: str, context: CheckContext
    ) -> LimitCheckResult:
        ...
        with self._handle_error("checking drawdown limits"):
            ...
            daily_check = self._check_daily_drawdown(
                drawdowns["daily"], potential_loss, portfolio_value
            )
            ...
            weekly_check = self._check_weekly_drawdown(
                drawdowns["weekly"], potential_loss, portfolio_value
            )
            ...
            total_check = self._check_total_drawdown(
                drawdowns["total"], potential_loss, portfolio_value
            )
            ...
            if self.drawdown_config.require_recovery_period:
                recovery_check = self._check_recovery_requirements(
                    drawdowns, context.portfolio_state
                )
            ...
            if self.drawdown_config.reduce_position_size and drawdowns["total"] > 0.05:
                ...
                warnings.append(f"Position size reduced to {adjusted_quantity:.0f} due to drawdown")
    ...
    async def _calculate_drawdowns(self, portfolio_state: PortfolioState) -> dict[str, float]:
        ...
        if history:
            ...
            if weekly_values:
                ...
                weekly_drawdown = (
                    (weekly_peak - current_value) / weekly_peak if weekly_peak > 0 else 0
                )
        ...
        if history:
            ...
            if monthly_values:
                ...
                monthly_drawdown = (
                    (monthly_peak - current_value) / monthly_peak if monthly_peak > 0 else 0
                )
        ...
        self._drawdown_history = [(t, dd) for t, dd in self._drawdown_history if t > cutoff]
    ...
    def _check_daily_drawdown(
        self, current_dd: float, potential_loss: float, portfolio_value: float
    ) -> RiskCheckResult:
        ...
        utilization = (
            potential_dd / self.drawdown_config.max_daily_drawdown * 100
            if self.drawdown_config.max_daily_drawdown > 0
            else 0
        )
    ...
    def _check_weekly_drawdown(
        ...
        utilization = (
            potential_dd / self.drawdown_config.max_weekly_drawdown * 100
            if self.drawdown_config.max_weekly_drawdown > 0
            else 0
        )
    ...
    def _check_total_drawdown(
        ...
        utilization = (
            potential_dd / self.drawdown_config.max_total_drawdown * 100
            if self.drawdown_config.max_total_drawdown > 0
            else 0
        )
    ...
    def _check_recovery_requirements(
        self, drawdowns: dict[str, float], portfolio_state: PortfolioState
    ) -> RiskCheckResult:
        ...
        passed = (
            recovery_days >= self.drawdown_config.min_recovery_days
            or recovery_progress >= self.drawdown_config.recovery_threshold
        )

        warning = None
        if not passed:
            warning = (
                f"In drawdown recovery period: {recovery_days} days, "
        ...
        return RiskCheckResult(
            ...
            limit_value=self.drawdown_config.min_recovery_days,
            utilization=(
                recovery_days / self.drawdown_config.min_recovery_days * 100
                if self.drawdown_config.min_recovery_days > 0
                else 0
            ),
            message=f"Recovery period: {recovery_days} days",
            metadata={"warning": warning, "recovery_progress": recovery_progress},
        )
    ...
    def _calculate_recovery_progress(
        self, drawdowns: dict[str, float], portfolio_state: PortfolioState
    ) -> float:
