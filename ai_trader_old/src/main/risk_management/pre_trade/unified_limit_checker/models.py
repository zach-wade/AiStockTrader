...
class LimitDefinition:
    ...
    # Optional thresholds
    soft_threshold: float | None = None  # Warning threshold
    upper_threshold: float | None = None  # For range checks (BETWEEN operator)
        ...
        if self.soft_threshold is not None:
            if self.operator in [ComparisonOperator.LESS_THAN, ComparisonOperator.LESS_EQUAL]:
                if self.soft_threshold >= self.threshold_value:
                    raise ValueError(
                        "soft_threshold must be less than threshold_value for LESS operators"
                    )
    ...
    def get_effective_threshold(self, context: dict[str, Any] | None = None) -> float:
        """
...
class LimitViolation:
    ...
    # Metrics
    breach_magnitude: float | None = None  # How much the limit was exceeded
    duration_seconds: float | None = None  # How long the violation lasted
    ...
    def calculate_breach_magnitude(self):
        ...
        if self.operator == ComparisonOperator.LESS_EQUAL:
            self.breach_magnitude = (
                self.current_value - self.effective_threshold
            ) / self.effective_threshold
        elif self.operator == ComparisonOperator.GREATER_EQUAL:
            self.breach_magnitude = (
                self.effective_threshold - self.current_value
            ) / self.effective_threshold
...
class AggregatedCheckResult:
    ...
    def get_most_severe_violation(self) -> ViolationSeverity | None:
