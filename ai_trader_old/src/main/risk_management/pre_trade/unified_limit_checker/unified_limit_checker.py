...
from .checkers import DrawdownChecker, PositionSizeChecker
...
from .registry import CheckerRegistry, SimpleThresholdChecker, create_default_registry
...
class UnifiedLimitChecker:
    ...
    def _create_default_registry(self) -> CheckerRegistry:
        ...
        asyncio.create_task(
            registry.register_checker(DrawdownChecker(checker_id="drawdown", config=self.config))
        )

        # Note: registry.register_default_checker might not exist
        # We should just register it as a normal checker
        asyncio.create_task(registry.register_checker(SimpleThresholdChecker(self.config)))
    ...
    def check_limit(
        self, limit_id: str, current_value: float, context: dict[str, Any] | None = None
    ) -> LimitCheckResult:
    ...
    def check_all_limits(
        self, values: dict[str, float], context: dict[str, Any] | None = None
    ...
    def get_violation_history(
        self, limit_id: str | None = None, limit: int | None = None
    ) -> list[LimitViolation]:
