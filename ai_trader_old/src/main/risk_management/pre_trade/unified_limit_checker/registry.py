...
from .events import CheckEvent, EventManager
...
class LimitChecker(ABC):
    ...
    async def check_with_circuit_breaker(
        ...
        try:
            return await self.circuit_breaker.call(self.check_limit, limit, current_value, context)
            ...
            return LimitCheckResult(
                ...
                message=f"Check failed due to error: {e!s}",
                warnings=[f"Circuit breaker triggered: {e!s}"],
            )
...
class CheckerMetrics:
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "violations_by_severity": {k.value: v for k, v in self.violations_by_severity.items()},
        }
...
class CheckerRegistry:
    ...
    def __init__(self, config: LimitConfig, event_manager: EventManager | None = None):
        """
    ...
    def get_checker_for_limit(self, limit: LimitDefinition) -> LimitChecker | None:
        """
    ...
    async def check_multiple_limits(
        self, limits: list[LimitDefinition], context: dict[str, Any], parallel: bool = True
        ...
        if parallel and self.config.parallel_checking:
            ...
            tasks = [self.check_limit(limit, context) for limit in limits]
    ...
    def get_checker_metrics(self) -> dict[str, dict[str, Any]]:
        ...
        return {checker_id: metrics.get_metrics() for checker_id, metrics in self.metrics.items()}
    ...
    def get_registry_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "type_coverage": {lt.value: len(self.type_to_checkers.get(lt, [])) for lt in LimitType},
...
class SimpleThresholdChecker(LimitChecker):
    ...
    async def check_limit(
        ...
        passed = self._compare_values(
            current_value, effective_threshold, limit.operator, limit.upper_threshold
        )
        ...
        if not passed:
            ...
            soft_passed = self._compare_values(
                current_value, limit.soft_threshold, limit.operator, limit.upper_threshold
            )
            if not soft_passed:
...
def create_default_registry(
    config: LimitConfig, event_manager: EventManager | None = None
) -> CheckerRegistry:
    """
