...
class UniverseManager:
    ...
    async def get_qualified_symbols(self, layer: str = "0", limit: int | None = None) -> list[str]:
        """
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(symbols)} symbols for layer {layer_num} (limit: {limit or 'none'})"
            )
            return symbols
    ...
    async def get_universe_for_backfill(
        self, preferred_layer: str = "0", fallback_limit: int | None = None
    ) -> list[str]:
        """
        ...
        """
        logger.info(
            f"Getting universe for backfill, preferred layer: {preferred_layer}, limit: {fallback_limit}"
        )
        ...
        logger.warning(
            f"No symbols found for layer {preferred_layer}, falling back to active companies"
        )
    ...
    async def qualify_layer1(self, force_refresh: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Layer 1 qualification complete: {len(qualified_symbols)} qualified out of {len(active_symbols)}"
            )
            ...
            return {
                ...
                "qualification_rate": (
                    len(qualified_symbols) / len(active_symbols) * 100 if active_symbols else 0
                ),
    ...
    async def get_universe_stats(self) -> dict[str, Any]:
        ...
        stats["layer0_percentage"] = (
            (stats.get("layer0_count", 0) / active * 100) if active and active > 0 else 0
        )
        stats["layer1_percentage"] = (
            (stats.get("layer1_count", 0) / active * 100) if active and active > 0 else 0
        )
        stats["layer2_percentage"] = (
            (stats.get("layer2_count", 0) / active * 100) if active and active > 0 else 0
        )
        stats["layer3_percentage"] = (
            (stats.get("layer3_count", 0) / active * 100) if active and active > 0 else 0
        )
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            if health_status["healthy"]:
                logger.info(
                    f"✅ Universe health check passed. {stats['active_companies']} active companies"
                )
            else:
                logger.warning(
                    "⚠️ Universe health issues detected. Run universe population if needed."
                )
