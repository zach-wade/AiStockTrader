...
class ProcessingOrchestrator(ErrorHandlingMixin):
        ...
        with timer(
            ...
            try:
                ...
                if "clean" not in skip_steps:
                    ...
                    processed_data = await self.cleaner.clean(processed_data, data_type, layer)
    ...
    async def process_and_load(
        ...
        """
        with timer(
            "processing.process_and_load",
            tags={"symbol": symbol, "data_type": str(data_type), "layer": layer.name},
        ):
    ...
    async def process_batch(
        ...
        async def process_symbol(symbol: str):
            async with semaphore:
                return await self.process_and_load(symbol, data_type, start_date, end_date, layer)
        ...
        for symbol, result in zip(symbols, symbol_results):
            if isinstance(result, Exception):
                results[symbol] = ProcessingResult(success=False, errors=[str(result)])
            else:
