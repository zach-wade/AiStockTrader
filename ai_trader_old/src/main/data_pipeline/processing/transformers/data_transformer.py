...
from main.utils.data import DataFrameStreamer
from main.utils.data import ProcessingUtils as DataProcessingUtils
...
class DataTransformer(BaseTransformer):
    ...
    async def _standard_transform(
        ...
        if context.get("normalize", False):
            df = DataProcessingUtils.normalize_data(
                df, method="robust" if layer >= DataLayer.CATALYST else "zscore"
            )
    ...
    async def _stream_transform(
        self, df: pd.DataFrame, layer: DataLayer, context: dict[str, Any], config: StreamingConfig
    ) -> pd.DataFrame:
    ...
    def _ensure_dataframe(self, data: Any, source_format: str) -> pd.DataFrame | None:
        """Ensure data is in DataFrame format."""
        if isinstance(data, pd.DataFrame):
            return data
        elif source_format == "dict" or source_format == "list":
