...
from main.interfaces.ingestion import BulkLoadConfig, IBulkLoader
...
class LoaderCoordinator(ErrorHandlingMixin):
    ...
    def __init__(
        ...
        # Loader statistics
        self._loader_stats = {"loaders_created": 0, "loads_performed": 0, "total_records": 0}
    ...
    async def get_loader(self, data_type: DataType) -> IBulkLoader | None:
        """
    ...
    async def _create_loader(self, data_type: DataType) -> IBulkLoader | None:
        """Create a new bulk loader for the specified data type."""
        try:
            if data_type == DataType.MARKET_DATA:
                return MarketDataSplitBulkLoader(
                    db_adapter=self.db_adapter, archive=self.archive, config=self.bulk_config
                )
        ...
        with timer("loader.flush_all"):
            for data_type, loader in self._loaders.items():
                try:
                    ...
                    # Update stats
                    self._loader_stats["total_records"] += getattr(
                        flush_result, "records_loaded", 0
                    )
                    ...
                    results[str(data_type)] = {"success": False, "error": str(e)}
    ...
    async def get_loader_stats(self, data_type: DataType | None = None) -> dict[str, Any]:
        """
    ...
    async def shutdown(self) -> None:
        ...
        for data_type, result in flush_results.items():
            if result["success"]:
                self.logger.info(
                    f"Flushed {result.get('records_flushed', 0)} records from {data_type}"
                )
            else:
