...
class PolygonFundamentalsClient(BaseIngestionClient[list[dict[str, Any]]]):
    ...
    def __init__(
        ...
        metric_extractor: MetricExtractionService | None = None,
    ):
        ...
        if result.success and result.data:
            ...
            if result.data:
                ...
                if "filing_date" in latest:
                    days_old = (datetime.now(UTC) - latest["filing_date"]).days
                    gauge(
                        "polygon.fundamentals.filing_age_days",
                        days_old,
                        tags={"symbol": symbol, "timeframe": timeframe},
                    )
        else:
    ...
    async def fetch_annual(
        ...
        return await self.fetch_financials(
            symbol=symbol, timeframe="annual", start_date=start_date, end_date=end_date, limit=limit
        )
    ...
    async def fetch_multiple_symbols(
        ...
        async def fetch_symbol(symbol: str) -> FetchResult:
            return await self.fetch_financials(symbol, timeframe, start_date, end_date)

        # Track batch operation
        batch_start = datetime.now()
        gauge("polygon.fundamentals.batch_symbols", len(symbols), tags={"timeframe": timeframe})
        ...
        record_metric(
            "polygon.fundamentals.batch_duration",
            batch_duration,
            MetricType.HISTOGRAM,
            tags={"timeframe": timeframe, "symbols": len(symbols)},
        )
        record_metric(
            "polygon.fundamentals.batch_success_rate",
            successful / len(symbols) if symbols else 0,
            MetricType.GAUGE,
            tags={"timeframe": timeframe},
        )
        record_metric(
            "polygon.fundamentals.batch_total_filings",
            total_filings,
            MetricType.COUNTER,
            tags={"timeframe": timeframe},
        )
        ...
        if len(symbols) - successful > 0:
            self.logger.warning(
                f"Fundamentals batch fetch failed for {len(symbols) - successful}/{len(symbols)} symbols"
            )

        return results

    def _normalize_filing(self, record: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def _extract_metrics(self, financials: dict[str, Any]) -> dict[str, Any]:
        ...
        with timer("polygon.fundamentals.metric_extraction"):
            ...
            if total_count > 0:
                ...
                record_metric(
                    "polygon.fundamentals.metric_completeness", completeness, MetricType.GAUGE
                )
    ...
    def _extract_value(self, *fields) -> float | None:
