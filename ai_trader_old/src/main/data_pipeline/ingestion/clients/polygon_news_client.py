...
class PolygonNewsClient(BaseIngestionClient[list[dict[str, Any]]]):
    ...
    def __init__(
        ...
        text_processor: TextProcessingService | None = None,
    ):
    ...
    async def fetch_news(
        ...
        if result.success and result.data:
            ...
            if len(result.data) > 0:
                ...
                record_metric(
                    "polygon.news.dedup_rate", dedup_rate, MetricType.GAUGE, tags={"symbol": symbol}
                )
            ...
            record_metric(
                ...
                1,
                MetricType.COUNTER,
                tags={"data_type": "news", "symbol": symbol, "error": result.error or "unknown"},
            )

        return result

    async def fetch_multiple_symbols(
        self, symbols: list[str], start_date: datetime, end_date: datetime, max_concurrent: int = 5
        ...
        record_metric(
            "polygon.news.batch_duration",
            batch_duration,
            MetricType.HISTOGRAM,
            tags={"symbols": len(symbols)},
        )
        record_metric(
            "polygon.news.batch_success_rate",
        ...
        if len(symbols) - successful > 0:
            self.logger.warning(
                f"News batch fetch failed for {len(symbols) - successful}/{len(symbols)} symbols"
            )
    ...
    def _process_text_if_available(self, article: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if text.strip():
            ...
            if sentiment and "score" in sentiment:
                record_metric(
                    "polygon.news.sentiment_score", sentiment["score"], MetricType.HISTOGRAM
                )
