...
from main.utils.core import AsyncCircuitBreaker, RateLimiter, async_retry, get_logger, timer
...
class BaseIngestionClient(ABC, Generic[T]):
    ...
    def _check_cache(self, cache_key: str) -> T | None:
            ...
            async with self.circuit_breaker:
                ...
                async with session.get(url, params=params, headers=self.get_headers()) as response:
                    ...
                    if not await self.validate_response(response):
                        ...
                        self._error_count += 1
                        return FetchResult(
                            success=False, error=error_msg, metadata={"status": response.status}
                        )
                    ...
                    return FetchResult(
                        success=True,
                        data=data,
                        metadata={"status": response.status, "cached": False},
                    )
    ...
    async def batch_fetch(
        self, requests: list[tuple[str, dict[str, Any]]], max_concurrent: int = 5
    ) -> list[FetchResult[T]]:
        """
        ...
        tasks = [fetch_with_semaphore(endpoint, params) for endpoint, params in requests]
