...
class FundamentalsBulkLoader(BaseBulkLoader[dict[str, Any]]):
    ...
    def __init__(
        ...
        format_factory: FundamentalsFormatFactory | None = None,
        ...
        """
        super().__init__(
            db_adapter=db_adapter, archive=archive, config=config, data_type="fundamentals"
        )
        ...
        logger.info("FundamentalsBulkLoader initialized with format factory")

    async def load(
        self, data: list[dict[str, Any]], symbols: list[str], source: str = "yahoo", **kwargs
    ) -> BulkLoadResult:
        """
        ...
        async with self.db_adapter.acquire() as conn:
            try:
                ...
                await conn.copy_records_to_table(
                    "temp_financials", records=copy_records, columns=columns
                )
    ...
    def _format_date_for_copy(self, date_value: Any) -> date | None:
        ...
        for period_key, group_records in period_groups.items():
            try:
                ...
                metadata = {
                    ...
                    "metrics": {
                        "has_revenue": sum(
                            1 for r in group_records if r.get("revenue") is not None
                        ),
                        "has_eps": sum(1 for r in group_records if r.get("eps_basic") is not None),
                        "has_net_income": sum(
                            1 for r in group_records if r.get("net_income") is not None
                        ),
                    },
                }
