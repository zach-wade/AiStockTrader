...
class MarketDataSplitBulkLoader(BaseBulkLoader[dict[str, Any]]):
    ...
    def __init__(
        ...
        """
        super().__init__(
            db_adapter=db_adapter, archive=archive, config=config, data_type="market_data_split"
        )
    ...
    def _prepare_records(
        self, data: pd.DataFrame, symbol: str, interval: str, source: str, table_name: str
    ) -> list[dict[str, Any]]:
        """
        ...
        with timer("market_data_split_flush") as t:
            ...
            for table_name, records in records_by_table.items():
                try:
                    ...
                    result.errors.append(f"{table_name}: {e!s}")
    ...
    async def _load_table_records(self, table_name: str, records: list[dict[str, Any]]) -> int:
        """
        ...
        async with self.db_adapter.acquire() as conn:
            ...
            await conn.execute(f"CREATE TEMP TABLE {temp_table} (LIKE {table_name} INCLUDING ALL)")

            try:
                # Use COPY to load data to temp table
                await conn.copy_records_to_table(temp_table, records=copy_records, columns=columns)
