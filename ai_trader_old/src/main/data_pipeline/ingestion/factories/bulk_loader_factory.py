...
class BulkLoaderFactory:
    ...
    def _parse_layer_configs(self) -> dict[int, LayerConfig]:
        ...
        for layer_name, layer_data in layer_config_data.items():
            ...
            try:
                ...
                self.logger.debug(
                    f"Loaded config for layer {layer_num}: {layer_configs[layer_num]}"
                )
            except ValueError:
    ...
    def create_market_data_loader(
        ...
        routing_service: TableRoutingService | None = None,
        ...
        **kwargs,
        ...
        self.logger.debug(
            f"Created MarketDataSplitBulkLoader for layer {layer} with buffer {config.buffer_size}"
        )
        return loader
    ...
    def create_news_loader(
        ...
        **kwargs,
        ...
        loader = NewsBulkLoader(db_adapter=db_adapter, archive=archive, config=config)

        self.logger.debug(
            f"Created NewsBulkLoader for layer {layer} with buffer {config.buffer_size}"
        )
        return loader
    ...
    def create_fundamentals_loader(
        ...
        **kwargs,
        ...
        loader = FundamentalsBulkLoader(db_adapter=db_adapter, archive=archive, config=config)

        self.logger.debug(
            f"Created FundamentalsBulkLoader for layer {layer} with buffer {config.buffer_size}"
        )
        return loader
    ...
    def create_corporate_actions_loader(
        ...
        **kwargs,
        ...
        loader = CorporateActionsBulkLoader(db_adapter=db_adapter, archive=archive, config=config)

        self.logger.debug(
            f"Created CorporateActionsBulkLoader for layer {layer} with buffer {config.buffer_size}"
        )
        return loader
...
# Singleton instance
_factory_instance: BulkLoaderFactory | None = None


def get_bulk_loader_factory(config: dict[str, Any] | None = None) -> BulkLoaderFactory:
    """
