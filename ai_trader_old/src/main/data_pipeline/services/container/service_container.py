...
class SimpleServiceContainer:
    ...
    async def setup_data_pipeline_services(self, db_adapter: IAsyncDatabase):
        ...
        self.register(QualificationService, QualificationService(db_adapter=db_adapter))

        # Deduplication service
        self.register(DeduplicationService, DeduplicationService(db_adapter=db_adapter))

        # Table routing service
        self.register(TableRoutingService, TableRoutingService())

        # Partition manager
        self.register(PartitionManager, PartitionManager(db_adapter=db_adapter))

        # Metric extraction service
        self.register(MetricExtractionService, MetricExtractionService())

        # Text processing service
        self.register(TextProcessingService, TextProcessingService())

        # Corporate actions service
        self.register(CorporateActionsService, CorporateActionsService())
    ...
    def setup_ingestion_clients(self, api_key: str, layer: DataLayer = DataLayer.BASIC):
        ...
        self.register(PolygonMarketClient, PolygonMarketClient(api_key, layer))

        self.register(
            PolygonNewsClient, PolygonNewsClient(api_key, layer, text_processor=text_processor)
        )
        ...
        self.register(PolygonCorporateActionsClient, PolygonCorporateActionsClient(api_key, layer))
    ...
    def setup_coordinators(self):
        ...
        self.register(
            LayerCoordinator,
            LayerCoordinator(db_adapter=db_adapter, qualification_service=qualification_service),
        )
    ...
    def get_all_services(self) -> dict[str, Any]:
        ...
        """
        return {
            service_type.__name__: instance for service_type, instance in self._services.items()
        }
