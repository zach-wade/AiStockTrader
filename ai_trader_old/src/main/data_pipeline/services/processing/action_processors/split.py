...
class SplitProcessor(BaseActionProcessor):
    ...
    def can_process(self, action: dict[str, Any]) -> bool:
        ...
        split_fields = ["split_from", "split_to", "execution_date", "split_ratio"]

        return any(field in action for field in split_fields)

    def process(self, action: dict[str, Any], source: str) -> dict[str, Any] | None:
        """
        ...
        if not self._validate_base_fields(action):
            if not self.config.skip_invalid:
                logger.warning("Invalid split action: missing required fields")
            return None
    ...
    def _process_raw_split(self, action: dict[str, Any], source: str) -> dict[str, Any]:
        """
    ...
    def _process_transformed_split(self, action: dict[str, Any], source: str) -> dict[str, Any]:
        """
        ...
        mapped = {
            ...
            "execution_date": action.get(
                "execution_date", action.get("ex_date", action.get("timestamp"))
            ),
