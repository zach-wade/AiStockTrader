...
class DividendProcessor(BaseActionProcessor):
    ...
    def process(self, action: dict[str, Any], source: str) -> dict[str, Any] | None:
        ...
        if not self._validate_base_fields(action):
            if not self.config.skip_invalid:
                logger.warning("Invalid dividend action: missing required fields")
            return None
    ...
    def _process_raw_dividend(self, action: dict[str, Any], source: str) -> dict[str, Any]:
        """
    ...
    def _process_transformed_dividend(self, action: dict[str, Any], source: str) -> dict[str, Any]:
        """
        ...
        mapped = {
            ...
            "ex_dividend_date": action.get(
                "ex_date", action.get("ex_dividend_date", action.get("timestamp"))
            ),
