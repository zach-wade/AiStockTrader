...
from main.data_pipeline.ingestion.clients.base_client import ClientConfig, FetchResult
from main.utils.core import ensure_utc, get_logger
from main.utils.layer_metrics import LayerMetricsCollector
from main.utils.layer_utils import get_api_rate_limit, get_layer_cache_ttl
...
class PolygonApiHandler:
    ...
    async def paginate(
        self, client: Any, endpoint: str, params: dict[str, Any], max_pages: int = 10
    ...
    async def batch_fetch(
        self, fetch_fn: Callable, items: list[Any], batch_size: int = 50, max_concurrent: int = 5
        ...
        for i in range(0, len(items), batch_size):
            ...
            for item in batch:
                task = self._fetch_with_semaphore(semaphore, fetch_fn, item)
                tasks.append(task)
            ...
            for item, result in zip(batch, batch_results):
                if isinstance(result, Exception):
                    results[item] = FetchResult(success=False, error=str(result))
                else:
                    results[item] = result
    ...
    async def _fetch_with_semaphore(
        self, semaphore: asyncio.Semaphore, fetch_fn: Callable, item: Any
    ) -> FetchResult:
    ...
    async def parse_polygon_response(
    ...
    def build_date_params(
        self, start_date: datetime, end_date: datetime, date_field: str = "timestamp"
    ) -> dict[str, str]:
        """
    ...
    async def fetch_with_pagination(
        ...
        try:
            ...
            return FetchResult(
                success=True,
                data=all_results,
                metadata={"pages_fetched": pages_fetched, "total_records": len(all_results)},
            )

        except Exception as e:
            self.logger.error(f"Error fetching with pagination: {e}")
            return FetchResult(
                success=False, error=str(e), data=all_results if all_results else None
            )
