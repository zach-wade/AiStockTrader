...
from main.utils.data import DataFrameStreamer, ProcessingUtils, ValidationUtils
...
class BaseProcessor(ABC):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._processing_stats["start_time"] = ensure_utc(datetime.now(UTC))
    ...
    async def process(
        ...
        context = context or {}
        processing_id = context.get(
            "processing_id", f"{self.processor_name}_{ensure_utc(datetime.now(UTC)).isoformat()}"
        )
        ...
        try:
            ...
            if self.metrics_collector:
                self.metrics_collector.record_processing_success(
                    processor=self.processor_name, priority=priority.name, context=context
                )
            ...
            error = ProcessingError(
                ...
                original_error=e,
                context={"processing_id": processing_id, "priority": priority.name, **context},
            )
    ...
    async def _process_data(
        self, data: Any, priority: ProcessingPriority, context: dict[str, Any]
    ) -> Any:
    ...
    def _update_stats(self, success: bool) -> None:
        ...
        self._processing_stats["last_processed"] = ensure_utc(datetime.now(UTC))
    ...
    def _should_use_streaming(self, data: Any) -> bool:
        ...
        if isinstance(data, pd.DataFrame):
            ...
            return len(data) > self.streaming_chunk_size or memory_mb > self.streaming_max_memory_mb

        return False

    async def _process_with_streaming(
        self, data: pd.DataFrame, priority: ProcessingPriority, context: dict[str, Any]
    ) -> pd.DataFrame:
        """Process large DataFrame using streaming."""
        self.logger.info(f"Processing large dataset ({len(data)} rows) using streaming")
        ...
        config = StreamingConfig(
            chunk_size=self.streaming_chunk_size, max_memory_mb=self.streaming_max_memory_mb
        )
        ...
        result = await streamer.process_stream(
            data_source=data, processor=process_chunk, output_path=None  # Collect results in memory
        )
