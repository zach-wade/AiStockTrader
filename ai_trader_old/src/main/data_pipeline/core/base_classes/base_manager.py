...
class BaseManager(ABC):
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self._operations_stats["start_time"] = ensure_utc(datetime.now(UTC))
            self._health_status = "healthy"
    ...
    async def _wait_for_operations(self, timeout_seconds: int = 60) -> None:
        ...
        while (
            ...
            and (ensure_utc(datetime.now(UTC)) - start_time).seconds < timeout_seconds
        ):
    ...
    async def execute_operation(
        self, operation_id: str, operation_func, *args, layer: DataLayer | None = None, **kwargs
    ) -> Any:
        """
        ...
        try:
            ...
            error = convert_exception(
                e, f"Operation failed in manager {self.manager_name}", component="manager"
            )
    ...
    async def _validate_operation(self, operation_id: str, layer: DataLayer | None) -> None:
    ...
    def _update_operation_stats(self, success: bool) -> None:
        ...
        self._operations_stats["last_operation"] = ensure_utc(datetime.now(UTC))
    ...
    async def execute_batch_operations(
        ...
        results = await process_in_batches(operations, execute_one, max_concurrent=max_concurrent)

        return results

    async def execute_operations_with_exceptions(
        self, operations: list[dict[str, Any]], layer: DataLayer | None = None
    ) -> dict[str, Any]:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                errors.append({"operation_id": operations[i]["id"], "error": str(result)})
            else:
                successful_results.append({"operation_id": operations[i]["id"], "result": result})
