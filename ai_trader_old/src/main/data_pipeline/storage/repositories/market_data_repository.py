...
class MarketDataRepository(BaseRepository, IMarketDataRepository):
    ...
    def __init__(self, db_adapter: IAsyncDatabase, config: RepositoryConfig | None = None):
        ...
        super().__init__(
            db_adapter, type("MarketData", (), {"__tablename__": "market_data_1h"}), config
        )
        ...
        self.metrics = RepositoryMetricsCollector(
            "MarketDataRepository", enable_metrics=config.enable_metrics if config else True
        )
    ...
    async def get_ohlcv(
        ...
        try:
            ...
            await self.metrics.record_operation(
                "get_ohlcv", duration, success=True, records=len(df)
            )
    ...
    async def get_price_range(
        ...
        try:
            ...
            result = await self.db_adapter.fetch_one(
                query, self._normalize_symbol(symbol), ensure_utc(start_date), ensure_utc(end_date)
            )
    ...
    async def get_market_hours_data(
        ...
        try:
            ...
            return await self.get_ohlcv(symbol, market_open, market_close, interval="5min")
