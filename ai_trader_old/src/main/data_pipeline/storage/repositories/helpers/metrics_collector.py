...
# Local imports
from main.utils.core import get_logger, secure_uniform
...
class RepositoryMetricsCollector:
    ...
    def __init__(self, repository_name: str, enable_metrics: bool = True, sample_rate: float = 1.0):
        """
        ...
        if secure_uniform(0, 1) <= self.sample_rate:
            try:
                ...
                if records > 0:
                    await record_metric(
                        MetricType.COUNTER,
                        "repository_records_processed_total",
                        records,
                        labels={"repository": self.repository_name, "operation": operation},
                    )
    ...
    async def record_cache_access(self, hit: bool) -> None:
        ...
        try:
            ...
            await record_metric(
                MetricType.COUNTER, metric_name, 1, labels={"repository": self.repository_name}
            )
        except Exception as e:
            logger.debug(f"Failed to record cache metric: {e}")

    async def record_error(self, operation: str, error_type: str, error_message: str) -> None:
        """
