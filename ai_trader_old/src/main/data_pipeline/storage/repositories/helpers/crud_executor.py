...
from main.interfaces.repositories.base import OperationResult, TransactionStrategy
...
class CrudExecutor:
    ...
    async def execute_insert(self, query: str, params: list[Any]) -> OperationResult:
        """
        ...
        try:
            result = await _retry_operation(
                _insert, max_retries=self.max_retries, initial_delay=self.retry_delay
            )
    ...
    async def execute_bulk_insert(self, records: list[dict[str, Any]]) -> OperationResult:
        """
        ...
        if self.transaction_strategy == TransactionStrategy.BATCH:
            ...
            try:
                ...
                return OperationResult(
                    success=False,
                    error=f"Transaction failed: {transaction_error!s}",
                    duration_seconds=time.time() - start_time,
                )
    ...
    async def execute_update(self, query: str, params: list[Any]) -> OperationResult:
        """
        ...
        try:
            affected = await _retry_operation(
                _update, max_retries=self.max_retries, initial_delay=self.retry_delay
            )
    ...
    async def execute_delete(self, query: str, params: list[Any]) -> OperationResult:
        """
        ...
        try:
            affected = await _retry_operation(
                _delete, max_retries=self.max_retries, initial_delay=self.retry_delay
            )
    ...
    async def execute_upsert(self, query: str, params: list[Any]) -> OperationResult:
        """
