...
from .helpers import BatchProcessor, CrudExecutor, QueryBuilder, RepositoryMetricsCollector
...
class FeatureRepository(BaseRepository, IFeatureRepository):
    ...
    def __init__(self, db_adapter: IAsyncDatabase, config: RepositoryConfig | None = None):
        ...
        self.metrics = RepositoryMetricsCollector(
            "FeatureRepository", enable_metrics=config.enable_metrics if config else True
        )
    ...
    async def get_features(
        ...
        try:
            ...
            params = [self._normalize_symbol(symbol), ensure_utc(start_date), ensure_utc(end_date)]
            ...
            duration = time.time() - start_time
            await self.metrics.record_operation(
                "get_features", duration, success=True, records=len(df)
            )
    ...
    async def get_latest_features(
        self, symbol: str, feature_names: list[str] | None = None
    ) -> dict[str, Any] | None:
            ...
            async def store_batch(batch: list[dict[str, Any]]) -> Any:
                ...
                for record in batch:
                    await self.store_features(
                        record["symbol"], record["timestamp"], json.loads(record["features"])
                    )
                return len(batch)
    ...
    async def get_feature_statistics(
        self, symbol: str, feature_names: list[str], start_date: datetime, end_date: datetime
    ...
    async def get_feature_correlation(
        self, symbol: str, features: list[str], start_date: datetime, end_date: datetime
    ) -> pd.DataFrame:
    ...
    async def cleanup_old_features(
        self, days_to_keep: int, feature_names: list[str] | None = None
    ) -> OperationResult:
    ...
    async def get_feature_availability(
        self, symbols: list[str], feature_names: list[str], date: datetime
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if df.empty:
                    ...
                    availability[symbol] = {}
                    for feature in feature_names:
                        availability[symbol][feature] = (
                            feature in df.columns and df[feature].notna().any()
                        )
    ...
    async def get_missing_features(
        self, symbol: str, expected_features: list[str], start_date: datetime, end_date: datetime
    ) -> list[dict[str, Any]]:
