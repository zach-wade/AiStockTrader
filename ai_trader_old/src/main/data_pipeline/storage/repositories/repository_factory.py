...
class RepositoryRegistry:
    ...
    def register(self, name: str, repository_type: type, config: RepositoryConfig | None = None):
    ...
    def get_repository_type(self, name: str) -> type | None:
        """Get repository type by name."""
        return self._repository_types.get(name)

    def get_repository_config(self, name: str) -> RepositoryConfig | None:
    ...
    def get_instance(self, name: str) -> RepositoryT | None:
...
class RepositoryFactory:
    ...
    def create_repository(
        ...
        if not repo_type:
            raise ValueError(
                f"Repository type '{name}' not registered. Available types: {list(self.registry.list_repositories().keys())}"
            )
    ...
    def create_market_data_repository(
        self, db_adapter: IAsyncDatabase, config: RepositoryConfig | None = None
    ...
    def create_social_sentiment_repository(
        ...
        return cast(
            SocialSentimentRepository,
            self.create_repository("social_sentiment", db_adapter, config),
        )
    ...
    def register_custom_repository(
        self, name: str, repository_type: type, config: RepositoryConfig | None = None
    ):
    ...
    def _create_instance(
        self, repo_type: type[RepositoryT], db_adapter: IAsyncDatabase, config: RepositoryConfig
    ) -> RepositoryT:
...
def create_repository(
    name: str, db_adapter: IAsyncDatabase, config: RepositoryConfig | None = None
) -> RepositoryT:
