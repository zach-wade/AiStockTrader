...
class NewsRepository(BaseRepository):
    ...
    def __init__(self, db_adapter: IAsyncDatabase, config: RepositoryConfig | None = None):
        ...
        super().__init__(
            db_adapter, type("NewsArticle", (), {"__tablename__": "news_data"}), config
        )
    ...
    async def get_news(
        ...
        try:
            ...
            await self.metrics.record_operation("get_news", duration, True, len(df))

            return df

        except Exception as e:
            logger.error(f"Error getting news: {e}")
            await self.metrics.record_operation("get_news", time.time() - start_time, False)
            return pd.DataFrame()

    async def store_news(self, articles: list[dict[str, Any]]) -> OperationResult:
        """Store news articles."""
        start_time = time.time()
    ...
    async def get_sentiment_summary(self, symbol: str, days: int = 7) -> dict[str, Any]:
        ...
        try:
            ...
            result = await self.db_adapter.fetch_one(
                query, self._normalize_symbol(symbol), start_date, end_date
            )
            ...
            if result:
                return {
                    ...
                    "min_sentiment": (
                        float(result["min_sentiment"]) if result["min_sentiment"] else 0
                    ),
                    "max_sentiment": (
                        float(result["max_sentiment"]) if result["max_sentiment"] else 0
                    ),
                    "sentiment_stddev": (
                        float(result["sentiment_stddev"]) if result["sentiment_stddev"] else 0
                    ),
    ...
    async def get_trending_topics(
        self, symbols: list[str] | None = None, days: int = 1
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            conditions = ["published_at >= $1", "published_at <= $2"]
    ...
    async def cleanup_old_news(self, days_to_keep: int = 365) -> OperationResult:
