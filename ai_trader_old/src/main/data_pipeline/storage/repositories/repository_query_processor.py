...
from main.interfaces.repositories.base import OperationResult, QueryFilter
...
class RepositoryQueryProcessor:
    ...
    async def get_by_filter(self, query_filter: QueryFilter) -> pd.DataFrame:
        ...
        try:
            ...
            duration = time.time() - start_time
            await self._record_operation_metric_if_enabled("get_by_filter", duration, success=False)
            raise
    ...
    async def get_aggregated_data(
        self, query_filter: QueryFilter, aggregation: str, group_by: list[str] | None = None
    ) -> pd.DataFrame:
        """
    ...
    async def bulk_upsert(
        ...
        try:
            if not records:
                return OperationResult(success=True, records_affected=0, duration_seconds=0)
            ...
            duration = time.time() - start_time
            await self._record_operation_metric_if_enabled("bulk_upsert", duration, success=False)
            return OperationResult(success=False, error=str(e), duration_seconds=duration)

    async def get_distinct_values(
        self, column: str, query_filter: QueryFilter | None = None
    ) -> list[Any]:
        """
        ...
        try:
            ...
            query = (
                f"SELECT DISTINCT {column} FROM {self.table_name}{where_clause} ORDER BY {column}"
            )
