...
# Local imports
from main.utils.cache import CacheTier, get_global_cache
...
class DualStorageMixin:
    ...
    def _get_hot_storage_cutoff(self) -> datetime:
        ...
        return datetime.now(UTC) - timedelta(days=self.hot_storage_days)
    ...
    async def _route_to_storage(self, query_filter: "QueryFilter", operation: str) -> str:
        """
...
class CacheMixin:
    ...
    async def _set_in_cache(self, key: str, value: Any) -> None:
        ...
        try:
            await self._cache.set(key, value, ttl=self.cache_ttl, tier=CacheTier.MEMORY)
        except Exception as e:
            logger.debug(f"Cache set error for {key}: {e}")

    async def _invalidate_cache(self, pattern: str | None = None) -> None:
    ...
    async def _record_operation_metric(
        ...
        try:
            ...
            if records > 0:
                await record_metric(
                    MetricType.COUNTER,
                    "repository_records_processed",
                    records,
                    labels={"repository": self._repo_name, "operation": operation},
                )
        except Exception as e:
