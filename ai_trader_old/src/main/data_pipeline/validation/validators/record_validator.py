...
class RecordValidator:
    ...
    def __init__(self, config: dict[str, Any] | DataPipelineConfig.ValidationConfig | None = None):
        ...
        if isinstance(config, DataPipelineConfig.ValidationConfig):
            ...
            self.validation_rules = {"field_ranges": {}, "field_types": {}}
    ...
    async def validate(self, data: Any, context: IValidationContext) -> IValidationResult:
        ...
        try:
            ...
            all_errors = [f"Validation error: {e!s}"]
            all_warnings = []
            metrics = {}
            passed = False
    ...
    async def get_validation_rules(self, context: IValidationContext) -> list[str]:
        ...
        if context.data_type == DataType.MARKET_DATA:
            ...
            rules.extend(["required_fields_check", "content_validation", "sentiment_range_check"])
        elif context.data_type == DataType.FINANCIALS:
            rules.extend(["required_fields_check", "financial_ratios_check", "earnings_validation"])
    ...
    async def _validate_ohlc_relationships(self, record: dict[str, Any]) -> list[str]:
    ...
    def _check_field_exists(self, record: dict[str, Any], field: str) -> bool:
        ...
        if field in variations:
            return any(var in record and record[var] is not None for var in variations[field])
