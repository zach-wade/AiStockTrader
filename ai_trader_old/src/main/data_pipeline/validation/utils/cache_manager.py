...
class ValidationCacheManager:
    ...
    def __init__(
        ...
        self.cache = SimpleCache(backend=MemoryBackend(max_size=max_size))
        ...
        logger.info(
            f"ValidationCacheManager initialized with {cache_tier} backend, TTL={ttl_seconds}s"
        )

    # Validation Results Caching
    def cache_validation_result(
        self, key: str, result: Any, stage: str, ttl: int | None = None
    ) -> None:
        """
        ...
        cache_key = self._generate_cache_key(
            namespace=self.namespaces["validation_results"], stage=stage, key=key
        )

        # SimpleCache doesn't have ttl parameter, so we'll store the result directly
        self.cache.backend.set(key=cache_key, value=result, ttl=ttl or self.ttl_seconds)

        logger.debug(f"Cached validation result for {stage}:{key}")

    def get_validation_result(self, key: str, stage: str) -> Any | None:
        """
    ...
    def cache_quality_score(
        ...
        """
        cache_key = self._generate_cache_key(
            namespace=self.namespaces["quality_scores"], symbol=symbol, source=source
        )
        ...
        self.cache.backend.set(key=cache_key, value=cache_value, ttl=ttl or self.ttl_seconds)

        logger.debug(f"Cached quality score for {symbol}:{source} = {score:.2f}")

    def get_quality_score(self, symbol: str, source: str) -> dict[str, Any] | None:
        """
    ...
    def cache_coverage_analysis(
        ...
        key_data = {"symbols": sorted(symbols), "intervals": sorted(intervals)}
        key_hash = self._hash_dict(key_data)

        cache_key = self._generate_cache_key(
            namespace=self.namespaces["coverage_data"], key=key_hash
        )
        ...
        logger.debug(
            f"Cached coverage analysis for {len(symbols)} symbols, {len(intervals)} intervals"
        )
    ...
    def cache_rule_evaluation(
        self, rule_name: str, data_hash: str, result: bool, message: str, ttl: int | None = None
    ) -> None:
        """
        ...
        """
        cache_key = self._generate_cache_key(
            namespace=self.namespaces["rule_evaluations"], rule=rule_name, data=data_hash
        )
    ...
    def get_rule_evaluation(self, rule_name: str, data_hash: str) -> dict[str, Any] | None:
        """
    ...
    def cache_profile_config(
        self, profile_name: str, config: dict[str, Any], ttl: int | None = None
    ) -> None:
        """
        ...
        """
        cache_key = self._generate_cache_key(
            namespace=self.namespaces["profile_configs"], profile=profile_name
        )
    ...
    def get_profile_config(self, profile_name: str) -> dict[str, Any] | None:
        """
...
_global_cache_manager: ValidationCacheManager | None = None
...
def get_validation_cache_manager(
    ...
    if _global_cache_manager is None:
        _global_cache_manager = ValidationCacheManager(
            cache_tier=cache_tier, ttl_seconds=ttl_seconds, max_size=max_size, config=config
        )
