...
# Interface imports
from main.interfaces.validation.config import IFieldMappingConfig
...
class ValidationProfileManager:
    ...
    def __init__(
        self,
        field_mapping_config: IFieldMappingConfig | None = None,
    ...
    async def create_profile(self, profile_name: str, profile_config: dict[str, Any]) -> None:
        """Create or update a validation profile."""
        # Validate profile configuration
        required_fields = ["quality_threshold", "max_null_percentage", "severity_threshold"]
        ...
        if isinstance(profile_config["severity_threshold"], str):
            try:
                ...
                logger.warning("Invalid severity threshold, using WARNING")
    ...
    async def get_field_mapping(
        self, source: str, target_schema: str = "standard"
    ) -> dict[str, str]:
    ...
    async def create_field_mapping(
        self, source: str, mapping: dict[str, str], target_schema: str = "standard"
    ) -> None:
    ...
    def _load_field_mappings_from_file(
            ...
            for mapping_name, mapping in mappings.items():
                ...
                for source_field, target_field in mapping.items():
                    if not isinstance(source_field, str) or not isinstance(target_field, str):
                        logger.error(
                            f"Invalid field mapping in {mapping_name}: {source_field} -> {target_field}"
                        )
                        return False
...
def create_validation_profile_manager(
    ...
    return ValidationProfileManager(
        field_mapping_config=field_mapping_config, custom_profiles=custom_profiles
    )
