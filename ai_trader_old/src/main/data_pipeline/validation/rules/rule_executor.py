...
from .rule_definitions import FailureAction, RuleExecutionResult, ValidationRule
...
class RuleExecutor:
    ...
    def __init__(self):
        ...
        self.execution_stats = {"total_executed": 0, "passed": 0, "failed": 0, "errors": 0}
    ...
    def execute_rule(
        ...
        try:
            ...
            return RuleExecutionResult(
                rule_name=rule.name,
                passed=False,
                error_message=f"Rule execution error: {e!s}",
    ...
    def _evaluate_dataframe_rule(
        self, expression: str, df: pd.DataFrame, context: dict[str, Any]
    ) -> bool:
    ...
    def _evaluate_dict_rule(self, expression: str, data: dict, context: dict[str, Any]) -> bool:
    ...
    def _evaluate_list_rule(self, expression: str, data: list, context: dict[str, Any]) -> bool:
    ...
    def _get_dataframe_helpers(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        return {
            ...
            "column_unique": lambda col: (
                df[col].nunique() == len(df) if col in df.columns else False
            ),
        }
