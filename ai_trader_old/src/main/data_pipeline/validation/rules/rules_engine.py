...
from .rule_definitions import RuleExecutionResult, RuleProfile, ValidationRule
...
class ValidationRulesEngine:
    ...
    def __init__(
        self, config_path: str | None = None, custom_rules: list[dict[str, Any]] | None = None
    ):
        """
        ...
        for rule_config in self.custom_rules:
            try:
                rule = self.parser._create_rule_from_config(
                    rule_config.get("name", "custom_rule"), rule_config
                )
    ...
    async def evaluate_rule(
        self, rule: ValidationRule, data: Any, context: dict[str, Any] | None = None
    ) -> bool:
    ...
    async def evaluate_rules(
        self, rules: list[ValidationRule], data: Any, context: dict[str, Any] | None = None
    ) -> tuple[bool, list[str]]:
    ...
    async def get_rules_for_stage(
        self, stage: ValidationStage, data_type: str | None = None
    ) -> list[ValidationRule]:
    ...
    async def get_rule_by_name(self, name: str) -> ValidationRule | None:
