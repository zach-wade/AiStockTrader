...
class UnifiedPipeline:
    ...
    def __init__(self, container: SimpleServiceContainer, event_bus: IEventBus | None = None):
        """
    ...
    async def execute(self, request: PipelineRequest) -> PipelineResult:
        ...
        try:
            ...
            for layer in request.layers:
                ...
                if request.dry_run:
                    self.logger.info(
                        f"[DRY RUN] Would process {len(symbols)} symbols in {layer.value}"
                    )
                    continue
                ...
                await self._emit_event(
                    "layer_completed",
                    {"layer": layer.value, "symbols": len(symbols), "records": layer_records},
                )
            ...
            await self._emit_event(
                "pipeline_completed", {"duration": duration, "records": total_records}
            )
    ...
    async def _process_layer(
        ...
        fetch_request = FetchRequest(
            symbols=symbols, stages=stages, start_date=start_date, end_date=end_date
        )
    ...
    async def _emit_event(self, event_type: str, data: Any):
        ...
        if self.event_bus:
            event = Event(
                ...
                data={"event": event_type, **(data if isinstance(data, dict) else {"data": data})},
            )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            "storage": {},
            "coordinators": {"layer": "active", "fetch": "active", "storage": "active"},
        }
