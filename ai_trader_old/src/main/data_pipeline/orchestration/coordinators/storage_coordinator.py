...
class StorageCoordinator:
    ...
    def __init__(
        ...
        self.routing_service = routing_service or TableRoutingService(TableRoutingConfig())

        self.partition_manager = partition_manager or PartitionManager(
            db_adapter=db_adapter, config=PartitionConfig()
        )

        self.bulk_loader_factory = bulk_loader_factory or BulkLoaderFactory(db_adapter=db_adapter)
    ...
    async def store_data(self, request: StorageRequest) -> StorageResult:
        ...
        try:
            ...
            if "market_data" in table_name and request.data:
                ...
                if timestamps:
                    ...
                    partitions_created = await self.partition_manager.ensure_partitions_exist(
                        table_name=table_name, start_date=min_date, end_date=max_date
                    )
            ...
            return StorageResult(
                records_stored=0, table_name="", partitions_created=0, success=False, error=str(e)
            )
    ...
    async def get_statistics(self) -> dict[str, Any]:
        ...
        """
        stats = {"active_loaders": len(self.bulk_loaders), "loader_stats": {}}
