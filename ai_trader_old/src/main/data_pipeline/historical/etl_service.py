...
from main.interfaces.ingestion import BulkLoadResult, IBulkLoader
...
class ETLConfig:
    ...
    layer_configs: dict[int, dict[str, Any]] = field(
...
class ETLService:
    ...
    def __init__(
        ...
        data_fetch_service: DataFetchService | None = None,
        transformer: Any | None = None,  # DataTransformer to be implemented
        bulk_loader_factory: BulkLoaderFactory | None = None,
    ):
        """
        ...
        # Initialize services
        self.data_fetch_service = data_fetch_service or DataFetchService(db_adapter=db_adapter)
    ...
    async def process_etl_request(self, request: ETLRequest) -> ETLResult:
        ...
        result = ETLResult(success=False, symbol=request.symbol, data_type=request.data_type)
        ...
        try:
            ...
            layer_config = self.config.layer_configs.get(
                request.layer, self.config.layer_configs.get(3)  # Default to layer 3 config
            )
            ...
            if self.config.enable_transformation and self.transformer:
                ...
                transformed_data = await self._transform_data(extracted_data, request)
            ...
            if request.use_bulk_loader:
                ...
                load_result = await self._load_data(transformed_data, request, layer_config)
            ...
            if request.archive_enabled and self.archive:
                ...
                archive_count = await self._archive_data(
                    extracted_data if self.config.archive_raw_data else transformed_data, request
                )
            ...
            result.success = (
                ...
                or result.records_archived > 0
                or result.records_extracted == 0  # No data is OK
            )
    ...
    async def process_batch_etl(
        self, requests: list[ETLRequest], max_parallel: int | None = None
    ) -> list[ETLResult]:
        """
    ...
    async def _load_data(
        self, data: Any, request: ETLRequest, layer_config: dict[str, Any]
    ) -> BulkLoadResult:
        ...
        # Load data
        return await bulk_loader.load(data=data, symbols=[request.symbol], source=request.source)
    ...
    def _create_bulk_loader(self, data_type: DataType, layer: int) -> IBulkLoader | None:
        ...
        if data_type == DataType.MARKET_DATA:
            return self.bulk_loader_factory.create_market_data_loader(
                db_adapter=self.db_adapter, layer=layer, archive=self.archive
            )
        elif data_type == DataType.NEWS:
            return self.bulk_loader_factory.create_news_loader(
                db_adapter=self.db_adapter, archive=self.archive
            )
    ...
    def _count_records(self, data: Any) -> int:
        ...
        if data is None:
            return 0
        elif isinstance(data, list) or hasattr(data, "__len__"):
            return len(data)
    ...
    async def _emit_etl_event(self, event_type: str, request: ETLRequest, result: ETLResult):
