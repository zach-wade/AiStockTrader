...
class DataExistenceChecker:
    ...
    async def get_actual_data_points(
        ...
        self.logger.debug(f"Checking existence for {symbol} {data_type.value} {interval.value}")

        # Check hot storage first
        hot_data = await self._query_hot_storage(symbol, data_type, interval, start_date, end_date)
    ...
    async def _query_hot_storage(
        ...
        try:
            if data_type == DataType.MARKET_DATA:
                ...
                existence_data = await self._check_news_hot(symbol, start_date, end_date)
            else:
    ...
    def _merge_existence_data(
        self, hot_data: list[DataExistenceInfo], cold_data: list[DataExistenceInfo]
    ) -> list[DataExistenceInfo]:
        """
    ...
    def get_existence_summary(self, existence_data: list[DataExistenceInfo]) -> dict[str, Any]:
        """
