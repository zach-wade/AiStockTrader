...
class TestArchiveStress:
    ...
    async def stress_archive(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            metrics = {"write_count": 0, "read_count": 0, "errors": [], "start_time": time.time()}
    ...
    async def test_high_volume_stress(self, stress_archive):
        ...
        print("Writing 100,000 records rapidly...")
        ...
        try:
            for batch_idx in range(num_records // batch_size):
                ...
                if batch_idx % 10 == 0:
                    ...
                    print(
                        f"  Progress: {records_written}/{num_records} records "
    ...
    async def test_concurrent_operations_stress(self, stress_archive):
        ...
        print(
            f"Write operations: {write_results['success']} success, {write_results['failure']} failure"
        )
        print(
            f"Read operations: {read_results['success']} success, {read_results['failure']} failure"
        )
        print(
            f"Write success rate: {write_results['success'] / (write_results['success'] + write_results['failure']) * 100:.1f}%"
        )
        print(
            f"Read success rate: {read_results['success'] / (read_results['success'] + read_results['failure']) * 100:.1f}%"
        )
    ...
    async def test_large_file_stress(self, stress_archive):
        ...
        large_df = pd.DataFrame(
            np.random.randn(rows, cols), columns=[f"col_{i}" for i in range(cols)]
        )

        # Add some string columns for variety
        large_df["symbol"] = np.random.choice(["AAPL", "GOOGL", "MSFT", "AMZN"], rows)
        large_df["timestamp"] = pd.date_range(start="2024-01-01", periods=rows, freq="1min")
        ...
        try:
            ...
            records = await archive.query_raw_records(
                source="stress_test", data_type="large_file", symbol="LARGE_TEST"
            )
        ...
        async def monitor_resources():
            ...
            while not stop_flag:
                ...
                if len(memory_samples) > 1:
                    ...
                    print(
                        f"  [{len(memory_samples) * sample_interval}s] "
    ...
    async def test_error_recovery_stress(self, stress_archive):
        ...
        try:
            records = await archive.query_raw_records(source="stress_test", data_type="test_data")
        ...
        assert (
            concurrent_success > 45
        ), f"Too many failures in concurrent access: {50 - concurrent_success}"
...
if __name__ == "__main__":
    ...
    result = subprocess.run(
        ["pytest", __file__, "-v", "-s", "--tb=short"], capture_output=False, check=False
    )
