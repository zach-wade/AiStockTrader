...
async def test_event_flow():
    ...
    print("   Event Coordinator Stats:")
    print(
        f"     - Symbol qualified events: {stats['event_statistics'].get('symbol_qualified_events', 0)}"
    )
    ...
    if handler_stats_after["statistics"]["deduplicated"] > 0:
        print(
            f"   ✓ Deduplication working: {handler_stats_after['statistics']['deduplicated']} duplicates prevented"
        )
    else:
