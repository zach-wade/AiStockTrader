...
class MockAlpacaMarketClient(BaseSource):
    ...
    async def fetch_data(
        self, data_type: str, symbols: list[str], start_date: datetime, end_date: datetime, **kwargs
    ) -> list[RawDataRecord]:
    ...
    async def fetch_and_archive(
        ...
        **kwargs,
        ...
        return {"records_processed": archived_count, "data": records}
    ...
    async def get_current_price(self, symbol: str) -> float | None:
        ...
        base_prices = {"AAPL": 150.0, "MSFT": 300.0, "GOOGL": 2500.0, "TSLA": 200.0}
...
class MockPolygonMarketClient(BaseSource):
    ...
    async def get_current_price(self, symbol: str) -> float | None:
        """Get current price for a symbol."""
        # Return mock price based on symbol
        base_prices = {"AAPL": 150.5, "MSFT": 300.5, "GOOGL": 2500.5, "TSLA": 200.5}
...
class MockNewsClient(BaseSource):
    ...
    async def fetch_data(
        ...
        for symbol in symbols:
            ...
            for i in range(3):
                news_data = create_news_record(symbol, datetime.now(UTC), self.source_name)
                record = RawDataRecord(
        ...
        for symbol in symbols:
            ...
            for i in range(5):
                social_data = create_social_sentiment_record(
                    symbol, datetime.now(UTC), self.platform
                )
                record = RawDataRecord(
