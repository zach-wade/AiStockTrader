...
# Third-party imports
from models.common import AccountInfo, OrderType, Position
...
class MockBroker(BrokerInterface):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Background tasks
        self._fill_simulation_task: asyncio.Task | None = None
    ...
    async def submit_order(self, **order_params) -> dict[str, Any]:
        ...
        for callback in self.order_callbacks:
            callback(
                {"order_id": broker_order_id, "status": "submitted", "timestamp": datetime.now()}
            )

        return {"order_id": broker_order_id, "status": "submitted", "timestamp": datetime.now()}
    ...
    async def cancel_order(self, broker_order_id: str) -> dict[str, Any]:
        ...
        for callback in self.order_callbacks:
            callback(
                {"order_id": broker_order_id, "status": "cancelled", "timestamp": datetime.now()}
            )

        return {"order_id": broker_order_id, "status": "cancelled", "timestamp": datetime.now()}
    ...
    async def get_order(self, broker_order_id: str) -> dict[str, Any] | None:
    ...
    def _validate_order(self, **order_params) -> list[str]:
        ...
        if order_params.get("side") == Side.BUY:
            estimated_cost = quantity * self.market_prices.get(
                order_params.get("symbol", ""), 100.0
            )
    ...
    async def _fill_order(self, order_id: str, order: dict[str, Any]):
        ...
        for callback in self.order_callbacks:
            callback({"order_id": order_id, "status": order["status"], "timestamp": datetime.now()})
