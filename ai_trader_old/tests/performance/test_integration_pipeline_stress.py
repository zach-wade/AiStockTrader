...
class TestIntegrationPipelineStress:
    ...
    def config(self):
        ...
        config = OmegaConf.create(
            {
                "database": {"url": "sqlite:///test_stress.db"},
                "testing": {"mode": True},
                "logging": {"level": "INFO"},
                "data_sources": {
                    "alpaca": {"paper": True, "api_key": "test_key", "secret_key": "test_secret"},
                    "polygon": {"api_key": "test_key"},
                },
                "backtesting": {"commission": 0.005, "slippage": 0.001},
                "feature_pipeline": {"unified_config_path": "configs/features/unified_config.yaml"},
            }
        )
        return config
    ...
    @pytest.mark.asyncio
    @pytest.mark.stress
    async def test_full_pipeline_stress_bull_market(
        self, config, test_symbols, test_date_ranges, memory_monitor
    ):
        """
        ...
        try:
            ...
            with (
                patch(
                    "main.data_pipeline.storage.database_factory.DatabaseFactory"
                ) as mock_db_factory,
                patch(
                    "main.data_pipeline.ingestion.data_source_manager.DataSourceManager"
                ) as mock_dsm,
                patch(
                    "main.backtesting.run_system_backtest.SystemBacktestRunner"
                ...
                mock_clients = {"alpaca": Mock(), "polygon": Mock()}
                ...
                assert (
                    memory_stats["peak_mb"] < 8192
                ), f"Memory usage too high: {memory_stats['peak_mb']:.1f}MB (>8GB)"

                print("✅ PERF-TEST 5.1 PASSED")
            ...
            print(f"❌ PERF-TEST 5.1 FAILED: {e!s}")
            raise
        finally:
            gc.collect()
    ...
    @pytest.mark.asyncio
    @pytest.mark.stress
    async def test_high_volatility_stress(
        self, config, test_symbols, test_date_ranges, memory_monitor
    ):
        """
        ...
        try:
            ...
            with patch(
                ...
                assert (
                    avg_feature_time < 2.0
                ), f"Feature generation too slow: {avg_feature_time:.2f}s per symbol"
                assert (
                    elapsed_time < 300
                ), f"High volatility test took too long: {elapsed_time:.2f}s"
                assert (
                    memory_stats["peak_mb"] < 6144
                ), f"Memory usage during volatility: {memory_stats['peak_mb']:.1f}MB"

                print("✅ PERF-TEST 5.2 PASSED")
            ...
            print(f"❌ PERF-TEST 5.2 FAILED: {e!s}")
            raise
        finally:
            gc.collect()
    ...
    async def test_multi_strategy_concurrent_execution(self, config, test_symbols, memory_monitor):
        ...
        try:
            ...
            assert (
                memory_stats["peak_mb"] < 4096
            ), f"Memory usage too high: {memory_stats['peak_mb']:.1f}MB"
            ...
            print(f"❌ PERF-TEST 5.3 FAILED: {e!s}")
            raise
        finally:
            gc.collect()
        ...
        try:
            ...
            for scenario in error_scenarios:
                ...
                try:
                    ...
                    print(f"⚠️  Scenario {scenario} failed: {e!s}")
            ...
            ), f"At least 3 scenarios should recover, got {len(recovery_times)}"
            assert avg_recovery_time < 30, f"Average recovery too slow: {avg_recovery_time:.2f}s"
            assert (
                memory_stats["peak_mb"] < 6144
            ), f"Memory during error handling: {memory_stats['peak_mb']:.1f}MB"

            print("✅ PERF-TEST 5.4 PASSED")
            ...
            print(f"❌ PERF-TEST 5.4 FAILED: {e!s}")
            raise
        finally:
            gc.collect()
    ...
    def _generate_volatile_market_data(
        self, symbols: list[str], start_date: datetime, end_date: datetime
    ) -> dict[str, pd.DataFrame]:
...
class TestPipelineIntegrationBenchmarks:
    ...
    def test_feature_generation_throughput(self, benchmark):
        ...
        # Benchmark assertion
        assert (
            features_per_second > 50000
        ), f"Feature generation too slow: {features_per_second:,.0f} features/sec"
