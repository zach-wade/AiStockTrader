...
class TestArchivePerformance:
    ...
    async def archive_setup(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            metrics = ArchiveMetricsCollector(archive_path=temp_dir, metrics_window_size=10000)

            # Create archive with metrics
            archive = DataArchive(config={"local_path": temp_dir}, metrics_collector=metrics)
    ...
    async def test_single_record_write_speed(self, archive_setup):
        ...
        for size in sizes:
            ...
            print(
                f"Size {size}: {duration*1000:.2f}ms, {results[size]['throughput']:.0f} records/sec"
            )
    ...
    async def test_batch_write_performance(self, archive_setup):
        ...
        for batch_size in batch_sizes:
            ...
            print(
                f"Batch {batch_size}: {duration*1000:.2f}ms total, "
    ...
    async def test_concurrent_operations(self, archive_setup):
        ...
        print(
            f"Concurrent writes: {total_writes} records in {write_duration:.2f}s "
        ...
        for symbol in symbols:
            read_tasks.append(
                archive.query_raw_records(source="test", data_type="market_data", symbol=symbol)
            )
        ...
        print(
            f"Concurrent reads: {total_read} records in {read_duration:.2f}s "
        ...
        for size in data_sizes:
            for fmt in formats:
                ...
                read_results = await archive.query_raw_records(
                    source="test", data_type=data_type, symbol="TEST"
                )
                read_duration = time.time() - start
    ...
    async def test_query_performance(self, archive_setup):
        ...
        for test in query_tests:
            ...
            results = await archive.query_raw_records(
                source="test", data_type="market_data", **test["params"]
            )
            duration = time.time() - start

            print(
                f"{test['name']}: {len(results)} records in {duration*1000:.2f}ms "
                f"({len(results)/duration if duration > 0 else 0:.0f} records/sec)"
            )

            # Allow some tolerance for async timing
            assert (
                len(results) >= test["expected_count"] * 0.9
    ...
    async def test_storage_efficiency(self, archive_setup):
        ...
        for data_type, config in data_types.items():
            # Generate data
            data = {data_type: [config["data_generator"](i) for i in range(config["records"])]}
            ...
            storage_results[data_type] = {
                ...
                "compression_ratio": (
                    original_size / storage_metrics.used_size_bytes
                    if storage_metrics.used_size_bytes > 0
                    else 1
                ),
            ...
            print(
                f"{data_type}: Original={original_size/1024/1024:.2f}MB, "
        ...
        assert (
            storage_results["market_data"]["compression_ratio"] > 2
