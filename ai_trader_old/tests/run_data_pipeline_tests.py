...
class DataPipelineTestRunner:
    ...
    def run_specific_test(self, test_path, verbose=False):
        ...
        cmd = ["pytest", test_path, "-v" if verbose else "-q", "--tb=short", "-x"]
    ...
    def generate_coverage_report(self):
        ...
        cmd = ["coverage", "report", "--include=src/main/data_pipeline/*", "--show-missing"]

        subprocess.run(cmd, check=False)

        # Also generate XML for CI/CD
        subprocess.run(
            ["coverage", "xml", "-o", "coverage.xml", "--include=src/main/data_pipeline/*"],
            check=False,
        )
...
def main():
    ...
    parser.add_argument(
        "-i", "--integration-only", action="store_true", help="Run only integration tests"
    )
