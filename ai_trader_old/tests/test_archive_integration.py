...
async def test_news_archive_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        # Setup archive
        archive_config = {"storage_type": "local", "local_path": temp_dir}
        ...
        config = BulkLoadConfig(buffer_size=10, max_memory_mb=100, batch_timeout_seconds=10.0)
        ...
        result = await loader.load(data=test_articles, symbols=["AAPL", "GOOGL"], source="polygon")
...
async def test_corporate_actions_archive():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        test_actions = [
            ...
            {"symbol": "GOOGL", "type": "split", "ex_date": datetime.now(UTC), "split_ratio": 2.0},
        ]

        # Load data (should trigger archive)
        result = await loader.load(data=test_actions, symbols=["AAPL", "GOOGL"], source="polygon")
        ...
        if archive_path.exists():
            ...
            if dividend_files:
                print("✓ Archived dividend actions")
            if split_files:
                print("✓ Archived split actions")
        else:
            print("✗ Archive directory not created")
...
async def test_fundamentals_archive():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = await loader.load(
            data=test_fundamentals, symbols=["AAPL", "GOOGL"], source="polygon"
        )
        ...
        if archive_path.exists():
            ...
            if q1_files:
                print("✓ Archived Q1 2024 fundamentals")
                ...
                if meta_file.exists():
                    with open(meta_file) as f:
                        metadata = json.load(f)
                    print(
                        f"✓ Metadata shows {metadata.get('metrics', {}).get('has_revenue', 0)} records with revenue"
                    )
        else:
            print("✗ Archive directory not created")
