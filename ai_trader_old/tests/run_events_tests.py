...
class EventsTestRunner:
    ...
    def run_events_tests(
        ...
        cmd = self._build_pytest_command(test_paths, verbose, coverage, parallel, debug)
    ...
    def generate_test_report(self) -> str:
        ...
        report_file = (
            self.tests_dir
            / "reports"
            / f"events_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
    ...
    def _get_test_paths(self, test_type: str, component: str | None) -> list[Path]:
    ...
    def _build_pytest_command(
        self, test_paths: list[Path], verbose: bool, coverage: bool, parallel: bool, debug: bool
    ) -> list[str]:
    ...
    def _display_test_plan(self, test_type: str, component: str | None, test_paths: list[Path]):
    ...
    def _display_results(
        self, return_code: int, start_time: datetime, end_time: datetime, coverage: bool
    ):
        ...
        if coverage:
            print("📊 Coverage report: htmlcov/events/index.html")
    ...
    def _analyze_test_components(self) -> dict[str, dict[str, int]]:
        ...
        for component, patterns in component_patterns.items():
            ...
            components[component] = {"unit": unit_count, "integration": integration_count}
...
Examples:
  ...
  %(prog)s --test-report             # Generate test report
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Debug mode (no capture, full tracebacks)"
    )
    ...
    parser.add_argument(
        "--coverage-report", action="store_true", help="Generate coverage analysis report"
    )

    parser.add_argument(
        "--test-report", action="store_true", help="Generate comprehensive test report"
    )
