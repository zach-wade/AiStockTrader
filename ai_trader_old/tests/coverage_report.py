...
class CoverageAnalyzer:
    ...
    def parse_coverage_xml(self) -> dict[str, float]:
        ...
        return {"overall": overall_coverage, "packages": package_coverage}

    def identify_untested_components(self) -> dict[str, list[str]]:
        """Identify components without tests."""
        untested = {"modules": [], "classes": [], "functions": []}
...
"""
    ...
    def save_report(self, report: str, output_path: Path | None = None):
        """Save report to file."""
...
def main():
    ...
    parser.add_argument(
        "--run", action="store_true", help="Run tests with coverage before analysis"
    )
    parser.add_argument(
        "--report", type=str, default="coverage_report.md", help="Output report filename"
    )
    parser.add_argument(
        "--github-summary", action="store_true", help="Generate GitHub Actions summary"
    )
