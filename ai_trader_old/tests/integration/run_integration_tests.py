...
def run_integration_tests(
    verbose: bool = False, specific_test: str = None, markers: str = None, coverage: bool = False
):
    ...
    if coverage:
        cmd.extend(["--cov=src/main", "--cov-report=html", "--cov-report=term-missing"])
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run AI Trader integration tests")
    ...
    parser.add_argument(
        "-m", "--markers", help="Run tests with specific markers (e.g., 'slow', 'requires_db')"
    )
    ...
    exit_code = run_integration_tests(
        verbose=args.verbose, specific_test=args.test, markers=args.markers, coverage=args.coverage
    )
