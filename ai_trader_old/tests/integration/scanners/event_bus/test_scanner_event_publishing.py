...
class TestScannerEventPublishing:
    ...
    async def test_volume_scanner_event_publishing(
        self, volume_scanner: IScanner, event_collector, test_symbols, sample_volume_data
    ):
    ...
    async def test_technical_scanner_event_publishing(
        self, technical_scanner: IScanner, event_collector, test_symbols, sample_technical_data
    ):
    ...
    async def test_news_scanner_event_publishing(
        self, news_scanner: IScanner, event_collector, test_symbols, sample_news_data
    ):
        ...
        assert (
            "NEWS" in str(event.alert_type).upper() or "SENTIMENT" in str(event.alert_type).upper()
        )
    ...
    async def test_event_publishing_with_multiple_alerts(
        self, mock_event_scanner, event_collector, test_symbols
    ):
    ...
    async def test_event_publishing_performance(
        self, mock_event_scanner, event_collector, test_symbols, event_performance_thresholds
    ):
    ...
    async def test_event_publishing_error_handling(self, mock_event_scanner, test_symbols):
    ...
    async def test_event_metadata_preservation(
        self, volume_scanner: IScanner, event_collector, sample_volume_data
    ):
    ...
    async def test_concurrent_event_publishing(self, event_collector, test_symbols):
        ...
        scanners = [MockEventPublishingScanner(f"scanner_{i}", event_collector) for i in range(3)]

        # Run scanners concurrently
        tasks = [scanner.scan(symbols=test_symbols[:2]) for scanner in scanners]
    ...
    async def test_event_bus_subscription_integration(
        self, test_event_bus: IEventBus, mock_event_scanner
    ):
    ...
    async def test_event_publishing_with_batch_processing(
        self, test_event_bus: IEventBus, event_performance_thresholds
    ):
    ...
    async def test_event_publishing_with_no_event_bus(self, mock_scanner_repository, test_symbols):
        ...
        config = OmegaConf.create(
            {"enabled": True, "timeout_seconds": 30.0, "min_volume_ratio": 2.0}
        )

        scanner = VolumeScanner(
            config=config, repository=mock_scanner_repository, event_bus=None  # No event bus
        )
    ...
    async def test_event_deduplication(self, mock_event_scanner, event_collector, test_symbols):
    ...
    async def test_event_priority_handling(self, event_collector, test_symbols):
