...
# Local imports
from main.events.types.event_types import AlertType, ScannerAlertEvent
...
class TestCompleteScannerWorkflow:
    ...
    async def test_full_orchestrated_scan_workflow(
        ...
        accuracy_ratio = (
            len(high_confidence_alerts) / len(published_alerts) if published_alerts else 0
        )
        ...
        assert (
            ...
            < end_to_end_performance_thresholds["event_publishing_time_ms"]
        )
    ...
    async def test_volume_scanner_end_to_end_workflow(
        ...
        for symbol in realistic_market_data.keys():
            ...
            historical_volumes = df[df["timestamp"] < datetime.now(UTC) - timedelta(hours=1)][
                "volume"
            ]
    ...
    async def test_news_scanner_end_to_end_workflow(
        ...
        breaking_news_alerts = [
            alert for alert in alerts if alert.metadata.get("breaking_news", False)
        ]
    ...
    async def test_earnings_scanner_end_to_end_workflow(
        ...
        earnings_scanner.repository.get_earnings_data = AsyncMock(
            return_value=realistic_earnings_data
        )
    ...
    async def test_multi_scanner_correlation_workflow(
        ...
        end_to_end_orchestrator.repository.get_social_sentiment = AsyncMock(
            return_value=realistic_social_data
        )

        # Mock volume statistics
        volume_stats = {"AAPL": {"avg_volume": 50000000, "std_volume": 10000000, "data_points": 30}}
        end_to_end_orchestrator.repository.get_volume_statistics = AsyncMock(
            return_value=volume_stats
        )
        ...
        total_signal_types = len(
            [alerts for alerts in [volume_alerts, news_alerts, social_alerts] if alerts]
        )
    ...
    async def test_real_time_scanning_workflow(
        ...
        volume_scanner.repository.get_volume_statistics = AsyncMock(
            return_value={
        ...
        if all_alerts:
            ...
            volume_ratios = [alert.metadata.get("relative_volume", 1.0) for alert in all_alerts]
    ...
    async def test_data_quality_validation_workflow(
        self, end_to_end_scanner_factory, event_collector, comprehensive_test_symbols
    ):
    ...
    async def test_complete_feature_pipeline_integration(
        ...
        await end_to_end_event_bus.subscribe(EventType.SCANNER_ALERT, feature_pipeline_handler)
    ...
    async def test_system_scalability_workflow(
        ...
        scaled_time_threshold = (
            end_to_end_performance_thresholds["scan_completion_time_ms"] * symbol_scale_factor
        )
        scaled_memory_threshold = (
            end_to_end_performance_thresholds["memory_usage_mb"] * symbol_scale_factor
        )
