...
class TestSocialScannerRepository:
    ...
    async def test_viral_post_detection(
        ...
        with patch.object(scanner_repository, "get_social_sentiment") as mock_social:
            ...
            viral_posts = [post for post in aapl_posts if post.get("viral_score", 0) > 0.8]
    ...
    async def test_influencer_impact_analysis(
        ...
        with patch.object(scanner_repository, "get_social_sentiment") as mock_social:
            ...
            high_influence_posts = [
                post for post in aapl_posts if post.get("influence_score", 0) > 0.7
            ]
    ...
    async def test_sentiment_momentum_tracking(
        ...
        with patch.object(scanner_repository, "get_social_sentiment") as mock_social:
                    ...
                    momentum_social["AAPL"].append(
                        {
                            ...
                            "timestamp": base_time
                            - timedelta(hours=3 - hour, minutes=post_in_hour * 12),
                            "platform": "twitter",
                        }
                    )
            ...
            early_sentiment = sum(post["sentiment_score"] for post in early_posts) / len(
                early_posts
            )
    ...
    async def test_social_anomaly_detection(
        ...
        with patch.object(scanner_repository, "get_social_sentiment") as mock_social:
            ...
            recent_hour_posts = [
                post
                for post in aapl_posts
                if (datetime.now(UTC) - post["timestamp"]).total_seconds() < 3600
            ]
