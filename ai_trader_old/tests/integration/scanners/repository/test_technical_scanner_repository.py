...
class TestTechnicalScannerRepository:
    ...
    async def test_price_momentum_data(self, scanner_repository: IScannerRepository, test_symbols):
        """Test data retrieval for momentum calculations."""
        with patch.object(scanner_repository, "get_market_data") as mock_market:
            # Create trending price data
            dates = [datetime.now(UTC) - timedelta(days=i) for i in range(20)]
            ...
            query_filter = QueryFilter(start_date=dates[0], end_date=dates[-1], symbols=["AAPL"])
    ...
    async def test_breakout_pattern_data(
        ...
        with patch.object(scanner_repository, "get_market_data") as mock_market:
            ...
            dates = [datetime.now(UTC) - timedelta(days=i) for i in range(30)]
            dates.reverse()
            ...
            breakout_move = breakout_data["close"].iloc[-1] - consolidation_data["close"].iloc[-1]
    ...
    async def test_multi_timeframe_data(self, scanner_repository: IScannerRepository, test_symbols):
        """Test multi-timeframe data retrieval for comprehensive analysis."""
        with (
            patch.object(scanner_repository, "get_market_data") as mock_market_daily,
            ...
            daily_result = await scanner_repository.get_market_data(
                symbols=["AAPL"], query_filter=daily_filter
            )
    ...
    async def test_gap_detection_data(self, scanner_repository: IScannerRepository, test_symbols):
        """Test data retrieval for gap detection."""
        with patch.object(scanner_repository, "get_market_data") as mock_market:
            # Create data with price gaps
            dates = [datetime.now(UTC) - timedelta(days=i) for i in range(10)]
            dates.reverse()
    ...
    async def test_error_handling_insufficient_data(
        ...
        with patch.object(scanner_repository, "get_market_data") as mock_market:
            ...
            limited_data = pd.DataFrame(
                [{"symbol": "AAPL", "date": datetime.now(UTC), "close": 150.0, "volume": 1000000}]
            )
    ...
    async def test_performance_large_dataset(
        self, scanner_repository: IScannerRepository, performance_thresholds
    ):
        ...
        with patch.object(scanner_repository, "get_market_data") as mock_market:
            ...
            result = await scanner_repository.get_market_data(
                symbols=list(symbol_data.keys()), query_filter=query_filter
            )
