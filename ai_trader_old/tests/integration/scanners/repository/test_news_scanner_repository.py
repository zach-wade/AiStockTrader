...
class TestNewsScannerRepository:
    ...
    async def test_news_source_diversity(
        ...
        with patch.object(scanner_repository, "get_news_data") as mock_news:
            ...
            high_credibility = [item for item in result if item.get("credibility_score", 0) > 0.8]
    ...
    async def test_breaking_news_detection(
        ...
        with patch.object(scanner_repository, "get_news_data") as mock_news:
            ...
            breaking_news = [
                {
                    ...
                    "sentiment_score": 0.8,
                    "timestamp": datetime.now(UTC) - timedelta(minutes=5),  # Very recent
                    "source": "Reuters",
            ...
            breaking_items = [item for item in result if item.get("breaking_news", False)]
            assert len(breaking_items) == 1

            # Breaking news should be very recent
            breaking_item = breaking_items[0]
            time_diff = datetime.now(UTC) - breaking_item["timestamp"]
    ...
    async def test_sector_news_correlation(
        ...
        with patch.object(scanner_repository, "get_news_data") as mock_news:
            ...
            sector_impact_news = [item for item in result if item.get("sector_impact", False)]
