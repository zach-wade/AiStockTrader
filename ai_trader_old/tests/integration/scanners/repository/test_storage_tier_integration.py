...
class TestStorageTierIntegration:
    ...
    async def test_hot_storage_routing_recent_data(
        ...
        recent_filter = QueryFilter(
            start_date=datetime.now(UTC) - timedelta(hours=6),
        ...
        with patch.object(
            ...
            days_back = (datetime.now(UTC) - called_filter.start_date).days
    ...
    async def test_cold_storage_routing_historical_data(
        ...
        with patch.object(
            ...
            await scanner_repository.get_market_data(
                symbols=test_symbols[:2], query_filter=historical_filter
            )
    ...
    async def test_mixed_storage_query_handling(
        ...
        mixed_filter = QueryFilter(
            start_date=datetime.now(UTC) - timedelta(days=45),  # Spans 30-day boundary
        ...
        with patch.object(
            ...
            mock_query.return_value = [
                {"symbol": "AAPL", "date": datetime.now(UTC) - timedelta(days=10), "close": 150.0},
                {"symbol": "AAPL", "date": datetime.now(UTC) - timedelta(days=40), "close": 145.0},
            ]

            result = await scanner_repository.get_market_data(
                symbols=test_symbols[:3], query_filter=mixed_filter
            )
    ...
    async def test_storage_tier_performance_difference(
        ...
        with patch.object(
            ...
            await scanner_repository.get_market_data(
                symbols=test_symbols[:5], query_filter=hot_filter
            )
            ...
            start_time = datetime.now()
            await scanner_repository.get_market_data(
                symbols=test_symbols[:5], query_filter=cold_filter
            )
    ...
    async def test_storage_tier_data_consistency(
        ...
        with patch.object(
            ...
            consistent_data = [
                {"symbol": "AAPL", "date": boundary_date, "close": 150.0, "volume": 1000000}
            ]
            mock_query.return_value = consistent_data

            result = await scanner_repository.get_market_data(
                symbols=test_symbols[:2], query_filter=hot_filter
            )
    ...
    async def test_query_optimization_by_storage_tier(
        ...
        queries = [
            ...
            QueryFilter(
                ...
                end_date=datetime.now(UTC) - timedelta(days=150),
                symbols=test_symbols[:3],
            ),
        ]
        ...
        with patch.object(
            ...
            for query_filter in queries:
                await scanner_repository.get_market_data(
                    symbols=query_filter.symbols, query_filter=query_filter
                )
