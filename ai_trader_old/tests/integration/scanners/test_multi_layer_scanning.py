...
class TestMultiLayerScanning:
    ...
    async def test_layer_progression(self, scanner_orchestrator, universe_manager):
        """Test symbols progress through scanner layers."""
        # Run Layer 0 scan
        layer0_results = await scanner_orchestrator.run_layer_scan(layer=0, scan_type="daily")
        ...
        await universe_manager.update_layer_qualification(
            layer=0, qualified_symbols=layer0_results["qualified_symbols"]
        )

        # Run Layer 1 scan
        layer1_results = await scanner_orchestrator.run_layer_scan(layer=1, scan_type="daily")
    ...
    async def test_concurrent_layer_scanning(self, scanner_orchestrator, performance_threshold):
    ...
    async def test_scan_failure_recovery(self, scanner_orchestrator, monkeypatch):
        ...
        monkeypatch.setattr(scanner_orchestrator, "_execute_layer_scan", mock_failing_scan)

        # Run scan with retry
        result = await scanner_orchestrator.run_layer_scan(
            layer=0, scan_type="daily", retry_on_failure=True
        )
    ...
    async def test_universe_consistency(self, scanner_orchestrator, universe_manager, test_db_pool):
    ...
    async def test_scan_metadata_tracking(self, scanner_orchestrator, test_db_pool):
        """Test scan metadata and history tracking."""
        # Run scan
        result = await scanner_orchestrator.run_layer_scan(layer=1, scan_type="daily")
        ...
        async with test_db_pool.acquire() as conn:
            scan_record = await conn.fetchrow(
                "SELECT * FROM scan_history WHERE scan_id = $1", result["scan_id"]
            )
