...
# Local imports
from main.utils.core import secure_choice, secure_randint, secure_uniform
...
class MockAlpacaClient:
    ...
    async def get_bars(
        ...
        if self.rate_limit and self.call_count > self.rate_limit_threshold:
            raise RateLimitError("Rate limit exceeded", status_code=429, api_name="alpaca")

        # Simulate random failures
        if random.random() < self.fail_rate:
            raise APIError("Mock API failure", status_code=500, api_name="alpaca")
    ...
    async def get_trades(self, symbol: str, start: datetime, limit: int = 100) -> list[dict]:
        """Mock trade data."""
        trades = []
        current_time = start
        base_price = 100.0
...
class MockNewsClient:
    ...
    async def get_news(
        self, symbols: list[str], start_date: datetime, end_date: datetime, limit: int = 50
    ) -> list[dict]:
        """Mock news articles."""
        articles = []
