...
from main.interfaces.events import AsyncEventHandler, EventType
...
class TestCompleteDualStorageIntegration:
    ...
    async def event_collector(self, app_context):
        ...
        handler = AsyncEventHandler(
            handler_func=collect_event, event_types=[EventType.DATA_WRITTEN]
        )
    ...
    async def test_market_data_repository_full_cycle(self, repository_factory, event_collector):
        ...
        read_data = await repo.get_with_filters({"symbols": [test_symbol], "interval": "1day"})
        ...
        test_data = [
            {
                ...
                "ex_date": (datetime.now(UTC) + timedelta(days=7)).date(),
                "record_date": (datetime.now(UTC) + timedelta(days=8)).date(),
                "payment_date": (datetime.now(UTC) + timedelta(days=14)).date(),
                "amount": 0.24,
    ...
    async def test_guidance_repository_full_cycle(self, repository_factory):
        ...
        test_data = [
            {
                "symbol": "AAPL",
                "guidance_date": datetime.now(UTC),
                "period": "Q1",
                "year": 2025,
    ...
    async def test_features_repository_full_cycle(self, repository_factory):
        ...
        test_features = {"rsi": 65.5, "macd": 1.2, "volume_ratio": 1.5, "price_change": 0.03}
    ...
    async def test_all_repositories_concurrent_writes(self, repository_factory):
        ...
        for repo_type, repo in repos.items():
            has_writer = (
                hasattr(repo, "dual_storage_writer") and repo.dual_storage_writer is not None
            ) or (hasattr(repo, "_dual_storage_writer") and repo._dual_storage_writer is not None)
    ...
    async def test_event_publishing_reliability(
        self, app_context, repository_factory, event_collector
    ):
        ...
        data_written_events = [e for e in event_collector if e.event_type == EventType.DATA_WRITTEN]
        assert (
            len(data_written_events) >= 5
        ), f"Expected at least 5 events, got {len(data_written_events)}"
