...
# Local imports
from main.models.common import MarketData, Position, Signal
...
class TestPositionSizingIntegration:
    ...
    async def test_kelly_criterion_sizing(self, position_sizer, portfolio_state, market_data):
        ...
        assert (
            position_size["dollar_amount"] <= portfolio_state["total_equity"] * 0.20
        )  # Max position size

    @pytest.mark.asyncio
    async def test_fixed_fractional_sizing(self, position_sizer, portfolio_state):
        """Test fixed fractional position sizing."""
        fixed = FixedFractional(base_fraction=0.02, scale_by_confidence=True)  # 2% per trade
    ...
    async def test_volatility_based_sizing(self, position_sizer, market_data):
        ...
        vol_sizer = VolatilityBased(target_volatility=0.15, lookback_days=20)  # 15% annual
        ...
        signal = Signal(
            timestamp=datetime.now(), symbol="AAPL", direction="buy", strength=0.8, confidence=0.75
        )
    ...
    async def test_dynamic_sizing_with_regime(self, position_sizer, portfolio_state, market_data):
        ...
        dynamic = DynamicSizing(
            base_method="kelly",
            regime_adjustments={"bull": 1.2, "bear": 0.5, "neutral": 1.0, "high_volatility": 0.3},
        )
    ...
    async def test_position_sizing_with_correlation(self, position_sizer, portfolio_state):
        ...
        signal = Signal(
            timestamp=datetime.now(), symbol="QQQ", direction="buy", strength=0.8, confidence=0.8
        )
    ...
    async def test_risk_parity_sizing(self, position_sizer, portfolio_state, market_data):
        ...
        signal = Signal(
            timestamp=datetime.now(), symbol="MSFT", direction="buy", strength=0.7, confidence=0.75
        )
    ...
    async def test_position_sizing_with_stops(self, position_sizer, portfolio_state):
        ...
        signal = Signal(
            ...
            strength=0.8,
            confidence=0.8,
            metadata={"stop_loss": 0.02, "take_profit": 0.06},
        )
    ...
    async def test_multi_strategy_position_sizing(self, position_sizer, portfolio_state):
        ...
        for signal in signals:
            ...
            strategy_positions = sum(1 for s in sized_positions if s["strategy"] == strategy)
            ...
            sized_positions.append(
                {"symbol": signal.symbol, "strategy": strategy, "size": size["dollar_amount"]}
            )

        # Verify strategy allocations respected
        momentum_total = sum(p["size"] for p in sized_positions if p["strategy"] == "momentum")

        mean_rev_total = sum(
            p["size"] for p in sized_positions if p["strategy"] == "mean_reversion"
        )
