...
from main.data_pipeline.storage.dual_storage_writer import (
    DualStorageConfig,
    DualStorageWriter,
    WriteMode,
)
...
class TestDualStorageIntegration:
    ...
    async def test_dual_storage_write_flow(self, mock_components):
        ...
        config = DualStorageConfig(
            mode=WriteMode.ASYNC, enable_compression=True, enable_deduplication=True
        )

        # Create writer
        writer = DualStorageWriter(config=config, event_bus=mock_components["event_bus"])
        ...
        # Write data
        result = await writer.write(repository_name="market_data", data=test_data)
    ...
    async def test_cold_storage_query_engine(self, mock_components):
        ...
        with patch.object(engine, "_find_files", return_value=["/path/to/file.parquet"]):
            with patch("pandas.read_parquet", return_value=test_df):
                ...
                result = await engine.query(table_name="market_data", query_filter=query_filter)
    ...
    async def test_data_lifecycle_archival(self, mock_components):
        ...
        manager = DataLifecycleManager(
            config=config, db_adapter=mock_components["db"], archive=mock_components["archive"]
        )
        ...
        with patch.object(manager, "_identify_archive_candidates") as mock_identify:
            mock_identify.return_value = [
                {
                    "symbol": "AAPL",
                    "archive_date": datetime(2024, 1, 1).date(),
                    "record_count": 100,
                },
                {
                    "symbol": "GOOGL",
    ...
    async def test_end_to_end_dual_storage_flow(self, mock_components):
        ...
        writer = DualStorageWriter(
            config=DualStorageConfig(mode=WriteMode.ASYNC), event_bus=mock_components["event_bus"]
        )
        writer.repository_factory = mock_components["repo_factory"]
        writer.archive = mock_components["archive"]
        ...
        test_data = pd.DataFrame(
            {
                "timestamp": [datetime.now(UTC) - timedelta(days=i) for i in range(100)],
                "symbol": ["AAPL"] * 100,
                "close": [100.0 + i for i in range(100)],
            }
        )
        ...
        old_records = [
            ...
            if (datetime.now(UTC) - row["timestamp"]).days > 30
        ]
    ...
    async def test_query_performance_metrics(self):
        ...
        filters = [
            (
                QueryFilter(symbols=["AAPL"], start_date=datetime.now(UTC) - timedelta(days=5)),
                QueryType.REAL_TIME,
            ),
            (
                QueryFilter(symbols=["GOOGL"], start_date=datetime.now(UTC) - timedelta(days=90)),
                QueryType.ANALYSIS,
            ),
            (
                QueryFilter(
