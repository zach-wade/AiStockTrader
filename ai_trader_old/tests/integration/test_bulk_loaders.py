...
async def test_market_data_archive(metrics: TestMetrics):
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            loader = MarketDataBulkLoader(db_adapter=db_adapter, archive=archive, config=config)
            ...
            # Load data
            result = await loader.load(data=test_data, symbols=["AAPL"], source="polygon")
            ...
            if len(files) > 0:
                ...
                if len(archived_records) >= len(test_data):
                    metrics.record_pass(
                        f"{test_name} - Archive created with {len(archived_records)} records"
                    )
                else:
                    metrics.record_fail(
                        test_name,
                        f"Archive has {len(archived_records)} records, expected {len(test_data)}",
                    )
            else:
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if len(intervals_found) >= 2:  # Should have multiple intervals
                metrics.record_pass(
                    f"{test_name} - Archived {len(intervals_found)} different intervals"
                )
            else:
...
async def test_news_loader_deduplication(metrics: TestMetrics):
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Load data
            result = await loader.load(data=test_articles, symbols=["AAPL"], source="polygon")
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            async def load_symbol_data(symbol: str):
                ...
                return await loader.load(data=data, symbols=[symbol], source="polygon")

            # Load all symbols concurrently
            start_time = time.time()
            results = await asyncio.gather(*[load_symbol_data(symbol) for symbol in symbols])
            ...
            if duration < 2.0:  # Should complete within 2 seconds
                metrics.record_pass(
                    f"{test_name} - Loaded {len(symbols)} symbols in {duration:.2f}s"
                )
            else:
                metrics.record_fail(
                    test_name, f"Too slow: {duration:.2f}s for {len(symbols)} symbols"
                )
...
async def test_large_batch_handling(metrics: TestMetrics):
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            db_adapter = create_mock_db_adapter()
            config = BulkLoadConfig(
                buffer_size=1000, max_memory_mb=1  # Very small limit to trigger flush
            )
            ...
            # Load large batch
            result = await loader.load(data=large_data, symbols=["AAPL"], source="polygon")
