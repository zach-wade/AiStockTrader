...
from main.data_pipeline.types import DataPipelineStatus
from main.data_pipeline.validation.validation_pipeline import ValidationPipeline
from tests.fixtures.data_pipeline.database_fixtures import MockArchive, MockAsyncDatabase
...
class TestCompletePipelineFlow:
    ...
    async def ingestion_orchestrator(self, mock_clients, mock_archive):
        ...
        with patch(
            "main.data_pipeline.ingestion.orchestrator.get_archive", return_value=mock_archive
        ):
    ...
    async def test_market_data_full_flow(
        ...
        if ingested_data:
            validation_result = await validation_pipeline.validate_ingest(
                data=ingested_data, source="alpaca", data_type="market_data"
            )
        ...
        processing_result = await processing_manager.process_data(
            data_type="market_data", raw_records=mock_raw_records
        )
        ...
        if hasattr(processing_result, "data"):
            ...
            post_validation_result = await validation_pipeline.validate_post_etl(
                data=processed_data, data_type="market_data"
            )

            assert post_validation_result.passed is True

    async def test_multi_source_ingestion(self, test_config, mock_clients, mock_archive):
    ...
    async def test_validation_failure_handling(self, validation_pipeline):
        ...
        result = await validation_pipeline.validate_ingest(
            data=invalid_data, source="test", data_type="market_data"
        )
    ...
    async def test_news_data_flow(self, ingestion_orchestrator, validation_pipeline, mock_archive):
    ...
    async def test_concurrent_data_type_processing(self, ingestion_orchestrator, mock_archive):
    ...
    async def test_error_recovery_flow(self, test_config, mock_clients, mock_archive):
    ...
    async def test_batch_processing_performance(self, ingestion_orchestrator, mock_archive):
    ...
    async def test_end_to_end_with_storage(
        self, test_config, mock_clients, mock_db_adapter, mock_archive
    ):
        """Test complete end-to-end flow including database storage."""
        # Mock repository
        mock_repo = Mock()
        mock_repo.bulk_upsert = AsyncMock(
            return_value=Mock(success=True, processed_records=10, failed_records=0)
        )

        # Create data pipeline orchestrator
        with patch("main.data_pipeline.orchestrator.get_archive", return_value=mock_archive):
            with patch(
                "main.data_pipeline.orchestrator.MarketDataRepository", return_value=mock_repo
            ):
