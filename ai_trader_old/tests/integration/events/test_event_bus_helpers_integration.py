...
from main.events.types import FeatureRequestEvent, OrderEvent, ScannerAlertEvent
...
async def event_bus_with_helpers():
    ...
    dlq_manager = DeadLetterQueueManager(db_pool=mock_db_pool, max_retries=3, retention_days=7)
...
def sample_events():
    ...
    return [
        ...
        ScannerAlertEvent(
            data={"symbol": "GOOGL", "alert_type": "high_volume", "confidence": 0.85}
        ),
        FeatureRequestEvent(
            symbols=["MSFT"], features=["price_features"], requester="test_scanner"
        ),
    ]

    @pytest.mark.asyncio
    async def test_complete_event_lifecycle_tracking(self, event_bus_with_helpers, sample_events):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_with_dlq_integration(self, event_bus_with_helpers):
        ...
        test_event = OrderEvent(
            symbol="FAIL", quantity=50, price=100.0, side="sell", order_type="market"
        )
    ...
    @pytest.mark.asyncio
    async def test_high_volume_event_processing_with_helpers(self, event_bus_with_helpers):
    ...
    @pytest.mark.asyncio
    async def test_event_replay_with_helpers(self, event_bus_with_helpers, sample_events):
        ...
        historical_events = await history.get_events_by_type(EventType.ORDER_PLACED, limit=10)
    ...
    @pytest.mark.asyncio
    async def test_dlq_retry_mechanism_integration(self, event_bus_with_helpers):
        ...
        # Create test event
        test_event = ScannerAlertEvent(data={"symbol": "RETRY_TEST", "alert_type": "test_retry"})
    ...
    @pytest.mark.asyncio
    async def test_memory_management_under_load(self, event_bus_with_helpers):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_helper_operations(self, event_bus_with_helpers):
        ...
        await asyncio.gather(
            publisher_task(), stats_reader_task(), history_reader_task(), dlq_operations_task()
        )
    ...
    @pytest.mark.asyncio
    async def test_helper_cleanup_and_shutdown(self, event_bus_with_helpers):
