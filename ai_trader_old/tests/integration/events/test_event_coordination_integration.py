...
from main.events.types import ScanAlert, ScannerAlertEvent
...
async def coordinated_event_system():
    ...
    mock_feature_service.compute_features = AsyncMock(
        return_value={
            "AAPL": {
...
class TestEventCoordinationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_end_to_end_flow(self, coordinated_event_system, end_to_end_test_data):
        ...
        for event_type in published_events:
            event_bus.subscribe(
                event_type, lambda evt, et=event_type: asyncio.create_task(event_tracker(et, evt))
            )
    ...
    @pytest.mark.asyncio
    async def test_multi_stage_event_correlation(self, coordinated_event_system):
        ...
        original_alert = next(
            (
                evt
                for evt in correlation_tracking["scanner_alerts"]
        ...
        if correlation_tracking["feature_requests"]:
            correlated_requests = [
                evt
                for evt in correlation_tracking["feature_requests"]
                if evt["correlation_id"] == test_correlation_id
            ]
    ...
    @pytest.mark.asyncio
    async def test_priority_propagation_across_stages(self, coordinated_event_system):
        ...
        test_alerts = [
            ...
            # Medium priority
            ScannerAlertEvent(
                symbol="MED_PRIORITY", alert_type="breakout", score=0.6, scanner_name="test_scanner"
            ),
        ]
        ...
        feature_request_priorities = [
            track
            for track in priority_tracking
        ...
        if feature_request_priorities:
            ...
            catalyst_priority = next(
                (
                    track["priority"]
                    for track in feature_request_priorities
    ...
    @pytest.mark.asyncio
    async def test_concurrent_processing_coordination(self, coordinated_event_system):
        ...
        publish_tasks = [event_bus.publish(alert) for alert in concurrent_alerts]
    ...
    @pytest.mark.asyncio
    async def test_error_isolation_across_components(self, coordinated_event_system, caplog):
        ...
        mixed_alerts = [
            ScannerAlertEvent(
                symbol="SUCCESS_1", alert_type="high_volume", score=0.7, scanner_name="test_scanner"
            ),
            ScannerAlertEvent(
                symbol="FAIL_SYMBOL", alert_type="breakout", score=0.8, scanner_name="test_scanner"
            ),
            ScannerAlertEvent(
                symbol="SUCCESS_2",
                alert_type="catalyst_detected",
    ...
    @pytest.mark.asyncio
    async def test_batching_coordination_across_components(self, coordinated_event_system):
    ...
    @pytest.mark.asyncio
    async def test_system_performance_under_load(self, coordinated_event_system):
    ...
    @pytest.mark.asyncio
    async def test_graceful_shutdown_coordination(self, coordinated_event_system):
