...
from main.events.feature_pipeline_helpers.feature_handler_stats_tracker import (
...
async def feature_pipeline_components():
    ...
    mock_config = {
        ...
        "features": {"default_priority": 5, "cache_timeout": 300, "computation_timeout": 60},
    }
    ...
    worker = FeatureComputationWorker(feature_service=mock_feature_service, config=mock_config)
...
def sample_feature_requests():
    ...
    return [
        FeatureRequestEvent(
            symbols=["AAPL"], features=["price_features"], requester="test_scanner_1", priority=5
        ),
        FeatureRequestEvent(
            symbols=["MSFT", "GOOGL"],
        ...
        FeatureRequestEvent(
            symbols=["TSLA"], features=["all_features"], requester="urgent_scanner", priority=9
        ),
    ]
...
class TestFeaturePipelineHelpersIntegration:
    ...
    async def test_complete_feature_request_lifecycle(
        self, feature_pipeline_components, sample_feature_requests, feature_event_bus
    ):
    ...
    async def test_feature_group_mapping_integration(
        self, feature_pipeline_components, feature_event_bus
    ):
        ...
        test_cases = [
            ...
            {"input_features": ["volume_features"], "expected_groups": ["volume_features"]},
            {
        ...
        for case in test_cases:
            ...
            request = FeatureRequestEvent(
                symbols=["TEST_SYMBOL"], features=case["input_features"], requester="mapper_test"
            )
    ...
    async def test_queue_manager_priority_handling(
        ...
        for req in priority_requests:
            await queue_manager.enqueue_request(req["symbols"], req["features"], req["priority"])
    ...
    async def test_deduplication_across_components(
        ...
        duplicate_requests = [
            ...
            {"symbols": ["MSFT"], "features": ["volume_features"], "priority": 7},
            {
                ...
                "priority": 6,
            },  # Similar but different priority
            {
                "symbols": ["MSFT"],
    ...
    async def test_error_handling_across_pipeline(
        self, feature_pipeline_components, feature_event_bus, caplog
    ):
        ...
        assert (
            "Feature computation failed" in caplog.text
            or "Error processing feature request" in caplog.text
        )
    ...
    @pytest.mark.asyncio
    async def test_configuration_integration(self, feature_pipeline_components, feature_event_bus):
        ...
        for i in range(original_batch_size):
            await queue_manager.enqueue_request([f"BATCH_TEST_{i}"], ["price_features"], 5)
    ...
    @pytest.mark.asyncio
    async def test_pipeline_shutdown_cleanup(self, feature_pipeline_components, feature_event_bus):
