...
async def performance_optimized_system():
    ...
    config = EventBusConfig(
        max_queue_size=10000, max_workers=4, enable_history=False, enable_dlq=False
    )
...
class TestPerformanceIntegration:
    ...
    async def test_high_throughput_alert_processing(
        self, performance_optimized_system, performance_test_data
    ):
        ...
        ), f"Publish throughput {publish_throughput:.2f} alerts/sec too low"
        ...
        print(
            f"High throughput test: {publish_throughput:.2f} pub/sec, {total_throughput:.2f} total/sec"
        )
    ...
    async def test_concurrent_processing_scalability(
        ...
        assert (
            concurrent_throughput > 800
        ), f"Concurrent throughput {concurrent_throughput:.2f} events/sec too low"

        # Verify each scenario maintained good performance
        for result in results:
            assert (
                result["throughput"] > 200
            ), f"Scenario {result['scenario']} throughput {result['throughput']:.2f} too low"
        ...
        print(
            f"Concurrent scalability: {concurrent_throughput:.2f} events/sec across {len(scenarios)} scenarios"
        )
    ...
    async def test_memory_efficiency_under_load(
        ...
        assert (
            avg_memory_per_wave < 15
        ), f"Average memory per wave {avg_memory_per_wave:.2f}MB too high"
        ...
        print(
            f"Memory efficiency: {total_memory_growth:.2f}MB growth, {avg_memory_per_wave:.2f}MB avg/wave"
        )
    ...
    async def test_latency_characteristics(
        ...
        if computation_latencies:
            ...
            p95_computation_latency = sorted(computation_latencies)[
                int(len(computation_latencies) * 0.95)
            ]
        else:
        ...
        assert (
            avg_publish_latency < 0.01
        ), f"Average publish latency {avg_publish_latency:.4f}s too high"
        assert (
            p95_publish_latency < 0.05
        ), f"P95 publish latency {p95_publish_latency:.4f}s too high"
        assert max_publish_latency < 0.1, f"Max publish latency {max_publish_latency:.4f}s too high"

        if computation_latencies:
            assert (
                avg_computation_latency < 0.01
            ), f"Average computation latency {avg_computation_latency:.4f}s too high"
            assert (
                p95_computation_latency < 0.05
            ), f"P95 computation latency {p95_computation_latency:.4f}s too high"
        ...
        print(
            f"Latency: pub avg={avg_publish_latency:.4f}s p95={p95_publish_latency:.4f}s, "
            f"comp avg={avg_computation_latency:.4f}s p95={p95_computation_latency:.4f}s"
        )

    @pytest.mark.asyncio
    async def test_batching_efficiency(self, performance_optimized_system, performance_test_data):
        ...
        # Batching tracking
        batch_tracking = {"feature_requests": [], "computations": [], "batching_efficiency": []}
        ...
        for scenario in batching_scenarios:
            ...
            scenario_requests = [
                req
                for req in batch_tracking["feature_requests"]
                if req["timestamp"] >= scenario_start and req["timestamp"] <= scenario_end
            ]
            scenario_computations = [
                comp
                for comp in batch_tracking["computations"]
            ...
            batching_efficiency = {
                ...
                "avg_symbols_per_request": sum(req["symbol_count"] for req in scenario_requests)
                / max(len(scenario_requests), 1),
                "avg_symbols_per_computation": sum(
                    comp["symbol_count"] for comp in scenario_computations
                )
                / max(len(scenario_computations), 1),
        ...
        assert (
            overall_batching_ratio < 0.8
        ), f"Overall batching ratio {overall_batching_ratio:.3f} indicates poor batching"
        ...
        for efficiency in batch_tracking["batching_efficiency"]:
            ...
            assert (
                efficiency["batching_ratio"] <= 1.0
            ), f"Scenario {efficiency['scenario']} batching ratio too high"
        ...
        print(
            f"Batching efficiency: {overall_batching_ratio:.3f} computation/alert ratio, "
    ...
    async def test_resource_utilization_efficiency(
        ...
        for phase in load_phases:
            ...
            print(
                f"Phase {phase['name']}: {phase_events} events in {phase_duration:.2f}s "
        ...
        ), f"Processing efficiency {processing_efficiency:.3f} too low"

        # Resource per event efficiency
        avg_cpu_per_event = avg_cpu / (
            total_events_generated / sum(phase["duration"] for phase in load_phases)
        )
        assert avg_cpu_per_event < 0.5, f"CPU per event {avg_cpu_per_event:.3f}% too high"

        print(
            f"Resource efficiency: {avg_cpu:.1f}% avg CPU, {peak_memory:.1f}MB peak memory, "
            f"{processing_efficiency:.3f} processing efficiency"
        )

    @pytest.mark.asyncio
    async def test_scalability_limits(self, performance_optimized_system, performance_test_data):
        ...
        for test in scalability_tests:
            ...
            performance_metrics = {"events_published": 0, "publish_errors": 0, "processing_time": 0}
            ...
            if test["name"] == "event_rate":
                ...
                for event in events:
                    ...
                    if (
                        test["name"] != "event_rate"
                        and performance_metrics["events_published"] % 100 == 0
                    ):
                        await asyncio.sleep(0.01)
            ...
            processing_efficiency = (
                bridge_stats["alerts_received_total"] / performance_metrics["events_published"]
            )
            ...
            assert processing_efficiency > (
                1.0 - test["expected_degradation"]
            ...
            ), f"Publish throughput {publish_throughput:.2f} too low for {test['name']}"

            print(
                f"  {test['name']}: {publish_throughput:.1f} pub/sec, {processing_efficiency:.3f} efficiency, "
        ...
        print(
            f"Scalability limits: min throughput {min_throughput:.1f} events/sec, "
