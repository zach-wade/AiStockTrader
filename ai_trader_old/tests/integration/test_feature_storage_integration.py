...
class TestDataArchiveIntegration:
    ...
    async def test_feature_key_generation_patterns(self, mock_data_archive):
        ...
        test_cases = [
            ...
            {
                "symbol": "MSFT",
                "date": datetime(2024, 12, 31, tzinfo=UTC),
...
class TestHotColdStorageRouting:
    ...
    @pytest.mark.asyncio
    async def test_storage_tier_routing_by_age(
        self, mock_db_adapter, mock_data_archive, mock_database_config
    ):
    ...
    async def test_hybrid_query_spanning_storage_tiers(self, mock_db_adapter, mock_data_archive):
        ...
        hot_data = pd.DataFrame(
            {
                "timestamp": pd.date_range("2024-01-08", periods=3, freq="D", tz=UTC),
                "symbol": "AAPL",
        ...
        hot_result = await repo.get_features(
            symbol="AAPL",
            start_time=datetime(2024, 1, 8, tzinfo=UTC),
            end_time=datetime(2024, 1, 10, tzinfo=UTC),
        )

        # Query cold storage
        cold_result = await mock_data_archive.load_dataframe(
            "features/AAPL/2024/01/01/features.parquet"
        )
    ...
    @pytest.mark.asyncio
    async def test_storage_fallback_mechanisms(
        self, mock_db_adapter, mock_data_archive, sample_feature_data
    ):
        ...
        try:
            ...
            result = await mock_data_archive.load_dataframe(
                "features/AAPL/2024/01/01/features.parquet"
            )
...
class TestFeatureVersioning:
    ...
    async def test_feature_schema_versioning(self, mock_db_adapter):
        ...
        features_v1 = {"sma_10": 100.0, "sma_20": 99.5, "rsi": 65.0}
        ...
        versions = [("1.0", features_v1), ("2.0", features_v2), ("3.0", features_v3)]
    ...
    async def test_feature_migration_simulation(self, mock_db_adapter, mock_data_archive):
        ...
        # Load and migrate
        old_data = await mock_data_archive.load_dataframe(
            "features/AAPL/2023/12/31/features.parquet"
        )
