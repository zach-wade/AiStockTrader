...
class TestEnvironmentSetup:
    ...
    def _install_test_dependencies(self) -> bool:
        ...
        if self.requirements_file.exists():
            ...
            if result.returncode == 0:
                ...
                print("⚠️  Warning: Could not install all project requirements")
    ...
    def _validate_test_config(self) -> bool:
        ...
        required_files = [self.tests_dir / "pytest.ini", self.tests_dir / "conftest.py"]
...
def main():
    ...
    parser.add_argument(
        "--check-only", action="store_true", help="Only check environment, don't install packages"
    )

    parser.add_argument(
        "--minimal", action="store_true", help="Install only core test dependencies"
    )
    ...
    try:
        if args.check_only:
            ...
            success = setup._check_python_version() and setup._validate_test_config()
        else:
            # Full setup
