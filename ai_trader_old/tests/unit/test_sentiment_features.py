...
class TestSentimentConfig(unittest.TestCase):
    ...
    def test_custom_initialization(self):
        ...
        config = SentimentConfig(
            sentiment_windows=[1, 5, 10], news_lookback_days=14, mention_threshold=50
        )
...
class TestSentimentFeaturesCalculator(unittest.TestCase):
    ...
    def test_get_feature_names(self):
        ...
        options_features = [
            f for f in feature_names if any(x in f for x in ["put_call", "iv_", "options"])
        ]
    ...
    def test_calculate_basic_features(self):
        ...
        price_features = [
            col
            for col in columns
            if any(x in col for x in ["price_momentum", "volume_sentiment", "near_", "above_sma"])
        ]
        ...
        basic_options = [
            col
            for col in columns
            if any(x in col for x in ["implied_move", "fear_gauge", "vol_term_structure"])
        ]
    ...
    def test_calculate_with_social_data(self):
        ...
        mention_features = [
            col for col in columns if any(x in col for x in ["mention_velocity", "is_viral", "wsb"])
        ]
    ...
    def test_calculate_with_options_data(self):
        ...
        columns = result.columns.tolist()
        options_features = [
            col for col in columns if any(x in col for x in ["put_call", "iv_", "options_flow"])
        ]
        self.assertGreater(len(options_features), 0)
    ...
    def test_calculate_with_analyst_data(self):
        ...
        columns = result.columns.tolist()
        analyst_features = [
            col for col in columns if any(x in col for x in ["analyst", "rating", "price_target"])
        ]
    ...
    def test_text_sentiment_calculation(self):
        ...
        # Test positive text
        positive_score = self.calculator._calculate_text_sentiment(
            "Great news! Stock is performing well!"
        )
        self.assertGreater(positive_score, 0)

        # Test negative text
        negative_score = self.calculator._calculate_text_sentiment(
            "Bad news, company is struggling"
        )
    ...
    def test_time_decay_weights(self):
        ...
        timestamps = pd.Series([datetime(2023, 1, 1), datetime(2023, 1, 2), datetime(2023, 1, 3)])
        reference_time = datetime(2023, 1, 3)
        decay_factor = 0.9

        weights = self.calculator._calculate_time_decay_weights(
            timestamps, reference_time, decay_factor
        )
        ...
        self.news_data = pd.DataFrame(
            {
                ...
                "text": [
                    f'Market update: Company performance {"strong" if s > 0 else "weak"}'
        ...
        self.social_data = pd.DataFrame(
            {
                ...
                "source": ["reddit"] * (len(social_dates) // 2)
                + ["twitter"] * (len(social_dates) // 2),
