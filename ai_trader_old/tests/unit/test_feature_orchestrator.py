...
def mock_data_collector():
    ...
    sample_df = pd.DataFrame(
        {"close": [100, 101]}, index=pd.to_datetime(["2025-01-01", "2025-01-02"])
    )
...
@pytest.mark.asyncio
async def test_orchestrator_full_run_success(
    MockFeatureStore, mock_data_collector, mock_cache_manager, mock_calculators, mock_feature_store
):
    """
    ...
    try:
        ...
        try:
            ...
            try:
                orchestrator = FeatureEngineeringOrchestrator(
                    data_source=mock_data_collector, cache=mock_cache_manager
                )
            except TypeError:
    ...
    # Act
    result_store = await orchestrator.calculate_and_store_features(
        symbols=symbols, lookback_days=10
    )
...
@patch("main.feature_pipeline.feature_orchestrator.FeatureStore")
@pytest.mark.asyncio
async def test_orchestrator_handles_no_market_data(
    MockFeatureStore, mock_data_collector, mock_cache_manager, mock_calculators, mock_feature_store
):
    """
...
@patch("main.feature_pipeline.feature_orchestrator.FeatureStore")
@pytest.mark.asyncio
async def test_orchestrator_handles_calculator_error(
    MockFeatureStore, mock_data_collector, mock_cache_manager, mock_feature_store, caplog
):
    """
...
@pytest.mark.asyncio
async def test_orchestrator_empty_symbols_list(
    mock_data_collector, mock_cache_manager, mock_feature_store
):
    """Test that the orchestrator handles empty symbols list gracefully."""
    with patch(
        "feature_pipeline.feature_orchestrator.FeatureStore", return_value=mock_feature_store
    ):
        # Arrange
        orchestrator = FeatureEngineeringOrchestrator()
        orchestrator.data_collector = mock_data_collector
...
@pytest.mark.asyncio
async def test_orchestrator_invalid_lookback_days(
    mock_data_collector, mock_cache_manager, mock_feature_store
):
...
async def test_orchestrator_with_full_mock():
    ...
    with patch(
        "feature_pipeline.feature_orchestrator.FeatureEngineeringOrchestrator"
    ) as MockOrchestrator:
