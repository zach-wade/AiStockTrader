...
def problematic_data():
    ...
    data = pd.DataFrame(
        {"close": close_prices, "volume": [1000] * 20, "timestamp": dates}, index=dates
    )
...
class TestDataPreprocessing:
    ...
    def test_preprocess_data_handles_missing_values(self, test_calculator):
        ...
        data_with_nan = pd.DataFrame(
            {"close": [100, np.nan, 102, np.nan, 104], "volume": [1000, 1500, np.nan, 2000, 2500]}
        )
    ...
    def test_preprocess_data_forward_fill_option(self, default_config):
        ...
        data_with_nan = pd.DataFrame(
            {"close": [100, np.nan, np.nan, 103], "volume": [1000, 1500, np.nan, 2000]}
        )
    ...
    def test_preprocess_data_drop_option(self, default_config):
        ...
        data_with_nan = pd.DataFrame(
            {"close": [100, np.nan, 102, 103], "volume": [1000, 1500, 2000, 2500]}
        )
...
class TestFeaturePostprocessing:
    ...
    def test_postprocess_features_removes_infinite(self, test_calculator):
        ...
        features = pd.DataFrame(
            {"feature1": [1, 2, np.inf, 4, 5], "feature2": [1, -np.inf, 3, 4, 5]}
        )
    ...
    def test_postprocess_features_fills_nan(self, test_calculator):
        """Test NaN filling."""
        features = pd.DataFrame(
            {"feature1": [1, np.nan, 3, 4, 5], "feature2": [np.nan, 2, 3, np.nan, 5]}
        )
...
class TestValidationPipeline:
    ...
    def test_calculate_with_validation_output_validation(
        self, test_calculator, sample_market_data, caplog
    ):
