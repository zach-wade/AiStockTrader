    ...
    data = pd.DataFrame(
        {
            ...
            "high": 100
            + np.cumsum(secure_numpy_normal(0, 0.02, 100))
            + np.abs(secure_numpy_normal(0, 0.01, 100)),
            "low": 100
            + np.cumsum(secure_numpy_normal(0, 0.02, 100))
            - np.abs(secure_numpy_normal(0, 0.01, 100)),
...
class TestFeatureCalculation:
    ...
    def test_calculate_for_dataframe_handles_calculator_errors(
        self, mock_engine_with_mocks, sample_market_data, caplog
    ):
    ...
    def test_calculate_features_with_specific_calculators(
        self, mock_engine_with_mocks, sample_market_data
    ):
    ...
    def test_calculate_features_with_validation(
        self, mock_engine_with_mocks, sample_market_data, caplog
    ):
        ...
        result = engine.calculate_features(data=sample_market_data, symbol="TEST")
    ...
    def test_calculate_features_adds_metadata(self, mock_engine_with_mocks, sample_market_data):
        ...
        result = engine.calculate_features(
            data=sample_market_data, symbol="TEST", calculators=["technical"]
        )
    ...
    def test_calculate_features_all_calculators_by_default(
        self, mock_engine_with_mocks, sample_market_data
    ):
...
class TestErrorHandling:
    ...
    def test_calculator_instantiation_failure(self, mock_config, caplog):
        ...
        with patch("main.feature_pipeline.unified_feature_engine.TechnicalIndicators") as mock_tech:
            with patch(
                "main.feature_pipeline.unified_feature_engine.MarketRegimeCalculator"
            ) as mock_regime:
    ...
    def test_missing_calculator_graceful_handling(self, mock_engine_with_mocks, sample_market_data):
        ...
        result = engine.calculate_features(
            data=sample_market_data, symbol="TEST", calculators=["technical", "nonexistent"]
        )
