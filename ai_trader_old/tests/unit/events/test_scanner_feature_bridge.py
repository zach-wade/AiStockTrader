...
from main.events.types import AlertType, EventType
...
class TestScannerFeatureBridge:
    ...
    async def test_deduplication(self, bridge):
        ...
        bridge.alert_mapper.map_alert_to_features = Mock(return_value=["price_features"])
        bridge.request_batcher.add_symbol_features = Mock()
    ...
    async def test_error_handling_in_alert_processing(self, bridge):
        ...
        bridge.alert_mapper.map_alert_to_features = Mock(side_effect=Exception("Mapping failed"))
    ...
    async def test_cleanup_old_symbols(self, bridge):
        ...
        # Add old symbol
        old_time = datetime.now(UTC).timestamp() - 120  # 2 minutes ago
        bridge._processed_symbols["OLD_STOCK"] = old_time

        # Add recent symbol
        bridge._processed_symbols["NEW_STOCK"] = datetime.now(UTC).timestamp()
    ...
    async def test_integration_with_helpers(self, bridge):
        ...
        # Create alert
        alert = create_scan_alert(symbol="AAPL", alert_type=AlertType.VOLATILITY_SPIKE, score=0.95)
