...
class TestRequestDispatcher:
    ...
    def sample_batch(self):
        ...
        batch.created_at = datetime.now(UTC) - timedelta(seconds=5)
        return batch
    ...
    @pytest.mark.asyncio
    async def test_send_feature_request_batch_success(
        self, dispatcher, mock_event_bus, sample_batch
    ):
        """Test successful sending of feature request batch."""
        with patch(
            "main.events.handlers.scanner_bridge_helpers.request_dispatcher.record_metric"
        ) as mock_metric:
            ...
            mock_metric.assert_called_once_with(
                ...
                1,
                tags={"symbols": 3, "features": 3, "priority": 7},
            )
    ...
    async def test_send_batch_with_single_symbol(self, dispatcher, mock_event_bus):
        ...
        single_batch = FeatureRequestBatch(symbols={"AAPL"}, features={"price_action"}, priority=10)
    ...
    async def test_send_batch_no_correlation_ids(self, dispatcher, mock_event_bus):
        ...
        batch = FeatureRequestBatch(symbols={"AAPL"}, features={"price"}, priority=5)
    ...
    async def test_request_age_calculation(self, dispatcher, mock_event_bus):
        ...
        batch.created_at = datetime.now(UTC) - timedelta(seconds=10.5)
    ...
    async def test_multiple_batches_sequential(self, dispatcher, mock_event_bus):
        ...
        batches = [
            FeatureRequestBatch(symbols={f"STOCK{i}"}, features={"price"}, priority=i)
            for i in range(5)
        ]
    ...
    async def test_concurrent_batch_sending(self, dispatcher, mock_event_bus):
        ...
        batches = [
            FeatureRequestBatch(symbols={f"STOCK{i}"}, features={"feature"}, priority=5)
            for i in range(10)
        ]
    ...
    async def test_large_batch_handling(self, dispatcher, mock_event_bus):
        ...
        with patch(
            ...
            mock_metric.assert_called_once_with(
                "scanner_bridge.request_dispatched",
                1,
                tags={"symbols": 1000, "features": 4, "priority": 8},
            )
    ...
    async def test_metadata_preservation(self, dispatcher, mock_event_bus):
        ...
        assert all(
            s in event.metadata["scanner_sources"] for s in ["source1", "source2", "source3"]
        )
        assert all(
            id in event.metadata["correlation_ids"] for id in ["id1", "id2", "id3", "id4", "id5"]
        )
    ...
    async def test_feature_order_consistency(self, dispatcher, mock_event_bus):
        ...
        for _ in range(10):
            batch = FeatureRequestBatch(
                symbols={"TEST"}, features={"gamma", "alpha", "beta", "delta"}, priority=5
            )
    ...
    async def test_logging_output(self, dispatcher, mock_event_bus, sample_batch):
        ...
        with patch(
            "main.events.handlers.scanner_bridge_helpers.request_dispatcher.logger"
        ) as mock_logger:
            await dispatcher.send_feature_request_batch(sample_batch)
