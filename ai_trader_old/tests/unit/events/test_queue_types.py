...
class TestQueuedRequest:
    ...
    def test_queued_request_creation(self, sample_feature_request):
        ...
        queued = QueuedRequest(
            request=sample_feature_request, request_id="test_123", worker_id=1, attempt_count=0
        )
    ...
    def test_queued_request_defaults(self, sample_feature_request):
        ...
        queued = QueuedRequest(request=sample_feature_request, request_id="test_456")
    ...
    def test_queued_request_with_attempts(self, sample_feature_request):
        ...
        last_attempt_time = datetime.now(UTC)
    ...
    def test_queued_request_immutability(self, sample_feature_request):
        ...
        queued = QueuedRequest(request=sample_feature_request, request_id="test_999")
    ...
    def test_queued_request_to_dict(self, sample_feature_request):
        ...
        queued = QueuedRequest(
            request=sample_feature_request, request_id="test_dict", worker_id=3, attempt_count=1
        )
...
class TestQueueStats:
    ...
    def test_queue_stats_calculated_properties(self):
        ...
        stats = QueueStats(
            total_queued=1000, total_processed=800, total_failed=150, total_expired=50
        )
    ...
    def test_queue_stats_to_dict(self):
        ...
        stats = QueueStats(current_queue_size=25, requests_by_symbol={"TSLA": 50})
        ...
        assert all(
            key in data
            for key in ["total_queued", "total_processed", "total_failed", "avg_queue_time_seconds"]
        )
...
class TestCreateQueuedRequest:
    ...
    def test_create_queued_request_basic(self):
        ...
        feature_request = FeatureRequest(symbol="MSFT", feature_groups=[FeatureGroup.VOLATILITY])

        queued = create_queued_request(feature_request, request_id="helper_123")
    ...
    def test_create_queued_request_with_worker(self):
        ...
        feature_request = FeatureRequest(symbol="AMZN", feature_groups=[FeatureGroup.TREND])

        queued = create_queued_request(feature_request, request_id="worker_test", worker_id=5)

        assert queued.worker_id == 5

    def test_create_queued_request_auto_id(self):
        """Test creating queued request with auto-generated ID."""
        feature_request = FeatureRequest(symbol="NFLX", feature_groups=[FeatureGroup.ML_SIGNALS])
...
class TestUpdateQueueStats:
    ...
    def test_update_queue_stats_increment(self):
        ...
        stats = QueueStats(total_queued=100, total_processed=80)

        # Update with new processed request
        updated = update_queue_stats(stats, processed=1, queue_time=1.5)
...
class TestQueueTypesIntegration:
    ...
    def test_full_request_lifecycle(self):
        ...
        feature_req = FeatureRequest(symbol="AAPL", feature_groups=[FeatureGroup.PRICE], priority=8)
        ...
        dequeue_time = datetime.now(UTC).timestamp()
        ...
        stats = QueueStats()
        stats = QueueStats(
            total_queued=stats.total_queued + 1, current_queue_size=stats.current_queue_size + 1
        )
    ...
    def test_queue_priority_ordering(self):
        ...
        for i, priority in enumerate([3, 9, 1, 7, 5]):
            feature_req = FeatureRequest(
                symbol=f"STOCK{i}", feature_groups=[FeatureGroup.PRICE], priority=priority
            )
            queued = create_queued_request(feature_req, f"priority_{i}")
            requests.append(queued)

        # Sort by priority (highest first)
        sorted_requests = sorted(requests, key=lambda x: x.request.priority, reverse=True)
    ...
    def test_queue_stats_accuracy(self):
        ...
        operations = [("queued", 10), ("processed", 7), ("failed", 2), ("expired", 1)]
