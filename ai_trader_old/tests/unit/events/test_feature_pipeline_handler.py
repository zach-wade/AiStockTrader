...
class TestFeaturePipelineHandler:
    ...
    async def handler(self, mock_feature_orchestrator, mock_event_bus, config):
        ...
        with patch("main.events.handlers.feature_pipeline_handler.get_event_bus") as mock_get_bus:
            ...
            handler = FeaturePipelineHandler(
                feature_orchestrator=mock_feature_orchestrator, config=config
            )
            return handler

    def test_initialization_with_orchestrator(
        self, mock_feature_orchestrator, mock_event_bus, config
    ):
    ...
    def test_initialization_without_orchestrator(self, mock_event_bus, config):
        ...
        with patch("main.events.handlers.feature_pipeline_handler.get_event_bus") as mock_get_bus:
            with patch(
                "main.events.handlers.feature_pipeline_handler.FeatureOrchestrator"
            ) as mock_orch_class:
                mock_get_bus.return_value = mock_event_bus
    ...
    def test_initialization_without_config(self, mock_feature_orchestrator, mock_event_bus):
        ...
        with patch("main.events.handlers.feature_pipeline_handler.get_event_bus") as mock_get_bus:
            with patch(
                "main.events.handlers.feature_pipeline_handler.get_config"
            ) as mock_get_config:
                mock_get_bus.return_value = mock_event_bus
                mock_config = create_feature_pipeline_config()
                mock_get_config.return_value = mock_config

                handler = FeaturePipelineHandler(feature_orchestrator=mock_feature_orchestrator)
    ...
    async def test_handle_feature_request_event(self, handler):
        ...
        event = create_feature_request_event(
            symbols=["AAPL", "GOOGL"], features=["price_features", "volume_features"]
        )
    ...
    async def test_queue_management(self, handler):
        ...
        events = []
        for i in range(5):
            event = create_feature_request_event(symbols=[f"STOCK{i}"], features=["price_features"])
            events.append(event)
    ...
    async def test_feature_group_mapping(self, handler):
        ...
        handler._feature_group_mapper.map_alert_to_features = Mock(
            return_value=Mock(feature_groups=["PRICE", "VOLUME"], priority=7)
        )
    ...
    async def test_concurrent_workers(self, handler):
        ...
        for i in range(10):
            ...
            await handler._request_queue_manager.enqueue_request(request, f"request_{i}")
    ...
    def test_get_stats(self, handler):
        ...
        handler._stats_tracker.get_stats = Mock(
            return_value={"requests_received": 100, "requests_processed": 95}
        )

        handler._request_queue_manager.get_stats = Mock(
            return_value=Mock(
                total_queued=100, total_processed=95, total_failed=5, current_queue_size=0
            )
        )
