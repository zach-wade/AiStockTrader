...
# Local imports
from main.models.common import Order, OrderStatus, Position
from main.trading_engine.core.execution_engine import ExecutionEngine, ExecutionMode, SignalStatus
...
class TestExecutionEngine:
    ...
    def test_config(self):
        ...
        config_dict = {
            ...
            "broker": {"api_key": "test_key", "secret_key": "test_secret", "paper_trading": True},
        }
    ...
    def execution_engine(
        self, test_config, mock_order_manager, mock_portfolio_manager, mock_risk_manager
    ):
        """Create ExecutionEngine with mocked dependencies."""
        with (
            patch(
                "trading_engine.core.execution_engine.OrderManager", return_value=mock_order_manager
            ),
            patch(
                "trading_engine.core.execution_engine.PortfolioManager",
                return_value=mock_portfolio_manager,
            ),
            patch(
                "trading_engine.core.execution_engine.RiskManager", return_value=mock_risk_manager
            ),
        ):
    ...
    @pytest.mark.asyncio
    async def test_signal_generation_with_risk_manager_block(
        self, execution_engine, mock_risk_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_auto_execution_high_confidence_signal(
    ...
    @pytest.mark.asyncio
    async def test_emergency_liquidation_all_positions(
        self, execution_engine, mock_portfolio_manager, mock_order_manager
    ):
        ...
        mock_order_manager.submit_new_order.return_value = Mock(
            spec=Order, order_id="emergency_123"
        )
    ...
    async def test_get_system_health(
        self, execution_engine, mock_portfolio_manager, mock_risk_manager
    ):
        """Test system health reporting."""
        # Mock system state
        mock_portfolio_manager.get_total_value.return_value = 105000.0
        mock_risk_manager.get_risk_metrics.return_value = {"var_1d": 1500.0, "max_drawdown": 0.02}
