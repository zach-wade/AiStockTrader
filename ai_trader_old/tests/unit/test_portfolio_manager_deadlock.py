...
# Local imports
from src.main.models.common import AccountInfo, OrderSide
from src.main.models.common import Position as CommonPosition
from src.main.trading_engine.core.portfolio_manager import PortfolioManager
...
class TestPortfolioManagerDeadlock:
    ...
    async def portfolio_manager(self, mock_broker):
        ...
        config = {"trading.starting_cash": 100000.0, "risk.max_positions": 10}
        config_mock = Mock()
    ...
    async def test_no_deadlock_update_and_read_concurrent(self, portfolio_manager):
        ...
        results = await asyncio.wait_for(
            asyncio.gather(*all_tasks, return_exceptions=True), timeout=15.0
        )
        ...
        with patch.object(portfolio_manager.broker, "get_account_info", slow_broker_call):
            with patch.object(portfolio_manager.broker, "get_positions", slow_broker_call):
                ...
                with pytest.raises((asyncio.TimeoutError, RuntimeError)):
                    await asyncio.wait_for(portfolio_manager.update_portfolio(), timeout=5.0)
    ...
    async def test_position_operations_concurrent(self, portfolio_manager):
        ...
        async def open_position_task(symbol: str):
            return await portfolio_manager.open_position(symbol, 10.0, 100.0, OrderSide.BUY, "test")
    ...
    async def test_lock_timeout_configuration(self, portfolio_manager):
        ...
        async def hold_lock():
            async with portfolio_manager._timed_lock(
                portfolio_manager._calculation_lock, timeout=1.0
            ):
                await asyncio.sleep(0.5)  # Hold lock for 500ms

        async def try_acquire_lock():
            async with portfolio_manager._timed_lock(
                portfolio_manager._calculation_lock, timeout=0.1
            ):
                pass
