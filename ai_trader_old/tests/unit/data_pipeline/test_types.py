...
class TestDataPipelineResult:
    ...
    def test_data_pipeline_result_success_rate(self):
        ...
        result1 = DataPipelineResult(
            status=DataPipelineStatus.SUCCESS, records_processed=100, records_failed=0
        )
        assert result1.success_rate == 1.0

        # Partial success
        result2 = DataPipelineResult(
            status=DataPipelineStatus.PARTIAL, records_processed=80, records_failed=20
        )
        assert result2.success_rate == 0.8

        # No records
        result3 = DataPipelineResult(
            status=DataPipelineStatus.FAILED, records_processed=0, records_failed=0
        )
...
class TestDataQualityMetrics:
    ...
    def test_overall_quality_calculation(self):
        ...
        metrics = DataQualityMetrics(
            completeness=0.80, timeliness=0.90, accuracy=0.95, consistency=0.85
        )
