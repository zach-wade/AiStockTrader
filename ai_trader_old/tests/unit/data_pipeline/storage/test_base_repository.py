...
class ConcreteRepository(BaseRepository):
    ...
    def get_unique_constraint_name(self) -> str | None:
...
class TestModel:
    ...
    mock_columns = Mock()
    mock_columns.__contains__ = Mock(
        side_effect=lambda x: x in ["id", "symbol", "timestamp", "value"]
    )
...
class TestBaseRepository:
    ...
    async def repository(self, mock_db_adapter, mock_config):
        ...
        with patch(
            "main.data_pipeline.storage.repositories.base_repository.get_global_cache",
            return_value=None,
        ):
            repo = ConcreteRepository(
                db_adapter=mock_db_adapter, model_class=TestModel, config=mock_config
            )
    ...
    async def test_bulk_upsert_success(self, repository, mock_db_adapter):
        ...
        # Prepare test data
        records = [
            {"id": 1, "symbol": "AAPL", "timestamp": datetime.now(UTC), "value": 150.0},
            {"id": 2, "symbol": "MSFT", "timestamp": datetime.now(UTC), "value": 300.0},
        ]
    ...
    async def test_bulk_upsert_with_validation_errors(self, mock_db_adapter):
        ...
        with patch(
            ...
            repository = ConcreteRepository(
                db_adapter=mock_db_adapter, model_class=TestModel, config=strict_config
            )

        # Prepare test data with invalid record
        records = [
            {"id": 1, "symbol": "AAPL", "timestamp": datetime.now(UTC), "value": 150.0},
            {"id": 2, "symbol": "MSFT", "timestamp": datetime.now(UTC), "value": -50.0},  # Invalid
        ]
    ...
    async def test_get_by_symbol(self, repository, mock_db_adapter):
        ...
        mock_records = [
            {"id": 1, "symbol": "AAPL", "timestamp": datetime.now(UTC), "value": 150.0},
            {"id": 2, "symbol": "AAPL", "timestamp": datetime.now(UTC), "value": 151.0},
        ]
    ...
    async def test_query_with_filters(self, repository, mock_db_adapter):
        ...
        mock_records = [{"id": 1, "symbol": "AAPL", "timestamp": datetime.now(UTC), "value": 150.0}]
    ...
    async def test_health_check(self, repository, mock_db_adapter):
        ...
        repository.get_data_range = AsyncMock(
            return_value=(datetime.now(UTC) - timedelta(days=30), datetime.now(UTC))
        )
    ...
    async def test_record_validation(self, repository):
        ...
        # Valid record
        valid_record = {"id": 1, "symbol": "AAPL", "timestamp": datetime.now(UTC), "value": 150.0}

        # Invalid record (negative value)
        invalid_record = {"id": 2, "symbol": "MSFT", "timestamp": datetime.now(UTC), "value": -50.0}
