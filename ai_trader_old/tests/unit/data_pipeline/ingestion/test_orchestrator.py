...
from main.data_pipeline.types import DataPipelineResult, DataPipelineStatus
...
from tests.fixtures.data_pipeline.test_configs import get_error_test_config, get_test_config
...
class TestIngestionOrchestrator:
    ...
    def mock_validation_pipeline(self):
        ...
        pipeline.validate_ingest = AsyncMock(
            return_value=Mock(passed=True, errors=[], warnings=[], has_warnings=False)
        )
        return pipeline
    ...
    async def orchestrator(self, test_config, mock_clients, mock_archive, mock_validation_pipeline):
        ...
        with patch(
            "main.data_pipeline.ingestion.orchestrator.get_config", return_value=test_config
        ):
            with patch(
                "main.data_pipeline.ingestion.orchestrator.get_archive", return_value=mock_archive
            ):
                with patch(
                    "main.data_pipeline.ingestion.orchestrator.ValidationPipeline",
    ...
    async def test_validation_integration(self, orchestrator, mock_validation_pipeline):
        ...
        mock_validation_pipeline.validate_ingest.return_value = Mock(
            passed=False, errors=["Invalid data format"], warnings=[], has_warnings=False
        )
    ...
    async def test_resilience_retry_logic(self, mock_clients, mock_archive, test_config):
        ...
        with patch(
            "main.data_pipeline.ingestion.orchestrator.get_config", return_value=error_config
        ):
            with patch(
                "main.data_pipeline.ingestion.orchestrator.get_archive", return_value=mock_archive
            ):
