...
# Local imports
from main.feature_pipeline.calculators.microstructure import (
    MicrostructureCalculator,  # Fix: Import correct class
)
...
def test_basic_microstructure_fallback(microstructure_calculator):
    ...
    assert (
        len(available_basic) > 0
    ), f"No basic features found. Available: {list(features_df.columns)}"
