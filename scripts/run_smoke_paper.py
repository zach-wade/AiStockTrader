...
class RiskEngine:
    ...
    def check_order(self, order: dict[str, Any]) -> tuple[bool, str | None]:
        ...
        # Position size check
        return self.check_position_size(order["symbol"], order["quantity"], order["price"])
...
class SmokeTester:
    ...
    async def run_smoke_test(self) -> bool:
        ...
        try:
            ...
            print(
                f"Health Checks: {self.metrics['health_checks_passed']}/{self.metrics['health_checks_passed'] + self.metrics['health_checks_failed']}"
            )
