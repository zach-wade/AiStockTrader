...
class TestPositionInitialization:
    ...
    def test_default_initialization(self):
        ...
        assert position.quantity == Quantity(Decimal("10"))
    ...
    def test_custom_initialization(self):
        ...
        position = Position(
            ...
            current_price=Price(Decimal("2550.00")),
            realized_pnl=Money(Decimal("100.00")),
            commission_paid=Money(Decimal("5.00")),
            stop_loss_price=Price(Decimal("2400.00")),
            take_profit_price=Price(Decimal("2600.00")),
            strategy="momentum",
            tags={"sector": "tech"},
        )
        ...
        assert position.average_entry_price == Price(Decimal("2500.00"))
        assert position.current_price == Price(Decimal("2550.00"))
        assert position.realized_pnl == Money(Decimal("100.00"))
        assert position.commission_paid == Money(Decimal("5.00"))
        assert position.stop_loss_price == Price(Decimal("2400.00"))
        assert position.take_profit_price == Price(Decimal("2600.00"))
    ...
    def test_validation_negative_entry_price(self):
        ...
        with pytest.raises(ValueError, match="Price must be positive"):
            Position(symbol="AAPL", average_entry_price=Price(Decimal("-100")))
...
class TestPositionFactory:
    ...
    def test_open_position_long(self):
        ...
        assert position.average_entry_price == Price(Decimal("150.00"))
        assert position.commission_paid == Money(Decimal("1.00"))
    ...
    def test_open_position_short(self):
        ...
        assert position.quantity == Quantity(Decimal("-50"))
        assert position.average_entry_price == Price(Decimal("700.00"))
        assert position.commission_paid == Money(Decimal("2.00"))
    ...
    def test_open_position_zero_quantity(self):
        ...
        with pytest.raises(ValueError, match="Cannot open position with zero quantity"):
            Position.open_position(
                symbol="AAPL", quantity=Quantity(Decimal("0")), entry_price=Price(Decimal("150.00"))
            )
...
class TestPositionManagement:
    ...
    def test_add_to_long_position(self, long_position):
        ...
        assert long_position.average_entry_price == Price(expected_avg)
        assert long_position.commission_paid == Money(Decimal("2.00"))
    ...
    def test_add_to_short_position(self, short_position):
        ...
        short_position.add_to_position(
            ...
            price=Price(Decimal("680.00")),
            commission=Money(Decimal("1.00")),
        )

        assert short_position.quantity == Quantity(Decimal("-75"))
        # New average: (50*700 + 25*680) / 75 = 693.33...
        expected_avg = (Decimal("50") * Decimal("700") + Decimal("25") * Decimal("680")) / Decimal(
            "75"
        )
        assert short_position.average_entry_price == Price(expected_avg)
        assert short_position.commission_paid == Money(Decimal("3.00"))
    ...
    def test_add_zero_quantity(self, long_position):
        ...
        long_position.add_to_position(
            quantity=Quantity(Decimal("0")), price=Price(Decimal("160.00"))
        )
    ...
    def test_add_wrong_direction_to_long(self, long_position):
        ...
        with pytest.raises(ValueError, match="Cannot add short quantity to long position"):
            long_position.add_to_position(
                quantity=Quantity(Decimal("-50")), price=Price(Decimal("160.00"))
            )
    ...
    def test_add_wrong_direction_to_short(self, short_position):
        ...
        with pytest.raises(ValueError, match="Cannot add long quantity to short position"):
            short_position.add_to_position(
                quantity=Quantity(Decimal("50")), price=Price(Decimal("680.00"))
            )
    ...
    def test_reduce_long_position_partial(self, long_position):
        ...
        pnl = long_position.reduce_position(
            ...
            exit_price=Price(Decimal("160.00")),
            commission=Money(Decimal("1.00")),
        )

        expected_pnl = Decimal("30") * (Decimal("160") - Decimal("150")) - Decimal("1")
        assert pnl == Money(expected_pnl)  # 300 - 1 = 299
        assert long_position.quantity == Quantity(Decimal("70"))
        assert long_position.realized_pnl == Money(expected_pnl)
        assert long_position.commission_paid == Money(Decimal("2.00"))
    ...
    def test_reduce_short_position_partial(self, short_position):
        ...
        pnl = short_position.reduce_position(
            ...
            exit_price=Price(Decimal("680.00")),
            commission=Money(Decimal("1.00")),
        )

        expected_pnl = Decimal("20") * (Decimal("700") - Decimal("680")) - Decimal("1")
        assert pnl == Money(expected_pnl)  # 400 - 1 = 399
        assert short_position.quantity == Quantity(Decimal("-30"))
        assert short_position.realized_pnl == Money(expected_pnl)
        assert short_position.commission_paid == Money(Decimal("3.00"))
    ...
    def test_reduce_position_fully(self, long_position):
        ...
        assert pnl == Money(expected_pnl)  # 1000 - 1 = 999
    ...
    def test_reduce_position_zero_quantity(self, long_position):
        ...
        pnl = long_position.reduce_position(
            quantity=Quantity(Decimal("0")), exit_price=Price(Decimal("160.00"))
        )

        assert pnl == Money(Decimal("0"))
        assert long_position.quantity == Quantity(Decimal("100"))
    ...
    def test_reduce_position_excessive_quantity(self, long_position):
        ...
        with pytest.raises(ValueError, match="Cannot reduce position by"):
            long_position.reduce_position(
                quantity=Quantity(Decimal("150")), exit_price=Price(Decimal("160.00"))
            )

    def test_close_long_position_profit(self, long_position):
        """Test closing long position with profit."""
        pnl = long_position.close_position(
            exit_price=Price(Decimal("160.00")), commission=Money(Decimal("1.00"))
        )
    ...
    def test_close_short_position_loss(self, short_position):
        ...
        pnl = short_position.close_position(
            exit_price=Price(Decimal("720.00")), commission=Money(Decimal("1.00"))
        )

        expected_pnl = Decimal("50") * (Decimal("700") - Decimal("720")) - Decimal("1")
        assert pnl == Money(expected_pnl)  # -1000 - 1 = -1001
    ...
    def test_close_already_closed_position(self, long_position):
        ...
        with pytest.raises(ValueError, match="Position is already closed"):
            long_position.close_position(Price(Decimal("170.00")))

    def test_close_method(self, long_position):
        """Test the close method."""
        close_time = datetime.now(UTC)

        long_position.close(final_price=Price(Decimal("160.00")), close_time=close_time)

        assert long_position.is_closed() is True
        assert long_position.quantity == Quantity(Decimal("0"))
        assert long_position.current_price == Price(Decimal("160.00"))
...
class TestPositionPriceUpdates:
    ...
    def test_update_market_price(self, position):
        ...
        position.update_market_price(Price(Decimal("155.00")))
    ...
    def test_update_market_price_negative(self, position):
        ...
        with pytest.raises(ValueError, match="Price must be positive"):
            position.update_market_price(Price(Decimal("-100.00")))
...
class TestPositionRiskManagement:
    ...
    def short_position(self):
        ...
        position = Position.open_position(
            symbol="TSLA", quantity=Quantity(Decimal("-50")), entry_price=Price(Decimal("700.00"))
        )
        position.current_price = Price(Decimal("680.00"))
        return position

    def test_set_stop_loss_long_valid(self, long_position):
        """Test setting valid stop loss for long position."""
        long_position.set_stop_loss(Price(Decimal("145.00")))

        assert long_position.stop_loss_price == Price(Decimal("145.00"))
    ...
    def test_set_stop_loss_long_invalid(self, long_position):
        ...
        with pytest.raises(
            ...
            long_position.set_stop_loss(Price(Decimal("160.00")))

    def test_set_stop_loss_short_valid(self, short_position):
        """Test setting valid stop loss for short position."""
        short_position.set_stop_loss(Price(Decimal("690.00")))

        assert short_position.stop_loss_price == Price(Decimal("690.00"))
    ...
    def test_set_stop_loss_short_invalid(self, short_position):
        ...
        with pytest.raises(
            ...
            short_position.set_stop_loss(Price(Decimal("670.00")))

    def test_set_stop_loss_negative(self, long_position):
        """Test setting negative stop loss fails."""
        with pytest.raises(ValueError, match="Price must be positive"):
            long_position.set_stop_loss(Price(Decimal("-100.00")))

    def test_set_take_profit_long_valid(self, long_position):
        """Test setting valid take profit for long position."""
        long_position.set_take_profit(Price(Decimal("165.00")))

        assert long_position.take_profit_price == Price(Decimal("165.00"))
    ...
    def test_set_take_profit_long_invalid(self, long_position):
        ...
        with pytest.raises(
            ...
            long_position.set_take_profit(Price(Decimal("145.00")))

    def test_set_take_profit_short_valid(self, short_position):
        """Test setting valid take profit for short position."""
        short_position.set_take_profit(Price(Decimal("670.00")))

        assert short_position.take_profit_price == Price(Decimal("670.00"))
    ...
    def test_set_take_profit_short_invalid(self, short_position):
        ...
        with pytest.raises(
            ...
            short_position.set_take_profit(Price(Decimal("690.00")))
    ...
    def test_should_stop_loss_long_triggered(self, long_position):
        ...
        long_position.current_price = Price(Decimal("144.00"))
    ...
    def test_should_stop_loss_long_not_triggered(self, long_position):
        ...
        long_position.stop_loss_price = Price(Decimal("145.00"))
        long_position.current_price = Price(Decimal("146.00"))
    ...
    def test_should_stop_loss_short_triggered(self, short_position):
        ...
        short_position.stop_loss_price = Price(Decimal("690.00"))
        short_position.current_price = Price(Decimal("691.00"))
    ...
    def test_should_take_profit_long_triggered(self, long_position):
        ...
        long_position.take_profit_price = Price(Decimal("165.00"))
        long_position.current_price = Price(Decimal("166.00"))
    ...
    def test_should_take_profit_short_triggered(self, short_position):
        ...
        short_position.take_profit_price = Price(Decimal("670.00"))
        short_position.current_price = Price(Decimal("669.00"))
...
class TestPositionCalculations:
    ...
    def test_get_unrealized_pnl_long_profit(self, long_position_with_price):
        ...
        # 100 * (160 - 150) = 1000
        assert unrealized_pnl == Money(Decimal("1000"))
    ...
    def test_get_unrealized_pnl_long_loss(self):
        ...
        position.current_price = Price(Decimal("140.00"))
    ...
    def test_get_unrealized_pnl_closed_position(self, long_position_with_price):
        ...
        long_position_with_price.quantity = Quantity(Decimal("0"))
    ...
    def test_get_total_pnl_no_unrealized(self):
        ...
        # Only realized P&L
        assert total_pnl == Money(Decimal("500"))
    ...
    def test_get_position_value(self, long_position_with_price):
        ...
        # 100 * 160 = 16000
        assert value == Money(Decimal("16000"))
    ...
    def test_get_position_value_short(self, short_position_with_price):
        ...
        # abs(-50) * 680 = 34000
        assert value == Money(Decimal("34000"))
    ...
    def test_get_return_percentage(self, long_position_with_price):
        ...
        long_position_with_price.commission_paid = Money(Decimal("10"))
    ...
    def test_get_return_percentage_zero_entry(self):
        """Test that zero entry price is rejected during Position creation."""
        # Zero entry prices are now correctly invalid - this should raise ValueError
        with pytest.raises(ValueError, match="Price must be positive"):
            Position(
                symbol="AAPL",
                quantity=Quantity(Decimal("100")),
...
class TestPositionStateQueries:
    ...
    def test_is_closed_by_timestamp(self):
        ...
        position = Position(
            symbol="AAPL",
            quantity=Quantity(Decimal("100")),  # Non-zero quantity
            average_entry_price=Price(Decimal("150.00")),
            closed_at=datetime.now(UTC),
        )
...
class TestPositionRepresentation:
    ...
    def test_str_short_closed(self):
        ...
        position.close_position(Price(Decimal("680.00")))
...
class TestPositionEdgeCases:
    ...
    def test_very_small_quantity(self):
        ...
        position = Position.open_position(
            symbol="AAPL", quantity=Quantity(Decimal("0.001")), entry_price=Price(Decimal("150.00"))
        )
    ...
    def test_very_large_quantity(self):
        ...
        position = Position.open_position(
            ...
            entry_price=Price(Decimal("500000.00")),
        )
    ...
    def test_high_precision_prices(self):
        ...
        position = Position.open_position(
            symbol="CRYPTO",
            quantity=Quantity(Decimal("100.123456")),
            entry_price=Price(Decimal("0.000001234567")),
        )

        position.add_to_position(
            quantity=Quantity(Decimal("50.654321")), price=Price(Decimal("0.000001345678"))
        )

        assert position.quantity == Quantity(Decimal("150.777777"))

    def test_fractional_shares(self):
        """Test position with fractional shares."""
        position = Position.open_position(
            symbol="AAPL", quantity=Quantity(Decimal("10.5")), entry_price=Price(Decimal("150.00"))
        )

        pnl = position.reduce_position(
            quantity=Quantity(Decimal("5.25")), exit_price=Price(Decimal("160.00"))
        )

        assert position.quantity == Quantity(Decimal("5.25"))
        expected_pnl = Decimal("5.25") * Decimal("10")
        assert pnl == Money(expected_pnl)
