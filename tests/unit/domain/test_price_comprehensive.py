...
class TestPriceUtilityMethods:
    ...
    def test_calculate_spread(self):
        """Test calculating spread between prices"""
        bid = Price(Decimal("100.00"))
        ask = Price(Decimal("100.50"))
        
        spread = ask.calculate_spread(bid)
        assert spread == Decimal("0.50")
        
        # Reverse should give same absolute spread
        spread = bid.calculate_spread(ask)
        assert spread == Decimal("0.50")

    def test_calculate_spread_percentage(self):
        """Test calculating spread percentage"""
        bid = Price(Decimal("100.00"))
        ask = Price(Decimal("101.00"))
        
        # Spread is 1.00, average price is 100.50
        # Percentage = (1.00 / 100.50) * 100 ≈ 0.995%
        spread_pct = ask.calculate_spread_percentage(bid)
        assert abs(spread_pct - Decimal("0.9950248756218906")) < Decimal("0.0001")
...
class TestPriceFormatting:
    ...
    def test_format_method(self):
        ...
        assert price.format() == "100.12"
        
        # Format to 2 decimal places
        assert price.format(decimal_places=2) == "100.12"
        
        # Format to 4 decimal places
        assert price.format(decimal_places=4) == "100.1235"
        
        # Format to 0 decimal places
        assert price.format(decimal_places=0) == "100"
...
class TestPriceEdgeCases:
    ...
    def test_very_large_price(self):
        ...
        assert large_price.value == Decimal("999999999999.99")
    ...
    def test_price_with_many_decimal_places(self):
        ...
        assert price.value == Decimal("100.123456789")
        
        # Rounding should work correctly
        rounded = price.round_to_tick()
        assert rounded.value == Decimal("100.12")  # Stock default tick is 0.01
    ...
    def test_price_immutability(self):
        ...
        assert price.value == original_value
